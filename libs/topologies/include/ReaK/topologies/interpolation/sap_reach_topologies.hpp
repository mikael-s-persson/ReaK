...
#include "ReaK/topologies/interpolation/sap_metrics.hpp"
#include "ReaK/topologies/interpolation/sap_samplers.hpp"
...
struct sap_reach_topo_impl {
  ...
  using rt_metric_type = sap_reach_time_metric<time_topology>;
  using sampler_type =
      generic_sampler<sap_rate_limited_sampler<time_topology>, BaseTopology>;
  ...
  static double get_proper_distance(const BaseTopology& b_space,
    ...
    sap_reach_time_metric<time_topology, true> p_rt_dist(rt_dist);
...
struct sap_reach_topo_impl<BaseTopology, true> {
  ...
  using rt_metric_type = sap_reach_time_metric<base_time_topo>;
  using sampler_type = generic_sampler<sap_rate_limited_sampler<base_time_topo>,
                                       base_space_topo>;
  ...
  static double get_proper_distance(const BaseTopology& b_space,
                                    ...
                                    const point_type& a, const point_type& b) {
    using std::abs;
    sap_reach_time_metric<base_time_topo, true> p_rt_dist(rt_dist);
  ...
  static bool is_in_bounds(const BaseTopology& b_space, const point_type& a) {
    return sap_is_in_bounds(a.pt, b_space.get_space_topology(),
...
struct sap_reach_topo_selector {
  using type =
      sap_reach_topo_impl<BaseTopology, is_temporal_space_v<BaseTopology>>;
};
 ...
 */
template <typename BaseTopology>
class interpolated_topology<BaseTopology, sap_interpolation_tag>
    : public interpolated_topology_base<BaseTopology> {
 public:
  using base_type = interpolated_topology_base<BaseTopology>;
  using self = interpolated_topology<BaseTopology, sap_interpolation_tag>;
 ...
 protected:
  using Impl = typename detail::sap_reach_topo_selector<BaseTopology>::type;
...
struct get_tagged_temporal_interpolator<sap_interpolation_tag,
                                        TemporalSpaceType> {
  using type = generic_interpolator<sap_interpolator_factory<TemporalSpaceType>,
                                    sap_interpolator>;
};
