...
private:
  ...
  double travel_distance_impl( const_waypoint_descriptor a, const_waypoint_descriptor b ) const {
...
public:
  struct point_distance_iterator {
    ...
    point_distance_iterator( const discrete_point_path* aParent, const const_waypoint_descriptor& aWPt )
    ...
    const point_type& operator*() const { return *current_wpt; };
  };
  ...
  struct point_fraction_iterator {
    ...
    point_fraction_iterator( const discrete_point_path* aParent, const const_waypoint_descriptor& aWPt )
        : parent( aParent ), current_wpt( aWPt ){};
   ...
   * \param aDist The distance metric functor that the path should use.
   */
  discrete_point_path( const shared_ptr< topology >& aSpace, const point_type& aStart, const point_type& aEnd,
                       const distance_metric& aDist = distance_metric() )
  ...
  discrete_point_path( ForwardIter aBegin, ForwardIter aEnd, const shared_ptr< topology >& aSpace,
                       const distance_metric& aDist = distance_metric() )
   ...
   */
  point_type get_point_at_distance( double d ) const { return *( move_away_from_impl( this->waypoints.begin(), d ) ); };
   ...
   */
  point_type get_start_point() const { return *( this->waypoints.begin() ); };
