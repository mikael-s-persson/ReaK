...
protected:
  ...
  virtual double interp_topo_get_distance_pred( const point_type& a, const point_type& b, double min_dist_interval,
                                                validity_predicate_type predicate ) const {
...
public:
  explicit interpolated_topology_base( const BaseTopology& aTopo ) : BaseTopology( aTopo ){};

#ifndef BOOST_NO_CXX11_VARIADIC_TEMPLATES
  template < typename... Args >
  interpolated_topology_base( Args&&... args )
      : BaseTopology( std::forward< Args >( args )... ){};
#else
  interpolated_topology_base() : BaseTopology(){};

  template < typename A1 >
  explicit interpolated_topology_base( const A1& a1 )
      : BaseTopology( a1 ){};
   ...
   */
  double distance( const point_type& a, const point_type& b ) const { return this->interp_topo_get_distance( a, b ); };

  /**
   * Returns the distance between two points.
   */
  double distance( const point_type& a, const point_type& b, double min_dist_interval,
                   validity_predicate_type predicate ) const {
   ...
   */
  double norm( const point_difference_type& delta ) const { return this->interp_topo_get_norm( delta ); };
   ...
   * Returns the norm of the difference between two points.
   */
  double proper_norm( const point_difference_type& delta ) const { return this->interp_topo_get_proper_norm( delta ); };
  ...
  * **********************************************************************/
   ...
   */
  bool is_in_bounds( const point_type& a ) const { return this->interp_topo_is_in_bounds( a ); };
   ...
   * Returns a point which is at a fraction between two points a to b.
   */
  point_type move_position_toward( const point_type& a, double fraction, const point_type& b, double min_dist_interval,
                                   validity_predicate_type predicate ) const {
   ...
   * Returns a point which is at a fraction between two points a to b.
   */
  point_type move_position_back_to( const point_type& a, double fraction, const point_type& b, double min_dist_interval,
                                    validity_predicate_type predicate ) const {
...
public:
  ...
  point_type random_point( validity_predicate_type predicate ) const { return m_space->random_point( predicate ); };
...
typename boost::disable_if< is_temporal_space< BaseTopology >,
                            typename topology_traits< BaseTopology >::point_type >::type
  interp_topo_move_pt_impl( const BaseTopology& b_space, const typename topology_traits< BaseTopology >::point_type& a,
                            double fraction, const typename topology_traits< BaseTopology >::point_type& b,
                            double dist_tot ) {
  typedef typename get_tagged_spatial_interpolator< InterpMethodTag, BaseTopology, time_topology >::type InterpType;
  typedef typename get_tagged_spatial_interpolator< InterpMethodTag, BaseTopology, time_topology >::pseudo_factory_type
    InterpFactoryType;
...
typename boost::enable_if< is_temporal_space< BaseTopology >,
  ...
  typedef typename get_tagged_spatial_interpolator< InterpMethodTag, SpaceTopoType, time_topology >::pseudo_factory_type
    InterpFactoryType;
  typedef typename topology_traits< BaseTopology >::point_type PointType;
  ...
  interp.initialize( a.pt, b.pt, dt_total, b_space.get_space_topology(), b_space.get_time_topology(),
                     InterpFactoryType() );
  double dt = dt_total * fraction;
  PointType result = a;
  interp.compute_point( result.pt, a.pt, b.pt, b_space.get_space_topology(), b_space.get_time_topology(), dt, dt_total,
                        InterpFactoryType() );
...
typename boost::enable_if< is_temporal_space< BaseTopology >,
  ...
  PointType last_result = a;
  while( d < dt ) {
    interp.compute_point( result.pt, a.pt, b.pt, b_space.get_space_topology(), b_space.get_time_topology(), d, dt_total,
                          InterpFactoryType() );
...
typename boost::disable_if< is_temporal_space< BaseTopology >,
                            typename topology_traits< BaseTopology >::point_type >::type
  interp_topo_move_pt_back_impl( const BaseTopology& b_space,
                                 const typename topology_traits< BaseTopology >::point_type& a, double fraction,
                                 const typename topology_traits< BaseTopology >::point_type& b, double dist_tot ) {
  typedef typename get_tagged_spatial_interpolator< InterpMethodTag, BaseTopology, time_topology >::type InterpType;
  typedef typename get_tagged_spatial_interpolator< InterpMethodTag, BaseTopology, time_topology >::pseudo_factory_type
    InterpFactoryType;
  typedef typename topology_traits< BaseTopology >::point_type PointType;
...
typename boost::disable_if< is_temporal_space< BaseTopology >,
  ...
  interp_topo_move_pt_back_impl( const BaseTopology& b_space,
                                 const typename topology_traits< BaseTopology >::point_type& a, double fraction,
                                 const typename topology_traits< BaseTopology >::point_type& b, double dist_tot,
                                 double min_interval, PredFunction predicate ) {
  typedef typename get_tagged_spatial_interpolator< InterpMethodTag, BaseTopology, time_topology >::type InterpType;
  typedef typename get_tagged_spatial_interpolator< InterpMethodTag, BaseTopology, time_topology >::pseudo_factory_type
    InterpFactoryType;
  typedef typename topology_traits< BaseTopology >::point_type PointType;
...
template < typename BaseTopology, typename InterpMethodTag >
struct get_rate_illimited_space< interpolated_topology< BaseTopology, InterpMethodTag > >
  : get_rate_illimited_space< BaseTopology > {};
...
struct is_metric_symmetric< interpolated_topology< BaseTopology, InterpMethodTag > >
  : boost::mpl::and_< is_metric_symmetric< InterpMethodTag >, is_metric_symmetric< BaseTopology > > {};
...
template < typename BaseTopology >
struct arithmetic_tuple_size< pp::interpolated_topology_base< BaseTopology > > : arithmetic_tuple_size< BaseTopology > {
};
