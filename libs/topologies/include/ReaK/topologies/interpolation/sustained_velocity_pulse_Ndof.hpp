...
PointType svp_Ndof_interpolate( const PointType& a, const PointType& b, double t, const Topology& space,
  ...
  detail::svp_Ndof_interpolate_impl< max_derivation_order< SpaceType, TimeSpaceType > >(
    result.pt, a.pt, b.pt, peak_velocity, space.get_space_topology(), space.get_time_topology(), dt, delta_time );
...
bool svp_Ndof_is_in_bounds( const typename topology_traits< Topology >::point_type& pt, const Topology& space,
...
public:
  ...
  svp_Ndof_interpolator( const point_type& start_point, const point_type& end_point, double dt, const SpaceType& space,
  ...
  void initialize( const point_type& start_point, const point_type& end_point, double dt, const SpaceType& space,
    ...
    RK_UNUSED( factory );
    try {
      min_delta_time = detail::svp_compute_Ndof_interpolation_data_impl( start_point, end_point, peak_velocity, space,
                                                                         t_space, dt, &best_peak_velocity );
  ...
  void compute_point( point_type& result, const point_type& start_point, const point_type& end_point,
    ...
    detail::svp_Ndof_interpolate_impl< max_derivation_order< SpaceType, TimeSpaceType > >(
      result, start_point, end_point, peak_velocity, space, t_space, dt, dt_total );
  };
...
public:
  svp_Ndof_interp_factory( const shared_ptr< topology >& aSpace = shared_ptr< topology >() ) : space( aSpace ){};
...
class svp_Ndof_interp_traj
  : public interpolated_trajectory< Topology, svp_Ndof_interp_factory< Topology >, DistanceMetric > {
public:
  BOOST_CONCEPT_ASSERT( ( TemporalSpaceConcept< Topology > ) );
...
public:
   ...
   */
  explicit svp_Ndof_interp_traj( const shared_ptr< topology >& aSpace = shared_ptr< topology >( new topology() ),
   ...
   * \param aDist The distance metric functor that the path should use.
   */
  svp_Ndof_interp_traj( const shared_ptr< topology >& aSpace, const point_type& aStart, const point_type& aEnd,
                        const distance_metric& aDist = distance_metric() )
  ...
  svp_Ndof_interp_traj( ForwardIter aBegin, ForwardIter aEnd, const shared_ptr< topology >& aSpace,
                        const distance_metric& aDist = distance_metric() )
...
struct get_type_id< pp::svp_Ndof_interpolation_tag > {
  ...
  BOOST_STATIC_CONSTEXPR auto type_name = RK_LSA( "svp_Ndof_interpolation_tag" );
#else
