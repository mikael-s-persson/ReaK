...
private:
  ...
  waypoint_pair get_point_at_time_impl( double t, const const_waypoint_bounds& wpb_a ) const {
    ...
    return waypoint_pair(
      wpb_a.first, this->space->move_position_toward( wpb_a.first->second, dt / t_total, wpb_a.second->second ) );
  };
...
public:
   ...
   * \param aDist The distance metric functor that the trajectory should use.
   */
  point_to_point_trajectory( const shared_ptr< topology >& aSpace, const point_type& aStart, const point_type& aEnd,
                             const distance_metric& aDist = distance_metric() )
  ...
  point_to_point_trajectory( ForwardIter aBegin, ForwardIter aEnd, const shared_ptr< topology >& aSpace,
                             const distance_metric& aDist = distance_metric() )
