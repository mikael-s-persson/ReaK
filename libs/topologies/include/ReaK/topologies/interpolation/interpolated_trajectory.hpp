    ...
    if( wpb_a.first == wpb_a.second ) {
      ...
      if( it_int == interp_segments.end() ) {
        interp_segments[&( wpb_a.first->second )]
          = interp_fact.create_interpolator( &( wpb_a.first->second ), &( wpb_a.second->second ) );
      ...
      if( it_int == interp_segments.end() ) {
        if( wpb_b.first == wpb_b.second ) {
          ...
          interp_segments[&( wpb_b.first->second )]
            = interp_fact.create_interpolator( &( wpb_b.first->second ), &( wpb_b.second->second ) );
  ...
  waypoint_pair get_point_at_time_impl( double t, const const_waypoint_bounds& wpb_a ) const {
    ...
    if( it_int == interp_segments.end() ) {
      return waypoint_pair( wpb_a.first,
                            ( interp_segments[&( wpb_a.first->second )] = interp_fact.create_interpolator(
                                &( wpb_a.first->second ), &( wpb_a.second->second ) ) ).get_point_at_time( t ) );
    } else if( it_int->second.get_end_point() != &( wpb_a.second->second ) ) {
      it_int->second.set_segment( &( wpb_a.first->second ), &( wpb_a.second->second ) );
    };
...
public:
   ...
   */
  explicit interpolated_trajectory( const shared_ptr< topology >& aSpace = shared_ptr< topology >( new topology() ),
                                    const distance_metric& aDist = distance_metric(),
                                    const interpolator_factory_type& aInterpFactory = interpolator_factory_type() )
   ...
   * \param aInterp The interpolator functor that the trajectory should use.
   */
  interpolated_trajectory( const shared_ptr< topology >& aSpace, const point_type& aStart, const point_type& aEnd,
                           const distance_metric& aDist = distance_metric(),
                           const interpolator_factory_type& aInterpFactory = interpolator_factory_type() )
  ...
  interpolated_trajectory( ForwardIter aBegin, ForwardIter aEnd, const shared_ptr< topology >& aSpace,
                           const distance_metric& aDist = distance_metric(),
                           const interpolator_factory_type& aInterpFactory = interpolator_factory_type() )
