...
template < bool IsTemporalSpace, typename Topology,
           typename DistanceMetric = typename metric_space_traits< Topology >::distance_metric_type >
...
public:
  ...
  typedef waypoint_container_base< IsTemporalSpace, Topology, DistanceMetric > self;
...
protected:
  ...
  const_waypoint_bounds get_waypoint_bounds( const point_type& p, const_waypoint_descriptor start ) const {
    ...
    //       double c12 = (d1 * d1 + d2 * d2 - d12 * d12) / (2.0 * d1 * d2); //cosine of the summet angle of the
    //       triangle.
    //       const_waypoint_descriptor it3 = it2; ++it3;
    // //       NOTE basically this loop will select the interval with the triangle (start,p,it2) with the largest angle
    // at p.
    //       while(it3 != it_end) {
...
public:
   ...
   */
  explicit waypoint_container_base( const shared_ptr< topology >& aSpace = shared_ptr< topology >( new topology() ),
   ...
   * \param aDist The distance metric functor that the waypoint-container should use.
   */
  waypoint_container_base( const shared_ptr< topology >& aSpace, const point_type& aStart, const point_type& aEnd,
                           const distance_metric& aDist = distance_metric() )
      : space( aSpace ), dist( aDist ), waypoints() {
  ...
  waypoint_container_base( ForwardIter aBegin, ForwardIter aEnd, const shared_ptr< topology >& aSpace,
                           const distance_metric& aDist = distance_metric() )
  ...
  *******************************************************************************/
  ...
  virtual void RK_CALL save( serialization::oarchive& A, unsigned int ) const {
    ...
    A& RK_SERIAL_SAVE_WITH_NAME( space ) & RK_SERIAL_SAVE_WITH_NAME( dist ) & RK_SERIAL_SAVE_WITH_NAME( waypoints );
  };

  virtual void RK_CALL load( serialization::iarchive& A, unsigned int ) {
    ReaK::shared_object::load( A, shared_object::getStaticObjectType()->TypeVersion() );
    A& RK_SERIAL_LOAD_WITH_NAME( space ) & RK_SERIAL_LOAD_WITH_NAME( dist ) & RK_SERIAL_LOAD_WITH_NAME( waypoints );
  };
...
class waypoint_container_base< true, Topology, DistanceMetricBase > : public shared_object {
public:
  BOOST_CONCEPT_ASSERT( ( MetricSpaceConcept< Topology > ) );

  typedef waypoint_container_base< true, Topology, DistanceMetricBase > self;
  typedef Topology topology;
...
protected:
  ...
  virtual double travel_distance_impl( const point_type& a, const point_type& b ) const { return 0.0; };
...
public:
  struct point_time_iterator {
    ...
    friend bool operator==( const point_time_iterator& lhs, const point_time_iterator& rhs ) {
      using std::fabs;
      if( ( lhs.parent == rhs.parent ) && ( lhs.current_wpb.first == rhs.current_wpb.first )
          && ( lhs.current_wpb.second == rhs.current_wpb.second )
          && ( ( lhs.current_wpb.first == lhs.current_wpb.second )
               || ( fabs( lhs.current_pt.time - rhs.current_pt.time )
                    <= 1e-6 * ( lhs.current_wpb.second->first - lhs.current_wpb.first->first ) ) ) )
        return true;
      else
        return false;
    };
   ...
   * \return The starting point of the waypoints.
   */
  const point_type& get_start_point() const { return waypoints.begin()->second; };
  ...
  point_time_iterator end_time_travel() const {
    return point_time_iterator(
      this, const_waypoint_bounds( ( ++waypoints.rbegin() ).base(), ( ++waypoints.rbegin() ).base() ) );
  };
...
class waypoint_container
  : public waypoint_container_base< is_temporal_space< Topology >::type::value, Topology, DistanceMetric > {
public:
  BOOST_CONCEPT_ASSERT( ( MetricSpaceConcept< Topology > ) );
  BOOST_CONCEPT_ASSERT( ( DistanceMetricConcept< DistanceMetric, Topology > ) );

  typedef waypoint_container< Topology, DistanceMetric > self;
  typedef waypoint_container_base< is_temporal_space< Topology >::type::value, Topology, DistanceMetric >
    base_class_type;
...
public:
   ...
   * \param aDist The distance metric functor that the waypoint-container should use.
   */
  explicit waypoint_container( const shared_ptr< topology >& aSpace = shared_ptr< topology >( new topology() ),
                               const distance_metric& aDist = distance_metric() )
   ...
   * \param aDist The distance metric functor that the waypoint-container should use.
   */
  waypoint_container( const shared_ptr< topology >& aSpace, const point_type& aStart, const point_type& aEnd,
                      const distance_metric& aDist = distance_metric() )
   ...
   */
  template < typename ForwardIter >
  waypoint_container( ForwardIter aBegin, ForwardIter aEnd, const shared_ptr< topology >& aSpace,
                      const distance_metric& aDist = distance_metric() )
  ...
  typename container_type::const_reverse_iterator rbegin() const { return this->waypoints.rbegin(); };

  typename container_type::const_reverse_iterator rend() const { return this->waypoints.rend(); };

  waypoint_descriptor begin() { return this->waypoints.begin(); };

  waypoint_descriptor end() { return this->waypoints.end(); };

  typename container_type::reverse_iterator rbegin() { return this->waypoints.rbegin(); };

  typename container_type::reverse_iterator rend() { return this->waypoints.rend(); };
  ...
  const point_type& front() const { return base_class_type::extract_point( *( this->waypoints.begin() ) ); };

  const point_type& back() const { return base_class_type::extract_point( *( this->waypoints.rbegin() ) ); };

  point_type& front() { return base_class_type::extract_point( *( this->waypoints.begin() ) ); };

  point_type& back() { return base_class_type::extract_point( *( this->waypoints.rbegin() ) ); };
  ...
  void pop_back() { this->waypoints.erase( ( ++( this->waypoints.rbegin() ) ).base() ); };
