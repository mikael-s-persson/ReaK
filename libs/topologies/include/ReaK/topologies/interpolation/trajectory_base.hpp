...
public:
  ...
  typedef typename spatial_trajectory_traits< SpatialTrajectory >::const_waypoint_descriptor
    wrapped_const_waypoint_descriptor;
...
public:
   ...
   */
  explicit trajectory_wrapper( const std::string& aName = "", const SpatialTrajectory& aTraj = SpatialTrajectory() )
      : base_type( aName ), m_traj( aTraj ), m_last_waypoint(){};

  virtual const topology& get_temporal_space() const { return m_traj.get_temporal_space(); };
  ...
  virtual double travel_distance( waypoint_pair& a, waypoint_pair& b ) const {
    m_last_waypoint
      = wrapped_waypoint_pair( boost::any_cast< wrapped_const_waypoint_descriptor >( a.first ), a.second );
  ...
  virtual waypoint_pair move_time_diff_from( const waypoint_pair& a, double dt ) const {
    m_last_waypoint = m_traj.move_time_diff_from(
      wrapped_waypoint_pair( boost::any_cast< wrapped_const_waypoint_descriptor >( a.first ), a.second ), dt );
  ...
  virtual double get_start_time() const { return m_traj.get_start_time(); };

  virtual double get_end_time() const { return m_traj.get_end_time(); };
