 ...
 */
template < typename JointSpace, typename DKTopoMap, typename JointSpaceMapping = identity_topo_map,
...
public:
  ...
  *******************************************************************************/
  ...
  virtual void RK_CALL save( ReaK::serialization::oarchive& A, unsigned int ) const {
    ...
    A& RK_SERIAL_SAVE_WITH_NAME( dk_topomap ) & RK_SERIAL_SAVE_WITH_NAME( map_to_jt_space )
      & RK_SERIAL_SAVE_WITH_NAME( inter_space );
  };
  virtual void RK_CALL load( ReaK::serialization::iarchive& A, unsigned int ) {
    base_type::load( A, base_type::getStaticObjectType()->TypeVersion() );
    A& RK_SERIAL_LOAD_WITH_NAME( dk_topomap ) & RK_SERIAL_LOAD_WITH_NAME( map_to_jt_space )
...
shared_ptr< any_model_applicator< JointSpace, DKTopoMap > > make_any_model_applicator( const DKTopoMap& aDKMap ) {
  return shared_ptr< any_model_applicator< JointSpace, DKTopoMap > >(
    new any_model_applicator< JointSpace, DKTopoMap >( aDKMap ) );
};
...
shared_ptr< any_model_applicator< JointSpace, DKTopoMap, JointSpaceMapping, IntermediateSpace > >
  ...
  return shared_ptr< any_model_applicator< JointSpace, DKTopoMap, JointSpaceMapping, IntermediateSpace > >(
    new any_model_applicator< JointSpace, DKTopoMap, JointSpaceMapping, IntermediateSpace >( aDKMap, aMapToJtSpace,
                                                                                             aInterSpace ) );
};
};
};
