...
struct temporal_point : public serializable {
   ...
   */
  temporal_point( const TimePoint& aTime, const SpacePoint& aPt ) : time( aTime ), pt( aPt ){};
...
struct temporal_point_difference : public serializable {
   ...
   */
  temporal_point_difference( const TimeDiff& aTime, const SpaceDiff& aPt ) : time( aTime ), pt( aPt ){};
  ...
  friend self operator*( const self& a, double b ) { return self( a.time * b, a.pt * b ); };

  friend self operator*( double a, const self& b ) { return self( a * b.time, a * b.pt ); };
...
public:
  ...
  typename topology_traits< OutSpace >::point_type map_to_space( const PointType& pt, const InSpace& space_in,
    ...
    return OutPointType(
      pt.time, spatial_map.map_to_space( pt.pt, space_in.get_space_topology(), space_out.get_space_topology() ) );
  };
  ...
  *******************************************************************************/
  ...
  virtual void RK_CALL load( serialization::iarchive& A, unsigned int ) { A& RK_SERIAL_LOAD_WITH_NAME( spatial_map ); };
...
public:
   ...
   */
  template < typename PointType, typename InSpace, typename OutSpace >
  typename topology_traits< OutSpace >::point_type map_to_space( const PointType& pt, const InSpace&,
                                                                 const OutSpace& ) const {
    return pt.pt;
  };
...
public:
   ...
   */
  explicit temporal_space( const std::string& aName = "", const space_topology& aSpace = space_topology(),
  ...
  typedef temporal_point_difference< typename topology_traits< space_topology >::point_difference_type,
                                     typename topology_traits< time_topology >::point_difference_type >
    point_difference_type;

  BOOST_STATIC_CONSTANT( std::size_t, dimensions = topology_traits< space_topology >::dimensions
  ...
  friend const TemporalDistanceMetric& get( distance_metric_t, const self& space ) { return space.dist; };
  ...
  friend TemporalDistanceMetric& get( distance_metric_t, self& space ) { return space.dist; };
   ...
   */
  point_type origin() const { return point_type( time.origin(), space.origin() ); };

  /**
   * Tests if a given point is within the boundary of this space.
   */
  bool is_in_bounds( const point_type& a ) const { return time.is_in_bounds( a.time ) && space.is_in_bounds( a.pt ); };
  ...
  * **********************************************************************/
   ...
   */
  double distance( const point_type& a, const point_type& b ) const { return dist( a, b, *this ); };
  ...
  *******************************************************************************/
  ...
  virtual void RK_CALL save( serialization::oarchive& A, unsigned int ) const {
    ...
    A& RK_SERIAL_SAVE_WITH_NAME( space ) & RK_SERIAL_SAVE_WITH_NAME( time ) & RK_SERIAL_SAVE_WITH_NAME( dist );
  };

  virtual void RK_CALL load( serialization::iarchive& A, unsigned int ) {
    ReaK::named_object::load( A, named_object::getStaticObjectType()->TypeVersion() );
    A& RK_SERIAL_LOAD_WITH_NAME( space ) & RK_SERIAL_LOAD_WITH_NAME( time ) & RK_SERIAL_LOAD_WITH_NAME( dist );
  };
...
struct is_metric_space< temporal_space< Topology, TimeTopology, TemporalDistanceMetric > >
  : boost::mpl::and_< is_metric_space< Topology >, is_metric_space< TimeTopology > > {};

template < typename Topology, typename TimeTopology, typename TemporalDistanceMetric >
struct is_reversible_space< temporal_space< Topology, TimeTopology, TemporalDistanceMetric > >
  : boost::mpl::and_< is_reversible_space< Topology >, is_reversible_space< TimeTopology > > {};

template < typename Topology, typename TimeTopology, typename TemporalDistanceMetric >
struct is_point_distribution< temporal_space< Topology, TimeTopology, TemporalDistanceMetric > >
  : boost::mpl::and_< is_point_distribution< Topology >, is_point_distribution< TimeTopology > > {};
...
struct is_metric_symmetric< temporal_space< Topology, TimeTopology, TemporalDistanceMetric > >
  : boost::mpl::and_< is_metric_symmetric< Topology >, is_metric_symmetric< TimeTopology >,
