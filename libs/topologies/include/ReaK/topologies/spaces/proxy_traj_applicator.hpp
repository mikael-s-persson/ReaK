...
public:
   ...
   */
  proxy_traj_applicator( const shared_ptr< proxy_model_applicator< joint_space_type > >& aStaticApplicator
                         = shared_ptr< proxy_model_applicator< joint_space_type > >(),
  ...
  *******************************************************************************/
  ...
  virtual void RK_CALL save( ReaK::serialization::oarchive& A, unsigned int ) const {
    ...
    A& RK_SERIAL_SAVE_WITH_NAME( static_applicator ) & RK_SERIAL_SAVE_WITH_NAME( traj );
  };
  virtual void RK_CALL load( ReaK::serialization::iarchive& A, unsigned int ) {
    proxy_model_updater::load( A, proxy_model_updater::getStaticObjectType()->TypeVersion() );
    A& RK_SERIAL_LOAD_WITH_NAME( static_applicator ) & RK_SERIAL_LOAD_WITH_NAME( traj );
    if( traj )
      last_wp = traj->get_waypoint_at_time( traj->get_start_time() );
  };
