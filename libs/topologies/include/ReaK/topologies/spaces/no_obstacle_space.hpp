...
template < typename Topology, typename DistanceMetric = typename metric_space_traits< Topology >::distance_metric_type,
           typename RandomSampler = typename point_distribution_traits< Topology >::random_sampler_type >
class no_obstacle_space : public named_object {
public:
  typedef no_obstacle_space< Topology, DistanceMetric, RandomSampler > self;
  typedef Topology super_space_type;
...
public:
   ...
   */
  double norm( const point_difference_type& dp ) const { return m_distance( dp, m_space ); };
  ...
  std::pair< point_type, bool > random_walk( const point_type& p_u ) const {
    return std::pair< point_type, bool >(
      move_position_toward( p_u, 1.0,
                            m_space.adjust( p_u, m_space.difference( m_rand_sampler( m_space ), m_space.origin() ) ) ),
      true );
  };
  ...
  std::pair< point_type, bool > random_back_walk( const point_type& p_u ) const {
    return std::pair< point_type, bool >(
      move_position_back_to( m_space.adjust( p_u, -m_space.difference( m_rand_sampler( m_space ), m_space.origin() ) ),
                             1.0, p_u ),
      true );
  };
   ...
   */
  double bird_fly_to_goal( const point_type& p_u ) const { return m_distance( p_u, m_goal_point, m_space ); };
   ...
   */
  double bird_fly_to_start( const point_type& p_u ) const { return m_distance( m_start_point, p_u, m_space ); };
   ...
   */
  no_obstacle_space( const std::string& aName = "", const super_space_type& aSpace = super_space_type(),
                     double aMaxEdgeLength = std::numeric_limits< double >::infinity(),
                     DistanceMetric aDistMetric = DistanceMetric(), RandomSampler aRandSampler = RandomSampler() )
      : named_object(), max_edge_length( aMaxEdgeLength ), m_space( aSpace ), m_distance( aDistMetric ),
        m_rand_sampler( aRandSampler ), m_start_point( aRandSampler( aSpace ) ),
  ...
  *******************************************************************************/
  ...
  virtual void RK_CALL save( serialization::oarchive& A, unsigned int ) const {
    ...
    A& RK_SERIAL_SAVE_WITH_NAME( max_edge_length ) & RK_SERIAL_SAVE_WITH_NAME( m_space )
      & RK_SERIAL_SAVE_WITH_NAME( m_distance ) & RK_SERIAL_SAVE_WITH_NAME( m_rand_sampler )
      & RK_SERIAL_SAVE_WITH_NAME( m_start_point ) & RK_SERIAL_SAVE_WITH_NAME( m_goal_point );
  };

  virtual void RK_CALL load( serialization::iarchive& A, unsigned int ) {
    ReaK::named_object::load( A, named_object::getStaticObjectType()->TypeVersion() );
    A& RK_SERIAL_LOAD_WITH_NAME( max_edge_length ) & RK_SERIAL_LOAD_WITH_NAME( m_space )
      & RK_SERIAL_LOAD_WITH_NAME( m_distance ) & RK_SERIAL_LOAD_WITH_NAME( m_rand_sampler )
      & RK_SERIAL_LOAD_WITH_NAME( m_start_point ) & RK_SERIAL_LOAD_WITH_NAME( m_goal_point );
  };

  RK_RTTI_MAKE_CONCRETE_1BASE( self, 0xC2400021, 1, "no_obstacle_space", named_object )
};
...
template < typename Topology, typename DistanceMetric, typename RandomSampler >
struct is_reversible_space< no_obstacle_space< Topology, DistanceMetric, RandomSampler > >
...
struct is_metric_symmetric< no_obstacle_space< Topology, DistanceMetric, RandomSampler > >
  : boost::mpl::and_< is_metric_symmetric< DistanceMetric >, is_metric_symmetric< Topology > > {};
