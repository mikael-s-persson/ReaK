 ...
 *   p2 = space.move_position_back_to(p1, d, p3);  A point (p2) can be obtained by moving a fraction (d) back from a final point (p3) to previous point (p1).
 * 
 * if(is_steerable_space<ReversibleSpace>)
 *   tie(p2, st_rec) = space.steer_position_back_to(p1, d, p3);  A point (p2) and a steer-record (st_rec) can be obtained from attempting to steer a fraction (d) back from a final point (p3) to previous point (p1).
 *  
