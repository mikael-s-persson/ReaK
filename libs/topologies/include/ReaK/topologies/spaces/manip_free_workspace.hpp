...
public:
  ...
  manip_dk_proxy_env_impl( const shared_ptr< proxy_model_applicator< BaseJointSpace > >& aApplicator
                           = shared_ptr< proxy_model_applicator< BaseJointSpace > >() )
...
public:
  ...
  explicit manip_quasi_static_env( InterpMethodTag aInterpTag, const BaseJointSpace& aSpace = BaseJointSpace(),
                                   const shared_ptr< applicator_type >& aApplicator = shared_ptr< applicator_type >(),
                                   double aMinInterval = 0.1 )
      : min_interval( aMinInterval ), m_space( shared_ptr< super_space_base_type >(
                                        new interpolated_topology< BaseJointSpace, InterpMethodTag >( aSpace ) ) ),
        m_prox_env( aApplicator ){};
