...
void try_interpolation( const std::string& aMethodName, std::size_t& succ_count, const Vector& curve_ampl,
                        const Vector& curve_phase, double curve_freq, double interp_steps,
                        const ReaK::shared_ptr< TempTopoType >& topo, const PtContainer& pts,
  ...
  try {
    RK_SCOPE_EXIT_ROUTINE( report_construct_except ) {
      fail_reports << aMethodName << " exception construct 0 " << norm_2( curve_ampl ) << " " << curve_freq << " "
    ...
    for( double t = 0.0; t < 1.0 - 0.5 * interp_steps; ) {
      ...
      for( std::size_t j = 0; j < 100; ++j ) {
        ...
        RK_SCOPE_EXIT_ROUTINE( report_interp_except ) {
          fail_reports << aMethodName << " exception interp " << t << " " << norm_2( curve_ampl ) << " " << curve_freq
                       << " " << interp_steps << std::endl;
        };
        ...
        if( vect_is_nan( ( get< 0 >( p.pt ) ) ) || vect_is_nan( ( get< 1 >( p.pt ) ) )
          ...
          fail_reports << aMethodName << " NaN interp " << t << " " << norm_2( curve_ampl ) << " " << curve_freq << " "
                       << interp_steps << std::endl;
        ...
        if( vect_is_inf( ( get< 0 >( p.pt ) ) ) || vect_is_inf( ( get< 1 >( p.pt ) ) )
          ...
          fail_reports << aMethodName << " INF interp " << t << " " << norm_2( curve_ampl ) << " " << curve_freq << " "
                       << interp_steps << std::endl;
      ...
      if( norm_2( get< 0 >( p.pt ) - ref_pos ) > 1e-3 ) {
        fail_reports << aMethodName << " pos_tol interp " << t << " " << norm_2( curve_ampl ) << " " << curve_freq
                     << " " << interp_steps << std::endl;
...
struct interp_mc_test_space {
  ...
  static ReaK::shared_ptr< temp_topo_type > create( const vector_type& lb, const vector_type& ub, const vector_type& sb,
    ...
    return ReaK::shared_ptr< temp_topo_type >(
      new temp_topo_type( "temporal_space", ReaK::pp::make_Ndof_rl_space< StaticSpDim >( lb, ub, sb, ab, jb ) ) );
  };
};
...
struct interp_mc_test_space< 0 > {
  ...
  static ReaK::shared_ptr< temp_topo_type > create( const vector_type& lb, const vector_type& ub, const vector_type& sb,
                                                    const vector_type& ab, const vector_type& jb ) {
    return ReaK::shared_ptr< temp_topo_type >(
      new temp_topo_type( "temporal_space", ReaK::pp::make_Ndof_rl_space( lb, ub, sb, ab, jb ) ) );
  };
};
  ...
  for( std::size_t i = 0; i < mc_runs; ++i ) {
    ...
    for( double t = 0.0; t < 1.0 + 0.5 * interp_steps; t += interp_steps ) {
      ...
      for( std::size_t j = 0; j < pos.size(); ++j ) {
        ...
        acc[j] *= -curve_freq * curve_freq * std::sin( curve_freq * t + curve_phase[j] )
    ...
    if( vm.count( "all-interpolators" ) || vm.count( "linear" ) ) {
      try_interpolation< pp::linear_interp_traj< TempTopoType > >( "linear", linear_succ_count, curve_ampl, curve_phase,
                                                                   curve_freq, interp_steps, topo, pts, fail_reports );
    };
...
int main( int argc, char** argv ) {
  ...
  mc_options.add_options()( "space-dimensionality", po::value< std::size_t >()->default_value( 1 ),
    ...
    "interp-steps", po::value< double >()->default_value( 0.05 ), "the time-step between the interpolator's "
