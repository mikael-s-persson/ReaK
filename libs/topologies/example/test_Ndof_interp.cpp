...
int main( int argc, char** argv ) {
  ...
  task_src >> max_vel >> max_accel >> start_pt >> start_vel >> end_pt >> end_vel;

  typedef ReaK::pp::Ndof_rl_space< double, TEST_SIZE, 2 >::type SpaceType;
  typedef ReaK::pp::hyperbox_topology< ReaK::vect< double, TEST_SIZE >, ReaK::pp::inf_norm_distance_metric >
    BoxTopoType;
  ...
  ReaK::shared_ptr< SpaceType > topo = ReaK::shared_ptr< SpaceType >( new SpaceType(
    SpaceTupleType( BoxTopoType( "pos_topo", -max_pos, max_pos ), BoxTopoType( "vel_topo", -max_vel, max_vel ),
                    BoxTopoType( "acc_topo", -max_accel, max_accel ) ),
    ReaK::pp::manhattan_tuple_distance(), DiffTuple( DiffRule( max_vel ), DiffRule( max_accel ) ) ) );
  ...
  try {
    {
      ...
      for( double dt = 0.0; ( ( dt_total >= min_dt_final ) && ( dt <= dt_total ) ); dt += time_step ) {
        ...
        ReaK::pp::detail::svp_Ndof_interpolate_impl< boost::mpl::size_t< 2 > >(
          result_point, start_point, end_point, peak_velocity, *topo, t_topo, dt, dt_total );

        output_rec << ( min_time + dt );
        for( std::size_t i = 0; i < TEST_SIZE; ++i ) {
          output_rec << ReaK::get< 0 >( result_point )[i] << ReaK::get< 1 >( result_point )[i]
...
#if 1
    {
      ...
      double min_dt_final = ReaK::pp::detail::sap_compute_Ndof_interpolation_data_impl(
        start_point, end_point, peak_velocity, *topo, ReaK::pp::time_topology(), dt_total );
