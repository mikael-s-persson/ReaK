...
void try_interpolation( const std::string& aMethodName, const std::string& aTestData, std::size_t& succ_count,
                        std::size_t& graceful_failures, const InterpTopoType& topo, const PtContainer& pts,
...
void perform_mc_tests( const po::variables_map& vm, std::size_t dyn_sp_dim ) {
  ...
  for( std::size_t i = 0; i < mc_runs; ++i ) {
    ...
    if( vm.count( "all-interpolators" ) || vm.count( "linear" ) ) {
      try_interpolation( "linear", test_data_str, linear_succ_count, linear_graceful_fails, linear_topo, pts,
                         fail_reports );
    };
    ...
    if( vm.count( "all-interpolators" ) || vm.count( "cubic" ) ) {
      try_interpolation( "cubic", test_data_str, cubic_succ_count, cubic_graceful_fails, cubic_topo, pts,
                         fail_reports );
    };
    ...
    if( vm.count( "all-interpolators" ) || vm.count( "quintic" ) ) {
      try_interpolation( "quintic", test_data_str, quintic_succ_count, quintic_graceful_fails, quintic_topo, pts,
                         fail_reports );
    };
    ...
    if( vm.count( "all-interpolators" ) || vm.count( "svp-Ndof" ) ) {
      try_interpolation( "svp_Ndof", test_data_str, svp_Ndof_succ_count, svp_Ndof_graceful_fails, svp_Ndof_topo, pts,
                         fail_reports );
    };
    ...
    if( vm.count( "all-interpolators" ) || vm.count( "sap-Ndof" ) ) {
      try_interpolation( "sap_Ndof", test_data_str, sap_Ndof_succ_count, sap_Ndof_graceful_fails, sap_Ndof_topo, pts,
                         fail_reports );
    };
  ...
  if( vm.count( "all-interpolators" ) || vm.count( "linear" ) )
    succ_reports << "Linear interp"
                 ...
                 << "\%\n"
                 << "\t Hard-fail-rate: "
                 << ( 100.0 * ( 1.0 - double( linear_succ_count + linear_graceful_fails ) / double( total_segments ) ) )
                 << "\%." << std::endl;
#endif
#ifdef RK_ENABLE_TEST_CUBIC_INTERPOLATOR
  if( vm.count( "all-interpolators" ) || vm.count( "cubic" ) )
    succ_reports << "Cubic interp"
                 ...
                 << "\%\n"
                 << "\t Hard-fail-rate: "
                 << ( 100.0 * ( 1.0 - double( cubic_succ_count + cubic_graceful_fails ) / double( total_segments ) ) )
                 << "\%." << std::endl;
#endif
#ifdef RK_ENABLE_TEST_QUINTIC_INTERPOLATOR
  if( vm.count( "all-interpolators" ) || vm.count( "quintic" ) )
    succ_reports << "Quintic interp"
                 ...
                 << "\t Hard-fail-rate: "
                 << ( 100.0
                      * ( 1.0 - double( quintic_succ_count + quintic_graceful_fails ) / double( total_segments ) ) )
                 << "\%." << std::endl;
#endif
#ifdef RK_ENABLE_TEST_SVP_NDOF_INTERPOLATOR
  if( vm.count( "all-interpolators" ) || vm.count( "svp-Ndof" ) )
