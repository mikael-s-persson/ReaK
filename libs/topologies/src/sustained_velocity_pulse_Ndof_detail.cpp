...
static void svp_Ndof_compute_interpolated_values_balanced( double start_position, double end_position,
                                                           double start_velocity, double end_velocity,
                                                           double peak_velocity, double max_velocity, double dt,
...
void svp_Ndof_compute_interpolated_values( double start_position, double end_position, double start_velocity,
  ...
  svp_Ndof_compute_interpolated_values_balanced( start_position, end_position, start_velocity, end_velocity,
                                                 peak_velocity, max_velocity, dt, dt_total, result_pos, result_vel,
                                                 result_desc_acc );
};


inline void svp_Ndof_compute_ramp_dist_and_time( double v1, double v2, double vmax, double& d_pos, double& dt ) {
  using std::fabs;
...
struct svp_Ndof_no_cruise_calculator {
  ...
  svp_Ndof_no_cruise_calculator( double a_dp, double a_v1, double a_v2, double a_vmax )
...
static double svp_Ndof_compute_min_delta_time_numsolve( double start_position, double end_position,
                                                        double start_velocity, double end_velocity,
...
double svp_Ndof_compute_min_delta_time( double start_position, double end_position, double start_velocity,
  ...
  double min_delta_time = svp_Ndof_compute_min_delta_time_numsolve( start_position, end_position, start_velocity,
...
static double svp_Ndof_compute_min_delta_time_closedform( double start_position, double end_position,
                                                          double start_velocity, double end_velocity,
                                                          double& peak_velocity, double max_velocity ) {
  using std::fabs;
  using std::sqrt;
  ...
  peak_velocity = sqrt( max_velocity * fabs( end_position - start_position ) + 0.5 * start_velocity * start_velocity
  ...
  if( max_velocity * fabs( end_position - start_position )
      < 0.5 * ( start_velocity * start_velocity + end_velocity * end_velocity ) ) {
    // this means there exists a solution for the magnitude of vp:
    peak_velocity = sqrt( 0.5 * start_velocity * start_velocity + 0.5 * end_velocity * end_velocity
                          - max_velocity * fabs( end_position - start_position ) );
    if( ( peak_velocity < sign_p1_p0 * start_velocity ) && ( peak_velocity < sign_p1_p0 * end_velocity ) ) {
...
double svp_Ndof_compute_min_delta_time( double start_position, double end_position, double start_velocity,
  ...
  double min_delta_time = svp_Ndof_compute_min_delta_time_closedform( start_position, end_position, start_velocity,
                                                                      end_velocity, peak_velocity, max_velocity );
...
struct svp_Ndof_pos_diff_calculator {
  ...
  svp_Ndof_pos_diff_calculator( double a_dp, double a_v1, double a_v2, double a_vmax, double a_dt )
...
void svp_Ndof_compute_peak_velocity_closedform( double start_position, double end_position, double start_velocity,
                                                double end_velocity, double& peak_velocity, double max_velocity,
                                                double delta_time ) {
  ...
  if( ( fabs( start_velocity ) > max_velocity ) || ( fabs( end_velocity ) > max_velocity ) ) {
    ...
    throw optim::infeasible_problem(
      "Violation of the velocity bounds on invocation of the SVP peak-velocity solver!" );
  };

  svp_Ndof_pos_diff_calculator pd_calc( end_position - start_position, start_velocity, end_velocity, max_velocity,
                                        delta_time );
  ...
  if( v0_v1_ts * v0_v1_ts >= 4.0 * ( vm_p1_p0 + vsqr_avg ) ) {
    ...
    if( ( fabs( r1 ) < 1.001 * max_velocity )
        && ( pd_calc.get_delta_time_diff( sign_p1_p0 * r1 ) >= -1e-3 * max_velocity )
  ...
  if( end_velocity > start_velocity ) {
    ...
    if( fabs( v0_v1_ts ) > 1e-6 * max_velocity ) {
      ...
      if( ( fabs( peak_velocity ) < 1.001 * max_velocity )
          && ( pd_calc.get_delta_time_diff( peak_velocity ) >= -1e-3 * max_velocity )
  ...
  peak_velocity = -sign_p1_p0 * max_velocity;
  throw optim::infeasible_problem(
    "The SVP peak-velocity solver could not find a solution for the given boundary conditions!" );
};
...
void svp_Ndof_compute_peak_velocity( double start_position, double end_position, double start_velocity,
  ...
  svp_Ndof_compute_peak_velocity_closedform( start_position, end_position, start_velocity, end_velocity, peak_velocity,
