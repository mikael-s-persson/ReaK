...
public:
  ...
  ptrobot2D_test_world_impl( const std::string& aFileName, double aRobotRadius = 1.0 ) {
    ...
    if( world_map_image.empty() )
      throw std::ios_base::failure( "Could not open the world map image '" + aFileName
    ...
    for( int y = 0; y < grid_height; ++y ) {
      ...
      for( int x = 0; x < grid_width; ++x ) {
        if( ( color_bits[2] == 0 ) && ( color_bits[0] == 255 ) && ( color_bits[1] == 0 ) ) {
          // this is the start position.
          start_pos[0] = x;
          start_pos[1] = y;
        } else if( ( color_bits[2] == 0 ) && ( color_bits[0] == 0 ) && ( color_bits[1] == 255 ) ) {
...
//       };
  ...
  ptrobot2D_test_world_impl( const ptrobot2D_test_world_impl& rhs )
      :
        ...
        world_map_output( rhs.world_map_output.clone() ), bpp( rhs.bpp ),
#endif
        grid_width( rhs.grid_width ), grid_height( rhs.grid_height ), start_pos( rhs.start_pos ),
        goal_pos( rhs.goal_pos ){};
  ...
  bool is_free( const ptrobot2D_test_world::point_type& p ) const {
    ...
    if( ( color_bits[1] < 250 ) && ( color_bits[2] < 250 ) && ( color_bits[0] < 250 ) ) {
      return false;
    } else {
      return true;
    };
...
bool ptrobot2D_test_world::is_free( const ptrobot2D_test_world::point_type& p ) const { return pimpl->is_free( p ); };

void ptrobot2D_test_world::reset_output() const { pimpl->reset_output(); };

void ptrobot2D_test_world::save_output( const std::string& aFilename ) const {
  pimpl->save_output( aFilename + ".bmp" );
};

void ptrobot2D_test_world::draw_edge( const ptrobot2D_test_world::point_type& p_u,
...
double ptrobot2D_test_world::distance( const ptrobot2D_test_world::point_type& p1,
...
ptrobot2D_test_world::point_difference_type
  ptrobot2D_test_world::difference( const ptrobot2D_test_world::point_type& p1,
                                    const ptrobot2D_test_world::point_type& p2 ) const {
  return m_space.difference( p1, p2 );
};

ptrobot2D_test_world::point_type ptrobot2D_test_world::origin() const { return m_space.origin(); };

ptrobot2D_test_world::point_type
  ptrobot2D_test_world::adjust( const ptrobot2D_test_world::point_type& p,
                                const ptrobot2D_test_world::point_difference_type& dp ) const {
  return move_position_toward( p, 1.0, m_space.adjust( p, dp ) );
};

ptrobot2D_test_world::point_type
  ptrobot2D_test_world::move_position_toward( const ptrobot2D_test_world::point_type& p1, double fraction,
                                              const ptrobot2D_test_world::point_type& p2 ) const {
...
ptrobot2D_test_world::point_type
  ptrobot2D_test_world::move_position_back_to( const ptrobot2D_test_world::point_type& p1, double fraction,
                                               const ptrobot2D_test_world::point_type& p2 ) const {
  double dist = m_distance( p1, p2, m_space );
  if( dist * fraction > max_edge_length )
    fraction = max_edge_length / dist;
...
std::pair< ptrobot2D_test_world::point_type, bool >
  ptrobot2D_test_world::random_walk( const ptrobot2D_test_world::point_type& p_u ) const {
  ...
  do {
    ...
    p_v = move_position_toward(
      p_u, boost::uniform_01< global_rng_type&, double >( get_global_rng() )() * max_edge_length / dist, p_rnd );
    ++i;
...
std::pair< ptrobot2D_test_world::point_type, bool >
  ptrobot2D_test_world::random_back_walk( const ptrobot2D_test_world::point_type& p_u ) const {
...
const ptrobot2D_test_world::point_type& ptrobot2D_test_world::get_start_pos() const { return pimpl->start_pos; };

const ptrobot2D_test_world::point_type& ptrobot2D_test_world::get_goal_pos() const { return pimpl->goal_pos; };

void ptrobot2D_test_world::set_start_pos( const ptrobot2D_test_world::point_type& aStart ) {
  pimpl->start_pos = aStart;
};

void ptrobot2D_test_world::set_goal_pos( const ptrobot2D_test_world::point_type& aGoal ) { pimpl->goal_pos = aGoal; };
...
ptrobot2D_test_world::ptrobot2D_test_world( const std::string& aWorldMapImage, double aMaxEdgeLength,
                                            double aRobotRadius )
    : pimpl( new ptrobot2D_test_world_impl( aWorldMapImage, aRobotRadius ) ), world_map_file_name( aWorldMapImage ),
      robot_radius( aRobotRadius ), max_edge_length( aMaxEdgeLength ),
...
ptrobot2D_test_world::ptrobot2D_test_world( const ptrobot2D_test_world& rhs )
    : pimpl( new ptrobot2D_test_world_impl( *rhs.pimpl ) ), world_map_file_name( rhs.world_map_file_name ),
      robot_radius( rhs.robot_radius ), max_edge_length( rhs.max_edge_length ),
      m_space( "ptrobot2D_space", ptrobot2D_test_world::point_type( 0, 0 ),
               ptrobot2D_test_world::point_type( pimpl->grid_width, pimpl->grid_height ) ),
      m_distance( get( distance_metric, m_space ) ), m_rand_sampler( get( random_sampler, m_space ) ) {
  setName( "ptrobot2D_space_with_obstacles" );
...
ptrobot2D_test_world::~ptrobot2D_test_world() { delete pimpl; };

void RK_CALL ptrobot2D_test_world::save( serialization::oarchive& A, unsigned int ) const {
  ReaK::named_object::save( A, named_object::getStaticObjectType()->TypeVersion() );
  A& RK_SERIAL_SAVE_WITH_NAME( world_map_file_name ) & RK_SERIAL_SAVE_WITH_NAME( robot_radius )
    & RK_SERIAL_SAVE_WITH_NAME( max_edge_length ) & RK_SERIAL_SAVE_WITH_NAME( m_space )
    & RK_SERIAL_SAVE_WITH_NAME( m_distance ) & RK_SERIAL_SAVE_WITH_NAME( m_rand_sampler );
};

void RK_CALL ptrobot2D_test_world::load( serialization::iarchive& A, unsigned int ) {
  ReaK::named_object::load( A, named_object::getStaticObjectType()->TypeVersion() );
  A& RK_SERIAL_LOAD_WITH_NAME( world_map_file_name ) & RK_SERIAL_LOAD_WITH_NAME( robot_radius )
    & RK_SERIAL_LOAD_WITH_NAME( max_edge_length ) & RK_SERIAL_LOAD_WITH_NAME( m_space )
    & RK_SERIAL_LOAD_WITH_NAME( m_distance ) & RK_SERIAL_LOAD_WITH_NAME( m_rand_sampler );
  delete pimpl;
