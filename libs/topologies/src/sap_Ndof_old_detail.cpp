...
static void sap_Ndof_compute_interpolated_values_closedform( double start_position, double end_position,
                                                             double start_velocity, double end_velocity,
                                                             double peak_velocity, double max_velocity,
                                                             double max_acceleration, double dt, double dt_total,
                                                             double& result_pos, double& result_vel, double& result_acc,
                                                             double& result_desc_jerk ) {
  ...
  if( dt < ( 2.0 * dt_ap1 + dt_vp1 ) ) {
    ...
    if( dt < dt_ap1 ) {
      ...
      result_vel += ( dt_ap1 * ( dt + dt_vp1 + 0.5 * dt_ap1 ) - 0.5 * dt * dt ) * sgn_vp1 / max_acceleration;
  ...
  result_pos += ( peak_velocity * ( 2.0 * dt_ap1 + dt_vp1 )
                  - dt_ap1 * ( dt_ap1 + dt_vp1 ) * ( dt_ap1 + 0.5 * dt_vp1 ) * sgn_vp1 / max_acceleration )
                / max_velocity;
  ...
  if( dt < ( 2.0 * dt_ap1 + dt_vp1 + dt_total + 2.0 * dt_ap2 + dt_vp2 ) ) {
    ...
    if( dt < ( 2.0 * dt_ap1 + dt_vp1 + dt_total + dt_ap2 ) ) {
      ...
      result_pos += ( result_vel * ( max_acceleration + dt )
                      + ( max_acceleration * max_acceleration / 6.0 + 0.5 * max_acceleration * dt + 0.5 * dt * dt )
                        * sgn_vp2 ) / max_velocity;
      ...
      result_pos += ( result_vel * ( dt_ap2 + dt_vp2 + dt )
                      + ( dt_ap2 * dt_ap2 * dt_ap2 / 6.0 + 0.5 * dt_vp2 * dt_ap2 * dt_ap2
                          + 0.5 * dt_vp2 * dt_vp2 * dt_ap2 + dt * dt_vp2 * dt_ap2 + 0.5 * dt * dt_ap2 * dt_ap2
                          + 0.5 * dt * dt * dt_ap2 - dt * dt * dt / 6.0 ) * sgn_vp2 / max_acceleration ) / max_velocity;
      result_vel += ( dt_ap2 * sgn_vp2 - 0.5 * dt * sgn_vp2 ) * dt / max_acceleration
  ...
  result_pos = start_position
               + ( peak_velocity * dt_total + 0.5 * ( start_velocity + peak_velocity ) * ( 2.0 * dt_ap1 + dt_vp1 )
...
static void sap_Ndof_compute_interpolated_values_incremental_logged_version(
  double start_position, double end_position, double start_velocity, double end_velocity, double peak_velocity,
  double max_velocity, double max_acceleration, double dt, double dt_total, double& result_pos, double& result_vel,
  double& result_acc, double& result_desc_jerk, std::ostream& log_output ) {
  ...
  if( dt_ap1 > std::numeric_limits< double >::epsilon() ) {
    ...
    log_output << "-----------------------------------------------------------\n"
               << " Phase 1: in the jerk-up phase of velocity ramp-up.\n"
               << " dt = " << dt_ap1 << "\n"
               << " delta_pos = " << ( ( result_vel + ( dt_ap1 * descended_jerk / 6.0 ) * dt_ap1 / max_acceleration )
                                       * dt_ap1 / max_velocity ) << "\n"
    ...
    // assume result_acc == 0
    result_pos += ( result_vel + ( dt_ap1 * descended_jerk / 6.0 ) * dt_ap1 / max_acceleration ) * dt_ap1
                  / max_velocity;
    ...
    log_output << "-----------------------------------------------------------\n"
               << " Phase 3: in the jerk-down phase of velocity ramp-up.\n"
               << " dt = " << dt_ap1 << "\n"
               << " delta_pos = " << ( ( result_vel
                                         + ( 0.5 * result_acc + ( 1.0 / 6.0 ) * dt_ap1 * descended_jerk ) * dt_ap1
                                           / max_acceleration ) * dt_ap1 / max_velocity ) << "\n"
    ...
    result_pos
      += ( result_vel + ( 0.5 * result_acc + ( 1.0 / 6.0 ) * dt_ap1 * descended_jerk ) * dt_ap1 / max_acceleration )
         * dt_ap1 / max_velocity;
  ...
  if( dt_ap2 > std::numeric_limits< double >::epsilon() ) {
    ...
    log_output << "-----------------------------------------------------------\n"
               << " Phase 5: in the jerk-up phase of velocity ramp-down.\n"
               << " dt = " << dt_ap2 << "\n"
               << " delta_pos = " << ( ( result_vel + ( dt_ap2 * descended_jerk / 6.0 ) * dt_ap2 / max_acceleration )
                                       * dt_ap2 / max_velocity ) << "\n"
    ...
    result_pos += ( result_vel + ( dt_ap2 * descended_jerk / 6.0 ) * dt_ap2 / max_acceleration ) * dt_ap2
                  / max_velocity;
    ...
    log_output << "-----------------------------------------------------------\n"
               ...
               << " dt = " << dt_ap2 << "\n"
               << " delta_pos = " << ( ( result_vel
                                         + ( 0.5 * result_acc + ( 1.0 / 6.0 ) * dt_ap2 * descended_jerk ) * dt_ap2
                                           / max_acceleration ) * dt_ap2 / max_velocity ) << "\n"
    ...
    result_pos
      += ( result_vel + ( 0.5 * result_acc + ( 1.0 / 6.0 ) * dt_ap2 * descended_jerk ) * dt_ap2 / max_acceleration )
         * dt_ap2 / max_velocity;
...
void sap_Ndof_compute_interpolated_values( double start_position, double end_position, double start_velocity,
  ...
  sap_Ndof_compute_interpolated_values_incremental_logged_version(
    start_position, end_position, start_velocity, end_velocity, peak_velocity, max_velocity, max_acceleration, dt,
    dt_total, result_pos, result_vel, result_acc, result_desc_jerk, log_output );
...
inline void sap_Ndof_compute_ramp_dist_and_time( double v1, double v2, double vmax, double amax, double& d_pos,
                                                 double& dt ) {
...
static double sap_Ndof_compute_min_delta_time_logged_version( double start_position, double end_position,
                                                              double start_velocity, double end_velocity,
                                                              double& peak_velocity, double max_velocity,
  ...
  log_output << "-----------------------------------------------------------\n"
             ...
             << " vsqr_avg = 0.5 * (start_velocity * start_velocity + end_velocity * end_velocity) = " << vsqr_avg
             << std::endl;
  ...
  sap_Ndof_compute_ramp_dist_and_time( start_velocity, peak_velocity, max_velocity, max_acceleration, dp_ramp1,
                                       dt_ramp1 );
  sap_Ndof_compute_ramp_dist_and_time( peak_velocity, end_velocity, max_velocity, max_acceleration, dp_ramp2,
                                       dt_ramp2 );
  ...
  log_output << "-----------------------------------------------------------\n"
             ...
             << " 'if(delta_first_order * peak_velocity > 0.0)' with delta_first_order * peak_velocity = "
  ...
  for( double cur_vp = sign_p1_p0 * max_velocity; cur_vp * sign_p1_p0 > -1.05 * max_velocity;
       cur_vp -= 0.1 * sign_p1_p0 * max_velocity ) {
    double cur_dt_ramp1 = 0.0, cur_dp_ramp1 = 0.0, cur_dt_ramp2 = 0.0, cur_dp_ramp2 = 0.0;

    sap_Ndof_compute_ramp_dist_and_time( start_velocity, cur_vp, max_velocity, max_acceleration, cur_dp_ramp1,
                                         cur_dt_ramp1 );
    sap_Ndof_compute_ramp_dist_and_time( cur_vp, end_velocity, max_velocity, max_acceleration, cur_dp_ramp2,
                                         cur_dt_ramp2 );

    log_output << "  vp = " << cur_vp << " dt = " << ( cur_dt_ramp1 + cur_dt_ramp2 )
               << " gives dp = " << sign_p1_p0 * ( end_position - start_position - cur_dp_ramp1 - cur_dp_ramp2 )
               << std::endl;
  };
  ...
  if( descrim > 0.0 ) {
    ...
    log_output
      ...
      << "  'if((fabs(vp) <= vm) && (sign_p1_p0 * vp >= sign_p1_p0 * vs) && (sign_p1_p0 * vp >= sign_p1_p0 * ve))'"
      << std::endl;

    if( ( fabs( peak_velocity ) <= max_velocity ) && ( sign_p1_p0 * peak_velocity >= sign_p1_p0 * start_velocity )
        && ( sign_p1_p0 * peak_velocity >= sign_p1_p0 * end_velocity ) ) {
      log_output << "   condition passed!"
                 << "\n"
                 << "   min-dt = " << ( fabs( peak_velocity - start_velocity ) + fabs( peak_velocity - end_velocity )
                                        + 2.0 * max_acceleration ) << std::endl;
  ...
  delta_first_order = end_position - start_position
                      - 0.5 * ( fabs( peak_velocity - end_velocity ) + max_acceleration )
  ...
  log_output
    << "-----------------------------------------------------------\n"
    << " peak_velocity = sign_p1_p0 * max_velocity = " << peak_velocity << "\n"
    << " delta_first_order = pe - ps - 0.5 * (fabs(vp - ve) + am) * (vp + ve) / vm - fabs(vp - vs) * (vp + vs) / vm = "
    << delta_first_order << "\n"
    << " 'if(delta_first_order * peak_velocity > 0.0)' with delta_first_order * peak_velocity = "
    << ( delta_first_order * peak_velocity ) << std::endl;
  ...
  if( delta_first_order * peak_velocity > 0.0 ) {
    log_output << "  condition passed!"
               << "\n"
               << "  min-dt = " << ( fabs( delta_first_order ) + fabs( peak_velocity - end_velocity )
                                     + 2.0 * fabs( peak_velocity - start_velocity ) + max_acceleration ) << std::endl;
    // this means that we guessed correctly (we can reach max cruise speed in the direction of the end-point):
    return fabs( delta_first_order ) + fabs( peak_velocity - end_velocity )
           + 2.0 * fabs( peak_velocity - start_velocity ) + max_acceleration;
  };
  ...
  descrim = max_acceleration * max_acceleration / 9.0
            - 4.0 / 3.0 * ( sign_p1_p0 * max_acceleration * end_velocity - 2.0 * vm_p1_p0
                            - 2.0 * start_velocity * start_velocity - end_velocity * end_velocity );

  log_output
    << "-----------------------------------------------------------\n"
    << " descrim = am * am / 9.0 - 4.0 / 3.0 * (sign_p1_p0 * am * ve - 2.0 * vm_p1_p0 - 2.0 * vs * vs - ve * ve) = "
    << descrim << "\n"
    << " 'if(descrim > 0.0)'" << std::endl;
  ...
  if( descrim > 0.0 ) {
    ...
    if( ( fabs( peak_velocity ) <= max_velocity ) && ( sign_p1_p0 * peak_velocity >= sign_p1_p0 * start_velocity )
      ...
      log_output << "   condition passed!"
                 << "\n"
                 << "   min-dt = " << ( fabs( peak_velocity - end_velocity )
                                        + 2.0 * fabs( peak_velocity - start_velocity ) + max_acceleration )
                 << std::endl;
      // this means that this root works for this case.
  ...
  //   min-dt):
  descrim = max_acceleration * max_acceleration / 9.0
            - 4.0 / 3.0 * ( 2.0 * vm_p1_p0 + 2.0 * start_velocity * start_velocity + end_velocity * end_velocity
                            - sign_p1_p0 * max_acceleration * end_velocity );

  log_output
    << "-----------------------------------------------------------\n"
    << " descrim = am * am / 9.0 - 4.0 / 3.0 * (2.0 * vm_p1_p0 + 2.0 * vs * vs + ve * ve - sign_p1_p0 * am * ve) = "
    << descrim << "\n"
    << " 'if(descrim > 0.0)'" << std::endl;
  ...
  delta_first_order = end_position - start_position
                      ...
                      - 0.5 * ( fabs( peak_velocity - start_velocity ) + max_acceleration )
                        * ( peak_velocity + start_velocity ) / max_velocity;

  log_output
    << "-----------------------------------------------------------\n"
    << " peak_velocity = sign_p1_p0 * max_velocity = " << peak_velocity << "\n"
    << " delta_first_order = pe - ps - fabs(vp - ve) * (vp + ve) / vm - 0.5 * (fabs(vp - vs) + am) * (vp + vs) / vm = "
    << delta_first_order << "\n"
    << " 'if(delta_first_order * peak_velocity > 0.0)' with delta_first_order * peak_velocity = "
    << ( delta_first_order * peak_velocity ) << std::endl;
  ...
  if( delta_first_order * peak_velocity > 0.0 ) {
    log_output << "  condition passed!"
               << "\n"
               << "  min-dt = " << ( fabs( delta_first_order ) + 2.0 * fabs( peak_velocity - end_velocity )
                                     + fabs( peak_velocity - start_velocity ) + max_acceleration ) << std::endl;
    // this means that we guessed correctly (we can reach max cruise speed in the direction of the end-point):
    return fabs( delta_first_order ) + 2.0 * fabs( peak_velocity - end_velocity )
           + fabs( peak_velocity - start_velocity ) + max_acceleration;
  };
  ...
  //   first try if vp is more in the direction (p1-p0) than both v1 and v0:
  descrim = max_acceleration * max_acceleration / 9.0
            - 4.0 / 3.0 * ( sign_p1_p0 * max_acceleration * start_velocity - 2.0 * vm_p1_p0
                            - start_velocity * start_velocity - 2.0 * end_velocity * end_velocity );

  log_output
    << "-----------------------------------------------------------\n"
    << " descrim = am * am / 9.0 - 4.0 / 3.0 * (sign_p1_p0 * am * vs - 2.0 * vm_p1_p0 - vs * vs - 2.0 * ve * ve) = "
    << descrim << "\n"
    << " 'if(descrim > 0.0)'" << std::endl;
  ...
  if( descrim > 0.0 ) {
    ...
    if( ( fabs( peak_velocity ) <= max_velocity ) && ( sign_p1_p0 * peak_velocity >= sign_p1_p0 * start_velocity )
      ...
      log_output << "   condition passed!"
                 << "\n"
                 << "   min-dt = " << ( 2.0 * fabs( peak_velocity - end_velocity )
                                        + fabs( peak_velocity - start_velocity ) + max_acceleration ) << std::endl;
      // this means that this root works for this case.
  ...
  //   min-dt):
  descrim = max_acceleration * max_acceleration / 9.0
            - 4.0 / 3.0 * ( 2.0 * vm_p1_p0 + start_velocity * start_velocity + 2.0 * end_velocity * end_velocity
                            - sign_p1_p0 * max_acceleration * start_velocity );

  log_output
    << "-----------------------------------------------------------\n"
    << " descrim = am * am / 9.0 - 4.0 / 3.0 * ( 2.0 * vm_p1_p0 + vs * vs + 2.0 * ve * ve - sign_p1_p0 * am * vs) = "
    << descrim << "\n"
    << " 'if(descrim > 0.0)'" << std::endl;
  ...
  log_output << "-----------------------------------------------------------\n"
             << " peak_velocity = sign_p1_p0 * max_velocity = " << peak_velocity << "\n"
             << " delta_first_order = pe - ps - fabs(vp - ve) * (vp + ve) / vm - fabs(vp - vs) * (vp + vs) / vm = "
             << delta_first_order << "\n"
             << " 'if(delta_first_order * peak_velocity > 0.0)' with delta_first_order * peak_velocity = "
             << ( delta_first_order * peak_velocity ) << std::endl;
  ...
  if( delta_first_order * peak_velocity > 0.0 ) {
    log_output << "  condition passed!"
               << "\n"
               << "  min-dt = " << ( fabs( delta_first_order ) + 2.0 * fabs( peak_velocity - end_velocity )
                                     + 2.0 * fabs( peak_velocity - start_velocity ) ) << std::endl;
    // this means that we guessed correctly (we can reach max cruise speed in the direction of the end-point):
    return fabs( delta_first_order ) + 2.0 * fabs( peak_velocity - end_velocity )
           + 2.0 * fabs( peak_velocity - start_velocity );
  };
  ...
  if( ( fabs( peak_velocity ) <= max_velocity ) && ( peak_velocity >= sign_p1_p0 * start_velocity )
    ...
    log_output << "  condition passed!"
               << "\n"
               << "  peak_velocity = " << peak_velocity << "\n"
               << "  min-dt = " << ( fabs( peak_velocity - end_velocity ) + 2.0 * fabs( peak_velocity - start_velocity )
                                     + max_acceleration ) << std::endl;
    return fabs( peak_velocity - end_velocity ) + 2.0 * fabs( peak_velocity - start_velocity ) + max_acceleration;
  };
  ...
  log_output << "-----------------------------------------------------------\n"
             << "Reached the end of the calculation and did not find a suitable root! This should never happen!"
             << std::endl;
...
double sap_Ndof_compute_min_delta_time( double start_position, double end_position, double start_velocity,
  ...
  sap_Ndof_compute_interpolated_values_incremental_logged_version(
    start_position, end_position, start_velocity, end_velocity, peak_velocity, max_velocity, max_acceleration,
    min_delta_time, min_delta_time, result_pos, result_vel, result_acc, result_desc_jerk, log_output );
...
void sap_Ndof_compute_peak_velocity( double start_position, double end_position, double start_velocity,
  ...
  if( descrim > 0.0 ) {
    ...
    if( ( fabs( peak_velocity ) <= max_velocity ) && ( sign_p1_p0 * peak_velocity >= start_velocity * sign_p1_p0 )
  ...
  if( end_velocity > start_velocity ) {
    ...
    if( fabs( v0_v1_ts_sa ) > 1e-6 * max_velocity ) {
      ...
      if( ( fabs( peak_velocity ) <= max_velocity ) && ( peak_velocity >= start_velocity )
    ...
    if( fabs( v0_v1_ts_sa ) > 1e-6 * max_velocity ) {
      peak_velocity = ( vm_p1_p0 + vsqr_avg - a_v0_v1_2 ) / v0_v1_ts_sa;
      if( ( fabs( peak_velocity ) <= max_velocity ) && ( peak_velocity >= end_velocity )
          && ( peak_velocity <= start_velocity ) )
        return;
      // then, the solution doesn't fit the assumption.
    };
  ...
  if( descrim > 0.0 ) {
    // then there is a real root to the quadratic equation.
    peak_velocity = 0.5 * ( v0_v1_ts_sa + sqrt( descrim ) );
    if( ( fabs( peak_velocity ) <= max_velocity ) && ( peak_velocity * sign_p1_p0 <= start_velocity * sign_p1_p0 )
  ...
  // try to assume that vp more in the direction (p1-p0) than both v1 and v0 (i.e., ramp-up and ramp-down).
  v0_v1_ts_sa = ( 2.0 * start_velocity + end_velocity + ( delta_time - 0.5 * max_acceleration ) * sign_p1_p0 ) * 2.0
                / 3.0;
  ...
  // try to assume that vp more in the direction (p1-p0) than both v1 and v0 (i.e., ramp-up and ramp-down).
  v0_v1_ts_sa = ( 2.0 * end_velocity + start_velocity + ( delta_time - 0.5 * max_acceleration ) * sign_p1_p0 ) * 2.0
                / 3.0;
  vm_p1_p0 = max_velocity * fabs( end_position - start_position ) * 2.0 / 3.0;
...
static void sap_Ndof_compute_peak_velocity_numsolve( double start_position, double end_position, double start_velocity,
                                                     double end_velocity, double& peak_velocity, double max_velocity,
                                                     double max_acceleration, double delta_time ) {
  ...
  if( ( fabs( start_velocity ) > max_velocity ) || ( fabs( end_velocity ) > max_velocity ) ) {
    ...
    throw optim::infeasible_problem(
      "Violation of the velocity bounds on invocation of the SAP peak-velocity solver!" );
  };
  ...
  sap_Ndof_pos_diff_calculator pd_calc( end_position - start_position, start_velocity, end_velocity, max_velocity,
                                        max_acceleration, delta_time );

  double prev_vp = 1.03 * sign_p1_p0 * max_velocity;
  double prev_pd = pd_calc( prev_vp );
  for( double cur_vp = prev_vp - 0.02 * sign_p1_p0 * max_velocity; cur_vp * sign_p1_p0 > -1.04 * max_velocity;
  ...
  peak_velocity = 0.0;
  throw optim::infeasible_problem(
    "The SAP peak-velocity solver could not find a solution for the given boundary conditions!" );
};
};
};
};
