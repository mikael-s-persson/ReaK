...
static void sap_Ndof_compute_interpolated_values_closedform(
    ...
    double& result_vel, double& result_acc, double& result_desc_jerk) {
  ...
  if (dt < (2.0 * dt_ap1 + dt_vp1)) {
    ...
    if (dt < dt_ap1) {
      ...
      // assume result_acc == 0
      result_pos +=
          (result_vel + (dt * sgn_vp1 / 6.0) * dt / max_acceleration) * dt /
          max_velocity;
      ...
      result_pos +=
          ((result_vel + 0.5 * dt * sgn_vp1) * (max_acceleration + dt) +
      ...
      result_pos +=
          (result_vel * (dt_ap1 + dt_vp1 + dt) +
           (dt_ap1 * dt * (0.5 * dt + dt_vp1 + 0.5 * dt_ap1) +
            0.5 * dt_ap1 *
                (dt_ap1 * dt_ap1 / 3.0 + dt_vp1 * dt_ap1 + dt_vp1 * dt_vp1) -
            dt * dt * dt / 6.0) *
               sgn_vp1 / max_acceleration) /
          max_velocity;
      result_vel += (dt_ap1 * (dt + dt_vp1 + 0.5 * dt_ap1) - 0.5 * dt * dt) *
                    sgn_vp1 / max_acceleration;
      ...
      //   + ( mdt * mdt * mdt / 6.0 - dt_ap1 * ( dt_ap1 + dt_vp1 ) * ( dt_ap1 + 0.5 * dt_vp1 ) ) * sgn_vp1 /
  ...
  result_pos += (peak_velocity * (2.0 * dt_ap1 + dt_vp1) -
                 dt_ap1 * (dt_ap1 + dt_vp1) * (dt_ap1 + 0.5 * dt_vp1) *
                     sgn_vp1 / max_acceleration) /
                max_velocity;
  ...
  if (dt < (2.0 * dt_ap1 + dt_vp1 + dt_total + 2.0 * dt_ap2 + dt_vp2)) {
    ...
    if (dt < (2.0 * dt_ap1 + dt_vp1 + dt_total + dt_ap2)) {
      ...
      result_pos +=
          (result_vel + (dt * sgn_vp2 / 6.0) * dt / max_acceleration) * dt /
          max_velocity;
      ...
      result_pos +=
          (result_vel * (dt_ap2 + dt_vp2 + dt) +
           (dt_ap2 * dt_ap2 * dt_ap2 / 6.0 + 0.5 * dt_vp2 * dt_ap2 * dt_ap2 +
            0.5 * dt_vp2 * dt_vp2 * dt_ap2 + dt * dt_vp2 * dt_ap2 +
            0.5 * dt * dt_ap2 * dt_ap2 + 0.5 * dt * dt * dt_ap2 -
            dt * dt * dt / 6.0) *
               sgn_vp2 / max_acceleration) /
          max_velocity;
      result_vel +=
          (dt_ap2 * sgn_vp2 - 0.5 * dt * sgn_vp2) * dt / max_acceleration +
          (dt_vp2 + 0.5 * dt_ap2) * dt_ap2 * sgn_vp2 / max_acceleration;
      ...
      //   + ( mdt * mdt * mdt / 6.0 - dt_ap2 * ( dt_ap2 + dt_vp2 ) * ( dt_ap2 + 0.5 * dt_vp2 ) ) * sgn_vp2 /
      //   max_acceleration ) / max_velocity;
  ...
  //     result_pos += ( result_vel + 0.5 * ( dt_vp2 + dt_ap2 ) * dt_ap2 * sgn_vp2 / max_acceleration ) * ( 2.0 * dt_ap2
  //     + dt_vp2 ) / max_velocity;
  ...
  //       + 0.5 * ( dt_vp2 + dt_ap2 ) * ( 2.0 * dt_ap2 + dt_vp2 ) * dt_ap2 * sgn_vp2 / max_acceleration ) /
  //       max_velocity;

  result_pos =
      start_position +
      (peak_velocity * dt_total +
       0.5 * (start_velocity + peak_velocity) * (2.0 * dt_ap1 + dt_vp1) +
       0.5 * (peak_velocity + end_velocity) * (2.0 * dt_ap2 + dt_vp2)) /
          max_velocity;
  result_acc = 0.0;
  result_desc_jerk = 0.0;
};
...
static void sap_Ndof_compute_interpolated_values_incremental_logged_version(
  ...
  if (dt_ap1 > std::numeric_limits<double>::epsilon()) {
    ...
    log_output
        ...
        << " delta_pos = "
        << ((result_vel +
             (dt_ap1 * descended_jerk / 6.0) * dt_ap1 / max_acceleration) *
            dt_ap1 / max_velocity)
        << "\n"
        << " delta_vel = "
        << ((0.5 * dt_ap1 * descended_jerk) * dt_ap1 / max_acceleration) << "\n"
        << " delta_acc = " << (dt_ap1 * descended_jerk) << std::endl;

    {
      double exp_delta_pos = 0.0;
      for (double exp_t = 0.0; exp_t < 1.00005 * dt_ap1;
           exp_t += 0.0001 * dt_ap1) {
        double exp_dv =
            (0.5 * exp_t * descended_jerk) * exp_t / max_acceleration;
        exp_delta_pos += 0.0001 * dt_ap1 * (result_vel + exp_dv) / max_velocity;
      };
      log_output << " num-integrated delta_pos = " << exp_delta_pos
                 << std::endl;
    };

    // assume result_acc == 0
    result_pos += (result_vel + (dt_ap1 * descended_jerk / 6.0) * dt_ap1 /
                                    max_acceleration) *
                  dt_ap1 / max_velocity;
    ...
    if (dt_vp1 > std::numeric_limits<double>::epsilon()) {
      ...
      log_output
          ...
          << " dt = " << dt_vp1 << "\n"
          << " delta_pos = "
          << ((result_vel + 0.5 * dt_vp1 * descended_accel) * dt_vp1 /
              max_velocity)
          << "\n"
      ...
      {
        double exp_delta_pos = 0.0;
        for (double exp_t = 0.0; exp_t < 1.00005 * dt_vp1;
             exp_t += 0.0001 * dt_vp1) {
          double exp_dv = (exp_t * descended_accel);
          exp_delta_pos +=
              0.0001 * dt_vp1 * (result_vel + exp_dv) / max_velocity;
        };
        log_output << " num-integrated delta_pos = " << exp_delta_pos
                   << std::endl;
      };
      ...
      result_pos +=
          (result_vel + 0.5 * dt_vp1 * descended_accel) * dt_vp1 / max_velocity;
    ...
    log_output
        ...
        << " delta_pos = "
        << ((result_vel +
             (0.5 * result_acc + (1.0 / 6.0) * dt_ap1 * descended_jerk) *
                 dt_ap1 / max_acceleration) *
            dt_ap1 / max_velocity)
        << "\n"
        << " delta_vel = "
        << ((result_acc + 0.5 * dt_ap1 * descended_jerk) * dt_ap1 /
            max_acceleration)
        << "\n"
        << " delta_acc = " << (dt_ap1 * descended_jerk) << std::endl;

    {
      ...
      for (double exp_t = 0.0; exp_t < 1.00005 * dt_ap1;
           exp_t += 0.0001 * dt_ap1) {
        double exp_dv = (result_acc + 0.5 * exp_t * descended_jerk) * exp_t /
                        max_acceleration;
        exp_delta_pos += 0.0001 * dt_ap1 * (result_vel + exp_dv) / max_velocity;
      };
      log_output << " num-integrated delta_pos = " << exp_delta_pos
    ...
    result_pos += (result_vel +
                   (0.5 * result_acc + (1.0 / 6.0) * dt_ap1 * descended_jerk) *
                       dt_ap1 / max_acceleration) *
                  dt_ap1 / max_velocity;
    result_vel += (result_acc + 0.5 * dt_ap1 * descended_jerk) * dt_ap1 /
                  max_acceleration;
    result_acc += dt_ap1 * descended_jerk;
  ...
  log_output << "-----------------------------------------------------------\n"
             << " Phase 4: in the cruise phase.\n"
             << " dt = " << dt_total << "\n"
             << " delta_pos = " << (dt_total * peak_velocity / max_velocity)
             << std::endl;

  {
    double exp_delta_pos = 0.0;
    for (double exp_t = 0.0; exp_t < 1.00005 * dt_total;
         exp_t += 0.0001 * dt_total) {
  ...
  if (dt_ap2 > std::numeric_limits<double>::epsilon()) {
    ...
    log_output
        ...
        << " delta_pos = "
        << ((result_vel +
             (dt_ap2 * descended_jerk / 6.0) * dt_ap2 / max_acceleration) *
            dt_ap2 / max_velocity)
        << "\n"
        << " delta_vel = "
        << ((0.5 * dt_ap2 * descended_jerk) * dt_ap2 / max_acceleration) << "\n"
        << " delta_acc = " << (dt_ap2 * descended_jerk) << std::endl;

    {
      double exp_delta_pos = 0.0;
      for (double exp_t = 0.0; exp_t < 1.00005 * dt_ap2;
           exp_t += 0.0001 * dt_ap2) {
        double exp_dv =
            (0.5 * exp_t * descended_jerk) * exp_t / max_acceleration;
    ...
    result_pos += (result_vel + (dt_ap2 * descended_jerk / 6.0) * dt_ap2 /
                                    max_acceleration) *
                  dt_ap2 / max_velocity;
    ...
    if (dt_vp2 > std::numeric_limits<double>::epsilon()) {
      ...
      log_output
          ...
          << " dt = " << dt_vp2 << "\n"
          << " delta_pos = "
          << ((result_vel + 0.5 * dt_vp2 * descended_accel) * dt_vp2 /
              max_velocity)
          << "\n"
      ...
      {
        double exp_delta_pos = 0.0;
        for (double exp_t = 0.0; exp_t < 1.00005 * dt_vp2;
             exp_t += 0.0001 * dt_vp2) {
          double exp_dv = exp_t * descended_accel;
          exp_delta_pos +=
              0.0001 * dt_vp2 * (result_vel + exp_dv) / max_velocity;
        };
        log_output << " num-integrated delta_pos = " << exp_delta_pos
                   << std::endl;
      };
      ...
      result_pos +=
          (result_vel + 0.5 * dt_vp2 * descended_accel) * dt_vp2 / max_velocity;
    ...
    log_output
        ...
        << " delta_pos = "
        << ((result_vel +
             (0.5 * result_acc + (1.0 / 6.0) * dt_ap2 * descended_jerk) *
                 dt_ap2 / max_acceleration) *
            dt_ap2 / max_velocity)
        << "\n"
        << " delta_vel = "
        << ((result_acc + 0.5 * dt_ap2 * descended_jerk) * dt_ap2 /
            max_acceleration)
        << "\n"
        << " delta_acc = " << (dt_ap2 * descended_jerk) << std::endl;
    ...
    result_pos += (result_vel +
                   (0.5 * result_acc + (1.0 / 6.0) * dt_ap2 * descended_jerk) *
                       dt_ap2 / max_acceleration) *
                  dt_ap2 / max_velocity;
    result_vel += (result_acc + 0.5 * dt_ap2 * descended_jerk) * dt_ap2 /
                  max_acceleration;
    result_acc += dt_ap2 * descended_jerk;
...
void sap_Ndof_compute_interpolated_values(
  ...
  sap_Ndof_compute_interpolated_values_incremental_logged_version(
      ...
      result_acc, result_desc_jerk, log_output);

  log_output << "The calculation of the minimum delta-time has finished and "
                "got the following:\n";
};
...
static double sap_Ndof_compute_min_delta_time_logged_version(
    double start_position, double end_position, double start_velocity,
    double end_velocity, double& peak_velocity, double max_velocity,
    double max_acceleration, std::ostream& log_output) {
  using std::abs;
  using std::sqrt;
  ...
  if (start_position > end_position)
    sign_p1_p0 = -1.0;
  double sa_v0_v1_2 =
      sign_p1_p0 * max_acceleration * (start_velocity + end_velocity) * 0.5;
  double vm_p1_p0 = max_velocity * abs(end_position - start_position);
  double vsqr_avg =
      0.5 * (start_velocity * start_velocity + end_velocity * end_velocity);
  ...
  log_output
      ...
      << "\n"
      << " sa_v0_v1_2 = sign_p1_p0 * max_acceleration * (start_velocity + "
         "end_velocity) * 0.5 = "
      << sa_v0_v1_2 << "\n"
      << " vm_p1_p0 = max_velocity * abs(end_position - start_position) = "
      << vm_p1_p0 << "\n"
      << " vsqr_avg = 0.5 * (start_velocity * start_velocity + end_velocity * "
         "end_velocity) = "
      << vsqr_avg << std::endl;

  // try to assume that a0 and a1 are a_max
  // giving:
  //  p1 - p0 == (abs(vp - v0) + a_max) (vp + v0) / 2*v_max
  //           + vp/v_max (dt - abs(vp - v0) - abs(vp - v1) - 2*a_max)
  //           + (abs(vp - v1) + a_max) (vp + v1) / 2*v_max
  ...
  double delta_first_order =
      end_position - start_position - dp_ramp1 - dp_ramp2;

  log_output << "-----------------------------------------------------------\n"
             << " peak_velocity = sign_p1_p0 * max_velocity = " << peak_velocity
             << "\n"
             << " delta_first_order = pe - ps - dp_ramp1 - dp_ramp2 = "
             << delta_first_order << "\n"
             << " 'if(delta_first_order * peak_velocity > 0.0)' with "
                "delta_first_order * peak_velocity = "
             << (delta_first_order * peak_velocity) << std::endl;

  if (delta_first_order * peak_velocity > 0.0) {
    log_output << "  condition passed!"
               << "\n"
               << "  min-dt = "
               << (abs(delta_first_order) + dt_ramp1 + dt_ramp2) << std::endl;
    // this means that we guessed correctly (we can reach max cruise speed in the direction of the end-point):
    return abs(delta_first_order) + dt_ramp1 + dt_ramp2;
  };
  ...
  for (double cur_vp = sign_p1_p0 * max_velocity;
       cur_vp * sign_p1_p0 > -1.05 * max_velocity;
       cur_vp -= 0.1 * sign_p1_p0 * max_velocity) {
    double cur_dt_ramp1 = 0.0, cur_dp_ramp1 = 0.0, cur_dt_ramp2 = 0.0,
           cur_dp_ramp2 = 0.0;

    sap_Ndof_compute_ramp_dist_and_time(start_velocity, cur_vp, max_velocity,
                                        max_acceleration, cur_dp_ramp1,
                                        cur_dt_ramp1);
    sap_Ndof_compute_ramp_dist_and_time(cur_vp, end_velocity, max_velocity,
                                        max_acceleration, cur_dp_ramp2,
                                        cur_dt_ramp2);

    log_output << "  vp = " << cur_vp
               << " dt = " << (cur_dt_ramp1 + cur_dt_ramp2) << " gives dp = "
               << sign_p1_p0 * (end_position - start_position - cur_dp_ramp1 -
                                cur_dp_ramp2)
               << std::endl;
  };
  ...
  //             + (abs(vp - v1) + a_max) (vp + v1) / 2*v_max
  //  or
  //    p1 - p0 == sqrt(abs(vp - v0) * a_max) (vp + v0) / v_max
  //             + (abs(vp - v1) + a_max) (vp + v1) / 2*v_max
  //  or
  //    p1 - p0 == (abs(vp - v0) + a_max) (vp + v0) / 2*v_max
  //             + sqrt(abs(vp - v1) * a_max) (vp + v1) / v_max
  //  or
  //    p1 - p0 == sqrt(abs(vp - v0) * a_max) (vp + v0) / v_max
  //             + sqrt(abs(vp - v1) * a_max) (vp + v1) / v_max

  //   first try if vp is more in the direction (p1-p0) than both v1 and v0:
  double descrim = max_acceleration * max_acceleration -
                   4.0 * (sa_v0_v1_2 - vm_p1_p0 - vsqr_avg);

  log_output
      << "-----------------------------------------------------------\n"
      << " descrim = am * am - 4.0 * (sa_v0_v1_2 - vm_p1_p0 - vsqr_avg) = "
      << descrim << "\n"
  ...
  if (descrim > 0.0) {
    ...
    log_output
        << "  condition passed!"
        << "\n"
        << "  peak_velocity = (-sign_p1_p0 * am + sqrt(descrim)) * 0.5 = "
        << peak_velocity << "\n"
        << "  'if((abs(vp) <= vm) && (sign_p1_p0 * vp >= sign_p1_p0 * vs) && "
           "(sign_p1_p0 * vp >= sign_p1_p0 * ve))'"
        << std::endl;

    if ((abs(peak_velocity) <= max_velocity) &&
        (sign_p1_p0 * peak_velocity >= sign_p1_p0 * start_velocity) &&
        (sign_p1_p0 * peak_velocity >= sign_p1_p0 * end_velocity)) {
      log_output << "   condition passed!"
                 << "\n"
                 << "   min-dt = "
                 << (abs(peak_velocity - start_velocity) +
                     abs(peak_velocity - end_velocity) + 2.0 * max_acceleration)
                 << std::endl;
      // this means that this root works for this case.
      return abs(peak_velocity - start_velocity) +
             abs(peak_velocity - end_velocity) + 2.0 * max_acceleration;
    };
    peak_velocity = (-sign_p1_p0 * max_acceleration - sqrt(descrim)) * 0.5;
  ...
  //   else, try if vp is less in the direction (p1-p0) than both v0 and v1 (because in-between is impossible for
  //   min-dt):
  descrim = max_acceleration * max_acceleration -
            4.0 * (vm_p1_p0 - sa_v0_v1_2 - vsqr_avg);

  log_output
      << "-----------------------------------------------------------\n"
      << " descrim = am * am - 4.0 * (vm_p1_p0 - sa_v0_v1_2 - vsqr_avg) = "
      << descrim << "\n"
      << " 'if(descrim > 0.0)'" << std::endl;
  ...
  if (descrim > 0.0) {
    ...
    log_output << "  condition passed!"
               ...
               << "  peak_velocity = (sign_p1_p0 * am + sqrt(descrim)) * 0.5 = "
               << peak_velocity << "\n"
               << "  'if((abs(vp) <= vm) && (sign_p1_p0 * vp <= sign_p1_p0 * "
                  "vs) && (sign_p1_p0 * vp <= sign_p1_p0 * ve))'"
               << std::endl;
    ...
    log_output << "  peak_velocity = (sign_p1_p0 * am - sqrt(descrim)) * 0.5 = "
               << peak_velocity << "\n"
               << "  'if((abs(vp) <= vm) && (sign_p1_p0 * vp <= sign_p1_p0 * "
                  "vs) && (sign_p1_p0 * vp <= sign_p1_p0 * ve))'"
               << std::endl;
  ...
  // try to assume that abs(a0) = abs(vp - v0) and abs(a1) = a_max.
  // giving:
  //  p1 - p0 == abs(vp - v0) (vp + v0) / v_max
  //           + vp/v_max (dt - 2*abs(vp - v0) - abs(vp - v1) - a_max)
  //           + (abs(vp - v1) + a_max) (vp + v1) / 2*v_max
  ...
  delta_first_order =
      ...
      0.5 * (abs(peak_velocity - end_velocity) + max_acceleration) *
          (peak_velocity + end_velocity) / max_velocity -
      abs(peak_velocity - start_velocity) * (peak_velocity + start_velocity) /
          max_velocity;

  log_output << "-----------------------------------------------------------\n"
             << " peak_velocity = sign_p1_p0 * max_velocity = " << peak_velocity
             << "\n"
             << " delta_first_order = pe - ps - 0.5 * (abs(vp - ve) + am) * "
                "(vp + ve) / vm - abs(vp - vs) * (vp + vs) / vm = "
             << delta_first_order << "\n"
             << " 'if(delta_first_order * peak_velocity > 0.0)' with "
                "delta_first_order * peak_velocity = "
             << (delta_first_order * peak_velocity) << std::endl;
  ...
  if (delta_first_order * peak_velocity > 0.0) {
    log_output << "  condition passed!"
               << "\n"
               << "  min-dt = "
               << (abs(delta_first_order) + abs(peak_velocity - end_velocity) +
                   2.0 * abs(peak_velocity - start_velocity) + max_acceleration)
               << std::endl;
    // this means that we guessed correctly (we can reach max cruise speed in the direction of the end-point):
    return abs(delta_first_order) + abs(peak_velocity - end_velocity) +
           2.0 * abs(peak_velocity - start_velocity) + max_acceleration;
  };

  //   if not, then can try to see if we simply can't quite reach max velocity before having to ramp-down:
  //   this assumes that we have
  //  p1 - p0 == abs(vp - v0) (vp + v0) / v_max + (abs(vp - v1) + a_max) (vp + v1) / 2*v_max

  //   first try if vp is more in the direction (p1-p0) than both v1 and v0:
  descrim =
      max_acceleration * max_acceleration / 9.0 -
      4.0 / 3.0 *
          (sign_p1_p0 * max_acceleration * end_velocity - 2.0 * vm_p1_p0 -
           2.0 * start_velocity * start_velocity - end_velocity * end_velocity);

  log_output << "-----------------------------------------------------------\n"
             << " descrim = am * am / 9.0 - 4.0 / 3.0 * (sign_p1_p0 * am * ve "
                "- 2.0 * vm_p1_p0 - 2.0 * vs * vs - ve * ve) = "
             << descrim << "\n"
             << " 'if(descrim > 0.0)'" << std::endl;
  ...
  if (descrim > 0.0) {
    // this means there exists a quadratic root for vp:
    peak_velocity =
        (-sign_p1_p0 * max_acceleration / 3.0 + sqrt(descrim)) * 0.5;

    log_output
        << "  condition passed!"
        << "\n"
        << "  peak_velocity = (-sign_p1_p0 * am / 3.0 + sqrt(descrim)) * 0.5 = "
        << peak_velocity << "\n"
        << "  'if((abs(vp) <= vm) && (sign_p1_p0 * vp >= sign_p1_p0 * vs) && "
           "(sign_p1_p0 * vp >= sign_p1_p0 * ve))'"
        << std::endl;
    ...
    if ((abs(peak_velocity) <= max_velocity) &&
      ...
      log_output << "   condition passed!"
                 << "\n"
                 << "   min-dt = "
                 << (abs(peak_velocity - end_velocity) +
                     2.0 * abs(peak_velocity - start_velocity) +
                     max_acceleration)
                 << std::endl;
      // this means that this root works for this case.
      return abs(peak_velocity - end_velocity) +
             2.0 * abs(peak_velocity - start_velocity) + max_acceleration;
    };
    peak_velocity =
        (-sign_p1_p0 * max_acceleration / 3.0 - sqrt(descrim)) * 0.5;
  ...
  //   else, try if vp is less in the direction (p1-p0) than both v0 and v1 (because in-between is impossible for
  //   min-dt):
  descrim = max_acceleration * max_acceleration / 9.0 -
            4.0 / 3.0 *
                (2.0 * vm_p1_p0 + 2.0 * start_velocity * start_velocity +
                 end_velocity * end_velocity -
                 sign_p1_p0 * max_acceleration * end_velocity);

  log_output << "-----------------------------------------------------------\n"
             << " descrim = am * am / 9.0 - 4.0 / 3.0 * (2.0 * vm_p1_p0 + 2.0 "
                "* vs * vs + ve * ve - sign_p1_p0 * am * ve) = "
             << descrim << "\n"
             << " 'if(descrim > 0.0)'" << std::endl;
  ...
  // try to assume that abs(a1) = abs(vp - v1) and abs(a0) = a_max.
  // giving:
  //  p1 - p0 == (abs(vp - v0) + a_max) (vp + v0) / 2*v_max
  //           + vp/v_max (dt - abs(vp - v0) - 2*abs(vp - v1) - a_max)
  //           + abs(vp - v1) (vp + v1) / v_max

  //   try to assume that vp = sign(p1-p0) * v_max
  peak_velocity = sign_p1_p0 * max_velocity;
  delta_first_order =
      end_position - start_position -
      abs(peak_velocity - end_velocity) * (peak_velocity + end_velocity) /
          max_velocity -
      0.5 * (abs(peak_velocity - start_velocity) + max_acceleration) *
          (peak_velocity + start_velocity) / max_velocity;

  log_output << "-----------------------------------------------------------\n"
             << " peak_velocity = sign_p1_p0 * max_velocity = " << peak_velocity
             << "\n"
             << " delta_first_order = pe - ps - abs(vp - ve) * (vp + ve) / vm "
                "- 0.5 * (abs(vp - vs) + am) * (vp + vs) / vm = "
             << delta_first_order << "\n"
             << " 'if(delta_first_order * peak_velocity > 0.0)' with "
                "delta_first_order * peak_velocity = "
             << (delta_first_order * peak_velocity) << std::endl;
  ...
  if (delta_first_order * peak_velocity > 0.0) {
    log_output << "  condition passed!"
               << "\n"
               << "  min-dt = "
               << (abs(delta_first_order) +
                   2.0 * abs(peak_velocity - end_velocity) +
                   abs(peak_velocity - start_velocity) + max_acceleration)
               << std::endl;
    // this means that we guessed correctly (we can reach max cruise speed in the direction of the end-point):
    return abs(delta_first_order) + 2.0 * abs(peak_velocity - end_velocity) +
           abs(peak_velocity - start_velocity) + max_acceleration;
  };

  //   if not, then can try to see if we simply can't quite reach max velocity before having to ramp-down:
  //   this assumes that we have
  //  p1 - p0 == abs(vp - v1) (vp + v1) / v_max + (abs(vp - v0) + a_max) (vp + v0) / 2*v_max

  //   first try if vp is more in the direction (p1-p0) than both v1 and v0:
  descrim =
      max_acceleration * max_acceleration / 9.0 -
      4.0 / 3.0 *
          (sign_p1_p0 * max_acceleration * start_velocity - 2.0 * vm_p1_p0 -
           start_velocity * start_velocity - 2.0 * end_velocity * end_velocity);

  log_output << "-----------------------------------------------------------\n"
             << " descrim = am * am / 9.0 - 4.0 / 3.0 * (sign_p1_p0 * am * vs "
                "- 2.0 * vm_p1_p0 - vs * vs - 2.0 * ve * ve) = "
             << descrim << "\n"
             << " 'if(descrim > 0.0)'" << std::endl;
  ...
  if (descrim > 0.0) {
    ...
    if ((abs(peak_velocity) <= max_velocity) &&
      ...
      log_output << "   condition passed!"
                 ...
                 << std::endl;
      // this means that this root works for this case.
      return 2.0 * abs(peak_velocity - end_velocity) +
             abs(peak_velocity - start_velocity) + max_acceleration;
    };
    peak_velocity =
        (-sign_p1_p0 * max_acceleration / 3.0 - sqrt(descrim)) * 0.5;
  ...
  descrim = max_acceleration * max_acceleration / 9.0 -
            4.0 / 3.0 *
                (2.0 * vm_p1_p0 + start_velocity * start_velocity +
                 2.0 * end_velocity * end_velocity -
                 sign_p1_p0 * max_acceleration * start_velocity);

  log_output << "-----------------------------------------------------------\n"
             << " descrim = am * am / 9.0 - 4.0 / 3.0 * ( 2.0 * vm_p1_p0 + vs "
                "* vs + 2.0 * ve * ve - sign_p1_p0 * am * vs) = "
             << descrim << "\n"
             << " 'if(descrim > 0.0)'" << std::endl;
  ...
  // try to assume that abs(a0) = abs(vp - v0) and abs(a1) = abs(vp - v1).
  // giving:
  //  p1 - p0 == abs(vp - v0) (vp + v0) / v_max
  //           + vp/v_max (dt - 2*abs(vp - v0) - 2*abs(vp - v1))
  //           + abs(vp - v1) (vp + v1) / v_max
  ...
  //   if not, then can try to see if we simply can't quite reach max velocity before having to ramp-down:
  //   this assumes that we have
  //  p1 - p0 == abs(vp - v0) (vp + v0) / v_max + abs(vp - v1) (vp + v1) / v_max
  ...
  log_output << "-----------------------------------------------------------\n"
             << " peak_velocity = sqrt(0.5 * vm_p1_p0 + vsqr_avg) = "
             << peak_velocity << "\n"
             << " 'if((abs(vp) <= vm) && (vp >= sign_p1_p0 * vs) && (vp >= "
                "sign_p1_p0 * ve))'"
             << std::endl;

  if ((abs(peak_velocity) <= max_velocity) &&
      (peak_velocity >= sign_p1_p0 * start_velocity) &&
      (peak_velocity >= sign_p1_p0 * end_velocity)) {
    // this means that this root works for this case.
  ...
  if (descrim > 0.0) {
    ...
    log_output << "  'if((abs(vp) <= vm) && (-vp <= sign_p1_p0 * vs) && (-vp "
                  "<= sign_p1_p0 * ve))'"
               << std::endl;
  ...
  log_output << "-----------------------------------------------------------\n"
             << "Reached the end of the calculation and did not find a "
                "suitable root! This should never happen!"
             << std::endl;
...
double sap_Ndof_compute_min_delta_time(
  ...
  double min_delta_time = sap_Ndof_compute_min_delta_time_logged_version(
      start_position, end_position, start_velocity, end_velocity, peak_velocity,
      max_velocity, max_acceleration, log_output);
  ...
  sap_Ndof_compute_interpolated_values_incremental_logged_version(
      start_position, end_position, start_velocity, end_velocity, peak_velocity,
      max_velocity, max_acceleration, min_delta_time, min_delta_time,
...
void sap_Ndof_compute_peak_velocity(double start_position, double end_position,
  ...
  if ((abs(start_velocity) > max_velocity) ||
    ...
    peak_velocity = 0.0;
    RK_NOTICE(1,
              " Warning: violation of the velocity bounds was detected on SAP "
              "interpolations!");
    return;
  };
  ...
  double v0_v1_ts_sa = start_velocity + end_velocity +
                       (delta_time - max_acceleration) * sign_p1_p0;
  double vm_p1_p0 = max_velocity * abs(end_position - start_position);
  ...
  if (descrim > 0.0) {
    ...
    if ((abs(peak_velocity) <= max_velocity) &&
        (sign_p1_p0 * peak_velocity >= start_velocity * sign_p1_p0) &&
        (sign_p1_p0 * peak_velocity >= end_velocity * sign_p1_p0)) {
      return;
    };
    peak_velocity = 0.5 * (v0_v1_ts_sa - sqrt(descrim));
    if ((abs(peak_velocity) <= max_velocity) &&
  ...
  if (end_velocity > start_velocity) {
    ...
    vsqr_avg =
        0.5 * (start_velocity * start_velocity - end_velocity * end_velocity);
    if (abs(v0_v1_ts_sa) > 1e-6 * max_velocity) {
    ...
    vm_p1_p0 = max_velocity * (end_position - start_position);
    vsqr_avg =
        0.5 * (end_velocity * end_velocity - start_velocity * start_velocity);
    if (abs(v0_v1_ts_sa) > 1e-6 * max_velocity) {
      peak_velocity = (vm_p1_p0 + vsqr_avg - a_v0_v1_2) / v0_v1_ts_sa;
      if ((abs(peak_velocity) <= max_velocity) &&
  ...
  v0_v1_ts_sa = start_velocity + end_velocity +
                (max_acceleration - delta_time) * sign_p1_p0;
  vm_p1_p0 = max_velocity * abs(end_position - start_position);
  vsqr_avg =
      0.5 * (start_velocity * start_velocity + end_velocity * end_velocity);
  ...
  if (descrim > 0.0) {
    ...
    peak_velocity = 0.5 * (v0_v1_ts_sa + sqrt(descrim));
    if ((abs(peak_velocity) <= max_velocity) &&
        (peak_velocity * sign_p1_p0 <= start_velocity * sign_p1_p0) &&
        (peak_velocity * sign_p1_p0 <= end_velocity * sign_p1_p0)) {
      return;
    };
    peak_velocity = 0.5 * (v0_v1_ts_sa - sqrt(descrim));
    if ((abs(peak_velocity) <= max_velocity) &&
  ...
  // try to assume that vp more in the direction (p1-p0) than both v1 and v0 (i.e., ramp-up and ramp-down).
  v0_v1_ts_sa = (2.0 * start_velocity + end_velocity +
                 (delta_time - 0.5 * max_acceleration) * sign_p1_p0) *
                2.0 / 3.0;
  vm_p1_p0 = max_velocity * abs(end_position - start_position) * 2.0 / 3.0;
  ...
  if (end_velocity > start_velocity) {
    // ramp-up ramp-up
    v0_v1_ts_sa = 2.0 * (2.0 * start_velocity - end_velocity + delta_time -
    ...
    vsqr_avg =
        2.0 * start_velocity * start_velocity - end_velocity * end_velocity;
    descrim =
        v0_v1_ts_sa * v0_v1_ts_sa - 4.0 * (vsqr_avg - a_v0_v1_2 + vm_p1_p0);
    if (descrim > 0.0) {
      peak_velocity = (v0_v1_ts_sa + sqrt(descrim)) * 0.5;
      if ((abs(peak_velocity) <= max_velocity) &&
          (peak_velocity >= start_velocity) &&
          (peak_velocity <= end_velocity)) {
        return;
      };
      peak_velocity = 0.5 * (v0_v1_ts_sa - sqrt(descrim));
      if ((abs(peak_velocity) <= max_velocity) &&
  ...
  // try to assume that vp more in the direction (p1-p0) than both v1 and v0 (i.e., ramp-up and ramp-down).
  v0_v1_ts_sa = (2.0 * end_velocity + start_velocity +
                 (delta_time - 0.5 * max_acceleration) * sign_p1_p0) *
                2.0 / 3.0;
  vm_p1_p0 = max_velocity * abs(end_position - start_position) * 2.0 / 3.0;
  a_v0_v1_2 = max_acceleration * start_velocity * sign_p1_p0 / 3.0;
  vsqr_avg =
      (2.0 * end_velocity * end_velocity + start_velocity * start_velocity) /
      3.0;
  descrim = v0_v1_ts_sa * v0_v1_ts_sa - 4.0 * (vsqr_avg - a_v0_v1_2 + vm_p1_p0);
  if (descrim > 0.0) {
    // then there is a real root to the quadratic equation.
  ...
  if (end_velocity < start_velocity) {
    // ramp-down ramp-down
    v0_v1_ts_sa = 2.0 * (2.0 * end_velocity - start_velocity + delta_time -
                         0.5 * max_acceleration);
    vm_p1_p0 = 2.0 * max_velocity * (end_position - start_position);
  ...
  vm_p1_p0 = 0.5 * max_velocity * abs(end_position - start_position);
  vsqr_avg =
      0.5 * (start_velocity * start_velocity + end_velocity * end_velocity);
  descrim = v0_v1_ts_sa * v0_v1_ts_sa - 4.0 * (vsqr_avg + vm_p1_p0);
  if (descrim > 0.0) {
...
static void sap_Ndof_compute_peak_velocity_numsolve(
  ...
  for (double cur_vp = prev_vp - 0.02 * sign_p1_p0 * max_velocity;
       cur_vp * sign_p1_p0 > -1.04 * max_velocity;
       cur_vp -= 0.02 * sign_p1_p0 * max_velocity) {
    ...
    if (cur_pd * prev_pd < 0.0) {
      ...
      if ((pd_calc.get_delta_time_diff(cur_vp) >= -1e-3 * max_velocity) &&
          (abs(cur_pd) < 1e-3 * max_velocity)) {
