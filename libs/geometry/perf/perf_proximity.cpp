...
struct proxy_query_generator {
  ...
  static pose_3D< double > random_pose() {
    ...
    rel_pose.Quat = quaternion< double >( vect< double, 4 >( ud( get_global_rng() ), ud( get_global_rng() ),
  ...
  proxy_query_generator()
      : query( new geom::proxy_query_model_3D() ), anchor( new pose_3D< double >( random_pose() ) ){};
  ...
  void addOneRandomShape() const {
    ...
    if( chosen_shape < 0.2 ) {
      // create plane:
      query->addShape( shared_ptr< geom::plane >(
      ...
      query->addShape( shared_ptr< geom::box >( new geom::box(
        "box", anchor, rel_pose, vect< double, 3 >( ud( get_global_rng() ) * 2.0, ud( get_global_rng() ) * 2.0,
                                                    ud( get_global_rng() ) * 2.0 ) ) ) );
    } else if( chosen_shape < 0.6 ) {
      // create sphere:
      query->addShape(
        shared_ptr< geom::sphere >( new geom::sphere( "sphere", anchor, rel_pose, ud( get_global_rng() ) * 2.0 ) ) );
    } else if( chosen_shape < 0.8 ) {
      // create cylinder:
      query->addShape( shared_ptr< geom::cylinder >( new geom::cylinder(
        "cylinder", anchor, rel_pose, ud( get_global_rng() ) * 2.0, ud( get_global_rng() ) * 1.0 ) ) );
    } else {
      // create capped_cylinder:
      query->addShape( shared_ptr< geom::capped_cylinder >( new geom::capped_cylinder(
        "capped_cylinder", anchor, rel_pose, ud( get_global_rng() ) * 2.0, ud( get_global_rng() ) * 1.0 ) ) );
    };
  };
};
