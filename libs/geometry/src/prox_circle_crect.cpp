...
proximity_record_2D compute_proximity( const circle& aCircle, const shape_2D_precompute_pack& aPack1,
  ...
  const bool in_x_range = ( ( ci_c_rel[0] > -0.5 * cr_dim[0] ) && ( ci_c_rel[0] < 0.5 * cr_dim[0] ) );
...
proximity_record_2D prox_circle_crect::computeProximity( const shape_2D_precompute_pack& aPack1,
...
prox_circle_crect::prox_circle_crect( const circle* aCircle, const capped_rectangle* aCRect )
    : proximity_finder_2D(), mCircle( aCircle ), mCRect( aCRect ){};
};
};
