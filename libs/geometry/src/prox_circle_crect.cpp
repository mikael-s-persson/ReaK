...
proximity_record_2D compute_proximity(const circle& aCircle,
  ...
  const bool in_x_range =
      ((ci_c_rel[0] > -0.5 * cr_dim[0]) && (ci_c_rel[0] < 0.5 * cr_dim[0]));

  if (in_x_range) {
    if (ci_c_rel[1] > 0.0) {
      result.mPoint1 = re_pose.transformToGlobal(
          vect<double, 2>(ci_c_rel[0], ci_c_rel[1] - ci_rad));
      result.mPoint2 = re_pose.transformToGlobal(
          vect<double, 2>(ci_c_rel[0], 0.5 * cr_dim[1]));
  ...
  result.mPoint1 =
      re_pose.transformToGlobal(ci_c_rel - (ci_rad / diff_d_rel) * diff_v_rel);
  result.mPoint2 = re_pose.transformToGlobal(
      re_endc + (0.5 * cr_dim[1] / diff_d_rel) * diff_v_rel);
...
proximity_record_2D compute_proximity(const capped_rectangle& aCRect,
  ...
  using std::swap;
  proximity_record_2D result =
      compute_proximity(aCircle, aPack2, aCRect, aPack1);  // NOLINT
  swap(result.mPoint1, result.mPoint2);
  return result;
}

proximity_record_2D prox_circle_crect::computeProximity(
    const shape_2D_precompute_pack& aPack1,
    const shape_2D_precompute_pack& aPack2) {
  if ((mCircle == nullptr) || (mCRect == nullptr)) {
    return {};
  }

  if (aPack1.parent == mCircle) {
    return compute_proximity(*mCircle, aPack1, *mCRect, aPack2);
  }
  return compute_proximity(*mCRect, aPack1, *mCircle, aPack2);
}

prox_circle_crect::prox_circle_crect(const circle* aCircle,
                                     const capped_rectangle* aCRect)
    : mCircle(aCircle), mCRect(aCRect) {}
