...
proximity_record_3D compute_proximity( const capped_cylinder& aCCylinder1, const shape_3D_precompute_pack& aPack1,
  ...
  if( sqrt( cy2_t_rel[0] * cy2_t_rel[0] + cy2_t_rel[1] * cy2_t_rel[1] ) < 1e-5 ) {
    ...
    if( ( cy2_c_rel[2] + 0.5 * cy2_len > -0.5 * cy1_len ) || ( cy2_c_rel[2] - 0.5 * cy2_len < 0.5 * cy1_len ) ) {
      // there is an overlap between the capped-cylinder sides.
      const double max_z_rel
        = ( ( cy2_c_rel[2] + 0.5 * cy2_len < 0.5 * cy1_len ) ? ( cy2_c_rel[2] + 0.5 * cy2_len ) : ( 0.5 * cy1_len ) );
      ...
      result.mPoint1
        = c1_pose.transformToGlobal( vect< double, 3 >( cy1_rad * cy2_r_rel[0], cy1_rad * cy2_r_rel[1], avg_z_rel ) );
      result.mPoint2 = c1_pose.transformToGlobal(
        vect< double, 3 >( cy2_c_rel[0] - cy2_rad * cy2_r_rel[0], cy2_c_rel[1] - cy2_rad * cy2_r_rel[1], avg_z_rel ) );
...
proximity_record_3D prox_ccylinder_ccylinder::computeProximity( const shape_3D_precompute_pack& aPack1,
...
prox_ccylinder_ccylinder::prox_ccylinder_ccylinder( const capped_cylinder* aCCylinder1,
    ...
    : proximity_finder_3D(), mCCylinder1( aCCylinder1 ), mCCylinder2( aCCylinder2 ){};
};
};
