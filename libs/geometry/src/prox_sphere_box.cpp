...
proximity_record_3D compute_proximity( const sphere& aSphere, const shape_3D_precompute_pack& aPack1, const box& aBox,
  ...
  proximity_record_3D bxpt_result = findProximityBoxToPoint( aBox, bx_pose, sp_c );
...
proximity_record_3D compute_proximity( const box& aBox, const shape_3D_precompute_pack& aPack1, const sphere& aSphere,
                                       const shape_3D_precompute_pack& aPack2 ) {
  using std::swap;
  proximity_record_3D result = compute_proximity( aSphere, aPack2, aBox, aPack1 );
...
prox_sphere_box::prox_sphere_box( const sphere* aSphere, const box* aBox )
    : proximity_finder_3D(), mSphere( aSphere ), mBox( aBox ){};
};
};
