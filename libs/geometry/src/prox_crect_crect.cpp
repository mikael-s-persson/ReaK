  ...
  if( fabs( cr2_t_rel[1] ) < 1e-5 ) {
    ...
    if( ( cr2_c_rel[0] + 0.5 * cr2_dim[0] > -0.5 * cr1_dim[0] )
      ...
      const double max_x_rel
        = ( ( cr2_c_rel[0] + 0.5 * cr2_dim[0] < 0.5 * cr1_dim[0] ) ? ( cr2_c_rel[0] + 0.5 * cr2_dim[0] )
                                                                   : ( 0.5 * cr1_dim[0] ) );
      ...
      result.mPoint2
        = cr1_pose.transformToGlobal( vect< double, 2 >( avg_x_rel, cr2_c_rel[1] - 0.5 * cr2_dim[1] * cr2_r_rel[1] ) );
...
prox_crect_crect::prox_crect_crect( const capped_rectangle* aCRect1, const capped_rectangle* aCRect2 )
    : proximity_finder_2D(), mCRect1( aCRect1 ), mCRect2( aCRect2 ){};
};
};
