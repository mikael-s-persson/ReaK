...
void compute_proximity_of_point( const plane& aPlane, const pose_3D< double >& aPlGblPose,
  ...
  if( ( pt_rel[0] > -0.5 * aPlane.getDimensions()[0] ) && ( pt_rel[0] < 0.5 * aPlane.getDimensions()[0] )
      && ( pt_rel[1] > -0.5 * aPlane.getDimensions()[1] ) && ( pt_rel[1] < 0.5 * aPlane.getDimensions()[1] ) ) {
    ...
    if( ( pt_rel[0] > -0.5 * aPlane.getDimensions()[0] ) && ( pt_rel[0] < 0.5 * aPlane.getDimensions()[0] ) ) {
    ...
    } else if( ( pt_rel[1] > -0.5 * aPlane.getDimensions()[1] ) && ( pt_rel[1] < 0.5 * aPlane.getDimensions()[1] ) ) {
      ...
      vect< double, 3 > rim_pt
        = vect< double, 3 >( 0.5 * aPlane.getDimensions()[0], 0.5 * aPlane.getDimensions()[1], 0.0 );
      if( pt_rel[0] < 0.0 )
        rim_pt[0] = -rim_pt[0];
      if( pt_rel[1] < 0.0 )
        rim_pt[1] = -rim_pt[1];
...
proximity_record_3D compute_proximity( const plane& aPlane1, const shape_3D_precompute_pack& aPack1,
  ...
  vect< double, 3 > corner = vect< double, 3 >( 0.5 * p2_dim[0], 0.5 * p2_dim[1], 0.0 );
  ...
  corner = vect< double, 3 >( 0.5 * p1_dim[0], 0.5 * p1_dim[1], 0.0 );
...
prox_plane_plane::prox_plane_plane( const plane* aPlane1, const plane* aPlane2 )
    : proximity_finder_3D(), mPlane1( aPlane1 ), mPlane2( aPlane2 ){};
};
};
