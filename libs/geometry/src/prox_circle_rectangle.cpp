...
proximity_record_2D compute_proximity( const circle& aCircle, const shape_2D_precompute_pack& aPack1,
  ...
  bool in_x_range = ( ( ci_c_rel[0] > -0.5 * re_dim[0] ) && ( ci_c_rel[0] < 0.5 * re_dim[0] ) );
  bool in_y_range = ( ( ci_c_rel[1] > -0.5 * re_dim[1] ) && ( ci_c_rel[1] < 0.5 * re_dim[1] ) );

  if( in_x_range && in_y_range ) {
    // The circle is inside the rectangle.
    const vect< double, 2 > bound_dists( 0.5 * re_dim[0] - fabs( ci_c_rel[0] ), 0.5 * re_dim[1] - fabs( ci_c_rel[1] ) );
...
proximity_record_2D compute_proximity( const rectangle& aRectangle, const shape_2D_precompute_pack& aPack1,
  ...
  using std::swap;
  proximity_record_2D result = compute_proximity( aCircle, aPack2, aRectangle, aPack1 );
  swap( result.mPoint1, result.mPoint2 );
  return result;
};

proximity_record_2D prox_circle_rectangle::computeProximity( const shape_2D_precompute_pack& aPack1,
...
prox_circle_rectangle::prox_circle_rectangle( const circle* aCircle, const rectangle* aRectangle )
    : proximity_finder_2D(), mCircle( aCircle ), mRectangle( aRectangle ){};
};
};
