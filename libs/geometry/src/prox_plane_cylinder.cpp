...
proximity_record_3D compute_proximity(const plane& aPlane, 
  ...
  if(fabs(cy_t_rel[2]) < 1e-6) {
    ...
    result.mPoint1 = pl_pose.transformToGlobal(vect<double,3>(cy_c_rel[0],cy_c_rel[1],0.0));
    result.mPoint2 = pl_pose.transformToGlobal(vect<double,3>(cy_c_rel[0],cy_c_rel[1],cy_c_rel[2] - 0.5 * cy_len));
...
void prox_plane_cylinder::computeProximity(const shape_3D_precompute_pack& aPack1, 
  ...
  if( aPack1.parent == mPlane ) 
    mLastResult = compute_proximity(*mPlane,aPack1,*mCylinder,aPack2);
  else
    mLastResult = compute_proximity(*mCylinder,aPack1,*mPlane,aPack2);
