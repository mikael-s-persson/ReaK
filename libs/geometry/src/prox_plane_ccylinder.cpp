...
proximity_record_3D compute_proximity(const plane& aPlane,
  ...
  if (abs(cy_t_rel[2]) < 1e-6) {
    ...
    vect<double, 3> cypt_rel = cy_c_rel + (0.5 * cy_len) * cy_t_rel +
                               vect<double, 3>(0.0, 0.0, -cy_rad);
    result.mPoint1 = pl_pose.transformToGlobal(
...
proximity_record_3D compute_proximity(const capped_cylinder& aCCylinder,
  ...
  using std::swap;
  proximity_record_3D result =
      compute_proximity(aPlane, aPack2, aCCylinder, aPack1);  // NOLINT
  swap(result.mPoint1, result.mPoint2);
  return result;
}

proximity_record_3D prox_plane_ccylinder::computeProximity(
    const shape_3D_precompute_pack& aPack1,
    const shape_3D_precompute_pack& aPack2) {
  if ((mCCylinder == nullptr) || (mPlane == nullptr)) {
    return {};
  }
...
prox_plane_ccylinder::prox_plane_ccylinder(const plane* aPlane,
                                           const capped_cylinder* aCCylinder)
    : mPlane(aPlane), mCCylinder(aCCylinder) {}
