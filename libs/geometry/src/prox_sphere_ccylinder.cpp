...
proximity_record_3D compute_proximity(const sphere& aSphere,
  ...
  if (abs(sp_c_rel[2]) <= 0.5 * cy_len) {
    ...
    vect<double, 3> cy_c2 = cy_pose.transformToGlobal(
...
proximity_record_3D compute_proximity(const capped_cylinder& aCCylinder,
  ...
  using std::swap;
  proximity_record_3D result =
      compute_proximity(aSphere, aPack2, aCCylinder, aPack1);  // NOLINT
  swap(result.mPoint1, result.mPoint2);
  return result;
}

proximity_record_3D prox_sphere_ccylinder::computeProximity(
    const shape_3D_precompute_pack& aPack1,
    const shape_3D_precompute_pack& aPack2) {
  if ((mSphere == nullptr) || (mCCylinder == nullptr)) {
    return {};
  }
...
prox_sphere_ccylinder::prox_sphere_ccylinder(const sphere* aSphere,
                                             const capped_cylinder* aCCylinder)
    : mSphere(aSphere), mCCylinder(aCCylinder) {}
