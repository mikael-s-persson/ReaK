...
proximity_record_3D compute_proximity( const sphere& aSphere, const shape_3D_precompute_pack& aPack1,
  ...
  if( fabs( sp_c_rel[2] ) <= 0.5 * cy_len ) {
    ...
    result.mPoint1
      = cy_pose.transformToGlobal( vect< double, 3 >( sp_c_rel[0], sp_c_rel[1], sp_c_rel[2] - fact * sp_rad ) );
...
prox_sphere_cylinder::prox_sphere_cylinder( const sphere* aSphere, const cylinder* aCylinder )
    : proximity_finder_3D(), mSphere( aSphere ), mCylinder( aCylinder ){};
};
};
