...
proximity_record_2D compute_proximity_of_line(const rectangle& aRectangle, 
  ...
  if(fabs(ln_t_rel[0]) < 1e-5) {
    ...
    if((ln_c_rel[1] + half_length > -0.5 * re_dim[1]) || 
      ...
      double max_y_rel = ((ln_c_rel[1] + half_length <  0.5 * re_dim[1]) ? (ln_c_rel[1] + half_length) : ( 0.5 * re_dim[1]));
      double min_y_rel = ((ln_c_rel[1] - half_length > -0.5 * re_dim[1]) ? (ln_c_rel[1] - half_length) : (-0.5 * re_dim[1]));
  ...
  if(fabs(ln_t_rel[1]) < 1e-5) {
    ...
    if((ln_c_rel[0] + half_length > -0.5 * re_dim[0]) || 
      ...
      // there is an overlap between the rectangle side and the line.
      double max_x_rel = ((ln_c_rel[0] + half_length <  0.5 * re_dim[0]) ? (ln_c_rel[0] + half_length) : ( 0.5 * re_dim[0]));
      double min_x_rel = ((ln_c_rel[0] - half_length > -0.5 * re_dim[0]) ? (ln_c_rel[0] - half_length) : (-0.5 * re_dim[0]));
