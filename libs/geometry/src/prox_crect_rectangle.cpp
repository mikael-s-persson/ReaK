...
proximity_record_2D compute_proximity_of_line( const rectangle& aRectangle, const pose_2D< double >& aGblPose,
  ...
  if( fabs( ln_t_rel[0] ) < 1e-5 ) {
    ...
    if( ( ln_c_rel[1] + half_length > -0.5 * re_dim[1] ) || ( ln_c_rel[1] - half_length < 0.5 * re_dim[1] ) ) {
      // there is an overlap between the rectangle side and the line.
      double max_y_rel
        = ( ( ln_c_rel[1] + half_length < 0.5 * re_dim[1] ) ? ( ln_c_rel[1] + half_length ) : ( 0.5 * re_dim[1] ) );
      double min_y_rel
        = ( ( ln_c_rel[1] - half_length > -0.5 * re_dim[1] ) ? ( ln_c_rel[1] - half_length ) : ( -0.5 * re_dim[1] ) );
  ...
  if( fabs( ln_t_rel[1] ) < 1e-5 ) {
    ...
    if( ( ln_c_rel[0] + half_length > -0.5 * re_dim[0] ) || ( ln_c_rel[0] - half_length < 0.5 * re_dim[0] ) ) {
      // there is an overlap between the rectangle side and the line.
      double max_x_rel
        = ( ( ln_c_rel[0] + half_length < 0.5 * re_dim[0] ) ? ( ln_c_rel[0] + half_length ) : ( 0.5 * re_dim[0] ) );
      double min_x_rel
        = ( ( ln_c_rel[0] - half_length > -0.5 * re_dim[0] ) ? ( ln_c_rel[0] - half_length ) : ( -0.5 * re_dim[0] ) );
  ...
  if( fabs( t_tmp ) > half_length ) {
    ...
    double in_x_range = fabs( ln_pt_rel[0] ) - 0.5 * re_dim[0];
    double in_y_range = fabs( ln_pt_rel[1] ) - 0.5 * re_dim[1];

    if( ( in_x_range < 0.0 ) && ( in_y_range > in_x_range ) ) {
      corner_pt[0] = ln_pt_rel[0];
      dist_tmp = fabs( ln_pt_rel[1] ) - 0.5 * re_dim[1];
    } else if( ( in_y_range < 0.0 ) && ( in_x_range > in_y_range ) ) {
      corner_pt[1] = ln_pt_rel[1];
      dist_tmp = fabs( ln_pt_rel[0] ) - 0.5 * re_dim[0];
    } else {
      if( ln_pt_rel[0] < 0.0 )
        corner_pt[0] = -0.5 * re_dim[0];
      else
    ...
    result.mPoint1 = aGblPose.transformToGlobal( corner_pt + dist_tmp * ln_n_rel );
    result.mPoint2 = aGblPose.transformToGlobal( corner_pt );
    result.mDistance = dist_tmp;
  };
...
proximity_record_2D prox_crect_rectangle::computeProximity( const shape_2D_precompute_pack& aPack1,
...
prox_crect_rectangle::prox_crect_rectangle( const capped_rectangle* aCRect, const rectangle* aRectangle )
    : proximity_finder_2D(), mCRect( aCRect ), mRectangle( aRectangle ){};
};
};
