...
void compute_proximity_of_point( const rectangle& aRectangle, const pose_2D< double >& aRecGblPose,
  ...
  bool in_x_range = ( ( pt_rel[0] > -0.5 * re_dim[0] ) && ( pt_rel[0] < 0.5 * re_dim[0] ) );
  bool in_y_range = ( ( pt_rel[1] > -0.5 * re_dim[1] ) && ( pt_rel[1] < 0.5 * re_dim[1] ) );

  if( in_x_range && in_y_range ) {
    // The circle is inside the rectangle.
    vect< double, 2 > bound_dists
      = vect< double, 2 >( 0.5 * re_dim[0] - fabs( pt_rel[0] ), 0.5 * re_dim[1] - fabs( pt_rel[1] ) );
...
proximity_record_2D compute_proximity( const rectangle& aRectangle1, const shape_2D_precompute_pack& aPack1,
  ...
  compute_proximity_of_point( aRectangle1, r1_pose, corner_gbl, temp_pt, temp_dist );
  ...
  compute_proximity_of_point( aRectangle2, r2_pose, corner_gbl, temp_pt, temp_dist );
  if( temp_dist < result.mDistance ) {
    result.mDistance = temp_dist;
...
proximity_record_2D prox_rectangle_rectangle::computeProximity( const shape_2D_precompute_pack& aPack1,
...
prox_rectangle_rectangle::prox_rectangle_rectangle( const rectangle* aRectangle1, const rectangle* aRectangle2 )
    : proximity_finder_2D(), mRectangle1( aRectangle1 ), mRectangle2( aRectangle2 ){};
};
};
