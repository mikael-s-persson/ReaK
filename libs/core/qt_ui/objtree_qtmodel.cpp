...
QVariant ObjTreeQtModel::data( const QModelIndex& index, int role ) const {
  ...
  if( index.column() == 0 ) {
    ...
    if( s_tmp.length() > 40 ) {
      ...
      return QVariant(
        QString( ( std::string( s_tmp.begin(), it ) + "<..>" + std::string( rit.base(), s_tmp.end() ) ).c_str() ) );
    } else {
      return QVariant( QString( s_tmp.c_str() ) );
    };
  };
...
int ObjTreeQtModel::columnCount( const QModelIndex& parent ) const { return 2; };

void ObjTreeQtModel::treeChanged() { refreshObjTree(); };
