...
ObjPropertiesQtModel::ObjPropertiesQtModel( const shared_ptr< serialization::object_graph >& aObjGraph,
                                            serialization::object_node_desc aCurrentNode )
    : obj_graph_editor( aObjGraph ), current_node( aCurrentNode ),
...
int ObjPropertiesQtModel::columnCount( const QModelIndex& ) const { return 2; };
...
QVariant ObjPropertiesQtModel::headerData( int section, Qt::Orientation orientation, int role ) const {
...
bool ObjPropertiesQtModel::setData( const QModelIndex& index, const QVariant& value, int role ) {
  ...
  } else if( ( p_type == serialization::primitive_scheme< unsigned char >::getStaticObjectType() )
             || ( p_type == serialization::primitive_scheme< unsigned int >::getStaticObjectType() )
             || ( p_type == serialization::primitive_scheme< long unsigned int >::getStaticObjectType() ) ) {
    convertWorked = tmp.convert( QVariant::UInt );
  } else if( ( p_type == serialization::primitive_scheme< float >::getStaticObjectType() )
             || ( p_type == serialization::primitive_scheme< double >::getStaticObjectType() ) ) {
    convertWorked = tmp.convert( QVariant::Double );
  } else if( p_type == serialization::primitive_scheme< char >::getStaticObjectType() ) {
    convertWorked = tmp.convert( QVariant::Char );
  } else if( p_type == serialization::primitive_scheme< bool >::getStaticObjectType() ) {
    convertWorked = tmp.convert( QVariant::Bool );
  } else if( p_type == serialization::primitive_scheme< std::string >::getStaticObjectType() ) {
  ...
  if( p_type == serialization::primitive_scheme< std::string >::getStaticObjectType() )
...
void ObjPropertiesQtModel::setDataNewObject( const QModelIndex& index, const shared_ptr< shared_object >& aNewPtr ) {
...
QWidget* ObjPropertiesQtDelegate::createEditor( QWidget* parent, const QStyleOptionViewItem& option,
                                                const QModelIndex& index ) const {
  if( ( !index.isValid() ) || ( index.column() != 1 ) )
    return QStyledItemDelegate::createEditor( parent, option, index );

  std::pair< std::string, shared_ptr< serialization::type_scheme > > fld
  ...
  if( p_type == serialization::primitive_scheme< bool >::getStaticObjectType() ) {
    ...
    return box;
  } else if( p_type == serialization::primitive_scheme< int >::getStaticObjectType() ) {
...
void ObjPropertiesQtDelegate::setEditorData( QWidget* editor, const QModelIndex& index ) const {
  if( ( !index.isValid() ) || ( index.column() != 1 ) )
  ...
  if( p_type == serialization::primitive_scheme< bool >::getStaticObjectType() ) {
    ...
    QComboBox* box = static_cast< QComboBox* >( editor );
    box->setCurrentIndex( box->findText( parentModel->data( index, Qt::DisplayRole ).toString() ) );
    return;
  };
...
void ObjPropertiesQtDelegate::setModelData( QWidget* editor, QAbstractItemModel* model,
                                            const QModelIndex& index ) const {
  if( ( !index.isValid() ) || ( index.column() != 1 ) )
  ...
  if( p_type == serialization::primitive_scheme< bool >::getStaticObjectType() ) {
    QComboBox* box = static_cast< QComboBox* >( editor );
    parentModel->setData( index, box->itemData( box->currentIndex() ), Qt::EditRole );
    return;
  } else if( p_type == serialization::primitive_scheme< int >::getStaticObjectType() ) {
    QSpinBox* box = static_cast< QSpinBox* >( editor );
    box->interpretText();
    ...
    if( value != "New object..." )
      ...
      while( !btype_stack.empty() ) {
        ...
        if( tmp_bt->isConcrete() ) {
          ...
          itm_ptr->setData( Qt::UserRole, QVariant( reinterpret_cast< quint64 >( tmp_bt->TypeID_begin() ) ) );
...
void ObjPropertiesQtDelegate::updateEditorGeometry( QWidget* editor, const QStyleOptionViewItem& option,
                                                    const QModelIndex& index ) const {
