...
iarchive& RK_CALL
  xml_iarchive::load_serializable_ptr( const std::pair< std::string, serializable_shared_pointer& >& Item ) {
  ...
  rtti::so_type* p = rtti::so_type_repo::getInstance().findType( &( typeID[0] ) );
...
xml_oarchive::~xml_oarchive() { ( *file_stream ) << "</reak_serialization>" << std::endl; };
...
oarchive& RK_CALL
  xml_oarchive::saveToNewArchive_impl( const serializable_shared_pointer& Item, const std::string& FileName ) {
  return xml_oarchive::saveToNewArchiveNamed_impl(
    std::pair< std::string, const serializable_shared_pointer& >( "Item", Item ), FileName );
};

oarchive& RK_CALL
  xml_oarchive::saveToNewArchiveNamed_impl( const std::pair< std::string, const serializable_shared_pointer& >& Item,
  ...
  if( Item.second ) {
    ...
    ( *file_stream ) << "0\" version=\"" << hdr.type_version << "\" object_ID=\"" << hdr.object_ID
                     << "\" is_external=\"true\">" << std::endl;
  } else {
    already_saved = true;

    addTabulations();
    ( *file_stream ) << "<" << Item.first << " type_ID=\"0\" version=\"0\" object_ID=\"0\" is_external=\"false\">"
                     << std::endl;
  };
...
oarchive& RK_CALL
  xml_oarchive::save_serializable_ptr( const std::pair< std::string, const serializable_shared_pointer& >& Item ) {
...
oarchive& RK_CALL xml_oarchive::save_char( const std::pair< std::string, char >& i ) {
  addTabulations();
  ( *file_stream ) << "<" << i.first << ">\"" << static_cast< int >( i.second ) << "\"</" << i.first << ">"
                   << std::endl;
  return *this;
};
...
oarchive& RK_CALL xml_oarchive::save_unsigned_char( const std::pair< std::string, unsigned char >& u ) {
  addTabulations();
  ( *file_stream ) << "<" << u.first << ">\"" << static_cast< unsigned int >( u.second ) << "\"</" << u.first << ">"
                   << std::endl;
  return *this;
};
...
oarchive& RK_CALL xml_oarchive::save_bool( const std::pair< std::string, bool >& b ) {
  addTabulations();
  ( *file_stream ) << "<" << b.first << ">\"" << ( b.second ? "true" : "false" ) << "\"</" << b.first << ">"
                   << std::endl;
  return *this;
};
