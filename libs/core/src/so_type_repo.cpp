...
so_type_repo& getRKSharedObjTypeRepo() { return so_type_repo::getInstance(); };
...
void so_type_repo::merge( so_type_repo* aRepo ) {
  ...
  if( isInRing( aRepo ) )
    return;
  if( next == this ) { // this means that 'this' is a unique instance (not really a ring). 'this' can simply be linked
                       // into aRepo.
    prev = aRepo->prev;
    aRepo->prev = this;
    next = aRepo;
    prev->next = this;
  } else if( aRepo->next == aRepo ) { // this means that aRepo is a unique instance (not really a ring). aRepo can
...
so_type* so_type_repo::findType( so_type* aTypeID ) const { return findType( aTypeID->TypeID_begin() ); };
