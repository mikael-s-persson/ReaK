...
#define RK_RPC_LOG( X )                                                                                     \
  {                                                                                                         \
    ...
    std::cout << "ln: " << std::setw( 5 ) << __LINE__ << " thd: " << std::hex << std::this_thread::get_id() \
              << std::dec << " time: " << std::setw( 4 )                                                    \
              << ( std::chrono::duration_cast< std::chrono::seconds >( t ).count() % 1000 ) << "."          \
              << std::setfill( '0' ) << std::setw( 6 )                                                      \
              << ( std::chrono::duration_cast< std::chrono::microseconds >( t ).count() % 1000000 )         \
...
static volatile bool server_is_alive alignas( void* ) = true;


static std::unique_ptr< serialization::iarchive > make_iarchiver( std::istream& in, msg_format fmt = xml_format ) {
  std::unique_ptr< serialization::iarchive > p_ar;
  switch( fmt ) {
    case binary_format:
      p_ar.reset( new serialization::bin_iarchive( in ) );
      break;
    case xml_format:
    default:
      p_ar.reset( new serialization::xml_iarchive( in ) );
      break;
    case protobuf_format:
      p_ar.reset( new serialization::protobuf_iarchive( in ) );
      break;
  };
  return p_ar;
};

static std::unique_ptr< serialization::oarchive > make_oarchiver( std::ostream& out, msg_format fmt = xml_format ) {
  std::unique_ptr< serialization::oarchive > p_ar;
  switch( fmt ) {
    case binary_format:
      p_ar.reset( new serialization::bin_oarchive( out ) );
      break;
    case xml_format:
    default:
      p_ar.reset( new serialization::xml_oarchive( out ) );
      break;
    case protobuf_format:
      p_ar.reset( new serialization::protobuf_oarchive( out ) );
      break;
  };
  return p_ar;
};
...
struct sort_published_functions {
  bool operator()( detail::remote_function* lhs, detail::remote_function* rhs ) const {
    return ( ( lhs->name < rhs->name )
             || ( ( lhs->name == rhs->name ) && ( lhs->get_params_hash() < rhs->get_params_hash() ) ) );
  };
};
typedef std::set< detail::remote_function*, sort_published_functions > pub_func_set;

struct server_config_data {
  std::string name;
  boost::asio::ip::tcp::endpoint endpoint;

  explicit server_config_data( const std::string& aName = "" ) : name( aName ), endpoint(){};

  bool operator<( const server_config_data& rhs ) const { return this->name < rhs.name; };
};
typedef std::set< server_config_data > server_set;
...
typedef std::map< boost::asio::ip::tcp::endpoint, server_set::iterator, compare_server_endpoints > server_endpt_map;
...
struct pending_return {
  ...
  pending_return( boost::asio::io_service& io_service, detail::remote_function* aPFunc,
  ...
  void handle_connect( const boost::system::error_code& e ) {
    ...
    if( !e ) {
      ...
      temp_out << detail::generate_rpc_header( detail::rpc_call, pre_data.fmt, pre_data_dump.size() );
      ...
      RK_RPC_LOG( "An error occurred when given a connection to handle for seq: " << pre_data.call_seq
                                                                                  << " With message: " << e.message() );
      socket.async_connect( endpoint, [this]( const boost::system::error_code& e ) { this->handle_connect( e ); } );
    };
  };

  void async_connect() {
    RK_RPC_LOG( "Triggering an async connection for the call to " << endpoint.address().to_string() << " "
                                                                  << endpoint.port() );
    socket.async_connect( endpoint, [this]( const boost::system::error_code& e ) { this->handle_connect( e ); } );
  };
};
...
struct server::impl {
  ...
  std::map< unsigned int, pending_return > pending_calls;
  ...
  struct call_executer {
    ...
    call_executer( impl* aParent, msg_format aFmt, const std::string& aPayload,
                   boost::asio::ip::tcp::endpoint aSenderEndPt )
        : parent( aParent ), fmt( aFmt ), ss( new std::stringstream( aPayload ) ), sender_endpt( aSenderEndPt ){};
    ...
    void operator()() {
      ...
      if( it == parent->pub_funcs.end() ) {
        ...
        detail::rpc_unrecognized_header reply_hdr( r_hdr.name, r_hdr.params_hash, r_hdr.call_seq );
        ( *p_aro ) << reply_hdr;

        // Produce the return packet.
        boost::asio::streambuf out_buf;
        std::ostream out( &out_buf );
        auto res_data = temp_out.str(); // TODO Is there a way to avoid double copy?
        out << detail::generate_rpc_header( detail::rpc_unrecognized, fmt, res_data.size() );
        out << res_data;

        RK_RPC_LOG( "Connecting to sender at " << sender_endpt.address().to_string() << ":" << r_hdr.reply_port );
      ...
      try {
        ...
        std::stringstream temp_out;
        auto p_aro = make_oarchiver( temp_out, fmt );
        detail::rpc_return_header reply_hdr( r_hdr.name, r_hdr.params_hash, r_hdr.call_seq );
        ( *p_aro ) << reply_hdr;
        ...
        std::ostream out( &out_buf );
        auto res_data = temp_out.str(); // TODO Is there a way to avoid double copy?
        out << detail::generate_rpc_header( detail::rpc_return, fmt, res_data.size() );
        out << res_data;
        ...
        std::stringstream temp_out;
        auto p_aro = make_oarchiver( temp_out, fmt );
        detail::rpc_exception_header reply_hdr( r_hdr.name, r_hdr.params_hash, r_hdr.call_seq,
        ...
        std::ostream out( &out_buf );
        auto res_data = temp_out.str(); // TODO Is there a way to avoid double copy?
        out << detail::generate_rpc_header( detail::rpc_exception, fmt, res_data.size() );
        out << res_data;
  ...
  struct connection {
    ...
    connection( impl* aParent ) : parent( aParent ), socket_stream(){};

    bool check_call_seq_match( std::map< unsigned int, pending_return >::iterator it,
                               const detail::rpc_basic_header& r_hdr ) {
      if( ( it->second.p_func->name != r_hdr.name ) || ( it->second.p_func->get_params_hash() != r_hdr.params_hash ) ) {
        std::stringstream ss;
        ss << "The remote call " << r_hdr.call_seq << " returned a mismatch of function! Expected '"
           << it->second.p_func->name << ":" << it->second.p_func->get_params_hash() << "' but got '" << r_hdr.name
           << ":" << r_hdr.params_hash << "'!" << std::flush;
        it->second.out_promise.set_exception( std::make_exception_ptr( communication_error( ss.str() ) ) );
    ...
    void operator()( std::shared_ptr< connection > /* to keep self alive */, const boost::system::error_code& e ) {

      parent->start_accept();

      {
        if( e.value() != 0 ) {
          RK_RPC_LOG( "Error on tcp socket: '" << e.message() << "' with error-code value = " << e.value() );
        } else {
      ...
      while( socket_stream ) {
        try {
          ...
          std::tie( hdr_type, fmt, msg_size ) = detail::parse_rpc_header( socket_stream );
          RK_RPC_LOG( "Received RPC packet: " << detail::rpc_header_type_to_str[hdr_type] << " of size " << msg_size );
          ...
          switch( hdr_type ) {
            case detail::rpc_call: {
              ...
              auto f = std::async( std::launch::async, call_executer( parent, fmt, payload, endpoint ) );
            ...
            case detail::rpc_return: {
              ...
              std::string payload( msg_size, '\0' ); // TODO Is there a way to avoid double copy?
              socket_stream.read( &payload[0], msg_size );
              res.ss.reset( new std::stringstream( payload ) );
            ...
            case detail::rpc_exception: {
              ...
              auto e_p = std::make_exception_ptr( std::runtime_error( "RPC Server reported an exception of type '"
                                                                      + r_hdr.except_type + "' with message: '"
                                                                      + r_hdr.except_msg + "'." ) );
            ...
            case detail::rpc_unrecognized: {
              ...
              it->second.out_promise.set_exception(
                std::make_exception_ptr( unrecognized_function( it->second.p_func ) ) );
              parent->pending_calls.erase( it );
              break;
            };
  ...
  public:
    discovery_handler( impl* aParent )
        : parent( aParent ), udp_socket( parent->io_service ),
          multicast_endpt( boost::asio::ip::address::from_string( "239.255.17.1" ), 17001 ),
          self_is_port_server( false ), self_config( gen_random_name() ), self_config_confirmed( false ) {

      // Temporarily set local endpoint to whatever:
      self_config.endpoint = boost::asio::ip::tcp::endpoint( boost::asio::ip::tcp::v4(), 17000 );

      // Create a socket for receiving udp multicasting messages from discovery protocol.
      boost::asio::ip::udp::endpoint listen_endpoint( boost::asio::ip::udp::v4(), 17001 );
      ...
      udp_socket.set_option( boost::asio::ip::multicast::join_group( multicast_endpt.address() ) );

      udp_socket.async_receive_from( buf.prepare( 1024 ), sender_endpt,
                                     boost::bind( &discovery_handler::handle_receive_from, this,
                                                  boost::asio::placeholders::error,
                                                  boost::asio::placeholders::bytes_transferred ) );

      startup_executer
        = std::async( std::launch::async, boost::bind( &discovery_handler::startup_procedure, this, self_config.name,
    ...
    void finalize() {
      ...
      if( self_config.endpoint.port() > 17000 ) {
        ...
        if( self_is_port_server ) {
          ...
          boost::system::error_code asio_err = boost::asio::error::host_not_found;
          // mark self as NOT PSv
          self_is_port_server = false;
          // select any active server
          for( auto& sv : all_servers ) { // DO
            ...
            try {
              ...
              boost::asio::streambuf temp_out_buf;
              std::ostream temp_out( &temp_out_buf );
              temp_out << detail::generate_rpc_header( detail::rpc_startportserver, parent->preferred_fmt, 0 );
    ...
    void rebind_acceptor() {
      ...
      if( self_config.endpoint.port() > 17000 ) {
        parent->acceptor = boost::asio::ip::tcp::acceptor(
          parent->io_service,
          boost::asio::ip::tcp::endpoint( boost::asio::ip::tcp::v4(), self_config.endpoint.port() ) );
      };
    };
    ...
    std::future< detail::call_results > make_remote_call( pending_return&& pre_connection ) {
      bool got_ep = false;
      {
        ...
        if( it != all_servers.end() ) {
          RK_RPC_LOG( "Found the host! at address: " << it->endpoint.address().to_string() << " "
                                                     << it->endpoint.port() );
          pre_connection.endpoint = it->endpoint;
          if( pre_connection.endpoint.address() == self_config.endpoint.address() )
            pre_connection.endpoint.address( boost::asio::ip::address::from_string( "127.0.0.1" ) );
          got_ep = true;
        };
      };
      if( got_ep ) {
        ...
        auto it = parent->pending_calls.insert( std::make_pair( sq, std::move( pre_connection ) ) ).first;
        it->second.async_connect();
        RK_RPC_LOG( "Triggered the async connection for the pending call, returning future.." );
        ...
        auto it = parent->pending_connects.insert( std::make_pair( pf->get_host(), std::move( pre_connection ) ) );
        RK_RPC_LOG( "Added pre-connection object to the connection queue, returning future.." );
        return it->second.out_promise.get_future();
      };
    };

  private:
    void startup_procedure( const std::string& aSelfName, const std::string& aSelfAddr ) {
      try {
        ...
        out << detail::generate_disc_header( detail::disc_newport, parent->preferred_fmt, res_data.size() );
        out << res_data;
        ...
        if( obtained_port.wait_for( std::chrono::seconds( 1 ) ) != std::future_status::ready ) {
    ...
    void delayed_startup_procedure( const std::string& aSelfName, const std::string& aSelfAddr ) {
    ...
    void advertise_new_port() {
      ...
      std::stringstream temp_out;
      auto p_aro = make_oarchiver( temp_out, parent->preferred_fmt );
      detail::disc_with_port_header wp_hdr( self_config.name, self_config.endpoint.address().to_string(),
      ...
      std::ostream out( &out_buf );
      std::string res_data = temp_out.str();
      out << detail::generate_disc_header( detail::disc_advertise, parent->preferred_fmt, res_data.size() );
      out << res_data;
    ...
    void echo_self_port() {
      ...
      std::ostream out( &out_buf );
      std::string res_data = temp_out.str();
      out << detail::generate_disc_header( detail::disc_echo, parent->preferred_fmt, res_data.size() );
      out << res_data;
    ...
    void unadvertise_bad_port() {
      ...
      std::stringstream temp_out;
      auto p_aro = make_oarchiver( temp_out, parent->preferred_fmt );
      detail::disc_basic_header np_hdr( self_config.name, self_config.endpoint.address().to_string() );
      ( *p_aro ) << np_hdr;
      ...
      std::ostream out( &out_buf );
      std::string res_data = temp_out.str();
      out << detail::generate_disc_header( detail::disc_unadvertise, parent->preferred_fmt, res_data.size() );
      out << res_data;
    ...
    void assign_addr_port_advertised( const detail::disc_with_port_header& r_hdr ) {
      server_config_data tmp_sv_data( r_hdr.name );
      tmp_sv_data.endpoint.address( boost::asio::ip::address::from_string( r_hdr.addr ) );
      ...
      if( tmp_sv_data.endpoint.address() == self_config.endpoint.address() )
        tmp_sv_data.endpoint.address( boost::asio::ip::address::from_string( "127.0.0.1" ) );
      ...
      if( rg.first != rg.second ) {
        ...
        while( rg.first != rg.second ) {
          ...
          auto it2 = parent->pending_calls.insert( std::make_pair( sq, std::move( rg.first->second ) ) ).first;
    ...
    void handle_receive_from( const boost::system::error_code& e, std::size_t bytes_recvd ) {
      if( !e && ( bytes_recvd != 0 ) ) {
        try {
          ...
          switch( hdr_type ) {
            case detail::disc_newport: {
              ...
              ( *p_ari ) >> r_hdr;
              RK_RPC_LOG( "Got NEWPORT from " << r_hdr.name << " with addr " << r_hdr.addr );
              ...
              if( r_hdr.name == self_config.name ) {
                if( sender_endpt.address() != self_config.endpoint.address() ) {
                  ...
                  RK_RPC_LOG( "Self-recognized address as " << sender_endpt.address() );
                  std::unique_lock< std::mutex > _( self_config_mtx );
                ...
                if( self_config.endpoint.port() == 17000 ) {
                  ...
                  auto ep_lo
                    = assigned_endpts.lower_bound( boost::asio::ip::tcp::endpoint( sender_endpt.address(), 17002 ) );
                  auto ep_up
                    = assigned_endpts.upper_bound( boost::asio::ip::tcp::endpoint( sender_endpt.address(), 17999 ) );
                  while( ep_lo != ep_up ) {
                ...
                RK_RPC_LOG( "Assigned the port " << new_port << " for the server" );

                // send GIVEPORT "N" to source of request
                std::stringstream temp_out;
                auto p_aro = make_oarchiver( temp_out, parent->preferred_fmt );
                detail::disc_with_port_header wp_hdr( r_hdr.name, sender_endpt.address().to_string(), new_port );
                ( *p_aro ) << wp_hdr;
                ...
                std::ostream out( &out_buf );
                std::string res_data = temp_out.str();
                out << detail::generate_disc_header( detail::disc_giveport, parent->preferred_fmt, res_data.size() );
                out << res_data;
                ...
                if( must_advertise_self ) {
                  ...
                  RK_RPC_LOG( "Advertising the self-assigned new port " << self_config.endpoint.port() );
                  // broadcast ADVERTISE
                  advertise_new_port();
                ...
                try {
                  ...
                  startup_executer = std::async(
                    std::launch::async, boost::bind( &discovery_handler::delayed_startup_procedure, this,
            ...
            case detail::disc_giveport: {
              ...
              ( *p_ari ) >> r_hdr;
              RK_RPC_LOG( "Got GIVEPORT from " << r_hdr.name << " with addr " << r_hdr.addr << ":" << r_hdr.port );
              ...
              self_config.endpoint.address( boost::asio::ip::address::from_string( r_hdr.addr ) );
              given_addr_port.set_value( true );
            ...
            case detail::disc_advertise: {
              detail::disc_with_port_header r_hdr;
              ( *p_ari ) >> r_hdr;
              RK_RPC_LOG( "Got ADVERTISE from " << r_hdr.name << " with addr " << r_hdr.addr << ":" << r_hdr.port );

              if( ( r_hdr.name == self_config.name ) && ( sender_endpt.address() != self_config.endpoint.address() ) ) {
                RK_RPC_LOG( "Self-recognized address as " << sender_endpt.address() );
              ...
              if( ( r_hdr.name != self_config.name ) && ( self_config.endpoint.port() > 17001 ) ) {
            ...
            case detail::disc_unadvertise: {
              detail::disc_basic_header r_hdr;
              ( *p_ari ) >> r_hdr;
              RK_RPC_LOG( "Got UNADVERTISE from " << r_hdr.name << " with addr " << r_hdr.addr );
            ...
            case detail::disc_echo: {
              detail::disc_with_port_header r_hdr;
              ( *p_ari ) >> r_hdr;
              RK_RPC_LOG( "Got ECHO from " << r_hdr.name << " with addr " << r_hdr.addr << ":" << r_hdr.port );
              ...
              if( self_config.endpoint == boost::asio::ip::tcp::endpoint(
                                            boost::asio::ip::address::from_string( r_hdr.addr ), r_hdr.port ) ) {
                RK_RPC_LOG( "Found conflict with self addr-port... trying to take a lock on self-config" );
                std::unique_lock< std::mutex > _( self_config_mtx );
                // broadcast UNADVERTISE
                unadvertise_bad_port();
                ...
                try {
                  ...
                  given_addr_port = std::promise< bool >();
                  startup_executer = std::async(
                    std::launch::async, boost::bind( &discovery_handler::startup_procedure, this, self_config.name,
                                                     self_config.endpoint.address().to_string() ) );
                };
              }; // END
              break;
  ...
  detail::call_preparations prepare_call( const detail::remote_function* aPFunc ) {
    ...
    result.ss.reset( new std::stringstream() );
    result.p_aro = make_oarchiver( *result.ss, result.fmt );
    result.call_seq = cur_seq_number++;
    detail::rpc_call_header call_hdr( aPFunc->name, aPFunc->get_params_hash(), result.call_seq,
                                      discovery_hdl.get_self_port() );
    ( *result.p_aro ) << call_hdr;
    return result;
  };

  std::future< detail::call_results > make_remote_call( detail::remote_function* aPFunc,
                                                        detail::call_preparations&& pre_data ) {
    RK_RPC_LOG( "Initiating remote call with seq number: " << pre_data.call_seq );
    return discovery_hdl.make_remote_call( pending_return( io_service, aPFunc, std::move( pre_data ) ) );
  };
  ...
  void start_accept() {
    ...
    std::shared_ptr< connection > new_conn( new connection( this ) );
    acceptor.async_accept( *( new_conn->socket_stream.rdbuf() ), new_conn->endpoint,
                           [new_conn]( const boost::system::error_code& e ) { ( *new_conn )( new_conn, e ); } );
  };

  void set_self_name( const std::string& aName ) { discovery_hdl.set_self_name( aName ); };

  impl() : listening_thd(), io_service(), acceptor( io_service ), cur_seq_number( 1 ), discovery_hdl( this ) {
...
static bool are_same_remotes( detail::remote_function* lhs, detail::remote_function* rhs ) {
  return ( ( lhs->name == rhs->name ) && ( lhs->get_params_hash() == rhs->get_params_hash() ) );
};
...
void server::set_name( const std::string& aName ) { instance().pimpl->set_self_name( aName ); };
...
detail::call_preparations server::prepare_call( detail::remote_function* aPFunc ) const {
  assert( server_is_alive ); // NOTE: this will only occur during static destruction, can't throw here.
  return pimpl->prepare_call( aPFunc );
};

std::future< detail::call_results > server::make_remote_call( detail::remote_function* aPFunc,
