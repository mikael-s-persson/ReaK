...
#define RK_RPC_LOG(X) \
  ...
  std::cout << "ln: " << std::setw(5) << __LINE__ \
            << " thd: " << std::hex << std::this_thread::get_id() << std::dec \
            << " time: " << std::setw(4) \
  ...
  struct pending_return {
    ...
    void handle_connect(const boost::system::error_code& e) {
      ...
      if(!e) {
        ...
        RK_RPC_LOG("An error occurred when given a connection to handle for seq: " << pre_data.call_seq << " With message: " << e.message());
    ...
    void async_connect() {
      RK_RPC_LOG("Triggering an async connection for the call to " << endpoint.address().to_string() << " " << endpoint.port());
      socket.async_connect(endpoint, 
        [this](const boost::system::error_code& e) {
          this->handle_connect(e);
        });
...
struct server::impl {
  ...
  struct call_executer {
    ...
    call_executer(impl* aParent, msg_format aFmt, const std::string& aPayload,
                  ...
                  parent(aParent), fmt(aFmt), 
                  ss(new std::stringstream(aPayload)),
                  sender_endpt(aSenderEndPt) {};
    ...
    bool check_call_seq_match(std::map<unsigned int, pending_return>::iterator it, 
      ...
      if((it->second.p_func->name != r_hdr.name) || (it->second.p_func->get_params_hash() != r_hdr.params_hash)) {
        ...
        ss << "The remote call " << r_hdr.call_seq << " returned a mismatch of function! Expected '" 
            << it->second.p_func->name << ":" << it->second.p_func->get_params_hash() 
            << "' but got '" << r_hdr.name << ":" << r_hdr.params_hash << "'!" << std::flush;
          ...
          switch(hdr_type) {
            case detail::rpc_call: {
              ...
              auto f = std::async(std::launch::async, 
                                  call_executer(parent, fmt, payload, endpoint));
            ...
            case detail::rpc_exception: {
              ...
              auto e_p = std::make_exception_ptr(std::runtime_error("RPC Server reported an exception of type '" + r_hdr.except_type + "' with message: '" + r_hdr.except_msg + "'."));
  ...
  public:
    discovery_handler(impl* aParent) : 
      ...
      startup_executer = std::async(std::launch::async, 
        boost::bind(&discovery_handler::startup_procedure, this, 
    ...
    void finalize() {
      ...
      try { given_addr_port.set_value(true); } catch(...) {};
    ...
    void rebind_acceptor() {
      ...
      if( self_config.endpoint.port() > 17000 ) {
        parent->acceptor = boost::asio::ip::tcp::acceptor(parent->io_service,
    ...
    std::future< detail::call_results > make_remote_call(pending_return&& pre_connection) {
      bool got_ep = false;
      {
        ...
        if( it != all_servers.end() ) {
          RK_RPC_LOG("Found the host! at address: " << it->endpoint.address().to_string() << " " << it->endpoint.port());
  ...
  private:
    ...
    void advertise_new_port() {
      ...
      std::stringstream temp_out;
      auto p_aro = make_oarchiver(temp_out, parent->preferred_fmt);
      detail::disc_with_port_header wp_hdr(self_config.name, 
    ...
    void unadvertise_bad_port() {
      ...
      std::stringstream temp_out;
      auto p_aro = make_oarchiver(temp_out, parent->preferred_fmt);
      detail::disc_basic_header np_hdr(self_config.name, 
                                       self_config.endpoint.address().to_string());
      (*p_aro) << np_hdr;
              ...
              if( r_hdr.name == self_config.name ) {
                ...
                if( self_config.endpoint.port() == 17000 ) {
                  ...
                  auto ep_lo = assigned_endpts.lower_bound(
                    boost::asio::ip::tcp::endpoint(sender_endpt.address(), 17002));
                  auto ep_up = assigned_endpts.upper_bound(
                    boost::asio::ip::tcp::endpoint(sender_endpt.address(), 17999));
                  while( ep_lo != ep_up ) {
  ...
  detail::call_preparations prepare_call(const detail::remote_function* aPFunc) {
    ...
    detail::rpc_call_header call_hdr(aPFunc->name, aPFunc->get_params_hash(), 
                                     result.call_seq, discovery_hdl.get_self_port());
...
std::future< detail::call_results > server::make_remote_call(detail::remote_function* aPFunc, detail::call_preparations&& pre_data) const {
  assert(server_is_alive); // NOTE: this will only occur during static destruction, can't throw here.
