...
std::string data_stream_options::get_URI() const {
  ...
  if( ( dot != std::string::npos ) && ( result.size() - dot == 4 )
      && ( ( result.substr( dot, 4 ) == ".bin" ) || ( result.substr( dot, 4 ) == ".ssv" )
           || ( result.substr( dot, 4 ) == ".tsv" ) || ( result.substr( dot, 4 ) == ".csv" ) ) ) {
    result.erase( dot, 4 );
  };
  switch( kind ) {
    case binary:
  ...
  if( file_name != "stdout" ) {
    ...
    if( ( d != std::string::npos ) || ( t != std::string::npos ) ) {
      ...
      if( d < t ) {
        ...
        new_fname.append( cdate_as_str );
        if( t != std::string::npos ) {
          new_fname.append( file_name.begin() + d + 2, file_name.begin() + t );
          new_fname.append( ctime_as_str );
...
void data_stream_options::load_all_configs( const std::string& aFileName ) {
  ...
  ( *p_ia ) & RK_SERIAL_LOAD_WITH_NAME( stream_kind ) & RK_SERIAL_LOAD_WITH_NAME( file_name )
    & RK_SERIAL_LOAD_WITH_NAME( names ) & RK_SERIAL_LOAD_WITH_NAME( time_sync_name )
    & RK_SERIAL_LOAD_WITH_NAME( flush_rate ) & RK_SERIAL_LOAD_WITH_NAME( buffer_size );
...
void data_stream_options::save_all_configs( const std::string& aFileName ) const {
  ...
  ( *p_oa ) & RK_SERIAL_SAVE_WITH_NAME( stream_kind ) & RK_SERIAL_SAVE_WITH_NAME( file_name )
    & RK_SERIAL_SAVE_WITH_NAME( names ) & RK_SERIAL_SAVE_WITH_NAME( time_sync_name )
    & RK_SERIAL_SAVE_WITH_NAME( flush_rate ) & RK_SERIAL_SAVE_WITH_NAME( buffer_size );
};
};
};
