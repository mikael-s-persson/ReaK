...
scheme_builder::scheme_builder() {
  field_stack.push( shared_ptr< serializable_obj_scheme >( new serializable_obj_scheme( "RootScheme", nullptr, 0 ) ) );
};
...
oarchive& RK_CALL
  scheme_builder::saveToNewArchive_impl( const serializable_shared_pointer& Item, const std::string& FileName ) {
  return scheme_builder::save_serializable_ptr(
    std::pair< std::string, const serializable_shared_pointer& >( "item", Item ) );
};

oarchive& RK_CALL
  scheme_builder::saveToNewArchiveNamed_impl( const std::pair< std::string, const serializable_shared_pointer& >& Item,
...
oarchive& RK_CALL
  scheme_builder::save_serializable_ptr( const std::pair< std::string, const serializable_shared_pointer& >& Item ) {
  if( !Item.second )
    return *this;
...
// See if we need to add the shared_ptr< Type > type-scheme for this object type.
  ...
  if( itm == scheme_map.end() ) {
...
#else
    std::map< std::string, shared_ptr< type_scheme > >::iterator objID_itm
      = scheme_map.find( rtti::get_type_id< unsigned int >::type_name() );
#endif
    ...
    shared_ptr< serializable_ptr_scheme > ser_sch_ptr
      = shared_ptr< serializable_ptr_scheme >( new serializable_ptr_scheme(
        so_type_ptr->TypeName(), so_type_ptr->TypeID_begin(), so_type_ptr->TypeVersion(), objID_sch ) );
  ...
  // type is registered).
  field_stack.push( shared_ptr< serializable_obj_scheme >( new serializable_obj_scheme( "DummyType", nullptr, 0 ) ) );
...
oarchive& RK_CALL scheme_builder::save_serializable( const std::pair< std::string, const serializable& >& Item ) {
  ...
  if( itm == scheme_map.end() ) {
    shared_ptr< serializable_obj_scheme > ser_sch_ptr = shared_ptr< serializable_obj_scheme >(
      new serializable_obj_scheme( so_type_ptr->TypeName(), so_type_ptr->TypeID_begin(), so_type_ptr->TypeVersion() ) );
...
#else
  std::map< std::string, shared_ptr< type_scheme > >::iterator itm
    = scheme_map.find( rtti::get_type_id< T >::type_name() );
#endif
  shared_ptr< type_scheme > sch_ptr;
  if( itm == scheme_map.end() ) {
...
void RK_CALL scheme_builder::signal_polymorphic_field( const std::string& aBaseTypeName, const unsigned int* aTypeID,
...
// check if the scheme already exists
  ...
  if( itm == scheme_map.end() ) {
    ...
    if( so_type_sptr ) {
      shared_ptr< serializable_ptr_scheme > ser_sch_ptr
        = shared_ptr< serializable_ptr_scheme >( new serializable_ptr_scheme(
          so_type_sptr->TypeName(), so_type_sptr->TypeID_begin(), so_type_sptr->TypeVersion(), objID_sch ) );
      scheme_map[ptr_type_name] = ser_sch_ptr;
      sch_ptr = ser_sch_ptr;

      // create a dummy type-scheme to accumulate the "fields" of all the descendants.
      field_stack.push(
        shared_ptr< serializable_obj_scheme >( new serializable_obj_scheme( "DummyType", nullptr, 0 ) ) );
      ...
      for( unsigned int i = 0; i < so_type_sptr->getDirectDescendantCount(); ++i ) {
        ...
        save_serializable_ptr(
          tmp->CreateObject() ); // if creation fails, the shared_ptr saver will not save it (abstract class).
      };
...
void RK_CALL scheme_builder::finish_repeated_field() {
  ...
  shared_ptr< type_scheme > sch_ptr = shared_ptr< type_scheme >(
    new vector_type_scheme( "std::vector<" + value_name_stack.top().second + ">", val_sch_ptr ) );
...
void RK_CALL scheme_builder::finish_repeated_pair() {
  ...
  // look for the current top of the value_name_stack to find the recorded type scheme.
  std::map< std::string, shared_ptr< type_scheme > >::iterator val_itm
    = scheme_map.find( value_name_stack.top().second );
  shared_ptr< type_scheme > val_sch_ptr;
  ...
  std::map< std::string, shared_ptr< type_scheme > >::iterator key_itm
    = scheme_map.find( value_name_stack.top().second );
  ...
  shared_ptr< type_scheme > sch_ptr = shared_ptr< type_scheme >( new map_type_scheme(
    "std::map<" + value_name_stack.top().second + "," + value_type_name + ">", key_sch_ptr, val_sch_ptr ) );
