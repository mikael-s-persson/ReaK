 ...
 */
boost::program_options::options_description get_data_stream_options_po_desc(
    bool aInput, bool aOutput = false) {
  ...
  if (aInput) {
    ...
    input_options.add_options()(
        "input", value<std::string>(),
        "specify the filename for the input data-stream")(
        "input-ip", value<std::string>(),
        ...
        "input-raw-udp",
        "if set, will try to listen to an input RAW UDP data-stream, for this "
        "to work, you must specify "
        ...
        "file-extension of input-file (ssv, tsv, csv, bin, "
        "etc.), or if piped, use 'ssv')")(
        "input-flush-freq", value<unsigned int>()->default_value(50),
        "specify the flushing frequency of the input datastream (default is "
        "50Hz, if "
        "set to 0, input will be immediate (as soon as available))")(
        "input-buffer-size", value<unsigned int>()->default_value(500),
        "specify the desired buffer size of the input datastream, without "
        "guarantee that the buffer will be limited to "
        "that amount (default is 500 bytes, if set to 0, input will not be "
        "buffered, i.e., read as you go)")(
        "input-immediate-mode",
        "if set, will make the input datastream without buffering or regular "
        "flushing, i.e., the "
        "operations are immediate (note: this option overrides the flush-freq "
        "and buffer-size "
        "options)");
  ...
  if (aOutput) {
    ...
    output_options.add_options()(
        ...
        "specify the filename for the output file for the "
        "datastream (default is to output to the console "
        "'stdout', for piping the output)")(
        "output-ip", value<std::string>(),
        "specify the IP-address of the RAW UDP data-stream, this is because a "
        "raw "
        "udp stream is connection-less and requires a pre-defined output "
        "IP-address")(
        "output-port", value<unsigned int>()->default_value(17017),
        "specify the IP-port for the data-server that will be created (default "
        "is 17017)")("output-tcp", "if set, will output a TCP data-stream")(
        ...
        "file-extension of input-file (ssv, tsv, csv, bin, "
        "etc.), or if piped, use 'ssv')")(
        "output-flush-freq", value<unsigned int>()->default_value(50),
        "specify the flushing frequency of the output datastream (default is "
        "50Hz, "
        "if set to 0, output will be immediate)")(
        "output-buffer-size", value<unsigned int>()->default_value(500),
        "specify the desired buffer size of the output datastream, without "
        "guarantee that the buffer will be limited to "
        "that amount (default is 500 bytes, if set to 0, output will not be "
        "buffered, i.e., sent as you go)")(
        "output-immediate-mode",
        "if set, will make the output datastream without buffering or regular "
        "flushing, i.e., "
        "the operations are immediate (note: this option overrides the "
        "flush-freq and "
        "buffer-size options)");
  ...
  filt_options.add_options()(
      "time-column-sync", value<std::string>(),
      "name of the time-column from the datastream to use as a timed-output, "
      "i.e., for a "
      "network streaming, this will deliver the rows at each correct time")(
      "keep-columns", value<std::string>(),
      "specify a semi-colon-separated list of the columns to output (keep) in "
      "the datastream");
 ...
 */
data_stream_options get_data_stream_options_from_po(
    boost::program_options::variables_map& vm, bool aForOutput = false) {
  ...
  if (!aForOutput) {
    ...
    if (vm.count("input-tcp") + vm.count("input-udp") +
            vm.count("input-raw-udp") >
        0) {
      std::stringstream ss;
      ss << vm["input-ip"].as<std::string>() << ":"
         << vm["input-port"].as<unsigned int>();
    ...
    if (vm.count("output-tcp") + vm.count("output-udp") +
            vm.count("output-raw-udp") ==
        0) {
      ...
      if (vm.count("output-raw-udp")) {
        ss << vm["output-ip"].as<std::string>() << ":"
           << vm["output-port"].as<unsigned int>();
        result.kind = data_stream_options::raw_udp_stream;
      } else {
  ...
  if (vm.count("keep-columns")) {
    ...
    for (std::string::iterator it = tmp_keep_columns.begin();
         it != tmp_keep_columns.end(); ++it) {
      std::string::iterator it_next =
          std::find(it, tmp_keep_columns.end(), ';');
