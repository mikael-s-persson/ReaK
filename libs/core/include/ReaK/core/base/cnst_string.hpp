...
struct ct_itoa {
  ...
  static BOOST_CONSTEXPR cnst_string<detail::gen_digit_seq<N>::count> text_impl(detail::digit_seq<S...>) {
    return cnst_string<detail::gen_digit_seq<N>::count>{std::array<char, detail::gen_digit_seq<N>::count >{detail::str_digits[S]...}};
  };
