...
void keyboard_press_hdl( void* userData, SoEventCallback* eventCB ) {
  ...
  if( IK_enabled ) {
    try {
      *( r_info->builder.getDependentFrame3D( 0 )->mFrame ) = *( r_info->target_frame );
...
int main( int argc, char** argv ) {
  ...
  r_info.target_frame = shared_ptr< frame_3D< double > >( new frame_3D< double >(
    weak_ptr< pose_3D< double > >(), vect< double, 3 >( 0.0, 5.0, 5.0 ), quaternion< double >(),
...

  {
...

    {
      ...
      sg << ( *r_info.kin_chain );
      sg << geom::coord_arrows_3D( "target_arrows", r_info.target_frame, pose_3D< double >(), 0.3 );
