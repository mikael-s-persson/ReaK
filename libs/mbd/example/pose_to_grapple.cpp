...
int main( int argc, char** argv ) {
  ...
  mdl_options.add_options()(
    "target-model", po::value< std::string >()->default_value( "models/airship3D.model.rkx" ),
    "specify the input file for the kinematic model of the target (default is 'models/airship3D.model.rkx')" );
  ...
  try {
    ...
    ( *serialization::open_iarchive( target_mdl_fname ) ) >> global_base >> airship3D_kin_model >> airship3D_grasp_frame
      >> geom_mdl >> proxy_mdl;
  } catch( std::exception& e ) {
    std::cerr << "An error occurred while trying to load the target's kinematics model! Got exception: '" << e.what()
              << "'." << std::endl;
    return 2;
  };
  ...
  try {
    while( true ) {
      ...
      if( is_measured_pose ) {
        ...
        cur_pose.Quat
          = quaternion< double >( vect< double, 4 >( nvr_in["q_0"], nvr_in["q_1"], nvr_in["q_2"], nvr_in["q_3"] ) );
      } else {
        cur_pose.Position = vect< double, 3 >( nvr_in["pos_x"], nvr_in["pos_y"], nvr_in["pos_z"] );
        cur_pose.Quat
          = quaternion< double >( vect< double, 4 >( nvr_in["q0"], nvr_in["q1"], nvr_in["q2"], nvr_in["q3"] ) );
      ...
      *( airship3D_kin_model->getFrame3D( 0 ) ) = cur_pose;
      ...
      ( *data_out ) << nvr_in["time"] << cur_grapple.Position << cur_grapple.Quat[0] << cur_grapple.Quat[1]
