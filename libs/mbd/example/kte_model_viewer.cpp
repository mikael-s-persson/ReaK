...
KTEModelViewerEditor::KTEModelViewerEditor( QWidget* parent, Qt::WindowFlags flags )
    : QMainWindow( parent, flags ), ui( new Ui::KTEModelView() ),
      objtree_graph( shared_ptr< serialization::object_graph >( new serialization::object_graph() ) ),
      objtree_root( add_vertex( *objtree_graph ) ), objtree_sch_bld(), objtree( objtree_graph, objtree_root ),
      propedit( &( objtree.mdl ) ), objtree_edit( propedit.mdl.get_object_editor() ), view3d_menu( this ) {
  ui->setupUi( this );
...
void KTEModelViewerEditor::onLoad() {
  QString fileName = QFileDialog::getOpenFileName( this, tr( "Load a Model..." ), last_used_path, tr( "\
...
void KTEModelViewerEditor::onSave() {
  ...
  serialization::object_node_desc current_obj_id = propedit.mdl.get_current_node();
  std::string current_obj_name = objtree_edit.get_object_name( current_obj_id );

  QString diag_title;
  QString diag_filter;

  std::map< std::string, shared_ptr< kte::kte_chain_geometry_3D > >::iterator kte_geom_it
    = kte_geometries.find( current_obj_name );
...
KTE-chain Geometry Specification - Protobuf (*.geom.pbuf)" );
  ...
  std::map< std::string, shared_ptr< geom::colored_model_3D > >::iterator geom_it
    = geom_models.find( current_obj_name );
...
Geometric Model - Protobuf (*.geom_mdl.pbuf)" );
  ...
  std::map< std::string, shared_ptr< geom::proxy_query_model_3D > >::iterator prox_it
    = proxy_models.find( current_obj_name );
...
Proximity Model - Protobuf (*.prox_mdl.pbuf)" );
  ...
  std::map< std::string, shared_ptr< kte::inverse_dynamics_model > >::iterator dyn_it
    = dyn_models.find( current_obj_name );
  std::map< std::string, shared_ptr< kte::inverse_kinematics_model > >::iterator ik_it
    = ik_models.find( current_obj_name );
  std::map< std::string, shared_ptr< kte::direct_kinematics_model > >::iterator dk_it
    = dk_models.find( current_obj_name );
...
ReaK Archive - Protobuf (*.pbuf)" );
  ...
  QString fileName = QFileDialog::getSaveFileName( this, diag_title, last_used_path, diag_filter );
  ...
  shared_ptr< serialization::oarchive > out_ar;
  if( fileExt == tr( "rkb" ) ) {
    out_ar = shared_ptr< serialization::oarchive >( new serialization::bin_oarchive( fileName.toStdString() ) );
  } else if( fileExt == tr( "pbuf" ) ) {
    out_ar = shared_ptr< serialization::oarchive >( new serialization::protobuf_oarchive( fileName.toStdString() ) );
  } else {
    out_ar = shared_ptr< serialization::oarchive >( new serialization::xml_oarchive( fileName.toStdString() ) );
  };
  ...
  if( kte_geom_it != kte_geometries.end() ) {
    ...
    shared_ptr< serializable > current_obj_ptr = ( *objtree_graph )[current_obj_id].p_obj;
