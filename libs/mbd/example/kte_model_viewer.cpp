...
KTEModelViewerEditor::KTEModelViewerEditor(QWidget * parent, Qt::WindowFlags flags ) : 
  ...
  */
  ...
  ui->actionClose_all->setIcon( style()->standardIcon( QStyle::SP_DialogCloseButton, NULL, ui->actionClose_all->menu() ) );
...
void KTEModelViewerEditor::onSave() {
  ...
  std::map< std::string, shared_ptr< kte::kte_chain_geometry_3D > >::iterator kte_geom_it = kte_geometries.find(current_obj_name);
...
KTE-chain Geometry Specification - Protobuf (*.geom.pbuf)");
  ...
  std::map< std::string, shared_ptr< geom::colored_model_3D > >::iterator geom_it = geom_models.find(current_obj_name);
...
Geometric Model - Protobuf (*.geom_mdl.pbuf)");
  ...
  std::map< std::string, shared_ptr< geom::proxy_query_model_3D > >::iterator prox_it = proxy_models.find(current_obj_name);
...
Proximity Model - Protobuf (*.prox_mdl.pbuf)");
  ...
  std::map< std::string, shared_ptr< kte::inverse_dynamics_model > >::iterator dyn_it = dyn_models.find(current_obj_name);
  std::map< std::string, shared_ptr< kte::inverse_kinematics_model > >::iterator ik_it = ik_models.find(current_obj_name);
  std::map< std::string, shared_ptr< kte::direct_kinematics_model > >::iterator dk_it = dk_models.find(current_obj_name);
