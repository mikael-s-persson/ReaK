...
int main(int argc, char ** argv) {
  ...
  io_options.add_options()
    ...
    ("output-name,o", po::value< std::string >()->default_value("airship3D"), "specify the output base-name (default is 'airship3D')")
...
//   (Position = (0.176438; -0.356764; -0.0521845); Quaternion = (0.999986; 0.00301881; -0.00257751; 0.00343278))
  ...
  shared_ptr< free_joint_3D > 
    airship3D_joint( new free_joint_3D("airship3D_joint",
                                       airship3D_frame,
                                       global_base,
  ...
  shared_ptr< inertia_3D > 
    airship3D_inertia( new inertia_3D("airship3D_inertia",
                                      airship3D_dep_frame,
                                      1.0,
                                      mat<double,mat_structure::symmetric>(mat<double,mat_structure::identity>(3))));
  
  shared_ptr< driving_actuator_3D > 
    airship3D_actuator( new driving_actuator_3D("airship3D_actuator",
                                                airship3D_frame,
                                                airship3D_joint));
  
  shared_ptr< position_measure_3D > 
    airship3D_position( new position_measure_3D("airship3D_position",
                                                airship3D_frame));
  
  shared_ptr< rotation_measure_3D > 
    airship3D_rotation( new rotation_measure_3D("airship3D_rotation", airship3D_frame));
  
  shared_ptr< kte_map_chain > 
    airship3D_model( new kte_map_chain("airship3D_model"));
  ...
  shared_ptr< manipulator_dynamics_model > airship3D_dyn_model(new manipulator_dynamics_model("airship3D_dyn_model"));
...
//   double gr_arc_length = 0.225;
  ...
  shared_ptr< colored_model_3D > geom_mdl = shared_ptr< colored_model_3D >(new colored_model_3D("airship3D_geom_render"));
  (*geom_mdl)
  ...
  shared_ptr< proxy_query_model_3D > proxy_mdl = shared_ptr< proxy_query_model_3D >(new proxy_query_model_3D("airship3D_geom_render"));
  (*proxy_mdl)
    .addShape(hull);
