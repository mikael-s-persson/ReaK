...
int main( int argc, char** argv ) {
  ...
  generic_options.add_options()( "help,h", "produce this help message. This program expects, as input, a data file "
                                           "with robot configurations and corresponding world coordinates." );
  ...
  pose_3D< double > EE_to_marker( weak_ptr< pose_3D< double > >(), vect< double, 3 >( -0.033, 0.0, 0.107 ),
                                  quaternion< double >() );
  ...
  try {
    while( true ) {
      ...
      vect_n< double > CRS_jt( nvr_in["track"], nvr_in["q_0"], nvr_in["q_1"], nvr_in["q_2"], nvr_in["q_3"],
                               nvr_in["q_4"], nvr_in["q_5"] );
  ...
  for( std::size_t i = 0; i < calib_pts.size(); ++i ) {
    ...
    avg_err += cur_err;
    std::cout << " CRS = " << calib_pts[i].first << " New CRS = " << CRS_new_pt << " ErrNorm = " << cur_err
              << std::endl;
  };
