...
std::string KTEModelViewerEditor::loadKinModelFromArchive( serialization::iarchive& in ) {
  ...
  if( !base_frame || !kin_mdl || !mdl_jt_lim ) {
    ...
    QMessageBox::information( this, tr( "Failed to load file!" ),
                              tr( "Sorry, some required objects could not be loaded from this file! Culprits are " )
                              + culprit,
                              QMessageBox::Ok );
    return std::string();
  };
  ...
  shared_ptr< kte::inverse_kinematics_model > kin_ik_mdl
...
void KTEModelViewerEditor::loadFromArchive( serialization::iarchive& in, QString fileContentExt ) {
  ...
  if( ( fileContentExt == tr( "kte_ik" ) ) || ( fileContentExt == tr( "kte_dk" ) ) ) {
    ...
    if( !kte_geom ) {
      QMessageBox::information( this, tr( "Failed to load file!" ),
                                tr( "Sorry, KTE geometry could not be loaded from this file!" ), QMessageBox::Ok );
      return;
    };
    ...
    for( std::map< std::string, shared_ptr< kte::direct_kinematics_model > >::iterator it = dk_models.begin();
         it != dk_models.end(); ++it ) {
...
void KTEModelViewerEditor::saveKTEChainGeometry( serialization::oarchive& out, const std::string&,
                                                 const shared_ptr< kte::kte_chain_geometry_3D >& kte_geom ) {
  out << kte_geom;
};

void KTEModelViewerEditor::saveGeometricModel( serialization::oarchive& out, const std::string&,
                                               const shared_ptr< geom::colored_model_3D >& geom_mdl ) {
  out << geom_mdl;
};

void KTEModelViewerEditor::saveProximityModel( serialization::oarchive& out, const std::string&,
                                               const shared_ptr< geom::proxy_query_model_3D >& prox_mdl ) {
  out << prox_mdl;
};

void KTEModelViewerEditor::saveDirectKinModel( serialization::oarchive& out, const std::string& mdl_name,
...
void KTEModelViewerEditor::saveInverseKinModel( serialization::oarchive& out, const std::string& mdl_name,
...
void KTEModelViewerEditor::saveDynamicsModel( serialization::oarchive& out, const std::string& mdl_name,
