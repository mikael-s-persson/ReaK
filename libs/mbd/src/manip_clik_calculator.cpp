...
struct clik_eq_function {
  ...
  vect_n< double > operator()( const vect_n< double >& x ) const {
    ...
    vect_n< double > result( pmdl->getDependentVelocitiesCount() * 2 + pmdl->getFrames2DCount()
                             + pmdl->getFrames3DCount() );
    ...
    for( std::size_t i = 0; i < pmdl->getDependentCoordsCount(); ++i ) {
      result[j] = pmdl->getDependentCoord( i )->mFrame->q - parent->desired_gen_coords[i].q;
      ++j;
      result[k] = pmdl->getDependentCoord( i )->mFrame->q_dot - parent->desired_gen_coords[i].q_dot;
      ++k;
    };
...
struct clik_eq_jac_filler {
  ...
  void operator()( Matrix& J, const vect_n< double >& x, const vect_n< double >& h ) const {
    ...
    for( std::size_t i = 0; i < pmdl->getFrames3DCount(); ++i ) {
      ...
      sub( J )( range( 0, n1 * 2 ), range( j - 3, j + 1 ) ) = sub( J )( range( 0, n1 * 2 ), range( k - 2, k + 1 ) )
                                                              * H_inv;
...
shared_ptr< optim::quadratic_cost_evaluator > create_clik_quad_cost( const vect_n< double >& aPreferredPosture,
  ...
  mat< double, mat_structure::symmetric > Qmat
    = mat< double, mat_structure::symmetric >( mat< double, mat_structure::identity >( N1 + N2 ) );
...
void manip_clik_calculator::runOptimizer( vect_n< double >& x ) {
  ...
  if( !cost_eval ) {
    tmp_cost_eval = shared_ptr< optim::cost_evaluator >( new optim::quadratic_cost_evaluator(
  ...
  typedef optim::nlip_newton_tr_factory< optim::oop_cost_function, optim::oop_cost_grad, optim::oop_cost_hess, double,
                                         detail::clik_eq_function, detail::clik_eq_jac_filler,
                                         detail::clik_ineq_function, detail::clik_ineq_jac_filler > optim_factory_type;
  ...
  optim_factory_type optimizer
    = optim_factory_type( optim::oop_cost_function( tmp_cost_eval ), optim::oop_cost_grad( tmp_cost_eval ),
                          optim::oop_cost_hess( tmp_cost_eval ), max_radius, mu, max_iter,
                          detail::clik_eq_function( this ), detail::clik_eq_jac_filler( this ),
                          detail::clik_ineq_function( this ), detail::clik_ineq_jac_filler( this ), tol, eta, tau );
