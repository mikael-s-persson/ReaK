  ...
  if( ( aFlag == store_kinematics ) && ( aStorage ) ) {
    if( !( aStorage->gen_coord_mapping[mAnchor1] ) )
      aStorage->gen_coord_mapping[mAnchor1]
        = shared_ptr< gen_coord< double > >( new gen_coord< double >( ( *mAnchor1 ) ), scoped_deleter() );
    else
      ( *( aStorage->gen_coord_mapping[mAnchor1] ) ) = ( *mAnchor1 );
    if( !( aStorage->gen_coord_mapping[mAnchor2] ) )
      aStorage->gen_coord_mapping[mAnchor2]
        = shared_ptr< gen_coord< double > >( new gen_coord< double >( ( *mAnchor2 ) ), scoped_deleter() );
    else
      ( *( aStorage->gen_coord_mapping[mAnchor2] ) ) = ( *mAnchor2 );
  };
};

void damper_gen::doForce( kte_pass_flag aFlag, const shared_ptr< frame_storage >& aStorage ) {
  RK_UNUSED( aFlag );
  RK_UNUSED( aStorage );
  if( ( !mAnchor1 ) || ( !mAnchor2 ) )
    return;
...
void damper_2D::doForce( kte_pass_flag aFlag, const shared_ptr< frame_storage >& aStorage ) {
  RK_UNUSED( aFlag );
  RK_UNUSED( aStorage );
  if( ( !mAnchor1 ) || ( !mAnchor2 ) )
    return;
...
void damper_3D::doForce( kte_pass_flag aFlag, const shared_ptr< frame_storage >& aStorage ) {
  RK_UNUSED( aFlag );
  RK_UNUSED( aStorage );
  if( ( !mAnchor1 ) || ( !mAnchor2 ) )
    return;
