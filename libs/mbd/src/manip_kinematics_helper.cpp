...
void manip_kin_mdl_joint_io::getJointPositions( double* result ) const {
  ...
  for( std::size_t i = 0; i < model->getFrames2DCount(); ++i ) {
    shared_ptr< frame_2D< double > > i_ptr = model->getFrame2D( i );
  ...
  for( std::size_t i = 0; i < model->getFrames3DCount(); ++i ) {
    shared_ptr< frame_3D< double > > i_ptr = model->getFrame3D( i );
    ( *result++ ) = i_ptr->Position[0];
    ( *result++ ) = i_ptr->Position[1];
...
void manip_kin_mdl_joint_io::setJointPositions( const double* aJointPositions ) {
  ...
  for( std::size_t i = 0; i < model->getFrames2DCount(); ++i ) {
    ...
    i_ptr->Rotation = rot_mat_2D< double >( vect< double, 2 >( aJointPositions[0], aJointPositions[1] ) );
    aJointPositions += 2;
  };
  ...
  for( std::size_t i = 0; i < model->getFrames3DCount(); ++i ) {
    ...
    i_ptr->Quat = quaternion< double >(
      vect< double, 4 >( aJointPositions[0], aJointPositions[1], aJointPositions[2], aJointPositions[3] ) );
    aJointPositions += 4;
  };
};
...
void manip_kin_mdl_joint_io::setJointVelocities( const double* aJointVelocities ) {
...
void manip_kin_mdl_joint_io::setJointAccelerations( const double* aJointAccelerations ) {
...
void manip_kin_mdl_joint_io::getDependentPositions( double* result ) const {
  ...
  for( std::size_t i = 0; i < model->getDependentFrames2DCount(); ++i ) {
    shared_ptr< joint_dependent_frame_2D > i_ptr = model->getDependentFrame2D( i );
  ...
  for( std::size_t i = 0; i < model->getDependentFrames3DCount(); ++i ) {
    shared_ptr< joint_dependent_frame_3D > i_ptr = model->getDependentFrame3D( i );
    ( *result++ ) = i_ptr->mFrame->Position[0];
    ( *result++ ) = i_ptr->mFrame->Position[1];
...
void manip_kin_mdl_jac_calculator::getJacobianMatrixAndDerivativeImpl(
  ...
  typedef mat_sub_block< mat< double, mat_structure::rectangular > > SubMat;
  ...
  for( std::size_t i = 0; i < model->getCoordsCount(); ++i ) {
    RowInd = 0;
    shared_ptr< gen_coord< double > > i_joint = model->getCoord( i );
    ...
    for( std::size_t j = 0; j < model->getDependentFrames2DCount(); ++j ) {
      ...
      if( j_dep->mUpStreamJoints.find( i_joint ) != j_dep->mUpStreamJoints.end() ) {
        ...
        if( JacDot ) {
          ...
          j_dep->mUpStreamJoints[i_joint]->get_jac_relative_to( j_dep->mFrame ).write_to_matrices( subJac, subJacDot );
        } else {
          j_dep->mUpStreamJoints[i_joint]->get_jac_relative_to( j_dep->mFrame ).write_to_matrices( subJac );
        };
      };
      RowInd += 3;
    };
  ...
  for( std::size_t i = 0; i < model->getFrames2DCount(); ++i ) {
    RowInd = 0;
    shared_ptr< frame_2D< double > > i_joint = model->getFrame2D( i );
    ...
    for( std::size_t j = 0; j < model->getDependentFrames2DCount(); ++j ) {
      ...
      if( j_dep->mUpStream2DJoints.find( i_joint ) != j_dep->mUpStream2DJoints.end() ) {
        ...
        if( JacDot ) {
          ...
          j_dep->mUpStream2DJoints[i_joint]->get_jac_relative_to( j_dep->mFrame ).write_to_matrices( subJac,
                                                                                                     subJacDot );
        } else {
          j_dep->mUpStream2DJoints[i_joint]->get_jac_relative_to( j_dep->mFrame ).write_to_matrices( subJac );
        };
  ...
  for( std::size_t i = 0; i < model->getFrames3DCount(); ++i ) {
    RowInd = 0;
    shared_ptr< frame_3D< double > > i_joint = model->getFrame3D( i );
    ...
    for( std::size_t j = 0; j < model->getDependentFrames2DCount(); ++j ) {
      ...
      if( j_dep->mUpStream3DJoints.find( i_joint ) != j_dep->mUpStream3DJoints.end() ) {
        ...
        if( JacDot ) {
          ...
          j_dep->mUpStream3DJoints[i_joint]->get_jac_relative_to( j_dep->mFrame ).write_to_matrices( subJac,
                                                                                                     subJacDot );
        } else {
          j_dep->mUpStream3DJoints[i_joint]->get_jac_relative_to( j_dep->mFrame ).write_to_matrices( subJac );
        };
