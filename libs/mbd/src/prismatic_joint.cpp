...
void prismatic_joint_2D::doMotion(
  ...
  if (!mCoord) {
    ...
    mEnd->Velocity =
        mBase->Velocity +
        mBase->Rotation * ((mBase->AngVelocity % tmp_pos) + tmp_vel);
    mEnd->Acceleration =
        mBase->Acceleration +
        mBase->Rotation *
            ((-mBase->AngVelocity * mBase->AngVelocity) * tmp_pos +
    ...
    if (mCoord) {
      if (!(aStorage->gen_coord_mapping[mCoord])) {
        aStorage->gen_coord_mapping[mCoord] =
            std::make_shared<gen_coord<double>>(*mCoord);
      } else {
...
void prismatic_joint_2D::doForce(
    kte_pass_flag aFlag, const std::shared_ptr<frame_storage>& aStorage) {
  if ((!mEnd) || (!mBase)) {
    return;
  }
...
void prismatic_joint_3D::doMotion(
    kte_pass_flag aFlag, const std::shared_ptr<frame_storage>& aStorage) {
  if ((!mEnd) || (!mBase)) {
    return;
  }
  ...
  if (!mCoord) {
    ...
    mEnd->Velocity =
        mBase->Velocity + R * ((mBase->AngVelocity % tmp_pos) + tmp_vel);
    mEnd->Acceleration =
        mBase->Acceleration +
        R * ((mBase->AngVelocity % (mBase->AngVelocity % tmp_pos)) +
             2.0 * (mBase->AngVelocity % tmp_vel) +
             (mBase->AngAcceleration % tmp_pos) + (mCoord->q_ddot * mAxis));
...
void prismatic_joint_3D::doForce(
    kte_pass_flag aFlag, const std::shared_ptr<frame_storage>& aStorage) {
  if ((!mEnd) || (!mBase)) {
    return;
  }
