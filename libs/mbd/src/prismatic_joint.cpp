...
void prismatic_joint_2D::doMotion( kte_pass_flag aFlag, const shared_ptr< frame_storage >& aStorage ) {
  ...
  if( !mCoord ) {
    ...
    mEnd->Acceleration = mBase->Acceleration
                         + mBase->Rotation * ( ( -mBase->AngVelocity * mBase->AngVelocity ) * tmp_pos
                                               + ( 2.0 * mBase->AngVelocity ) % tmp_vel
                                               + mBase->AngAcceleration % tmp_pos + ( mCoord->q_ddot * mAxis ) );
  ...
  if( ( aFlag == store_kinematics ) && ( aStorage ) ) {
    ...
    if( mCoord ) {
      if( !( aStorage->gen_coord_mapping[mCoord] ) )
        aStorage->gen_coord_mapping[mCoord]
          = shared_ptr< gen_coord< double > >( new gen_coord< double >( ( *mCoord ) ), scoped_deleter() );
      else
...
void prismatic_joint_2D::doForce( kte_pass_flag aFlag, const shared_ptr< frame_storage >& aStorage ) {
  if( ( !mEnd ) || ( !mBase ) )
    return;
...
void prismatic_joint_3D::doMotion( kte_pass_flag aFlag, const shared_ptr< frame_storage >& aStorage ) {
  ...
  if( !mCoord ) {
    ...
    mEnd->Acceleration
      = mBase->Acceleration
        + R * ( ( mBase->AngVelocity % ( mBase->AngVelocity % tmp_pos ) ) + 2.0 * ( mBase->AngVelocity % tmp_vel )
                + ( mBase->AngAcceleration % tmp_pos ) + ( mCoord->q_ddot * mAxis ) );
...
void prismatic_joint_3D::doForce( kte_pass_flag aFlag, const shared_ptr< frame_storage >& aStorage ) {
  if( ( !mEnd ) || ( !mBase ) )
    return;
