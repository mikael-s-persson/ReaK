...
void rigid_link_gen::doMotion(kte_pass_flag aFlag,
...
void rigid_link_gen::doForce(kte_pass_flag aFlag,
                             const std::shared_ptr<frame_storage>& aStorage) {
  if ((!mBase) || (!mEnd)) {
    return;
  }
...
void rigid_link_2D::doMotion(kte_pass_flag aFlag,
                             const std::shared_ptr<frame_storage>& aStorage) {
  if ((!mBase) || (!mEnd)) {
    return;
  }
  ...
  mEnd->Velocity = mBase->Velocity + mBase->Rotation * (mBase->AngVelocity %
                                                        mPoseOffset.Position);
  mEnd->Acceleration =
      mBase->Acceleration +
      mBase->Rotation *
          ((-mBase->AngVelocity * mBase->AngVelocity) * mPoseOffset.Position +
...
void rigid_link_2D::doForce(kte_pass_flag aFlag,
                            const std::shared_ptr<frame_storage>& aStorage) {
  if ((!mBase) || (!mEnd)) {
    return;
  }
...
void rigid_link_3D::doMotion(kte_pass_flag aFlag,
                             const std::shared_ptr<frame_storage>& aStorage) {
  if ((!mBase) || (!mEnd)) {
    return;
  }
...
void rigid_link_3D::doForce(kte_pass_flag aFlag,
                            const std::shared_ptr<frame_storage>& aStorage) {
  if ((!mBase) || (!mEnd)) {
    return;
  }
