...
manip_3R3R_kinematics::manip_3R3R_kinematics( const std::string& aName,
    ...
    : inverse_kinematics_model( aName ), m_base_frame( aBaseFrame ), base_to_shoulder( aBaseToShoulder ),
      shoulder_to_elbow( aShoulderToElbow ), elbow_to_joint_4( aElbowToJoint4 ), joint_4_to_wrist( aJoint4ToWrist ),
      wrist_to_flange( aWristToFlange ), joint_lower_bounds( aJointLowerBounds ),
  ...
  shared_ptr< kte::revolute_joint_3D > joint_1(
    new kte::revolute_joint_3D( "manip_3R3R_joint_1", m_joints[0], vect< double, 3 >( 0.0, 0.0, 1.0 ), m_base_frame,
  ...
  shared_ptr< kte::rigid_link_3D > link_1(
    new kte::rigid_link_3D( "manip_3R3R_link_1", joint_1_end, joint_2_base,
  ...
  shared_ptr< kte::revolute_joint_3D > joint_2(
    new kte::revolute_joint_3D( "manip_3R3R_joint_2", m_joints[1], vect< double, 3 >( 0.0, -1.0, 0.0 ), joint_2_base,
                                joint_2_end, joint_2_jacobian ),
    scoped_deleter() );

  // create link
  shared_ptr< kte::rigid_link_3D > link_2(
    new kte::rigid_link_3D( "manip_3R3R_link_2", joint_2_end, joint_3_base,
                            pose_3D< double >( weak_ptr< pose_3D< double > >(),
  ...
  shared_ptr< kte::revolute_joint_3D > joint_3(
    new kte::revolute_joint_3D( "manip_3R3R_joint_3", m_joints[2], vect< double, 3 >( 0.0, -1.0, 0.0 ), joint_3_base,
                                joint_3_end, joint_3_jacobian ),
    scoped_deleter() );

  // create link
  shared_ptr< kte::rigid_link_3D > link_3(
    new kte::rigid_link_3D( "manip_3R3R_link_3", joint_3_end, joint_4_base,
                            pose_3D< double >( weak_ptr< pose_3D< double > >(),
  ...
  // create revolute joint
  shared_ptr< kte::revolute_joint_3D > joint_4(
    new kte::revolute_joint_3D( "manip_3R3R_joint_4", m_joints[3], vect< double, 3 >( 0.0, 0.0, 1.0 ), joint_4_base,
                                joint_4_end, joint_4_jacobian ),
    scoped_deleter() );

  // create link
  shared_ptr< kte::rigid_link_3D > link_4(
    new kte::rigid_link_3D( "manip_3R3R_link_4", joint_4_end, joint_5_base,
                            pose_3D< double >( weak_ptr< pose_3D< double > >(),
  ...
  // create revolute joint
  shared_ptr< kte::revolute_joint_3D > joint_5(
    new kte::revolute_joint_3D( "manip_3R3R_joint_5", m_joints[4], vect< double, 3 >( 0.0, -1.0, 0.0 ), joint_5_base,
                                joint_5_end, joint_5_jacobian ),
    scoped_deleter() );

  // create link
  shared_ptr< kte::rigid_link_3D > link_5(
    new kte::rigid_link_3D( "manip_3R3R_link_5", joint_5_end, joint_6_base,
                            pose_3D< double >( weak_ptr< pose_3D< double > >(),
  ...
  // create revolute joint
  shared_ptr< kte::revolute_joint_3D > joint_6( new kte::revolute_joint_3D( "manip_3R3R_joint_6", m_joints[5],
  ...
  *m_chain << joint_1 << link_1 << joint_2 << link_2 << joint_3 << link_3 << joint_4 << link_4 << joint_5 << link_5
           << joint_6;
};


void manip_3R3R_kinematics::doDirectMotion() { m_chain->doMotion(); };
...
void manip_3R3R_kinematics::doInverseMotion() {
  ...
  if( wrist_pos[2] > ( shoulder_to_wrist - extend_epsilon ) * c2_max ) {
    if( wrist_pos[0] * wrist_pos[0] + wrist_pos[1] * wrist_pos[1] + wrist_pos[2] * wrist_pos[2]
        > ( shoulder_to_wrist - extend_epsilon ) * ( shoulder_to_wrist - extend_epsilon ) )
    ...
    if( low_elbow_to_desired_height * low_elbow_to_desired_height
        + pow( sqrt( wrist_pos[0] * wrist_pos[0] + wrist_pos[1] * wrist_pos[1] ) - shoulder_to_elbow * s2_max, 2 )
  ...
  if( j3tmp1 < 0.0 )
    throw optim::infeasible_problem( "Inverse kinematics problem is infeasible! End-effector pose is out-of-reach! "
                                     "Cannot compute an elbow angle that would reach the desired wrist position." );
  ...
  vect< double, 2 > to_wrist_along_l1( shoulder_to_elbow * cos( solns[fun_posture][2] ) + elbow_to_wrist,
                                       s2e_projection );
  ...
  for( std::size_t i = 0; i < 2; ++i ) {
    ...
    double s23 = /*reach_sign * */ ( s2e_projection_i * wrist_pos[2] - to_wrist_along_l1[0] * baseplane_dist )
                 * inv_wrist_dist_sqr;
  ...
  for( std::size_t i = 0; i < 8; ++i ) {
    ...
    for( std::size_t j = 0; j < 6; ++j ) {
      if( ( solns[i][j] > joint_lower_bounds[j] ) && ( solns[i][j] < joint_upper_bounds[j] ) ) {
        ...
        //         cost += fabs( 2.0 * solns[i][j] - joint_lower_bounds[j] - joint_upper_bounds[j] ) /
  ...
  if( best_cost > 99.0 )
    throw optim::infeasible_problem(
      "Inverse kinematics problem is infeasible! None of the inverse kinematics solutions respect the joint limits." );
...
void manip_3R3R_kinematics::getJacobianMatrixAndDerivative( mat< double, mat_structure::rectangular >& Jac,
  ...
  vect< double, 3 > a2_dot
    = shoulder_to_elbow
      * ( q1 * ( m_joints[0]->q_dot * ( vect_k % ( q2 * vk ) ) + q2 * ( -m_joints[1]->q_dot * vi ) ) );

  vect< double, 3 > e4_dot
    = q1 * ( m_joints[0]->q_dot * ( vect_k % ( q2 * q3 * vk ) )
             - q2 * ( m_joints[1]->q_dot * ( vect_j % ( q3 * vk ) ) + q3 * ( m_joints[2]->q_dot * vi ) ) );
  ...
  vect< double, 3 > e5_dot
    = q1 * ( -m_joints[0]->q_dot * ( vect_k % ( q2 * q3 * q4 * vj ) )
             + q2 * ( m_joints[1]->q_dot * ( vect_j % ( q3 * q4 * vj ) )
                      + q3 * ( m_joints[2]->q_dot * ( vect_j % ( q4 * vj ) ) + q4 * ( m_joints[3]->q_dot * vi ) ) ) );
...
vect_n< double > manip_3R3R_kinematics::getJointVelocities() const {
  return vect_n< double >( m_joints[0]->q_dot, m_joints[1]->q_dot, m_joints[2]->q_dot, m_joints[3]->q_dot,
                           m_joints[4]->q_dot, m_joints[5]->q_dot );
};
...
vect_n< double > manip_3R3R_kinematics::getJointAccelerations() const {
  return vect_n< double >( m_joints[0]->q_ddot, m_joints[1]->q_ddot, m_joints[2]->q_ddot, m_joints[3]->q_ddot,
                           m_joints[4]->q_ddot, m_joints[5]->q_ddot );
};
...
void RK_CALL manip_3R3R_kinematics::save( serialization::oarchive& A, unsigned int ) const {
  ...
  A& RK_SERIAL_SAVE_WITH_NAME( m_base_frame ) & RK_SERIAL_SAVE_WITH_NAME( m_joints ) & RK_SERIAL_SAVE_WITH_NAME( m_EE )
    & RK_SERIAL_SAVE_WITH_NAME( base_to_shoulder ) & RK_SERIAL_SAVE_WITH_NAME( shoulder_to_elbow )
    & RK_SERIAL_SAVE_WITH_NAME( elbow_to_joint_4 ) & RK_SERIAL_SAVE_WITH_NAME( joint_4_to_wrist )
    & RK_SERIAL_SAVE_WITH_NAME( wrist_to_flange ) & RK_SERIAL_SAVE_WITH_NAME( joint_lower_bounds )
    & RK_SERIAL_SAVE_WITH_NAME( joint_upper_bounds ) & RK_SERIAL_SAVE_WITH_NAME( m_chain );
};
...
void RK_CALL manip_3R3R_kinematics::load( serialization::iarchive& A, unsigned int ) {
  ...
  A& RK_SERIAL_LOAD_WITH_NAME( m_base_frame ) & RK_SERIAL_LOAD_WITH_NAME( m_joints ) & RK_SERIAL_LOAD_WITH_NAME( m_EE )
    & RK_SERIAL_LOAD_WITH_NAME( base_to_shoulder ) & RK_SERIAL_LOAD_WITH_NAME( shoulder_to_elbow )
    & RK_SERIAL_LOAD_WITH_NAME( elbow_to_joint_4 ) & RK_SERIAL_LOAD_WITH_NAME( joint_4_to_wrist )
    & RK_SERIAL_LOAD_WITH_NAME( wrist_to_flange ) & RK_SERIAL_LOAD_WITH_NAME( joint_lower_bounds )
    & RK_SERIAL_LOAD_WITH_NAME( joint_upper_bounds ) & RK_SERIAL_LOAD_WITH_NAME( m_chain );
};
};
};
