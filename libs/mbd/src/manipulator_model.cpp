...
void manipulator_kinematics_model::getJacobianMatrixAndDerivative(
  ...
  manip_kin_mdl_jac_calculator(this).getJacobianMatrixAndDerivative(Jac,
                                                                    JacDot);
}
...
manipulator_kinematics_model& manipulator_dynamics_model::operator<<(
    const std::shared_ptr<gen_coord<double>>& aCoord) {
  if (aCoord) {
    mMassCalc << aCoord;
...
manipulator_dynamics_model& manipulator_dynamics_model::operator<<(
...
vect_n<double> manipulator_dynamics_model::getJointStates() const {
  ...
  vect_ref_view<vect_n<double>> pos_result =
      result[range(0, getJointPositionsCount())];
  manip_kin_mdl_joint_io(this).getJointPositions(pos_result);

  vect_ref_view<vect_n<double>> vel_result =
      result[range(getJointPositionsCount(),
                   getJointPositionsCount() + getJointVelocitiesCount())];
  manip_kin_mdl_joint_io(this).getJointVelocities(vel_result);

  return result;
}

void manipulator_dynamics_model::setJointStates(
    const vect_n<double>& aJointStates) {
  if (aJointStates.size() != getJointStatesCount())
    throw std::range_error("Joint-state vector has incorrect dimensions!");

  vect_const_ref_view<vect_n<double>> positions =
      aJointStates[range(0, getJointPositionsCount())];
  manip_kin_mdl_joint_io(this).setJointPositions(positions);

  vect_const_ref_view<vect_n<double>> velocities =
      aJointStates[range(getJointPositionsCount(),
                         getJointPositionsCount() + getJointVelocitiesCount())];
  manip_kin_mdl_joint_io(this).setJointVelocities(velocities);
}

void manipulator_dynamics_model::computeOutput(
    double aTime, const ReaK::vect_n<double>& aState,
  ...
  unsigned int i = 0;
  for (std::vector<std::shared_ptr<system_output>>::iterator it =
           mOutputs.begin();
       it != mOutputs.end(); ++it)
...
void manipulator_dynamics_model::setInput(const ReaK::vect_n<double>& aInput) {
  if (aInput.size() != getInputsCount())
    throw std::range_error(
        "The size of the input-vector to the manipulator model is not "
        "correct!");

  unsigned int i = 0;
  for (std::vector<std::shared_ptr<system_input>>::iterator it =
           mInputs.begin();
       it != mInputs.end(); ++it)
...
vect_n<double> manipulator_dynamics_model::getInput() const {
  ...
  unsigned int i = 0;
  for (std::vector<std::shared_ptr<system_input>>::const_iterator it =
           mInputs.begin();
       it != mInputs.end(); ++it)
    for (unsigned int k = 0; k < (*it)->getInputCount(); ++k)
...
void manipulator_dynamics_model::computeStateRate(double aTime,
                                                  const vect_n<double>& aState,
...
vect_n<double> manipulator_dynamics_model::getDependentStates() const {
  ...
  vect_ref_view<vect_n<double>> pos_result =
      result[range(0, getDependentPositionsCount())];
  manip_kin_mdl_joint_io(this).getDependentPositions(pos_result);

  vect_ref_view<vect_n<double>> vel_result = result[range(
      getDependentPositionsCount(),
      getDependentPositionsCount() + getDependentVelocitiesCount())];
  manip_kin_mdl_joint_io(this).getDependentVelocities(vel_result);

  return result;
}

void manipulator_dynamics_model::getMassMatrix(
    mat<double, mat_structure::symmetric>& M) {
  mMassCalc.getMassMatrix(M);
}

void manipulator_dynamics_model::getMassMatrixAndDerivative(
    mat<double, mat_structure::symmetric>& M,
    mat<double, mat_structure::square>& M_dot) {
  mMassCalc.getMassMatrixAndDerivative(M, M_dot);
}

void manipulator_dynamics_model::get_TMT_TdMT(
