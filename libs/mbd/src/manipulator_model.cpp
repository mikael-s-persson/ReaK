...
manipulator_kinematics_model& manipulator_kinematics_model::
...
manipulator_kinematics_model& manipulator_kinematics_model::
  operator<<( const shared_ptr< joint_dependent_gen_coord >& aDependentGenCoord ) {
...
manipulator_kinematics_model& manipulator_kinematics_model::
  operator<<( const shared_ptr< joint_dependent_frame_2D >& aDependent2DFrame ) {
...
manipulator_kinematics_model& manipulator_kinematics_model::
  operator<<( const shared_ptr< joint_dependent_frame_3D >& aDependent3DFrame ) {
...
vect_n< double > manipulator_dynamics_model::getJointStates() const {
  ...
  vect_ref_view< vect_n< double > > vel_result
    = result[range( getJointPositionsCount(), getJointPositionsCount() + getJointVelocitiesCount() )];
...
void manipulator_dynamics_model::setJointStates( const vect_n< double >& aJointStates ) {
  ...
  vect_const_ref_view< vect_n< double > > velocities
    = aJointStates[range( getJointPositionsCount(), getJointPositionsCount() + getJointVelocitiesCount() )];
...
vect_n< double > manipulator_dynamics_model::getDependentStates() const {
  ...
  vect_ref_view< vect_n< double > > vel_result
    = result[range( getDependentPositionsCount(), getDependentPositionsCount() + getDependentVelocitiesCount() )];
