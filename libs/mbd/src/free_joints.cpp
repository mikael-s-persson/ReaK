    ...
    if( mCoord ) {
      if( !( aStorage->frame_2D_mapping[mCoord] ) )
        aStorage->frame_2D_mapping[mCoord]
          = shared_ptr< frame_2D< double > >( new frame_2D< double >( ( *mCoord ) ), scoped_deleter() );
      else
    ...
    *mEnd = ( *mBase ) * ( *mCoord );
    ...
    if( mJacobian ) {
      ...
      mJacobian->vel_vel = vect< vect< double, 3 >, 3 >(
        vect< double, 3 >( 1.0, 0.0, 0.0 ), vect< double, 3 >( 0.0, 1.0, 0.0 ), vect< double, 3 >( 0.0, 0.0, 1.0 ) );
    ...
    if( mCoord ) {
      if( !( aStorage->frame_3D_mapping[mCoord] ) )
        aStorage->frame_3D_mapping[mCoord]
          = shared_ptr< frame_3D< double > >( new frame_3D< double >( ( *mCoord ) ), scoped_deleter() );
      else
