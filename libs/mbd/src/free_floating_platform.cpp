...
#include <ReaK/mbd/models/free_floating_platform.hpp>
...
free_floater_2D_kinematics::free_floater_2D_kinematics( const std::string& aName,
  ...
  // create free-joint
  shared_ptr< free_joint_2D > joint_1(
    new free_joint_2D( "free_floater_joint_1", m_state_frame, m_base_frame, m_output_frame, m_state_jacobian ),
    scoped_deleter() );
...
void free_floater_2D_kinematics::doInverseMotion() {
  ...
  for( std::size_t i = 1; i < m_EEs.size(); ++i ) {
    ...
    EE_fr.Rotation
      *= rot_mat_2D< double >( ( invert( EE_fr.Rotation ) * EE_outputs[i].Rotation ).getAngle() / double( i + 1 ) );
...
void free_floater_2D_kinematics::getJacobianMatrixAndDerivative(
  mat< double, mat_structure::rectangular >& Jac, mat< double, mat_structure::rectangular >& JacDot ) const {
  ...
  for( std::size_t j = 0; j < m_EEs.size(); ++j ) {
    if( m_EEs[j]->mUpStream2DJoints.find( m_state_frame ) != m_EEs[j]->mUpStream2DJoints.end() ) {
      ...
      if( JacDot ) {
        ...
        m_EEs[j]->mUpStream2DJoints[m_state_frame]->get_jac_relative_to( m_EEs[j]->mFrame ).write_to_matrices(
          subJac, subJacDot );
      } else {
...
vect_n< double > free_floater_2D_kinematics::getJointPositions() const {
  return vect_n< double >( m_state_frame->Position[0], m_state_frame->Position[1], m_state_frame->Rotation[0],
                           m_state_frame->Rotation[1] );
};
...
vect_n< double > free_floater_2D_kinematics::getJointAccelerations() const {
  return vect_n< double >( m_state_frame->Acceleration[0], m_state_frame->Acceleration[1],
...
void free_floater_2D_kinematics::setDependentPositions( const vect_n< double >& aDepPositions ) {
  ...
  for( std::size_t i = 0; i < m_EEs.size(); ++i ) {
    ...
    pose_2D< double > p_inc(
      weak_ptr< pose_2D< double > >(), vect< double, 2 >( aDepPositions[j], aDepPositions[j + 1] ) - p_gbl.Position,
      invert( p_gbl.Rotation )
      * rot_mat_2D< double >( vect< double, 2 >( aDepPositions[j + 2], aDepPositions[j + 3] ) ) );
...
void free_floater_2D_kinematics::setDependentVelocities( const vect_n< double >& aDepVelocities ) {
  ...
  for( std::size_t i = 0; i < m_EEs.size(); ++i ) {
    frame_2D< double > p_gbl = m_EEs[i]->mFrame->getGlobalFrame();
    m_EEs[i]->mFrame->Velocity
      += p_gbl.rotateFromParent( vect< double, 2 >( aDepVelocities[j], aDepVelocities[j + 1] ) - p_gbl.Velocity );
...
void free_floater_2D_kinematics::setDependentAccelerations( const vect_n< double >& aDepAccelerations ) {
  ...
  for( std::size_t i = 0; i < m_EEs.size(); ++i ) {
    frame_2D< double > p_gbl = m_EEs[i]->mFrame->getGlobalFrame();
    m_EEs[i]->mFrame->Acceleration += p_gbl.rotateFromParent(
      vect< double, 2 >( aDepAccelerations[j], aDepAccelerations[j + 1] ) - p_gbl.Acceleration );
...
void RK_CALL free_floater_2D_kinematics::save( serialization::oarchive& A, unsigned int ) const {
  ...
  resyncEndEffectors();
  A& RK_SERIAL_SAVE_WITH_NAME( m_base_frame ) & RK_SERIAL_SAVE_WITH_NAME( m_state_frame )
    & RK_SERIAL_SAVE_WITH_NAME( m_state_jacobian ) & RK_SERIAL_SAVE_WITH_NAME( m_output_frame )
    & RK_SERIAL_SAVE_WITH_NAME( m_EEs ) & RK_SERIAL_SAVE_WITH_NAME( m_EEposes ) & RK_SERIAL_SAVE_WITH_NAME( m_chain );
};
...
void RK_CALL free_floater_2D_kinematics::load( serialization::iarchive& A, unsigned int ) {
  ...
  A& RK_SERIAL_LOAD_WITH_NAME( m_base_frame ) & RK_SERIAL_LOAD_WITH_NAME( m_state_frame )
    & RK_SERIAL_LOAD_WITH_NAME( m_state_jacobian ) & RK_SERIAL_LOAD_WITH_NAME( m_output_frame )
    & RK_SERIAL_LOAD_WITH_NAME( m_EEs ) & RK_SERIAL_LOAD_WITH_NAME( m_EEposes ) & RK_SERIAL_LOAD_WITH_NAME( m_chain );
  resyncEndEffectors();
};
...
free_floater_3D_kinematics::free_floater_3D_kinematics( const std::string& aName,
  ...
  // create free-joint
  shared_ptr< free_joint_3D > joint_1(
    new free_joint_3D( "free_floater_joint_1", m_state_frame, m_base_frame, m_output_frame, m_state_jacobian ),
    scoped_deleter() );
  ...
  for( std::size_t j = 0; j < m_EEs.size(); ++j ) {
    if( m_EEs[j]->mUpStream3DJoints.find( m_state_frame ) != m_EEs[j]->mUpStream3DJoints.end() ) {
      ...
      if( JacDot ) {
        ...
        m_EEs[j]->mUpStream3DJoints[m_state_frame]->get_jac_relative_to( m_EEs[j]->mFrame ).write_to_matrices(
          subJac, subJacDot );
      } else {
...
vect_n< double > free_floater_3D_kinematics::getJointPositions() const {
  return vect_n< double >( m_state_frame->Position[0], m_state_frame->Position[1], m_state_frame->Position[2],
                           m_state_frame->Quat[0], m_state_frame->Quat[1], m_state_frame->Quat[2],
...
void free_floater_3D_kinematics::setJointPositions( const vect_n< double >& aJointPositions ) {
  ...
  m_state_frame->Quat = quaternion< double >(
    vect< double, 4 >( aJointPositions[3], aJointPositions[4], aJointPositions[5], aJointPositions[6] ) );
};
...
vect_n< double > free_floater_3D_kinematics::getJointVelocities() const {
  return vect_n< double >( m_state_frame->Velocity[0], m_state_frame->Velocity[1], m_state_frame->Velocity[2],
                           m_state_frame->AngVelocity[0], m_state_frame->AngVelocity[1],
                           m_state_frame->AngVelocity[2] );
};
...
void free_floater_3D_kinematics::setDependentPositions( const vect_n< double >& aDepPositions ) {
  ...
  for( std::size_t i = 0; i < m_EEs.size(); ++i ) {
    ...
    pose_3D< double > p_inc(
      ...
      invert( p_gbl.Quat ) * quaternion< double >( vect< double, 4 >( aDepPositions[j + 3], aDepPositions[j + 4],
...
void free_floater_3D_kinematics::setDependentVelocities( const vect_n< double >& aDepVelocities ) {
  ...
  for( std::size_t i = 0; i < m_EEs.size(); ++i ) {
    frame_3D< double > p_gbl = m_EEs[i]->mFrame->getGlobalFrame();
    m_EEs[i]->mFrame->Velocity += p_gbl.rotateFromParent(
      vect< double, 3 >( aDepVelocities[j], aDepVelocities[j + 1], aDepVelocities[j + 2] ) - p_gbl.Velocity );
    m_EEs[i]->mFrame->AngVelocity
      += vect< double, 3 >( aDepVelocities[j + 3], aDepVelocities[j + 4], aDepVelocities[j + 5] ) - p_gbl.AngVelocity;
    j += 6;
  };
};
...
void free_floater_3D_kinematics::setDependentAccelerations( const vect_n< double >& aDepAccelerations ) {
  ...
  for( std::size_t i = 0; i < m_EEs.size(); ++i ) {
    frame_3D< double > p_gbl = m_EEs[i]->mFrame->getGlobalFrame();
    m_EEs[i]->mFrame->Acceleration
      += p_gbl.rotateFromParent( vect< double, 3 >( aDepAccelerations[j], aDepAccelerations[j + 1],
                                                    aDepAccelerations[j + 2] ) - p_gbl.Acceleration );
    m_EEs[i]->mFrame->AngAcceleration += vect< double, 3 >( aDepAccelerations[j + 3], aDepAccelerations[j + 4],
                                                            aDepAccelerations[j + 5] ) - p_gbl.AngAcceleration;
    j += 6;
  };
};
