...
manip_ERA_kinematics::manip_ERA_kinematics( const std::string& aName,
    ...
    : inverse_kinematics_model( aName ), m_base_frame( aBaseFrame ), link_lengths( aLinkLengths ),
      preferred_elbow_dir( aPreferredElbowDir ), joint_lower_bounds( aJointLowerBounds ),
  ...
  shared_ptr< revolute_joint_3D > joint_1( new revolute_joint_3D( "manip_ERA_joint_1", m_joints[0],
                                                                  vect< double, 3 >( 0.0, 0.0, 1.0 ), m_base_frame,
                                                                  joint_1_end, joint_1_jacobian ),
                                           scoped_deleter() );

  // create link
  shared_ptr< rigid_link_3D > link_1(
    new rigid_link_3D( "manip_ERA_link_1", joint_1_end, joint_2_base,
                       pose_3D< double >( weak_ptr< pose_3D< double > >(),
                                          vect< double, 3 >( 0.0, 0.0, link_lengths[0] ), quaternion< double >() ) ),
    scoped_deleter() );

  // create revolute joint
  shared_ptr< revolute_joint_3D > joint_2( new revolute_joint_3D( "manip_ERA_joint_2", m_joints[1],
  ...
  shared_ptr< rigid_link_3D > link_2(
    new rigid_link_3D( "manip_ERA_link_2", joint_2_end, joint_3_base,
                       pose_3D< double >( weak_ptr< pose_3D< double > >(),
                                          vect< double, 3 >( 0.0, 0.0, link_lengths[1] ), quaternion< double >() ) ),
    scoped_deleter() );

  // create revolute joint
  shared_ptr< revolute_joint_3D > joint_3( new revolute_joint_3D( "manip_ERA_joint_3", m_joints[2],
                                                                  vect< double, 3 >( 1.0, 0.0, 0.0 ), joint_3_base,
                                                                  joint_3_end, joint_3_jacobian ),
                                           scoped_deleter() );

  // create link
  shared_ptr< rigid_link_3D > link_3(
    new rigid_link_3D( "manip_ERA_link_3", joint_3_end, joint_4_base,
                       pose_3D< double >( weak_ptr< pose_3D< double > >(),
                                          vect< double, 3 >( 0.0, 0.0, link_lengths[2] ), quaternion< double >() ) ),
    scoped_deleter() );

  // create revolute joint
  shared_ptr< revolute_joint_3D > joint_4( new revolute_joint_3D( "manip_ERA_joint_4", m_joints[3],
                                                                  vect< double, 3 >( 1.0, 0.0, 0.0 ), joint_4_base,
                                                                  joint_4_end, joint_4_jacobian ),
                                           scoped_deleter() );

  // create link
  shared_ptr< rigid_link_3D > link_4(
    new rigid_link_3D( "manip_ERA_link_4", joint_4_end, joint_5_base,
                       pose_3D< double >( weak_ptr< pose_3D< double > >(),
                                          vect< double, 3 >( 0.0, 0.0, link_lengths[3] ), quaternion< double >() ) ),
    scoped_deleter() );

  // create revolute joint
  shared_ptr< revolute_joint_3D > joint_5( new revolute_joint_3D( "manip_ERA_joint_5", m_joints[4],
                                                                  vect< double, 3 >( 1.0, 0.0, 0.0 ), joint_5_base,
                                                                  joint_5_end, joint_5_jacobian ),
                                           scoped_deleter() );

  // create link
  shared_ptr< rigid_link_3D > link_5(
    new rigid_link_3D( "manip_ERA_link_5", joint_5_end, joint_6_base,
                       pose_3D< double >( weak_ptr< pose_3D< double > >(),
                                          vect< double, 3 >( 0.0, 0.0, link_lengths[4] ), quaternion< double >() ) ),
    scoped_deleter() );

  // create revolute joint
  shared_ptr< kte::revolute_joint_3D > joint_6(
    new kte::revolute_joint_3D( "manip_ERA_joint_6", m_joints[5], vect< double, 3 >( 0.0, 1.0, 0.0 ), joint_6_base,
                                joint_6_end, joint_6_jacobian ),
    scoped_deleter() );

  // create link
  shared_ptr< rigid_link_3D > link_6(
    new rigid_link_3D( "manip_ERA_link_6", joint_6_end, joint_7_base,
                       pose_3D< double >( weak_ptr< pose_3D< double > >(),
                                          vect< double, 3 >( 0.0, 0.0, link_lengths[5] ), quaternion< double >() ) ),
    scoped_deleter() );

  // create revolute joint
  shared_ptr< kte::revolute_joint_3D > joint_7( new kte::revolute_joint_3D( "manip_ERA_joint_7", m_joints[6],
  ...
  m_chain = shared_ptr< kte_map_chain >( new kte_map_chain( "manip_ERA_kin_model" ), scoped_deleter() );

  *m_chain << joint_1 << link_1 << joint_2 << link_2 << joint_3 << link_3 << joint_4 << link_4 << joint_5 << link_5
           << joint_6 << link_6 << joint_7;
};


void manip_ERA_kinematics::doDirectMotion() { m_chain->doMotion(); };
...
void manip_ERA_kinematics::getJacobianMatrixAndDerivative( mat< double, mat_structure::rectangular >& Jac,
...
void RK_CALL manip_ERA_kinematics::save( serialization::oarchive& A, unsigned int ) const {
  ...
  A& RK_SERIAL_SAVE_WITH_NAME( m_base_frame ) & RK_SERIAL_SAVE_WITH_NAME( m_joints ) & RK_SERIAL_SAVE_WITH_NAME( m_EE )
    & RK_SERIAL_SAVE_WITH_NAME( link_lengths ) & RK_SERIAL_SAVE_WITH_NAME( joint_lower_bounds )
...
void RK_CALL manip_ERA_kinematics::load( serialization::iarchive& A, unsigned int ) {
  ...
  A& RK_SERIAL_LOAD_WITH_NAME( m_base_frame ) & RK_SERIAL_LOAD_WITH_NAME( m_joints ) & RK_SERIAL_LOAD_WITH_NAME( m_EE )
    & RK_SERIAL_LOAD_WITH_NAME( link_lengths ) & RK_SERIAL_LOAD_WITH_NAME( joint_lower_bounds )
