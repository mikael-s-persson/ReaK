...
UAV_kinematics::UAV_kinematics( const std::string& aName, const shared_ptr< frame_3D< double > >& aBaseFrame )
  ...
  shared_ptr< free_joint_3D > joint_1(
    new free_joint_3D( "UAV_free_joint", m_motion_frame, m_base_frame, EE_frame, joint_jacobian ), scoped_deleter() );
...
void UAV_kinematics::doDirectMotion() { m_chain->doMotion(); };
...
void UAV_kinematics::getJacobianMatrixAndDerivative( mat< double, mat_structure::rectangular >& Jac,
...
vect_n< double > UAV_kinematics::getJointPositions() const {
  return vect_n< double >( m_motion_frame->Position[0], m_motion_frame->Position[1], m_motion_frame->Position[2],
                           m_motion_frame->Quat[0], m_motion_frame->Quat[1], m_motion_frame->Quat[2],
...
vect_n< double > UAV_kinematics::getJointVelocities() const {
  return vect_n< double >( m_motion_frame->Velocity[0], m_motion_frame->Velocity[1], m_motion_frame->Velocity[2],
                           m_motion_frame->AngVelocity[0], m_motion_frame->AngVelocity[1],
...
vect_n< double > UAV_kinematics::getJointAccelerations() const {
  return vect_n< double >( m_motion_frame->Acceleration[0], m_motion_frame->Acceleration[1],
...
vect_n< double > UAV_kinematics::getDependentPositions() const {
  return vect_n< double >( m_output_frame->mFrame->Position[0], m_output_frame->mFrame->Position[1],
...
void UAV_kinematics::setDependentPositions( const vect_n< double >& aDepPositions ) {
  ...
  m_output_frame->mFrame->Quat = quaternion< double >(
...
vect_n< double > UAV_kinematics::getDependentVelocities() const {
  return vect_n< double >( m_output_frame->mFrame->Velocity[0], m_output_frame->mFrame->Velocity[1],
                           m_output_frame->mFrame->Velocity[2], m_output_frame->mFrame->AngVelocity[0],
                           m_output_frame->mFrame->AngVelocity[1], m_output_frame->mFrame->AngVelocity[2] );
};
...
vect_n< double > UAV_kinematics::getDependentAccelerations() const {
  return vect_n< double >( m_output_frame->mFrame->Acceleration[0], m_output_frame->mFrame->Acceleration[1],
                           m_output_frame->mFrame->Acceleration[2], m_output_frame->mFrame->AngAcceleration[0],
                           m_output_frame->mFrame->AngAcceleration[1], m_output_frame->mFrame->AngAcceleration[2] );
};
...
void RK_CALL UAV_kinematics::save( serialization::oarchive& A, unsigned int ) const {
  ...
  A& RK_SERIAL_SAVE_WITH_NAME( m_base_frame ) & RK_SERIAL_SAVE_WITH_NAME( m_motion_frame )
    & RK_SERIAL_SAVE_WITH_NAME( m_output_frame ) & RK_SERIAL_SAVE_WITH_NAME( m_chain );
};
...
void RK_CALL UAV_kinematics::load( serialization::iarchive& A, unsigned int ) {
  ...
  A& RK_SERIAL_LOAD_WITH_NAME( m_base_frame ) & RK_SERIAL_LOAD_WITH_NAME( m_motion_frame )
    & RK_SERIAL_LOAD_WITH_NAME( m_output_frame ) & RK_SERIAL_LOAD_WITH_NAME( m_chain );
};
};
};
