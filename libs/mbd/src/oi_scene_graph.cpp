...
void oi_scene_graph::update_anchors( void* aData, SoSensor* ) {
  ...
  for( Iter3D it = SG->mAnchor3DMap.begin(); it != SG->mAnchor3DMap.end(); ++it ) {
    it->second.second->rotation.setValue( it->first->Quat[1], it->first->Quat[2], it->first->Quat[3],
                                          it->first->Quat[0] );
...
void oi_scene_graph::enableAnchorUpdates() { mTimer->schedule(); };


void oi_scene_graph::disableAnchorUpdates() { mTimer->unschedule(); };
...
oi_scene_graph& operator<<( oi_scene_graph& aSG, const geometry_3D& aGeom3D ) {
  ...
  SoTransform* trans = new SoTransform;
  trans->translation.setValue( aGeom3D.getPose().Position[0], aGeom3D.getPose().Position[1],
                               aGeom3D.getPose().Position[2] );
  trans->rotation.setValue( aGeom3D.getPose().Quat[1], aGeom3D.getPose().Quat[2], aGeom3D.getPose().Quat[3],
