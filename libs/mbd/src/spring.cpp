...
void spring_gen::doMotion(kte_pass_flag aFlag,
  ...
  if ((aFlag == store_kinematics) && (aStorage)) {
    if (!(aStorage->gen_coord_mapping[mAnchor1])) {
      aStorage->gen_coord_mapping[mAnchor1] =
          std::make_shared<ReaK::gen_coord<double>>(*mAnchor1);
    } else {
      (*(aStorage->gen_coord_mapping[mAnchor1])) = (*mAnchor1);
    }
    if (!(aStorage->gen_coord_mapping[mAnchor2])) {
      aStorage->gen_coord_mapping[mAnchor2] =
          std::make_shared<ReaK::gen_coord<double>>(*mAnchor2);
    } else {
...
void spring_gen::doForce(kte_pass_flag aFlag,
  ...
  if (mAnchor1->q > mAnchor2->q) {
    double force_mag =
        (mAnchor1->q - mAnchor2->q - mRestLength) *
        mStiffness;  // Positive if tension, negative if compression.
    if ((mSaturation > 0) && (abs(force_mag) > mSaturation)) {
      if (force_mag > 0) {
    ...
    double force_mag =
        (mAnchor2->q - mAnchor1->q - mRestLength) *
        mStiffness;  // Positive if tension, negative if compression.
    if ((mSaturation > 0) && (abs(force_mag) > mSaturation)) {
      if (force_mag > 0) {
        mAnchor1->f += mSaturation;
...
void spring_2D::doMotion(kte_pass_flag aFlag,
                         const std::shared_ptr<frame_storage>& aStorage) {
  if ((!mAnchor1) || (!mAnchor2)) {
    return;
  }
...
void spring_2D::doForce(kte_pass_flag aFlag,
  ...
  if (diff_mag > 1E-7) {
    ...
    if ((mSaturation > 0) && (abs(force_mag) > mSaturation)) {
      diff *= mSaturation /
              diff_mag;  // Positive if tension, negative if compression.
      if (force_mag > 0) {
...
void spring_3D::doMotion(kte_pass_flag aFlag,
                         const std::shared_ptr<frame_storage>& aStorage) {
  if ((!mAnchor1) || (!mAnchor2)) {
    return;
  }
