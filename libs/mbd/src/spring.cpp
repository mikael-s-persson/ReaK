...
void spring_gen::doMotion( kte_pass_flag aFlag, const shared_ptr< frame_storage >& aStorage ) {
  ...
  if( ( aFlag == store_kinematics ) && ( aStorage ) ) {
    if( !( aStorage->gen_coord_mapping[mAnchor1] ) )
      aStorage->gen_coord_mapping[mAnchor1] = shared_ptr< ReaK::gen_coord< double > >(
        new ReaK::gen_coord< double >( ( *mAnchor1 ) ), ReaK::scoped_deleter() );
    else
      ( *( aStorage->gen_coord_mapping[mAnchor1] ) ) = ( *mAnchor1 );
    if( !( aStorage->gen_coord_mapping[mAnchor2] ) )
      aStorage->gen_coord_mapping[mAnchor2] = shared_ptr< ReaK::gen_coord< double > >(
        new ReaK::gen_coord< double >( ( *mAnchor2 ) ), ReaK::scoped_deleter() );
    else
      ( *( aStorage->gen_coord_mapping[mAnchor2] ) ) = ( *mAnchor2 );
  };
};

void spring_gen::doForce( kte_pass_flag aFlag, const shared_ptr< frame_storage >& aStorage ) {
  RK_UNUSED( aFlag );
  RK_UNUSED( aStorage );
  if( ( !mAnchor1 ) || ( !mAnchor2 ) )
    return;
  ...
  if( mAnchor1->q > mAnchor2->q ) {
    double force_mag = ( mAnchor1->q - mAnchor2->q - mRestLength )
                       * mStiffness; // Positive if tension, negative if compression.
    ...
    double force_mag = ( mAnchor2->q - mAnchor1->q - mRestLength )
                       * mStiffness; // Positive if tension, negative if compression.
    if( ( mSaturation > 0 ) && ( fabs( force_mag ) > mSaturation ) ) {
      if( force_mag > 0 ) {
        mAnchor1->f += mSaturation;
...
void spring_2D::doForce( kte_pass_flag aFlag, const shared_ptr< frame_storage >& aStorage ) {
  RK_UNUSED( aFlag );
  RK_UNUSED( aStorage );
  if( ( !mAnchor1 ) || ( !mAnchor2 ) )
    return;
  ...
  if( ( aFlag == store_kinematics ) && ( aStorage ) ) {
    if( !( aStorage->frame_3D_mapping[mAnchor1] ) )
      aStorage->frame_3D_mapping[mAnchor1]
        = shared_ptr< ReaK::frame_3D< double > >( new frame_3D< double >( ( *mAnchor1 ) ), scoped_deleter() );
    else
      ( *( aStorage->frame_3D_mapping[mAnchor1] ) ) = ( *mAnchor1 );
    if( !( aStorage->frame_3D_mapping[mAnchor2] ) )
      aStorage->frame_3D_mapping[mAnchor2]
        = shared_ptr< ReaK::frame_3D< double > >( new frame_3D< double >( ( *mAnchor2 ) ), scoped_deleter() );
    else
      ( *( aStorage->frame_3D_mapping[mAnchor2] ) ) = ( *mAnchor2 );
  };
};

void spring_3D::doForce( kte_pass_flag aFlag, const shared_ptr< frame_storage >& aStorage ) {
  RK_UNUSED( aFlag );
  RK_UNUSED( aStorage );
  if( ( !mAnchor1 ) || ( !mAnchor2 ) )
    return;
