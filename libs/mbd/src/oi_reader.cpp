...
static void read_sg_into_models( SoSeparator* aRoot, colored_model_3D* aModel, proxy_query_model_3D* aProxy ) {
  ...
  while( !so_group_stack.empty() ) {
    ...
    if( current_node->getTypeId() == SoBaseColor::getClassTypeId() ) {
      ...
      shared_ptr< box > new_box(
        new box( ptmp->getName().getString(), anchor_ptr, *anchor_stack.top(),
                 vect< double, 3 >( ptmp->width.getValue(), ptmp->height.getValue(), ptmp->depth.getValue() ) ) );
      if( aModel )
      ...
      final_pose.addBefore( pose_3D< double >( weak_ptr< pose_3D< double > >(), vect< double, 3 >( 0.0, 0.0, 0.0 ),
                                               quaternion< double >::xrot( M_PI * 0.5 ).getQuaternion() ) );
      shared_ptr< cylinder > new_cyl( new cylinder( ptmp->getName().getString(), anchor_ptr, final_pose,
      ...
      if( !anchor_stack.top()->Parent.expired() )
        anchor_ptr = anchor_stack.top()->Parent.lock();
      shared_ptr< sphere > new_sph(
        new sphere( ptmp->getName().getString(), anchor_ptr, *anchor_stack.top(), ptmp->radius.getValue() ) );
      if( aModel )
      ...
      shared_ptr< box > new_box(
        new box( ptmp->getName().getString(), anchor_ptr, *anchor_stack.top(),
                 vect< double, 3 >( ptmp->size.getValue()[0], ptmp->size.getValue()[1], ptmp->size.getValue()[2] ) ) );
      if( aModel )
        aModel->addElement( color_stack.top(), new_box );
      if( aProxy )
        aProxy->addShape( new_box );
      ...
      anchor_stack.top()->addBefore(
        pose_3D< double >( weak_ptr< pose_3D< double > >(), vect< double, 3 >( 0.0, 0.0, 0.0 ),
                           axis_angle< double >( rot_angle, vect< double, 3 >( rot_axis[0], rot_axis[1], rot_axis[2] ) )
                             .getQuaternion() ) );
      ...
      anchor_stack.top()->addBefore(
        pose_3D< double >( weak_ptr< pose_3D< double > >(), vect< double, 3 >( 0.0, 0.0, 0.0 ), rot ) );
      ...
      anchor_stack.top()->addBefore( pose_3D< double >(
        ...
        vect< double, 3 >( ptmp->translation.getValue()[0], ptmp->translation.getValue()[1],
      ...
      anchor_stack.top()->addBefore( pose_3D< double >(
        weak_ptr< pose_3D< double > >(), vect< double, 3 >( translation[0], translation[1], translation[2] ),
        axis_angle< double >( rot_angle, vect< double, 3 >( rot_axis[0], rot_axis[1], rot_axis[2] ) )
          .getQuaternion() ) );
      ...
      if( ( bbox.getVolume() > std::numeric_limits< float >::epsilon() ) && ( aProxy ) ) {
        ...
        shared_ptr< box > new_box(
          new box( current_node->getName().getString(), anchor_ptr, final_pose, vect< double, 3 >( x, y, z ) ) );
        aProxy->addShape( new_box );
      };
    };
    ...
    if( next_childlist && next_childlist->getLength() ) {
      ...
      anchor_stack.push( shared_ptr< pose_3D< double > >(
        new pose_3D< double >( anchor_stack.top(), vect< double, 3 >( 0.0, 0.0, 0.0 ), quaternion< double >() ) ) );
      if( aModel )
        aModel->addAnchor( anchor_stack.top() );
