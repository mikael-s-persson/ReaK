...
void dry_revolute_joint_2D::doForce(
  ...
  if (!mAngle) {
    ...
    if (abs(mAngle->q_dot) > mSlipVelocity) {
      ...
      mAngle->f += mEnd->Torque - mAngle->q_dot / mSlipVelocity *
                                      mStictionCoef * norm_2(tmp_f);
    }
  }
...
void dry_revolute_joint_3D::doForce(
    kte_pass_flag aFlag, const std::shared_ptr<frame_storage>& aStorage) {
  if ((!mEnd) || (!mBase)) {
    return;
  }
  ...
  if (!mAngle) {
    ...
    if (abs(mAngle->q_dot) > mSlipVelocity) {
      ...
      mAngle->f +=
          tmp_t - mAngle->q_dot / mSlipVelocity * mStictionCoef * norm_2(tmp_f);
    }
