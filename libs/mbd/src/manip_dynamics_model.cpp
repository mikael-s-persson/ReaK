...
void manipulator_dynamics_model::computeOutput(
  ...
  unsigned int i = 0;
  for (auto& mOutput : mOutputs) {
    for (unsigned int k = 0; k < mOutput->getOutputCount(); ++k) {
      aOutput[i++] = mOutput->getOutput(k);
    }
  }
}
...
void manipulator_dynamics_model::setInput(const ReaK::vect_n<double>& aInput) {
  ...
  for (auto& mInput : mInputs) {
    for (unsigned int k = 0; k < mInput->getInputCount(); ++k) {
      mInput->setInput(k, aInput[i++]);
    }
  }
}
...
void manipulator_dynamics_model::computeStateRate(double aTime,
  ...
  unsigned int j = 0;
  for (auto it = mCoords.begin(); it < mCoords.end(); ++it, ++j) {
    aStateRate[j] = (*it)->q_dot;
  }

  for (auto it = mFrames2D.begin(); it < mFrames2D.end(); ++it) {
  ...
  for (auto it = mFrames3D.begin(); it < mFrames3D.end(); ++it) {
    aStateRate[j] = (*it)->Velocity[0];
    ++j;
    aStateRate[j] = (*it)->Velocity[1];
    ++j;
