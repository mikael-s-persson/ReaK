...
chaser_target_data::chaser_target_data() : named_object() { this->setName( "chaser_target_model_data" ); };


void chaser_target_data::load_chaser( const std::string& fileName ) {

  ( *serialization::open_iarchive( fileName ) ) >> chaser_base_frame >> chaser_kin_model >> chaser_jt_limits
...
void chaser_target_data::save_chaser( const std::string& fileName ) const {
  ...
  ( *serialization::open_oarchive( fileName ) ) << chaser_base_frame << chaser_kin_model << chaser_jt_limits
                                                << chaser_geom_model << chaser_proxy;
};
...
void chaser_target_data::load_target( const std::string& fileName ) {
  ...
  ( *serialization::open_iarchive( fileName ) ) >> target_base >> target_kin_model >> target_frame >> target_geom_model
    >> target_proxy;
...
void chaser_target_data::save_target( const std::string& fileName ) const {
  ...
  ( *serialization::open_oarchive( fileName ) ) << shared_ptr< frame_3D< double > >() << target_kin_model
                                                << target_frame << target_geom_model << target_proxy;
};
...
void chaser_target_data::create_chaser_target_proxy() {
  ...
  if( chaser_proxy && target_proxy ) {
    chaser_target_proxy = shared_ptr< geom::proxy_query_pair_3D >(
      new geom::proxy_query_pair_3D( "chaser_target_proxy", chaser_proxy, target_proxy ) );
...
void chaser_target_data::create_chaser_env_proxies() {
  ...
  if( chaser_proxy ) {
    ...
    for( std::size_t i = chaser_env_proxies.size(); i < env_proxy_models.size(); ++i ) {
      chaser_env_proxies.push_back( shared_ptr< geom::proxy_query_pair_3D >( new geom::proxy_query_pair_3D(
        "chaser_env_proxy:" + env_proxy_models[i]->getName(), chaser_proxy, env_proxy_models[i] ) ) );
    };
  };
};
...
void chaser_target_data::create_target_env_proxies() {
  ...
  if( target_proxy ) {
    ...
    for( std::size_t i = target_env_proxies.size(); i < env_proxy_models.size(); ++i ) {
      target_env_proxies.push_back( shared_ptr< geom::proxy_query_pair_3D >( new geom::proxy_query_pair_3D(
        "target_env_proxy:" + env_proxy_models[i]->getName(), target_proxy, env_proxy_models[i] ) ) );
    };
  };
};
...
void RK_CALL chaser_target_data::save( serialization::oarchive& A, unsigned int ) const {
  ...
  A& RK_SERIAL_SAVE_WITH_NAME( chaser_base_frame ) & RK_SERIAL_SAVE_WITH_NAME( chaser_kin_model )
    & RK_SERIAL_SAVE_WITH_NAME( chaser_jt_limits ) & RK_SERIAL_SAVE_WITH_NAME( chaser_proxy )
    & RK_SERIAL_SAVE_WITH_NAME( chaser_geom_model ) & RK_SERIAL_SAVE_WITH_NAME( target_kin_model )
    & RK_SERIAL_SAVE_WITH_NAME( target_frame ) & RK_SERIAL_SAVE_WITH_NAME( target_proxy )
    & RK_SERIAL_SAVE_WITH_NAME( target_geom_model ) & RK_SERIAL_SAVE_WITH_NAME( chaser_target_proxy )
    & RK_SERIAL_SAVE_WITH_NAME( env_geom_models ) & RK_SERIAL_SAVE_WITH_NAME( env_proxy_models )
    & RK_SERIAL_SAVE_WITH_NAME( chaser_env_proxies ) & RK_SERIAL_SAVE_WITH_NAME( target_env_proxies );
};

void RK_CALL chaser_target_data::load( serialization::iarchive& A, unsigned int ) {
  named_object::load( A, named_object::getStaticObjectType()->TypeVersion() );
  A& RK_SERIAL_LOAD_WITH_NAME( chaser_base_frame ) & RK_SERIAL_LOAD_WITH_NAME( chaser_kin_model )
    & RK_SERIAL_LOAD_WITH_NAME( chaser_jt_limits ) & RK_SERIAL_LOAD_WITH_NAME( chaser_proxy )
    & RK_SERIAL_LOAD_WITH_NAME( chaser_geom_model ) & RK_SERIAL_LOAD_WITH_NAME( target_kin_model )
    & RK_SERIAL_LOAD_WITH_NAME( target_frame ) & RK_SERIAL_LOAD_WITH_NAME( target_proxy )
    & RK_SERIAL_LOAD_WITH_NAME( target_geom_model ) & RK_SERIAL_LOAD_WITH_NAME( chaser_target_proxy )
    & RK_SERIAL_LOAD_WITH_NAME( env_geom_models ) & RK_SERIAL_LOAD_WITH_NAME( env_proxy_models )
    & RK_SERIAL_LOAD_WITH_NAME( chaser_env_proxies ) & RK_SERIAL_LOAD_WITH_NAME( target_env_proxies );
};
};
};
