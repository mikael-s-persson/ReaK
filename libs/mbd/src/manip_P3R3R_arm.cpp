...
manip_P3R3R_kinematics::manip_P3R3R_kinematics(
    ...
    : inverse_kinematics_model(aName),
      ...
      m_track_coord(std::make_shared<gen_coord<double>>()),
      ...
      m_arm_model(aName + "_arm", m_output_frame, aBaseToShoulder,
                  aShoulderToElbow, aElbowToJoint4, aJoint4ToWrist,
                  aWristToFlange,
                  vect_n<double>(aJointLowerBounds.begin() + 1,
                                 aJointLowerBounds.end()),
                  vect_n<double>(aJointUpperBounds.begin() + 1,
                                 aJointUpperBounds.end())) {
  ...
  auto track_jacobian = std::make_shared<jacobian_gen_3D<double>>();

  // create prismatic joint
  m_track_joint = std::make_shared<prismatic_joint_3D>(
      "manip_P3R3R_track", m_track_coord, vect<double, 3>(1.0, 0.0, 0.0),
      m_base_frame, m_output_frame, track_jacobian);

  m_chain = std::make_shared<kte_map_chain>("manip_P3R3R_kin_model");
...
void manip_P3R3R_kinematics::doInverseMotion() {
  ...
  frame_3D<double> EE_fr =
      m_arm_model.getDependentFrame3D(0)->mFrame->getFrameRelativeTo(
          m_base_frame);
  ...
  double elbow_to_wrist_dist =
      m_arm_model.getElbowToJoint4() + m_arm_model.getJoint4ToWrist();

  double perp_wrist_dist_sqr =
      wrist_pos[2] * wrist_pos[2] + wrist_pos[1] * wrist_pos[1];
  ...
  double j23_lower_bound =
      m_arm_model.joint_lower_bounds[1] + m_arm_model.joint_lower_bounds[2];
  double j23_upper_bound =
      m_arm_model.joint_upper_bounds[1] + m_arm_model.joint_upper_bounds[2];
  double c23_max = 1.0;
  ...
  if (wrist_pos[2] >= 0.0) {
    double elbow_to_desired_height_min =
        wrist_pos[2] - m_arm_model.getShoulderToElbow() * c2_max;
    if (elbow_to_desired_height_min >
        (elbow_to_wrist_dist - extend_epsilon) * c23_max) {
    ...
    double elbow_to_desired_height_min =
        wrist_pos[2] - m_arm_model.getShoulderToElbow() * c2_min;
    if (elbow_to_desired_height_min <
        (elbow_to_wrist_dist - extend_epsilon) * c23_min) {
      throw optim::infeasible_problem(
          "Inverse kinematics problem is infeasible! End-effector pose is "
          "out-of-reach! "
  ...
  double shoulder_to_wrist_max_sqr =
      m_arm_model.getShoulderToElbow() + elbow_to_wrist_dist * c3_max;
  shoulder_to_wrist_max_sqr =
      shoulder_to_wrist_max_sqr * shoulder_to_wrist_max_sqr +
  ...
  double shoulder_to_wrist_min_sqr =
      m_arm_model.getShoulderToElbow() + elbow_to_wrist_dist * c3_min;
  shoulder_to_wrist_min_sqr =
      shoulder_to_wrist_min_sqr * shoulder_to_wrist_min_sqr +
  ...
  if (wrist_pos[0] - x_max > m_track_upper_bound) {
    throw optim::infeasible_problem(
        "Inverse kinematics problem is infeasible! End-effector pose is "
        "out-of-reach! "
        "Desired wrist position is too far ahead of the track (beyond upper "
        "track "
        "limit).");
  }
  if (wrist_pos[0] + x_max < m_track_lower_bound) {
    throw optim::infeasible_problem(
        "Inverse kinematics problem is infeasible! End-effector pose is "
        "out-of-reach! "
        "Desired wrist position is too far behind the track (beyond lower "
        "track limit).");
  }
  if ((wrist_pos[0] + x_min > m_track_upper_bound) &&
      (wrist_pos[0] - x_min < m_track_lower_bound)) {
    throw optim::infeasible_problem(
        "Inverse kinematics problem is infeasible! End-effector pose is "
        "out-of-reach! The "
...
void manip_P3R3R_kinematics::getJacobianMatrix(
...
void manip_P3R3R_kinematics::getJacobianMatrixAndDerivative(
...
void manip_P3R3R_kinematics::setJointPositionLowerBounds(
    const vect_n<double>& aJointLowerBounds) {
  m_track_lower_bound = aJointLowerBounds[0];
  m_arm_model.setJointPositionLowerBounds(
...
void manip_P3R3R_kinematics::setJointPositionUpperBounds(
    const vect_n<double>& aJointUpperBounds) {
  m_track_upper_bound = aJointUpperBounds[0];
  m_arm_model.setJointPositionUpperBounds(
...
vect_n<double> manip_P3R3R_kinematics::getJointPositions() const {
  ...
  return {m_track_coord->q, arm_jtpos[0], arm_jtpos[1], arm_jtpos[2],
          arm_jtpos[3],     arm_jtpos[4], arm_jtpos[5]};
}

void manip_P3R3R_kinematics::setJointPositions(
    const vect_n<double>& aJointPositions) {
  m_track_coord->q = aJointPositions[0];
  m_arm_model.setJointPositions(
      vect_n<double>(aJointPositions.begin() + 1, aJointPositions.end()));
}

vect_n<double> manip_P3R3R_kinematics::getJointVelocities() const {
  vect_n<double> arm_jtvel = m_arm_model.getJointVelocities();
  return {m_track_coord->q_dot, arm_jtvel[0], arm_jtvel[1], arm_jtvel[2],
          arm_jtvel[3],         arm_jtvel[4], arm_jtvel[5]};
}

void manip_P3R3R_kinematics::setJointVelocities(
    const vect_n<double>& aJointVelocities) {
  m_track_coord->q_dot = aJointVelocities[0];
  m_arm_model.setJointVelocities(
      vect_n<double>(aJointVelocities.begin() + 1, aJointVelocities.end()));
}
...
vect_n<double> manip_P3R3R_kinematics::getJointAccelerations() const {
  ...
  return {m_track_coord->q_ddot, arm_jtacc[0], arm_jtacc[1], arm_jtacc[2],
          arm_jtacc[3],          arm_jtacc[4], arm_jtacc[5]};
}

void manip_P3R3R_kinematics::setJointAccelerations(
    const vect_n<double>& aJointAccelerations) {
  m_track_coord->q_ddot = aJointAccelerations[0];
  m_arm_model.setJointAccelerations(vect_n<double>(
      aJointAccelerations.begin() + 1, aJointAccelerations.end()));
}
...
void manip_P3R3R_kinematics::save(serialization::oarchive& A,
  ...
  A& RK_SERIAL_SAVE_WITH_NAME(m_base_frame) &
      ...
      RK_SERIAL_SAVE_WITH_NAME(m_track_joint) &
...
void manip_P3R3R_kinematics::load(serialization::iarchive& A,
  ...
  A& RK_SERIAL_LOAD_WITH_NAME(m_base_frame) &
      ...
      RK_SERIAL_LOAD_WITH_NAME(m_track_joint) &
