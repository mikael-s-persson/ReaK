...
void inertia_gen::doMotion( kte_pass_flag aFlag, const shared_ptr< frame_storage >& aStorage ) {
  ...
  if( ( aFlag == store_kinematics ) && ( aStorage ) ) {
    if( !( aStorage->gen_coord_mapping[mCenterOfMass->mFrame] ) )
      aStorage->gen_coord_mapping[mCenterOfMass->mFrame] = shared_ptr< ReaK::gen_coord< double > >(
        new ReaK::gen_coord< double >( ( *mCenterOfMass->mFrame ) ), ReaK::scoped_deleter() );
    else
      ( *( aStorage->gen_coord_mapping[mCenterOfMass->mFrame] ) ) = ( *mCenterOfMass->mFrame );
  };
};

void inertia_gen::doForce( kte_pass_flag aFlag, const shared_ptr< frame_storage >& aStorage ) {
  RK_UNUSED( aFlag );
  RK_UNUSED( aStorage );
  if( ( !mCenterOfMass ) || ( !mCenterOfMass->mFrame ) )
    return;
  ...
  if( ( aFlag == store_kinematics ) && ( aStorage ) ) {
    if( !( aStorage->frame_2D_mapping[mCenterOfMass->mFrame] ) )
      aStorage->frame_2D_mapping[mCenterOfMass->mFrame]
        = shared_ptr< frame_2D< double > >( new frame_2D< double >( ( *mCenterOfMass->mFrame ) ), scoped_deleter() );
    else
      ( *( aStorage->frame_2D_mapping[mCenterOfMass->mFrame] ) ) = ( *mCenterOfMass->mFrame );
  };
};

void inertia_2D::doForce( kte_pass_flag aFlag, const shared_ptr< frame_storage >& aStorage ) {
  RK_UNUSED( aFlag );
  RK_UNUSED( aStorage );
  if( ( !mCenterOfMass ) || ( !mCenterOfMass->mFrame ) )
    return;
  ...
  if( ( aFlag == store_kinematics ) && ( aStorage ) ) {
    if( !( aStorage->frame_3D_mapping[mCenterOfMass->mFrame] ) )
      aStorage->frame_3D_mapping[mCenterOfMass->mFrame]
        = shared_ptr< frame_3D< double > >( new frame_3D< double >( ( *mCenterOfMass->mFrame ) ), scoped_deleter() );
    else
      ( *( aStorage->frame_3D_mapping[mCenterOfMass->mFrame] ) ) = ( *mCenterOfMass->mFrame );
  };
};

void inertia_3D::doForce( kte_pass_flag aFlag, const shared_ptr< frame_storage >& aStorage ) {
  RK_UNUSED( aFlag );
  RK_UNUSED( aStorage );
  if( ( !mCenterOfMass ) || ( !mCenterOfMass->mFrame ) )
    return;
  ...
  mCenterOfMass->mFrame->Torque -= mInertiaTensor * global_frame.AngAcceleration
