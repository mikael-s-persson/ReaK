...
void flexible_beam_2D::doMotion( kte_pass_flag aFlag, const shared_ptr< frame_storage >& aStorage ) {
  ...
  if( mObjectFrame ) {
    ...
    mObjectFrame->Rotation
      = rot_mat_2D< double >( ( mAnchor1->Rotation.getAngle() + mAnchor2->Rotation.getAngle() ) * 0.5 );
    ...
    if( mObjectFrame ) {
      if( !( aStorage->frame_2D_mapping[mObjectFrame] ) )
        aStorage->frame_2D_mapping[mObjectFrame]
          = shared_ptr< frame_2D< double > >( new frame_2D< double >( ( *mObjectFrame ) ), scoped_deleter() );
      else
...
void flexible_beam_3D::doMotion( kte_pass_flag aFlag, const shared_ptr< frame_storage >& aStorage ) {
  ...
  if( mObjectFrame ) {
    ...
    mObjectFrame->AngAcceleration = ( ( R_a1 * mAnchor1->AngAcceleration + R_a2 * mAnchor2->AngAcceleration ) * 0.5 )
                                    * R_o;
  };
    ...
    if( mObjectFrame ) {
      if( !( aStorage->frame_3D_mapping[mObjectFrame] ) )
        aStorage->frame_3D_mapping[mObjectFrame]
          = shared_ptr< frame_3D< double > >( new frame_3D< double >( ( *mObjectFrame ) ), scoped_deleter() );
      else
