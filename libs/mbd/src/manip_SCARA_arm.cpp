...
manip_SCARA_kinematics::manip_SCARA_kinematics(
    const std::string& aName, std::shared_ptr<frame_3D<double>> aBaseFrame,
    double aLink1Length, double aLink2Length, double aLink3Height)
  ...
  auto joint_1 = std::make_shared<kte::revolute_joint_3D>(
      "manip_SCARA_joint_1", m_joints[0], vect<double, 3>(0.0, 0.0, 1.0),
  ...
  auto link_1 = std::make_shared<kte::rigid_link_3D>(
      "manip_SCARA_link_1", joint_1_end, joint_2_base,
  ...
  auto joint_2 = std::make_shared<kte::revolute_joint_3D>(
      "manip_SCARA_joint_2", m_joints[1], vect<double, 3>(0.0, 0.0, 1.0),
      joint_2_base, joint_2_end, joint_2_jacobian);

  // create link
  auto link_2 = std::make_shared<kte::rigid_link_3D>(
      "manip_SCARA_link_2", joint_2_end, joint_3_base,
      pose_3D<double>(std::weak_ptr<pose_3D<double>>(),
                      vect<double, 3>(m_link2_length, 0.0, 0.0),
                      quaternion<double>()));

  // create revolute joint
  auto joint_3 = std::make_shared<kte::prismatic_joint_3D>(
      "manip_SCARA_joint_3", m_joints[2], vect<double, 3>(0.0, 0.0, 1.0),
      joint_3_base, joint_3_end, joint_3_jacobian);

  // create link
  auto link_3 = std::make_shared<kte::rigid_link_3D>(
      "manip_SCARA_link_3", joint_3_end, arm_EE,
      pose_3D<double>(std::weak_ptr<pose_3D<double>>(),
  ...
  m_chain = std::make_shared<kte_map_chain>("manip_SCARA_kin_model");
...
void manip_SCARA_kinematics::getJacobianMatrix(
...
void manip_SCARA_kinematics::getJacobianMatrixAndDerivative(
...
void manip_SCARA_kinematics::setJointPositions(
...
void manip_SCARA_kinematics::setJointVelocities(
...
void manip_SCARA_kinematics::setJointAccelerations(
...
void manip_SCARA_kinematics::setDependentPositions(
...
void manip_SCARA_kinematics::setDependentVelocities(
...
void manip_SCARA_kinematics::setDependentAccelerations(
...
void manip_SCARA_kinematics::save(serialization::oarchive& A,
...
void manip_SCARA_kinematics::load(serialization::iarchive& A,
                                  unsigned int /*unused*/) {
