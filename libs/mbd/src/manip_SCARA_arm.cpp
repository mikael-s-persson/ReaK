...
manip_SCARA_kinematics::manip_SCARA_kinematics( const std::string& aName,
                                                ...
                                                double aLink2Length, double aLink3Height )
    : inverse_kinematics_model( aName ), m_base_frame( aBaseFrame ), m_link1_length( aLink1Length ),
      m_link2_length( aLink2Length ), m_link3_height( aLink3Height ) {
  if( !m_base_frame )
  ...
  shared_ptr< kte::revolute_joint_3D > joint_1(
    new kte::revolute_joint_3D( "manip_SCARA_joint_1", m_joints[0], vect< double, 3 >( 0.0, 0.0, 1.0 ), m_base_frame,
  ...
  shared_ptr< kte::rigid_link_3D > link_1(
    new kte::rigid_link_3D( "manip_SCARA_link_1", joint_1_end, joint_2_base,
  ...
  shared_ptr< kte::revolute_joint_3D > joint_2(
    new kte::revolute_joint_3D( "manip_SCARA_joint_2", m_joints[1], vect< double, 3 >( 0.0, 0.0, 1.0 ), joint_2_base,
                                joint_2_end, joint_2_jacobian ),
    scoped_deleter() );

  // create link
  shared_ptr< kte::rigid_link_3D > link_2(
    new kte::rigid_link_3D( "manip_SCARA_link_2", joint_2_end, joint_3_base,
                            pose_3D< double >( weak_ptr< pose_3D< double > >(),
  ...
  // create revolute joint
  shared_ptr< kte::prismatic_joint_3D > joint_3(
    new kte::prismatic_joint_3D( "manip_SCARA_joint_3", m_joints[2], vect< double, 3 >( 0.0, 0.0, 1.0 ), joint_3_base,
                                 joint_3_end, joint_3_jacobian ),
    scoped_deleter() );

  // create link
  shared_ptr< kte::rigid_link_3D > link_3(
    new kte::rigid_link_3D( "manip_SCARA_link_3", joint_3_end, arm_EE,
                            pose_3D< double >( weak_ptr< pose_3D< double > >(),
...
void manip_SCARA_kinematics::doDirectMotion() { m_chain->doMotion(); };
...
void manip_SCARA_kinematics::doInverseMotion() {
  ...
  if( w_dist > m_link1_length + m_link2_length )
    throw optim::infeasible_problem( "Inverse kinematics problem is infeasible! Desired end-effector pose of the SCARA "
...
void manip_SCARA_kinematics::getJacobianMatrixAndDerivative( mat< double, mat_structure::rectangular >& Jac,
...
void RK_CALL manip_SCARA_kinematics::save( serialization::oarchive& A, unsigned int ) const {
  ...
  A& RK_SERIAL_SAVE_WITH_NAME( m_base_frame ) & RK_SERIAL_SAVE_WITH_NAME( m_joints ) & RK_SERIAL_SAVE_WITH_NAME( m_EE )
    ...
    & RK_SERIAL_SAVE_WITH_NAME( m_link3_height ) & RK_SERIAL_SAVE_WITH_NAME( m_chain );
};
...
void RK_CALL manip_SCARA_kinematics::load( serialization::iarchive& A, unsigned int ) {
  ...
  A& RK_SERIAL_LOAD_WITH_NAME( m_base_frame ) & RK_SERIAL_LOAD_WITH_NAME( m_joints ) & RK_SERIAL_LOAD_WITH_NAME( m_EE )
    ...
    & RK_SERIAL_LOAD_WITH_NAME( m_link3_height ) & RK_SERIAL_LOAD_WITH_NAME( m_chain );
};
};
};
