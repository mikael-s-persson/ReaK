...
manip_SSRMS_kinematics::manip_SSRMS_kinematics( const std::string& aName,
    ...
    : inverse_kinematics_model( aName ), m_base_frame( aBaseFrame ), link_lengths( aLinkLengths ),
      joint_offsets( aJointOffsets ), joint_lower_bounds( aJointLowerBounds ), joint_upper_bounds( aJointUpperBounds ) {
  if( !m_base_frame )
  ...
  // create link
  shared_ptr< rigid_link_3D > link_0(
    new rigid_link_3D( "manip_SSRMS_link_0", m_base_frame, joint_1_base,
                       pose_3D< double >( weak_ptr< pose_3D< double > >(),
                                          vect< double, 3 >( 0.0, 0.0, joint_offsets[0] ), quaternion< double >() ) ),
    scoped_deleter() );

  // create revolute joint
  shared_ptr< revolute_joint_3D > joint_1( new revolute_joint_3D( "manip_SSRMS_joint_1", m_joints[0],
                                                                  vect< double, 3 >( 0.0, 0.0, 1.0 ), joint_1_base,
                                                                  joint_1_end, joint_1_jacobian ),
                                           scoped_deleter() );

  // create link
  shared_ptr< rigid_link_3D > link_1(
    new rigid_link_3D( "manip_SSRMS_link_1", joint_1_end, joint_2_base,
                       pose_3D< double >( weak_ptr< pose_3D< double > >(),
  ...
  // create revolute joint
  shared_ptr< revolute_joint_3D > joint_2( new revolute_joint_3D( "manip_SSRMS_joint_2", m_joints[1],
                                                                  vect< double, 3 >( 0.0, -1.0, 0.0 ), joint_2_base,
                                                                  joint_2_end, joint_2_jacobian ),
                                           scoped_deleter() );

  // create link
  shared_ptr< rigid_link_3D > link_2(
    new rigid_link_3D( "manip_SSRMS_link_2", joint_2_end, joint_3_base,
                       pose_3D< double >( weak_ptr< pose_3D< double > >(),
  ...
  // create revolute joint
  shared_ptr< revolute_joint_3D > joint_3( new revolute_joint_3D( "manip_SSRMS_joint_3", m_joints[2],
                                                                  vect< double, 3 >( 0.0, 0.0, 1.0 ), joint_3_base,
                                                                  joint_3_end, joint_3_jacobian ),
                                           scoped_deleter() );

  // create link
  shared_ptr< rigid_link_3D > link_3(
    new rigid_link_3D( "manip_SSRMS_link_3", joint_3_end, joint_4_base,
                       pose_3D< double >( weak_ptr< pose_3D< double > >(),
  ...
  // create revolute joint
  shared_ptr< revolute_joint_3D > joint_4( new revolute_joint_3D( "manip_SSRMS_joint_4", m_joints[3],
                                                                  vect< double, 3 >( 0.0, 0.0, 1.0 ), joint_4_base,
                                                                  joint_4_end, joint_4_jacobian ),
                                           scoped_deleter() );

  // create link
  shared_ptr< rigid_link_3D > link_4(
    new rigid_link_3D( "manip_SSRMS_link_4", joint_4_end, joint_5_base,
                       pose_3D< double >( weak_ptr< pose_3D< double > >(),
  ...
  // create revolute joint
  shared_ptr< revolute_joint_3D > joint_5( new revolute_joint_3D( "manip_SSRMS_joint_5", m_joints[4],
                                                                  vect< double, 3 >( 0.0, 0.0, 1.0 ), joint_5_base,
                                                                  joint_5_end, joint_5_jacobian ),
                                           scoped_deleter() );

  // create link
  shared_ptr< rigid_link_3D > link_5(
    new rigid_link_3D( "manip_SSRMS_link_5", joint_5_end, joint_6_base,
                       pose_3D< double >( weak_ptr< pose_3D< double > >(),
                                          vect< double, 3 >( link_lengths[4], 0.0, 0.0 ), quaternion< double >() ) ),
    scoped_deleter() );

  // create revolute joint
  shared_ptr< kte::revolute_joint_3D > joint_6(
    new kte::revolute_joint_3D( "manip_SSRMS_joint_6", m_joints[5], vect< double, 3 >( 0.0, -1.0, 0.0 ), joint_6_base,
                                joint_6_end, joint_6_jacobian ),
    scoped_deleter() );

  // create link
  shared_ptr< rigid_link_3D > link_6(
    new rigid_link_3D( "manip_SSRMS_link_6", joint_6_end, joint_7_base,
                       pose_3D< double >( weak_ptr< pose_3D< double > >(),
  ...
  // create revolute joint
  shared_ptr< kte::revolute_joint_3D > joint_7(
    new kte::revolute_joint_3D( "manip_SSRMS_joint_7", m_joints[5], vect< double, 3 >( 0.0, 0.0, 1.0 ), joint_7_base,
                                joint_7_end, joint_7_jacobian ),
    scoped_deleter() );

  // create link
  shared_ptr< rigid_link_3D > link_7(
    new rigid_link_3D( "manip_SSRMS_link_7", joint_7_end, arm_EE,
                       pose_3D< double >( weak_ptr< pose_3D< double > >(),
                                          vect< double, 3 >( 0.0, 0.0, joint_offsets[6] ), quaternion< double >() ) ),
    scoped_deleter() );
  ...
  *m_chain << link_0 << joint_1 << link_1 << joint_2 << link_2 << joint_3 << link_3 << joint_4 << link_4 << joint_5
           << link_5 << joint_6 << link_6 << joint_7 << link_7;
};


void manip_SSRMS_kinematics::doDirectMotion() { m_chain->doMotion(); };
...
void manip_SSRMS_kinematics::doInverseMotion() {
  ...
  if( 1.0 - fabs( EE_z_axis[2] ) > pos_epsilon ) {
    ...
    if( fabs( wrist_plane_to_base_offset ) < plane_offset ) {
      ...
      w2_y_axis = wrist_to_wrist_planar
                  - plane_offset * ( cos( w2_w2w_ang ) * y1_cross_w2w
                                     + ( sin( w2_w2w_ang ) / w2w_planar_mag ) * wrist_to_wrist_planar );
      ...
      w1_y_axis = wrist_to_wrist_planar
                  - plane_offset * ( cos( w1_w2w_ang ) * y2_cross_w2w
                                     - ( sin( w1_w2w_ang ) / w2w_planar_mag ) * wrist_to_wrist_planar );
    ...
    if( w2w_planar_mag < plane_offset ) {
      throw optim::infeasible_problem( "Inverse kinematics problem is infeasible! End-effector pose is out-of-reach! "
                                       "Desired wrist position is too close to the center axis of the SSRMS arm." );
    };
    // now the planar w2w vector should be in the w2 plane.
  ...
  vect< double, 3 > shoulder_to_shoulder = wrist_to_wrist - link_lengths[1] * w1_x_axis - link_lengths[4] * w2_x_axis;

  shoulder_to_shoulder -= joint_offsets[1] * w1_y_axis;
  shoulder_to_shoulder -= joint_offsets[5] * w2_y_axis;
  double a3_offset = atan2( -shoulder_to_shoulder * w1_y_axis, shoulder_to_shoulder * w1_x_axis );
  double a5_offset = atan2( shoulder_to_shoulder * w2_y_axis, shoulder_to_shoulder * w2_x_axis );
...
void manip_SSRMS_kinematics::getJacobianMatrixAndDerivative( mat< double, mat_structure::rectangular >& Jac,
...
vect_n< double > manip_SSRMS_kinematics::getJointPositions() const {
  return vect_n< double >( m_joints[0]->q, m_joints[1]->q, m_joints[2]->q, m_joints[3]->q, m_joints[4]->q,
                           m_joints[5]->q, m_joints[6]->q );
};
...
vect_n< double > manip_SSRMS_kinematics::getJointVelocities() const {
  return vect_n< double >( m_joints[0]->q_dot, m_joints[1]->q_dot, m_joints[2]->q_dot, m_joints[3]->q_dot,
                           m_joints[4]->q_dot, m_joints[5]->q_dot, m_joints[6]->q_dot );
};
...
vect_n< double > manip_SSRMS_kinematics::getJointAccelerations() const {
  return vect_n< double >( m_joints[0]->q_ddot, m_joints[1]->q_ddot, m_joints[2]->q_ddot, m_joints[3]->q_ddot,
                           m_joints[4]->q_ddot, m_joints[5]->q_ddot, m_joints[6]->q_ddot );
};
...
void RK_CALL manip_SSRMS_kinematics::save( serialization::oarchive& A, unsigned int ) const {
  ...
  A& RK_SERIAL_SAVE_WITH_NAME( m_base_frame ) & RK_SERIAL_SAVE_WITH_NAME( m_joints ) & RK_SERIAL_SAVE_WITH_NAME( m_EE )
    & RK_SERIAL_SAVE_WITH_NAME( link_lengths ) & RK_SERIAL_SAVE_WITH_NAME( joint_offsets )
    & RK_SERIAL_SAVE_WITH_NAME( joint_lower_bounds ) & RK_SERIAL_SAVE_WITH_NAME( joint_upper_bounds )
...
void RK_CALL manip_SSRMS_kinematics::load( serialization::iarchive& A, unsigned int ) {
  ...
  A& RK_SERIAL_LOAD_WITH_NAME( m_base_frame ) & RK_SERIAL_LOAD_WITH_NAME( m_joints ) & RK_SERIAL_LOAD_WITH_NAME( m_EE )
    & RK_SERIAL_LOAD_WITH_NAME( link_lengths ) & RK_SERIAL_LOAD_WITH_NAME( joint_offsets )
    & RK_SERIAL_LOAD_WITH_NAME( joint_lower_bounds ) & RK_SERIAL_LOAD_WITH_NAME( joint_upper_bounds )
