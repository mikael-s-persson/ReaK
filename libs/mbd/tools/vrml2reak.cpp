...
int main(int argc, char** argv) {
  ...
  io_options.add_options()("input-file,i", po::value<std::string>(),
  ...
  format_options.add_options()("xml,x",
                               "output an XML serialization format for the "
                               "ReaK geometry (this is the default format)")(
      "bin,b", "output a binary serialization format for the ReaK geometry")(
      "proto-buf,p",
      "output a proto-buf serialization format for the ReaK geometry");

  po::options_description geom_options("Geometry options");
  geom_options.add_options()("geometry,g",
                             "output the colored model of the geometry (for "
                             "rendering) (this is the default output)")(
      "proxy-query,q",
  ...
  if (vm.count("input-file")) {
    file_source = std::shared_ptr<std::istream>(
        new std::ifstream(vm["input-file"].as<std::string>().c_str()));
    if (file_source->fail()) {
  ...
  if (vm.count("output-file")) {
    file_dest = std::shared_ptr<std::ostream>(
        new std::ofstream(vm["output-file"].as<std::string>().c_str()));
    if (file_dest->fail()) {
  ...
  if (vm.count("geometry")) {
    geom_model = std::shared_ptr<colored_model_3D>(
        new colored_model_3D("geometric_model"));
    if (vm.count("proxy-query"))
      proxy_model = std::shared_ptr<proxy_query_model_3D>(
          new proxy_query_model_3D("proximity_query_model"));
  } else {
    if (vm.count("proxy-query"))
      proxy_model = std::shared_ptr<proxy_query_model_3D>(
          new proxy_query_model_3D("proximity_query_model"));
  ...
  if (vm.count("bin")) {
    serial_dest = std::shared_ptr<serialization::oarchive>(
        new serialization::bin_oarchive(*file_dest));
  } else if (vm.count("proto-buf")) {
    serial_dest = std::shared_ptr<serialization::oarchive>(
        new serialization::protobuf_oarchive(*file_dest));
  } else {
    serial_dest = std::shared_ptr<serialization::oarchive>(
        new serialization::xml_oarchive(*file_dest));
  };
