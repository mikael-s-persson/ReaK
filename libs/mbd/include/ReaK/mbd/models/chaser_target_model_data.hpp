...
class chaser_target_data : public named_object {
  public:
    ...
    /** Holds the target's frame, i.e., the goal of the interception is that the chaser's end-effector frame meets this target frame. */
    ...
    /** Holds the environment's geometric models, i.e., for displaying, in 3D, objects of the environment (static in the chaser's base-frame). */
    std::vector< shared_ptr< geom::colored_model_3D > > env_geom_models;
    /** Holds the environment's proximity-query models, i.e., for checking collisions involving objects of the environment (static in the chaser's base-frame). */
    std::vector< shared_ptr< geom::proxy_query_model_3D > > env_proxy_models;
    
    /** Holds the chaser vs. environment proximity-query pairs, i.e., for checking collisions between the chaser and the environment. */
    std::vector< shared_ptr< geom::proxy_query_pair_3D > > chaser_env_proxies;
    /** Holds the target vs. environment proximity-query pairs, i.e., for checking collisions between the target and the environment. */
