...
struct joint_limits_collection : public named_object {
   ...
   * Default constructor.
   */
  joint_limits_collection( const std::string& aName = "" ) : named_object() { this->setName( aName ); };
  ...
  *******************************************************************************/
  ...
  virtual void RK_CALL save( ReaK::serialization::oarchive& A, unsigned int ) const {
    ...
    A& RK_SERIAL_SAVE_WITH_NAME( gen_speed_limits ) & RK_SERIAL_SAVE_WITH_NAME( gen_accel_limits )
      & RK_SERIAL_SAVE_WITH_NAME( gen_jerk_limits ) & RK_SERIAL_SAVE_WITH_NAME( frame2D_speed_limits )
      & RK_SERIAL_SAVE_WITH_NAME( frame2D_accel_limits ) & RK_SERIAL_SAVE_WITH_NAME( frame2D_jerk_limits )
      & RK_SERIAL_SAVE_WITH_NAME( frame3D_speed_limits ) & RK_SERIAL_SAVE_WITH_NAME( frame3D_accel_limits )
      & RK_SERIAL_SAVE_WITH_NAME( frame3D_jerk_limits );
  };
  virtual void RK_CALL load( ReaK::serialization::iarchive& A, unsigned int ) {
    named_object::load( A, named_object::getStaticObjectType()->TypeVersion() );
    A& RK_SERIAL_LOAD_WITH_NAME( gen_speed_limits ) & RK_SERIAL_LOAD_WITH_NAME( gen_accel_limits )
      & RK_SERIAL_LOAD_WITH_NAME( gen_jerk_limits ) & RK_SERIAL_LOAD_WITH_NAME( frame2D_speed_limits )
      & RK_SERIAL_LOAD_WITH_NAME( frame2D_accel_limits ) & RK_SERIAL_LOAD_WITH_NAME( frame2D_jerk_limits )
      & RK_SERIAL_LOAD_WITH_NAME( frame3D_speed_limits ) & RK_SERIAL_LOAD_WITH_NAME( frame3D_accel_limits )
