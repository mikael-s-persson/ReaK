...
public:
   ...
   * Default constructor.
   */
  kte_chain_geometry_2D( const std::string& aName = "" ) : named_object(), mGeomList(), mProxyShapeList() {
    this->setName( aName );
  };
  ...
  kte_chain_geometry_2D& addElement( const std::string& aKTEObjName, const geom::color& aColor,
                                     const shared_ptr< geom::geometry_2D >& aGeom ) {
  ...
  *******************************************************************************/
  ...
  virtual void RK_CALL save( ReaK::serialization::oarchive& A, unsigned int ) const {
    ...
    A& RK_SERIAL_SAVE_WITH_NAME( mGeomList ) & RK_SERIAL_SAVE_WITH_NAME( mProxyShapeList );
  };

  virtual void RK_CALL load( ReaK::serialization::iarchive& A, unsigned int ) {
    named_object::load( A, named_object::getStaticObjectType()->TypeVersion() );
    A& RK_SERIAL_LOAD_WITH_NAME( mGeomList ) & RK_SERIAL_LOAD_WITH_NAME( mProxyShapeList );
  };
...
public:
   ...
   * Default constructor.
   */
  kte_chain_geometry_3D( const std::string& aName = "" ) : named_object(), mGeomList(), mProxyShapeList() {
    this->setName( aName );
  };
  ...
  kte_chain_geometry_3D& addElement( const std::string& aKTEObjName, const geom::color& aColor,
                                     const shared_ptr< geom::geometry_3D >& aGeom ) {
