...
public:
  ...
  */
  dry_revolute_joint_2D( const std::string& aName = "" )
      : revolute_joint_2D( aName ), mStictionCoef( 0.5 ), mSlipCoef( 0.3 ), mSlipVelocity( 1E-5 ){};
   ...
   */
  dry_revolute_joint_2D( const std::string& aName, const shared_ptr< gen_coord< double > >& aAngle,
                         ...
                         double aStictionCoef = 0.5, double aSlipCoef = 0.3, double aSlipVelocity = 1E-5 )
      : revolute_joint_2D( aName, aAngle, aBase, aEnd, aJacobian ), mStictionCoef( aStictionCoef ),
        mSlipCoef( aSlipCoef ), mSlipVelocity( aSlipVelocity ){};
  ...
  virtual void RK_CALL save( serialization::oarchive& A, unsigned int ) const {
    ...
    A& RK_SERIAL_SAVE_WITH_NAME( mStictionCoef ) & RK_SERIAL_SAVE_WITH_NAME( mSlipCoef )
  ...
  virtual void RK_CALL load( serialization::iarchive& A, unsigned int ) {
    ...
    A& RK_SERIAL_LOAD_WITH_NAME( mStictionCoef ) & RK_SERIAL_LOAD_WITH_NAME( mSlipCoef )
      & RK_SERIAL_LOAD_WITH_NAME( mSlipVelocity );
  };

  RK_RTTI_MAKE_CONCRETE_1BASE( dry_revolute_joint_2D, 0xC210002A, 1, "dry_revolute_joint_2D", revolute_joint_2D )
};
...
public:
  ...
  dry_revolute_joint_3D( const std::string& aName = "" )
      : revolute_joint_3D( aName ), mStictionCoef( 0.5 ), mSlipCoef( 0.3 ), mSlipVelocity( 1E-5 ){};
   ...
   * \param aSlipVelocity the micro-slip tolerance in joint speed.
   */
  dry_revolute_joint_3D( const std::string& aName, const shared_ptr< gen_coord< double > >& aAngle,
                         ...
                         double aStictionCoef = 0.5, double aSlipCoef = 0.3, double aSlipVelocity = 1E-5 )
      : revolute_joint_3D( aName, aAngle, aAxis, aBase, aEnd, aJacobian ), mStictionCoef( aStictionCoef ),
        mSlipCoef( aSlipCoef ), mSlipVelocity( aSlipVelocity ){};
  ...
  RK_RTTI_MAKE_CONCRETE_1BASE( dry_revolute_joint_3D, 0xC210002B, 1, "dry_revolute_joint_3D", revolute_joint_3D )
};
};
};
