...
class manip_kin_mdl_joint_io {
 public:
   ...
   * Default constructor.
   */
  manip_kin_mdl_joint_io(const manipulator_kinematics_model* aModel)
      : model(aModel) {}
 ...
 public:
  ...
  void getJointPositions(Vector& result) const {
    ...
    for (std::vector<std::shared_ptr<gen_coord<double>>>::const_iterator it =
             model->mCoords.begin();
         it < model->mCoords.end(); ++it, ++j)
      result[j] = (*it)->q;

    for (std::vector<std::shared_ptr<frame_2D<double>>>::const_iterator it =
             model->mFrames2D.begin();
    ...
    for (std::vector<std::shared_ptr<frame_3D<double>>>::const_iterator it =
             model->mFrames3D.begin();
  ...
  void setJointPositions(const Vector& aJointPositions) {
    ...
    for (std::vector<std::shared_ptr<frame_2D<double>>>::const_iterator it =
      ...
      ++j;
      (*it)->Rotation = rot_mat_2D<double>(
          vect<double, 2>(aJointPositions[j], aJointPositions[j + 1]));
      j += 2;
    }
    ...
    for (std::vector<std::shared_ptr<frame_3D<double>>>::const_iterator it =
      ...
      ++j;
      (*it)->Quat = quaternion<double>(
          vect<double, 4>(aJointPositions[j], aJointPositions[j + 1],
  ...
  void getDependentPositions(Vector& result) const {
    ...
    for (std::vector<std::shared_ptr<joint_dependent_gen_coord>>::const_iterator
             it = model->mDependentGenCoords.begin();
         it < model->mDependentGenCoords.end(); ++it, ++j)
      result[j] = (*it)->mFrame->q;

    for (std::vector<std::shared_ptr<joint_dependent_frame_2D>>::const_iterator
             it = model->mDependent2DFrames.begin();
    ...
    for (std::vector<std::shared_ptr<joint_dependent_frame_3D>>::const_iterator
             it = model->mDependent3DFrames.begin();
...
class manip_kin_mdl_jac_calculator {
 public:
   ...
   * Default constructor.
   */
  manip_kin_mdl_jac_calculator(const manipulator_kinematics_model* aModel)
      : model(aModel) {}
  ...
  void getJacobianMatrixAndDerivativeImpl(Matrix1& Jac, Matrix2* JacDot) const {
    ...
    for (std::size_t i = 0; i < model->mCoords.size(); ++i) {
      ...
      for (std::size_t j = 0; j < model->mDependentGenCoords.size(); ++j) {
        if (model->mDependentGenCoords[j]->mUpStreamJoints.find(
                model->mCoords[i]) !=
            model->mDependentGenCoords[j]->mUpStreamJoints.end()) {
          mat_sub_block<Matrix1> subJac =
              sub(Jac)(range(RowInd, RowInd + 1), range(i, i + 1));
          if (JacDot) {
      ...
      for (std::size_t j = 0; j < model->mDependent2DFrames.size(); ++j) {
        if (model->mDependent2DFrames[j]->mUpStreamJoints.find(
                model->mCoords[i]) !=
            model->mDependent2DFrames[j]->mUpStreamJoints.end()) {
          mat_sub_block<Matrix1> subJac =
              sub(Jac)(range(RowInd, RowInd + 3), range(i, i + 1));
          if (JacDot) {
            mat_sub_block<Matrix2> subJacDot =
      ...
      for (std::size_t j = 0; j < model->mDependent3DFrames.size(); ++j) {
        if (model->mDependent3DFrames[j]->mUpStreamJoints.find(
                model->mCoords[i]) !=
            model->mDependent3DFrames[j]->mUpStreamJoints.end()) {
          mat_sub_block<Matrix1> subJac =
              sub(Jac)(range(RowInd, RowInd + 6), range(i, i + 1));
          if (JacDot) {
            mat_sub_block<Matrix2> subJacDot =
    ...
    for (std::size_t i = 0; i < model->mFrames2D.size(); ++i) {
      ...
      for (std::size_t j = 0; j < model->mDependentGenCoords.size(); ++j) {
        if (model->mDependentGenCoords[j]->mUpStream2DJoints.find(
                model->mFrames2D[i]) !=
            model->mDependentGenCoords[j]->mUpStream2DJoints.end()) {
          mat_sub_block<Matrix1> subJac =
              sub(Jac)(range(RowInd, RowInd + 1),
      ...
      for (std::size_t j = 0; j < model->mDependent2DFrames.size(); ++j) {
        if (model->mDependent2DFrames[j]->mUpStream2DJoints.find(
                model->mFrames2D[i]) !=
            model->mDependent2DFrames[j]->mUpStream2DJoints.end()) {
          mat_sub_block<Matrix1> subJac =
              sub(Jac)(range(RowInd, RowInd + 3),
                       range(3 * i + base_i, 3 * i + base_i + 3));
          if (JacDot) {
            mat_sub_block<Matrix2> subJacDot =
      ...
      for (std::size_t j = 0; j < model->mDependent3DFrames.size(); ++j) {
        if (model->mDependent3DFrames[j]->mUpStream2DJoints.find(
                model->mFrames2D[i]) !=
            model->mDependent3DFrames[j]->mUpStream2DJoints.end()) {
          mat_sub_block<Matrix1> subJac =
              sub(Jac)(range(RowInd, RowInd + 6),
                       range(3 * i + base_i, 3 * i + base_i + 3));
          if (JacDot) {
            mat_sub_block<Matrix2> subJacDot =
    ...
    for (std::size_t i = 0; i < model->mFrames3D.size(); ++i) {
      ...
      for (std::size_t j = 0; j < model->mDependent2DFrames.size(); ++j) {
        if (model->mDependent2DFrames[j]->mUpStream3DJoints.find(
                model->mFrames3D[i]) !=
      ...
      for (std::size_t j = 0; j < model->mDependent3DFrames.size(); ++j) {
        if (model->mDependent3DFrames[j]->mUpStream3DJoints.find(
                model->mFrames3D[i]) !=
...
class manip_clik_calculator {
 ...
 public:
  std::shared_ptr<const optim::cost_evaluator> cost_eval;
  ...
  struct ineq_function {
    ...
    vect_n<double> operator()(const vect_n<double>& x) const {
      ...
      std::size_t j = 0;
      for (std::size_t i = 0;
           (i < parent->lower_bounds.size()) && (i < x.size()); ++i) {
        if (parent->lower_bounds[i] !=
      ...
      for (std::size_t i = 0;
           (i < parent->upper_bounds.size()) && (i < x.size()); ++i) {
        if (parent->upper_bounds[i] !=
            std::numeric_limits<double>::infinity()) {
  ...
  struct eq_function {
    ...
    vect_n<double> operator()(const vect_n<double>& x) const {
      ...
      const std::vector<std::shared_ptr<joint_dependent_gen_coord>>&
          dep_gen_coords = parent->model->DependentCoords();
      const std::vector<std::shared_ptr<joint_dependent_frame_2D>>&
          dep_frames_2D = parent->model->DependentFrames2D();
      const std::vector<std::shared_ptr<joint_dependent_frame_3D>>&
          dep_frames_3D = parent->model->DependentFrames3D();
      ...
      manip_kin_mdl_joint_io(parent->model)
          .setJointPositions(
              x[range(0, parent->model->getJointPositionsCount())]);
      manip_kin_mdl_joint_io(parent->model)
          .setJointVelocities(
              x[range(parent->model->getJointPositionsCount(),
                      parent->model->getJointPositionsCount() +
                          parent->model->getJointVelocitiesCount())]);

      parent->model->doMotion();

      vect_n<double> result(parent->model->getDependentVelocitiesCount() * 2 +
                            parent->model->Frames2D().size() +
      ...
      for (std::size_t i = 0; i < dep_gen_coords.size(); ++i) {
        result[j] =
            dep_gen_coords[i]->mFrame->q - parent->desired_gen_coords[i].q;
        ++j;
        result[k] = dep_gen_coords[i]->mFrame->q_dot -
  ...
  struct eq_jac_filler {
    ...
    void operator()(Matrix& J, const vect_n<double>& x,
      ...
      mat_sub_block<Matrix> Jac_tmp =
          sub(J)(range(0, parent->model->getDependentVelocitiesCount()),
                 range(0, (x.size() - parent->model->Frames2D().size() -
                           parent->model->Frames3D().size()) /
                              2));
      mat_sub_block<Matrix> JacDot_tmp =
          sub(J)(range(parent->model->getDependentVelocitiesCount(),
                       parent->model->getDependentVelocitiesCount() * 2),
                 range(0, (x.size() - parent->model->Frames2D().size() -
                           parent->model->Frames3D().size()) /
                              2));

      manip_kin_mdl_jac_calculator(parent->model)
          .getJacobianMatrixAndDerivative(Jac_tmp, JacDot_tmp);

      sub(J)(range(parent->model->getDependentVelocitiesCount(),
                   parent->model->getDependentVelocitiesCount() * 2),
             range((x.size() + parent->model->Frames2D().size() +
                    parent->model->Frames3D().size()) /
                       2,
                   x.size())) = Jac_tmp;

      // j is the index to the last position element of x.
      std::size_t j = (x.size() + parent->model->Frames2D().size() +
                       parent->model->Frames3D().size()) /
                          2 -
                      1;
      // k is the index to the last valid column of J.
      std::size_t k = (x.size() - parent->model->Frames2D().size() -
                       parent->model->Frames3D().size()) /
                          2 -
                      1;
      ...
      for (std::size_t i = 0; i < parent->model->Frames3D().size(); ++i) {
        ...
        for (std::size_t r = 0; r < 3; ++r)
          for (std::size_t s = 0;
               s < parent->model->getDependentVelocitiesCount() * 2; ++s)
            J(s, j - r) = J(s, k - r);
        k -= 3;
        j -= 3;
      }
  ...
  typedef optim::nlip_newton_tr_factory<
      optim::oop_cost_function, optim::oop_cost_grad, optim::oop_cost_hess,
      double, eq_function, eq_jac_filler, ineq_function, ineq_jac_filler>
      optim_factory_type;
  ...
  manip_clik_calculator(
      manipulator_kinematics_model* aModel,
      const std::shared_ptr<const optim::cost_evaluator>& aCostEvaluator =
          std::shared_ptr<const optim::cost_evaluator>(),
      double aMaxRadius = 1.0, double aMu = 0.1, std::size_t aMaxIter = 300,
      double aTol = 1e-6, double aEta = 1e-3, double aTau = 0.99)
      : model(aModel),
        cost_eval(aCostEvaluator),
        optimizer(optim::oop_cost_function(aCostEvaluator),
                  optim::oop_cost_grad(aCostEvaluator),
                  optim::oop_cost_hess(aCostEvaluator), aMaxRadius, aMu,
                  aMaxIter, eq_function(nullptr), eq_jac_filler(nullptr),
                  ineq_function(nullptr), ineq_jac_filler(nullptr), aTol, aEta,
                  aTau) {
    if (!model)
      throw optim::improper_problem(
          "CLIK error: The model pointer cannot be null!");
  ...
  vect_n<double> readJointStatesFromModel() {
    ...
    vect_ref_view<vect_n<double>> pos_x =
        x[range(0, model->getJointPositionsCount())];
    manip_kin_mdl_joint_io(model).getJointPositions(pos_x);

    vect_ref_view<vect_n<double>> vel_x = x[range(
  ...
  void writeJointStatesToModel(const vect_n<double>& x) const {
    manip_kin_mdl_joint_io(model).setJointPositions(
        x[range(0, model->getJointPositionsCount())]);
    manip_kin_mdl_joint_io(model).setJointVelocities(x[range(
        model->getJointPositionsCount(),
        model->getJointPositionsCount() + model->getJointVelocitiesCount())]);
  }

  void runOptimizer(vect_n<double>& x) {
    std::shared_ptr<const optim::cost_evaluator> tmp_cost_eval = cost_eval;
    if (!cost_eval) {
      tmp_cost_eval = std::shared_ptr<const optim::cost_evaluator>(
