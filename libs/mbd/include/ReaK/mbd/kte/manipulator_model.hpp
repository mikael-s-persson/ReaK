...
class manipulator_kinematics_model : public kte_map {
 ...
 public:
   ...
   */
  std::size_t getJointPositionsCount() const {
   ...
   */
  std::size_t getJointVelocitiesCount() const {
  ...
  DependentCoords() const {
    return mDependentGenCoords;
  }

  /** Get read-only access to the list of 2D coordinate frames. */
  const std::vector<std::shared_ptr<joint_dependent_frame_2D>>&
  DependentFrames2D() const {
    return mDependent2DFrames;
  }

  /** Get read-only access to the list of 3D coordinate frames. */
  const std::vector<std::shared_ptr<joint_dependent_frame_3D>>&
  DependentFrames3D() const {
    return mDependent3DFrames;
  }
   ...
   * DependentFrames2D() and DependentFrames3D(), respectively. In other words, the first DependentCoords().size()
   * elements
   * are the positions of dependent generalized coordinates, the next 4 * DependentFrames2D().size()
   * elements are the position (and orientation) of the 2D dependent frames, and the final 7 *
   * DependentFrames3D().size()
   ...
   * elements are the velocities (and angular vel.) of the 2D dependent frames, and the final 6 *
   * DependentFrames3D().size()
   ...
   * elements are the accelerations (and angular acc.) of the 2D dependent frames, and the final 6 *
   * DependentFrames3D().size()
  ...
  RK_RTTI_MAKE_CONCRETE_1BASE(manipulator_kinematics_model, 0xC210FFFE, 1,
                              "manipulator_kinematics_model", kte_map)
}
...
class manipulator_dynamics_model : public manipulator_kinematics_model,
 ...
 public:
  ...
  manipulator_dynamics_model(const std::string& aName = "")
      : manipulator_kinematics_model(aName), mMassCalc(aName + "_mass_calc") {}
   ...
   */
  std::size_t getJointStatesCount() const {
  ...
  std::size_t getOutputsCount() const {
    std::size_t result = 0;
    for (std::vector<std::shared_ptr<system_output>>::const_iterator it =
             mOutputs.begin();
         it != mOutputs.end(); ++it)
   ...
   */
  virtual void computeOutput(double aTime, const ReaK::vect_n<double>& aState,
   ...
   */
  virtual void setInput(const ReaK::vect_n<double>& aInput);
  ...
  RK_RTTI_MAKE_CONCRETE_2BASE(manipulator_dynamics_model, 0xC210FFFF, 1,
