...
public:
  ...
  line_point_mindist_2D( const std::string& aName = "" )
      : kte_map( aName ), mBase(), mEnd(), mTangent(), mOriginMinDist(){};
   ...
   */
  line_point_mindist_2D( const std::string& aName, const shared_ptr< frame_2D< double > >& aBase,
                         const shared_ptr< frame_2D< double > >& aEnd, const vect< double, 2 >& aTangent,
                         const vect< double, 2 >& aOriginMinDist )
      : kte_map( aName ), mBase( aBase ), mEnd( aEnd ), mTangent( aTangent ), mOriginMinDist( aOriginMinDist ){};
  ...
  virtual void RK_CALL save( serialization::oarchive& A, unsigned int ) const {
    ...
    A& RK_SERIAL_SAVE_WITH_NAME( mBase ) & RK_SERIAL_SAVE_WITH_NAME( mEnd ) & RK_SERIAL_SAVE_WITH_NAME( mTangent )
      & RK_SERIAL_SAVE_WITH_NAME( mOriginMinDist );
  };

  virtual void RK_CALL load( serialization::iarchive& A, unsigned int ) {
    kte_map::load( A, kte_map::getStaticObjectType()->TypeVersion() );
    A& RK_SERIAL_LOAD_WITH_NAME( mBase ) & RK_SERIAL_LOAD_WITH_NAME( mEnd ) & RK_SERIAL_LOAD_WITH_NAME( mTangent )
...
public:
   ...
   * \param aOriginMinDist the minimum-distance vector from origin to the line, in global coordinates.
   */
  line_point_mindist_3D( const std::string& aName, const shared_ptr< frame_3D< double > >& aBase,
                         const shared_ptr< frame_3D< double > >& aEnd, const vect< double, 3 >& aTangent,
