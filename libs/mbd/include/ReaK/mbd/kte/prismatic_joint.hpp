...
public:
  ...
  prismatic_joint_2D( const std::string& aName = "" )
      : reacting_kte_gen( aName ), mCoord(), mAxis(), mBase(), mEnd(), mJacobian(){};
   ...
   */
  prismatic_joint_2D( const std::string& aName, const shared_ptr< gen_coord< double > >& aCoord,
                      const vect< double, 2 >& aAxis, const shared_ptr< frame_2D< double > >& aBase,
                      const shared_ptr< frame_2D< double > >& aEnd,
                      const shared_ptr< jacobian_gen_2D< double > >& aJacobian
                      = shared_ptr< jacobian_gen_2D< double > >() )
      : reacting_kte_gen( aName ), mCoord( aCoord ), mAxis( aAxis ), mBase( aBase ), mEnd( aEnd ),
        mJacobian( aJacobian ){};
  ...
  virtual void RK_CALL save( serialization::oarchive& A, unsigned int Version ) const {
    ...
    A& RK_SERIAL_SAVE_WITH_NAME( mCoord ) & RK_SERIAL_SAVE_WITH_NAME( mAxis ) & RK_SERIAL_SAVE_WITH_NAME( mBase )
  ...
  virtual void RK_CALL load( serialization::iarchive& A, unsigned int Version ) {
    ...
    A& RK_SERIAL_LOAD_WITH_NAME( mCoord ) & RK_SERIAL_LOAD_WITH_NAME( mAxis ) & RK_SERIAL_LOAD_WITH_NAME( mBase )
...
public:
   ...
   * Jacobian frame's calculation.
   */
  prismatic_joint_3D( const std::string& aName, const shared_ptr< gen_coord< double > >& aCoord,
