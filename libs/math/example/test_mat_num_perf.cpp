...
int main() {
  ...
  try {
    ...
    for( unsigned int i = 3; i <= 1000; i += inc ) {
      ...
      t1 = high_resolution_clock::now();
      eigensolve_SymQR( m_test, m_qr_Q, m_qr_E,
                        double( 1E-15 ) ); // this iterates forever!! (well, for a long time at least)
  ...
  RK_NOTICE( 2, "There were " << passed
                              << " successful tests passed on the math_gen library, out of 45 possible successes." );
