...
int main() {
  ...
  typedef void ( *JacFunctionPtr )( mat< double, mat_structure::rectangular >&, const vect_n< double >&,
                                    const vect_n< double >& );
  ...
  for( std::size_t i = 0; i < funcs.size(); ++i ) {
    ...
    try {
      ...
      std::cout << "  Gauss-Newton method failed with error: " << e.what() << std::endl << "    x = " << x
    ...
    try {
      ...
      std::cout << "  Jacobian-Transpose method failed with error: " << e.what() << std::endl << "    x = " << x
                << " with error = " << norm_2( x - func_sols[i] ) << "\n"
                << "    eval-count = " << evalCount << " and grad-eval-count = " << gradCount << std::endl;
    };
    ...
    try {
      ...
      std::cout << "  Levenberg-Marquardt method failed with error: " << e.what() << std::endl << "    x = " << x
                << " with error = " << norm_2( x - func_sols[i] ) << "\n"
                << "    eval-count = " << evalCount << " and grad-eval-count = " << gradCount << std::endl;
    };


    std::cout << "*********** with SVD linlsq solver **************************************************" << std::endl;
    ...
    try {
      ...
      std::cout << "  Box-limited Gauss-Newton method failed with error: " << e.what() << std::endl << "    x = " << x
                << " with error = " << norm_2( x - func_sols[i] ) << "\n"
                << "    eval-count = " << evalCount << " and grad-eval-count = " << gradCount << std::endl;
    };
