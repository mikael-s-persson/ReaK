...
void p01_j(mat<double, mat_structure::rectangular>& J, const vect_n<double>& x,
           const vect_n<double>& f) {
  ++gradCount;
  std::size_t M = f.size();
  std::size_t N = x.size();
  J.set_col_count(N);
...
void p02_j(mat<double, mat_structure::rectangular>& J, const vect_n<double>& x,
           const vect_n<double>& f) {
  ++gradCount;
  std::size_t M = f.size();
  std::size_t N = x.size();
...
int main() {
  ...
  for (std::size_t i = 0; i < funcs.size(); ++i) {
    std::cout << "*************************************************************"
                 "***************************"
              << std::endl;
    std::cout << "Searching for least-square of function #" << i << " called: '"
              << func_names[i] << "'" << std::endl;
    ...
    try {
      ...
      std::cout << "  Gauss-Newton method gives:\n"
                << "    x = " << x
                << " with error = " << norm_2(x - func_sols[i]) << "\n"
                << "    eval-count = " << evalCount
                << " and grad-eval-count = " << gradCount << std::endl;
    } catch (std::exception& e) {
      std::cout << "  Gauss-Newton method failed with error: " << e.what()
                << std::endl
                << "    x = " << x
                << " with error = " << norm_2(x - func_sols[i]) << "\n"
                << "    eval-count = " << evalCount
    ...
    try {
      ...
      std::cout << "  Jacobian-Transpose method failed with error: " << e.what()
                << std::endl
                << "    x = " << x
                << " with error = " << norm_2(x - func_sols[i]) << "\n"
                << "    eval-count = " << evalCount
    ...
    gradCount = 0;
    try {
      optim::levenberg_marquardt_nllsq(funcs[i], func_jacs[i], x, y, 200, 1e-4,
                                       1e-14, 1e-8, 1e-14);
      ...
      std::cout << "  Levenberg-Marquardt method failed with error: "
                << e.what() << std::endl
                << "    x = " << x
                << " with error = " << norm_2(x - func_sols[i]) << "\n"
                << "    eval-count = " << evalCount
    ...
    std::cout << "*********** with SVD linlsq solver "
                 "**************************************************"
              << std::endl;
    ...
    gradCount = 0;
    try {
      optim::make_levenberg_marquardt_nllsq(funcs[i], func_jacs[i], y, 200,
                                            1e-4, 1e-14, 1e-8, 1e-14)
          .set_lin_solver(SVD_linlsqsolver())(x);
    ...
    try {
      optim::limited_gauss_newton_nllsq(
          ...
          [i, &func_lowers, &func_uppers](const vect_n<double>& x,
      ...
      std::cout << "  Box-limited Gauss-Newton method failed with error: "
                << e.what() << std::endl
                << "    x = " << x
                << " with error = " << norm_2(x - func_sols[i]) << "\n"
                << "    eval-count = " << evalCount
    ...
    try {
      ...
      std::cout << "  Box-limited Jacobian-Transpose method failed with error: "
                << e.what() << std::endl
                << "    x = " << x
                << " with error = " << norm_2(x - func_sols[i]) << "\n"
                << "    eval-count = " << evalCount
    ...
    try {
      ...
      std::cout
          << "  Box-limited Levenberg-Marquardt method failed with error: "
          << e.what() << std::endl
