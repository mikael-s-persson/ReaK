...
struct so3_state {
  ...
  explicit so3_state( const unit_quat< double >& aQ = ( unit_quat< double >() ),
                      const vect< double, 3 >& aW = ( vect< double, 3 >( 0.0, 0.0, 0.0 ) ) )
      : q( aQ ), w( aW ){};
};
...
so3_state_tangent operator*( double s, const so3_state_tangent& x ) { return so3_state_tangent( s * x.dq, s * x.dw ); };

so3_state_tangent operator*( const so3_state_tangent& x, double s ) { return so3_state_tangent( s * x.dq, s * x.dw ); };
...
so3_state_tangent operator-( const so3_state_tangent& lhs ) { return so3_state_tangent( -lhs.dq, -lhs.dw ); };
...
double norm_2( const so3_state_tangent& x ) { return norm_2( x.dq ) + norm_2( x.dw ); };
...
struct const_ang_mom_sys {
  ...
  const_ang_mom_sys( const mat< double, mat_structure::symmetric >& aInertiaMoment ) : I( aInertiaMoment ) {
...
so3_state trapm_rule( so3_state q, const_ang_mom_sys f, double t_start, double t_end, double dt, double tol ) {
  using lie_group::oplus;
  using lie_group::ominus;
...
inline void save_state_vectors( std::ostream& out, double t, const so3_state& s, const vect< double, 7 >& sv ) {
  out << " " << t << " " << s.q[0] << " " << s.q[1] << " " << s.q[2] << " " << s.q[3] << " " << s.w[0] << " " << s.w[1]
      << " " << s.w[2] << " " << sv[0] << " " << sv[1] << " " << sv[2] << " " << sv[3] << " " << sv[4] << " " << sv[5]
      << " " << sv[6] << std::endl;
};
...
//     qv0 = lie_group::corrected_runge_kutta_4( qv0, f_sys, t, t + dt_samp, dt_int );
  ...
  for( double t = t_start; t < t_end + dt_samp * 0.5; t += dt_samp ) {
    ...
    s0_be = lie_group::backward_euler( s0_be, f_sys, t, t + dt_samp, dt_int, 0.0001 );
    sv0_be = lie_group::backward_euler( sv0_be, f_sys, t, t + dt_samp, dt_int, 0.0001 );
    s0_tr = lie_group::trapezoidal_rule( s0_tr, f_sys, t, t + dt_samp, dt_int, 0.0001 );
    sv0_tr = lie_group::trapezoidal_rule( sv0_tr, f_sys, t, t + dt_samp, dt_int, 0.0001 );
    ...
    s0_crk = lie_group::corrected_runge_kutta_4( s0_crk, f_sys, t, t + dt_samp, dt_int );
    s0_trm = trapm_rule( s0_trm, f_sys, t, t + dt_samp, dt_int, 0.0001 );
    vect< double, 4 > qv0_fe_u = unit( vect< double, 4 >( sv0_fe[0], sv0_fe[1], sv0_fe[2], sv0_fe[3] ) );
    vect< double, 7 > sv0_fe_u( qv0_fe_u[0], qv0_fe_u[1], qv0_fe_u[2], qv0_fe_u[3], sv0_fe[4], sv0_fe[5], sv0_fe[6] );
    vect< double, 4 > qv0_be_u = unit( vect< double, 4 >( sv0_be[0], sv0_be[1], sv0_be[2], sv0_be[3] ) );
    vect< double, 7 > sv0_be_u( qv0_be_u[0], qv0_be_u[1], qv0_be_u[2], qv0_be_u[3], sv0_be[4], sv0_be[5], sv0_be[6] );
    vect< double, 4 > qv0_tr_u = unit( vect< double, 4 >( sv0_tr[0], sv0_tr[1], sv0_tr[2], sv0_tr[3] ) );
    vect< double, 7 > sv0_tr_u( qv0_tr_u[0], qv0_tr_u[1], qv0_tr_u[2], qv0_tr_u[3], sv0_tr[4], sv0_tr[5], sv0_tr[6] );
    vect< double, 4 > qv0_rk_u = unit( vect< double, 4 >( sv0_rk[0], sv0_rk[1], sv0_rk[2], sv0_rk[3] ) );
    vect< double, 7 > sv0_rk_u( qv0_rk_u[0], qv0_rk_u[1], qv0_rk_u[2], qv0_rk_u[3], sv0_rk[4], sv0_rk[5], sv0_rk[6] );
