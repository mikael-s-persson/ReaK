...
void lp01( vect_n< double >& c, mat< double, mat_structure::rectangular >& A, vect_n< double >& b, vect_n< double >& l,
           vect_n< double >& u ) {
...
int main( int argc, const char** argv ) {
  ...
  for( int i = 1; i < argc; ++i ) {
    ...
    in& RK_SERIAL_LOAD_WITH_NAME( c ) & RK_SERIAL_LOAD_WITH_NAME( A ) & RK_SERIAL_LOAD_WITH_NAME( b );
  ...
  for( std::size_t i = 0; i < probs_c.size(); ++i ) {
    ...
    try {
      ...
      std::cout << "   Simplex method produced the following solution vector: " << x
                << " with cost: " << ( probs_c[i] * x ) << std::endl;
    ...
    try {
      ...
      std::cout << "   Mehrotra method produced the following solution vector: " << x
                << " with cost: " << ( probs_c[i] * x ) << std::endl;
    } catch( std::exception& e ) {
