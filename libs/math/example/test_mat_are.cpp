...
int main() {
...

  {
    ...
    */
    ...
    mat< double, mat_structure::rectangular > R
      = mat< double, mat_structure::rectangular >( mat< double, mat_structure::identity >( 1 ) );
    mat< double, mat_structure::rectangular > Q
      = mat< double, mat_structure::rectangular >( mat< double, mat_structure::identity >( 4 ) );
    Q( 0, 0 ) = 0.0;
    Q( 2, 2 ) = 0.0;
    ...
    solve_IHCT_LQR( sub( Ar )( range( 0, r ), range( 0, r ) ),
                    sub( Br )( range( 0, r ), range( 0, Br.get_col_count() ) ), Qr, Rr, Kr, Pr, 1e-6, false );
  ...
  solve_IHCT_LQR(A, B, Q, R, K, P, 1e-4, false);
    ...
    mat< double, mat_structure::rectangular > X = ( transpose_view( A ) * P + P * A - P * B * Msol_tmp + Q );
    std::cout << "Q + A^T P + P A - P B R^{-1} B^T P = " << X << std::endl;
    double err_norm = norm_1( X );
    double P_norm = norm_1( P );
    std::cout << "\t|| Err || = " << std::setw( 14 ) << err_norm << " relative to || P || = " << std::setw( 14 )
              << P_norm << " for a relative error of " << std::setw( 14 ) << ( err_norm / P_norm ) << std::endl;
  };
