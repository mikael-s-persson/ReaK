...
BOOST_AUTO_TEST_CASE( mat_operator_tests ) {
  ...
  BOOST_CHECK( (
    ( is_identity_mat( 0.5 * ( m1234_inv * m1234 + m1234_inv * m1234 ), std::numeric_limits< double >::epsilon() ) )
  ...
  BOOST_CHECK( ( ( is_identity_mat( 0.5 * ( m1234_inv_sqr * m1234_sqr + m1234_inv_sqr * m1234_sqr ),
                                    std::numeric_limits< double >::epsilon() ) )
                 && ( is_identity_mat( ( m1234_inv_sqr * m1234_sqr + m1234_inv_sqr * m1234_sqr ) * 0.5,
                                       std::numeric_limits< double >::epsilon() ) )
                 && ( is_identity_mat( ( m1234_inv_sqr * m1234_sqr - m1234_inv_sqr * m1234_sqr ) + m_ident2,
                                       std::numeric_limits< double >::epsilon() ) )
  ...
  BOOST_CHECK( ( ( is_identity_mat( m1234_inv * ( ( m1234_inv_sqr * m1234_sqr ) * m1234 ),
                                    std::numeric_limits< double >::epsilon() ) )
  ...
  BOOST_CHECK(
    ( ( is_identity_mat( 0.5 * ( m1234 * m1234_inv + ( m1234_inv_sqr * m1234_sqr ) ),
      ...
      && ( is_identity_mat( 0.5 * ( ( m1234_inv_sqr * m1234_sqr ) + m1234 * m1234_inv ),
                            std::numeric_limits< double >::epsilon() ) )
      && ( is_identity_mat( ( m1234_inv_sqr * m1234_sqr ) + ( m1234 * m1234_inv - ( m1234_inv_sqr * m1234_sqr ) ),
                            std::numeric_limits< double >::epsilon() ) )
      && ( is_identity_mat( ( m1234_inv_sqr * m1234_sqr ) + ( ( m1234_inv_sqr * m1234_sqr ) - m1234 * m1234_inv ),
                            std::numeric_limits< double >::epsilon() ) ) ) );
  ...
  BOOST_CHECK(
    ( ( is_identity_mat( 0.5 * ( m123_inv_sym * m123_sym + m123_inv_sym * m123_sym ),
                         std::numeric_limits< double >::epsilon() ) )
      && ( is_identity_mat( ( m123_inv_sym * m123_sym + m123_inv_sym * m123_sym ) * 0.5,
                            std::numeric_limits< double >::epsilon() ) )
      && ( is_identity_mat( ( m123_inv_sym * m123_sym - m123_inv_sym * m123_sym ) + m_ident2,
                            std::numeric_limits< double >::epsilon() ) )
  ...
  BOOST_CHECK( (
    ( is_identity_mat( m1234_inv * ( ( m123_sym * m123_inv_sym ) * m1234 ), std::numeric_limits< double >::epsilon() ) )
  ...
  BOOST_CHECK(
    ( ( is_identity_mat( 0.5 * ( m1234 * m1234_inv + ( m123_sym * m123_inv_sym ) ),
      ...
      && ( is_identity_mat( 0.5 * ( ( m123_sym * m123_inv_sym ) + m1234 * m1234_inv ),
                            std::numeric_limits< double >::epsilon() ) )
      && ( is_identity_mat( ( m123_sym * m123_inv_sym ) + ( m1234 * m1234_inv - ( m123_sym * m123_inv_sym ) ),
                            std::numeric_limits< double >::epsilon() ) )
      && ( is_identity_mat( ( m123_sym * m123_inv_sym ) + ( ( m123_sym * m123_inv_sym ) - m1234 * m1234_inv ),
                            std::numeric_limits< double >::epsilon() ) ) ) );

  //****************  Symmetric vs. Square  **************************
  BOOST_CHECK( ( ( is_identity_mat( m1234_inv_sqr * ( ( m123_sym * m123_inv_sym ) * m1234_sqr ),
                                    std::numeric_limits< double >::epsilon() ) )
                 && ( is_identity_mat( m1234_inv_sqr * ( m1234_sqr * ( m123_sym * m123_inv_sym ) ),
                                       std::numeric_limits< double >::epsilon() ) ) ) );
  BOOST_CHECK(
    ( ( is_identity_mat( 0.5 * ( m1234_sqr * m1234_inv_sqr + ( m123_sym * m123_inv_sym ) ),
                         std::numeric_limits< double >::epsilon() ) )
      && ( is_identity_mat( 0.5 * ( ( m123_sym * m123_inv_sym ) + m1234_sqr * m1234_inv_sqr ),
                            std::numeric_limits< double >::epsilon() ) )
      && ( is_identity_mat( ( m123_sym * m123_inv_sym ) + ( m1234_sqr * m1234_inv_sqr - ( m123_sym * m123_inv_sym ) ),
                            std::numeric_limits< double >::epsilon() ) )
      && ( is_identity_mat( ( m123_sym * m123_inv_sym ) + ( ( m123_sym * m123_inv_sym ) - m1234_sqr * m1234_inv_sqr ),
                            std::numeric_limits< double >::epsilon() ) ) ) );

  //****************  Rectangular vs. Diagonal  **************************
  mat< double, mat_structure::diagonal > m_ident_diag( 2, 1.0 );
  BOOST_CHECK(
    ( ( is_identity_mat( m1234_inv * ( m_ident_diag * m1234 ), std::numeric_limits< double >::epsilon() ) )
      && ( is_identity_mat( m1234_inv * ( m1234 * m_ident_diag ), std::numeric_limits< double >::epsilon() ) ) ) );
  BOOST_CHECK(
    ( ( is_identity_mat( 0.5 * ( m1234 * m1234_inv + m_ident_diag ), std::numeric_limits< double >::epsilon() ) )
  ...
  BOOST_CHECK(
    ( ( is_identity_mat( m1234_inv * ( m_ident_scalar * m1234 ), std::numeric_limits< double >::epsilon() ) )
      && ( is_identity_mat( m1234_inv * ( m1234 * m_ident_scalar ), std::numeric_limits< double >::epsilon() ) ) ) );
  BOOST_CHECK(
    ( ( is_identity_mat( 0.5 * ( m1234 * m1234_inv + m_ident_scalar ), std::numeric_limits< double >::epsilon() ) )
  ...
  BOOST_CHECK(
    ( ( is_identity_mat( m1234_inv * ( m_ident2 * m1234 ), std::numeric_limits< double >::epsilon() ) )
      && ( is_identity_mat( m1234_inv * ( m1234 * m_ident2 ), std::numeric_limits< double >::epsilon() ) ) ) );
  BOOST_CHECK( (
    ( is_identity_mat( 0.5 * ( m1234 * m1234_inv + m_ident2 ), std::numeric_limits< double >::epsilon() ) )
  ...
  //****************  Identity vs. Null  **************************
  BOOST_CHECK(
    ( ( is_identity_mat( ( m_zeroes * m_ident2 ) + m_ident2, std::numeric_limits< double >::epsilon() ) )
  ...
  BOOST_CHECK( ( is_identity_mat( m4321_cpy - m4321_orig - mat< double >( 2, 2, true ),
                                  std::numeric_limits< double >::epsilon() ) ) );
