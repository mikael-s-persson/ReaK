  ...
  for( std::size_t i = 0; i < F_list.size(); ++i ) {
    ...
    try {
      ...
      mat< double, mat_structure::rectangular > X
        = ( transpose_view( F_list[i] ) * P * F_list[i] - transpose_view( F_list[i] ) * P * G_list[i] * Msol_tmp
            + Q_list[i] );
      ...
      BOOST_CHECK_MESSAGE( ( err_norm < 1e-3 * P_norm ), "Failed to solve DAREX problem "
...
BOOST_AUTO_TEST_CASE( mat_carex_tests ) {
  ...
  for( std::size_t i = 0; i < A_list.size(); ++i ) {
    ...
    try {
      ...
      linlsq_QR( M_tmp, Msol_tmp, M2_tmp );
      mat< double, mat_structure::rectangular > X
        = ( transpose_view( A_list[i] ) * P + P * A_list[i] - P * B_list[i] * Msol_tmp + Q_list[i] );
      double err_norm = norm_1( X );
      double P_norm = norm_1( P );
      BOOST_WARN_MESSAGE( ( err_norm < 2e-5 * P_norm ), "Significant loss of precision on CAREX problem " << i << "!" );
      BOOST_CHECK_MESSAGE( ( err_norm < 1e-3 * P_norm ), "Failed to solve CAREX problem "
                                                         << i << "! Loss of precision unacceptable!" );
    } catch( ... ) {
