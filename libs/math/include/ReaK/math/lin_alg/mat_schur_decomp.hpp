...
void symmetric_QR_step( Matrix1& T, Matrix2* Z, typename mat_traits< Matrix1 >::value_type NumTol ) {
  ...
  for( SizeType k = 0; k < N - 1; ++k ) {
    ...
    mat_sub_block< Matrix1 > subT1( T, 2, ( k < N - 2 ? ( k > 0 ? 4 : 3 ) : ( k > 0 ? 3 : 2 ) ), k,
                                    ( k > 0 ? k - 1 : 0 ) );
    givens_rot_prod( g, subT1 ); // G * A

    mat_sub_block< Matrix1 > subT2( T, ( k < N - 2 ? ( k > 0 ? 4 : 3 ) : ( k > 0 ? 3 : 2 ) ), 2, ( k > 0 ? k - 1 : 0 ),
                                    k );
...
template < typename Matrix1, typename Matrix2 >
void francis_QR_step( Matrix1& H, Matrix2* Z, typename mat_traits< Matrix1 >::size_type Offset,
  ...
  vect< ValueType, 3 > v(
    H( Offset, 0 ) * H( Offset, 0 ) + H( Offset, 1 ) * H( Offset + 1, 0 ) - s * H( Offset, 0 ) + t,
    H( Offset + 1, 0 ) * ( H( Offset, 0 ) + H( Offset + 1, 1 ) - s ), H( Offset + 1, 0 ) * H( Offset + 2, 1 ) );
...
void deflate_hess_elem_down_impl( Matrix1& A, Matrix2& B, Matrix3* Q, Matrix4* Z,
  ...
  for( ; q < N - 1; ++p, ++q ) {
    ...
    mat_sub_block< Matrix1 > subA1( A, 2, ( p == 0 ? A.get_col_count() : A.get_col_count() - p + 1 ), q,
                                    ( p == 0 ? p : p - 1 ) );
...
void francis_QZ_step_impl( Matrix1& A, Matrix2& B, Matrix3* Q, Matrix4* Z,
  ...
  for( SizeType k = 0; k < N - 2; ++k ) {
    ...
    vect< ValueType, 3 > v2( B( Offset + k + 2, k ), B( Offset + k + 2, k + 1 ), B( Offset + k + 2, k + 2 ) );
    ...
    hhm2.set_inv( vect< ValueType, 2 >( B( Offset + k + 1, k ), B( Offset + k + 1, k + 1 ) ), NumTol );
  ...
  hhm3.set_inv( vect< ValueType, 2 >( B( Offset + N - 1, N - 2 ), B( Offset + N - 1, N - 1 ) ), NumTol );
...
template < typename Matrix1, typename Matrix2, typename Matrix3, typename Matrix4 >
void francis_QZ_step( Matrix1& A, Matrix2& B, Matrix3* Q, Matrix4* Z, typename mat_traits< Matrix1 >::size_type Offset,
                      typename mat_traits< Matrix1 >::value_type NumTol ) {
  typedef typename mat_traits< Matrix1 >::value_type ValueType;
  typedef typename mat_traits< Matrix1 >::size_type SizeType;
  ...
  v[0] = ( ( A( Offset + N - 2, N - 2 ) / B( Offset + N - 2, N - 2 ) - A( Offset, 0 ) / B( Offset, 0 ) )
           * ( A( Offset + N - 1, N - 1 ) / B( Offset + N - 1, N - 1 ) - A( Offset, 0 ) / B( Offset, 0 ) )
           ...
           + ( A( Offset + N - 1, N - 2 ) / B( Offset + N - 2, N - 2 ) )
             * ( B( Offset + N - 2, N - 1 ) / B( Offset + N - 1, N - 1 ) ) * ( A( Offset, 0 ) / B( Offset, 0 ) ) )
         * ( B( Offset, 0 ) / A( Offset + 1, 0 ) ) + A( Offset, 1 ) / B( Offset + 1, 1 )
...
void gen_schur_decomp_impl( Matrix1& A, Matrix2& B, Matrix3* Q, Matrix4* Z,
  ...
  while( q > 0 ) {
    ...
    mat_sub_block< Matrix1 > subA( A, q, N - p, 0,
                                   p ); // the QZ step will affect the entire upper corner block (1:q,p:N)
    mat_sub_block< Matrix2 > subB( B, q, N - p, 0, p );

    if( Q ) {
      mat_sub_block< Matrix3 > subQ( *Q, Q->get_row_count(), Q->get_col_count() - p, 0,
                                     p ); // Q_new will only change in the columns after p.
      if( Z ) {
        mat_sub_block< Matrix3 > subZ( *Z, Z->get_row_count(), Z->get_col_count() - p, 0,
  ...
  for( q = 1; q < N; ++q ) {
    ...
    ValueType p_val = ValueType( 0.5 )
                      * ( a_22 / B( q, q ) - ( B( q - 1, q ) * A( q, q - 1 ) ) / ( B( q - 1, q - 1 ) * B( q, q ) ) );
  ...
  enable_if< boost::mpl::
               and_< is_readable_matrix< Matrix1 >, is_readable_matrix< Matrix2 >,
                     boost::mpl::and_< is_fully_writable_matrix< Matrix3 >, is_fully_writable_matrix< Matrix4 >,
                                       is_fully_writable_matrix< Matrix5 >, is_fully_writable_matrix< Matrix6 > > >,
             void >::type
...
extern template void decompose_RealSchur( const mat< double, mat_structure::square >& A,
                                          mat< double, mat_structure::square >& Q,
                                          mat< double, mat_structure::square >& T, double NumTol );
extern template void decompose_RealSchur( const mat< double, mat_structure::rectangular >& A,
                                          mat< double, mat_structure::square >& Q,
...
extern template void decompose_GenRealSchur( const mat< double, mat_structure::square >& A,
...
extern template void decompose_GenRealSchur( const mat< double, mat_structure::rectangular >& A,
...
extern template void decompose_RealSchur( const mat< float, mat_structure::square >& A,
                                          mat< float, mat_structure::square >& Q,
                                          mat< float, mat_structure::square >& T, float NumTol );
extern template void decompose_RealSchur( const mat< float, mat_structure::rectangular >& A,
                                          mat< float, mat_structure::square >& Q,
...
extern template void decompose_GenRealSchur( const mat< float, mat_structure::square >& A,
                                             const mat< float, mat_structure::square >& B,
                                             mat< float, mat_structure::square >& Q,
                                             mat< float, mat_structure::square >& Z,
                                             mat< float, mat_structure::square >& T,
                                             mat< float, mat_structure::square >& R, float NumTol );
extern template void decompose_GenRealSchur( const mat< float, mat_structure::rectangular >& A,
