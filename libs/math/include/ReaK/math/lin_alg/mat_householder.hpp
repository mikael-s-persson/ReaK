...
public:
   ...
   * Default constructor.
   */
  explicit householder_matrix( const value_type& aBeta = 0.0, const Vector& aV = Vector() ) : beta( aBeta ), v( aV ){};
   ...
   */
  friend value_type trace( const self& M ) { return value_type( M.v.size() ) - M.beta * ( M.v * M.v ); };
};
