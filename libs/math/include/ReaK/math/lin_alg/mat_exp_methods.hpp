...
typename boost::enable_if_c< is_readable_matrix< Matrix1 >::value && is_square_matrix< Matrix1 >::value
  ...
  if( n1 <= 1.495585217958292e-2 ) { // m = 3
    ...
    mat< ValueType, mat_structure::square > U = A_tmp * ( A2 + ValueType( 60 ) * eye_N );
    mat< ValueType, mat_structure::square > V = ValueType( 12 ) * A2 + ValueType( 120 ) * eye_N;
    linsolve( V - U, X, U + V, NumTol );
  } else if( n1 <= 2.539398330063230e-1 ) { // m = 5
    mat< ValueType, mat_structure::square > A2 = A_tmp * A_tmp;
    mat< ValueType, mat_structure::square > A4 = A2 * A2;
    mat< ValueType, mat_structure::square > U = A_tmp * ( A4 + ValueType( 420 ) * A2 + ValueType( 15120 ) * eye_N );
    mat< ValueType, mat_structure::square > V = ValueType( 30 ) * A4 + ValueType( 3360 ) * A2
    ...
    mat< ValueType, mat_structure::square > U
      = A_tmp * ( A6 + ValueType( 1512 ) * A4 + ValueType( 277200 ) * A2 + ValueType( 8648640 ) * eye_N );
    mat< ValueType, mat_structure::square > V = ValueType( 56 ) * A6 + ValueType( 25200 ) * A4
                                                + ValueType( 1995840 ) * A2 + ValueType( 17297280 ) * eye_N;
    linsolve( V - U, X, U + V, NumTol );
    ...
    mat< ValueType, mat_structure::square > U
      = A_tmp * ( A8 + ValueType( 3960 ) * A6 + ValueType( 2162160 ) * A4 + ValueType( 302702400 ) * A2
                  + ValueType( 8821612800 ) * eye_N );
    mat< ValueType, mat_structure::square > V = ValueType( 90 ) * A8 + ValueType( 110880 ) * A6
                                                + ValueType( 30270240 ) * A4 + ValueType( 2075673600 ) * A2
    ...
    mat< ValueType, mat_structure::square > A6 = A2 * A4;
    mat< ValueType, mat_structure::square > U
      = A_tmp * ( A6 * ( A6 + ValueType( 16380 ) * A4 + ValueType( 40840800 ) * A2 ) + ValueType( 33522128640 ) * A6
                  + ValueType( 10559470521600 ) * A4 + ValueType( 1187353796428800 ) * A2
                  + ValueType( 32382376266240000 ) * eye_N );
    mat< ValueType, mat_structure::square > V
      = A6 * ( ValueType( 182 ) * A6 + ValueType( 960960 ) * A4 + ValueType( 1323241920 ) * A2 )
        + ValueType( 670442572800 ) * A6 + ValueType( 129060195264000 ) * A4 + ValueType( 7771770303897600 ) * A2
...
// b(0: 13) = [64764752532480000, 32382376266240000, 7771770303897600, 1187353796428800, 129060195264000,
...
extern template void exp_PadeSAS( const mat< double, mat_structure::square >& A,
                                  mat< double, mat_structure::square >& X, QR_linlsqsolver linsolve, double NumTol );
extern template void exp_PadeSAS( const mat< double, mat_structure::square >& A,
                                  mat< double, mat_structure::square >& X, RRQR_linlsqsolver linsolve, double NumTol );


extern template void exp_PadeSAS( const mat< float, mat_structure::square >& A, mat< float, mat_structure::square >& X,
