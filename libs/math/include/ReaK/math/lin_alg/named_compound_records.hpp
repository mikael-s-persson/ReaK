/**
 * \file named_compound_records.hpp
 *
...
public:
  ...
  struct compound_vect {
     ...
     */
    template < typename T >
    explicit compound_vect( const T& v )
        : data() {
      *this = v;
    };
  };
  ...
  void set( const std::string& aName, const compound_vect& aValue ) {
    ...
    if( c_it == compounds.end() )
      throw out_of_bounds();
    try {
      for( std::size_t i = 0; ( i < c_it->second.names.size() ) && ( i < aValue.data.size() ); ++i )
  ...
  compound_vect get( const std::string& aName ) const {
    std::map< std::string, compound_def >::const_iterator c_it = compounds.find( aName );
    if( c_it == compounds.end() )
      throw out_of_bounds();
    compound_vect result;
    result.data = c_it->second.default_values;
    try {
      for( std::size_t i = 0; ( i < c_it->second.names.size() ) && ( i < result.data.size() ); ++i )
  ...
  struct compound_proxy {
    ...
    std::string name;
    compound_proxy( named_compound_record* aParent, const std::string& aName ) : parent( aParent ), name( aName ){};
  ...
  struct compound_const_proxy {
    ...
    std::string name;
    compound_const_proxy( const named_compound_record* aParent, const std::string& aName )
   ...
   */
  compound_proxy operator[]( const std::string& aName ) { return compound_proxy( this, aName ); };
   ...
   *         internal vector representation) the value of the compound object.
   */
  compound_const_proxy operator[]( const std::string& aName ) const { return compound_const_proxy( this, aName ); };
   ...
   */
  compound_builder add( const std::string& aName ) { return compound_builder( compounds[aName] ); };
};
};
};
