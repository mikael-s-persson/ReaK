...
struct mat_indexer< mat_structure::symmetric, Alignment > {
  ...
  std::size_t mat_triangular_size( std::size_t Size ) { return ( Size * ( Size - 1 ) ) / 2 + Size; };
...
public:
  ...
  *******************************************************************************/
   ...
   * \test PASSED
   */
  mat( const_reference a11, const_reference a12, const_reference a22 ) : q( 3 ), rowCount( 2 ) {
    q[0] = a11;
    q[1] = a12;
    q[2] = a22;
  };
   ...
   * \test PASSED
   */
  mat( const_reference a11, const_reference a12, const_reference a13, const_reference a22, const_reference a23,
       const_reference a33 )
      : q( 6 ), rowCount( 3 ) {
    q[0] = a11;
    q[1] = a12;
   ...
   * \test PASSED
   */
  mat( const_reference a11, const_reference a12, const_reference a13, const_reference a14, const_reference a22,
       const_reference a23, const_reference a24, const_reference a33, const_reference a34, const_reference a44 )
      : q( 10 ), rowCount( 4 ) {
    q[0] = a11;
    q[1] = a12;
    q[2] = a22;
  ...
  *******************************************************************************/
   ...
   * \throw std::range_error if the two matrix dimensions don't match.
   */
  friend mat< T, mat_structure::square, alignment, allocator_type > operator*( const self& M1, const self& M2 ) {
  ...
  *******************************************************************************/
   ...
   */
  friend mat< T, mat_structure::rectangular, Alignment, Allocator > get_block( const self& M, size_type aRowOffset,
                                                                               size_type aColOffset,
                                                                               size_type aRowCountOut,
                                                                               size_type aColCountOut ) {
    if( ( aRowOffset + aRowCountOut > M.rowCount ) || ( aColOffset + aColCountOut > M.rowCount ) )
      throw std::range_error( "Matrix dimension mismatch." );
    mat< T, mat_structure::rectangular, Alignment, Allocator > result( aRowCountOut, aColCountOut, T( 0 ),
                                                                       M.get_allocator() );
    size_type k = mat_triangular_size( aColOffset );
    for( size_type j = 0; j < aColCountOut; k += ( ++j + aColOffset ) ) {
      size_type h = mat_triangular_size( aRowOffset );
      size_type i = 0;
   ...
   * \throw std::range_error If the sub-matrix's dimensions and position does not fit within this matrix.
   */
  friend mat< T, mat_structure::square, Alignment, Allocator > get_block( const self& M, size_type aRowOffset,
                                                                          size_type aColOffset, size_type aSizeOut ) {
