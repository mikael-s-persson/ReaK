...
typename mat_traits< Matrix1 >::value_type get_norm_gen_eigens_impl( const Matrix1& A, const Matrix2& B ) {
...
template < typename Matrix1, typename Matrix2 >
typename mat_traits< Matrix1 >::value_type get_real_val_gen_eigens_impl( const Matrix1& A, const Matrix2& B ) {
  typedef typename mat_traits< Matrix1 >::value_type ValueType;
  typedef typename mat_traits< Matrix1 >::size_type SizeType;
  using std::fabs;
  using std::sqrt;
  ...
  if( N == 1 ) {
    ...
    if( fabs( tmp ) < std::numeric_limits< ValueType >::epsilon() * fabs( l ) )
      return ( l < 0 ? -std::numeric_limits< ValueType >::infinity() : std::numeric_limits< ValueType >::infinity() );
    else
      return l / tmp;
  } else {
    ValueType l = fabs( A( 0, 0 ) * A( 1, 1 ) - A( 1, 0 ) * A( 0, 1 ) );
...
struct lesser_norm_eigen_first {
  ...
  int operator()( const Matrix1& A1, const Matrix2& B1, const Matrix3& A2, const Matrix4& B2 ) {
    typedef typename mat_traits< Matrix1 >::value_type ValueType;
...
void swap_schur_blocks21_impl( Matrix1& A, Matrix2& B, Matrix3* Q, Matrix4* Z,
  ...
  {
    ValueType x1 = a33 * B( row_offset + p + 1, p + 1 ) - b33 * A( row_offset + p + 1, p + 1 );
    ValueType x2 = a33 * B( row_offset + p + 1, p + 2 ) - b33 * A( row_offset + p + 1, p + 2 );
...

  {
    ...
    ValueType x2 = a33 * B( row_offset + p, p + 1 ) - b33 * A( row_offset + p, p + 1 );
...
void swap_schur_blocks22_impl( Matrix1& A, Matrix2& B, Matrix3* Q, Matrix4* Z,
  ...
  mat< ValueType, mat_structure::square > V
    = mat< ValueType, mat_structure::square >( mat< ValueType, mat_structure::identity >( 4 ) );
...
template < typename Matrix1, typename Matrix2, typename Matrix3, typename Matrix4, typename CompareFunc >
void partition_schur_pencil_impl( Matrix1& A, Matrix2& B, Matrix3* Q, Matrix4* Z, CompareFunc compare,
...
typename boost::enable_if_c< is_readable_matrix< Matrix1 >::value && is_readable_matrix< Matrix2 >::value
  ...
  if( ( A.get_row_count() != A.get_col_count() ) || ( B.get_row_count() != A.get_row_count() )
      || ( Q.get_row_count() != Q.get_col_count() ) || ( R.get_row_count() != R.get_col_count() )
      || ( B.get_col_count() != R.get_col_count() ) )
    throw std::range_error( "The dimensions of the CARE system matrices do not match! Should be A(n x n), B(n x m), "
                            "Q(n x n), and R(m x m)." );
  ...
  sub( A_aug )( range( 0, 2 * N ), range( 0, N ) ) = sub( Q_tmp )( range( M, M + 2 * N ), range( 0, N ) ) * A
  ...
  mat< ValueType, mat_structure::square > Q_aug
    = mat< ValueType, mat_structure::square >( mat< ValueType, mat_structure::identity >( 2 * N ) );
...
typename boost::enable_if_c< is_readable_matrix< Matrix1 >::value && is_readable_matrix< Matrix2 >::value
  ...
  if( ( A.get_row_count() != A.get_col_count() ) || ( B.get_row_count() != A.get_row_count() )
      || ( Q.get_row_count() != Q.get_col_count() ) || ( R.get_row_count() != R.get_col_count() )
      || ( B.get_col_count() != R.get_col_count() ) )
    throw std::range_error( "Infinite-horizon Continuous-time LQR with reduction: The dimensions of the system "
                            "matrices do not match! Should be A(n x n), B(n x m), Q(n x n), and R(m x m)." );
  ...
  solve_IHCT_LQR( sub( Ar )( range( 0, r ), range( 0, r ) ), sub( Br )( range( 0, r ), range( 0, M ) ), Q_reduced,
                  R_reduced, K_reduced, P_reduced, NumTol, UseBalancing );
...
typename boost::enable_if_c< is_readable_matrix< Matrix1 >::value && is_readable_matrix< Matrix2 >::value
  ...
  if( ( A.get_row_count() != A.get_col_count() ) || ( B.get_row_count() != A.get_row_count() )
      || ( Q.get_row_count() != Q.get_col_count() ) || ( R.get_row_count() != R.get_col_count() )
      || ( B.get_col_count() != R.get_col_count() ) )
    throw std::range_error( "Infinite-horizon Continuous-time AQR with reduction: The dimensions of the system "
                            "matrices do not match! Should be A(n x n), B(n x m), Q(n x n), and R(m x m)." );
  ...
  mat< ValueType, mat_structure::rectangular > P_reduced( r, r );
  mat< ValueType, mat_structure::rectangular > K_reduced( M, r );
  solve_IHCT_AQR( sub( Ar )( range( 0, r ), range( 0, r ) ), sub( Br )( range( 0, r ), range( 0, M ) ),
                  transpose_view( Qrr ) * c, Q_reduced, R_reduced, K_reduced, P_reduced, u_bias, NumTol, UseBalancing );
...
typename boost::enable_if_c< is_readable_matrix< Matrix1 >::value && is_readable_matrix< Matrix2 >::value
  ...
  solve_dare_problem( const Matrix1& F, const Matrix2& G, const Matrix3& Q, const Matrix4& R, Matrix5& P,
                      typename mat_traits< Matrix1 >::value_type NumTol = 1E-8, bool UseBalancing = false ) {
  if( ( F.get_row_count() != F.get_col_count() ) || ( G.get_row_count() != F.get_row_count() )
      || ( Q.get_row_count() != Q.get_col_count() ) || ( R.get_row_count() != R.get_col_count() )
      || ( G.get_col_count() != R.get_col_count() ) )
    throw std::range_error( "The dimensions of the DARE system matrices do not match! Should be F(n x n), G(n x m), "
                            "Q(n x n), and R(m x m)." );
  ...
  mat< ValueType, mat_structure::rectangular > A_aug( 2 * N, 2 * N );
  sub( A_aug )( range( 0, 2 * N ), range( 0, N ) ) = sub( Q_tmp )( range( M, M + 2 * N ), range( 0, N ) ) * F
                                                     - sub( Q_tmp )( range( M, M + 2 * N ), range( N, 2 * N ) ) * Q;
...
typename boost::enable_if_c< is_readable_matrix< Matrix1 >::value && is_readable_matrix< Matrix2 >::value
  ...
  solve_ctsf_problem( const Matrix1& A, const Matrix2& B, const Matrix3& C, const Matrix4& D, Matrix5& P,
                      typename mat_traits< Matrix1 >::value_type NumTol = 1E-8, bool UseBalancing = false ) {
  if( ( A.get_row_count() != A.get_col_count() ) || ( B.get_row_count() != A.get_row_count() )
      || ( C.get_col_count() != A.get_col_count() ) || ( C.get_row_count() != D.get_col_count() )
      || ( D.get_row_count() != D.get_col_count() ) || ( B.get_col_count() != D.get_col_count() ) )
    throw std::range_error( "The dimensions of the CTSF system matrices do not match! Should be A(n x n), B(n x m), "
                            "C(m x n), and D(m x m)." );
  ...
  try {
    ...
    throw singularity_error( "The Continuous-time Spectral Factorisation (CTSF) cannot be solved! Usually indicates "
                             "that the system is not stabilizable." );
  };
...
typename boost::enable_if_c< is_readable_matrix< Matrix1 >::value && is_readable_matrix< Matrix2 >::value
  ...
  solve_dtsf_problem( const Matrix1& F, const Matrix2& G, const Matrix3& H, const Matrix4& J, Matrix5& P,
                      typename mat_traits< Matrix1 >::value_type NumTol = 1E-8, bool UseBalancing = false ) {
  if( ( F.get_row_count() != F.get_col_count() ) || ( G.get_row_count() != F.get_row_count() )
      || ( H.get_col_count() != F.get_col_count() ) || ( H.get_row_count() != J.get_col_count() )
      || ( J.get_row_count() != J.get_col_count() ) || ( G.get_col_count() != J.get_col_count() ) )
    throw std::range_error( "The dimensions of the DTSF system matrices do not match! Should be F(n x n), G(n x m), "
                            "H(m x n), and J(m x m)." );
  ...
  try {
    ...
    throw singularity_error( "The Discrete-time Spectral Factorisation (CTSF) cannot be solved! Usually indicates that "
...
extern template void solve_care_problem( const mat< double, mat_structure::square >& A,
...
extern template void solve_care_problem( const mat< double, mat_structure::square >& A,
                                         ...
                                         mat< double, mat_structure::square >& P, double NumTol, bool UseBalancing );
extern template void solve_care_problem( const mat< double, mat_structure::rectangular >& A,
                                         const mat< double, mat_structure::rectangular >& B,
                                         const mat< double, mat_structure::square >& Q,
                                         const mat< double, mat_structure::square >& R,
                                         mat< double, mat_structure::rectangular >& P, double NumTol,
                                         bool UseBalancing );
extern template void solve_care_problem( const mat< double, mat_structure::rectangular >& A,
                                         const mat< double, mat_structure::rectangular >& B,
                                         const mat< double, mat_structure::symmetric >& Q,
...
extern template void solve_dare_problem( const mat< double, mat_structure::square >& F,
...
extern template void solve_dare_problem( const mat< double, mat_structure::square >& F,
                                         ...
                                         const mat< double, mat_structure::diagonal >& R,
                                         mat< double, mat_structure::square >& P, double NumTol, bool UseBalancing );
extern template void solve_dare_problem( const mat< double, mat_structure::rectangular >& F,
                                         const mat< double, mat_structure::rectangular >& G,
                                         const mat< double, mat_structure::square >& Q,
                                         const mat< double, mat_structure::square >& R,
                                         mat< double, mat_structure::rectangular >& P, double NumTol,
...
extern template void solve_ctsf_problem( const mat< double, mat_structure::square >& A,
                                         const mat< double, mat_structure::rectangular >& B,
                                         const mat< double, mat_structure::rectangular >& C,
                                         const mat< double, mat_structure::rectangular >& D,
                                         mat< double, mat_structure::square >& P, double NumTol, bool UseBalancing );
extern template void solve_ctsf_problem( const mat< double, mat_structure::rectangular >& A,
                                         const mat< double, mat_structure::rectangular >& B,
                                         const mat< double, mat_structure::rectangular >& C,
                                         const mat< double, mat_structure::rectangular >& D,
                                         mat< double, mat_structure::rectangular >& P, double NumTol,
...
extern template void solve_dtsf_problem( const mat< double, mat_structure::square >& F,
                                         const mat< double, mat_structure::rectangular >& G,
                                         const mat< double, mat_structure::rectangular >& H,
                                         const mat< double, mat_structure::rectangular >& J,
                                         mat< double, mat_structure::square >& P, double NumTol, bool UseBalancing );
extern template void solve_dtsf_problem( const mat< double, mat_structure::rectangular >& F,
                                         const mat< double, mat_structure::rectangular >& G,
                                         const mat< double, mat_structure::rectangular >& H,
                                         const mat< double, mat_structure::rectangular >& J,
                                         mat< double, mat_structure::rectangular >& P, double NumTol,
...
extern template void solve_care_problem( const mat< float, mat_structure::square >& A,
...
extern template void solve_care_problem( const mat< float, mat_structure::square >& A,
                                         ...
                                         mat< float, mat_structure::square >& P, float NumTol, bool UseBalancing );
extern template void solve_care_problem( const mat< float, mat_structure::rectangular >& A,
                                         const mat< float, mat_structure::rectangular >& B,
                                         const mat< float, mat_structure::square >& Q,
                                         const mat< float, mat_structure::square >& R,
...
extern template void solve_dare_problem( const mat< float, mat_structure::square >& F,
...
extern template void solve_dare_problem( const mat< float, mat_structure::square >& F,
                                         ...
                                         const mat< float, mat_structure::diagonal >& R,
                                         mat< float, mat_structure::square >& P, float NumTol, bool UseBalancing );
extern template void solve_dare_problem( const mat< float, mat_structure::rectangular >& F,
                                         const mat< float, mat_structure::rectangular >& G,
                                         const mat< float, mat_structure::square >& Q,
                                         const mat< float, mat_structure::square >& R,
                                         mat< float, mat_structure::rectangular >& P, float NumTol, bool UseBalancing );
...
extern template void solve_ctsf_problem( const mat< float, mat_structure::square >& A,
                                         const mat< float, mat_structure::rectangular >& B,
                                         const mat< float, mat_structure::rectangular >& C,
                                         const mat< float, mat_structure::rectangular >& D,
                                         mat< float, mat_structure::square >& P, float NumTol, bool UseBalancing );
extern template void solve_ctsf_problem( const mat< float, mat_structure::rectangular >& A,
                                         const mat< float, mat_structure::rectangular >& B,
                                         const mat< float, mat_structure::rectangular >& C,
                                         const mat< float, mat_structure::rectangular >& D,
                                         mat< float, mat_structure::rectangular >& P, float NumTol, bool UseBalancing );

extern template void solve_dtsf_problem( const mat< float, mat_structure::square >& F,
                                         const mat< float, mat_structure::rectangular >& G,
                                         const mat< float, mat_structure::rectangular >& H,
                                         const mat< float, mat_structure::rectangular >& J,
                                         mat< float, mat_structure::square >& P, float NumTol, bool UseBalancing );
extern template void solve_dtsf_problem( const mat< float, mat_structure::rectangular >& F,
                                         const mat< float, mat_structure::rectangular >& G,
                                         const mat< float, mat_structure::rectangular >& H,
                                         const mat< float, mat_structure::rectangular >& J,
                                         mat< float, mat_structure::rectangular >& P, float NumTol, bool UseBalancing );
