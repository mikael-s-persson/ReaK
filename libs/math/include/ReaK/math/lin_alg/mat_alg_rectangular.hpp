...
private:
  std::vector< value_type, allocator_type >
    q;                ///< Array which holds all the values of the matrix (dimension: rowCount x colCount).
...
public:
  ...
  *******************************************************************************/
   ...
   * \test PASSED
   */
  mat( const allocator_type& aAlloc = allocator_type() ) : q( 0, value_type(), aAlloc ), rowCount( 0 ), colCount( 0 ){};

  /**
   * Constructor for a sized matrix.
   * \test PASSED
   */
  mat( size_type aRowCount, size_type aColCount, const value_type& aFill = value_type(),
       const allocator_type& aAlloc = allocator_type() )
      : q( aRowCount * aColCount, aFill, aAlloc ), rowCount( aRowCount ), colCount( aColCount ){};
  ...
  mat( size_type aRowCount, size_type aColCount, bool aIdentity, const allocator_type& aAlloc = allocator_type() )
      : q( aRowCount * aColCount, 0, aAlloc ), rowCount( aRowCount ), colCount( aColCount ) {
    if( aIdentity ) {
   ...
   * \test PASSED
   */
  mat( const value_type& a11, const value_type& a12, const value_type& a21, const value_type& a22 )
   ...
   * \test PASSED
   */
  mat( const value_type& a11, const value_type& a12, const value_type& a13, const value_type& a21,
       const value_type& a22, const value_type& a23, const value_type& a31, const value_type& a32,
       const value_type& a33 )
  ...
  *******************************************************************************/
  ...
  std::pair< row_iterator, row_iterator > rows() { return std::make_pair( first_row(), last_row() ); };
  ...
  col_iterator first_col() { return col_iterator( q.begin(), rowCount ); };
  const_col_iterator first_col() const { return const_col_iterator( q.begin(), rowCount ); };
  col_iterator last_col() { return col_iterator( q.begin() + colCount * rowCount, rowCount ); };
  const_col_iterator last_col() const { return const_col_iterator( q.begin() + colCount * rowCount, rowCount ); };
  ...
  std::pair< col_iterator, col_iterator > cols() { return std::make_pair( first_col(), last_col() ); };
...
#define RK_CREATE_SUBMATRIX_MINUS_TRANSPOSE_OPERATORS( SUBMATRIX )                             \
  template < typename Matrix >                                                                 \
  mat< typename mat_traits< Matrix >::value_type, mat_structure::rectangular > operator-(      \
    ...
    return result;                                                                             \
                                                                                               ...
                                                                                               \
  template < typename Matrix >                                                                 \
  mat< typename mat_traits< Matrix >::value_type, mat_structure::rectangular > transpose(      \
    ...
    return result;                                                                             \
                                                                                               ...
                                                                                               \
  template < typename Matrix >                                                                 \
  mat< typename mat_traits< Matrix >::value_type, mat_structure::rectangular > transpose_move( \
    const SUBMATRIX< Matrix >& M ) {                                                           \
    typedef typename mat_traits< Matrix >::value_type ValueType;                               \
    typedef typename mat_traits< Matrix >::size_type SizeType;                                 \
    mat< ValueType, mat_structure::rectangular > result( M.colCount, M.rowCount );             \
...
public:
  ...
  *******************************************************************************/
  ...
  row_iterator first_row() { return row_iterator( q.begin(), colCount ); };
  const_row_iterator first_row() const { return const_row_iterator( q.begin(), colCount ); };
  row_iterator last_row() { return row_iterator( q.begin() + colCount * rowCount, colCount ); };
  const_row_iterator last_row() const { return const_row_iterator( q.begin() + colCount * rowCount, colCount ); };
  row_iterator first_row( col_iterator cit ) {
  ...
  col_iterator first_col() { return col_iterator( q.begin() ); };
  const_col_iterator first_col() const { return const_col_iterator( q.begin() ); };
  col_iterator last_col() { return col_iterator( q.begin() + colCount * rowCount ); };
  const_col_iterator last_col() const { return const_col_iterator( q.begin() + colCount * rowCount ); };
  col_iterator first_col( row_iterator rit ) {
...
mat< T, mat_structure::rectangular, Alignment, Allocator >
  get_block( const mat< T, Structure, Alignment, Allocator >& M, std::size_t aRowOffset, std::size_t aColOffset,
...
extern template class mat< double, mat_structure::rectangular, mat_alignment::column_major >;
extern template class mat< double, mat_structure::rectangular, mat_alignment::row_major >;

extern template class mat< float, mat_structure::rectangular, mat_alignment::column_major >;
extern template class mat< float, mat_structure::rectangular, mat_alignment::row_major >;

extern template mat< double, mat_structure::rectangular, mat_alignment::column_major >&
...
extern template mat< double, mat_structure::rectangular, mat_alignment::column_major >&
  mat< double, mat_structure::rectangular, mat_alignment::column_major >::
    operator*=( const mat< double, mat_structure::rectangular, mat_alignment::column_major >& M );
extern template mat< double, mat_structure::rectangular, mat_alignment::row_major >&
...
extern template mat< float, mat_structure::rectangular, mat_alignment::column_major >&
...
extern template mat< float, mat_structure::rectangular, mat_alignment::column_major >&
  mat< float, mat_structure::rectangular, mat_alignment::column_major >::
    operator*=( const mat< float, mat_structure::rectangular, mat_alignment::column_major >& M );
extern template mat< float, mat_structure::rectangular, mat_alignment::row_major >&
