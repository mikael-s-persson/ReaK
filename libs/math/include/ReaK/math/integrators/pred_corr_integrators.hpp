  ...
  public:
    ...
    virtual void RK_CALL save(ReaK::serialization::oarchive& A, unsigned int) const {
      ...
      A & RK_SERIAL_SAVE_WITH_NAME(mCorrectionCount)
        & RK_SERIAL_SAVE_WITH_NAME(prevY)
        & RK_SERIAL_SAVE_WITH_NAME(prevF1)
        & RK_SERIAL_SAVE_WITH_NAME(prevF2)
        & RK_SERIAL_SAVE_WITH_NAME(prevF3);
    };
    virtual void RK_CALL load(ReaK::serialization::iarchive& A, unsigned int) {
      integrator<T>::load(A,integrator<T>::getStaticObjectType()->TypeVersion());
      A & RK_SERIAL_LOAD_WITH_NAME(mCorrectionCount)
        & RK_SERIAL_LOAD_WITH_NAME(prevY)
        & RK_SERIAL_LOAD_WITH_NAME(prevF1)
        & RK_SERIAL_LOAD_WITH_NAME(prevF2)
        & RK_SERIAL_LOAD_WITH_NAME(prevF3);
    };
...
void adamsBM3_integrator<T>::initializePrevVectors() {
  ...
  //Order 2
  prevY2 = this->prevY - (this->prevF1 * T(3.0) - integrator<T>::mStateRate) * T(integrator<T>::mStepSize * 0.5);
  ...
  //Order 3
  prevY3 = prevY2 - (this->prevF2 * T(23.0) - this->prevF1 * T(16.0) + integrator<T>::mStateRate * T(5.0)) * T(integrator<T>::mStepSize / 12.0);

  back_time -= integrator<T>::mStepSize;
  for(unsigned int j=0;j<this->mCorrectionCount;++j) {
    func_ptr->computeStateRate(back_time,prevY3,this->prevF3);
    prevY3 = prevY2 - (this->prevF3 * T(5.0) + this->prevF2 * T(8.0) - this->prevF1) * T(integrator<T>::mStepSize / 12.0);
  };
...
void RK_CALL adamsBM3_integrator<T>::integrate(double aEndTime) {
  ...
  while(((integrator<T>::mStepSize > 0.0) && (integrator<T>::mTime < aEndTime)) || ((integrator<T>::mStepSize < 0.0) && (integrator<T>::mTime > aEndTime))) {
    ...
    integrator<T>::mState += ((this->prevF1 = integrator<T>::mStateRate) * T(23) - this->prevF2 * T(16) + this->prevF3 * T(5.0)) * T(integrator<T>::mStepSize / 12.0);

    integrator<T>::mTime += integrator<T>::mStepSize;
    for(unsigned int j=0;j<this->mCorrectionCount;++j) {
      func_ptr->computeStateRate(integrator<T>::mTime,integrator<T>::mState,integrator<T>::mStateRate);
      integrator<T>::mState = this->prevY + (integrator<T>::mStateRate * T(5.0) + this->prevF1 * T(8.0) - this->prevF2) * T(integrator<T>::mStepSize / 12.0);
    };
  ...
  public:
    ...
    virtual void RK_CALL save(ReaK::serialization::oarchive& A, unsigned int) const {
      ...
      A & RK_SERIAL_SAVE_WITH_NAME(mCorrectionCount)
        ...
        & RK_SERIAL_SAVE_WITH_NAME(prevF2)
        & RK_SERIAL_SAVE_WITH_NAME(prevF3)
        & RK_SERIAL_SAVE_WITH_NAME(prevF4)
        & RK_SERIAL_SAVE_WITH_NAME(prevF5);
    };
    virtual void RK_CALL load(ReaK::serialization::iarchive& A, unsigned int) {
      integrator<T>::load(A,integrator<T>::getStaticObjectType()->TypeVersion());
      A & RK_SERIAL_LOAD_WITH_NAME(mCorrectionCount)
        ...
        & RK_SERIAL_LOAD_WITH_NAME(prevF2)
        & RK_SERIAL_LOAD_WITH_NAME(prevF3)
        & RK_SERIAL_LOAD_WITH_NAME(prevF4)
        & RK_SERIAL_LOAD_WITH_NAME(prevF5);
    };
...
void adamsBM5_integrator<T>::initializePrevVectors() {
  ...
  //Order 4
  prevY4 = prevY3 - (this->prevF3 * T(55.0) - this->prevF2 *T(59.0) + this->prevF1 * T(37.0) - integrator<T>::mStateRate * T(9.0)) * T(integrator<T>::mStepSize / 24.0);

  back_time -= integrator<T>::mStepSize;
  for(unsigned int j=0;j<this->mCorrectionCount;++j) {
    func_ptr->computeStateRate(back_time,prevY4,this->prevF4);
    prevY4 = prevY3 - (this->prevF4 * T(9.0) + this->prevF3 * T(19.0) - this->prevF2 * T(5.0) + this->prevF1) * T(integrator<T>::mStepSize / 24.0);
  };
  ...
  //Order 5
  prevY5 = prevY4 - (this->prevF4 * T(1901.0) - this->prevF3 * T(2774.0) + this->prevF2 * T(2616.0) - this->prevF1 * T(1274.0) + integrator<T>::mStateRate * T(251.0)) * T(integrator<T>::mStepSize / 720.0);

  back_time -= integrator<T>::mStepSize;
  for(unsigned int j=0;j<this->mCorrectionCount;++j) {
    func_ptr->computeStateRate(back_time,prevY5,this->prevF5);
    prevY5 = prevY4 - (this->prevF5 * T(251.0) + this->prevF4 * T(646.0) - this->prevF3 * T(264.0) + this->prevF2 * T(106.0) - this->prevF1 * T(19.0)) * T(integrator<T>::mStepSize / 720.0);
  };
...
void RK_CALL adamsBM5_integrator<T>::integrate(double aEndTime) {
  ...
  while(((integrator<T>::mStepSize > 0.0) && (integrator<T>::mTime < aEndTime)) || ((integrator<T>::mStepSize < 0.0) && (integrator<T>::mTime > aEndTime))) {
    ...
    this->prevF3 = this->prevF2;
    this->prevF2 = this->prevF1;
    integrator<T>::mState += ((this->prevF1 = integrator<T>::mStateRate) * T(1901.0) - this->prevF2 * T(2774.0) + this->prevF3 * T(2616.0) - this->prevF4 * T(1274.0) + this->prevF5 * T(251.0)) * T(integrator<T>::mStepSize / 720.0);

    integrator<T>::mTime += integrator<T>::mStepSize;
    for(unsigned int j=0;j<this->mCorrectionCount;++j) {
      func_ptr->computeStateRate(integrator<T>::mTime,integrator<T>::mState,integrator<T>::mStateRate);
      integrator<T>::mState = this->prevY + (integrator<T>::mStateRate * T(251.0) + this->prevF1 * T(646.0) - this->prevF2 * T(264.0) + this->prevF3 * T(106.0) - this->prevF4 * T(19.0)) * T(integrator<T>::mStepSize / 720.0);
    };
...
class hamming_mod_integrator : public integrator<T> {
  ...
  public:
    ...
    hamming_mod_integrator(const std::string& aName,
                       ...
                       double aStartTime,
                       double aStepSize,
                       const weak_ptr< state_rate_function<T> >& aGetStateRate) : integrator<T>(aName,aState,aStartTime,aStepSize,aGetStateRate) {
      this->initializePrevVectors();
    };
    /**
     * Default destructor.
    ...
    virtual void RK_CALL save(ReaK::serialization::oarchive& A, unsigned int) const {
      integrator<T>::save(A,integrator<T>::getStaticObjectType()->TypeVersion());
      A & RK_SERIAL_SAVE_WITH_NAME(mY_n_3)
        & RK_SERIAL_SAVE_WITH_NAME(mY_n_2)
        & RK_SERIAL_SAVE_WITH_NAME(mY_n_1)
        & RK_SERIAL_SAVE_WITH_NAME(mYp_n_2)
        & RK_SERIAL_SAVE_WITH_NAME(mYp_n_1)
        & RK_SERIAL_SAVE_WITH_NAME(mP_n)
        & RK_SERIAL_SAVE_WITH_NAME(mC);
    };
    virtual void RK_CALL load(ReaK::serialization::iarchive& A, unsigned int) {
      integrator<T>::load(A,integrator<T>::getStaticObjectType()->TypeVersion());
      A & RK_SERIAL_LOAD_WITH_NAME(mY_n_3)
        & RK_SERIAL_LOAD_WITH_NAME(mY_n_2)
        & RK_SERIAL_LOAD_WITH_NAME(mY_n_1)
        & RK_SERIAL_LOAD_WITH_NAME(mYp_n_2)
        & RK_SERIAL_LOAD_WITH_NAME(mYp_n_1)
        & RK_SERIAL_LOAD_WITH_NAME(mP_n)
...
void hamming_mod_integrator<T>::initializePrevVectors() {
  ...
  func_ptr->computeStateRate(back_time,this->mY_n_1,this->mYp_n_1);
  this->mY_n_1 -= (k1 * T(276165.0) - k2 * T(1250865.0) + (k3 = this->mYp_n_1 * T(integrator<T>::mStepSize)) * T(1167360.0)) / T(351520.0);

  back_time -= 57.0 * integrator<T>::mStepSize / 104.0;
  func_ptr->computeStateRate(back_time,this->mY_n_1,this->mYp_n_1);
  this->mY_n_1 = w - k1 * T(439.0 / 216.0) + k2 * T(8.0) - k3 * T(3680.0 / 513.0) + (k4 = this->mYp_n_1 * T(integrator<T>::mStepSize)) * T(845.0 / 4104.0);

  back_time -= integrator<T>::mStepSize / 13.0;
  func_ptr->computeStateRate(back_time,this->mY_n_1,this->mYp_n_1);
  this->mY_n_1 = w + k1 * T(8.0 / 27.0) - k2 * T(2.0) + k3 * T(3544.0 / 2565.0) - k4 * T(1859.0 / 4104.0) + (k5 = this->mYp_n_1 * T(integrator<T>::mStepSize)) * T(11.0 / 40.0);

  back_time += integrator<T>::mStepSize * 0.5;
  func_ptr->computeStateRate(back_time,this->mY_n_1,this->mYp_n_1);
  this->mY_n_1 = w - k1 * T(16.0 / 135.0) - k3 * T(6656.0 / 12825.0) - k4 * T(28561.0 / 56430.0) + k5 * T(9.0 / 50.0) - this->mYp_n_1 * T(2.0 * integrator<T>::mStepSize / 55.0);
  ...
  back_time -= integrator<T>::mStepSize * 0.125;
  func_ptr->computeStateRate(back_time,this->mY_n_2,this->mYp_n_2);
  this->mY_n_2 -= (k1 * T(276165.0) - k2 * T(1250865.0) + (k3 = this->mYp_n_2 * T(integrator<T>::mStepSize)) * T(1167360.0)) / T(351520.0);

  back_time -= 57.0 * integrator<T>::mStepSize / 104.0;
  func_ptr->computeStateRate(back_time,this->mY_n_2,this->mYp_n_2);
  this->mY_n_2 = w - k1 * T(439.0 / 216.0) + k2 * T(8.0) - k3 * T(3680.0 / 513.0) + (k4 = this->mYp_n_2 * T(integrator<T>::mStepSize)) * T(845.0 / 4104.0);

  back_time -= integrator<T>::mStepSize / 13.0;
  func_ptr->computeStateRate(back_time,this->mY_n_2,this->mYp_n_2);
  this->mY_n_2 = w + k1 * T(8.0 / 27.0) - k2 * T(2.0) + k3 * T(3544.0 / 2565.0) - k4 * T(1859.0 / 4104.0) + (k5 = this->mYp_n_2 * T(integrator<T>::mStepSize)) * T(11.0 / 40.0);

  back_time += integrator<T>::mStepSize * 0.5;
  func_ptr->computeStateRate(back_time,this->mY_n_2,this->mYp_n_2);
  this->mY_n_2 = w - k1 * T(16.0 / 135.0) - k3 * T(6656.0 / 12825.0) - k4 * T(28561.0 / 56430.0) + k5 * T(9.0 / 50.0) - this->mYp_n_2 * T(2.0 * integrator<T>::mStepSize / 55.0);
  ...
  back_time -= integrator<T>::mStepSize * 0.125;
  func_ptr->computeStateRate(back_time,this->mY_n_3,this->mC);
  this->mY_n_3 -= (k1 * T(276165.0) - k2 * T(1250865.0) + (k3 = this->mC * T(integrator<T>::mStepSize)) * T(1167360.0)) / T(351520.0);

  back_time -= 57.0 * integrator<T>::mStepSize / 104.0;
  func_ptr->computeStateRate(back_time,this->mY_n_3,this->mC);
  this->mY_n_3 = w - k1 * T(439.0 / 216.0) + k2 * T(8.0) - k3 * T(3680.0 / 513.0) + (k4 = this->mC * T(integrator<T>::mStepSize)) * T(845.0 / 4104.0);

  back_time -= integrator<T>::mStepSize / 13.0;
  func_ptr->computeStateRate(back_time,this->mY_n_3,this->mC);
  this->mY_n_3 = w + k1 * T(8.0 / 27.0) - k2 * T(2.0) + k3 * T(3544.0 / 2565.0) - k4 * T(1859.0 / 4104.0) + (k5 = this->mC * T(integrator<T>::mStepSize)) * T(11.0 / 40.0);

  back_time += integrator<T>::mStepSize * 0.5;
  func_ptr->computeStateRate(back_time,this->mY_n_3,this->mC);
  this->mY_n_3 = w - k1 * T(16.0 / 135.0) - k3 * T(6656.0 / 12825.0) - k4 * T(28561.0 / 56430.0) + k5 * T(9.0 / 50.0) - this->mC * T(2.0 * integrator<T>::mStepSize / 55.0);
...
void RK_CALL hamming_mod_integrator<T>::integrate(double aEndTime) {
  ...
  while(((integrator<T>::mStepSize > 0.0) && (integrator<T>::mTime < aEndTime)) || ((integrator<T>::mStepSize < 0.0) && (integrator<T>::mTime > aEndTime))) {
    ...
    this->mP = this->mY_n_3 + ((integrator<T>::mStateRate + this->mYp_n_2) * T(2.0) - this->mYp_n_1) * T(integrator<T>::mStepSize * 4.0 / 3.0);
    this->mM = this->mP - (this->mP_n - this->mC) * T(112.0 / 121.0);

    func_ptr->computeStateRate(integrator<T>::mTime,this->mM,this->mMp);
    this->mC = (integrator<T>::mState * T(9.0) - this->mY_n_2 + (this->mMp + integrator<T>::mStateRate * T(2.0) - this->mYp_n_1) * T(integrator<T>::mStepSize * 3.0)) * T(0.125);
...
void RK_CALL hamming_iter_mod_integrator<T>::integrate(double aEndTime) {
  ...
  while(((integrator<T>::mStepSize > 0.0) && (integrator<T>::mTime < aEndTime)) || ((integrator<T>::mStepSize < 0.0) && (integrator<T>::mTime > aEndTime))) {
    ...
    hamming_mod_integrator<T>::mP = hamming_mod_integrator<T>::mY_n_3 + ((integrator<T>::mStateRate + hamming_mod_integrator<T>::mYp_n_2) * T(2.0) - hamming_mod_integrator<T>::mYp_n_1) * T(integrator<T>::mStepSize * 4.0 / 3.0);
    prevM = hamming_mod_integrator<T>::mP - (hamming_mod_integrator<T>::mP_n - hamming_mod_integrator<T>::mC) * T(112.0 / 121.0);
    ...
    while((ErrorEst > this->mTolerance) && (i < this->mMaxIter)) {
      ...
      hamming_mod_integrator<T>::mC = (integrator<T>::mState * T(9.0) - hamming_mod_integrator<T>::mY_n_2 + (hamming_mod_integrator<T>::mMp + integrator<T>::mStateRate * T(2.0) - hamming_mod_integrator<T>::mYp_n_1) * T(integrator<T>::mStepSize * 3.0)) * T(0.125);

      hamming_mod_integrator<T>::mM = hamming_mod_integrator<T>::mC + (hamming_mod_integrator<T>::mP - hamming_mod_integrator<T>::mC) * T(9.0 / 121.0);
      ErrorEst = 0.0;
