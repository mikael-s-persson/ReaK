...
public:
   ...
   */
  virtual void compute_cost_hessian( mat< double, mat_structure::symmetric >& H, const vect_n< double >& x, double f,
...
public:
  ...
  virtual vect_n< double > compute_cost_grad( const vect_n< double >& x ) const { return Q * ( x - c ); };

  virtual void compute_cost_hessian( mat< double, mat_structure::symmetric >& H, const vect_n< double >&, double,
...
public:
  ...
  added_cost_evaluator( const shared_ptr< cost_evaluator >& aFirstEval = shared_ptr< cost_evaluator >(),
      ...
      : first_eval( aFirstEval ), second_eval( aSecondEval ){};
  ...
  *******************************************************************************/
  ...
  virtual void RK_CALL save( ReaK::serialization::oarchive& A, unsigned int ) const {
    cost_evaluator::save( A, cost_evaluator::getStaticObjectType()->TypeVersion() );
    A& RK_SERIAL_SAVE_WITH_NAME( first_eval ) & RK_SERIAL_SAVE_WITH_NAME( second_eval );
  };
  virtual void RK_CALL load( ReaK::serialization::iarchive& A, unsigned int ) {
    cost_evaluator::load( A, cost_evaluator::getStaticObjectType()->TypeVersion() );
    A& RK_SERIAL_LOAD_WITH_NAME( first_eval ) & RK_SERIAL_LOAD_WITH_NAME( second_eval );
  };
...
struct oop_cost_function {
   ...
   */
  double operator()( const vect_n< double >& x ) const { return parent->compute_cost( x ); };
};
...
struct oop_cost_grad {
   ...
   */
  vect_n< double > operator()( const vect_n< double >& x ) const { return parent->compute_cost_grad( x ); };
};
...
struct oop_cost_hess {
   ...
   */
  void operator()( mat< double, mat_structure::symmetric >& H, const vect_n< double >& x, double f,
                   const vect_n< double >& x_grad ) const {
