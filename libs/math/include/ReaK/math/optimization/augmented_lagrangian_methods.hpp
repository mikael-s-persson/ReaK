...
void bcl_newton_method_tr_impl(
  ...
  IneqJacFunction fill_h_jac, Vector& x, typename vect_traits< Vector >::value_type max_radius, unsigned int max_iter,
  ...
  if( ( M == 0 ) && ( K == 0 ) ) { // this means it is an unconstrained problem.
    newton_method_tr_impl( f, df, fill_hessian, x, max_radius, max_iter, solve_step, impose_limits, abs_tol,
                           abs_grad_tol, kappa );
    return;
  };
  ...
  while( true ) {
    ...
    do {
      ...
      ValueType Lt_value = f( xt ) - l_g * gt_value - l_h * ht_value
    ...
    if( c_norm < eta * c_norm_max ) {
      ...
      if( ( c_norm < eta_star ) && ( norm_p < omega_star ) ) // in theory, this test should involve the projected
...
struct constraint_newton_method_tr_factory {
   ...
   * non-negativity).
   */
  constraint_newton_method_tr_factory( Function aF, GradFunction aDf, HessianFunction aFillHessian, T aMaxRadius,
                                       unsigned int aMaxIter, EqFunction aG = EqFunction(),
                                       EqJacFunction aFillGJac = EqJacFunction(), IneqFunction aH = IneqFunction(),
      ...
      : f( aF ), df( aDf ), fill_hessian( aFillHessian ), max_radius( aMaxRadius ), max_iter( aMaxIter ),
        abs_tol( aTol ), abs_grad_tol( aGradTol ), eta( aEta ), g( aG ), fill_g_jac( aFillGJac ), h( aH ),
        fill_h_jac( aFillHJac ), solve_step( aSolveStep ), impose_limits( aImposeLimits ){};
  /**
  ...
  void operator()( Vector& x ) const {
    detail::bcl_newton_method_tr_impl( f, df, fill_hessian, g, fill_g_jac, h, fill_h_jac, x, max_radius, max_iter,
                                       solve_step, impose_limits, abs_tol, abs_grad_tol, eta );
  };
   ...
   */
  constraint_newton_method_tr_factory< Function, GradFunction, HessianFunction, T, EqFunction, EqJacFunction,
                                       IneqFunction, IneqJacFunction, trust_region_solver_dogleg_reg< T >,
                                       LimitFunction >
    regularize( const T& tau ) const {
    return constraint_newton_method_tr_factory< Function, GradFunction, HessianFunction, T, EqFunction, EqJacFunction,
                                                IneqFunction, IneqJacFunction, trust_region_solver_dogleg_reg< T >,
                                                LimitFunction >(
      f, df, fill_hessian, max_radius, max_iter, g, fill_g_jac, h, fill_h_jac, abs_tol, abs_grad_tol, eta,
  ...
  constraint_newton_method_tr_factory< Function, GradFunction, HessianFunction, T, EqFunction, EqJacFunction,
    ...
    return constraint_newton_method_tr_factory< Function, GradFunction, HessianFunction, T, EqFunction, EqJacFunction,
                                                IneqFunction, IneqJacFunction, NewTrustRegionSolver, LimitFunction >(
      f, df, fill_hessian, max_radius, max_iter, g, fill_g_jac, h, fill_h_jac, abs_tol, abs_grad_tol, eta, new_solver,
      impose_limits );
  };
  ...
  constraint_newton_method_tr_factory< Function, GradFunction, HessianFunction, T, EqFunction, EqJacFunction,
    ...
    return constraint_newton_method_tr_factory< Function, GradFunction, HessianFunction, T, EqFunction, EqJacFunction,
                                                IneqFunction, IneqJacFunction, TrustRegionSolver, NewLimitFunction >(
      f, df, fill_hessian, max_radius, max_iter, g, fill_g_jac, h, fill_h_jac, abs_tol, abs_grad_tol, eta, solve_step,
      new_limits );
  };
  ...
  constraint_newton_method_tr_factory< Function, GradFunction, HessianFunction, T, NewEqFunction, NewEqJacFunction,
    ...
    return constraint_newton_method_tr_factory< Function, GradFunction, HessianFunction, T, NewEqFunction,
  ...
  constraint_newton_method_tr_factory< Function, GradFunction, HessianFunction, T, EqFunction, EqJacFunction,
    ...
    return constraint_newton_method_tr_factory< Function, GradFunction, HessianFunction, T, EqFunction, EqJacFunction,
                                                NewIneqFunction, NewIneqJacFunction, TrustRegionSolver, LimitFunction >(
      f, df, fill_hessian, max_radius, max_iter, g, fill_g_jac, new_h, new_fill_h_jac, abs_tol, abs_grad_tol, eta,
...
constraint_newton_method_tr_factory< Function, GradFunction, HessianFunction, T >
  make_constraint_newton_method_tr( Function f, GradFunction df, HessianFunction fill_hessian, T max_radius,
                                    unsigned int max_iter, T abs_tol = T( 1e-6 ), T abs_grad_tol = T( 1e-6 ),
                                    T eta = T( 1e-4 ) ) {
  return constraint_newton_method_tr_factory< Function, GradFunction, HessianFunction, T >(
    f, df, fill_hessian, max_radius, max_iter, no_constraint_functor(), no_constraint_jac_functor(),
    no_constraint_functor(), no_constraint_jac_functor(), abs_tol, abs_grad_tol, eta );
};
...
void constraint_newton_method_tr( Function f, GradFunction df, HessianFunction fill_hessian, Vector& x, EqFunction g,
 ...
 */
template < typename Function, typename GradFunction, typename HessianFunction, typename Vector, typename EqFunction,
           typename EqJacFunction >
void eq_cnstr_newton_method_tr( Function f, GradFunction df, HessianFunction fill_hessian, Vector& x, EqFunction g,
                                EqJacFunction fill_g_jac, typename vect_traits< Vector >::value_type max_radius,
                                unsigned int max_iter = 100, typename vect_traits< Vector >::value_type abs_tol
                                                             = typename vect_traits< Vector >::value_type( 1e-6 ),
                                typename vect_traits< Vector >::value_type abs_grad_tol
                                = typename vect_traits< Vector >::value_type( 1e-6 ) ) {

  detail::bcl_newton_method_tr_impl( f, df, fill_hessian, g, fill_g_jac, no_constraint_functor(),
                                     no_constraint_jac_functor(), x, max_radius, max_iter, trust_region_solver_dogleg(),
 ...
 * \param abs_grad_tol The tolerance on the norm of the gradient.
 */
template < typename Function, typename GradFunction, typename HessianFunction, typename Vector, typename IneqFunction,
           typename IneqJacFunction >
void ineq_cnstr_newton_method_tr( Function f, GradFunction df, HessianFunction fill_hessian, Vector& x, IneqFunction h,
                                  IneqJacFunction fill_h_jac, typename vect_traits< Vector >::value_type max_radius,
                                  unsigned int max_iter = 100, typename vect_traits< Vector >::value_type abs_tol
                                                               = typename vect_traits< Vector >::value_type( 1e-6 ),
                                  typename vect_traits< Vector >::value_type abs_grad_tol
                                  = typename vect_traits< Vector >::value_type( 1e-6 ) ) {

  detail::bcl_newton_method_tr_impl( f, df, fill_hessian, no_constraint_functor(), no_constraint_jac_functor(), h,
                                     fill_h_jac, x, max_radius, max_iter, trust_region_solver_dogleg(),
                                     no_limit_functor(), abs_tol, abs_grad_tol );
};
};
};
