...
template < typename Function, typename GradFunction, typename HessianFunction, typename Vector, typename EqFunction,
           typename EqJacFunction, typename TrustRegionSolver, typename LimitFunction >
void byrd_omojokun_sqp_method_tr_impl(
  Function f, GradFunction df, HessianFunction fill_hessian, EqFunction g, EqJacFunction fill_g_jac, Vector& x,
  ...
  while( ( norm_star > abs_tol ) || ( c_norm_star > abs_tol ) ) {
    ...
    ValueType pred = mu * ( c_norm_star - norm_2( g_value + Jac_g * p ) ) - x_grad * p - pHp;
    xt = x;
    xt += p;
    ...
    // RK_NOTICE(1,"  H = " << H << " Jac_g = " << Jac_g << " p = " << p << " pHp = " << pHp << " grad * p = " << x_grad
    // * p);
    // RK_NOTICE(1,"  pred = " << pred << " ared = " << pred << " mu = " << mu << " c_norm = " << c_norm_star << "
    // ct_norm = " << norm_2(gt_value));
...
struct bosqp_newton_tr_factory {
  ...
  typedef bosqp_newton_tr_factory< Function, GradFunction, HessianFunction, T, EqFunction, EqJacFunction,
   ...
   * non-negativity).
   */
  bosqp_newton_tr_factory( Function aF, GradFunction aDf, HessianFunction aFillHessian, T aMaxRadius,
                           unsigned int aMaxIter, EqFunction aG = EqFunction(),
                           EqJacFunction aFillGJac = EqJacFunction(), T aTol = T( 1e-6 ), T aEta = T( 1e-4 ),
                           T aRho = T( 1e-4 ), TrustRegionSolver aSolveStep = TrustRegionSolver(),
                           LimitFunction aImposeLimits = LimitFunction() )
      : f( aF ), df( aDf ), fill_hessian( aFillHessian ), max_radius( aMaxRadius ), max_iter( aMaxIter ), tol( aTol ),
        eta( aEta ), rho( aRho ), g( aG ), fill_g_jac( aFillGJac ), solve_step( aSolveStep ),
  ...
  void operator()( Vector& x ) const {
    detail::byrd_omojokun_sqp_method_tr_impl( f, df, hessian_update_dual_exact< HessianFunction >( fill_hessian ), g,
                                              fill_g_jac, x, max_radius, max_iter, solve_step, impose_limits, tol, eta,
                                              rho );
  };
   ...
   */
  bosqp_newton_tr_factory< Function, GradFunction, HessianFunction, T, EqFunction, EqJacFunction,
                           tr_solver_right_pinv_dogleg_reg< T >, LimitFunction >
    regularize( const T& tau ) const {
    return bosqp_newton_tr_factory< Function, GradFunction, HessianFunction, T, EqFunction, EqJacFunction,
                                    tr_solver_right_pinv_dogleg_reg< T >, LimitFunction >(
  ...
  bosqp_newton_tr_factory< Function, GradFunction, HessianFunction, T, EqFunction, EqJacFunction, NewTrustRegionSolver,
                           LimitFunction >
    set_tr_solver( NewTrustRegionSolver new_solver ) const {
    return bosqp_newton_tr_factory< Function, GradFunction, HessianFunction, T, EqFunction, EqJacFunction,
                                    NewTrustRegionSolver, LimitFunction >(
      f, df, fill_hessian, max_radius, max_iter, g, fill_g_jac, tol, eta, rho, new_solver, impose_limits );
  };
  ...
  bosqp_newton_tr_factory< Function, GradFunction, HessianFunction, T, EqFunction, EqJacFunction, TrustRegionSolver,
                           NewLimitFunction >
    set_limiter( NewLimitFunction new_limits ) const {
    return bosqp_newton_tr_factory< Function, GradFunction, HessianFunction, T, EqFunction, EqJacFunction,
                                    TrustRegionSolver, NewLimitFunction >(
      f, df, fill_hessian, max_radius, max_iter, g, fill_g_jac, tol, eta, rho, solve_step, new_limits );
  };
  ...
  bosqp_newton_tr_factory< Function, GradFunction, HessianFunction, T, NewEqFunction, NewEqJacFunction,
                           TrustRegionSolver, LimitFunction >
    set_eq_constraints( NewEqFunction new_g, NewEqJacFunction new_fill_g_jac ) const {
    return bosqp_newton_tr_factory< Function, GradFunction, HessianFunction, T, NewEqFunction, NewEqJacFunction,
                                    TrustRegionSolver, LimitFunction >(
      f, df, fill_hessian, max_radius, max_iter, new_g, new_fill_g_jac, tol, eta, rho, solve_step, impose_limits );
  };
};
...
bosqp_newton_tr_factory< Function, GradFunction, HessianFunction, T >
  make_bosqp_newton_tr( Function f, GradFunction df, HessianFunction fill_hessian, T max_radius, unsigned int max_iter,
                        T tol = T( 1e-6 ), T eta = T( 1e-4 ), T rho = T( 1e-4 ) ) {
  return bosqp_newton_tr_factory< Function, GradFunction, HessianFunction, T >(
    f, df, fill_hessian, max_radius, max_iter, no_constraint_functor(), no_constraint_jac_functor(), tol, eta, rho );
};
...
template < typename Function, typename GradFunction, typename T, typename EqFunction = no_constraint_functor,
           typename EqJacFunction = no_constraint_jac_functor, typename HessianUpdater = hessian_update_sr1,
           typename TrustRegionSolver = tr_solver_right_pinv_dogleg, typename LimitFunction = no_limit_functor >
struct bosqp_quasi_newton_tr_factory {
  ...
  typedef bosqp_quasi_newton_tr_factory< Function, GradFunction, T, EqFunction, EqJacFunction, HessianUpdater,
                                         TrustRegionSolver, LimitFunction > self;
   ...
   * non-negativity).
   */
  bosqp_quasi_newton_tr_factory( Function aF, GradFunction aDf, T aMaxRadius, unsigned int aMaxIter,
                                 ...
                                 LimitFunction aImposeLimits = LimitFunction() )
      : f( aF ), df( aDf ), max_radius( aMaxRadius ), max_iter( aMaxIter ), tol( aTol ), eta( aEta ), rho( aRho ),
        g( aG ), fill_g_jac( aFillGJac ), update_hessian( aUpdateHessian ), solve_step( aSolveStep ),
        impose_limits( aImposeLimits ){};
  /**
   * This function finds the minimum of a function, given its derivative and Hessian,
   * using a newton search direction and using a trust-region approach.
  ...
  template < typename Vector >
  void operator()( Vector& x ) const {
    detail::byrd_omojokun_sqp_method_tr_impl( f, df, hessian_update_dual_quasi< HessianUpdater >( update_hessian ), g,
                                              fill_g_jac, x, max_radius, max_iter, solve_step, impose_limits, tol, eta,
                                              rho );
  };
   ...
   * \param tau The initial relative damping factor to regularize the Hessian matrix.
   */
  bosqp_quasi_newton_tr_factory< Function, GradFunction, T, EqFunction, EqJacFunction, HessianUpdater,
                                 tr_solver_right_pinv_dogleg_reg< T >, LimitFunction >
    regularize( const T& tau ) const {
    return bosqp_quasi_newton_tr_factory< Function, GradFunction, T, EqFunction, EqJacFunction, HessianUpdater,
                                          tr_solver_right_pinv_dogleg_reg< T >, LimitFunction >(
      f, df, max_radius, max_iter, g, fill_g_jac, tol, eta, rho, update_hessian,
      tr_solver_right_pinv_dogleg_reg< T >( tau ), impose_limits );
  };
  ...
  bosqp_quasi_newton_tr_factory< Function, GradFunction, T, EqFunction, EqJacFunction, HessianUpdater,
    ...
    return bosqp_quasi_newton_tr_factory< Function, GradFunction, T, EqFunction, EqJacFunction, HessianUpdater,
                                          NewTrustRegionSolver, LimitFunction >(
      f, df, max_radius, max_iter, g, fill_g_jac, tol, eta, rho, update_hessian, new_solver, impose_limits );
  };
  ...
  bosqp_quasi_newton_tr_factory< Function, GradFunction, T, EqFunction, EqJacFunction, HessianUpdater,
    ...
    return bosqp_quasi_newton_tr_factory< Function, GradFunction, T, EqFunction, EqJacFunction, HessianUpdater,
                                          TrustRegionSolver, NewLimitFunction >(
      f, df, max_radius, max_iter, g, fill_g_jac, tol, eta, rho, update_hessian, solve_step, new_limits );
  };
   ...
   */
  template < typename NewEqFunction, typename NewEqJacFunction >
  bosqp_quasi_newton_tr_factory< Function, GradFunction, T, NewEqFunction, NewEqJacFunction, HessianUpdater,
                                 TrustRegionSolver, LimitFunction >
    set_eq_constraints( NewEqFunction new_g, NewEqJacFunction new_fill_g_jac ) const {
    return bosqp_quasi_newton_tr_factory< Function, GradFunction, T, NewEqFunction, NewEqJacFunction, HessianUpdater,
                                          TrustRegionSolver, LimitFunction >(
      f, df, max_radius, max_iter, new_g, new_fill_g_jac, tol, eta, rho, update_hessian, solve_step, impose_limits );
  };
  ...
  bosqp_quasi_newton_tr_factory< Function, GradFunction, T, EqFunction, EqJacFunction, NewHessianUpdater,
                                 TrustRegionSolver, LimitFunction >
    set_hessian_updater( NewHessianUpdater new_update_hessian ) const {
    return bosqp_quasi_newton_tr_factory< Function, GradFunction, T, EqFunction, EqJacFunction, NewHessianUpdater,
                                          TrustRegionSolver, LimitFunction >(
      f, df, max_radius, max_iter, g, fill_g_jac, tol, eta, rho, new_update_hessian, solve_step, impose_limits );
  };
};
...
bosqp_quasi_newton_tr_factory< Function, GradFunction, T >
  make_bosqp_quasi_newton_tr( Function f, GradFunction df, T max_radius, unsigned int max_iter, T tol = T( 1e-6 ),
                              T eta = T( 1e-4 ), T rho = T( 1e-4 ) ) {
  return bosqp_quasi_newton_tr_factory< Function, GradFunction, T >(
    f, df, max_radius, max_iter, no_constraint_functor(), no_constraint_jac_functor(), tol, eta, rho );
};
};
};
