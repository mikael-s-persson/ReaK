    ...
    do {
      ...
      while((++k <= max_iter) && (Err_value > abs_tol_mu)) {
        ...
        //if(ared >= kappa * pred) {
          ...
          mS = mat<ValueType,mat_structure::diagonal>(-s);
          log_s = log_st;
          c[range(0,M)] = gt_value;   gt_norm = norm_2(gt_value) / sqrt(ValueType(M));
          c[range(M,M+K)] = ht_value; ht_norm = norm_2(ht_value) / sqrt(ValueType(K));
...
//         sub(J_aug)(range(M,M+K),range(N,N+K)) = (ValueType(-1.0) / mu_sqrt) * mat<ValueType,mat_structure::diagonal>(s);
//         
