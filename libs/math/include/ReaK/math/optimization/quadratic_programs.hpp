...
void null_space_QP_method( const Matrix1& A, const Vector1& b, const Matrix2& G, const Vector2& c, Vector2& x,
 ...
 */
template < typename Matrix1, typename Vector1, typename Matrix2, typename Vector2 >
void null_space_RRQP_method( const Matrix1& A, const Vector1& b, const Matrix2& G, const Vector2& c, Vector2& x,
                             typename vect_traits< Vector1 >::value_type abs_tol
                             = std::numeric_limits< typename vect_traits< Vector1 >::value_type >::epsilon(),
                             typename vect_traits< Vector1 >::value_type max_norm
                             = std::numeric_limits< typename vect_traits< Vector1 >::value_type >::infinity(),
  ...
  mat< ValueType, mat_structure::square > Q
    = mat< ValueType, mat_structure::square >( mat< ValueType, mat_structure::identity >( N ) );
  ...
  // RK_NOTICE(1," reached!");
  x = Q * x_tmp;
  // RK_NOTICE(1," reached! with size(x) = " << x.size() << " and P.get_col_count() = " << P.get_col_count() << "
  // size(A) = (" << A.get_row_count() << ", " << A.get_col_count() << ")");
  // RK_NOTICE(1,"   with size(R) = (" << R.get_row_count() << ", " << R.get_col_count() << ") and Q.get_col_count() = "
  // << Q.get_col_count());
  x = P * x;
  // RK_NOTICE(1," reached!");
  if( lambda ) {
 ...
 */
template < typename Matrix1, typename Vector1, typename Matrix2, typename Vector2 >
void projected_CG_method( const Matrix1& A, const Vector1& b, const Matrix2& G, const Vector2& c, Vector2& x,
