...
public:
  ...
  *******************************************************************************/
   ...
   */
  explicit quat( const scalar_type& aRe = scalar_type() ) BOOST_NOEXCEPT {
    q[0] = aRe;
   ...
   */
  quat( const scalar_type& aRe, const vector_type& aIm ) BOOST_NOEXCEPT {
    q[0] = aRe;
    q[1] = aIm[0];
    q[2] = aIm[1];
    q[3] = aIm[2];
   ...
   */
  explicit quat( const vector_type& aIm ) BOOST_NOEXCEPT {
    q[0] = scalar_type();
    q[1] = aIm[0];
    q[2] = aIm[1];
    q[3] = aIm[2];
   ...
   */
  explicit quat( const vect< value_type, 4 >& V ) BOOST_NOEXCEPT {
    q[0] = V[0];
    q[1] = V[1];
    q[2] = V[2];
    q[3] = V[3];
  ...
  *******************************************************************************/
  ...
  self& operator*=( const self& C ) BOOST_NOEXCEPT { return ( *this = ( ( *this ) * C ) ); };
  ...
  *******************************************************************************/
  ...
  /** Negation operator. */
  friend self operator-( const self& Q ) BOOST_NOEXCEPT { return self( -Q.q[0], -Q.q[1], -Q.q[2], -Q.q[3] ); };
  ...
  *******************************************************************************/
  ...
  friend bool operator==( const self& C, const scalar_type& R ) BOOST_NOEXCEPT {
    return ( ( C.q[0] == R ) && ( C.q[1] == value_type() ) && ( C.q[2] == value_type() )
             && ( C.q[3] == value_type() ) );
  };
  ...
  friend self conj( const self& x ) BOOST_NOEXCEPT { return self( x.q[0], -x.q[1], -x.q[2], -x.q[3] ); };
   ...
   * \test PASSED
   */
  friend self unit( const self& v ) BOOST_NOEXCEPT { return v * ( 1.0 / norm_2( v ) ); };
  ...
  friend value_type fabs( const self& x ) BOOST_NOEXCEPT { return norm_2( x ); };
  ...
  friend self atan2( const self& y, const self& x ) BOOST_NOEXCEPT { return atan( y * invert( x ) ); };
...
public:
  ...
  *******************************************************************************/
   ...
   * \test PASSED
   */
  friend value_type norm_2_sqr( const self& v ) BOOST_NOEXCEPT { return value_type( 1.0 ); };

  /**
   * Magnitude of the quaternion.
   * \test PASSED
   */
  friend value_type norm_2( const self& v ) BOOST_NOEXCEPT { return value_type( 1.0 ); };
   ...
   * \test PASSED
   */
  friend self unit( const self& v ) BOOST_NOEXCEPT { return v; };
  ...
  /** Compute square root (function), for a quaternion value.*/
  friend self sqrt( const self& x ) BOOST_NOEXCEPT { return exp( 0.5 * log( x ) ); };

  /**
   * Inverts the quaternion.
   */
  friend self invert( const self& x ) BOOST_NOEXCEPT { return conj( x ); };
...
inline void from_vect_impl( unit_quat< T >& lhs, const Vector2& rhs, std::size_t& i ) {
