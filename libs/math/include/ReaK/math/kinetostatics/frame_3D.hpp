...
public:
  ...
  angular_vector_type AngAcceleration; ///< Angular Acceleration of this kinematic frame, relative to parent
  ...
  angular_rate_type QuatDot; ///< Quaternion time-derivative, relative to parent coordinate, expressed in this
   ...
   */
  frame_3D( const weak_ptr< base >& aParent, const position_type& aPosition, const rotation_type& aQuat,
      ...
      : base( aParent, aPosition, aQuat ), Velocity( aVelocity ), AngVelocity( aAngVelocity ),
  ...
  frame_3D( const self& aFrame )
      : base( aFrame ), Velocity( aFrame.Velocity ), AngVelocity( aFrame.AngVelocity ),
  ...
  explicit frame_3D( const base& Pose_ )
      : base( Pose_ ), Velocity(), AngVelocity(), Acceleration(), AngAcceleration(), Force(), Torque(), QuatDot(){};
...
public:
  ...
  self getGlobalFrame() const {
    if( !this->Parent.expired() ) {
      ...
      result.Acceleration += R * ( ( result.AngVelocity % ( result.AngVelocity % this->Position ) )
      ...
      result.AngAcceleration = ( R2 * result.AngAcceleration ) + ( ( R2 * result.AngVelocity ) % AngVelocity )
                               + AngAcceleration;
  ...
  self& addBefore( const self& aFrame ) {
    ...
    Acceleration
      += R * ( ( AngVelocity % ( AngVelocity % aFrame.Position ) ) + T( 2.0 ) * ( AngVelocity % aFrame.Velocity )
  ...
  self operator~() {
    ...
    result.Acceleration = -( ( result.AngVelocity % ( result.AngVelocity % this->Position ) )
                             + ( value_type( 2.0 ) * result.AngVelocity % Velocity )
                             + ( result.AngAcceleration % this->Position ) + Acceleration ) * R;
...
std::ostream& operator<<( std::ostream& out, const frame_3D< T >& g ) {
  out << "(Position = " << g.Position << "; Quaternion = " << g.Quat << "; Velocity = " << g.Velocity
