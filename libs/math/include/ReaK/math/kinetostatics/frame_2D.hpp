...
public:
  ...
  linear_vector_type
    Velocity; ///< Velocity vector of this kinematic frame, relative-to and expressed-in parent coordinates.
  angular_vector_type AngVelocity; ///< Angular velocity of this kinematic frame, relative to parent coordinates.

  linear_vector_type
    Acceleration; ///< Acceleration of this kinematic frame, relative-to and expressed-in parent coordinates.
  angular_vector_type
    AngAcceleration; ///< Angular Acceleration of this kinematic frame, relative to parent coordinates.
   ...
   */
  frame_2D()
      : pose_2D< T >(), Velocity(), AngVelocity( 0.0 ), Acceleration(), AngAcceleration( 0.0 ), Force(),
        Torque( 0.0 ){};

  /**
   * Parametrized constructor, all is set to corresponding parameters.
   */
  frame_2D( const weak_ptr< base >& aParent, const position_type& aPosition, const rotation_type& aRotation,
            const linear_vector_type& aVelocity, const angular_vector_type& aAngVelocity,
            const linear_vector_type& aAcceleration, const angular_vector_type& aAngAcceleration,
            const linear_vector_type& aForce, const angular_vector_type& aTorque )
      : pose_2D< T >( aParent, aPosition, aRotation ), Velocity( aVelocity ), AngVelocity( aAngVelocity ),
        Acceleration( aAcceleration ), AngAcceleration( aAngAcceleration ), Force( aForce ), Torque( aTorque ){};

  /**
   * Copy-constructor.
   */
  frame_2D( const self& aFrame )
      : pose_2D< T >( aFrame ), Velocity( aFrame.Velocity ), AngVelocity( aFrame.AngVelocity ),
        Acceleration( aFrame.Acceleration ), AngAcceleration( aFrame.AngAcceleration ), Force( aFrame.Force ),
  ...
  explicit frame_2D( const base& aPose )
      : pose_2D< T >( aPose ), Velocity(), AngVelocity( 0.0 ), Acceleration(), AngAcceleration( 0.0 ), Force(),
        Torque( 0.0 ){};
...
public:
  ...
  self getGlobalFrame() const {
    if( !this->Parent.expired() ) {
      ...
      result.Acceleration += result.Rotation * ( ( -result.AngVelocity * result.AngVelocity ) * this->Position
   ...
   */
  self getFrameRelativeTo( const base& F ) const { return getFrameRelativeToImpl( &F ); };
  ...
  self& addBefore( const self& aFrame ) {
    ...
    Acceleration += this->Rotation * ( ( -AngVelocity * AngVelocity ) * aFrame.Position
  ...
  self& addAfter( const self& aFrame ) {
    ...
    Acceleration = aFrame.Acceleration
                   + aFrame.Rotation * ( ( -aFrame.AngVelocity * aFrame.AngVelocity ) * this->Position
  ...
  self& operator*=( const self& F ) {
    ...
    Acceleration += this->Rotation
                    * ( ( -AngVelocity * AngVelocity ) * F.Position + ( value_type( 2.0 ) * AngVelocity ) % F.Velocity
  ...
  friend self operator*( const self& F1, const self& F2 ) {
    ...
    result.Acceleration = F1.Acceleration
  ...
  friend self operator*( const self& F, const base& P ) {
    ...
    result.Acceleration = F.Acceleration
                          + F.Rotation
                            * ( ( -F.AngVelocity * F.AngVelocity ) * P.Position + F.AngAcceleration % P.Position );
   ...
   */
  self operator~() const {
    return self( this->Parent, ( -this->Position ) * this->Rotation, invert( this->Rotation ),
                 ( ( AngVelocity % this->Position ) - Velocity ) * this->Rotation, -AngVelocity,
                 ( ( AngVelocity * AngVelocity * this->Position ) + ( value_type( 2.0 ) * AngVelocity ) % Velocity
                   + AngAcceleration % this->Position - Acceleration ) * this->Rotation,
  ...
  *******************************************************************************/
  ...
  virtual void RK_CALL save( ReaK::serialization::oarchive& A, unsigned int ) const {
    ...
    A& RK_SERIAL_SAVE_WITH_NAME( Velocity ) & RK_SERIAL_SAVE_WITH_NAME( AngVelocity )
      & RK_SERIAL_SAVE_WITH_NAME( Acceleration ) & RK_SERIAL_SAVE_WITH_NAME( AngAcceleration )
  ...
  virtual void RK_CALL load( ReaK::serialization::iarchive& A, unsigned int ) {
    ...
    A& RK_SERIAL_LOAD_WITH_NAME( Velocity ) & RK_SERIAL_LOAD_WITH_NAME( AngVelocity )
      & RK_SERIAL_LOAD_WITH_NAME( Acceleration ) & RK_SERIAL_LOAD_WITH_NAME( AngAcceleration )
...
std::ostream& operator<<( std::ostream& out, const frame_2D< T >& g ) {
  out << "(Position = " << g.Position << "; Rotation = " << g.Rotation << "; Velocity = " << g.Velocity
      << "; AngVelocity = " << g.AngVelocity << "; Acceleration = " << g.Acceleration
      << "; AngAcceleration = " << g.AngAcceleration << "; Force = " << g.Force << "; Torque = " << g.Torque << ")";
  return out;
};
