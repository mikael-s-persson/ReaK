...
protected:
  ...
  void compute_IHAQR_data( const point_type& a ) const {
    ...
    try {
      ...
      solve_IHCT_AQR( a.lin_data->A, a.lin_data->B, c_v, m_Q, m_R, a.IHAQR_data->K, a.IHAQR_data->M, u_bias_v, 1e-4,
                      true );
      a.IHAQR_data->u_bias = from_vect< system_input_type >( u_bias_v );
    } catch( std::exception& e ) {
      std::cout << "Warning! Solution to the CARE problem could not be found for the given state point: " << a.x
                << std::endl << "  The following exception was raised: " << e.what() << std::endl;
    };
  };
   ...
   */
  system_input_type get_bounded_input( const system_input_type& u_prev, system_input_type u_bias,
  ...
  point_type move_position_toward_impl( const point_type& a, double fraction, const point_type& b,
    ...
    while( ( current_time < m_max_time_horizon )
      ...
      system_input_type u_current
        = get_bounded_input( u_prev, b.lin_data->u - b.IHAQR_data->u_bias,
      ...
      ctrl::detail::runge_kutta4_integrate_impl( m_space, *m_system, x_current, x_next, input_traj, current_time,
...
public:
  ...
  IHAQR_topology( const std::string& aName = "IHAQR_topology",
                  ...
                  const StateSpace& aSpace = StateSpace(), const system_input_type& aMinInput = system_input_type(),
                  ...
                  double aTimeStep = 0.1, double aMaxTimeHorizon = 10.0, double aGoalProximityThreshold = 1.0,
                  state_sampler_type aGetSample = state_sampler_type() )
      : named_object(), m_system( aSystem ), m_space( aSpace ), m_get_sample( aGetSample ),
        m_input_space( aName + "_input_space", aMinInput, aMaxInput ),
        m_input_rate_space( aName + "_input_rate_space", -aInputBandwidth, aInputBandwidth ), m_R( aR ), m_Q( aQ ),
        m_time_step( aTimeStep ), m_max_time_horizon( aMaxTimeHorizon ),
  ...
  * **********************************************************************/
   ...
   */
  bool is_in_bounds( const point_type& a ) const { return m_space.is_in_bounds( a.x ); };
  ...
  *******************************************************************************/
  ...
  virtual void RK_CALL save( serialization::oarchive& A, unsigned int ) const {
    ...
    A& RK_SERIAL_SAVE_WITH_NAME( m_system ) & RK_SERIAL_SAVE_WITH_NAME( m_space )
      & RK_SERIAL_SAVE_WITH_NAME( m_get_sample ) & RK_SERIAL_SAVE_WITH_NAME( m_input_space )
      & RK_SERIAL_SAVE_WITH_NAME( m_input_rate_space ) & RK_SERIAL_SAVE_WITH_NAME( m_R )
      & RK_SERIAL_SAVE_WITH_NAME( m_Q ) & RK_SERIAL_SAVE_WITH_NAME( m_time_step )
      & RK_SERIAL_SAVE_WITH_NAME( m_max_time_horizon ) & RK_SERIAL_SAVE_WITH_NAME( m_goal_proximity_threshold );
  };

  virtual void RK_CALL load( serialization::iarchive& A, unsigned int ) {
    ReaK::named_object::load( A, named_object::getStaticObjectType()->TypeVersion() );
    A& RK_SERIAL_LOAD_WITH_NAME( m_system ) & RK_SERIAL_LOAD_WITH_NAME( m_space )
      & RK_SERIAL_LOAD_WITH_NAME( m_get_sample ) & RK_SERIAL_LOAD_WITH_NAME( m_input_space )
      & RK_SERIAL_LOAD_WITH_NAME( m_input_rate_space ) & RK_SERIAL_LOAD_WITH_NAME( m_R )
      & RK_SERIAL_LOAD_WITH_NAME( m_Q ) & RK_SERIAL_LOAD_WITH_NAME( m_time_step )
      & RK_SERIAL_LOAD_WITH_NAME( m_max_time_horizon ) & RK_SERIAL_LOAD_WITH_NAME( m_goal_proximity_threshold );
  };
...
public:
  ...
  typename topology_traits< StateSpace >::point_type
    map_to_space( const IHAQR_point_type< StateSpace, StateSpaceSystem >& pt,
                  const IHAQR_topology< StateSpace, StateSpaceSystem, StateSpaceSampler >&, const StateSpace& ) const {
    return pt.x;
  };
...
public:
  ...
  IHAQR_topology_with_CD(
    const std::string& aName = "IHAQR_topology_with_CD",
    const shared_ptr< StateSpaceSystem >& aSystem = shared_ptr< StateSpaceSystem >(),
    const StateSpace& aSpace = StateSpace(), const system_input_type& aMinInput = system_input_type(),
    const system_input_type& aMaxInput = system_input_type(),
    const system_input_type& aInputBandwidth = system_input_type(),
    const mat< double, mat_structure::diagonal >& aR = ( mat< double, mat_structure::diagonal >() ),
    const mat< double, mat_structure::diagonal >& aQ = ( mat< double, mat_structure::diagonal >() ),
    double aTimeStep = 0.1, double aMaxTimeHorizon = 5.0, double aGoalProximityThreshold = 1.0,
    state_sampler_type aGetSample = state_sampler_type(),
    const shared_ptr< kte::direct_kinematics_model >& aModel = shared_ptr< kte::direct_kinematics_model >() )
      : base_type( aName, aSystem, aSpace, aMinInput, aMaxInput, aInputBandwidth, aR, aQ, aTimeStep, aMaxTimeHorizon,
                   aGoalProximityThreshold, aGetSample ),
        m_model( aModel ), m_proxy_env_2D(), m_proxy_env_3D(){};
   ...
   *               state-space points to synchronize the collision-detection code which rely on KTE-based systems).
   */
  IHAQR_topology_with_CD( const base_type& aBaseSpace, const shared_ptr< kte::direct_kinematics_model >& aModel )
  ...
  double distance( const point_type& a, const point_type& b ) const {
    ...
    if( get( distance_metric, this->get_state_space() )( a.x, b.x, this->get_state_space() ) * 0.1
...
template < typename StateSpace, typename StateSpaceSystem, typename StateSpaceSampler >
struct is_metric_space< IHAQR_topology_with_CD< StateSpace, StateSpaceSystem, StateSpaceSampler > >
  : boost::mpl::true_ {};
...
template < typename StateSpace, typename StateSpaceSystem, typename StateSpaceSampler >
struct is_metric_symmetric< IHAQR_topology_with_CD< StateSpace, StateSpaceSystem, StateSpaceSampler > >
  : boost::mpl::false_ {};
};
};
