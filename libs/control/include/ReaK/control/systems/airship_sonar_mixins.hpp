...
#ifndef REAK_AIRSHIP_SONAR_MIXINS_HPP
#define REAK_AIRSHIP_SONAR_MIXINS_HPP
...
public:
  state_space_type create_state_space() const {
    return pp::line_segment_topology< double >( "room_angle_param_space", -std::numeric_limits< double >::infinity(),
  ...
  *******************************************************************************/
  ...
  RK_RTTI_MAKE_CONCRETE_1BASE( room_orientation_state_model, 0xC2310031, 1, "room_orientation_state_model",
                               named_object )
};
...
private:
  ...
  void get_sonar_distance_to_room( const vect< double, 3 >& spos_gbl, const vect< double, 3 >& sdir_gbl, double& y,
...
public:
  ...
  sonars_in_room_output_model( std::size_t N = 6 )
      : start_index( 0 ), inv_start_index( 0 ), sonar_pos( N ), sonar_dir( N ), lower_corner( 0.0, 0.0, 0.0 ),
  ...
  typename boost::enable_if< typename FlyWeight::state_models_type::template has_system< room_orientation_state_model >,
                             void >::type
    accum_rotation_from_ro( const FlyWeight& params, const StateSpaceType& space,
                            const typename pp::topology_traits< StateSpaceType >::point_type& x,
  ...
  template < typename FlyWeight, typename StateSpaceType >
  typename boost::
    disable_if< typename FlyWeight::state_models_type::template has_system< room_orientation_state_model >, void >::type
    accum_rotation_from_ro( const FlyWeight&, const StateSpaceType&,
  ...
  typename boost::enable_if< typename FlyWeight::state_models_type::template has_system< room_orientation_state_model >,
                             void >::type
    accum_output_del_from_ro( MatrixC& C, const FlyWeight& params, const vect< double, 3 >& x_pos,
                              const mat< double, mat_structure::square >& R, const vect< double, 3 >& spos_gbl,
                              const vect< double, 3 >& sdir_gbl, std::size_t i, std::size_t coord ) const {
    const std::size_t ro_r
      = params.get_state_models().template get_system< room_orientation_state_model >().get_inv_corr_start_index();
  ...
  typename boost::
    disable_if< typename FlyWeight::state_models_type::template has_system< room_orientation_state_model >, void >::type
    accum_output_del_from_ro( MatrixC&, const FlyWeight&, const vect< double, 3 >&,
  ...
  void add_output_function_blocks( MatrixC& C, MatrixD& D, const FlyWeight& params, const StateSpaceType& space,
                                   time_type t, const typename pp::topology_traits< StateSpaceType >::point_type& x,
  ...
  *******************************************************************************/
  ...
  virtual void RK_CALL save( ReaK::serialization::oarchive& A, unsigned int ) const {
    named_object::save( A, named_object::getStaticObjectType()->TypeVersion() );
    A& RK_SERIAL_SAVE_WITH_NAME( sonar_pos ) & RK_SERIAL_SAVE_WITH_NAME( sonar_dir )
      & RK_SERIAL_SAVE_WITH_NAME( lower_corner ) & RK_SERIAL_SAVE_WITH_NAME( upper_corner );
  };

  virtual void RK_CALL load( ReaK::serialization::iarchive& A, unsigned int ) {
    named_object::load( A, named_object::getStaticObjectType()->TypeVersion() );
    A& RK_SERIAL_LOAD_WITH_NAME( sonar_pos ) & RK_SERIAL_LOAD_WITH_NAME( sonar_dir )
      & RK_SERIAL_LOAD_WITH_NAME( lower_corner ) & RK_SERIAL_LOAD_WITH_NAME( upper_corner );
  };
