...
public:
  state_space_type create_state_space() const {
    return pp::line_segment_topology< double >( "linear_drag_param_space", 0.0,
  ...
  void add_state_transition_blocks( MatrixA& A, MatrixB& B, const FlyWeight& params, const StateSpaceType& space,
    ...
    const point_type d_f = params.get_state_models().template get_state_for_system< linear_drag_state_model >( p_0 );
    ...
    slice( A )( p_r, fd_r ) -= ( 0.5 * dt * dt * v_avg_mag / ( sys_params.effective_mass + sys_params.added_mass ) )
                               * v_avg;
...
public:
  state_space_type create_state_space() const {
    return pp::line_segment_topology< double >( "torsional_drag_param_space", 0.0,
                                                std::numeric_limits< double >::infinity() );
  };
  ...
  void add_state_transition_blocks( MatrixA& A, MatrixB& B, const FlyWeight& params, const StateSpaceType& space,
    ...
    const point_type d_t = params.get_state_models().template get_state_for_system< torsional_drag_state_model >( p_0 );
    const std::size_t sat3d_state_index
      = params.get_state_models().template get_system< satellite_state_model >().get_inv_corr_start_index();
