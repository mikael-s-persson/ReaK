...
public:
  ...
  struct zero_input_trajectory {
    input_type get_point( time_type ) const { return input_type( 0.0, 0.0, 0.0 ); };
  };
...
public:
   ...
   */
  satellite2D_imdt_sys( const std::string& aName = "", double aMass = 1.0, double aInertiaMoment = 1.0,
                        double aDt = 0.001 );
  ...
  void get_output_function_blocks( matrixC_type& C, matrixD_type& D, const state_space_type& space, const time_type& t,
                                   const point_type& p, const input_type& u ) const;
...
public:
   ...
   * sufficient).
   */
  satellite3D_imdt_sys( const std::string& aName = "", double aMass = 1.0,
  ...
  void get_state_transition_blocks( matrixA_type& A, matrixB_type& B, const state_space_type&, const time_type&,
                                    const time_type&, const point_type& p_0, const point_type& p_1, const input_type&,
                                    const input_type& ) const;

  point_type apply_correction( const state_space_type&, const point_type& x, const invariant_correction_type& c,
                               const input_type&, const time_type& ) const;

  invariant_frame_type get_invariant_prior_frame( const state_space_type&, const point_type& x_prev,
                                                  const point_type& x_prior, const input_type&,
                                                  const time_type& ) const;

  invariant_frame_type get_invariant_posterior_frame( const state_space_type& state_space, const point_type& x_prior,
...
public:
  ...
  output_type get_output( const state_space_type&, const point_type& x, const input_type& u,
                          const time_type& t = 0.0 ) const;
  ...
  invariant_error_type get_invariant_error( const state_space_type&, const point_type& x, const input_type& u,
                                            const output_type& y, const time_type& t ) const;
  ...
  *******************************************************************************/
  ...
  RK_RTTI_MAKE_CONCRETE_1BASE( satellite3D_gyro_imdt_sys, 0xC2310016, 1, "satellite3D_gyro_imdt_sys",
...
public:
   ...
   * sufficient).
   */
  satellite3D_IMU_imdt_sys(
    const std::string& aName = "", double aMass = 1.0,
    const mat< double, mat_structure::symmetric >& aInertiaMoment
    = ( mat< double, mat_structure::symmetric >( mat< double, mat_structure::identity >( 3 ) ) ),
    double aDt = 0.001, const unit_quat< double >& aIMUOrientation = unit_quat< double >(),
    const vect< double, 3 >& aIMULocation = ( vect< double, 3 >() ),
    const unit_quat< double >& aRoomOrientation = unit_quat< double >(),
    const vect< double, 3 >& aMagFieldVector = ( vect< double, 3 >( 1.0, 0.0, 0.0 ) ), std::size_t aApproxOrder = 1 );
  ...
  *******************************************************************************/
  ...
  virtual void RK_CALL save( ReaK::serialization::oarchive& A, unsigned int ) const {
    ...
    A& RK_SERIAL_SAVE_WITH_NAME( IMU_orientation ) & RK_SERIAL_SAVE_WITH_NAME( IMU_location )
      & RK_SERIAL_SAVE_WITH_NAME( room_orientation ) & RK_SERIAL_SAVE_WITH_NAME( mag_field_vector );
  };
  virtual void RK_CALL load( ReaK::serialization::iarchive& A, unsigned int ) {
    ...
    A& RK_SERIAL_LOAD_WITH_NAME( IMU_orientation ) & RK_SERIAL_LOAD_WITH_NAME( IMU_location )
      & RK_SERIAL_LOAD_WITH_NAME( room_orientation ) & RK_SERIAL_LOAD_WITH_NAME( mag_field_vector );
  };

  RK_RTTI_MAKE_CONCRETE_1BASE( satellite3D_IMU_imdt_sys, 0xC2310017, 1, "satellite3D_IMU_imdt_sys",
                               satellite3D_imdt_sys )
};
