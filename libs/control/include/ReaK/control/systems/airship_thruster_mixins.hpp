...
public:
  ...
  void add_state_transition_blocks( MatrixA& A, MatrixB& B, const FlyWeight& params, const StateSpaceType& space,
    ...
    for( std::size_t i = 0; i < 8; ++i ) {
      // (p,v)-f block:
      vect< double, 3 > f_jac = ( dt / ( sys_params.effective_mass + sys_params.added_mass ) )
                                * ( R_0 * thruster_dir[i] );
      ...
      // (q,w)-t block:
      vect< double, 3 > tau_jac = dt
                                  * ( R_0_1 * ( sys_params.effective_J_inv * ( thruster_pos[i] % thruster_dir[i] ) ) );
  ...
  *******************************************************************************/
  ...
  virtual void RK_CALL save( ReaK::serialization::oarchive& A, unsigned int ) const {
    named_object::save( A, named_object::getStaticObjectType()->TypeVersion() );
    A& RK_SERIAL_SAVE_WITH_NAME( thruster_pos ) & RK_SERIAL_SAVE_WITH_NAME( thruster_dir );
  };

  virtual void RK_CALL load( ReaK::serialization::iarchive& A, unsigned int ) {
    named_object::load( A, named_object::getStaticObjectType()->TypeVersion() );
    A& RK_SERIAL_LOAD_WITH_NAME( thruster_pos ) & RK_SERIAL_LOAD_WITH_NAME( thruster_dir );
  };
