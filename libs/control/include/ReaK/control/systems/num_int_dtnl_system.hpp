...
template < typename CTSystem,
           typename NumIntegrator
           = euler_integrator< typename vect_traits< typename ss_system_traits< CTSystem >::point_type >::value_type > >
...
private:
  ...
  public:
    ...
    virtual void RK_CALL computeStateRate( double aTime, const ReaK::vect_n< value_type >& aState,
...
public:
   ...
   * Default constructor.
   */
  num_int_dtnl_sys( const std::string& aName = "" ) : sys( new CTSystem() ), integ(), dt() { setName( aName ); };
   ...
   */
  num_int_dtnl_sys( const ct_system_ptr& aSys, const NumIntegrator& aInteg, const time_difference_type& aDt,
   ...
   */
  template < typename StateSpaceType >
  point_type get_next_state( const StateSpaceType& state_space, const point_type& p, const input_type& u,
                             const time_type& t = 0 ) {
    integ.setTime( t );
    integ.clearStateVector();
    vect_n< value_type > result = to_vect< value_type >( p );
    integ.addStateElements( result );
    shared_ptr< state_rate_function< value_type > > temp(
      new rate_function_impl< StateSpaceType >( state_space, sys, u ) );
    integ.setStateRateFunc( temp );
    integ.integrate( t + dt );
    size_type i = 0;
    for( typename std::vector< value_type >::const_iterator it = integ.getStateBegin(); it != integ.getStateEnd();
         ++it, ++i )
      result[i] = *it;
   ...
   */
  template < typename StateSpaceType >
  output_type get_output( const StateSpaceType& state_space, const point_type& p, const input_type& u,
                          const time_type& t = 0 ) {
    RK_UNUSED( t );
  ...
  virtual void RK_CALL save( ReaK::serialization::oarchive& aA, unsigned int ) const {
    ...
    aA& RK_SERIAL_SAVE_WITH_NAME( sys ) & RK_SERIAL_SAVE_WITH_NAME( integ ) & RK_SERIAL_SAVE_WITH_NAME( dt );
  };
  virtual void RK_CALL load( ReaK::serialization::iarchive& aA, unsigned int ) {
    ...
    aA& RK_SERIAL_LOAD_WITH_NAME( sys ) & RK_SERIAL_LOAD_WITH_NAME( integ ) & RK_SERIAL_LOAD_WITH_NAME( dt );
  };
...
class num_int_dtnl_sys< state_rate_function_with_io< T >, NumIntegrator< T > > : public named_object {
public:
  typedef num_int_dtnl_sys< state_rate_function_with_io< T >, NumIntegrator< T > > self;
  ...
  typedef vect_n< value_type > input_type;
  typedef vect_n< value_type > output_type;
...
public:
   ...
   * Default constructor.
   */
  num_int_dtnl_sys( const std::string& aName = "" ) : sys(), integ(), dt() { setName( aName ); };
   ...
   * \param aDt The time-step of this discrete-time system (not the integration time-step).
   */
  num_int_dtnl_sys( const ct_system_ptr& aSys, const NumIntegrator< value_type >& aInteg,
                    const time_difference_type& aDt, const std::string& aName = "" )
      : sys( aSys ), integ( aInteg ), dt( aDt ) {
    setName( aName );
  };
   ...
   */
  template < typename StateSpaceType >
  output_type get_output( const StateSpaceType&, const point_type& p, const input_type& u, const time_type& t = 0 ) {
    RK_UNUSED( t );
    output_type y;
