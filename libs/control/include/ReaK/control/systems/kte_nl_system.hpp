...
public:
  ...
  kte_nl_system( const self& rhs )
      : named_object(), dofs_gen( rhs.dofs_gen ), dofs_2D( rhs.dofs_2D ), dofs_3D( rhs.dofs_3D ), inputs( rhs.inputs ),
        outputs( rhs.outputs ), chain( rhs.chain ), mass_calc( rhs.mass_calc ) {
    setName( rhs.getName() );
  };
   ...
   */
  size_type get_state_dimensions() const { return 2 * dofs_gen.size() + 7 * dofs_2D.size() + 13 * dofs_3D.size(); };
  ...
  point_derivative_type get_state_derivative( const StateSpaceType&, const point_type& p, const input_type& u,
  ...
  virtual void RK_CALL save( ReaK::serialization::oarchive& aA, unsigned int ) const {
    ...
    aA& RK_SERIAL_SAVE_WITH_NAME( dofs_gen ) & RK_SERIAL_SAVE_WITH_NAME( dofs_2D ) & RK_SERIAL_SAVE_WITH_NAME( dofs_3D )
      & RK_SERIAL_SAVE_WITH_NAME( inputs ) & RK_SERIAL_SAVE_WITH_NAME( outputs ) & RK_SERIAL_SAVE_WITH_NAME( chain )
  ...
  virtual void RK_CALL load( ReaK::serialization::iarchive& aA, unsigned int ) {
    ...
    aA& RK_SERIAL_LOAD_WITH_NAME( dofs_gen ) & RK_SERIAL_LOAD_WITH_NAME( dofs_2D ) & RK_SERIAL_LOAD_WITH_NAME( dofs_3D )
      & RK_SERIAL_LOAD_WITH_NAME( inputs ) & RK_SERIAL_LOAD_WITH_NAME( outputs ) & RK_SERIAL_LOAD_WITH_NAME( chain )
