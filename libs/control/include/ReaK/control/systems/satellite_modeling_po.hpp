...
boost::program_options::options_description get_satellite_model_options_po_desc( bool aPredictOpt = false ) {
  ...
  model_options.add_options()( "sat-config-file", value< std::string >(),
                               "specify the filename for all the satellite modeling options" )(
    "init-motion", value< std::string >()->default_value( "models/satellite3D_init.rkx" ),
    "specify the filename for the satellite's initial motion (state) (default is 'models/satellite3D_init.rkx')" )(
    "inertia", value< std::string >()->default_value( "models/satellite3D_inertia.rkx" ),
    "specify the filename for the satellite's inertial data (default is 'models/satellite3D_inertia.rkx')" )(
    "Q-matrix", value< std::string >()->default_value( "models/satellite3D_Q.rkx" ),
    "specify the filename for the satellite's input disturbance covariance matrix (default is "
    "'models/satellite3D_Q.rkx')" )( "R-matrix", value< std::string >()->default_value( "models/satellite3D_R.rkx" ),
    ...
    "IMU-config", value< std::string >()->default_value( "models/satellite3D_IMU_config.rkx" ),
    ...
    "generate-mdl-files",
    "if set, the output will be the generation of all the modeling files (with default values) into the specified "
    ...
    "gyro", "if set, a set of gyros is added to the model (angular velocity measurements). This requires the "
 ...
 This option will trigger the use of the 'IMU-config' file to obtain the information necessary about the IMU and the Earth's inertial frame." )
    ...
    ( "iekf", "if set, results for the invariant extended Kalman filter (IEKF) will be generated." )(
      ...
      "imkf-em", "if set, results for the invariant momentum-tracking Kalman filter (IMKF) with adaptive "
                 "mass-eccentricity parameters will be generated." )(
      "imkf-emd", "if set, results for the invariant momentum-tracking Kalman filter (IMKF) with adaptive "
                  "mass-eccentricity-drag parameters will be generated." )(
      "imkf-emdJ", "if set, results for the invariant momentum-tracking Kalman filter (IMKF) with adaptive "
                   "mass-eccentricity-drag-inertia parameters will be generated." )(
      "tsosakf", "if set, the two-stage online-steady augmented Kalman filter (TSOSAKF) will be used for the augmented "
                 "systems (parameter identification)." )(
      "Pa-matrix", value< std::string >()->default_value( "models/satellite3D_Pa.rkx" ),
      "specify the filename for the satellite's steady augmented parameter covariance matrix (default is "
      "'models/satellite3D_Pa.rkx')" );
  result.add( model_options );

  if( aPredictOpt ) {
    options_description pred_options( "Prediction options" );
    pred_options.add_options()( "pred-config-file", value< std::string >(),
      ...
      "Pnorm-threshold", value< double >()->default_value( 1.0 ), "threshold on the norm of the state prediction "
                                                                  "covariance matrix below which predictions can start "
                                                                  "(default is 1.0)" )(
      "pred-assumption", value< int >()->default_value( 0 ),
      "prediction assumption to be used (0: no-measurements, 1: most-likely-measurements, 2: full-certainty" );
...
void fill_satellite_model_options_from_po( satellite_model_options& result,
...
void save_satellite_model_options_to_files( satellite_model_options& result,
  ...
  if( result.steady_param_covariance.get_row_count()
      != result.get_total_inv_state_count() - result.get_actual_inv_state_count() )
    result.steady_param_covariance = mat< double, mat_structure::diagonal >(
      result.get_total_inv_state_count() - result.get_actual_inv_state_count(), true );

  if( vm.count( "sat-config-file" ) ) {
    boost::filesystem::create_directories(
      boost::filesystem::path( vm["sat-config-file"].as< std::string >() ).parent_path() );
    ...
    boost::filesystem::create_directories(
      boost::filesystem::path( vm["IMU-config"].as< std::string >() ).parent_path() );
    result.save_IMU_configs( vm["IMU-config"].as< std::string >() );

    boost::filesystem::create_directories(
      boost::filesystem::path( vm["Q-matrix"].as< std::string >() ).parent_path() );
    result.save_input_disturbance( vm["Q-matrix"].as< std::string >() );

    boost::filesystem::create_directories(
      boost::filesystem::path( vm["R-matrix"].as< std::string >() ).parent_path() );
    result.save_measurement_noise( vm["R-matrix"].as< std::string >() );

    if( vm.count( "R-added" ) ) {
      boost::filesystem::create_directories(
        boost::filesystem::path( vm["R-added"].as< std::string >() ).parent_path() );
      result.save_artificial_noise( vm["R-added"].as< std::string >() );
    };

    if( vm.count( "tsosakf" ) ) {
      boost::filesystem::create_directories(
        boost::filesystem::path( vm["Pa-matrix"].as< std::string >() ).parent_path() );
      result.save_steady_param_covariance( vm["Pa-matrix"].as< std::string >() );
    };

    boost::filesystem::create_directories(
      boost::filesystem::path( vm["init-motion"].as< std::string >() ).parent_path() );
...
satellite_predictor_options get_satellite_predictor_options_from_po( boost::program_options::variables_map& vm ) {
  ...
  if( vm.count( "generate-mdl-files" ) ) {
    ...
    if( vm.count( "pred-config-file" ) ) {
      boost::filesystem::create_directories(
        boost::filesystem::path( vm["pred-config-file"].as< std::string >() ).parent_path() );
