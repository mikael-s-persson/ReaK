...
typename boost::enable_if< boost::mpl::and_< is_invariant_system< InvariantSystem >,
                           ...
                           void >::type
  invariant_kalman_predict( const InvariantSystem& sys, const StateSpaceType& state_space, BeliefState& b_x,
                            const InputBelief& b_u, typename discrete_sss_traits< InvariantSystem >::time_type t = 0 ) {
  ...
  P = W * ( ( A * P * transpose_view( A ) ) + B * b_u.get_covariance().get_matrix() * transpose_view( B ) )
      * transpose_view( W );
...
template < typename InvariantSystem, typename StateSpaceType, typename BeliefState, typename InputBelief >
typename boost::disable_if< is_invariant_system< InvariantSystem >, void >::type
  invariant_kalman_predict( const InvariantSystem& sys, const StateSpaceType& state_space, BeliefState& b_x,
                            const InputBelief& b_u, typename discrete_sss_traits< InvariantSystem >::time_type t = 0 ) {
...
typename boost::enable_if< boost::mpl::and_< is_invariant_system< InvariantSystem >,
                                             ...
                                             boost::mpl::bool_< ( belief_state_traits< BeliefState >::distribution
                                                                  == belief_distribution::unimodal ) > >,
                           void >::type
  invariant_kalman_update( const InvariantSystem& sys, const StateSpaceType& state_space, BeliefState& b_x,
...
typename boost::enable_if< boost::mpl::and_< is_invariant_system< InvariantSystem >,
                                             ...
                                             boost::mpl::bool_< ( belief_state_traits< BeliefState >::distribution
                                                                  == belief_distribution::unimodal ) > >,
                           void >::type
  invariant_kalman_filter_step( const InvariantSystem& sys, const StateSpaceType& state_space, BeliefState& b_x,
                                const InputBelief& b_u, const MeasurementBelief& b_z,
                                typename discrete_sss_traits< InvariantSystem >::time_type t = 0 ) {
  // here the requirement is that the system models a linear system which is at worse a linearized system
  // - if the system is LTI or LTV, then this will result in a basic Kalman Filter (KF) update
...
struct IKF_belief_transfer {
  ...
  typename pp::topology_traits< BeliefSpace >::point_type
    ...
    invariant_kalman_filter_step( *( factory->get_state_space_system() ), b_space.get_state_topology(), b,
  ...
  typename pp::topology_traits< BeliefSpace >::point_type
    ...
    invariant_kalman_predict( *( factory->get_state_space_system() ), b_space.get_state_topology(), b,
  ...
  typename pp::topology_traits< BeliefSpace >::point_type
    prediction_to_ML_belief( const BeliefSpace& b_space, typename pp::topology_traits< BeliefSpace >::point_type b,
                             const time_type& t, const input_type& u ) const {
    invariant_kalman_update( *( factory->get_state_space_system() ), b_space.get_state_topology(), b,
                             input_belief_type( u, covariance_type( factory->get_input_disturbance_cov() ) ),
  ...
  typename pp::topology_traits< BeliefSpace >::point_type
    ...
    invariant_kalman_predict( *( factory->get_state_space_system() ), b_space.get_state_topology(), b, b_u, t );
    invariant_kalman_update( *( factory->get_state_space_system() ), b_space.get_state_topology(), b, b_u,
                             output_belief_type( factory->get_state_space_system()->get_output(
                                                   b_space.get_state_topology(), b.get_mean_state(), u, t ),
                                                 covariance_type( factory->get_measurement_noise_cov() ) ),
