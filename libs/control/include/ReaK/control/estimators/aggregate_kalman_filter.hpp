...
typename boost::enable_if_c< is_continuous_belief_state< BeliefState >::value&&(
                             ...
                             void >::type
  aggregate_kalman_predict(
    const LinearSystem& sys, const StateSpaceType& state_space, BeliefState& b_x, const InputBelief& b_u,
    typename hamiltonian_mat< typename mat_traits< typename covariance_mat_traits< typename continuous_belief_state_traits< BeliefState >::
...
typename boost::enable_if_c< is_continuous_belief_state< BeliefState >::value&&(
  ...
  HamilMat Sm_tmp( HamilMatUp( HamilMatUL( mat< ValueType, mat_structure::identity >( N ) ),
                               HamilMatUR( mat< ValueType, mat_structure::nil >( N ) ) ),
                   HamilMatLo( HamilMatLL( transpose_view( C ) * b_z.get_covariance().get_inverse_matrix() * C ),
...
typename boost::enable_if_c< is_continuous_belief_state< BeliefState >::value&&(
  ...
  OutputType y = b_z.get_mean_state()
                 - sys.get_output( state_space, x_prior, b_u.get_mean_state(), t + sys.get_time_step() );
  mat< ValueType, mat_structure::rectangular, mat_alignment::column_major > CP = C * P;
  mat< ValueType, mat_structure::symmetric > S = CP * transpose_view( C ) + b_z.get_covariance().get_matrix();
  linsolve_Cholesky( S, CP );
  mat< ValueType, mat_structure::rectangular, mat_alignment::row_major > K( transpose_view( CP ) );
...
struct AKF_belief_transfer {
   ...
   */
  AKF_belief_transfer( const AKFTransferFactory* aFactory, const belief_space& b_space, const belief_state* b_x,
    ...
    aggregate_kalman_update( *( factory->get_state_space_system() ), b_space.get_state_topology(), b, b_u, b_y, Sm,
                             t + factory->get_state_space_system()->get_time_step() );
  };
...
public:
  ...
  *******************************************************************************/
  ...
  RK_RTTI_MAKE_ABSTRACT_1BASE( self, 0xC2320005, 1, "AKF_belief_transfer_factory", serializable )
};
};
};
