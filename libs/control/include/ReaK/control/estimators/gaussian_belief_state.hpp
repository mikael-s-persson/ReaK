...
template < typename BeliefState,
           ...
           = covariance_mat_traits< typename continuous_belief_state_traits< BeliefState >::covariance_type >::storage >
struct gaussian_pdf {
   ...
   */
  gaussian_pdf( const BeliefState& aBelief )
      : mean_state( aBelief.get_mean_state() ), L( aBelief.get_covariance().size() ), factor( -1 ) {
...
struct gaussian_pdf< BeliefState, covariance_storage::information > {
   ...
   */
  gaussian_pdf( const BeliefState& aBelief )
      : mean_state( aBelief.get_mean_state() ), E_inv( aBelief.get_covariance().get_inverse_matrix() ), factor( -1 ) {
...
struct gaussian_pdf< BeliefState, covariance_storage::decomposed > {
   ...
   * \param aBelief The belief-state of the gaussian distribution.
   */
  gaussian_pdf( const BeliefState& aBelief ) : mean_state( aBelief.get_mean_state() ), factor( -1 ) {
  ...
  gaussian_pdf( const self& rhs )
      : mean_state( rhs.mean_state ), QX( rhs.QX ), RX( rhs.RX ), QY( rhs.QY ), RY( rhs.RY ), factor( rhs.factor ){};
...
typename gaussian_pdf< BeliefState, Storage >::scalar_type
  symKL_divergence( const gaussian_pdf< BeliefState, Storage >& N0, const gaussian_pdf< BeliefState, Storage >& N1,
                    const Topology& space ) {
  using std::log;
  typedef typename gaussian_pdf< BeliefState, Storage >::scalar_type ScalarType;
  return ScalarType( -0.5 ) * log( N1( N0.mean_state, space ) * N0( N1.mean_state, space ) ) - entropy( N0 )
         - entropy( N1 );
};
...
struct gaussian_sampler {
   ...
   * \param aBelief The belief-state of the gaussian distribution.
   */
  gaussian_sampler( const BeliefState& aBelief )
      : mean_state( aBelief.get_mean_state() ), L( aBelief.get_covariance().size() ) {
    using std::sqrt;
    const matrix_type& C = aBelief.get_covariance().get_matrix();
    try {
      decompose_Cholesky( C, L );
    } catch( singularity_error& ) {
      mat< typename mat_traits< matrix_type >::value_type, mat_structure::diagonal > E(
        aBelief.get_covariance().size() );
      mat< typename mat_traits< matrix_type >::value_type, mat_structure::square > U( aBelief.get_covariance().size() ),
        V( aBelief.get_covariance().size() );
  ...
  state_type operator()( const Topology& space ) const {
    ...
    boost::variate_generator< global_rng_type&, boost::normal_distribution< scalar_type > > var_rnd(
      get_global_rng(), boost::normal_distribution< scalar_type >() );
...
template < typename Vector, typename Matrix >
typename boost::enable_if< boost::mpl::and_< is_writable_vector< Vector >, is_readable_matrix< Matrix > >,
                           Vector >::type
  sample_gaussian_point( const Vector& mean, const Matrix& cov ) {
  typedef typename mat_traits< Matrix >::value_type ValueType;
  typedef typename mat_traits< Matrix >::size_type SizeType;
  using std::sqrt;
  ...
  boost::variate_generator< global_rng_type&, boost::normal_distribution< ValueType > > var_rnd(
    get_global_rng(), boost::normal_distribution< ValueType >() );
...
typename boost::enable_if< is_writable_vector< Vector >, Vector >::type
  sample_gaussian_point( Vector mean, const mat< ValueType, mat_structure::diagonal >& cov ) {
...
typename boost::enable_if< is_readable_matrix< Matrix >, typename pp::topology_traits< StateSpace >::point_type >::type
  sample_gaussian_point( const StateSpace& space, const typename pp::topology_traits< StateSpace >::point_type& mean,
                         const Matrix& cov ) {
...
public:
   ...
   */
  random_sampler_type get_random_sampler() const { return random_sampler_type( *this ); };
   ...
   */
  gaussian_belief_state( const state_type& aMeanState = state_type(), const covariance_type& aCov = covariance_type() )
...
typename gaussian_belief_state< StateType, Covariance >::scalar_type
  symKL_divergence( const gaussian_belief_state< StateType, Covariance >& P,
                    const gaussian_belief_state< StateType, Covariance >& Q, const Topology& space ) {
