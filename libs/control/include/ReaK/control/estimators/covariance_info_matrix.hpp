...
public:
   ...
   */
  explicit covariance_info_matrix( const matrix_type& aMat = matrix_type(), const std::string& aName = "" )
      : mat_info( aMat ) {
    setName( aName );
  ...
  explicit covariance_info_matrix( size_type aSize,
      ...
      : mat_info( aSize, value_type( ( aLevel == covariance_initial_level::no_info
                                         ? 0
   ...
   */
  friend const matrix_type& invert( const self& aObj ) { return aObj.get_inverse_matrix(); };
