...
typename boost::enable_if< boost::mpl::and_< is_augmented_ss_system< LinearSystem >,
                           ...
                           void >::type
  tsos_aug_kalman_predict( const LinearSystem& sys, const StateSpaceType& state_space, BeliefState& b_x,
                           const InputBelief& b_u, typename discrete_sss_traits< LinearSystem >::time_type t = 0 ) {
  ...
  mat< ValueType, mat_structure::square > P_x_p( ( A_x * P_x + A_xa * P_ax ) * transpose_view( A_x )
                                                 + P_xa_p * transpose_view( A_xa )
                                                 + B_x * b_u.get_covariance().get_matrix() * transpose_view( B_x ) );

  b_x.set_covariance( CovType( ( P_x_p & P_xa_p ) | ( transpose_view( P_xa_p ) & P_a ) ) );
};


template < typename LinearSystem, typename StateSpaceType, typename BeliefState, typename InputBelief >
typename boost::disable_if< is_augmented_ss_system< LinearSystem >, void >::type
  tsos_aug_kalman_predict( const LinearSystem& sys, const StateSpaceType& state_space, BeliefState& b_x,
                           const InputBelief& b_u, typename discrete_sss_traits< LinearSystem >::time_type t = 0 ) {
...
typename boost::enable_if< boost::mpl::and_< is_augmented_ss_system< LinearSystem >,
                                             ...
                                             boost::mpl::bool_< ( belief_state_traits< BeliefState >::distribution
                                                                  == belief_distribution::unimodal ) > >,
                           void >::type
  tsos_aug_kalman_update( const LinearSystem& sys, const StateSpaceType& state_space, BeliefState& b_x,
...
typename boost::enable_if< boost::mpl::and_< is_augmented_ss_system< LinearSystem >,
                                             ...
                                             boost::mpl::bool_< ( belief_state_traits< BeliefState >::distribution
                                                                  == belief_distribution::unimodal ) > >,
                           void >::type
  tsos_aug_kalman_filter_step( const LinearSystem& sys, const StateSpaceType& state_space, BeliefState& b_x,
                               const InputBelief& b_u, const MeasurementBelief& b_z,
                               typename discrete_sss_traits< LinearSystem >::time_type t = 0 ) {
  // here the requirement is that the system models a linear system which is at worse a linearized system
  // - if the system is LTI or LTV, then this will result in a basic Kalman Filter (KF) update
...
struct TSOSAKF_belief_transfer {
   ...
   */
  explicit TSOSAKF_belief_transfer( const TSOSAKFTransferFactory* aFactory = nullptr ) : factory( aFactory ){};
  ...
  typename pp::topology_traits< BeliefSpace >::point_type
    ...
    tsos_aug_kalman_filter_step( *( factory->get_state_space_system() ), b_space.get_state_topology(), b,
  ...
  typename pp::topology_traits< BeliefSpace >::point_type
    ...
    tsos_aug_kalman_predict( *( factory->get_state_space_system() ), b_space.get_state_topology(), b,
  ...
  typename pp::topology_traits< BeliefSpace >::point_type
    prediction_to_ML_belief( const BeliefSpace& b_space, typename pp::topology_traits< BeliefSpace >::point_type b,
                             const time_type& t, const input_type& u ) const {
    tsos_aug_kalman_update( *( factory->get_state_space_system() ), b_space.get_state_topology(), b,
                            input_belief_type( u, covariance_type( factory->get_input_disturbance_cov() ) ),
  ...
  typename pp::topology_traits< BeliefSpace >::point_type
    ...
    tsos_aug_kalman_predict( *( factory->get_state_space_system() ), b_space.get_state_topology(), b, b_u, t );
    tsos_aug_kalman_update( *( factory->get_state_space_system() ), b_space.get_state_topology(), b, b_u,
                            output_belief_type( factory->get_state_space_system()->get_output(
                                                  b_space.get_state_topology(), b.get_mean_state(), u, t ),
                                                covariance_type( factory->get_measurement_noise_cov() ) ),
