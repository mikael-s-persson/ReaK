...
public:
  ...
  struct distance_metric_type {
    ...
    double operator()( const point_difference_type& dp, const self& ) const { return norm( dp ); };
...
public:
  ...
  point_type move_position_toward( const point_type& a, double fraction, const point_type& b ) const {
    return point_type(
      matrix_type( value_type( 1.0 - fraction ) * a.get_matrix() + value_type( fraction ) * b.get_matrix() ) );
  };
  ...
  point_type move_position_back_to( const point_type& a, double fraction, const point_type& b ) const {
    return point_type(
      matrix_type( value_type( fraction ) * a.get_matrix() + value_type( 1.0 - fraction ) * b.get_matrix() ) );
  };
  ...
  *******************************************************************************/
  ...
  virtual void RK_CALL save( serialization::oarchive& A, unsigned int ) const {
    ...
    A& RK_SERIAL_SAVE_WITH_NAME( max_eigenvalue ) & RK_SERIAL_SAVE_WITH_NAME( mat_size );
  };

  virtual void RK_CALL load( serialization::iarchive& A, unsigned int ) {
    ReaK::named_object::load( A, named_object::getStaticObjectType()->TypeVersion() );
    A& RK_SERIAL_LOAD_WITH_NAME( max_eigenvalue ) & RK_SERIAL_LOAD_WITH_NAME( mat_size );
  };
...
template < typename Covariance >
typename covar_topology< Covariance >::distance_metric_type get( ReaK::pp::distance_metric_t,
                                                                 const covar_topology< Covariance >& ) {
