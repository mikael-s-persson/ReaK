...
template < typename T, typename InvariantSystem, typename StateSpaceType, typename SystemNoiseCovariance,
...
struct invariant_kb_system : public state_rate_function< T > {
  ...
  invariant_kb_system( const InvariantSystem& aSys, const StateSpaceType& aStateSpace, const input_type& aU,
                       const output_type& aZ, const SystemNoiseCovariance& aQ, const MeasurementNoiseCovariance& aR )
  ...
  virtual void RK_CALL
    computeStateRate( double aTime, const ReaK::vect_n< value_type >& aState, ReaK::vect_n< value_type >& aStateRate ) {
    ...
    vect_n< value_type > xd = to_vect< value_type >(
      sys.apply_correction( state_space, x_state, sys.get_state_derivative( state_space, x_state, u, aTime ),
...
typename boost::enable_if_c< is_continuous_belief_state< BeliefState >::value&&(
                             ...
                             void >::type
  invariant_kalman_bucy_filter_step(
    const InvariantSystem& sys,
    integrator< typename mat_traits< typename covariance_mat_traits< typename continuous_belief_state_traits< BeliefState >::
                                                                       covariance_type >::matrix_type >::value_type >&
      integ,
    const StateSpaceType& state_space, BeliefState& b_x, const InputBelief& b_u, const MeasurementBelief& b_z,
  ...
  shared_ptr< state_rate_function< ValueType > > integ_sys = shared_ptr< state_rate_function< ValueType > >(
    new detail::invariant_kb_system< ValueType, InvariantSystem, StateSpaceType, InputCovType, OutputCovType >(
      sys, state_space, b_u.get_mean_state(), b_z.get_mean_state(), b_u.get_covariance(), b_z.get_covariance() ) );
