...
template < typename T, typename LinearSystem, typename StateSpaceType, typename SystemNoiseCovariance >
struct kalman_bucy_predictor : public state_rate_function< T > {
  ...
  BOOST_CONCEPT_ASSERT( (
    CovarianceMatrixConcept< SystemNoiseCovariance, pp::topology_traits< StateSpaceType >::point_difference_type > ) );
  ...
  kalman_bucy_system( const LinearSystem& aSys, const StateSpaceType& aStateSpace, const input_type& aU,
...
typename boost::enable_if_c< is_continuous_belief_state< BeliefState >::value&&(
  ...
  integ.setStateRateFunc( shared_ptr< state_rate_function< ValueType > >(
    new detail::kalman_bucy_system< ValueType, LinearSystem, StateSpaceType, InputCovType, OutputCovType >(
      sys, state_space, to_vect< ValueType >( b_u.get_mean_state() ), to_vect< ValueType >( b_z.get_mean_state() ),
