...
public:
  ...
  gaussian_belief_space( const state_topology_ptr& aMeanStateSpace = state_topology_ptr( new state_topology() ),
                         const covariance_topology_ptr& aCovarianceSpace
                         = covariance_topology_ptr( new covariance_topology() ),
                         const std::string& aName = "" )
      : mean_state_space( aMeanStateSpace ), covariance_space( aCovarianceSpace ) {
    setName( aName );
  };
   ...
   */
  point_type origin() const { return point_type( mean_state_space->origin(), covariance_space->origin() ); };
  ...
  point_type adjust( const point_type& p1, const point_difference_type& dp ) const {
    return point_type(
      mean_state_space->adjust( p1.get_mean_state(),
                                mean_state_space->difference( dp.first.get_mean_state(), dp.second.get_mean_state() ) ),
      covariance_space->adjust(
        p1.get_covariance(), covariance_space->difference( dp.first.get_covariance(), dp.second.get_covariance() ) ) );
  };
  ...
  * **********************************************************************/
  ...
  point_type move_position_back_to( const point_type& p1, double d, const point_type& p2 ) const {
    return point_type( mean_state_space->move_position_back_to( p1.get_mean_state(), d, p2.get_mean_state() ),
  ...
  * **********************************************************************/
   ...
   */
  const covariance_topology& get_covariance_topology() const { return *covariance_space; };
  ...
  *******************************************************************************/
  ...
  virtual void RK_CALL save( ReaK::serialization::oarchive& aA, unsigned int ) const {
    ...
    aA& RK_SERIAL_SAVE_WITH_NAME( mean_state_space ) & RK_SERIAL_SAVE_WITH_NAME( covariance_space );
  };
  virtual void RK_CALL load( ReaK::serialization::iarchive& aA, unsigned int ) {
    ...
    aA& RK_SERIAL_LOAD_WITH_NAME( mean_state_space ) & RK_SERIAL_LOAD_WITH_NAME( covariance_space );
  };
...
struct gaussian_ML_reduction {
  ...
  typename pp::topology_traits< StateSpaceOut >::point_type
    map_to_space( const BeliefPoint& b, const gaussian_belief_space< StateSpace, CovarSpace >&,
