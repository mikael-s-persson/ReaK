...
typename boost::enable_if_c< is_continuous_belief_state< BeliefState >::value&&(
                             ...
                             void >::type
  kalman_predict( const LinearSystem& sys, const StateSpaceType& state_space, BeliefState& b_x, const InputBelief& b_u,
                  typename discrete_sss_traits< LinearSystem >::time_type t = 0 ) {
  RK_UNUSED( state_space );
  ...
  sys.get_state_transition_blocks( A, B, state_space, t, t + sys.get_time_step(), x, b_x.get_mean_state(),
                                   b_u.get_mean_state(), b_u.get_mean_state() );
  b_x.set_covariance( CovType( ( A * b_x.get_covariance().get_matrix() * transpose_view( A ) )
...
typename boost::enable_if_c< is_continuous_belief_state< BeliefState >::value&&(
                             ...
                             && ( belief_state_traits< BeliefState >::distribution == belief_distribution::unimodal ),
                             void >::type
  kalman_update( const LinearSystem& sys, const StateSpaceType& state_space, BeliefState& b_x, const InputBelief& b_u,
                 const MeasurementBelief& b_z, typename discrete_sss_traits< LinearSystem >::time_type t = 0 ) {
  // here the requirement is that the system models a linear system which is at worse a linearized system
  ...
  b_x.set_covariance(
    CovType( MatType( ( mat< ValueType, mat_structure::identity >( K.get_row_count() ) - K * C ) * P ) ) );
};
...
typename boost::enable_if_c< is_continuous_belief_state< BeliefState >::value&&(
                             ...
                             && ( belief_state_traits< BeliefState >::distribution == belief_distribution::unimodal ),
                             void >::type
  kalman_filter_step( const LinearSystem& sys, const StateSpaceType& state_space, BeliefState& b_x,
  ...
  vect_n< ValueType > y = to_vect< ValueType >(
    b_z.get_mean_state() - sys.get_output( state_space, x, b_u.get_mean_state(), t + sys.get_time_step() ) );
  mat< ValueType, mat_structure::rectangular, mat_alignment::column_major > CP = C * P;
  mat< ValueType, mat_structure::symmetric > S( CP * transpose_view( C ) + b_z.get_covariance().get_matrix() );
  linsolve_Cholesky( S, CP );
  mat< ValueType, mat_structure::rectangular, mat_alignment::row_major > K( transpose_view( CP ) );
 ...
 * \tparam KFTransferFactory The factory type which can create this kalman predictor.
 */
template < typename KFTransferFactory >
struct KF_belief_transfer {
  typedef KF_belief_transfer< KFTransferFactory > self;
  typedef typename KFTransferFactory::state_space_system state_space_system;
  ...
  const KFTransferFactory* factory;
  ...
  typename pp::topology_traits< BeliefSpace >::point_type
    ...
    kalman_filter_step( *( factory->get_state_space_system() ), b_space.get_state_topology(), b,
                        input_belief_type( u, covariance_type( factory->get_input_disturbance_cov() ) ),
                        output_belief_type( y, covariance_type( factory->get_measurement_noise_cov() ) ), t );
    return b;
  };
  ...
  typename pp::topology_traits< BeliefSpace >::point_type
    ...
    kalman_predict( *( factory->get_state_space_system() ), b_space.get_state_topology(), b,
  ...
  typename pp::topology_traits< BeliefSpace >::point_type
    prediction_to_ML_belief( const BeliefSpace& b_space, typename pp::topology_traits< BeliefSpace >::point_type b,
                             const time_type& t, const input_type& u ) const {
    kalman_update( *( factory->get_state_space_system() ), b_space.get_state_topology(), b,
                   input_belief_type( u, covariance_type( factory->get_input_disturbance_cov() ) ),
                   output_belief_type( factory->get_state_space_system()->get_output( b_space.get_state_topology(),
                                                                                      b.get_mean_state(), u, t ),
  ...
  typename pp::topology_traits< BeliefSpace >::point_type
    ...
    kalman_predict( *( factory->get_state_space_system() ), b_space.get_state_topology(), b, b_u, t );
    kalman_update( *( factory->get_state_space_system() ), b_space.get_state_topology(), b, b_u,
                   output_belief_type( factory->get_state_space_system()->get_output( b_space.get_state_topology(),
                                                                                      b.get_mean_state(), u, t ),
                                       covariance_type( factory->get_measurement_noise_cov() ) ),
...
public:
   ...
   */
  KF_belief_transfer_factory( const state_space_system_ptr& aSys = state_space_system_ptr(),
