 ...
 */
template < typename BeliefTopology, typename BeliefPredictorFactory, typename InputTrajectory >
class belief_predicted_trajectory
  : public pp::waypoint_container< pp::temporal_space< BeliefTopology, pp::time_poisson_topology,
...
public:
  ...
  belief_predicted_trajectory()
      : base_class_type(), input(), pred_factory(), pred_assumption( no_measurements ), pred_seg_mutex(),
   ...
   */
  explicit belief_predicted_trajectory( const shared_ptr< topology >& aSpace, const point_type& aInitialPoint,
      ...
      : base_class_type( aSpace ), input( aInputTrajectory ), pred_factory( aPredFactory ),
        pred_assumption( aPredAssumption ), pred_seg_mutex(), pred_segments(), updated_end( this->waypoints.begin() ) {
  ...
  belief_predicted_trajectory( const self& rhs )
      : base_class_type( static_cast< const base_class_type& >( rhs ) ), input( rhs.input ),
        pred_factory( rhs.pred_factory ), pred_assumption( rhs.pred_assumption ), pred_seg_mutex(), pred_segments(),
    ...
    for( waypoint_descriptor it = rhs.waypoints.begin(); it != rhs.updated_end; ++it, ++last_valid ) {
      pred_segments[&( last_valid->second )]
        = pred_factory.create_predictor( this->space->get_space_topology(), &( last_valid->second.pt ),
   ...
   * object).
   */
  belief_predicted_trajectory( self&& rhs ) BOOST_NOEXCEPT_OR_NOTHROW : base_class_type(),
    ...
    swap( static_cast< base_class_type& >( *this ),
          static_cast< base_class_type& >( rhs ) ); // preserves iterators, except "end".
    if( is_end )
      updated_end = this->waypoints.end();
    else
  ...
  void set_minimal_horizon( const time_type& t ) {
      ...
      switch( pred_assumption ) {
        case no_measurements:
          updated_end->second.pt = pred_segments[&( last_valid->second )].predict_belief(
            this->space->get_space_topology(), last_valid->second.pt, last_valid->first,
    ...
    if( it == this->waypoints.end() ) {
      ...
      while( last_valid->first < t ) {
        ...
        pred = pred_factory.create_predictor( this->space->get_space_topology(), &( last_valid->second.pt ),
                                              last_valid->first, u );
        switch( pred_assumption ) {
          case no_measurements:
            this->push_back( point_type(
              last_valid->first + pred.get_time_step(),
              pred.predict_belief( this->space->get_space_topology(), last_valid->second.pt, last_valid->first, u ) ) );
            break;
          case most_likely_measurements:
            this->push_back( point_type( last_valid->first + pred.get_time_step(),
  ...
  void set_initial_point( const point_type& pt ) {
    ...
    if( ( it == this->waypoints.end() ) || ( fabs( pt.time - it->first ) > 1e-4 * pred_factory.get_time_step() ) ) {
      ...
      pred_segments[&( updated_end->second )]
        = pred_factory.create_predictor( this->space->get_space_topology(), &( updated_end->second.pt ),
    ...
    pred_segments[&( it->second )] = pred_factory.create_predictor(
      this->space->get_space_topology(), &( it->second.pt ), it->first, input.get_point_at_time( it->first ).pt );
    updated_end = it;
    ++updated_end;
  };
   ...
   */
  void set_initial_point( const waypoint_pair& wpt ) { set_initial_point( wpt.second ); };
  ...
  *******************************************************************************/
  ...
  virtual void RK_CALL save( serialization::oarchive& A, unsigned int ) const {
    ...
    A& RK_SERIAL_SAVE_WITH_NAME( input ) & RK_SERIAL_SAVE_WITH_NAME( pred_factory )
  ...
  virtual void RK_CALL load( serialization::iarchive& A, unsigned int ) {
    ...
    A& RK_SERIAL_LOAD_WITH_NAME( input ) & RK_SERIAL_LOAD_WITH_NAME( pred_factory )
