...
void midpoint_integrate_impl( const StateSpace& space, const StateSpaceSystem& sys,
...
public:
  ...
  public:
    ...
    point_type get_point_at_time( double end_time ) const {
      ...
      detail::midpoint_integrate_impl( m_parent->m_t_space->get_space_topology(), *( m_parent->m_sys ),
   ...
   */
  midpoint_integrator_factory( const std::string& aName = "",
  ...
  *******************************************************************************/
  ...
  RK_RTTI_MAKE_CONCRETE_1BASE( self, 0xC2431002, 1, "midpoint_integrator_factory", named_object )
};
};
};
