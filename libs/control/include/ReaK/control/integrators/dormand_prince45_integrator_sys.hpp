...
void dormand_prince45_integrate_impl( const StateSpace& space, const StateSpaceSystem& sys,
  ...
  while( ( ( time_step > 0.0 ) && ( t < end_time ) ) || ( ( time_step < 0.0 ) && ( t > end_time ) ) ) {
    ...
    end_point = space.adjust( prevY, ( 19372.0 / 6561.0 ) * k1 - ( 25360.0 / 2187.0 ) * k2 + ( 64448.0 / 6561.0 ) * k3
                                     - ( 212.0 / 729.0 ) * k4 );
    ...
    end_point = space.adjust( prevY, ( 9017.0 / 3168.0 ) * k1 - ( 355.0 / 33.0 ) * k2 - ( 46732.0 / 5247.0 ) * k3
    ...
    end_point = space.adjust( prevY, ( 35.0 / 384.0 ) * k1 + ( 500.0 / 1113.0 ) * k3 + ( 125.0 / 192.0 ) * k4
    ...
    vect_n< double > err_vect
      = to_vect< double >( ( 5170.0 / 57600.0 ) * k1 + ( 7571.0 / 16695.0 ) * k3 + ( 393.0 / 640.0 ) * k4
                           - ( 92097.0 / 339200.0 ) * k5 + ( 187.0 / 2100.0 ) * k6 - ( 39.0 / 40.0 ) * k7 );
    double Rmax = 0.0;
    ...
    if( Rmax > tolerance ) {
      ...
      end_point = space.adjust( prevY, ( 5170.0 / 57600.0 ) * k1 + ( 7571.0 / 16695.0 ) * k3 + ( 393.0 / 640.0 ) * k4
...
public:
  typedef dormand_prince45_integrator_factory< TemporalSpace, StateSpaceSystem, InputTrajectory > self;
...
public:
  ...
  public:
    ...
    point_type get_point_at_time( double end_time ) const {
      ...
      detail::dormand_prince45_integrate_impl( m_parent->m_t_space->get_space_topology(), *( m_parent->m_sys ),
  ...
  *******************************************************************************/
  ...
  RK_RTTI_MAKE_CONCRETE_1BASE( self, 0xC2431006, 1, "dormand_prince45_integrator_factory", named_object )
};
};
};
