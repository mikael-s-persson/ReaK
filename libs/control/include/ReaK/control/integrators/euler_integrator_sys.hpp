...
void euler_integrate_impl( const StateSpace& space, const StateSpaceSystem& sys,
...
public:
  ...
  public:
    ...
    point_type get_point_at_time( double end_time ) const {
      ...
      detail::euler_integrate_impl( m_parent->m_t_space->get_space_topology(), *( m_parent->m_sys ), m_start_point->pt,
                                    end_point.pt, *( m_parent->m_input_traj ), m_start_point->time, end_point.time,
   ...
   */
  euler_integrator_factory( const std::string& aName = "",
