...
void hamming_iter_mod_integrate_impl( const StateSpace& space, const StateSpaceSystem& sys,
...
public:
   ...
   */
  hamming_iter_mod_integrator_factory( const std::string& aName = "", const shared_ptr< const TemporalSpace >& aTSpace
                                       ...
                                       double aTimeStep = 1e-3, double aTolerance = 1e-6, std::size_t aMaxIter = 10 )
      : named_object(), m_t_space( aTSpace ), m_sys( aSystem ), m_input_traj( aInputTraj ), m_time_step( aTimeStep ),
        m_tolerance( aTolerance ), m_max_iter( aMaxIter ){};
  ...
  *******************************************************************************/
  ...
  virtual void RK_CALL save( serialization::oarchive& A, unsigned int ) const {
    ...
    A& RK_SERIAL_SAVE_WITH_NAME( m_t_space ) & RK_SERIAL_SAVE_WITH_NAME( m_sys )
      ...
      & RK_SERIAL_SAVE_WITH_NAME( m_tolerance ) & RK_SERIAL_SAVE_WITH_NAME( m_max_iter );
  };
  ...
  virtual void RK_CALL load( serialization::iarchive& A, unsigned int ) {
    ...
    A& RK_SERIAL_LOAD_WITH_NAME( m_t_space ) & RK_SERIAL_LOAD_WITH_NAME( m_sys )
      ...
      & RK_SERIAL_LOAD_WITH_NAME( m_tolerance ) & RK_SERIAL_LOAD_WITH_NAME( m_max_iter );
  };
