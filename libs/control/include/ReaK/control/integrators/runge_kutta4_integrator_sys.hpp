/**
 * \file runge_kutta4_integrator_sys.hpp
 *
...
void runge_kutta4_integrate_impl( const StateSpace& space, const StateSpaceSystem& sys,
  ...
  while( ( ( time_step > 0.0 ) && ( t < end_time ) ) || ( ( time_step < 0.0 ) && ( t > end_time ) ) ) {
    ...
    u_wp = u_traj.move_time_diff_from( u_wp, 0.5 * time_step );
    dp = sys.get_state_derivative( space, end_point, u_wp.second.pt, t );
    end_point = space.adjust( end_point,
                              ( 1.0 / 6.0 ) * k1 + ( 2.0 / 6.0 ) * k2 + ( time_step / 6.0 ) * dp - ( 2.0 / 3.0 ) * k3 );
...
public:
  typedef runge_kutta4_integrator_factory< TemporalSpace, StateSpaceSystem, InputTrajectory > self;
...
public:
  ...
  public:
    ...
    point_type get_point_at_time( double end_time ) const {
      ...
      detail::runge_kutta4_integrate_impl( m_parent->m_t_space->get_space_topology(), *( m_parent->m_sys ),
   ...
   */
  runge_kutta4_integrator_factory( const std::string& aName = "", const shared_ptr< const TemporalSpace >& aTSpace
  ...
  *******************************************************************************/
  ...
  RK_RTTI_MAKE_CONCRETE_1BASE( self, 0xC2431003, 1, "runge_kutta4_integrator_factory", named_object )
};
};
};
