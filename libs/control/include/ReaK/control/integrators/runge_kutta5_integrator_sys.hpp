 ...
 *
 * This library implements an integrator that uses the 5th Order Runge-Kutta Method. This
 * method is a single-step fixed-step algorithm for numerical integration based on a 5th order
...
void runge_kutta5_integrate_impl( const StateSpace& space, const StateSpaceSystem& sys,
  ...
  while( ( ( time_step > 0.0 ) && ( t < end_time ) ) || ( ( time_step < 0.0 ) && ( t > end_time ) ) ) {
    ...
    end_point = space.adjust( end_point, ( 276165.0 / 351520.0 ) * k1 - ( 1250865.0 / 351520.0 ) * k2
    ...
    end_point
      = space.adjust( w, ( 439.0 / 216.0 ) * k1 - 8.0 * k2 + ( 3680.0 / 513.0 ) * k3 - ( 845.0 / 4104.0 ) * k4 );

    t += time_step / 13.0;
    u_wp = u_traj.move_time_diff_from( u_wp, time_step / 13.0 );
    dp = sys.get_state_derivative( space, end_point, u_wp.second.pt, t );
    PointDiffType k5 = time_step * dp;
    end_point = space.adjust( w, 2.0 * k2 - ( 8.0 / 27.0 ) * k1 - ( 3544.0 / 2565.0 ) * k3 + ( 1859.0 / 4104.0 ) * k4
                                 - ( 11.0 / 40.0 ) * k5 );

    t -= time_step * 0.5;
    u_wp = u_traj.move_time_diff_from( u_wp, -0.5 * time_step );
    dp = sys.get_state_derivative( space, end_point, u_wp.second.pt, t );
    end_point = space.adjust( w, ( 16.0 / 135.0 ) * k1 + ( 6656.0 / 12825.0 ) * k3 + ( 28561.0 / 56430.0 ) * k4
                                 - ( 9.0 / 50.0 ) * k5 + ( 2.0 * time_step / 55.0 ) * dp );
...
/**
 * This class is a factory for integrators that use the 5th Order Runge-Kutta Method. This
 * method is a single-step fixed-step algorithm for numerical integration based on a 5th order
 * finite differencing scheme (Runge-Kutta).
...
public:
  ...
  public:
    ...
    point_type get_point_at_time( double end_time ) const {
      ...
      detail::runge_kutta5_integrate_impl( m_parent->m_t_space->get_space_topology(), *( m_parent->m_sys ),
   ...
   */
  runge_kutta5_integrator_factory( const std::string& aName = "", const shared_ptr< const TemporalSpace >& aTSpace
                                                                  = shared_ptr< const TemporalSpace >(),
  ...
  *******************************************************************************/
  ...
  RK_RTTI_MAKE_CONCRETE_1BASE( self, 0xC2431004, 1, "runge_kutta5_integrator_factory", named_object )
};
};
};
