...
void adams_BM5_integrate_impl( const StateSpace& space, const StateSpaceSystem& sys,
  ...
  for( std::size_t j = 0; j < correction_count; ++j ) {
    prevY3 = space.adjust( prevY2, ( time_step / 12.0 ) * prevF1 - ( 5.0 * time_step / 12.0 ) * prevF3
  ...
  back_time -= time_step;
  PointType prevY4
    = space.adjust( prevY3, ( 9.0 * time_step / 24.0 ) * dp - ( 37.0 * time_step / 24.0 ) * prevF1
                            + ( 59.0 * time_step / 24.0 ) * prevF2 - ( 55.0 * time_step / 24.0 ) * prevF3 );
  u_wp = u_traj.move_time_diff_from( u_wp, -time_step );
  PointDiffType prevF4 = sys.get_state_derivative( space, prevY4, u_wp.second.pt, back_time );

  for( std::size_t j = 0; j < correction_count; ++j ) {
    prevY4 = space.adjust( prevY3, ( 5.0 * time_step / 24.0 ) * prevF2 - ( time_step / 24.0 ) * prevF1
                                   - ( 19.0 * time_step / 24.0 ) * prevF3 - ( 9.0 * time_step / 24.0 ) * prevF4 );
  ...
  back_time -= time_step;
  PointType prevY5
    = space.adjust( prevY4, ( 2774.0 * time_step / 720.0 ) * prevF3 - ( 1901.0 * time_step / 720.0 ) * prevF4
                            - ( 2616.0 * time_step / 720.0 ) * prevF2 + ( 1274.0 * time_step / 720.0 ) * prevF1
  ...
  for( std::size_t j = 0; j < correction_count; ++j ) {
    PointType prevY5
      = space.adjust( prevY4, ( 19.0 * time_step / 720.0 ) * prevF1 - ( 106.0 * time_step / 720.0 ) * prevF2
                              + ( 264.0 * time_step / 720.0 ) * prevF3 - ( 646.0 * time_step / 720.0 ) * prevF4
  ...
  while( ( ( time_step > 0.0 ) && ( t < end_time ) ) || ( ( time_step < 0.0 ) && ( t > end_time ) ) ) {
    ...
    t += time_step;
    end_point
      = space.adjust( end_point, ( 1901.0 * time_step / 720.0 ) * prevF1 - ( 2774.0 * time_step / 720.0 ) * prevF2
                                 + ( 2616.0 * time_step / 720.0 ) * prevF3 - ( 1274.0 * time_step / 720.0 ) * prevF4
                                 + ( 251.0 * time_step / 720.0 ) * prevF5 );
    u_wp = u_traj.move_time_diff_from( u_wp, time_step );
    ...
    for( std::size_t j = 0; j < correction_count; ++j ) {
      end_point
        = space.adjust( prevY1, ( 251.0 * time_step / 720.0 ) * dp + ( 646.0 * time_step / 720.0 ) * prevF1
                                - ( 264.0 * time_step / 720.0 ) * prevF2 + ( 106.0 * time_step / 720.0 ) * prevF3
...
public:
  ...
  public:
    ...
    point_type get_point_at_time( double end_time ) const {
      ...
      detail::adams_BM5_integrate_impl( m_parent->m_t_space->get_space_topology(), *( m_parent->m_sys ),
   ...
   */
  adams_BM5_integrator_factory( const std::string& aName = "",
