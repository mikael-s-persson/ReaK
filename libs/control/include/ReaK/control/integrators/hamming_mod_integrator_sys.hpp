/**
 * \file hamming_mod_integrator_sys.hpp
 *
...
#ifndef REAK_HAMMING_MOD_INTEGRATOR_SYS_HPP
#define REAK_HAMMING_MOD_INTEGRATOR_SYS_HPP
...
void hamming_mod_integrate_impl( const StateSpace& space, const StateSpaceSystem& sys,
  ...
  for( std::size_t i = 0; i < 3; ++i ) {
    ...
    Y_n[i] = space.adjust( Y_n[i], ( 1250865.0 / 351520.0 ) * k2 - ( 276165.0 / 351520.0 ) * k1
    ...
    Y_n[i] = space.adjust( w, ( 8.0 / 27.0 ) * k1 - 2.0 * k2 + ( 3544.0 / 2565.0 ) * k3 - ( 1859.0 / 4104.0 ) * k4
                              + ( 11.0 / 40.0 ) * k5 );

    back_time += time_step * 0.5;
    u_wp = u_traj.move_time_diff_from( u_wp, 0.5 * time_step );
    Yp_n[i] = sys.get_state_derivative( space, Y_n[i], u_wp.second.pt, back_time );
    Y_n[i] = space.adjust( w, ( -16.0 / 135.0 ) * k1 - ( 6656.0 / 12825.0 ) * k3 - ( 28561.0 / 56430.0 ) * k4
                              + ( 9.0 / 50.0 ) * k5 - ( 2.0 * time_step / 55.0 ) * Yp_n[i] );
  ...
  while( ( ( time_step > 0.0 ) && ( t < end_time ) ) || ( ( time_step < 0.0 ) && ( t > end_time ) ) ) {
    ...
    C = space.adjust( end_point, 0.125 * space.difference( end_point, Y_n[1] )
...
public:
  typedef hamming_mod_integrator_factory< TemporalSpace, StateSpaceSystem, InputTrajectory > self;
...
public:
  ...
  public:
    ...
    point_type get_point_at_time( double end_time ) const {
      ...
      detail::hamming_mod_integrate_impl( m_parent->m_t_space->get_space_topology(), *( m_parent->m_sys ),
   ...
   */
  hamming_mod_integrator_factory( const std::string& aName = "", const shared_ptr< const TemporalSpace >& aTSpace
  ...
  *******************************************************************************/
  ...
  RK_RTTI_MAKE_CONCRETE_1BASE( self, 0xC2431009, 1, "hamming_mod_integrator_factory", named_object )
};
};
};
