...
quadrotor_system::quadrotor_system( const std::string& aName, double aMass,
    ...
    : named_object(), mMass( aMass ), mInertiaMoment( aInertiaMoment ), mTransDragCoefs( aTransDragCoefs ),
...
quadrotor_system::point_derivative_type quadrotor_system::get_state_derivative( const state_space_type&,
                                                                                const point_type& x,
                                                                                const input_type& u, time_type ) const {
  using std::fabs;
  ...
  vect< double, 3 > aacc = mInertiaMomentInv * ( vect< double, 3 >( u[1], u[2], u[3] ) - w % ( mInertiaMoment * w )
                                                 - mRotDragCoefs * w_sqr );
  ...
  return point_derivative_type(
    make_arithmetic_tuple( v, // velocity -> derivative of position
                           ...
                           - q * ( mTransDragCoefs * local_v + vect< double, 3 >( 0.0, 0.0, u[0] / mMass ) ) ),
    make_arithmetic_tuple( w, // angular velocity -> invariant derivative of rotation
                           aacc ) );
};

void quadrotor_system::get_linear_blocks( matrixA_type& A, matrixB_type& B, matrixC_type& C, matrixD_type& D,
                                          const state_space_type&, const time_type&, const point_type& x,
  ...
  mat< double, mat_structure::diagonal > dV( vect< double, 3 >(
    -2.0 * fabs( local_v[0] ) / mMass, -2.0 * fabs( local_v[1] ) / mMass, -2.0 * fabs( local_v[2] ) / mMass ) );
  ...
  mat< double, mat_structure::diagonal > dW(
    vect< double, 3 >( -2.0 * fabs( w[0] ), -2.0 * fabs( w[1] ), -2.0 * fabs( w[2] ) ) );
  set_block( A, mInertiaMomentInv
                * ( mRotDragCoefs * dW - mat< double, mat_structure::skew_symmetric >( w ) * mInertiaMoment
...
void RK_CALL quadrotor_system::save( ReaK::serialization::oarchive& A, unsigned int ) const {
  named_object::save( A, named_object::getStaticObjectType()->TypeVersion() );
  A& RK_SERIAL_SAVE_WITH_NAME( mMass ) & RK_SERIAL_SAVE_WITH_NAME( mInertiaMoment )
    & RK_SERIAL_SAVE_WITH_NAME( mTransDragCoefs ) & RK_SERIAL_SAVE_WITH_NAME( mRotDragCoefs );
};

void RK_CALL quadrotor_system::load( ReaK::serialization::iarchive& A, unsigned int ) {
  named_object::load( A, named_object::getStaticObjectType()->TypeVersion() );
  A& RK_SERIAL_LOAD_WITH_NAME( mMass ) & RK_SERIAL_LOAD_WITH_NAME( mInertiaMoment )
    & RK_SERIAL_LOAD_WITH_NAME( mTransDragCoefs ) & RK_SERIAL_LOAD_WITH_NAME( mRotDragCoefs );
  if( ( mInertiaMoment.get_row_count() != 3 ) || ( mMass < std::numeric_limits< double >::epsilon() ) )
