...
std::shared_ptr<satellite_model_options::system_em_type>
satellite_model_options::get_em_airship_system() const {
  return std::make_shared<system_em_type>("airship3D_em_system", mass,
                                          inertia_tensor, time_step,
                                          vect<double, 3>(0.0, 0.0, -9.81));
};

std::shared_ptr<satellite_model_options::system_emd_type>
satellite_model_options::get_emd_airship_system() const {
  return std::make_shared<system_emd_type>("airship3D_emd_system", mass,
                                           inertia_tensor, time_step,
                                           vect<double, 3>(0.0, 0.0, -9.81));
};

std::shared_ptr<satellite_model_options::system_gyro_emd_type>
satellite_model_options::get_gyro_emd_airship_system() const {
  return std::make_shared<system_gyro_emd_type>(
      "airship3D_gyro_emd_system", mass, inertia_tensor, time_step,
      vect<double, 3>(0.0, 0.0, -9.81));
};

std::shared_ptr<satellite_model_options::system_emdJ_type>
satellite_model_options::get_emdJ_airship_system() const {
  return std::make_shared<system_emdJ_type>("airship3D_emdJ_system", mass,
                                            inertia_tensor, time_step,
                                            vect<double, 3>(0.0, 0.0, -9.81));
};

std::shared_ptr<satellite_model_options::system_gyro_emdJ_type>
satellite_model_options::get_gyro_emdJ_airship_system() const {
  return std::make_shared<system_gyro_emdJ_type>(
      "airship3D_gyro_emdJ_system", mass, inertia_tensor, time_step,
      vect<double, 3>(0.0, 0.0, -9.81));
};

satellite_model_options::state_belief_type
satellite_model_options::get_init_state_belief(double aCovDiag) const {
  state_type x_init;
  set_frame_3D(x_init, initial_motion);
  return {x_init, covar_type(covar_type::matrix_type(
                      mat<double, mat_structure::diagonal>(12, aCovDiag)))};
};
...
satellite_model_options::get_init_state_em_belief(double aCovDiag) const {
  ...
  return {x_init, covar_type(covar_type::matrix_type(P_x))};
}
...
satellite_model_options::input_belief_type
satellite_model_options::get_zero_input_belief() const {
  return {input_type(vect_n<double>(6, 0.0)),
          covar_type(covar_type::matrix_type(input_disturbance))};
}

satellite_model_options::output_belief_type
satellite_model_options::get_zero_output_belief() const {
  vect_n<double> z(get_measurement_count(), 0.0);
  z[3] = 1.0;
  if (artificial_noise.get_row_count() == measurement_noise.get_row_count()) {
    return {output_type(z), covar_type(covar_type::matrix_type(
                                measurement_noise + artificial_noise))};
  }
  return {output_type(z),
          covar_type(covar_type::matrix_type(measurement_noise))};
}

void satellite_model_options::imbue_names_for_received_meas(
...
void satellite_model_options::imbue_names_for_generated_meas(
    recorder::data_stream_options& data_opt) const {
  data_opt.names.clear();
...
void satellite_model_options::imbue_names_for_meas_stddevs(
    recorder::data_stream_options& data_opt) const {
  data_opt.names.clear();
...
void satellite_model_options::imbue_names_for_state_estimates(
    recorder::data_stream_options& data_opt) const {
  data_opt.names.clear();
  data_opt.add_name("time")
      .add_name("pos_x")
...
void satellite_model_options::imbue_names_for_state_estimates_stddevs(
    recorder::data_stream_options& data_opt) const {
  data_opt.names.clear();
  data_opt.add_name("ep_x")
      .add_name("ep_y")
...
void satellite_predictor_options::load_all_configs_impl(
    serialization::iarchive& in) {
  satellite_model_options::load_all_configs_impl(in);
  load_predict_configs_impl(in);
}

void satellite_predictor_options::save_all_configs_impl(
    serialization::oarchive& out) const {
  satellite_model_options::save_all_configs_impl(out);
  save_predict_configs_impl(out);
}

void satellite_predictor_options::load_predict_configs_impl(
    serialization::iarchive& in) {
  unsigned int pred_assume = 0;
  in& RK_SERIAL_LOAD_WITH_NAME(predict_time_horizon) &
...
void satellite_predictor_options::save_predict_configs_impl(
    serialization::oarchive& out) const {
  unsigned int pred_assume = predict_assumption;
  out& RK_SERIAL_SAVE_WITH_NAME(predict_time_horizon) &
