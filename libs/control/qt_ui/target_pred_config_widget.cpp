...
void TargetPredConfigWidget::updateConfigs() {
  ...
  ui->Qf_spin->setValue((sat_options.input_disturbance(0,0) + sat_options.input_disturbance(1,1) + sat_options.input_disturbance(2,2)) / 3.0);
  ui->Qt_spin->setValue((sat_options.input_disturbance(3,3) + sat_options.input_disturbance(4,4) + sat_options.input_disturbance(5,5)) / 3.0);
  
  ui->Rpos_spin->setValue((sat_options.measurement_noise(0,0) + sat_options.measurement_noise(1,1) + sat_options.measurement_noise(2,2)) / 3.0);
  ui->Rang_spin->setValue((sat_options.measurement_noise(3,3) + sat_options.measurement_noise(4,4) + sat_options.measurement_noise(5,5)) / 3.0);
  if(sat_options.measurement_noise.get_col_count() > 6) {
    ui->Rgyro_spin->setValue((sat_options.measurement_noise(6,6) + sat_options.measurement_noise(7,7) + sat_options.measurement_noise(8,8)) / 3.0);
    if(sat_options.measurement_noise.get_col_count() > 9) {
      ui->Racc_spin->setValue((sat_options.measurement_noise(9,9) + sat_options.measurement_noise(10,10) + sat_options.measurement_noise(11,11)) / 3.0);
      ui->Rmag_spin->setValue((sat_options.measurement_noise(12,12) + sat_options.measurement_noise(13,13) + sat_options.measurement_noise(14,14)) / 3.0);
    };
  };
    ...
    switch(sat_options.system_kind & 15) {
      ...
      case sat_opt::invar_mom2:  // IMKFv2
        *target_anim_traj = start_state_predictions(sat_options.get_gyro_sat_system(), sat_options, data_in, 
        ...
        break;
      case sat_opt::invar_mom_em:  // IMKF_em
//         *target_anim_traj = start_state_predictions(sat_options.get_gyro_em_airship_system(), sat_options, data_in, 
//           data_logger, current_target_anim_time);
//         break;
      case sat_opt::invar_mom_emd:  // IMKF_emd
