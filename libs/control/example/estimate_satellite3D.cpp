...
struct sat3D_meas_true_from_extractor {
  ...
  bool step_once() {
    ...
    try {
      ...
      try {
        ...
        set_quaternion( gnd_pt, unit_quat< double >( nvr_in["q_0_true"], nvr_in["q_1_true"], nvr_in["q_2_true"],
                                                     nvr_in["q_3_true"] ) );
        ...
        set_quaternion( gnd_pt,
                        unit_quat< double >( meas_pt.pose[3], meas_pt.pose[4], meas_pt.pose[5], meas_pt.pose[6] ) );
      ...
      vect_n< double > added_noise = ctrl::sample_gaussian_point(
        vect_n< double >( sat_options.artificial_noise.get_row_count(), 0.0 ), sat_options.artificial_noise );
  ...
  sat3D_meas_true_from_extractor( const ReaK::shared_ptr< ReaK::recorder::data_extractor >& aDataIn,
                                  const ReaK::ctrl::satellite_model_options& aSatOptions, std::size_t aSkips = 1 )
      : data_in( aDataIn ), sat_options( aSatOptions ), skips( aSkips ), nvr_in( data_in->getFreshNamedValueRow() ),
...
static const sat3D_state_type& get_sat3D_state( const StateTuple& x ) {
  using ReaK::get;
  return get< 0 >( x );
};
...
template < typename StateTuple >
static void set_sat3D_state( StateTuple& x, const sat3D_state_type& val ) {
  using ReaK::get;
  get< 0 >( x ) = val;
};
};
...
struct sat3D_estimate_result_to_recorder {
  ...
  void add_record( const BeliefStateType& b, const InputBeliefType& b_u, const OutputBeliefType& b_z, double time,
    ...
    ( *rec ) << time << get_position( x_mean ) << get_quaternion( x_mean ) << get_velocity( x_mean )
             << get_ang_velocity( x_mean );
    ...
    if( true_state ) {
      ...
      ( *rec ) << ( get_position( x_mean ) - get_position( *true_state ) ) << ( aa_diff.angle() * aa_diff.axis() )
      ...
      ( *rec ) << ( get_position( x_mean ) - z[range( 0, 3 )] ) << ( aa_diff.angle() * aa_diff.axis() )
...
struct sat3D_collect_stddevs {
  ...
  sat3D_collect_stddevs( const ReaK::shared_ptr< ReaK::recorder::data_recorder >& aRec )
      : stddevs( aRec->getColCount(), 0.0 ), counter( 0 ), rec( aRec ){};
  ...
  void add_record( const BeliefStateType& b, const InputBeliefType& b_u, const OutputBeliefType& b_z, double time,
    ...
    stddevs[range( 9, 12 )] = ( counter * stddevs[range( 9, 12 )] + elem_product( ang_vel_err, ang_vel_err ) )
                              / ( counter + 1 );
...
struct sat3D_collect_prediction_stats {
  ...
  sat3D_collect_prediction_stats( ReaK::vect_n< double >* aStats )
      : stats( aStats ), counter( 0 ), start_time( std::numeric_limits< double >::infinity() ),
        time_since_pred( 0.0 ){};
  ...
  void add_record( const BeliefStateType& b, const InputBeliefType& b_u, const OutputBeliefType& b_z, double time,
    ...
    double pdf_to_est = ctrl::gaussian_pdf_at_diff(
      state_err, mat< double, mat_structure::square >( P_xx( range( 0, 12 ), range( 0, 12 ) ) ) );
    ...
    if( R.get_row_count() == 6 ) {
      ...
      pdf_to_meas = ctrl::gaussian_pdf_at_diff(
        meas_err, mat< double, mat_structure::square >( R( range( 0, 9 ), range( 0, 9 ) ) ) );
...
struct estimation_error_norm_calc {
  ...
  double evaluate_error( const StateBelief& curr_b, Sat3DSystemType&,
    ...
    P_a_inv.invert();
    ReaK::vect_n< double > dx
      = to_vect< double >( state_space.difference( prev_b.get_mean_state(), curr_b.get_mean_state() ) );
...
void batch_KF_on_timeseries( MeasureProvider meas_provider, ResultLogger result_logger,
                             const ReaK::ctrl::satellite_model_options& sat_options, Sat3DSystemType& sat_sys,
                             const typename Sat3DSystemType::state_space_type& state_space,
...
template < typename MeasureProvider, typename ResultLogger, typename Sat3DSystemType >
void batch_KF_no_meas_predict( MeasureProvider meas_provider, ResultLogger result_logger,
                               const ReaK::ctrl::satellite_predictor_options& sat_options, Sat3DSystemType& sat_sys,
                               const typename Sat3DSystemType::state_space_type& state_space, double cur_Pnorm,
                               typename Sat3DSystemType::state_belief_type b,
                               typename Sat3DSystemType::input_belief_type b_u,
                               typename Sat3DSystemType::output_belief_type b_z ) {
  using namespace ReaK;
...
template < typename MeasureProvider, typename ResultLogger, typename Sat3DSystemType >
void batch_KF_ML_meas_predict( MeasureProvider meas_provider, ResultLogger result_logger,
                               const ReaK::ctrl::satellite_predictor_options& sat_options, Sat3DSystemType& sat_sys,
                               const typename Sat3DSystemType::state_space_type& state_space, double cur_Pnorm,
                               typename Sat3DSystemType::state_belief_type b,
                               typename Sat3DSystemType::input_belief_type b_u,
  ...
  do {
    ...
    b_z_ml.set_mean_state(
      sat_sys.get_output( state_space, b.get_mean_state(), b_u.get_mean_state(), meas_provider.get_current_time() ) );
...
typename boost::
  enable_if< ReaK::ctrl::is_augmented_ss_system< Sat3DSystemType >,
             ReaK::shared_ptr< ReaK::pp::
                                 trajectory_base< ReaK::pp::
                                                    temporal_space< ReaK::pp::se3_1st_order_topology< double >::type,
                                                                    ReaK::pp::time_poisson_topology,
                                                                    ReaK::pp::time_distance_only > > > >::type
...
#define RK_D_INF std::numeric_limits< double >::infinity()
  shared_ptr< TemporalBaseSpaceType > sat_base_temp_space( new TemporalBaseSpaceType(
    ...
    make_se3_space( "satellite3D_state_space", vect< double, 3 >( -RK_D_INF, -RK_D_INF, -RK_D_INF ),
...
#undef RK_D_INF
  ...
  return shared_ptr< StateTrajType >(
    new WrappedStateTrajType( "sat3D_predicted_traj", BaseTrajType( sat_base_temp_space, ML_traj ) ) );
};

template < typename Sat3DSystemType, typename MLTrajType >
typename boost::
  enable_if< boost::mpl::not_< ReaK::ctrl::is_augmented_ss_system< Sat3DSystemType > >,
             ReaK::shared_ptr< ReaK::pp::
                                 trajectory_base< ReaK::pp::
                                                    temporal_space< ReaK::pp::se3_1st_order_topology< double >::type,
                                                                    ReaK::pp::time_poisson_topology,
  ...
  typedef ReaK::pp::trajectory_base< ReaK::pp::temporal_space< ReaK::pp::se3_1st_order_topology< double >::type,
                                                               ReaK::pp::time_poisson_topology,
                                                               ReaK::pp::time_distance_only > > StateTrajType;
...
struct prediction_updater {
  ...
  prediction_updater( ReaK::shared_ptr< BeliefPredTrajType > aPredictor, ReaK::shared_ptr< Sat3DSystemType > aSatSys,
                      ReaK::shared_ptr< TempSpaceType > aSatTempSpace, MeasureProvider& aMeasProvider,
                      ResultLogger& aResultLogger, StateBeliefType aB, InputBeliefType aBU, OutputBeliefType aBZ,
                      double aLastTime, double aDiffTolerance, ReaKaux::atomic< double >* aCurrentTargetAnimTime )
      : predictor( aPredictor ), satellite3D_system( aSatSys ), sat_temp_space( aSatTempSpace ),
        meas_provider( aMeasProvider ), result_logger( aResultLogger ), b( aB ), b_u( aBU ), b_z( aBZ ),
        last_time( aLastTime ), diff_tolerance( aDiffTolerance ), current_target_anim_time( aCurrentTargetAnimTime ){};

  int operator()() {

    typedef ReaK::pp::transformed_trajectory< TempSpaceType, BeliefPredTrajType, ReaK::ctrl::maximum_likelihood_map >
      MLTrajType;
    ...
    typedef ReaK::pp::temporal_space< BaseSpaceType, ReaK::pp::time_poisson_topology, ReaK::pp::time_distance_only >
    ...
    ReaK::shared_ptr< ReaK::pp::trajectory_base< TemporalBaseSpaceType > > pred_traj
      = construct_wrapped_trajectory< Sat3DSystemType >( ML_traj, sat_temp_space->get_time_topology().time_step,
    ...
    try {
      do {
        ...
        ReaK::ctrl::tsos_aug_inv_kalman_filter_step( *satellite3D_system, sat_temp_space->get_space_topology(), b, b_u,
                                                     b_z, last_time );
        ...
        result_logger.add_record( BaseStateBeliefType( x_pred.pt, CovarType( 12 ) ), b_u, b_z,
                                  meas_provider.get_current_time(), meas_provider.get_current_gnd_truth_ptr() );
#else
...
template < typename MeasureProvider, typename ResultLogger, typename Sat3DSystemType >
void batch_KF_meas_predict_with_predictor( MeasureProvider meas_provider, ResultLogger result_logger,
  ...
  shared_ptr< TempBeliefSpaceType > sat_temp_belief_space
    = sat_sys.get_temporal_belief_space( 0.0, sat_options.predict_time_horizon );

  shared_ptr< BeliefPredTrajType > predictor(
    new BeliefPredTrajType( sat_temp_belief_space, TempBeliefPointType( meas_provider.get_current_time(), b ),
  ...
  prediction_updater< Sat3DSystemType, MeasureProvider, ResultLogger >::executer
    = ReaKaux::thread( prediction_updater< Sat3DSystemType, MeasureProvider, ResultLogger >(
      predictor, sat_sys_ptr, sat_temp_space, meas_provider, result_logger, b, b_u, b_z,
      meas_provider.get_current_time(), cur_Pnorm, &current_target_anim_time ) );
...
void generate_timeseries( std::vector< std::pair< double, sat3D_measurement_point > >& measurements,
                          std::vector< std::pair< double, sat3D_state_type > >& ground_truth, Sat3DSystemType& sat_sys,
                          const typename Sat3DSystemType::state_space_type& state_space,
                          typename Sat3DSystemType::point_type x, double start_time, double end_time,
                          const cov_matrix_type& Qu, const cov_matrix_type& R,
                          ReaK::shared_ptr< ReaK::recorder::data_recorder > stat_results
                          = ReaK::shared_ptr< ReaK::recorder::data_recorder >() ) {
  using namespace ReaK;
  ...
  for( double t = start_time; t < end_time; t += time_step ) {
    ...
    if( stat_results ) {
      std_devs[range( 0, 6 )]
        = ( ( k - 1 ) * std_devs[range( 0, 6 )] + elem_product( y_noise[range( 0, 6 )], y_noise[range( 0, 6 )] ) ) / k;
    ...
    if( y.size() >= 10 ) {
      ...
      if( stat_results ) {
        std_devs[range( 8, 11 )] = ( ( k - 1 ) * std_devs[range( 8, 11 )]
                                     + elem_product( y_noise[range( 6, 9 )], y_noise[range( 6, 9 )] ) ) / k;
        std_devs[11] = ( ( k - 1 ) * std_devs[11] + norm_2_sqr( y_noise[range( 6, 9 )] ) ) / k;
      };
      if( y.size() >= 16 ) {
        meas.IMU_a_m = y[range( 10, 16 )] + y_noise[range( 9, 15 )];
        if( stat_results ) {
          std_devs[range( 12, 15 )] = ( ( k - 1 ) * std_devs[range( 12, 15 )]
                                        + elem_product( y_noise[range( 9, 12 )], y_noise[range( 9, 12 )] ) ) / k;
          std_devs[15] = ( ( k - 1 ) * std_devs[15] + norm_2_sqr( y_noise[range( 9, 12 )] ) ) / k;
          std_devs[range( 16, 19 )] = ( ( k - 1 ) * std_devs[range( 16, 19 )]
                                        + elem_product( y_noise[range( 12, 15 )], y_noise[range( 12, 15 )] ) ) / k;
...
template < typename Sat3DSystemType >
void
  do_online_run( ReaK::recorder::data_stream_options output_opt, const ReaK::ctrl::satellite_model_options& sat_options,
                 const ReaK::shared_ptr< ReaK::recorder::data_extractor >& data_in, Sat3DSystemType& sat_sys,
                 const typename Sat3DSystemType::state_space_type& state_space,
                 const typename Sat3DSystemType::state_belief_type& b, typename Sat3DSystemType::input_belief_type b_u,
                 const typename Sat3DSystemType::output_belief_type& b_z ) {

  std::stringstream ss;
  ss << "_" << std::setfill( '0' ) << std::setw( 4 ) << int( 1000 * sat_options.time_step ) << "_"
  ...
  batch_KF_on_timeseries( sat3D_meas_true_from_extractor( data_in, sat_options ),
                          sat3D_estimate_result_to_recorder( cur_out_opt.create_recorder() ), sat_options, sat_sys,
                          state_space, b, b_u, b_z );
};

template < typename Sat3DSystemType >
void do_all_single_runs( ReaK::recorder::data_stream_options output_opt,
                         const ReaK::ctrl::satellite_model_options& sat_options,
                         const std::vector< std::pair< double, sat3D_measurement_point > >& measurements,
                         const std::vector< std::pair< double, sat3D_state_type > >& ground_truth,
                         Sat3DSystemType& sat_sys, const typename Sat3DSystemType::state_space_type& state_space,
                         const typename Sat3DSystemType::state_belief_type& b,
                         typename Sat3DSystemType::input_belief_type b_u,
                         const typename Sat3DSystemType::output_belief_type& b_z, unsigned int min_skips,
  ...
  for( unsigned int skips = min_skips; skips <= max_skips; ++skips ) {
    ...
    std::stringstream ss;
    ss << "_" << std::setfill( '0' ) << std::setw( 4 ) << int( 1000 * skips * sat_options.time_step ) << "_"
       << sat_options.get_kf_accronym() << ".";
    recorder::data_stream_options cur_out_opt = output_opt;
    cur_out_opt.file_name += ss.str() + cur_out_opt.get_extension();
    sat_options.imbue_names_for_state_estimates( cur_out_opt );

    batch_KF_on_timeseries( sat3D_meas_true_from_vectors( &measurements, &ground_truth, skips ),
                            sat3D_estimate_result_to_recorder( cur_out_opt.create_recorder() ), sat_options, sat_sys,
                            state_space, b, b_u, b_z );
  };
...
template < typename Sat3DSystemType >
void do_online_prediction( ReaK::recorder::data_stream_options output_opt,
                           const ReaK::ctrl::satellite_predictor_options& sat_options,
                           const ReaK::shared_ptr< ReaK::recorder::data_extractor >& data_in, Sat3DSystemType& sat_sys,
                           const typename Sat3DSystemType::state_space_type& state_space,
                           const typename Sat3DSystemType::state_belief_type& b,
  ...
  std::stringstream ss;
  ss << "_pred_" << std::setfill( '0' ) << std::setw( 8 ) << int( 100000000 * cur_Pnorm ) << "_"
     << sat_options.get_kf_accronym() << ".";
  ReaK::recorder::data_stream_options cur_out_opt = output_opt;
  cur_out_opt.file_name += ss.str() + cur_out_opt.get_extension();
  sat_options.imbue_names_for_state_estimates( cur_out_opt );
  ...
  if( sat_options.predict_assumption == ReaK::ctrl::satellite_predictor_options::no_measurements ) {
    batch_KF_no_meas_predict( sat3D_meas_true_from_extractor( data_in, sat_options ),
                              sat3D_estimate_result_to_recorder( cur_out_opt.create_recorder() ), sat_options, sat_sys,
                              state_space, cur_Pnorm, b, b_u, b_z );
  } else {
    batch_KF_ML_meas_predict( sat3D_meas_true_from_extractor( data_in, sat_options ),
                              sat3D_estimate_result_to_recorder( cur_out_opt.create_recorder() ), sat_options, sat_sys,
                              state_space, cur_Pnorm, b, b_u, b_z );
  };
#endif
...
template < typename Sat3DSystemType >
void do_all_prediction_runs( ReaK::recorder::data_stream_options output_opt,
                             const ReaK::ctrl::satellite_predictor_options& sat_options,
                             const std::vector< std::pair< double, sat3D_measurement_point > >& measurements,
                             const std::vector< std::pair< double, sat3D_state_type > >& ground_truth,
                             Sat3DSystemType& sat_sys, const typename Sat3DSystemType::state_space_type& state_space,
  ...
  for( double cur_Pnorm = max_Pnorm; cur_Pnorm > min_Pnorm; cur_Pnorm *= 0.5 ) {
    ...
    if( sat_options.predict_assumption == ctrl::satellite_predictor_options::no_measurements ) {
      batch_KF_no_meas_predict( sat3D_meas_true_from_vectors( &measurements, &ground_truth ),
                                sat3D_estimate_result_to_recorder( cur_out_opt.create_recorder() ), sat_options,
                                sat_sys, state_space, cur_Pnorm, b, b_u, b_z );
    } else {
      batch_KF_ML_meas_predict( sat3D_meas_true_from_vectors( &measurements, &ground_truth ),
                                sat3D_estimate_result_to_recorder( cur_out_opt.create_recorder() ), sat_options,
                                sat_sys, state_space, cur_Pnorm, b, b_u, b_z );
    };
#endif
...
template < typename Sat3DSystemType >
void do_single_monte_carlo_run( std::map< std::string, ReaK::shared_ptr< ReaK::recorder::data_recorder > >& results_map,
  ...
  for( unsigned int skips = min_skips; skips <= max_skips; ++skips ) {
    ...
    batch_KF_on_timeseries( sat3D_meas_true_from_vectors( &measurements, &ground_truth, skips ),
                            sat3D_collect_stddevs( results ), sat_options, sat_sys, state_space, b, b_u, b_z );
  };
...
void do_prediction_monte_carlo_run(
  ReaK::recorder::data_stream_options output_opt, const ReaK::ctrl::satellite_predictor_options& sat_options,
  const std::vector< std::pair< double, sat3D_measurement_point > >& measurements,
  const std::vector< std::pair< double, sat3D_state_type > >& ground_truth, Sat3DSystemType& sat_sys,
  const typename Sat3DSystemType::state_space_type& state_space, const typename Sat3DSystemType::state_belief_type& b,
  typename Sat3DSystemType::input_belief_type b_u, const typename Sat3DSystemType::output_belief_type& b_z,
  ...
  for( double cur_Pnorm = max_Pnorm; cur_Pnorm > min_Pnorm; cur_Pnorm *= 0.5 ) {
    ...
    if( sat_options.predict_assumption == ctrl::satellite_predictor_options::no_measurements ) {
      batch_KF_no_meas_predict( sat3D_meas_true_from_vectors( &measurements, &ground_truth ),
                                sat3D_collect_prediction_stats( &stats ), sat_options, sat_sys, state_space, cur_Pnorm,
                                b, b_u, b_z );
    } else {
      batch_KF_ML_meas_predict( sat3D_meas_true_from_vectors( &measurements, &ground_truth ),
                                sat3D_collect_prediction_stats( &stats ), sat_options, sat_sys, state_space, cur_Pnorm,
...
static void get_timeseries_from_rec( const ReaK::shared_ptr< ReaK::recorder::data_extractor >& data_in,
  ...
  try {
    ...
    while( true ) {
      ...
      try {
        ...
        set_quaternion(
          x, unit_quat< double >( nvr_in["q_0_true"], nvr_in["q_1_true"], nvr_in["q_2_true"], nvr_in["q_3_true"] ) );
...
int do_required_tasks( ReaK::shared_ptr< Sat3DSystemType > satellite3D_system,
                       ...
                       ReaK::shared_ptr< ReaK::recorder::data_extractor > data_in,
                       const std::vector< std::string >& names_in, const std::string& sys_output_stem_name,
  ...
  if( vm.count( "generate-meas-file" ) && ( vm.count( "xml" ) + vm.count( "protobuf" ) + vm.count( "binary" ) > 0 ) )
    traj_ptr
      = shared_ptr< RecTrajType >( new RecTrajType( shared_ptr< sat3D_temp_space_type >( new sat3D_temp_space_type(
        "satellite3D_temporal_space", sat3D_state_space_type(),
        pp::time_poisson_topology( "satellite3D_time_space", satellite3D_system->get_time_step(),
  ...
  if( vm.count( "generate-mdl-files" ) ) {
    ...
    if( !vm.count( "prediction-runs" ) ) {
      ...
      do_online_run( data_out_stem_opt, sat_options, data_in, *satellite3D_system, sat_space->get_space_topology(),
                     b_init, b_u, b_z );

    } else if( !vm.count( "monte-carlo" ) ) {

      do_online_prediction( data_out_stem_opt, sat_options, data_in, *satellite3D_system,
                            sat_space->get_space_topology(), b_init, b_u, b_z, sat_options.predict_Pnorm_threshold );
    };
    ...
    if( data_in ) {
      ...
      generate_timeseries( measurements, ground_truth, *satellite3D_system, sat_space->get_space_topology(), x_init,
                           start_time, end_time, CovarMatType( sat_options.input_disturbance ), CovarMatType( R_tot ) );
    };
    ...
    if( !vm.count( "generate-meas-file" ) ) {
      ...
      for( std::size_t i = 0; i < measurements.size(); ++i ) {
        ...
        ( *data_meas ) << m.pose << m.gyro << m.IMU_a_m
    ...
    do_prediction_monte_carlo_run( data_out_stem_opt, sat_options, measurements, ground_truth, *satellite3D_system,
                                   sat_space->get_space_topology(), b_init, b_u, b_z,
                                   std::pow( 10.0, -double( max_skips ) ), std::pow( 10.0, -double( min_skips ) ) );
    ...
    for( unsigned int mc_i = 0; mc_i < mc_runs; ++mc_i ) {
      ...
      do_single_monte_carlo_run( results_map, data_out_stem_opt, sat_options, measurements, ground_truth,
                                 *satellite3D_system, sat_space->get_space_topology(), b_init, b_u, b_z, min_skips,
                                 max_skips );
...
int main( int argc, char** argv ) {
  ...
  io_options.add_options()( "generate-meas", "if set, the measurements used for the estimation will be generated from "
                                             "a simulation with the given initial conditions (default is not)" )(
    "generate-meas-file,g", "if set, the measurement file will be generated from the output of a simulation with the "
                            "given initial conditions (default is not)" );
  ...
  sim_options.add_options()( "start-time,s", po::value< double >()->default_value( 0.0 ),
    ...
    "prediction-interval", po::value< double >()->default_value( 1.0 ),
  ...
  output_options.add_options()( "output-traj-file", "if set, output results in a trajectory file (not data-stream)" )(
    "xml,x", "if set, output results in XML format (rkx)" )( "protobuf,p",
  ...
  if( !vm.count( "gyro" ) && !vm.count( "IMU" ) ) {
    ...
    if( vm.count( "imkf-em" ) ) {
      int errcode = do_required_tasks( sat_options.get_em_airship_system(), sat_options, vm, data_in, names_in,
                                       sys_output_stem_name, data_out_stem_opt );
      if( errcode )
        return errcode;
    } else if( vm.count( "imkf-emd" ) ) {
      int errcode = do_required_tasks( sat_options.get_emd_airship_system(), sat_options, vm, data_in, names_in,
                                       sys_output_stem_name, data_out_stem_opt );
      if( errcode )
        return errcode;
    } else if( vm.count( "imkf-emdJ" ) ) {
      int errcode = do_required_tasks( sat_options.get_emdJ_airship_system(), sat_options, vm, data_in, names_in,
                                       sys_output_stem_name, data_out_stem_opt );
      if( errcode )
        return errcode;
    } else {
      int errcode = do_required_tasks( sat_options.get_base_sat_system(), sat_options, vm, data_in, names_in,
                                       sys_output_stem_name, data_out_stem_opt );
      if( errcode )
        return errcode;
    };
    ...
    if( vm.count( "imkf-emd" ) ) {
      int errcode = do_required_tasks( sat_options.get_gyro_emd_airship_system(), sat_options, vm, data_in, names_in,
                                       sys_output_stem_name, data_out_stem_opt );
      if( errcode )
        return errcode;
    } else if( vm.count( "imkf-emdJ" ) ) {
      int errcode = do_required_tasks( sat_options.get_gyro_emdJ_airship_system(), sat_options, vm, data_in, names_in,
                                       sys_output_stem_name, data_out_stem_opt );
      if( errcode )
        return errcode;
    } else {
      int errcode = do_required_tasks( sat_options.get_gyro_sat_system(), sat_options, vm, data_in, names_in,
                                       sys_output_stem_name, data_out_stem_opt );
      if( errcode )
        return errcode;
    };
    ...
    //     int errcode = do_required_tasks(sat_options.get_IMU_sat_system(), sat_options, vm, data_in, names_in,
    //     sys_output_stem_name, data_out_stem_opt);
    //     if(errcode)
    //       return errcode;
  };
