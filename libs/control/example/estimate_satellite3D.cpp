...
typedef ReaK::ctrl::satellite_model_options::state_space_type
    sat3D_state_space_type;
typedef ReaK::ctrl::satellite_model_options::temp_state_space_type
    sat3D_temp_space_type;
...
typedef ReaK::pp::topology_traits<sat3D_temp_space_type>::point_type
    sat3D_temp_point_type;

typedef ReaK::ctrl::satellite_model_options::covar_type cov_type;
typedef cov_type::matrix_type cov_matrix_type;
typedef ReaK::ctrl::satellite_model_options::state_belief_type
    sat3D_state_belief_type;
typedef ReaK::ctrl::satellite_model_options::input_belief_type
    sat3D_input_belief_type;
typedef ReaK::ctrl::satellite_model_options::output_belief_type
    sat3D_output_belief_type;
...
static const sat3D_state_type& get_sat3D_state(const sat3D_state_type& x) {
  return x;
};
...
static void set_sat3D_state(sat3D_state_type& x, const sat3D_state_type& val) {
  x = val;
};
...
struct sat3D_estimate_result_to_recorder {
  ...
  sat3D_estimate_result_to_recorder(
...
struct estimation_error_norm_calc {
  ...
  typedef typename ReaK::ctrl::continuous_belief_state_traits<
      StateBelief>::covariance_type CovarType;
  typedef typename ReaK::ctrl::covariance_mat_traits<CovarType>::matrix_type
      CovMatType;

  double avg_aug_state_diff;
  StateBelief prev_b;

  estimation_error_norm_calc(const StateBelief& curr_b)
      : avg_aug_state_diff(0.0), prev_b(curr_b){};

  template <typename Sat3DSystemType>
  double evaluate_error(
      const StateBelief& curr_b, Sat3DSystemType&,
...
typename std::enable_if<
    ...
    std::shared_ptr<ReaK::pp::trajectory_base<ReaK::pp::temporal_space<
        ReaK::pp::se3_1st_order_topology<double>::type,
        ReaK::pp::time_poisson_topology, ReaK::pp::time_distance_only>>>>::type
construct_wrapped_trajectory(const std::shared_ptr<MLTrajType>& ML_traj, double,
                             double) {
  using namespace ReaK;
  using namespace pp;

  typedef ReaK::pp::trajectory_base<ReaK::pp::temporal_space<
      ReaK::pp::se3_1st_order_topology<double>::type,
...
struct prediction_updater {
  ...
  typedef typename ReaK::pp::topology_traits<TempBeliefSpaceType>::point_type
      TempBeliefPointType;

  typedef ReaK::pp::constant_trajectory<
      ReaK::pp::vector_topology<ReaK::vect_n<double>>>
      InputTrajType;

  typedef typename ReaK::ctrl::try_TSOSAIKF_belief_transfer_factory<
      Sat3DSystemType>::type PredFactoryType;
  typedef ReaK::ctrl::belief_predicted_trajectory<
      BeliefSpaceType, PredFactoryType, InputTrajType>
      BeliefPredTrajType;
  ...
  int operator()() {
    ...
    typedef ReaK::pp::transformed_trajectory<TempSpaceType, BeliefPredTrajType,
    ...
    typedef ReaK::pp::temporal_space<BaseSpaceType,
                                     ReaK::pp::time_poisson_topology,
                                     ReaK::pp::time_distance_only>
        TemporalBaseSpaceType;
    typedef ReaK::pp::topology_traits<TemporalBaseSpaceType>::point_type
        BaseTempStateType;
    ...
    typedef ReaK::ctrl::gaussian_belief_state<BaseStateType, CovarType>
        BaseStateBeliefType;
    ...
    try {
      do {
        ...
        result_logger.add_record(b_pred.pt, b_u, b_z,
                                 meas_provider.get_current_time(),
                                 meas_provider.get_current_gnd_truth_ptr());
#endif
...
void batch_KF_meas_predict_with_predictor(
  ...
  typedef
      typename Sat3DSystemType::temporal_belief_space_type TempBeliefSpaceType;
  typedef typename topology_traits<TempBeliefSpaceType>::point_type
      TempBeliefPointType;
  typedef typename Sat3DSystemType::covar_type CovarType;
  typedef typename CovarType::matrix_type CovarMatType;
...
int do_required_tasks(
  ...
  if (vm.count("generate-meas-file") &&
      (vm.count("xml") + vm.count("protobuf") + vm.count("binary") > 0))
  ...
  } else if (vm.count("monte-carlo") && vm.count("prediction-runs") &&
             data_in) {
...
int main(int argc, char** argv) {
  ...
  io_options.add_options()(
      "generate-meas",
      "if set, the measurements used for the estimation will be generated from "
      "a simulation with the given initial conditions (default is not)")(
      "generate-meas-file,g",
  ...
  sim_options.add_options()("start-time,s",
      ...
      "minimum number of time-step skips between estimations when generating a "
      "series of Monte-Carlo statistics (default "
      "is 1, i.e., one estimation point per measurement point)")(
      "max-skips", po::value<unsigned int>()->default_value(1),
      "maximum number of time-step skips between estimations when generating a "
      "series of Monte-Carlo statistics (default "
      "is 1, i.e., one estimation point per measurement point)")(
      "prediction-runs",
      ...
      "online-run",
      "if set, will perform estimation or prediction online (no pre-buffering "
      "of data stream)");
  ...
  po::options_description model_options =
      ctrl::get_satellite_model_options_po_desc(true);

  po::options_description data_io_options =
  ...
  if (!vm.count("gyro") && !vm.count("IMU")) {
    ...
    if (vm.count("imkf-em")) {
      int errcode = do_required_tasks(sat_options.get_em_airship_system(),
                                      sat_options, vm, data_in, names_in,
                                      sys_output_stem_name, data_out_stem_opt);
      if (errcode)
        return errcode;
    } else if (vm.count("imkf-emd")) {
      int errcode = do_required_tasks(sat_options.get_emd_airship_system(),
                                      sat_options, vm, data_in, names_in,
                                      sys_output_stem_name, data_out_stem_opt);
      if (errcode)
        return errcode;
    } else if (vm.count("imkf-emdJ")) {
      int errcode = do_required_tasks(sat_options.get_emdJ_airship_system(),
                                      sat_options, vm, data_in, names_in,
                                      sys_output_stem_name, data_out_stem_opt);
      if (errcode)
        return errcode;
    } else {
      int errcode = do_required_tasks(sat_options.get_base_sat_system(),
                                      sat_options, vm, data_in, names_in,
                                      sys_output_stem_name, data_out_stem_opt);
      if (errcode)
        return errcode;
    ...
    if (vm.count("imkf-emd")) {
      int errcode = do_required_tasks(sat_options.get_gyro_emd_airship_system(),
                                      sat_options, vm, data_in, names_in,
                                      sys_output_stem_name, data_out_stem_opt);
      if (errcode)
        return errcode;
      ...
      int errcode = do_required_tasks(
          sat_options.get_gyro_emdJ_airship_system(), sat_options, vm, data_in,
          names_in, sys_output_stem_name, data_out_stem_opt);
      if (errcode)
        return errcode;
    } else {
      int errcode = do_required_tasks(sat_options.get_gyro_sat_system(),
                                      sat_options, vm, data_in, names_in,
                                      sys_output_stem_name, data_out_stem_opt);
      if (errcode)
        return errcode;
