...
PlannerAlgConfigWidget::PlannerAlgConfigWidget( QWidget* parent, Qt::WindowFlags flags )
    : QDockWidget( tr( "Planner" ), parent, flags ), ui( new Ui::PlannerAlgConfig() ), planOptions() {
  ...
  connect( ui->planning_algo_selection, SIGNAL( currentIndexChanged(int)), this, SLOT( onUpdateAvailableOptions(int)) );
...
bool PlannerAlgConfigWidget::outputTiming() const { return ui->check_print_timing->isChecked(); };

bool PlannerAlgConfigWidget::outputNodeCounter() const { return ui->check_print_counter->isChecked(); };

bool PlannerAlgConfigWidget::outputMotionGraph() const { return ui->check_print_graph->isChecked(); };

bool PlannerAlgConfigWidget::outputSolution() const { return ui->check_print_best->isChecked(); };
};
};
