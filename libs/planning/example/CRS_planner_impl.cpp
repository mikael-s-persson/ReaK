...
void CRSPlannerGUI_animate_bestsol_trajectory( void* pv, SoSensor* ) {
  ...
  if( !manip_traj ) {
    ...
    animation_start = ReaKaux::chrono::high_resolution_clock::now()
                      - ReaKaux::chrono::duration_cast< ReaKaux::chrono::high_resolution_clock::duration >(
                          ReaKaux::chrono::duration< double >( p->current_target_anim_time
                                                               - p->plan_alg_config.planOptions.start_delay ) );
  };
  if( ( p->sol_anim.enabled ) && ( ( *cur_pit ).time < manip_traj->get_end_time() ) ) {
    if( ( *cur_pit ).time <= 0.001
                             * ( ReaKaux::chrono::duration_cast< ReaKaux::chrono::milliseconds >(
                                   ReaKaux::chrono::high_resolution_clock::now() - animation_start ) ).count() ) {
      //     if( ((*cur_pit).time - manip_traj->get_start_time()) <= 0.001 *
      //     (ReaKaux::chrono::duration_cast<ReaKaux::chrono::milliseconds>(ReaKaux::chrono::high_resolution_clock::now()
    ...
    if( ( *cur_pit ).time <= 0.001
      ...
      *( p->ct_config.sceneData.target_kin_model->getFrame3D( 0 ) ) = get_frame_3D( ( *cur_pit ).pt );
      ...
      if( p->ct_config.sceneData.chaser_kin_model && p->ct_interact.isIKEnabled() && !( p->sol_anim.enabled ) ) {
        try {
          *( p->ct_config.sceneData.chaser_kin_model->getDependentFrame3D( 0 )->mFrame )
            = *( p->ct_config.sceneData.target_frame );
...
void CRSPlannerGUI::stopTargetAnimation() { target_anim.enabled = false; };
...
void CRSPlannerGUI::startCompleteAnimation() {
  ...
  if( !sol_anim.trajectory || !target_anim.trajectory || !ct_config.sceneData.chaser_kin_model
...
void CRSPlannerGUI::runPlanner() {
  try {
    ...
    QMessageBox::critical( this, "Motion-Planning Error!",
...
void CRSPlannerGUI::threadedPlanningFunction( int mode ) {
  ...
  if( ( mode == 2 ) || ( mode == 3 ) ) {
    ...
    if( !target_anim.trajectory ) {
      emit notifyConsoleMessage( "State-Prediction Error!\nThe live state-estimation of the target failed to produce a "
...
void CRSPlannerGUI::executeSolutionTrajectory() {
  ...
  if( current_target_anim_time > cur_time ) {
    ...
    ReaKaux::chrono::high_resolution_clock::time_point delayed_start
      = exec_start + ReaKaux::chrono::duration_cast< ReaKaux::chrono::high_resolution_clock::duration >(
                       ReaKaux::chrono::duration< double >( cur_time - current_target_anim_time ) );
...
CRSPlannerGUI::CRSPlannerGUI( QWidget* parent, Qt::WindowFlags flags )
    : QMainWindow( parent, flags ), view3d_menu( this ), ct_config( &view3d_menu, this ),
      ct_interact( &ct_config.sceneData, this ), space_config( this ), plan_alg_config( this ),
      target_pred_config( &target_anim.trajectory, &current_target_anim_time, this ), run_dialog( this ),
      planning_thr(), exec_robot_thr() {
  setupUi( this );
  ...
  connect( this, SIGNAL( notifyConsoleMessage( QString ) ), &run_dialog, SLOT( publishConsoleMessage( QString ) ),
           Qt::QueuedConnection );
  connect( this, SIGNAL( notifyReset() ), &run_dialog, SLOT( onReset() ), Qt::QueuedConnection );
  connect( this, SIGNAL( notifyInitializationDone() ), &run_dialog, SLOT( onInitializationDone() ),
           Qt::QueuedConnection );
  connect( this, SIGNAL( notifyPlanningDone() ), &run_dialog, SLOT( onPlanningDone() ), Qt::QueuedConnection );
  connect( this, SIGNAL( notifyLaunchOpportunity() ), &run_dialog, SLOT( onLaunchOpportunity() ),
           Qt::QueuedConnection );
...
int main( int argc, char** argv ) {
  ...
  conf_options.add_options()( "scene-data", po::value< std::string >(), "file-name of a complete model file" )(
  ...
  if( vm.count( "predictor-cfg" ) )
    window.target_pred_config.loadPredictorConfigurations( vm["predictor-cfg"].as< std::string >() );
