...
typedef IHAQR_topology< quadrotor_system::state_space_type, quadrotor_system, position_only_sampler >
  X8_IHAQR_space_type;
typedef MEAQR_topology< quadrotor_system::state_space_type, quadrotor_system, position_only_sampler >
  X8_MEAQR_space_type;
typedef IHAQR_topology_with_CD< quadrotor_system::state_space_type, quadrotor_system, position_only_sampler >
  X8_IHAQRCD_space_type;
typedef MEAQR_topology_with_CD< quadrotor_system::state_space_type, quadrotor_system, position_only_sampler >
...
struct all_robot_info {
  ...
  shared_ptr< frame_3D< double > > X8_base_frame;
  shared_ptr< geom::colored_model_3D > X8_geom_model;
  shared_ptr< geom::proxy_query_model_3D > X8_geom_proxy;
...
void keyboard_press_hdl( void* userData, SoEventCallback* eventCB ) {
  ...
  if( PP_enabled ) {
    ...
    try {
      ...
      path_planning_p2p_query< X8_MEAQRCD_space_type > pp_query( "pp_query", r_info->X8_MEAQR_space, start_point,
                                                                 goal_point, r_info->X8_plan_params.max_results );
      ...
      frame_reporter_type temp_reporter( pp::make_any_model_applicator< X8_MEAQR_space_type >(
                                           manip_direct_kin_map( r_info->builder ), MEAQR_to_state_mapper(),
                                           shared_ptr< quadrotor_system::state_space_type >(
                                             &( r_info->X8_MEAQR_space->get_state_space() ), null_deleter() ) ),
                                         5 );
      ...
      typedef MEAQR_rrtstar_planner< quadrotor_system::state_space_type, quadrotor_system, position_only_sampler >
        X8_rrtstar_planner_type;
      X8_rrtstar_planner_type X8_planner( r_info->X8_MEAQR_space, r_info->X8_plan_params.max_vertices,
...
int main( int argc, char** argv ) {
  ...
  if( argc < 4 ) {
    std::cout
      << "Usage: ./X8_test_scene [world_filename.xml] [quadrotor_spaces.xml] [planner_parameters.xml]" << std::endl
      << "\t world_filename.xml: \tThe XML filename of the world geometry (including start and end position of "
         "planning)." << std::endl
      << "\t quadrotor_spaces.xml: \tThe XML filename of the definitions of the quad-rotor IHAQR and MEAQR topologies."
      << std::endl << "\t planner_parameters.xml: \tThe XML filename of the set of parameters for the planner."
      << std::endl;
    return 1;
  };
...

  {
    ...
    r_info.X8_IHAQR_space
      = shared_ptr< X8_IHAQRCD_space_type >( new X8_IHAQRCD_space_type( *tmp_IHAQR_space, r_info.builder ) );
    r_info.X8_MEAQR_space
      = shared_ptr< X8_MEAQRCD_space_type >( new X8_MEAQRCD_space_type( *tmp_MEAQR_space, r_info.builder ) );
  };

  shared_ptr< geom::proxy_query_pair_3D > X8_to_world_proxy(
    new geom::proxy_query_pair_3D( "X8_to_world_proxy", r_info.X8_geom_proxy, world_geom_proxy ) );
...

  {
...

    {
      ...
      sg << *world_geom_model;
      sg << geom::coord_arrows_3D( "base_arrows", shared_ptr< frame_3D< double > >( new frame_3D< double >() ),
                                   pose_3D< double >(), 0.3 );
      ...
      keypressCB->addEventCallback( SoKeyboardEvent::getClassTypeId(), keyboard_press_hdl, &r_info );
