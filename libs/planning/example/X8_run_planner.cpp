...
int main( int argc, char** argv ) {
  ...
  if( argc < 5 ) {
    std::cout
      ...
      << std::endl << "\t planner_params.xml: \tThe XML filename of the set of parameters for the planner." << std::endl
  ...
  shared_ptr< X8_MEAQRCD_space_type > X8_MEAQR_space;
  {
    ...
    X8_IHAQR_space = shared_ptr< X8_IHAQRCD_space_type >( new X8_IHAQRCD_space_type( *tmp_IHAQR_space, builder ) );
    X8_MEAQR_space = shared_ptr< X8_MEAQRCD_space_type >( new X8_MEAQRCD_space_type( *tmp_MEAQR_space, builder ) );
  };

  shared_ptr< geom::proxy_query_pair_3D > X8_to_world_proxy(
    new geom::proxy_query_pair_3D( "X8_to_world_proxy", X8_geom_proxy, world_geom_proxy ) );
  ...
  std::size_t knn_method = ReaK::pp::LINEAR_SEARCH_KNN; // (0) or ReaK::pp::DVP_BF2_TREE_KNN (3)
  {
    ...
    in& RK_SERIAL_LOAD_WITH_ALIAS( "planner_kind", planner_kind )
      ...
      & RK_SERIAL_LOAD_WITH_ALIAS( "max_results", max_results ) & RK_SERIAL_LOAD_WITH_ALIAS( "knn_method", knn_method );
  };
  ...
  try {
    ...
    path_planning_p2p_query< X8_MEAQRCD_space_type > pp_query( "pp_query", X8_MEAQR_space, start_point, goal_point,
                                                               max_results );
    ...
    X8_rrtstar_planner_type X8_planner( X8_MEAQR_space, max_vertices, prog_interval, ADJ_LIST_MOTION_GRAPH | knn_method,
