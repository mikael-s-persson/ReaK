...
template < typename ManipMdlType, typename InterpTag, int Order, typename TargetStateTrajectory,
...
void CRS_execute_dynamic_planner_impl( const ReaK::kte::chaser_target_data& scene_data,
  ...
  frame_reporter_type temp_reporter(
    make_any_model_applicator< dynamic_super_space_type >(
      rlDK_map_type( chaser_concrete_model, scene_data.chaser_jt_limits, normal_jt_space ), extract_spatial_component(),
      jt_space ),
  ...
  typedef transformed_trajectory< dynamic_super_space_type, TargetStateTrajectory, temporal_dkik_map_type >
    mapped_traj_type;
  ...
  shared_ptr< mapped_traj_type > mapped_goal_traj( new mapped_traj_type(
    ...
    temporal_dkik_map_type( dkik_map_type( manip_direct_kin_map( scene_data.target_kin_model ),
  ...
  motion_plan_intercept_query< dynamic_workspace_type, mapped_traj_type > pp_query(
    "intercept_query", workspace, temporal_start_point, mapped_goal_traj, target_state_traj->get_end_time(), min_travel,
  ...
  if( plan_options.planning_algo == 1 ) { // RRT*
    ...
    workspace_planner
      = shared_ptr< sample_based_planner< dynamic_workspace_type > >( new rrtstar_planner< dynamic_workspace_type >(
        workspace, plan_options.max_vertices, plan_options.prog_interval,
        plan_options.store_policy | plan_options.knn_method, plan_options.planning_options, 0.1, 0.05,
        workspace_dims + 1, report_chain ) );
  ...
  if( plan_options.planning_algo == 2 ) { // PRM
    ...
    shared_ptr< sbastar_planner< dynamic_workspace_type > > tmp( new sbastar_planner< dynamic_workspace_type >(
      workspace, plan_options.max_vertices, plan_options.prog_interval,
      plan_options.store_policy | plan_options.knn_method, plan_options.planning_options, 0.1, 0.05,
  ...
  if( bestsol_rltraj ) {
    ...
    for( PtIter it = bestsol_rltraj->begin_time_travel(); it != bestsol_rltraj->end_time_travel(); it += 0.001 ) {
      ...
      sol_traj.push_back(
        TCSpacePointType( cur_pt.time, get< 0 >( joint_limits_mapping< double >( scene_data.chaser_jt_limits )
...
void CRSPlannerGUI::executeDynamicPlanner() {
  ...
  if( ( space_config.space_order == 0 ) && ( space_config.interp_id == 0 ) ) {
    CRS_execute_dynamic_planner_impl< kte::manip_P3R3R_kinematics, linear_interpolation_tag, 0 >(
      ...
      print_timing, print_counter, jt_start, target_anim.trajectory, sol_anim.trajectory, stop_planner );
  } else
#if 0
