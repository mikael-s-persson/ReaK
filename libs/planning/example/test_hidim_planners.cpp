...
int main( int argc, char** argv ) {
  ...
  io_options.add_options()( "output-path,o", po::value< std::string >()->default_value( "pp_results" ),
  ...
  planner_select_options.add_options()( "max-edge-length", po::value< double >(),
  ...
  if( vm.count( "monte-carlo" ) + vm.count( "single-run" ) + vm.count( "generate-all-files" )
      + vm.count( "generate-planner-options" ) < 1 ) {
  ...
  shared_ptr< WorldNDType > world_ND = shared_ptr< WorldNDType >(
    new WorldNDType( world_ND_name + "_no_obstacles",
                     hyperbox_topology< vect< double, RK_HIDIM_PLANNER_N > >( world_ND_name, lb, ub ), max_radius ) );
  ...
  if( vm.count( "monte-carlo" ) ) {
    ...
    try {
      execute_p2p_planner( world_ND, plan_options, RK_HIDIM_PLANNER_N, mc_eng, world_ND->get_start_pos(),
                           world_ND->get_goal_pos() );
    } catch( std::exception& e ) {
  ...
  if( vm.count( "single-run" ) ) {
    ...
    try {
      execute_p2p_planner( world_ND, plan_options, RK_HIDIM_PLANNER_N, sr_eng, world_ND->get_start_pos(),
                           world_ND->get_goal_pos() );
    } catch( std::exception& e ) {
      std::cerr << "Error: An exception was raised during the planning:\nwhat(): " << e.what() << std::endl;
      return 3;
