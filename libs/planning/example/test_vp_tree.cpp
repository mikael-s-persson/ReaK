...
int main() {
  ...
  typedef boost::adjacency_list_BC< boost::vecBC, boost::vecBC, boost::undirectedS, WorldGridVertexProperties,
                                    WorldGridEdgeProperties > WorldGridType;

  typedef boost::graph_traits< WorldGridType >::vertex_descriptor VertexType;
  typedef ReaK::pp::dvp_tree< VertexType, TopologyType,
                              boost::property_map< WorldGridType, boost::vertex_position_t >::type, 4 > WorldPartition4;

  typedef ReaK::pp::dvp_tree< VertexType, TopologyType,
                              boost::property_map< WorldGridType, boost::vertex_position_t >::type, 2 > WorldPartition2;

  const unsigned int grid_sizes[]
    = {100,   200,   300,   400,   500,   800,    1000,   1100,   1300,    1500,    1700,   1900,  2000,
       2200,  2500,  3000,  3500,  4000,  4500,   5000,   6000,   7000,    8000,    9000,   10000, 12000,
       15000, 20000, 25000, 30000, 50000, 100000, 200000, 500000, 1000000, 2000000, 5000000};
  ...
  for( int i = 0; i < 37; ++i ) {
    ...
    {
      WorldPartition2 part2_fresh( grid, ReaK::shared_ptr< const TopologyType >( &m_space, ReaK::null_deleter() ),
                                   m_position );
    ...
    {
      WorldPartition4 part4_fresh( grid, ReaK::shared_ptr< const TopologyType >( &m_space, ReaK::null_deleter() ),
                                   m_position );
