...
bool test_propgraph_functions( Graph& g ) {
  ...
  for( std::size_t i = pts_v.size() / 2; i < pts_v.size(); ++i ) {
    if( get( vp_2_bundle, get_raw_vertex_property( g, pts_v[i] ) ).pos != get_raw_vertex_to_bundle( g, pts[i] ).pos ) {
  ...
  for( std::size_t i = 0; i < segs_e.size(); ++i ) {
    if( get( ep_2_bundle, get_raw_edge_property( g, segs_e[i] ) ).dist != get_raw_edge_to_bundle( g, segs[i] ).dist ) {
      RK_ERROR( "The dist property of edge " << i << " was not preserved in the edge-addition!" );
      return false;
    };
  };
...
int main() {
  ...
  ReaK::shared_ptr< TopologyType > m_space = ReaK::shared_ptr< TopologyType >(
    new TopologyType( "", ReaK::vect< double, 2 >( 0.0, 0.0 ), ReaK::vect< double, 2 >( 1.0, 1.0 ) ) );
  ...
  typedef boost::adjacency_list_BC< boost::vecBC, boost::poolBC, boost::bidirectionalS, WorldGridVertexProperties,
