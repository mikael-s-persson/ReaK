...
void CRS_execute_static_planner_impl( const ReaK::kte::chaser_target_data& scene_data,
  ...
  shared_ptr< static_workspace_type > workspace = make_manip_static_workspace< Order >(
    InterpTag(), chaser_concrete_model, scene_data.chaser_jt_limits, min_travel );
  ...
  path_planning_p2p_query< static_workspace_type > pp_query( "pp_query", workspace, start_point, goal_point,
  ...
  if( plan_options.planning_algo == 1 ) { // RRT*
    ...
    workspace_planner
      = shared_ptr< sample_based_planner< static_workspace_type > >( new rrtstar_planner< static_workspace_type >(
        workspace, plan_options.max_vertices, plan_options.prog_interval,
        plan_options.store_policy | plan_options.knn_method, plan_options.planning_options, 0.1, 0.05, workspace_dims,
        report_chain ) );
  ...
  if( plan_options.planning_algo == 2 ) { // PRM
    ...
    shared_ptr< sbastar_planner< static_workspace_type > > tmp( new sbastar_planner< static_workspace_type >(
      workspace, plan_options.max_vertices, plan_options.prog_interval,
  ...
  if( bestsol_rlpath ) {
    ...
    double t = 0.0;
    for( PtIter it = bestsol_rlpath->begin_fraction_travel(); it != bestsol_rlpath->end_fraction_travel();
         it += 0.1, t += 0.1 ) {
      sol_traj.push_back( TCSpacePointType( t, get< 0 >( joint_limits_mapping< double >( scene_data.chaser_jt_limits )
...
void CRSPlannerGUI::executePlanner() {
  ...
  *EE_frame = *( ct_config.sceneData.target_frame );
  ...
  if( ( space_config.space_order == 0 ) && ( space_config.interp_id == 0 ) ) {
    CRS_execute_static_planner_impl< kte::manip_P3R3R_kinematics, linear_interpolation_tag, 0 >(
      ct_config.sceneData, plan_alg_config.planOptions, sw_motion_graph, sw_solutions, space_config.min_travel,
      print_timing, print_counter, jt_start, jt_desired, sol_anim.trajectory, stop_planner );
  } else
#if 0
  ...
  if((space_config.space_order == 2) && (space_config.interp_id == 0)) {
    ...
    CRS_execute_static_planner_impl< kte::manip_P3R3R_kinematics, cubic_hermite_interpolation_tag, 1 >(
      ct_config.sceneData, plan_alg_config.planOptions, sw_motion_graph, sw_solutions, space_config.min_travel,
      print_timing, print_counter, jt_start, jt_desired, sol_anim.trajectory, stop_planner );
  } else
#if 0
  if((space_config.space_order == 2) && (space_config.interp_id == 1)) {
    ...
    CRS_execute_static_planner_impl< kte::manip_P3R3R_kinematics, quintic_hermite_interpolation_tag, 2 >(
      ct_config.sceneData, plan_alg_config.planOptions, sw_motion_graph, sw_solutions, space_config.min_travel,
      print_timing, print_counter, jt_start, jt_desired, sol_anim.trajectory, stop_planner );
  } else if( ( space_config.space_order == 1 ) && ( space_config.interp_id == 3 ) ) {
    CRS_execute_static_planner_impl< kte::manip_P3R3R_kinematics, svp_Ndof_interpolation_tag, 1 >(
      ct_config.sceneData, plan_alg_config.planOptions, sw_motion_graph, sw_solutions, space_config.min_travel,
      print_timing, print_counter, jt_start, jt_desired, sol_anim.trajectory, stop_planner );
  } else
#if 0
  if((space_config.space_order == 2) && (space_config.interp_id == 3)) {
    ...
    CRS_execute_static_planner_impl< kte::manip_P3R3R_kinematics, sap_Ndof_interpolation_tag, 2 >(
      ct_config.sceneData, plan_alg_config.planOptions, sw_motion_graph, sw_solutions, space_config.min_travel,
      print_timing, print_counter, jt_start, jt_desired, sol_anim.trajectory, stop_planner );
  };
};
