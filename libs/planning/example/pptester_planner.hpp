...
public:
  ...
  pptester_planner()
      : mRobotCollisionRadius( 0.4 ), mGridDim( 16.0, 16.0 ), mGridCenter( 8.0, 8.0 ), mStart( -6.43, -5.89 ),
        mGoal( 7.0, 7.0 ), mRobotRunning( false ), mBestPath(){};
...
protected:
  virtual void executePath_impl() {
    ...
    while( ( mRobotRunning ) && ( !pp.GetStall() ) && ( pt_it != mBestPath.end() ) ) {
      ...
      double pd_y
        = ( *pt_it )[1]; //(world_map_cvimage.size().height - (*pt_it)[1]) * pix_to_m - rrt_prop.GridCenterY->value();
      ...
      if( ( p_x - pd_x ) * ( p_x - pd_x ) + ( p_y - pd_y ) * ( p_y - pd_y ) < 2 * mRobotCollisionRadius
                                                                              * mRobotCollisionRadius ) {
        pt_it++;
      };
    };
