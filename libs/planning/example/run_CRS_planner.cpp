...
typedef ReaK::pp::temporal_space<sat_state_space_type, ReaK::pp::time_poisson_topology, ReaK::pp::time_distance_only> sat_temporal_space_type;
...
void CRS_execute_static_planner(const ReaK::kte::chaser_target_data& scene_data, 
  ...
  start_point = joint_limits_mapping<double>(scene_data.chaser_jt_limits).map_to_space(start_inter, *normal_jt_space, *jt_space);
  
  goal_inter = normal_jt_space->origin();
  get<0>(goal_inter) = jt_desired;
  goal_point = joint_limits_mapping<double>(scene_data.chaser_jt_limits).map_to_space(goal_inter, *normal_jt_space, *jt_space);
...
void CRS_execute_dynamic_planner(const ReaK::kte::chaser_target_data& scene_data, 
  ...
  shared_ptr< dynamic_workspace_type > workspace = 
    make_manip_dynamic_workspace<Order>(InterpTag(),
      ...
      space_def.min_travel, target_state_traj->get_end_time() - target_state_traj->get_start_time(), 
  ...
  get<0>(start_inter) = jt_start;
  rl_point_type start_point = joint_limits_mapping<double>(scene_data.chaser_jt_limits).map_to_space(start_inter, *normal_jt_space, *jt_space);
  ...
  typedef transformed_trajectory<dynamic_super_space_type, sat_trajectory_type, temporal_dkik_map_type> mapped_traj_type;
  ...
  execute_intercept_planner(workspace, plan_options, workspace_dims + 1,
                            target_state_traj->get_end_time(), space_def.min_travel, 
                            engine, temporal_start_point, mapped_goal_traj);
  
  // Difference with the GUI's dynexec "manual" code (now done within the 'execute_intercept_planner' function and the engine:
...
int main(int argc, char** argv) {
  ...
  io_options.add_options()
    ...
    ("target-trajectory", po::value< std::string >(), "specify the file containing the target trajectory of the capture target (satellite / airship), if not specified, the pose of the target model will be used.")
