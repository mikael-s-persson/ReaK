...
int main( int argc, char** argv ) {
  ...
  io_options.add_options()( "space-def-file,s",
                            po::value< std::string >()->default_value( "models/quadrotor_spaces.xml" ),
                            "specify the space-definition file (default is models/quadrotor_spaces.xml)" )(
  ...
  std::string space_def_file_name_only(
    std::find( space_def_file_name.rbegin(), space_def_file_name.rend(), '/' ).base(),
    std::find( space_def_file_name.rbegin(), space_def_file_name.rend(), '.' ).base() - 1 );
  ...
  typedef boost::adjacency_list_BC< boost::vecBC, boost::vecBC, boost::undirectedS, MEAQR_vprop, boost::no_property >
    WorldGridType;
  ...
  const unsigned int grid_sizes[]
    = {100, 200, 300, 400, 500, 600, 800, 1000, 1500, 2000, 2500, 3000, 4000, 5000, 7500, 10000};
