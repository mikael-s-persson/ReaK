...
CRSRunDialogWidget::CRSRunDialogWidget( QWidget* parent, Qt::WindowFlags flags )
    : QDialog( parent, flags ), ui( new Ui::CRSRunDialog() ), flashing_button_timer( this ) {
  ui->setupUi( this );
  ...
  connect( &( flashing_button_timer ), SIGNAL( timeout() ), this, SLOT( flipLaunchButtonColor() ) );
};

CRSRunDialogWidget::~CRSRunDialogWidget() { delete ui; };
...
void CRSRunDialogWidget::publishConsoleMessage( QString aMessage ) {
  ...
  ui->status_text->verticalScrollBar()->setSliderPosition( ui->status_text->verticalScrollBar()->maximum() );
};
};
};
