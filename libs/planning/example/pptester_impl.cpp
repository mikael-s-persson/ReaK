...
PPTestWindow::PPTestWindow( QWidget* parent, Qt::WindowFlags flags )
  ...
  connect( rrt_prop.buttonBox, SIGNAL( accepted() ), this, SLOT( acceptRRTProperties() ) );
...
PPTestWindow::~PPTestWindow() {
  ...
  for( std::vector< Ui::PPResultView* >::iterator it = result_tabs_views.begin(); it != result_tabs_views.end();
       ++it ) {
    delete ( *it )->canvas->scene();
    delete *it;
  };
  for( std::vector< QWidget* >::iterator it = result_tabs_widgets.begin(); it != result_tabs_widgets.end(); ++it )
    delete *it;
...
void PPTestWindow::loadWorldMap() {
  if( robot_running ) {
    QMessageBox::information( this, tr( "Error!" ), tr( "The robot is currently running, please stop it first!" ) );
    return;
  };

  closeTestScenario();

  QString filename = QFileDialog::getOpenFileName( this, tr( "Open a World Map Image..." ), QString(),
  ...
  if( !filename.isNull() ) {
    ...
    if( world_map_cvimage.empty() ) {
      QMessageBox::information( this, tr( "Error!" ), tr( "The image you have selected could not be loaded!" ) );
      return;
    };

    if( !world_map_pixmap ) {
      world_map_pixmap = world_map_scene->addPixmap(
        QPixmap::fromImage( QImage( world_map_cvimage.ptr(), world_map_cvimage.size().width,
                                    world_map_cvimage.size().height, QImage::Format_RGB888 ).rgbSwapped() ) );
    } else {
...
void PPTestWindow::loadPathPlanner() {
  QMessageBox::information( this, tr( "Not supported!" ),
                            tr( "Sorry. For this version of the application, the only planner available is the "
                                "Rapidly-Exploring Random Tree (RRT) and it is automatically loaded by default." ) );
};
...
void PPTestWindow::launchPlayerStage() {
  ...
  if( best_path.empty() ) {
    QMessageBox::information(
      this, tr( "No Solution!" ),
      tr( "The path-planner was either not run yet or has not been able to find a solution!" ) );
    return;
  };

  QString filename = QFileDialog::getOpenFileName( this, tr( "Open Player/Stage Configuration file..." ), QString(),
  ...
  QMessageBox mb( QMessageBox::Information, tr( "About to Start the Robot!" ),
                  tr( "The robot will start when you press OK." ), QMessageBox::Ok | QMessageBox::Cancel, this );
  int res = mb.exec();
...
void PPTestWindow::startPlanner() {
  ...
  if( world_map_cvimage.empty() ) {
    QMessageBox::information( this, tr( "World Map not loaded!" ),
                              tr( "You must first load a world map in order to run the path-planner!" ) );
    return;
  };

  double pix_to_m = rrt_prop.GridWidth->value() / world_map_cvimage.size().width;

  rrt_test_world planner( world_map_cvimage, rrt_prop.MaxEdgeLength->value() / pix_to_m,
                          rrt_prop.RobotRadius->value() / pix_to_m, rrt_prop.MaxNumVert->value(),
                          rrt_prop.MaxNumSol->value(), boost::bind( &PPTestWindow::updateWorldMap, this, _1, _2 ),
  ...
  p_start[1] = world_map_cvimage.size().height
               - ( rrt_prop.RobotStart_Y->value() + rrt_prop.GridCenterY->value() ) / pix_to_m;
  planner.set_start_pos( p_start );
  rrt_test_world::pixel_coord p_goal;
  p_goal[0] = ( rrt_prop.RobotGoal_X->value() + rrt_prop.GridCenterX->value() ) / pix_to_m;
  p_goal[1] = world_map_cvimage.size().height
              - ( rrt_prop.RobotGoal_Y->value() + rrt_prop.GridCenterY->value() ) / pix_to_m;
...
void PPTestWindow::saveResults() {
  if( result_tabs_views.size() == 0 ) {
    QMessageBox::information( this, tr( "No Results to Save!" ), tr( "There are no recorded results to be saved!" ) );
    return;
  };
  QString dir = QFileDialog::getExistingDirectory( this, tr( "Select a directory for the result files..." ) );
  if( dir.isEmpty() )
    return;
  for( unsigned int i = 0; i < result_tabs_pixmaps.size(); ++i ) {
    result_tabs_pixmaps[i]->pixmap().save( QString::fromStdString( dir.toStdString() + "/" + result_names[i] ) );
  };
};
...
void PPTestWindow::updateWorldMap( const cv::Mat& aImage, unsigned int aProgress ) {
  if( !world_map_pixmap ) {
    world_map_pixmap = world_map_scene->addPixmap( QPixmap::fromImage(
      QImage( aImage.ptr(), aImage.size().width, aImage.size().height, QImage::Format_RGB888 ).rgbSwapped() ) );
  } else {
...
void PPTestWindow::showResultMap( const cv::Mat& aImage, unsigned int aProgress, unsigned int aSolutionID,
  ...
  result_tabs_pixmaps.push_back( result_tabs_views.back()->canvas->scene()->addPixmap( QPixmap::fromImage(
    QImage( aImage.ptr(), aImage.size().width, aImage.size().height, QImage::Format_RGB888 ).rgbSwapped() ) ) );
  {
    std::stringstream ss;
...
void PPTestWindow::executePath() {
  ...
  while( ( robot_running ) && ( !pp.GetStall() ) && ( pt_it != best_path.end() ) ) {
    ...
    double pd_y = ( world_map_cvimage.size().height - ( *pt_it )[1] ) * pix_to_m - rrt_prop.GridCenterY->value();
    ...
    if( ( p_x - pd_x ) * ( p_x - pd_x ) + ( p_y - pd_y ) * ( p_y - pd_y ) < 2 * rrt_prop.RobotRadius->value()
                                                                            * rrt_prop.RobotRadius->value() ) {
      pt_it++;
    };
  };
