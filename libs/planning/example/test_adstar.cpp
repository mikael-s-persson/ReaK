...
public:
                                                  ...
                                                  property< vertex_distance_t, double,
                                                            property< vertex_color_t, default_color_type,
                                                                      property< vertex_predecessor_t,
                                                                                adjacency_list_BC_traits< vecBC, vecBC,
                                                                                                          bidirectionalS >::
                                                                                  vertex_descriptor,
                                                                                no_property > > > > > > >
  ...
  typedef adjacency_list_BC< vecBC, vecBC, bidirectionalS, WorldGridVertexProperties, WorldGridEdgeProperties >
    WorldGridType;
...
public:
  VertexType getStartNode() { return goal_pos; };
  ...
  void updatePath() {
    ...
    path.clear();
    std::stringstream ss;
    ss << "test_adstar_results/" << initial_epsilon << "_" << std::setfill( '0' ) << std::setw( 5 ) << current_time
       << "_" << total_distance << ".bmp";
    ...
    for( tie( ui, ui_end ) = vertices( grid ); ui != ui_end; ++ui ) {
      ...
      if( *ui != current_pos ) {
        put( m_heuristic, *ui,
             std::sqrt( double( pos[0] - current_coord[0] ) * double( pos[0] - current_coord[0] )
  ...
  adstar_test_world( FIBITMAP* aWorldMapImage, double aInitialEpsilon )
      : world_map_image( aWorldMapImage ), grid_width( FreeImage_GetWidth( aWorldMapImage ) ),
        grid_height( FreeImage_GetHeight( aWorldMapImage ) ),
        grid( FreeImage_GetHeight( aWorldMapImage ) * FreeImage_GetWidth( aWorldMapImage ) ), current_time( 0 ),
        blocked_periods( 0 ), initial_epsilon( aInitialEpsilon ) {
      ...
      for( int x = 0; x < grid_width; ++x ) {
        ...
        if( ( color_bits[FI_RGBA_RED] == color_bits[FI_RGBA_GREEN] )
          ...
          if( color_bits[FI_RGBA_RED] < 254 )
            ...
            pos[2] = 255;
        } else if( ( color_bits[FI_RGBA_RED] == 0 ) && ( color_bits[FI_RGBA_BLUE] == 255 )
                   && ( color_bits[FI_RGBA_GREEN] == 0 ) ) {
          // this is the start position.
          pos[2] = 255;
          current_pos = current_node;
        } else if( ( color_bits[FI_RGBA_RED] == 0 ) && ( color_bits[FI_RGBA_BLUE] == 0 )
                   && ( color_bits[FI_RGBA_GREEN] == 255 ) ) {
  ...
  double getHeuristicValue( VertexType u ) { return get( m_heuristic, u ); };
  ...
  void run() {
#ifdef TESTING_ASTAR
    current_time = 255;
    astar_search( grid, goal_pos, bind( &adstar_test_world::getHeuristicValue, this, _1 ), default_astar_visitor(),
                  m_pred, m_distance, get( vertex_rhs, grid ), m_weight, double( 0.0 ), m_color, std::less< double >(),
                  std::plus< double >(), std::numeric_limits< double >::infinity(), double( 0.0 ) );
    ...
    while( goal_pos != current_pos ) {
      ...
      ReaK::graph::adstar_search( grid, goal_pos, m_heuristic, visitor( this ), m_pred, m_distance,
                                  get( vertex_rhs, grid ), get( vertex_key, grid ), m_weight, m_color,
                                  initial_epsilon );
      blocked_periods = 0;
    };
#endif
...
int main( int argc, char** argv ) {
  if( argc < 3 ) {
    ...
    std::cout << "Error: Arguments to the program were incorrect!" << std::endl << "Usage:" << std::endl
              << "\t\t./test_adstar [world_image_filename.bmp] [initial_epsilon]" << std::endl;

    return 1;
  };
  std::string filename( argv[1] );
  FIBITMAP* world_image = FreeImage_Load( FIF_BMP, filename.c_str(), BMP_DEFAULT );
