...
public:
  ...
  planning_option_collection()
      : planning_algo( 0 ), max_vertices( 5000 ), prog_interval( 10 ), max_results( 50 ), planning_options( 0 ),
        store_policy( 0 ), knn_method( 0 ), init_SA_temp( 0.0 ), init_relax( 0.0 ), max_random_walk( 1.0 ),
        start_delay( 20.0 ){};
  ...
  *******************************************************************************/
  ...
  virtual void RK_CALL save( serialization::oarchive& A, unsigned int ) const {
    ...
    A& RK_SERIAL_SAVE_WITH_NAME( planning_algo ) & RK_SERIAL_SAVE_WITH_NAME( max_vertices )
      & RK_SERIAL_SAVE_WITH_NAME( prog_interval ) & RK_SERIAL_SAVE_WITH_NAME( max_results )
      & RK_SERIAL_SAVE_WITH_NAME( planning_options ) & RK_SERIAL_SAVE_WITH_NAME( store_policy )
      & RK_SERIAL_SAVE_WITH_NAME( knn_method ) & RK_SERIAL_SAVE_WITH_NAME( init_SA_temp )
      & RK_SERIAL_SAVE_WITH_NAME( init_relax ) & RK_SERIAL_SAVE_WITH_NAME( max_random_walk )
      & RK_SERIAL_SAVE_WITH_NAME( start_delay );
  };

  virtual void RK_CALL load( serialization::iarchive& A, unsigned int ) {
    shared_object::load( A, shared_object::getStaticObjectType()->TypeVersion() );
    A& RK_SERIAL_LOAD_WITH_NAME( planning_algo ) & RK_SERIAL_LOAD_WITH_NAME( max_vertices )
      & RK_SERIAL_LOAD_WITH_NAME( prog_interval ) & RK_SERIAL_LOAD_WITH_NAME( max_results )
      & RK_SERIAL_LOAD_WITH_NAME( planning_options ) & RK_SERIAL_LOAD_WITH_NAME( store_policy )
      & RK_SERIAL_LOAD_WITH_NAME( knn_method ) & RK_SERIAL_LOAD_WITH_NAME( init_SA_temp )
      & RK_SERIAL_LOAD_WITH_NAME( init_relax ) & RK_SERIAL_LOAD_WITH_NAME( max_random_walk )
