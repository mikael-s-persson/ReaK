...
typename boost::enable_if< is_steerable_space< FreeSpaceType >, SolutionRecPtr >::type
  register_basic_solution_path_impl( FreeSpaceType& space, const graph::any_graph& g,
                                     ...
                                     double goal_distance, std::map< double, SolutionRecPtr >& solutions ) {
  ...
  graph::any_graph::property_map_by_ptr< const SteerRecordType > steer_record
    = graph::get_dyn_prop< const SteerRecordType& >( "edge_steer_record", g );

  double solutions_total_dist = goal_distance;

  shared_ptr< SolutionWrapperType > new_sol( new SolutionWrapperType(
    "planning_solution", SolutionPathType( sup_space_ptr, get( distance_metric, *sup_space_ptr ) ) ) );
  SolutionPathType& waypoints = new_sol->get_wrapped_object();

  if( goal_distance > 0.0 ) {
    std::pair< point_type, SteerRecordType > goal_steer_result
      = space.steer_position_toward( position[goal_node], 1.0, goal_pos );
    waypoints.push_front( goal_pos );
    for( SteerIter it = goal_steer_result.second.end_fraction_travel();
         it != goal_steer_result.second.begin_fraction_travel(); it -= 1.0 )
...
template < typename SolutionWrapperType, typename FreeSpaceType, typename SolutionRecPtr >
typename boost::disable_if< is_steerable_space< FreeSpaceType >, SolutionRecPtr >::type
  register_basic_solution_path_impl( FreeSpaceType& space, const graph::any_graph& g,
                                     graph::any_graph::vertex_descriptor start_node,
                                     graph::any_graph::vertex_descriptor goal_node,
                                     const typename topology_traits< FreeSpaceType >::point_type& goal_pos,
...
template < typename SolutionWrapperType, typename FreeSpaceType, typename SolutionRecPtr >
typename boost::enable_if< is_steerable_space< FreeSpaceType >, SolutionRecPtr >::type
  register_optimal_solution_path_impl( FreeSpaceType& space, const graph::any_graph& g,
                                       graph::any_graph::vertex_descriptor start_node,
                                       graph::any_graph::vertex_descriptor goal_node,
                                       const typename topology_traits< FreeSpaceType >::point_type& goal_pos,
                                       double goal_distance, std::map< double, SolutionRecPtr >& solutions ) {
...
template < typename SolutionWrapperType, typename FreeSpaceType, typename SolutionRecPtr >
typename boost::enable_if< is_steerable_space< FreeSpaceType >, SolutionRecPtr >::type
  register_basic_solution_path_impl( FreeSpaceType& space, const graph::any_graph& g1, const graph::any_graph& g2,
                                     graph::any_graph::vertex_descriptor start_node,
                                     graph::any_graph::vertex_descriptor goal_node,
  ...
  graph::any_graph::property_map_by_ptr< const SteerRecordType > steer_record1
  ...
  graph::any_graph::property_map_by_ptr< const SteerRecordType > steer_record2
  ...
  if( joining_distance > 0.0 ) {
    std::pair< point_type, SteerRecordType > join_steer_result
      = space.steer_position_toward( position1[join1_node], 1.0, position2[join2_node] );
    for( SteerIter it = join_steer_result.second.end_fraction_travel();
         it != join_steer_result.second.begin_fraction_travel(); it -= 1.0 )
      waypoints.push_front( point_type( *it ) );
  };
  ...
  while( ( in_degree( join1_node, g1 ) ) && ( !g1.equal_descriptors( join1_node, start_node ) ) ) {
    ...
    for( SteerIter it = sr.end_fraction_travel(); it != sr.begin_fraction_travel(); it -= 1.0 )
      waypoints.push_front( point_type( *it ) );
    solutions_total_dist
      += get( distance_metric, *sup_space_ptr )( position1[u], position1[join1_node], *sup_space_ptr );
    join1_node = u;
    waypoints.push_front( position1[join1_node] );
  };
  ...
  while( ( in_degree( join2_node, g2 ) ) && ( !g2.equal_descriptors( join2_node, goal_node ) ) ) {
    ...
    solutions_total_dist
      += get( distance_metric, *sup_space_ptr )( position2[u], position2[join2_node], *sup_space_ptr );
    join2_node = u;
    waypoints.push_back( position2[join2_node] );
  };
...
typename boost::disable_if< is_steerable_space< FreeSpaceType >, SolutionRecPtr >::type
  ...
  while( in_degree( join1_node, g1 ) && ( !g1.equal_descriptors( join1_node, start_node ) ) ) {
    ...
    solutions_total_dist
      += get( distance_metric, *sup_space_ptr )( position1[v], position1[join1_node], *sup_space_ptr );
  ...
  while( in_degree( join2_node, g2 ) && ( !g2.equal_descriptors( join2_node, goal_node ) ) ) {
    ...
    solutions_total_dist
      += get( distance_metric, *sup_space_ptr )( position2[join2_node], position2[v], *sup_space_ptr );
...
template < typename SolutionWrapperType, typename FreeSpaceType, typename SolutionRecPtr >
typename boost::enable_if< is_steerable_space< FreeSpaceType >, SolutionRecPtr >::type
  register_optimal_solution_path_impl( FreeSpaceType& space, const graph::any_graph& g1, const graph::any_graph& g2,
                                       graph::any_graph::vertex_descriptor start_node,
                                       graph::any_graph::vertex_descriptor goal_node,
                                       graph::any_graph::vertex_descriptor join1_node,
                                       graph::any_graph::vertex_descriptor join2_node, double joining_distance,
