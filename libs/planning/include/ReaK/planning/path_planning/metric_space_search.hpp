...
struct position_caching_policy {
  ...
  struct position_map : boost::data_member_property_map< PointType, vertex_base_property< PointType > > {
    ...
    position_map( KeyMap k = KeyMap(), PositionMap p = PositionMap() )
 ...
 */
template < typename Key, typename Topology, typename PositionMap, unsigned int Arity = 2,
           typename VPChooser = random_vp_chooser, typename TreeStorageTag = boost::bfl_d_ary_tree_storage< Arity >,
...
private:
  ...
  typedef typename PositionCachingPolicy::template position_map< point_type, key_map_type, PositionMap >
    vertex_position_map;
  ...
  typedef dvp_tree_impl< tree_indexer, Topology, vp_to_key_map_type, ep_to_distance_map_type, vp_to_pos_map_type, Arity,
...
public:
  ...
  dvp_tree( const Graph& g, const shared_ptr< const Topology >& aSpace, PositionMap aPosition,
            VPChooser aVPChooser = VPChooser() )
      : m_tree(), m_position( aPosition ),
        m_vp_key( key_map_type( &vertex_properties::k ), get_raw_vertex_to_bundle_map( m_tree ) ),
        m_vp_pos( vertex_position_map( key_map_type( &vertex_properties::k ), aPosition ),
                  get_raw_vertex_to_bundle_map( m_tree ) ),
        m_impl(
          g, aPosition, m_tree, aSpace, m_vp_key,
          ep_to_distance_map_type( distance_map_type( &edge_properties::d ), get_raw_edge_to_bundle_map( m_tree ) ),
          m_vp_pos, aVPChooser ){};
  ...
  dvp_tree( ForwardIterator aBegin, ForwardIterator aEnd, const shared_ptr< const Topology >& aSpace,
      ...
      : m_tree(), m_position( aPosition ),
        ...
        m_vp_pos( vertex_position_map( key_map_type( &vertex_properties::k ), aPosition ),
                  get_raw_vertex_to_bundle_map( m_tree ) ),
        m_impl(
          aBegin, aEnd, aPosition, m_tree, aSpace, m_vp_key,
          ep_to_distance_map_type( distance_map_type( &edge_properties::d ), get_raw_edge_to_bundle_map( m_tree ) ),
          m_vp_pos, aVPChooser ){};

  dvp_tree( const self& rhs )
      : m_tree( rhs.m_tree ), m_position( rhs.m_position ), m_vp_key( rhs.m_vp_key ), m_vp_pos( rhs.m_vp_pos ),
