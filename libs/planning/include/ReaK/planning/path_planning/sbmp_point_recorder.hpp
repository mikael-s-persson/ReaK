 ...
 */
template < typename JointStateSpace, typename JointStateMapping = identity_topo_map,
...
public:
  ...
  explicit sbmp_point_recorder( const shared_ptr< JointStateSpace >& aJointSpace = shared_ptr< JointStateSpace >(),
      ...
      : next_reporter( aNextReporter ), jt_space( aJointSpace ), map_to_jt_space( aMapToJtSpace ),
        interval_size( aIntervalSize ), file_path( aFilePath ), solution_count( 0 ){};
  ...
  *******************************************************************************/
  ...
  virtual void RK_CALL save( serialization::oarchive& A, unsigned int ) const {
    ...
    A& RK_SERIAL_SAVE_WITH_NAME( next_reporter ) & RK_SERIAL_SAVE_WITH_NAME( jt_space )
      & RK_SERIAL_SAVE_WITH_NAME( map_to_jt_space ) & RK_SERIAL_SAVE_WITH_NAME( interval_size )
      & RK_SERIAL_SAVE_WITH_NAME( file_path );
  };

  virtual void RK_CALL load( serialization::iarchive& A, unsigned int ) {
    shared_object::load( A, shared_object::getStaticObjectType()->TypeVersion() );
    A& RK_SERIAL_LOAD_WITH_NAME( next_reporter ) & RK_SERIAL_LOAD_WITH_NAME( jt_space )
      & RK_SERIAL_LOAD_WITH_NAME( map_to_jt_space ) & RK_SERIAL_LOAD_WITH_NAME( interval_size )
...
public:
   ...
   * \param aNextReporter The instance of the SBMP/SBPP reporter to which calls are forwarded to.
   */
  explicit sbmp_point_recorder( const std::string& aFilePath = "", double aIntervalSize = 0.1,
                                NextReporter aNextReporter = NextReporter() )
      : next_reporter( aNextReporter ), interval_size( aIntervalSize ), file_path( aFilePath ), solution_count( 0 ){};
