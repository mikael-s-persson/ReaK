...
struct monte_carlo_mp_engine {
  ...
  monte_carlo_mp_engine( std::size_t aMCRuns, const std::string& aPlannerName, const std::string& aOutputPathStem )
      : mc_run_count( aMCRuns ) {

    boost::filesystem::create_directory( aOutputPathStem.c_str() );
  ...
  shared_ptr< any_sbmp_reporter_chain< Topology > > create_reporter( shared_ptr< Topology > ) {

    shared_ptr< any_sbmp_reporter_chain< Topology > > report_chain( new any_sbmp_reporter_chain< Topology >() );
  ...
  void operator()( const planning_option_collection& plan_options,
                   shared_ptr< sample_based_planner< Topology > > planner, planning_query< Topology >& mc_query ) {
    ...
    for( std::size_t i = 0; i < mc_run_count; ++i ) {
      ...
      while( std::getline( time_ss, tmp ) && ( tmp.size() ) ) {
        ...
        vertex_counts[j] = ( double( v_count ) + double( num_remaining_planners[j] ) * vertex_counts[j] )
                           / double( num_remaining_planners[j] + 1 );
        time_values[j] = ( double( t_val ) + double( num_remaining_planners[j] ) * time_values[j] )
                         / double( num_remaining_planners[j] + 1 );
      ...
      while( std::getline( cost_ss, tmp ) && ( tmp.size() ) ) {
        ...
        if( c_val > worst_costs[j] )
          worst_costs[j] = c_val;
        if( c_val < 1.0e9 ) {
          avg_costs[j] = ( double( c_val ) + double( num_successful_planners[j] ) * avg_costs[j] )
    ...
    for( std::size_t i = 0; i < mc_num_records; ++i ) {
      timing_output << std::setw( 9 ) << i << " " << std::setw( 9 ) << vertex_counts[i] << " " << std::setw( 9 )
                    << num_remaining_planners[i] << " " << std::setw( 9 ) << num_successful_planners[i] << " "
                    << std::setw( 9 ) << time_values[i] << " " << std::setw( 9 ) << best_costs[i] << " "
                    << std::setw( 9 ) << worst_costs[i] << " " << std::setw( 9 ) << avg_costs[i] << std::endl;
    };
...
struct vlist_print_mp_engine {
  ...
  boost::any vlist_report_te;

  std::ofstream cost_out;
  std::ofstream sol_out;

  vlist_print_mp_engine( const std::string& aPlannerName, const std::string& aOutputPathStem )
      : vlist_file_path( aOutputPathStem + "/" + aPlannerName ), vlist_report_te() {

    boost::filesystem::create_directory( aOutputPathStem.c_str() );

    std::string qualified_output_path = aOutputPathStem + "/" + aPlannerName;
    boost::filesystem::create_directory( qualified_output_path.c_str() );
  ...
  shared_ptr< any_sbmp_reporter_chain< Topology > > create_reporter( shared_ptr< Topology > ) {
    ...
    vlist_report_te = boost::any( vlist_sbmp_report< any_mg_vertex_printer< Topology > >(
      vlist_file_path + "_", any_mg_vertex_printer< Topology >( BASIC_MOTION_GRAPH_KIND ) ) );

    report_chain->add_reporter(
      boost::ref( boost::any_cast< vlist_sbmp_report< any_mg_vertex_printer< Topology > >& >( vlist_report_te ) ) );
  ...
  template < typename Topology >
  void operator()( const planning_option_collection& plan_options,
                   shared_ptr< sample_based_planner< Topology > > planner, planning_query< Topology >& pp_query ) {

    boost::any_cast< vlist_sbmp_report< any_mg_vertex_printer< Topology > >& >( vlist_report_te )
...
struct differ_report_mp_engine {
  ...
  differ_report_mp_engine( double segment_steps, const std::string& aPlannerName, const std::string& aOutputPathStem )
...
void execute_p2p_planner( const shared_ptr< Topology >& world_topo, const planning_option_collection& plan_options,
                          std::size_t world_dimensionality, PlanEngine& engine,
                          const typename topology_traits< Topology >::point_type& p_start,
                          const typename topology_traits< Topology >::point_type& p_goal ) {
  ...
  path_planning_p2p_query< Topology > pp_query( "pp_query", world_topo, p_start, p_goal, plan_options.max_results );
  ...
  if( plan_options.planning_algo == 0 ) { // RRT
    ...
    world_planner = shared_ptr< sample_based_planner< Topology > >(
      new rrt_planner< Topology >( world_topo, plan_options.max_vertices, plan_options.prog_interval,
                                   plan_options.store_policy | plan_options.knn_method, plan_options.planning_options,
    ...
    world_planner = shared_ptr< sample_based_planner< Topology > >( new rrtstar_planner< Topology >(
      world_topo, plan_options.max_vertices, plan_options.prog_interval,
      plan_options.store_policy | plan_options.knn_method, plan_options.planning_options, 0.1, 0.05,
    ...
    world_planner = shared_ptr< sample_based_planner< Topology > >(
      new prm_planner< Topology >( world_topo, plan_options.max_vertices, plan_options.prog_interval,
    ...
    shared_ptr< fadprm_planner< Topology > > tmp(
      new fadprm_planner< Topology >( world_topo, plan_options.max_vertices, plan_options.prog_interval,
                                      plan_options.store_policy | plan_options.knn_method, 0.1, 0.05,
                                      plan_options.max_random_walk, world_dimensionality, *p_report_chain ) );
    ...
    shared_ptr< sbastar_planner< Topology > > tmp( new sbastar_planner< Topology >(
      world_topo, plan_options.max_vertices, plan_options.prog_interval,
      plan_options.store_policy | plan_options.knn_method, plan_options.planning_options, 0.1, 0.05,
      plan_options.max_random_walk, world_dimensionality, *p_report_chain ) );
