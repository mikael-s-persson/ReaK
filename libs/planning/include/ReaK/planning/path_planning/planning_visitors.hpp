...
struct planning_visitor_base {
  ...
  void publish_path( Graph& g ) const {
    if( !m_goal_node.empty() ) {
      ...
      dispatched_register_solution( boost::any_cast< Vertex >( m_start_node ), goal_node, goal_node, g, g[goal_node] );
    };
  };
  ...
  ***************************************************/
  ...
  void dispatched_initialize_vertex( bidir_astar_mg_vertex< space_type >& vp, Vertex, Graph& g ) const {
    vp.distance_accum = get( distance_metric, m_query->space->get_super_space() )(
      g[boost::any_cast< Vertex >( m_start_node )].position, vp.position, m_query->space->get_super_space() );
    vp.predecessor = boost::graph_traits< Graph >::null_vertex();
  ...
  ***************************************************/
  ...
  void vertex_added( Vertex u, Graph& g ) const {
    ...
    if( ( ( m_planner->get_planning_method_flags() & PLANNING_DIRECTIONALITY_MASK ) == BIDIRECTIONAL_PLANNING )
        && ( !boost::is_convertible< typename Graph::vertex_bundled*,
                                     optimal_mg_vertex< space_type >* >::type::value ) )
    ...
    if( !m_goal_node.empty() ) {
      ...
      dispatched_register_solution( boost::any_cast< Vertex >( m_start_node ), boost::any_cast< Vertex >( m_goal_node ),
                                    u, g, g[u] );
    };
  };
  ...
  void edge_added( Edge e, Graph& g ) const {
    ...
    if( ( ( ( m_planner->get_planning_method_flags() & PLANNING_DIRECTIONALITY_MASK ) == BIDIRECTIONAL_PLANNING )
          && ( !boost::is_convertible< typename Graph::vertex_bundled*,
                                       optimal_mg_vertex< space_type >* >::type::value ) ) || ( !m_goal_node.empty() ) )
      return; // do not check goal connection for a bi-directional planner (wait for "joining vertex") or a planner that
    ...
    if( goal_dist < std::numeric_limits< double >::infinity() ) {
      solution_record_ptr srp
        = m_query->register_solution( boost::any_cast< Vertex >( m_start_node ), target( e, g ), goal_dist, g );
      if( srp )
        m_planner->report_solution( srp );
    };
  };

  bool keep_going() const { return m_planner->keep_going() && m_query->keep_going(); };
  ...
  ***************************************************/
  ...
  void joining_vertex_found( Vertex u1, Vertex u2, Graph& g1, Graph& g2 ) const {
    double join_dist = get( distance_metric, m_query->space->get_super_space() )( g1[u1].position, g2[u2].position,
                                                                                  m_query->space->get_super_space() );
    solution_record_ptr srp = m_query->register_joining_point(
      boost::any_cast< Vertex >( m_start_node ), boost::any_cast< Vertex >( m_goal_node ), u1, u2, join_dist, g1, g2 );
    if( srp )
      m_planner->report_solution( srp );
  };
  ...
  */
  bool is_position_free( const point_type& p ) const { return m_query->space->is_free( p ); };
  ...
  ***************************************************/
  ...
  typename boost::enable_if< is_steerable_space< SwitchFreeSpace >, double >::type
    dispatched_steer_towards_position( const SwitchFreeSpace& space, const point_type& p_src, const point_type& p_dest,
  ...
  template < typename SwitchFreeSpace >
  typename boost::disable_if< is_steerable_space< SwitchFreeSpace >, double >::type
    dispatched_steer_towards_position( const SwitchFreeSpace& space, const point_type& p_src, const point_type& p_dest,
                                       point_type& p_result, double fraction,
                                       optimal_mg_edge< space_type >& ep_result ) const {
  ...
  typename boost::disable_if< is_steerable_space< SwitchFreeSpace >, double >::type
    dispatched_steer_towards_position( const SwitchFreeSpace& space, const point_type& p_src, const point_type& p_dest,
                                       point_type& p_result, double fraction, mg_edge_data< space_type >& ) const {
    p_result = space.move_position_toward( p_src, fraction, p_dest );
    return get( distance_metric, space.get_super_space() )( p_src, p_result, space.get_super_space() );
  };
  ...
  template < typename SwitchFreeSpace >
  typename boost::enable_if< is_steerable_space< SwitchFreeSpace >, double >::type
    dispatched_steer_back_to_position( const SwitchFreeSpace& space, const point_type& p_src, const point_type& p_dest,
                                       point_type& p_result, double fraction,
                                       optimal_mg_edge< space_type >& ep_result ) const {
  ...
  ***************************************************/
  ...
  template < typename Vertex, typename Graph >
  boost::tuple< point_type, bool, typename Graph::edge_bundled > steer_towards_position( const point_type& p, Vertex u,
                                                                                         Graph& g ) const {
    typedef typename Graph::edge_bundled EdgeProp;
    typedef boost::tuple< point_type, bool, EdgeProp > ResultType;
    ResultType result;
    double traveled_dist = dispatched_steer_towards_position( *( m_query->space ), g[u].position, p, get< 0 >( result ),
                                                              1.0, get< 2 >( result ) );
    double best_case_dist = get( distance_metric, m_query->space->get_super_space() )(
      g[u].position, p, m_query->space->get_super_space() );
    get< 1 >( result ) = ( !std::isinf( traveled_dist ) ) && ( traveled_dist < 2.0 * best_case_dist )
  ...
  ***************************************************/
  ...
  template < typename Vertex, typename Graph >
  boost::tuple< point_type, bool, typename Graph::edge_bundled > steer_back_to_position( const point_type& p, Vertex u,
                                                                                         Graph& g ) const {
    typedef typename Graph::edge_bundled EdgeProp;
    typedef boost::tuple< point_type, bool, EdgeProp > ResultType;
    ResultType result;
    double traveled_dist = dispatched_steer_back_to_position( *( m_query->space ), p, g[u].position, get< 0 >( result ),
                                                              1.0, get< 2 >( result ) );
    double best_case_dist = get( distance_metric, m_query->space->get_super_space() )(
  ...
  ***************************************************/
  ...
  std::pair< bool, typename Graph::edge_bundled > can_be_connected( Vertex u, Vertex v, const Graph& g ) const {
    ...
    double traveled_dist = dispatched_steer_towards_position( *( m_query->space ), g[u].position, g[v].position,
                                                              p_result, 1.0, result.second );
    double remaining_dist = get( distance_metric, m_query->space->get_super_space() )(
  ...
  boost::tuple< point_type, bool, typename Graph::edge_bundled > random_walk( Vertex u, Graph& g ) const {
    ...
    do {
      ...
      double traveled_dist = dispatched_steer_towards_position(
        *( m_query->space ), g[u].position, p_rnd, get< 0 >( result ), target_dist / dist, get< 2 >( result ) );
      if( ( !std::isinf( traveled_dist ) )
          && ( traveled_dist > m_planner->get_steer_progress_tolerance() * target_dist ) ) {
        get< 1 >( result ) = true;
  ...
  boost::tuple< point_type, bool, typename Graph::edge_bundled > random_back_walk( Vertex u, Graph& g ) const {
    ...
    do {
      ...
      double traveled_dist = dispatched_steer_back_to_position(
        *( m_query->space ), p_rnd, g[u].position, get< 0 >( result ), target_dist / dist, get< 2 >( result ) );
      if( ( !std::isinf( traveled_dist ) )
          && ( traveled_dist > m_planner->get_steer_progress_tolerance() * target_dist ) ) {
        get< 1 >( result ) = true;
