...
struct rrtstar_bundle_factory {
  ...
  typedef typename boost::mpl::if_< is_bidir, bidir_optimal_mg_vertex< FreeSpaceType >,
  ...
  typedef typename boost::mpl::if_< is_bidir, boost::data_member_property_map< double, vertex_prop >,
  ...
  static ReaK::graph::rrtstar_bundle< MotionGraphType, super_space_type, visitor_type, NcSelector, position_map,
    ...
    return ReaK::graph::make_rrtstar_bundle(
      ...
      *s_ptr, vis, nc_selector, position_map( &vertex_prop::position ), weight_map( &edge_prop::weight ),
      distance_map( &vertex_prop::distance_accum ), predecessor_map( &vertex_prop::predecessor ),
      dispatched_make_fwd_dist_map< vertex_prop >(), dispatched_make_succ_map< MotionGraphType, vertex_prop >() );
  };
  ...
  static typename boost::disable_if< CallBidir >::type
    ...
    if( method_flags & USE_BRANCH_AND_BOUND_PRUNING_FLAG ) {
      ReaK::graph::generate_bnb_rrt_star( make_bundle( motion_graph, vis, nc_selector, s_ptr ),
                                          get( random_sampler, *s_ptr ) );
    } else { /* assume nominal method only. */
      ReaK::graph::generate_rrt_star( make_bundle( motion_graph, vis, nc_selector, s_ptr ),
                                      get( random_sampler, *s_ptr ) );
    };
  };
  ...
  static typename boost::enable_if< CallBidir >::type
    ...
    if( method_flags & USE_BRANCH_AND_BOUND_PRUNING_FLAG ) {
      ReaK::graph::generate_bnb_rrt_star_bidir( make_bundle( motion_graph, vis, nc_selector, s_ptr ),
                                                get( random_sampler, *s_ptr ) );
    } else { /* assume nominal method only. */
      ReaK::graph::generate_rrt_star_bidir( make_bundle( motion_graph, vis, nc_selector, s_ptr ),
                                            get( random_sampler, *s_ptr ) );
    };
  };
...
#define RK_RRTSTAR_PLANNER_SETUP_ALT_SUPPORT_STRUCTURES                                               \
  ...
  if( ( this->m_data_structure_flags & MOTION_GRAPH_STORAGE_MASK ) == ADJ_LIST_MOTION_GRAPH ) {
    ...
    if( ( this->m_data_structure_flags & KNN_METHOD_MASK ) == LINEAR_SEARCH_KNN ) {
      ...
      RRTStarFactory::make_call_to_planner( motion_graph, vis, nc_selector, sup_space_ptr,
