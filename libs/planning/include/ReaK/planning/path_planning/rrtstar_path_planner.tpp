...
struct rrtstar_bundle_factory {
  ...
  using fwd_distance_map = std::conditional_t<is_bidir,
                                              boost::data_member_property_map<double, vertex_prop>,
                                              ReaK::graph::detail::infinite_double_value_prop_map>;
  ...
  struct ls_motion_graph {
    using type = boost::adjacency_list_BC<boost::vecBC, boost::poolBC, directionality_tag, vertex_prop, edge_prop>;
  ...
  struct dvp_motion_graph {
    ...
    using graph_position_map = typename boost::property_map<type, point_type basic_vertex_prop::*>::type;
    using space_part_type = dvp_tree<vertex_type, super_space_type, graph_position_map, Arity, random_vp_chooser, TreeStorageTag>;
...
#define RK_RRTSTAR_PLANNER_SETUP_ALT_SUPPORT_STRUCTURES                        \
  ...
  if ((this->m_data_structure_flags & MOTION_GRAPH_STORAGE_MASK) ==
    ...
    if ((this->m_data_structure_flags & KNN_METHOD_MASK) == LINEAR_SEARCH_KNN) {
      ...
      using MGFactory = typename RRTStarFactory::template dvp_motion_graph<2, boost::bfl_d_ary_tree_storage<2>>;
      ...
      using MGFactory = typename RRTStarFactory::template dvp_motion_graph<4, boost::bfl_d_ary_tree_storage<4>>;
      ...
      using MGFactory = typename RRTStarFactory::template dvp_motion_graph<2, boost::vebl_d_ary_tree_storage<2>>;
      ...
      using MGFactory = typename RRTStarFactory::template dvp_motion_graph<4, boost::vebl_d_ary_tree_storage<4>>;
    ...
    if ((this->m_data_structure_flags & KNN_METHOD_MASK) == DVP_BF2_TREE_KNN) {
      ...
      using MGFactory = typename RRTStarFactory::template alt_motion_graph<2, boost::bfl_d_ary_tree_storage<2>>;
      ...
      using MGFactory = typename RRTStarFactory::template alt_motion_graph<4, boost::bfl_d_ary_tree_storage<4>>;
      ...
      using MGFactory = typename RRTStarFactory::template alt_motion_graph<2, boost::vebl_d_ary_tree_storage<2>>;
      ...
      using MGFactory = typename RRTStarFactory::template alt_motion_graph<4, boost::vebl_d_ary_tree_storage<4>>;
