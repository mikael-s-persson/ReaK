...
struct sbastar_bundle_factory {
  ...
  typedef typename boost::mpl::if_< is_bidir, recursive_dense_mg_vertex< bidir_astar_mg_vertex< FreeSpaceType > >,
  ...
  struct successor_map {
    typedef typename boost::mpl::if_< is_bidir, boost::data_member_property_map< std::size_t, vertex_prop >,
  ...
  struct ls_motion_graph {
    typedef boost::adjacency_list_BC< boost::vecBC, boost::poolBC, directionality_tag, vertex_prop, edge_prop > type;
    ...
    static nn_synchro_type get_nn_synchro( nn_finder_type& ) { return nn_synchro_type(); };
  ...
  struct dvp_motion_graph {
    ...
    typedef dvp_tree< vertex_type, super_space_type, graph_position_map, Arity, random_vp_chooser, TreeStorageTag >
      space_part_type;
    ...
    static nn_synchro_type get_nn_synchro( nn_finder_type& nn_finder ) { return nn_synchro_type( nn_finder ); };
  ...
  struct alt_motion_graph {
    typedef dvp_adjacency_list< vertex_prop, edge_prop, super_space_type, position_map, Arity, random_vp_chooser,
                                TreeStorageTag, boost::vecBC, directionality_tag, boost::listBC > alt_graph_type;
    ...
    static type get_motion_graph( space_part_type& space_part ) { return space_part.get_adjacency_list(); };
  };

#endif

  template < typename MotionGraphType >
  static void init_motion_graph( MotionGraphType& motion_graph, visitor_type& vis,
    ...
    path_planning_p2p_query< FreeSpaceType >* p2p_query_ptr
      = reinterpret_cast< path_planning_p2p_query< FreeSpaceType >* >(
        query.castTo( path_planning_p2p_query< FreeSpaceType >::getStaticObjectType() ) );
    if( p2p_query_ptr ) {
      vertex_prop vp_goal;
  ...
  static typename boost::disable_if< boost::is_convertible< VertexProp*, bidir_optimal_mg_vertex< FreeSpaceType >* >,
                                     fwd_distance_map >::type
    dispatched_make_fwd_dist_map() {
    return fwd_distance_map( &VertexProp::heuristic_value );
  };

  template < typename VertexProp >
  static typename boost::enable_if< boost::is_convertible< VertexProp*, bidir_optimal_mg_vertex< FreeSpaceType >* >,
                                    fwd_distance_map >::type
    dispatched_make_fwd_dist_map() {
  ...
  static typename boost::disable_if< boost::is_convertible< VertexProp*, bidir_optimal_mg_vertex< FreeSpaceType >* >,
                                     typename successor_map< MotionGraphType >::type >::type
    dispatched_make_succ_map() {
  ...
  static ReaK::graph::sbastar_bundle< MotionGraphType, super_space_type, visitor_type, NcSelector,
                                      ...
                                      typename successor_map< MotionGraphType >::type >
    make_bundle( MotionGraphType& motion_graph, visitor_type& vis, NcSelector nc_selector,
    ...
    return ReaK::graph::make_sbastar_bundle(
      ...
      ( vis.m_goal_node.empty() ? boost::graph_traits< MotionGraphType >::null_vertex()
                                : boost::any_cast< Vertex >( vis.m_goal_node ) ),
      *s_ptr, vis, nc_selector, get( &vertex_prop::key_value, motion_graph ), position_map( &vertex_prop::position ),
      weight_map( &edge_prop::weight ), density_map( &vertex_prop::density ),
      constriction_map( &vertex_prop::constriction ), distance_map( &vertex_prop::distance_accum ),
      predecessor_map( &vertex_prop::predecessor ), dispatched_make_fwd_dist_map< vertex_prop >(),
  ...
  static typename boost::disable_if< CallBidir >::type
    ...
    if( ( ( method_flags & ADDITIONAL_PLANNING_BIAS_MASK ) & PLAN_WITH_ANYTIME_HEURISTIC ) && ( init_relax > 1e-6 ) ) {
      if( ( method_flags & COLLISION_CHECKING_POLICY_MASK ) == EAGER_COLLISION_CHECKING ) {
        if( ( method_flags & ADDITIONAL_PLANNING_BIAS_MASK ) & PLAN_WITH_VORONOI_PULL ) {
          generate_anytime_sbarrtstar( make_bundle( motion_graph, vis, nc_selector, s_ptr ),
        ...
        if( ( method_flags & ADDITIONAL_PLANNING_BIAS_MASK ) & PLAN_WITH_VORONOI_PULL ) {
          if( method_flags & USE_BRANCH_AND_BOUND_PRUNING_FLAG ) {
            generate_anytime_lazy_bnb_sbarrtstar( make_bundle( motion_graph, vis, nc_selector, s_ptr ),
                                                  get( random_sampler, *s_ptr ), init_relax, init_temp );
          } else { /* assume nominal method only. */
            generate_anytime_lazy_sbarrtstar( make_bundle( motion_graph, vis, nc_selector, s_ptr ),
                                              get( random_sampler, *s_ptr ), init_relax, init_temp );
          };
        } else { /* assume nominal method only. */
          if( method_flags & USE_BRANCH_AND_BOUND_PRUNING_FLAG ) {
      ...
      if( ( method_flags & COLLISION_CHECKING_POLICY_MASK ) == EAGER_COLLISION_CHECKING ) {
        if( ( method_flags & ADDITIONAL_PLANNING_BIAS_MASK ) & PLAN_WITH_VORONOI_PULL ) {
          generate_sbarrtstar( make_bundle( motion_graph, vis, nc_selector, s_ptr ), get( random_sampler, *s_ptr ),
                               init_temp );
        } else { /* assume nominal method only. */
        ...
        if( ( method_flags & ADDITIONAL_PLANNING_BIAS_MASK ) & PLAN_WITH_VORONOI_PULL ) {
          if( method_flags & USE_BRANCH_AND_BOUND_PRUNING_FLAG ) {
            generate_lazy_bnb_sbarrtstar( make_bundle( motion_graph, vis, nc_selector, s_ptr ),
                                          get( random_sampler, *s_ptr ), init_temp );
          } else { /* assume nominal method only. */
            generate_lazy_sbarrtstar( make_bundle( motion_graph, vis, nc_selector, s_ptr ),
                                      get( random_sampler, *s_ptr ), init_temp );
          };
        } else { /* assume nominal method only. */
          if( method_flags & USE_BRANCH_AND_BOUND_PRUNING_FLAG ) {
  ...
  template < typename CallBidir, typename MotionGraphType, typename NcSelector >
  static typename boost::enable_if< CallBidir >::type
    dispatched_make_call_to_planner( MotionGraphType& motion_graph, visitor_type& vis, NcSelector nc_selector,
                                     shared_ptr< const super_space_type > s_ptr, std::size_t method_flags,
                                     double init_relax, double init_temp ) {
    using namespace graph;
    ...
    if( ( ( method_flags & ADDITIONAL_PLANNING_BIAS_MASK ) & PLAN_WITH_ANYTIME_HEURISTIC ) && ( init_relax > 1e-6 ) ) {
      if( ( method_flags & COLLISION_CHECKING_POLICY_MASK ) == EAGER_COLLISION_CHECKING ) {
        if( ( method_flags & ADDITIONAL_PLANNING_BIAS_MASK ) & PLAN_WITH_VORONOI_PULL ) {
          generate_anytime_sbarrtstar_bidir( make_bundle( motion_graph, vis, nc_selector, s_ptr ),
                                             get( random_sampler, *s_ptr ), init_relax, init_temp );
        } else { /* assume nominal method only. */
        ...
        if( ( method_flags & ADDITIONAL_PLANNING_BIAS_MASK ) & PLAN_WITH_VORONOI_PULL ) {
          if( method_flags & USE_BRANCH_AND_BOUND_PRUNING_FLAG ) {
            generate_anytime_lazy_bnb_sbarrtstar_bidir( make_bundle( motion_graph, vis, nc_selector, s_ptr ),
                                                        get( random_sampler, *s_ptr ), init_relax, init_temp );
          } else { /* assume nominal method only. */
            generate_anytime_lazy_sbarrtstar_bidir( make_bundle( motion_graph, vis, nc_selector, s_ptr ),
                                                    get( random_sampler, *s_ptr ), init_relax, init_temp );
          };
        } else { /* assume nominal method only. */
          if( method_flags & USE_BRANCH_AND_BOUND_PRUNING_FLAG ) {
      ...
      if( ( method_flags & COLLISION_CHECKING_POLICY_MASK ) == EAGER_COLLISION_CHECKING ) {
        if( ( method_flags & ADDITIONAL_PLANNING_BIAS_MASK ) & PLAN_WITH_VORONOI_PULL ) {
          generate_sbarrtstar_bidir( make_bundle( motion_graph, vis, nc_selector, s_ptr ),
                                     get( random_sampler, *s_ptr ), init_temp );
        } else { /* assume nominal method only. */
        ...
        if( ( method_flags & ADDITIONAL_PLANNING_BIAS_MASK ) & PLAN_WITH_VORONOI_PULL ) {
          if( method_flags & USE_BRANCH_AND_BOUND_PRUNING_FLAG ) {
            generate_lazy_bnb_sbarrtstar_bidir( make_bundle( motion_graph, vis, nc_selector, s_ptr ),
                                                get( random_sampler, *s_ptr ), init_temp );
          } else { /* assume nominal method only. */
            generate_lazy_sbarrtstar_bidir( make_bundle( motion_graph, vis, nc_selector, s_ptr ),
                                            get( random_sampler, *s_ptr ), init_temp );
          };
        } else { /* assume nominal method only. */
          if( method_flags & USE_BRANCH_AND_BOUND_PRUNING_FLAG ) {
  ...
  template < typename MotionGraphType, typename NcSelector >
  static void make_call_to_planner( MotionGraphType& motion_graph, visitor_type& vis, NcSelector nc_selector,
                                    shared_ptr< const super_space_type > s_ptr, std::size_t method_flags,
                                    double init_relax, double init_temp ) {
    dispatched_make_call_to_planner< is_bidir >( motion_graph, vis, nc_selector, s_ptr, method_flags, init_relax,
                                                 init_temp );
  };
};
