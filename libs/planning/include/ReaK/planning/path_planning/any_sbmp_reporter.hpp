...
public:
  ...
  typedef typename boost::mpl::if_< is_temporal_space< FreeSpaceType >, seq_trajectory_base< super_space_type >,
...
public:
  virtual void draw_any_motion_graph( const FreeSpaceType& space, const graph::any_graph& g ) const {
    ...
    reporter.draw_motion_graph( space, g,
                                graph::get_dyn_prop< const typename PropType::type& >( PropType::name(), g ) );
  };
...
public:
  virtual void reset_internal_state() { reporter.reset_internal_state(); };
...
public:
  ...
  void add_reporter( const boost::reference_wrapper< Reporter >& aReporter ) {
    reporters.push_back(
      pointer_type( new type_erased_sbmp_reporter< FreeSpaceType, Reporter* >( aReporter.get_pointer() ) ) );
  };
