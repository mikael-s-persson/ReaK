...
struct density_plan_visitor
  : planning_visitor_base< density_plan_visitor< FreeSpaceType, DensityCalc >, FreeSpaceType > {
  ...
  ***************************************************/
  ...
  void init_nonrecursive_density( Vertex u, Graph& g ) const {
    m_density_calc.update_density( u, g, *( this->m_query->space ), this->m_planner->get_sampling_radius(),
  ...
  bool dispatched_heuristic_potential( const mg_vertex_data< space_type >& ) const { return true; };
  bool dispatched_heuristic_potential( const astar_mg_vertex< space_type >& vp,
                                       const astar_mg_vertex< space_type >& sp ) const {
    return ( vp.heuristic_value > std::numeric_limits< double >::epsilon() * sp.heuristic_value );
  };
  bool dispatched_heuristic_potential( const bidir_astar_mg_vertex< space_type >& vp,
  ...
  bool has_search_potential( Vertex u, const Graph& g ) const {
    if( this->m_goal_node.empty() )
      ...
      return ( u != boost::any_cast< Vertex >( this->m_goal_node ) ) && dispatched_density_cutoff_test( g[u] );
  };
  template < typename Vertex, typename Graph >
  ...
  ***************************************************/
  ...
  void dispatched_travel_succeeded( recursive_dense_mg_vertex< BaseType >&, Vertex u, Vertex v, Graph& g ) const {
    m_density_calc.travel_succeeded( u, v, g, *( this->m_query->space ), this->m_planner->get_sampling_radius(),
                                     this->m_planner->get_space_dimensionality() );
  };
  ...
  void dispatched_travel_explored( recursive_dense_mg_vertex< BaseType >&, Vertex u, Vertex v, Graph& g ) const {
    m_density_calc.travel_explored( u, v, g, *( this->m_query->space ), this->m_planner->get_sampling_radius(),
                                    this->m_planner->get_space_dimensionality() );
  };
  ...
  void dispatched_travel_failed( recursive_dense_mg_vertex< BaseType >&, Vertex u, Vertex v, Graph& g ) const {
    m_density_calc.travel_failed( u, v, g, *( this->m_query->space ), this->m_planner->get_sampling_radius(),
                                  this->m_planner->get_space_dimensionality() );
  };
