...
public:
  ...
  RandomAccessIter operator()( RandomAccessIter aBegin, RandomAccessIter aEnd, DistanceMetric aDistance,
 ...
 */
template < typename TreeType, typename Topology, typename VertexKeyMap, typename DistanceMap, typename PositionMap,
...
public:
  ...
  struct dist_metric_only_impl {
    ...
    double distance( const point_type& a, const point_type& b ) const { return m_distance( a, b, *p_space ); };
    double proper_distance( const point_type& a, const point_type& b ) const { return m_distance( a, b, *p_space ); };
    double operator()( const point_type& a, const point_type& b ) const { return proper_distance( a, b, *p_space ); };

    dist_metric_only_impl( const shared_ptr< const Topology >& aSpace )
  ...
  struct dist_metric_pair_impl {
    ...
    dist_metric_pair_impl( const shared_ptr< const Topology >& aSpace )
        : p_space( aSpace ), m_distance( get( distance_metric, *aSpace ) ),
          m_proper_distance( get( proper_metric, *aSpace ) ){};
  };

  typedef typename boost::mpl::if_< boost::is_same< distance_metric_type, proper_metric_type >, dist_metric_only_impl,
                                    dist_metric_pair_impl >::type parting_metrics_type;
...
private:
  ...
  struct is_vertex_prop_valid {
    ...
    VertexKeyMap m_key;
    is_vertex_prop_valid( temporary_invalid_key_set_type* invalid_keys, const VertexKeyMap& key )
        : m_invalid_keys( invalid_keys ), m_key( key ){};

    bool operator()( const vertex_property& vp ) const { return ( m_invalid_keys->count( get( m_key, vp ) ) == 0 ); };
  };
  ...
  void construct_node( vertex_type aNode, prop_vector_iter aBegin, prop_vector_iter aEnd ) {
    ...
    while( !tasks.empty() ) {
      ...
      for( std::size_t i = 0; ( i < Arity ) && ( child_count[i] > 0 ); ++i ) {
        std::nth_element( cur_task.first, cur_task.first + ( child_count[i] - 1 ), cur_task.last,
                          closer( &dist_map, m_key ) );
  ...
  struct nearest_search_result_set {
    ...
    void register_vantage_point( const point_type&, const point_type&, distance_type current_dist,
                                 vertex_type current_vp, const parting_metrics_type& ) {
  ...
  struct pred_succ_search_result_set {
    ...
    void register_vantage_point( const point_type& query_point, const point_type& current_vp_point,
                                 distance_type current_dist, vertex_type current_vp,
  ...
  void find_nearest_impl( const point_type& aPoint, SearchResultSet& aResult ) const {
    ...
    while( !tasks.empty() ) {
      std::pair< vertex_type, distance_type > cur_node = tasks.top();
      tasks.pop();
      ...
      while( true ) {
        if( left_stopped ) {
          out_edge_iter ei_rightleft = ei_right;
          --ei_rightleft;
          distance_type temp_dist = 0.0;
          while( ( ei_right != ei_end ) && ( ( temp_dist = get( m_mu, get_raw_edge_property( *m_tree, *ei_rightleft ) )
          ...
          distance_type temp_dist = 0.0;
          while( ( ei_left != ei )
                 && ( ( temp_dist = current_dist - get( m_mu, get_raw_edge_property( *m_tree, *( --ei_leftleft ) ) ) )
                      < aResult.Radius ) ) {
          ...
          out_edge_iter ei_leftleft = ei_left;
          --ei_leftleft;
          distance_type d1 = get(
            m_mu, get_raw_edge_property( *m_tree, *ei_leftleft ) ); // greater than 0 if ei_leftleft should be searched.
          out_edge_iter ei_rightleft = ei_right;
          --ei_rightleft;
          distance_type d2 = get(
            m_mu, get_raw_edge_property( *m_tree, *ei_rightleft ) ); // less than 0 if ei_right should be searched.
          if( d1 + d2 > 2.0 * current_dist ) { // this means that ei_leftleft's boundary is closer to aPoint.
            if( d1 + aResult.Radius - current_dist > 0 ) {
              temp_invtasks.push(
                std::pair< vertex_type, distance_type >( target( *ei_leftleft, *m_tree ), current_dist - d1 ) );
              ei_left = ei_leftleft;
              if( d2 - aResult.Radius - current_dist < 0 ) {
                temp_invtasks.push(
                  std::pair< vertex_type, distance_type >( target( *ei_right, *m_tree ), d2 - current_dist ) );
                ++ei_right;
              } else
                right_stopped = true;
            } else
  ...
  vertex_type get_leaf( const point_type& aPoint, vertex_type aNode ) const {
    while( out_degree( aNode, *m_tree ) != 0 ) {
      ...
      distance_type current_dist
        = m_distance.proper_distance( aPoint, get( m_position, get_raw_vertex_property( *m_tree, aNode ) ) );
  ...
  vertex_type get_vertex( key_type aKey, const point_type& aPoint, vertex_type aNode ) const {
    ...
    while( get( m_key, get_raw_vertex_property( *m_tree, aNode ) ) != aKey ) {
      ...
      for( boost::tie( ei, ei_end ) = out_edges( aNode, *m_tree ); ei != ei_end; ++ei ) {
        result = target( *ei, *m_tree );
        //           std::cout << " -- Looking at child " << std::setw(6) << get(m_key, get_raw_vertex_property(*m_tree,
        //           result))
  ...
  void update_mu_upwards( const point_type& aPoint, vertex_type aNode ) {
    while( aNode != m_root ) {
      ...
      distance_type dist
        = m_distance.proper_distance( aPoint, get( m_position, get_raw_vertex_property( *m_tree, parent ) ) );
...
public:
  ...
  dvp_tree_impl( const Graph& g, GraphPositionMap aGraphPosition, tree_indexer& aTree,
                 const shared_ptr< const Topology >& aSpace, VertexKeyMap aKey, DistanceMap aMu, PositionMap aPosition,
                 VPChooser aVPChooser )
      : m_tree( &aTree ), m_root( boost::graph_traits< tree_indexer >::null_vertex() ), m_key( aKey ), m_mu( aMu ),
        m_position( aPosition ), m_distance( aSpace ), m_vp_chooser( aVPChooser ) {
  ...
  dvp_tree_impl( ForwardIterator aBegin, ForwardIterator aEnd, ElemPositionMap aElemPosition, tree_indexer& aTree,
                 const shared_ptr< const Topology >& aSpace, VertexKeyMap aKey, DistanceMap aMu, PositionMap aPosition,
                 VPChooser aVPChooser )
      : m_tree( &aTree ), m_root( boost::graph_traits< tree_indexer >::null_vertex() ), m_key( aKey ), m_mu( aMu ),
        m_position( aPosition ), m_distance( aSpace ), m_vp_chooser( aVPChooser ) {
   ...
   * \param aVPChooser The vantage-point chooser functor (policy class).
   */
  dvp_tree_impl( tree_indexer& aTree, const shared_ptr< const Topology >& aSpace, VertexKeyMap aKey, DistanceMap aMu,
                 PositionMap aPosition, VPChooser aVPChooser )
      : m_tree( &aTree ), m_root( boost::graph_traits< tree_indexer >::null_vertex() ), m_key( aKey ), m_mu( aMu ),
        m_position( aPosition ), m_distance( aSpace ), m_vp_chooser( aVPChooser ){};
  ...
  dvp_tree_impl( tree_indexer& aTree, const self& rhs )
      : m_tree( &aTree ), m_root( get_root_vertex( aTree ) ), m_key( rhs.m_key ), m_mu( rhs.m_mu ),
        m_position( rhs.m_position ), m_distance( rhs.m_distance ), m_vp_chooser( rhs.m_vp_chooser ){};
  ...
  void insert( ForwardIterator aBegin, ForwardIterator aEnd ) {
    for( ; aBegin != aEnd; ++aBegin )
      insert( *aBegin );
    // TODO: There's got to be a better way to insert many elements (most likely a similar strategy to the erase
    // multiple function).
  };
  ...
  void erase( ForwardIterator aBegin, ForwardIterator aEnd ) {
    ...
    for( ForwardIterator first = aBegin; first != aEnd; ++first ) {
      ...
      for( typename vertex_listing::iterator it = v_lists.begin(); it != v_lists.end(); ++it ) {
        ...
        typename std::vector< vertex_type >::iterator it_key
          = std::binary_search( it->second.begin(), it->second.end(), removal_trunk );
      ...
      if( !already_collected ) {
        ...
        for( typename vertex_listing::iterator it = v_lists.begin(); it != v_lists.end(); ) {
          ...
          typename std::vector< vertex_type >::iterator it_key
            = std::binary_search( removal_list.begin(), removal_list.end(), it->first );
    ...
    if( ( v_lists.size() == 1 ) && ( v_lists.front() == m_root ) ) {
      ...
      std::vector< vertex_property > prop_list;
      remove_branch( m_root, back_inserter( prop_list ), *m_tree );
      prop_list.erase( remove_if( prop_list.begin(), prop_list.end(), is_vertex_prop_valid( &invalid_keys, m_key ) ),
                       prop_list.end() );
      prop_vector_iter v_first = prop_list.begin();
      prop_vector_iter v_last = prop_list.end();
      rearrange_with_chosen_vp( v_first, v_last );
  ...
  OutputIterator find_nearest( const point_type& aPoint, OutputIterator aOutputBegin, std::size_t K,
    ...
    for( typename priority_queue_type::const_iterator it = result_set.Neighbors.begin();
         it != result_set.Neighbors.end(); ++it )
  ...
  std::pair< OutputIterator, OutputIterator >
    find_nearest( const point_type& aPoint, OutputIterator aPredBegin, OutputIterator aSuccBegin, std::size_t K,
                  distance_type R = std::numeric_limits< distance_type >::infinity() ) const {
    if( num_vertices( *m_tree ) == 0 )
   ...
   */
  template < typename OutputIterator >
  std::pair< OutputIterator, OutputIterator > find_in_range( const point_type& aPoint, OutputIterator aPredBegin,
  ...
  struct mutation_visitor {
    ...
    void remove_vertex( vertex_type v, tree_indexer& ) const { m_parent->erase( v ); };
