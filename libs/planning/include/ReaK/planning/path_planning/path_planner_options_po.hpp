...
boost::program_options::options_description get_planning_option_po_desc() {
  ...
  planner_alg_options.add_options()(
      "planner-options", po::value<std::string>(),
       ...
       "prm, sba_star, fadprm).")
          ...
          ("max-vertices", po::value<std::size_t>()->default_value(5000),
           "maximum number of vertices during runs (default is 5000)")(
              ...
              "number of vertices between progress reports during runs "
              "(default is 10)")

              ("max-random-walk", po::value<double>()->default_value(1.0),
               "specify the maximum random-walk distance allowed in the "
               "algorithm (default: 1.0). Only meaningful for expanding "
               "algorithms (SBA*, FADPRM, PRM).")

                  ("no-lazy-connect",
                   "if set, disable lazy connection strategy during planning.")(
                      "bi-directional",
                      "specify whether to use a bi-directional algorithm or "
                      "not during planning. Only supported for "
                      "some algorithms (RRT, RRT*, SBA*).")(
                      "with-bnb",
                      "specify whether to use a Branch-and-bound or not during "
                      "planning to prune useless nodes from the "
                      "motion-graph. Only supported for optimizing algorithms "
                      "(RRT*, SBA*).")(
                      "relaxation-factor",
                      po::value<double>()->default_value(0.0),
                      "specify the initial relaxation factor for the "
                      "algorithm (default: 0.0). Only supported for "
                      "heuristic-driven algorithms.")(
                      "start-delay", po::value<double>()->default_value(0.0),
                      "specify the starting time delay for the algorithm "
                      "(default: 0.0). Only for dynamic problems (temporal "
                      "space).")("with-voronoi-pull",
                                 "specify whether to use a Voronoi pull or not "
                                 "to add an exploratory "
                                 "bias to the search (default: not).")(
                      "sa-temperature",
                      po::value<double>()->default_value(-1.0),
                      "specify the initial Simulated Annealing temperature for "
                      "algorithms "
                      "that work on a exploration-exploitation "
                      "schedule (e.g., SA-SBA*).")

                      ("knn-method", po::value<std::string>(),
#ifdef RK_PLANNERS_ENABLE_VEBL_TREE
                       "specify the KNN method to use (supported options: "
                       "linear, bf2, "
                       "bf4, cob2, cob4) (default: bf2)"
#else
                       "specify the KNN method to use (supported options: "
                       "linear, bf2, "
                       "bf4) (default: bf2)"
...
planning_option_collection get_planning_option_from_po(
  ...
  if (vm.count("planner-options")) {
    try {
      (*serialization::open_iarchive(
          vm["planner-options"].as<std::string>())) >>
          plan_options;
  ...
  if (vm.count("knn-method")) {
    ...
    if ((vm["knn-method"].as<std::string>() == "linear") &&
        (vm["mg-storage"].as<std::string>() == "adj-list")) {
  ...
  if (vm["sa-temperature"].as<double>() > -1.0) {
