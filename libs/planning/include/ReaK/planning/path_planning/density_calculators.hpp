...
struct prm_density_calculator {
  ...
  void travel_explored( Vertex u, Vertex v, Graph& g, const SpaceType& space, double sampling_radius,
                        std::size_t space_dim ) const {};

  template < typename Vertex, typename Graph, typename SpaceType >
  void travel_succeeded( Vertex u, Vertex v, Graph& g, const SpaceType& space, double sampling_radius,
                         std::size_t space_dim ) const {};

  template < typename Vertex, typename Graph, typename SpaceType >
  void travel_failed( Vertex u, Vertex v, Graph& g, const SpaceType& space, double sampling_radius,
                      std::size_t space_dim ) const {};

  template < typename Vertex, typename Graph, typename SpaceType >
  void update_density( Vertex u, Graph& g, const SpaceType& space, double sampling_radius,
                       std::size_t space_dim ) const {
...
struct sbastar_density_calculator {
  double compute_sample_similarity( double travel_dist, double event_radius, double sampling_radius,
                                    std::size_t space_dim ) const {
    using std::exp;
    using std::log;
    ...
    return exp( -travel_dist * travel_dist / ( sig2_x * 2.0 )
                - 0.5 * double( space_dim ) * ( sig2_n / sig2_x - 1.0 - log( sig2_n / sig2_x ) ) );
  };
  ...
  void travel_explored( Vertex u, Vertex v, Graph& g, const SpaceType& space, double sampling_radius,
                        std::size_t space_dim ) const {
    double dist
      = get( distance_metric, space.get_super_space() )( g[u].position, g[v].position, space.get_super_space() );
