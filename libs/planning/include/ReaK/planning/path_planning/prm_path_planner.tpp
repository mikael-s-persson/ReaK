...
struct prm_planner_visitor : density_plan_visitor< FreeSpaceType, prm_density_calculator > {
  ...
  prm_planner_visitor( planner_base_type* aPlanner, query_type* aQuery = NULL, any_knn_synchro* aNNSynchro = NULL,
  ...
  void publish_path( Graph& g ) const {
    ...
    boost::astar_search( g, start_node, astar_heuristic_getter< Graph >( &g ), boost::default_astar_visitor(),
                         get( &VertexProp::predecessor, g ), get( &VertexProp::key_value, g ),
                         get( &VertexProp::distance_accum, g ), get( &EdgeProp::weight, g ),
                         boost::identity_property_map(), get( &VertexProp::astar_color, g ), std::less< double >(),
...
#define RK_PRM_PLANNER_SETUP_ALT_TREE_SYNCHRO( ARITY, TREE_STORAGE )                                                 \
  typedef dvp_adjacency_list< VertexProp, EdgeProp, SuperSpace, PositionMap, ARITY, random_vp_chooser, TREE_STORAGE, \
...
#define RK_PRM_PLANNER_MAKE_GENERATE_PRM_CALL                                                                   \
  ReaK::graph::generate_prm( motion_graph, *sup_space_ptr, vis, pos_map, get( random_sampler, *sup_space_ptr ), \
                             dens_map, nc_selector, 0.2 );


  if( ( this->m_data_structure_flags & MOTION_GRAPH_STORAGE_MASK ) == ADJ_LIST_MOTION_GRAPH ) {

    typedef boost::adjacency_list_BC< boost::vecBC, boost::vecBC, DirectionalityTag, VertexProp, EdgeProp >
      MotionGraphType;
    typedef typename boost::graph_traits< MotionGraphType >::vertex_descriptor Vertex;
