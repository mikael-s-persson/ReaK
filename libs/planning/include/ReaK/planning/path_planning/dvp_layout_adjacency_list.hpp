 ...
 * boost::adjacency_list_BC).
 */
template < typename VertexProperty, typename EdgeProperty, typename Topology, typename PositionMap,
           unsigned int Arity = 2, typename VPChooser = random_vp_chooser,
           typename TreeStorageTag = boost::bfl_d_ary_tree_storage< Arity >, typename OutEdgeListS = boost::vecBC,
           typename DirectedS = boost::directedS, typename EdgeListS = boost::vecBC >
class dvp_adjacency_list {
public:
  BOOST_CONCEPT_ASSERT( ( MetricSpaceConcept< Topology > ) );

  typedef dvp_adjacency_list< VertexProperty, EdgeProperty, Topology, PositionMap, Arity, VPChooser, TreeStorageTag,
                              OutEdgeListS, DirectedS, EdgeListS > self;
...
private:
  typedef ReaK::graph::adj_list_on_tree_tag< OutEdgeListS, DirectedS, VertexProperty, EdgeProperty, EdgeListS,
                                             TreeStorageTag > alt_tag;
  ...
  typedef boost::composite_property_map< boost::data_member_property_map< distance_type, edge_properties >,
                                         edge_r2b_map_type > distance_map_type;
  typedef boost::composite_property_map< PositionMap,
                                         typename boost::property_map< vertex_raw_property_type,
                                                                       boost::vertex_second_bundle_t >::type >
    position_map_type;


  typedef dvp_tree_impl< tree_indexer, Topology, key_map_type, distance_map_type, position_map_type, Arity, VPChooser >
    dvp_impl_type;
...
public:
   ...
   */
  dvp_adjacency_list( const ReaK::shared_ptr< const Topology >& aSpace, PositionMap aPosition,
                      VPChooser aVPChooser = VPChooser() )
      : m_tree(), m_position( aPosition ), m_vp_key( get( boost::vertex_key, vertex_raw_property_type() ) ),
        m_vp_pos( aPosition, get( boost::vertex_second_bundle, vertex_raw_property_type() ) ),
        m_impl(
          m_tree, aSpace, m_vp_key,
          distance_map_type( boost::data_member_property_map< distance_type, edge_properties >( &edge_properties::d ),
   ...
   */
  dvp_adjacency_list( self&& rhs ) BOOST_NOEXCEPT : m_tree( std::move( rhs.m_tree ) ),
   ...
   */
  adj_list_type get_adjacency_list() { return adj_list_type( m_tree, dvp_visitor_type( &m_impl ) ); };
