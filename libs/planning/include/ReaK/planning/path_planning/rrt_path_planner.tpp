...
#define RK_RRT_PLANNER_CALL_RRT_FUNCTION                                                                        \
  ReaK::graph::generate_rrt( motion_graph, *sup_space_ptr, vis, pos_map, get( random_sampler, *sup_space_ptr ), \
                             nn_finder );


    if( ( this->m_data_structure_flags & MOTION_GRAPH_STORAGE_MASK ) == ADJ_LIST_MOTION_GRAPH ) {

      typedef boost::adjacency_list_BC< boost::vecBC, boost::listBC, DirectionalityTag, VertexProp, EdgeProp >
        MotionGraphType;
...
#define RK_RRT_PLANNER_CALL_BIRRT_FUNCTION                                                             \
  ReaK::graph::generate_bidirectional_rrt( motion_graph1, motion_graph2, *sup_space_ptr, vis, pos_map, \
