...
public:
  ...
  MEAQR_sbastar_planner( const shared_ptr< space_type >& aWorld = shared_ptr< space_type >(),
      ...
      : base_type( "MEAQR_sbastar_planner", aWorld, aMaxVertexCount, aProgressInterval, aDataStructureFlags,
                   aPlanningMethodFlags, aSteerProgressTolerance, aConnectionTolerance, aSamplingRadius,
                   aSpaceDimensionality, aReporter ),
        m_init_dens_threshold( 0.8 ), m_init_relaxation( 0.0 ), m_SA_init_temperature( -1.0 ){};
  ...
  *******************************************************************************/
  ...
  virtual void RK_CALL save( serialization::oarchive& A, unsigned int ) const {
    ...
    A& RK_SERIAL_SAVE_WITH_NAME( m_init_dens_threshold ) & RK_SERIAL_SAVE_WITH_NAME( m_init_relaxation )
      & RK_SERIAL_SAVE_WITH_NAME( m_SA_init_temperature );
  };

  virtual void RK_CALL load( serialization::iarchive& A, unsigned int ) {
    base_type::load( A, base_type::getStaticObjectType()->TypeVersion() );
    A& RK_SERIAL_LOAD_WITH_NAME( m_init_dens_threshold ) & RK_SERIAL_LOAD_WITH_NAME( m_init_relaxation )
...
struct MEAQR_sbastar_visitor
  : density_plan_visitor< MEAQR_topology_with_CD< StateSpace, StateSpaceSystem, StateSpaceSampler >,
  ...
  boost::tuple< point_type, bool, EdgeProp > random_walk( Vertex u, Graph& g ) const {
    ...
    typename point_distribution_traits< super_space_type >::random_sampler_type get_sample
      = get( random_sampler, sup_space );
    ...
    do {
      ...
      double target_dist = boost::uniform_01< global_rng_type&, double >( get_global_rng() )()
      ...
      boost::tie( get< 0 >( result ), get< 2 >( result ).steer_record )
        = this->m_query->space->steer_position_toward( g[u].position, 1.0, p_inter );
...
template < typename StateSpace, typename StateSpaceSystem, typename StateSpaceSampler >
void MEAQR_sbastar_planner< StateSpace, StateSpaceSystem, StateSpaceSampler >::solve_planning_query(
  planning_query< typename MEAQR_sbastar_planner< StateSpace, StateSpaceSystem, StateSpaceSampler >::space_type >&
    aQuery ) {
  ...
  typedef typename MEAQR_sbastar_planner< StateSpace, StateSpaceSystem, StateSpaceSampler >::super_space_type
    SuperSpace;
...
#define RK_MEAQR_SBASTAR_PLANNER_MAKE_SBASTAR_BUNDLE                                                            \
  ReaK::graph::make_sbastar_bundle( motion_graph, boost::any_cast< Vertex >( vis.m_start_node ),                \
                                    this->m_space->get_super_space(), vis, nc_selector,                         \
                                    get( &VertexProp::key_value, motion_graph ), pos_map, weight_map, dens_map, \
                                    cons_map, dist_map, pred_map, heuristic_map )
...
#define RK_MEAQR_SBASTAR_PLANNER_CALL_LAZY_SBASTAR_FUNCTION \
  ReaK::graph::generate_lazy_sbastar( RK_MEAQR_SBASTAR_PLANNER_MAKE_SBASTAR_BUNDLE );

#define RK_MEAQR_SBASTAR_PLANNER_CALL_LAZY_BNB_SBASTAR_FUNCTION \
  ReaK::graph::generate_lazy_bnb_sbastar( RK_MEAQR_SBASTAR_PLANNER_MAKE_SBASTAR_BUNDLE_WITH_GOAL );


#define RK_MEAQR_SBASTAR_PLANNER_CALL_SBARRTSTAR_FUNCTION                                    \
  ReaK::graph::generate_sbarrtstar( RK_MEAQR_SBASTAR_PLANNER_MAKE_SBASTAR_BUNDLE,            \
...
#define RK_MEAQR_SBASTAR_PLANNER_CALL_LAZY_SBARRTSTAR_FUNCTION                                    \
  ReaK::graph::generate_lazy_sbarrtstar( RK_MEAQR_SBASTAR_PLANNER_MAKE_SBASTAR_BUNDLE,            \
                                         get( random_sampler, this->m_space->get_super_space() ), \
                                         this->m_SA_init_temperature );

#define RK_MEAQR_SBASTAR_PLANNER_CALL_LAZY_BNB_SBARRTSTAR_FUNCTION                                    \
  ReaK::graph::generate_lazy_bnb_sbarrtstar( RK_MEAQR_SBASTAR_PLANNER_MAKE_SBASTAR_BUNDLE_WITH_GOAL,  \
                                             get( random_sampler, this->m_space->get_super_space() ), \
                                             this->m_SA_init_temperature );


#define RK_MEAQR_SBASTAR_PLANNER_CALL_ANYTIME_SBASTAR_FUNCTION \
  ReaK::graph::generate_anytime_sbastar( RK_MEAQR_SBASTAR_PLANNER_MAKE_SBASTAR_BUNDLE, this->m_init_relaxation );

#define RK_MEAQR_SBASTAR_PLANNER_CALL_ANYTIME_LAZY_SBASTAR_FUNCTION \
  ReaK::graph::generate_anytime_lazy_sbastar( RK_MEAQR_SBASTAR_PLANNER_MAKE_SBASTAR_BUNDLE, this->m_init_relaxation );

#define RK_MEAQR_SBASTAR_PLANNER_CALL_ANYTIME_LAZY_BNB_SBASTAR_FUNCTION                                   \
  ReaK::graph::generate_anytime_lazy_bnb_sbastar( RK_MEAQR_SBASTAR_PLANNER_MAKE_SBASTAR_BUNDLE_WITH_GOAL, \
                                                  this->m_init_relaxation );


#define RK_MEAQR_SBASTAR_PLANNER_CALL_ANYTIME_SBARRTSTAR_FUNCTION                                    \
  ReaK::graph::generate_anytime_sbarrtstar( RK_MEAQR_SBASTAR_PLANNER_MAKE_SBASTAR_BUNDLE,            \
                                            get( random_sampler, this->m_space->get_super_space() ), \
                                            this->m_init_relaxation, this->m_SA_init_temperature );

#define RK_MEAQR_SBASTAR_PLANNER_CALL_ANYTIME_LAZY_SBARRTSTAR_FUNCTION                                    \
  ReaK::graph::generate_anytime_lazy_sbarrtstar( RK_MEAQR_SBASTAR_PLANNER_MAKE_SBASTAR_BUNDLE,            \
                                                 get( random_sampler, this->m_space->get_super_space() ), \
                                                 this->m_init_relaxation, this->m_SA_init_temperature );

#define RK_MEAQR_SBASTAR_PLANNER_CALL_ANYTIME_LAZY_BNB_SBARRTSTAR_FUNCTION                                    \
  ReaK::graph::generate_anytime_lazy_bnb_sbarrtstar( RK_MEAQR_SBASTAR_PLANNER_MAKE_SBASTAR_BUNDLE_WITH_GOAL,  \
                                                     get( random_sampler, this->m_space->get_super_space() ), \
                                                     this->m_init_relaxation, this->m_SA_init_temperature );


#define RK_MEAQR_SBASTAR_PLANNER_CALL_APPROPRIATE_SBASTAR_PLANNER_FUNCTION                                 \
  if( ( ( this->m_planning_method_flags & ADDITIONAL_PLANNING_BIAS_MASK ) & PLAN_WITH_ANYTIME_HEURISTIC )  \
