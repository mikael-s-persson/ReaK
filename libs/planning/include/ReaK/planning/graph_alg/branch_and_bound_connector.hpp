...
struct BNBConnectorVisitorConcept : MotionGraphConnectorVisitorConcept< ConnectorVisitor, Graph, Topology > {
  ...
  BOOST_CONCEPT_USAGE( BNBConnectorVisitorConcept ) { conn_vis.vertex_to_be_removed( u, g ); }
};
...
struct bnb_ordering_data {
  ...
  bnb_ordering_data( Graph& g, Vertex aStart, Vertex aGoal )
      : start_vertex( aStart ), goal_vertex( aGoal ), index_in_heap(), key(),
  ...
  void prune_worst_nodes( double threshold, Graph& g, const ConnectorVisitor& conn_vis ) {
...
struct bnb_connector {
  ...
  template < typename Topology, typename ConnectorVisitor, typename PositionMap, typename DistanceMap,
             typename PredecessorMap, typename WeightMap >
  void update_successors( Vertex v, Graph& g, const Topology& super_space, const ConnectorVisitor& conn_vis,
      ...
      for( boost::tie( eo, eo_end ) = out_edges( s, g ); eo != eo_end; ++eo ) {
        ...
        put( data->key, t,
             get( distance, g[t] ) + get( ReaK::pp::distance_metric, super_space )(
                                       get( position, g[t] ), get( position, g[data->goal_vertex] ), super_space ) );
    ...
    if( get( predecessor, g[data->goal_vertex] ) != boost::graph_traits< Graph >::null_vertex() )
      data->prune_worst_nodes( get( distance, g[data->goal_vertex] ), g, conn_vis );
  };

  template < typename Topology, typename ConnectorVisitor, typename PositionMap, typename FwdDistanceMap,
             typename SuccessorMap, typename WeightMap >
  void update_predecessors( Vertex v, Graph& g, const Topology& super_space, const ConnectorVisitor& conn_vis,
    ...
    while( !incons.empty() ) {
      ...
      for( boost::tie( ei, ei_end ) = in_edges( t, g ); ei != ei_end; ++ei ) {
        ...
        put( data->key, s, get( fwd_distance, g[s] )
                           + get( ReaK::pp::distance_metric, super_space )( get( position, g[data->start_vertex] ),
                                                                            get( position, g[s] ), super_space ) );
    ...
    if( get( successor, g[data->start_vertex] ) != boost::graph_traits< Graph >::null_vertex() )
      data->prune_worst_nodes( get( fwd_distance, g[data->start_vertex] ), g, conn_vis );
  };
   ...
   */
  template < typename Graph2, typename Topology, typename ConnectorVisitor, typename PositionMap, typename DistanceMap,
             typename PredecessorMap, typename WeightMap, typename NcSelector >
  typename boost::enable_if< boost::is_undirected_graph< Graph2 > >::type
    operator()( const typename boost::property_traits< PositionMap >::value_type& p, Vertex& x_near, EdgeProp& eprop,
                Graph2& g, const Topology& super_space, const ConnectorVisitor& conn_vis, PositionMap position,
    ...
    double dist_from_start
      = get( ReaK::pp::distance_metric, super_space )( get( position, g[data->start_vertex] ), p, super_space );
    double dist_to_goal
      = get( ReaK::pp::distance_metric, super_space )( p, get( position, g[data->goal_vertex] ), super_space );

    if( ( get( predecessor, g[data->goal_vertex] ) != boost::graph_traits< Graph >::null_vertex() )
        && ( dist_from_start + dist_to_goal > get( distance, g[data->goal_vertex] ) ) )
      return;

    std::vector< Vertex > Nc;
    select_neighborhood( p, std::back_inserter( Nc ), g, super_space,
    ...
    lazy_node_connector::connect_best_predecessor( v, x_near, eprop, g, super_space, conn_vis, position, distance,
                                                   predecessor, weight, Nc );
    pruned_node_connector::create_pred_edge( v, x_near, eprop, g, conn_vis, distance, predecessor, weight );

    if( ( get( predecessor, g[data->goal_vertex] ) != boost::graph_traits< Graph >::null_vertex() )
        && ( get( distance, g[v] ) + dist_to_goal > get( distance, g[data->goal_vertex] ) ) ) {
    ...
    lazy_node_connector::connect_successors( v, x_near, g, super_space, conn_vis, position, distance, predecessor,
                                             weight, Nc );
  ...
  template < typename Graph2, typename Topology, typename ConnectorVisitor, typename PositionMap, typename DistanceMap,
             typename PredecessorMap, typename WeightMap, typename NcSelector >
  typename boost::enable_if< boost::is_directed_graph< Graph2 > >::type
    operator()( const typename boost::property_traits< PositionMap >::value_type& p, Vertex& x_near, EdgeProp& eprop,
                Graph2& g, const Topology& super_space, const ConnectorVisitor& conn_vis, PositionMap position,
                DistanceMap distance, PredecessorMap predecessor, WeightMap weight,
                NcSelector select_neighborhood ) const {
    ...
    select_neighborhood( p, std::back_inserter( Pred ), std::back_inserter( Succ ), g, super_space,
                         boost::bundle_prop_to_vertex_prop( position, g ) );
  ...
  template < typename Graph2, typename Topology, typename ConnectorVisitor, typename PositionMap, typename DistanceMap,
             ...
             typename NcSelector >
  typename boost::enable_if< boost::is_undirected_graph< Graph2 > >::type
    operator()( const typename boost::property_traits< PositionMap >::value_type& p, Vertex& x_pred,
                EdgeProp& eprop_pred, Vertex& x_succ, EdgeProp& eprop_succ, Graph2& g, const Topology& super_space,
    ...
    lazy_node_connector::connect_best_predecessor( v, x_pred, eprop_pred, g, super_space, conn_vis, position, distance,
                                                   predecessor, weight, Nc );
    lazy_node_connector::connect_best_successor( v, x_succ, eprop_succ, g, super_space, conn_vis, position,
                                                 fwd_distance, successor, weight, Nc );
    ...
    lazy_node_connector::connect_successors( v, x_pred, g, super_space, conn_vis, position, distance, predecessor,
                                             weight, Nc, successor );
    update_successors( v, g, super_space, conn_vis, position, distance, predecessor, weight );
    lazy_node_connector::connect_predecessors( v, x_succ, g, super_space, conn_vis, position, fwd_distance, successor,
                                               weight, Nc, predecessor );
  ...
  typename boost::enable_if< boost::is_directed_graph< Graph2 > >::type
    ...
    lazy_node_connector::connect_best_predecessor( v, x_pred, eprop_pred, g, super_space, conn_vis, position, distance,
                                                   predecessor, weight, Pred );
    lazy_node_connector::connect_best_successor( v, x_succ, eprop_succ, g, super_space, conn_vis, position,
                                                 fwd_distance, successor, weight, Succ );
