 ...
 * \file bit_star.hpp
 *
 * This library provides function templates and concepts that implement the Batch Informed Trees (BIT*) 
 * algorithm. BIT* uses the A* search heuristic and branch-and-bounded sampling in batches to generate a 
 * motion-graph into the free-space in order to connect a start and goal location. This algorithm has 
 * many customization points because there are many choices to be made in the method, such as how to 
 * find nearest neighbors for attempting to connect them through free-space, how to expand vertices, 
 * when to stop the algorithm, etc. All these customization points are left to the user to implement, 
 * some are defined by the BITStarVisitorConcept (random-walk, edge-added, etc.).
 *
...
#ifndef REAK_BIT_STAR_HPP
#define REAK_BIT_STAR_HPP
...
/**
  ...
  * class for the BIT* algorithm. A visitor class is essentially a class that regroups a number of 
  * callback functions that can be used to inject customization into the BIT* algorithm. In other 
  * words, the visitor pattern in generic programming is an implementation of IoC 
  * (Inversion of Control), since the BIT* algorithm is in control of execution, but custom behavior can
  ...
  * 
  * \tparam Visitor The visitor class to be tested for modeling an BIT* visitor concept.
...
struct BITStarVisitorConcept : SBMPVisitorConcept<Visitor,Graph> {
  ...
  BOOST_CONCEPT_USAGE(BITStarVisitorConcept)
  {
    vis.publish_path(g);
  };
  Visitor vis;
...
/**
 * This class is simply an archetype visitor for the BIT* algorithm.
 */
template <typename Topology>
struct bitstar_visitor_archetype : 
...
/**
  * This concept class defines the valid expressions required of a class to be used as a visitor 
  * class for the bi-directional BIT* algorithm. A visitor class is essentially a class that regroups a number of 
  * callback functions that can be used to inject customization into the bi-directional BIT* algorithm. 
  * In other words, the visitor pattern in generic programming is an implementation of IoC 
  * (Inversion of Control), since the bi-directional BIT* algorithm is in control of execution, 
  ...
  */
template <typename Visitor, typename Graph, typename Topology>
struct BITStarBidirVisitorConcept : SBAStarVisitorConcept<Visitor,Graph,Topology> {
  
  BOOST_CONCEPT_ASSERT((NodeBackPushingVisitorConcept<Visitor,Graph,Topology>));
  
  BOOST_CONCEPT_USAGE(BITStarBidirVisitorConcept)
  { };
};

/**
 * This class is simply an archetype visitor for the bi-directional BIT* algorithm.
 */
template <typename Topology>
struct bitstar_bidir_visitor_archetype : 
  bitstar_visitor_archetype<Topology>, 
...
namespace detail {
  ...
  template <typename Graph, typename Topology, typename BITStarVisitor, typename MotionGraphConnector, 
            typename BITNodeGenerator, typename MutableQueue, typename SampleBatch, typename EdgeQueue, typename NcSelector>
  void bitstar_search_loop(
      Graph &g, const Topology& super_space, BITStarVisitor& bit_vis, 
      MotionGraphConnector connect_vertex, BITNodeGenerator sba_generate_node,
      MutableQueue& Qv, SampleBatch& Xsample, EdgeQueue& Qe, NcSelector select_neighborhood)
  { 
    typedef typename ReaK::pp::topology_traits<Topology>::point_type PositionValue;
    typedef typename boost::graph_traits<Graph>::vertex_descriptor Vertex;
    typedef typename Graph::edge_bundled EdgeProp;
    
    while (!Qv.empty() && bit_vis.keep_going()) { 
      
      if( Qe.empty() ) {
        // prune Xsample w.r.t. heuristic-key(x) >= key(goal)
        // shallow-prune g w.r.t. key(v) >= key(goal)
        // 
      };
      ...
      while( ! bit_vis.has_search_potential(u, g) ) {
        if( Q.empty() ) {
      ...
      bit_vis.examine_vertex(u, g);
      
      PositionValue p_new; Vertex x_near; EdgeProp eprop;
      boost::tie(x_near, p_new, eprop) = sba_generate_node(u, g, bit_vis, bit_vis.m_position);
      
      // then push it back on the OPEN queue.
      if( ( x_near != boost::graph_traits<Graph>::null_vertex() ) ||
          ( Q.empty() ) )
        bit_vis.requeue_vertex(u,g);
      
      if(x_near != boost::graph_traits<Graph>::null_vertex()) {
        connect_vertex(p_new, x_near, eprop, g, 
                        super_space, bit_vis, bit_vis.m_position, 
                        bit_vis.m_distance, bit_vis.m_predecessor, 
                        bit_vis.m_weight, select_neighborhood);
      };
