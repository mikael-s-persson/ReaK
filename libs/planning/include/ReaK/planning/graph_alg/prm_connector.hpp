...
struct prm_node_connector {
  ...
  template < typename Vertex, typename EdgeProp, typename Graph, typename ConnectorVisitor >
  void connect_to_first_pred( Vertex v, Vertex x_near, EdgeProp& eprop, Graph& g, ConnectorVisitor& conn_vis ) {
   ...
   */
  template < typename Graph, typename Topology, typename ConnectorVisitor, typename PositionMap, typename NcSelector >
  typename boost::enable_if< boost::is_undirected_graph< Graph > >::type
    operator()( const typename boost::property_traits< PositionMap >::value_type& p,
                ...
                Graph& g, const Topology& super_space, ConnectorVisitor& conn_vis, PositionMap position,
