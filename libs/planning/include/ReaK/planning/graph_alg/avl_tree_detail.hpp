...
struct avl_tree_helper {
  using value_type = graph_vertex_bundle_t<TreeType>;
...
class avl_tree_impl {
 public:
  ...
  using helper_type =
      detail::avl_tree_helper<TreeType, Compare, ContainerStyle>;
  ...
  using iterator =
      boost::bst_inorder_iterator<const TreeType, const value_type>;
 ...
 private:
  ...
  using vertex_type = graph_vertex_t<tree_indexer>;
  using edge_type = graph_edge_t<tree_indexer>;
  using out_edge_iter =
      typename boost::graph_traits<tree_indexer>::out_edge_iterator;
  using in_edge_iter =
      typename boost::graph_traits<tree_indexer>::in_edge_iterator;
  using child_vertex_iter =
      typename boost::tree_traits<tree_indexer>::child_vertex_iterator;
  ...
  void construct_node(vertex_type aCurrentNode,
                      typename construction_task::PropIter aBegin,
                      typename construction_task::PropIter aEnd) {
    using std::swap;
    ...
    while (!tasks.empty()) {
      ...
      std::pair<vertex_type, edge_type> new_child =
          add_child_vertex(cur_task.node, m_tree);
      tasks.push(
          construction_task(new_child.first, cur_task.first, cur_task_median));
      ...
      tasks.push(
          construction_task(new_child.first, cur_task_median, cur_task.last));
    }
  }

  bool has_left_child(vertex_type u) const {
    return out_degree(u, m_tree) >= 1;
  }
  bool has_right_child(vertex_type u) const {
    return out_degree(u, m_tree) >= 2;
  }
  ...
  std::pair<std::size_t, std::size_t> get_minmax_depth(
    ...
    while (!tasks.empty()) {
      std::pair<vertex_type, std::size_t> cur_task = tasks.front();
      tasks.pop();
      ...
      ++(cur_task.second);
      tasks.push(std::pair<vertex_type, std::size_t>(
          get_left_child(cur_task.first), cur_task.second));
      if (has_right_child(cur_task.first)) {
        tasks.push(std::pair<vertex_type, std::size_t>(
            get_right_child(cur_task.first), cur_task.second));
      }
    }
  ...
  void collect_nodes(vertex_type aRootNode,
                     std::vector<vertex_property>& aList) {
    // assume that a breadth-first traversal is more efficient:
  ...
  // re-balance the sub-tree rooted at the given node. Uses a "collect and re-construct" approach.
  void rebalance_and_insert_subtree(vertex_type aRootNode,
                                    vertex_property&& aAdded) {
    // collect and remove.
    std::vector<vertex_property> collected_nodes;
  ...
  std::pair<vertex_type, bool> find_imbalance_impl(vertex_type u,
    ...
    while (ei != ei_end) {  // until you hit the root node.
      ...
      if ((o_depth.first < c_depth.second - 1) ||
          (o_depth.second >
           c_depth.first +
               1)) {  // then, the sub-tree spanned from "p" will be unbalanced:
    ...
    return std::pair<vertex_type, bool>(
        orig_u, false);  // could not find an imbalance anywhere.
  }

  std::pair<iterator, bool> insert_after_terminal_impl(
      vertex_type aBefore, vertex_property&& aValue) {
    using std::swap;
    if (out_degree(
            aBefore,
            m_tree)) {  // then the before node is terminal but not a leaf, safe to insert:
      std::pair<vertex_type, edge_type> new_child =
  ...
  vertex_type insert_in_range_impl(iterator tvi, iterator tvi_end,
                                   vertex_property&& aValue) {
    // lets try to find a sweet spot (balance-wise) to place the new node:
    vertex_type tmp_u = tvi.base();
    for (; tvi != tvi_end; ++tvi) {
      if (out_degree(tvi.base(), m_tree) ==
          1) {  // then, this is definitely a good spot:
        std::pair<vertex_type, edge_type> new_child =
            add_child_vertex(tvi.base(), std::move(aValue), m_tree);
        return new_child.first;
      }
    }
    // if we reach this point, then, try to go up to some re-balanceable node.
    boost::graph::detail::bst_traversal_status dummy_status =
        boost::graph::detail::OnRightBranch;
  ...
  std::pair<iterator, bool> insert_impl(vertex_type aBefore, vertex_type aAfter,
    ...
    if ((aAfter == tree_indexer::null_vertex()) &&
        (aBefore != tree_indexer::null_vertex())) {
      // this means that all the vertices remaining after aBefore are all equal to the given value.
      vertex_type tmp_u =
          insert_in_range_impl(iterator(&m_tree, aBefore),
                               iterator::end(&m_tree), std::move(aValue));
      if (tmp_u == tree_indexer::null_vertex()) {
        aBefore =
            tmp_u;  // this will cause it to be inserted at the end (see below).
      } else {
        return std::pair<iterator, bool>(iterator(&m_tree, tmp_u), true);
      }
    }

    if ((aBefore == tree_indexer::null_vertex()) &&
        (aAfter == tree_indexer::null_vertex())) {
    ...
    if (aBefore == aAfter) {
      ...
      if (out_degree(aAfter, m_tree) == 1) {
        ...
        return std::pair<iterator, bool>(
            iterator(&m_tree, *(child_vertices(aAfter, m_tree).first)), true);
      }

      // else, aAfter is full, lets try after the previous node (which must be a leaf or non-full node):
      aBefore = boost::graph::detail::bst_go_down_right(
          m_tree, *(child_vertices(aAfter, m_tree).first));
      return insert_after_terminal_impl(aBefore, std::move(aValue));
    }
    ...
    vertex_type tmp_u =
        insert_in_range_impl(iterator(&m_tree, aBefore),
                             iterator(&m_tree, aAfter), std::move(aValue));
    if (tmp_u == tree_indexer::null_vertex()) {
  ...
  iterator erase_impl(iterator aBefore, iterator aAfter) {
    ...
    while (true) {
      if (m_compare(
              ...
              m_tree
                  [aBefore
                       .base()])) {  // if root is less than lower-bound than go right.
        mid_root = get_right_child(mid_root);
        continue;
      }
      if (m_compare(
              m_tree[befAfter.base()],
              m_tree
                  [mid_root])) {  // if root is greater than upper-bound than go left.
    ...
    // collect and remove.
    std::vector<vertex_property> collected_nodes;
    iterator it_near(&m_tree,
                     boost::graph::detail::bst_go_down_left(m_tree, mid_root));
    iterator it_far(&m_tree,
                    boost::graph::detail::bst_go_down_right(m_tree, mid_root));
 ...
 public:
   ...
   */
  bool empty() const noexcept { return (num_vertices(m_tree) == 0); }
  /**
   ...
   */
  size_type size() const noexcept { return num_vertices(m_tree); }
  /**
   ...
   */
  const_iterator begin() const { return const_iterator::begin(&m_tree); }

  /**
   * Returns a read-only (constant) iterator that points one past the last element in the set.
   * Iteration is done in ascending order according to the keys.
   */
  const_iterator end() const { return const_iterator::end(&m_tree); }
  ...
  std::pair<const_iterator, const_iterator> equal_range(
    ...
    if (m_root == boost::graph_traits<tree_indexer>::null_vertex()) {
      return {const_iterator::end(&m_tree), const_iterator::end(&m_tree)};
    }
    return {lower_bound(aKey), upper_bound(aKey)};
  }
  ...
  size_type count(const key_type& aKey) const {
    ...
    auto [first, last] = equal_range(aKey);
  ...
  iterator insert(const_iterator aPosition, P&& aValue) {
    ...
    // else, position is not less than value.
    if (aPosition == begin()) {
      return insert_impl(aPosition.base(), aPosition.base(),
                         value_type(std::forward<P>(aValue)))
          .first;
    }
    const_iterator p4 = aPosition;
    ...
    return insert_impl(aPosition.base(), p5.base(),
                       value_type(std::forward<P>(aValue)))
        .first;
  }
