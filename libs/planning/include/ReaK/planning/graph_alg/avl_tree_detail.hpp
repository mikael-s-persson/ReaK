...
enum avl_container_style { avl_set_style, avl_multiset_style, avl_map_style, avl_multimap_style };


template < typename TreeType, typename Compare, avl_container_style ContainerStyle >
...
struct avl_tree_helper< TreeType, Compare, avl_map_style > {
  ...
  struct value_compare {
    ...
    bool operator()( const value_type& lhs, const value_type& rhs ) const { return m_compare( lhs.first, rhs.first ); };
    bool operator()( const value_type& lhs, const key_type& rhs ) const { return m_compare( lhs.first, rhs ); };
    bool operator()( const key_type& lhs, const value_type& rhs ) const { return m_compare( lhs, rhs.first ); };
    bool operator()( const key_type& lhs, const key_type& rhs ) const { return m_compare( lhs, rhs ); };
  };
 ...
 */
template < typename TreeType, typename Compare, detail::avl_container_style ContainerStyle >
class avl_tree_impl {
public:
...
private:
  ...
  void construct_node( vertex_type aCurrentNode, typename std::vector< vertex_property >::iterator aBegin,
  ...
  vertex_type get_left_child( vertex_type u ) const { return *( child_vertices( u, m_tree ).first ); };
  vertex_type get_right_child( vertex_type u ) const { return *( ++( child_vertices( u, m_tree ).first ) ); };
  ...
  iterator erase_impl( iterator aBefore, iterator aAfter ) {
    ...
    while( true ) {
      ...
      if( m_compare( m_tree[befAfter.base()],
                     m_tree[mid_root] ) ) { // if root is greater than upper-bound than go left.
...
public:
  ...
  avl_tree_impl( InputIterator aFirst, InputIterator aLast, const Compare& comp = Compare(),
   ...
   * \param comp A comparison functor.
   */
  avl_tree_impl( std::initializer_list< value_type > aList, const Compare& comp = Compare(),
                 const allocator_type& = allocator_type() )
      : m_tree(), m_root( boost::graph_traits< tree_indexer >::null_vertex() ), m_compare( comp ) {
    m_root = create_root( m_tree );
   ...
   */
  const_reverse_iterator rbegin() const { return const_reverse_iterator( const_iterator::end( &m_tree ) ); };

  /**
   * Returns a read-only (constant) reverse iterator that points to the last element in the set.
   * Iteration is done in descending order according to the keys.
   */
  const_reverse_iterator rend() const { return const_reverse_iterator( const_iterator::begin( &m_tree ) ); };
  ...
  std::pair< const_iterator, const_iterator > equal_range( const key_type& aKey ) const {
    if( m_root == boost::graph_traits< tree_indexer >::null_vertex() )
      return std::pair< const_iterator, const_iterator >( const_iterator::end( &m_tree ),
                                                          const_iterator::end( &m_tree ) );
   ...
   */
  void insert( std::initializer_list< value_type > aList ) { insert( aList.begin(), aList.end() ); };
#endif
 ...
 */
  ...
  mapped_type& operator[]( key_type&& k ) {
    ...
    if( !m_compare( k, m_tree[u] ) )
      ...
      iterator it
        = insert( const_iterator( &m_tree, u ), helper_type::keymap_to_value( std::move( k ), mapped_type() ) );
      return helper_type::value_to_mapped( m_tree[it.base()] );
    };
  };
