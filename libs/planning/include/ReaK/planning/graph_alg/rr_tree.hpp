...
template < typename Graph, typename Topology, typename RRTVisitor, typename PositionMap >
...
template < typename Graph, typename Topology, typename RRTVisitor, typename RandomSampler, typename PositionMap >
typename boost::graph_traits< Graph >::vertex_descriptor
  rrt_get_or_create_root( Graph& g, const Topology& space, RRTVisitor vis, RandomSampler get_sample,
 ...
 *
 */
template < typename Graph, typename Topology, typename RRTVisitor, typename PositionMap, typename RandomSampler,
           typename NNFinder >
inline void generate_rrt( Graph& g, const Topology& space, RRTVisitor vis, PositionMap position,
                          RandomSampler get_sample, NNFinder find_nearest_neighbor ) {
...
struct rrt_generator {
  template < typename Graph, typename Topology, typename RRTVisitor, typename PositionMap, typename RandomSampler,
             typename NNFinder >
  void operator()( Graph& g, const Topology& space, RRTVisitor vis, PositionMap position, RandomSampler get_sample,
 ...
 *
 */
template < typename Graph, typename Topology, typename BiRRTVisitor, typename PositionMap, typename RandomSampler,
           typename NNFinder >
inline void generate_bidirectional_rrt( Graph& g1, Graph& g2, const Topology& space, BiRRTVisitor vis,
  ...
  std::pair< Vertex, bool > v_target2( detail::rrt_get_or_create_root( g1, space, vis, get_sample, position ), true );
  PositionValue p_target2 = get( position, g1[v_target2.first] );

  std::pair< Vertex, bool > v_target1( detail::rrt_get_or_create_root( g2, space, vis, get_sample, position ), true );
  ...
  while( vis.keep_going() ) {
    ...
    std::pair< Vertex, bool > v1 = detail::expand_rrt_vertex( g1, space, vis, position, u1, p_target1 );
    if( ( v1.second ) && ( v_target1.second ) ) {
      ...
      if( v1.first == u1 ) { // we didn't move at all! Unsuccessful expansion.
        ...
        v_target2.first = v1.first;
        v_target2.second = true;
      };
    };
    ...
    std::pair< Vertex, bool > v2 = detail::expand_rrt_vertex( g2, space, vis, position, u2, p_target2 );
    if( ( v2.second ) && ( v_target2.second ) ) {
      ...
      if( v2.first == u2 ) { // we didn't move at all! Unsuccessful expansion.
        ...
        v_target1.first = v2.first;
        v_target1.second = true;
      };
    };
  };
};
...
struct birrt_generator {
  template < typename Graph, typename Topology, typename BiRRTVisitor, typename PositionMap, typename RandomSampler,
             typename NNFinder >
  void operator()( Graph& g1, Graph& g2, const Topology& space, BiRRTVisitor vis, PositionMap position,
                   RandomSampler get_sample, NNFinder find_nearest_neighbor ) {
