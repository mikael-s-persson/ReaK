...
struct rrg_node_puller {
  ...
  static ResultType expand_to_nearest( PositionValue& p_new, const std::vector< Vertex >& Nc, Graph& g,
                                       RRGVisitor vis ) {
    PositionValue p_tmp;
  ...
  static ResultType expand_to_nearest( PositionValue& p_new, const std::vector< Vertex >& Pred, Graph& g,
                                       RRGVisitor vis, PredecessorMap predecessor ) {
    PositionValue p_tmp;
    bool expand_succeeded = false;
    EdgeProp ep;
  ...
  static ResultType retract_from_nearest( PositionValue& p_new, const std::vector< Vertex >& Succ, Graph& g,
                                          RRGVisitor vis, SuccessorMap successor ) {
    PositionValue p_tmp;
...
struct rrg_node_generator {
  ...
  rrg_node_generator( const Topology* aSpace, RandomSampler aGetSample, NcSelector aSelectNeighborhood )
      : space( aSpace ), get_sample( aGetSample ), select_neighborhood( aSelectNeighborhood ){};
  ...
  typename boost::enable_if< boost::is_undirected_graph< Graph >,
    ...
    operator()( Graph& g, RRGVisitor vis, PositionMap g_position ) const {
 ...
 * \tparam NcSelector The type of a functor that can be used to perform nearest-neighbor queries.
 */
template < typename Topology, typename RandomSampler, typename NcSelector, typename PredecessorMap,
...
struct rrg_bidir_generator {
  ...
  rrg_bidir_generator( const Topology* aSpace, RandomSampler aGetSample, NcSelector aSelectNeighborhood,
                       PredecessorMap aPred, SuccessorMap aSucc )
      : space( aSpace ), get_sample( aGetSample ), select_neighborhood( aSelectNeighborhood ), predecessor( aPred ),
        successor( aSucc ){};
  ...
  typename boost::enable_if< boost::is_directed_graph< Graph >,
    ...
    while( true ) {
      ...
      boost::tie( x_succ, was_retracted, ep_succ )
