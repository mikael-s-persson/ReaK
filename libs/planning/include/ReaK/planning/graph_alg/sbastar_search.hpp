 ...
 */
template < typename Topology >
struct sbastar_bidir_visitor_archetype : sbastar_visitor_archetype< Topology >,
...
struct sbastar_bfs_visitor {
  ...
  sbastar_bfs_visitor( UniformCostVisitor vis, UpdatableQueue& Q, IndexInHeapMap index_in_heap, KeyMap key,
                       PositionMap pos, WeightMap weight, DensityMap density, ConstrictionMap constriction,
                       DistanceMap dist, PredecessorMap pred, FwdDistanceMap fwd_dist,
                       SuccessorMap succ = SuccessorMap() )
      : m_vis( vis ), m_Q( Q ), m_index_in_heap( index_in_heap ), m_key( key ), m_position( pos ), m_weight( weight ),
        m_density( density ), m_constriction( constriction ), m_distance( dist ), m_predecessor( pred ),
  ...
  boost::tuple< PositionValue, bool, EdgeProp > steer_towards_position( const PositionValue& p, Vertex u,
                                                                        const Graph& g ) const {
    return m_vis.steer_towards_position( p, u, g );
  };

  boost::tuple< PositionValue, bool, EdgeProp > steer_back_to_position( const PositionValue& p, Vertex u,
                                                                        const Graph& g ) const {
  ...
  void travel_explored( Vertex u, Vertex v, Graph& g ) const { m_vis.travel_explored( u, v, g ); };

  void travel_succeeded( Vertex u, Vertex v, Graph& g ) const { m_vis.travel_succeeded( u, v, g ); };

  void travel_failed( Vertex u, Vertex v, Graph& g ) const { m_vis.travel_failed( u, v, g ); };
  ...
  bool has_search_potential( Vertex u, const Graph& g ) const { return m_vis.has_search_potential( u, g ); };
  bool should_close( Vertex u, const Graph& g ) const { return m_vis.should_close( u, g ); };
...
struct sba_node_generator {
  ...
  boost::tuple< typename boost::graph_traits< Graph >::vertex_descriptor,
                typename boost::property_traits< PositionMap >::value_type, typename Graph::edge_bundled >
    operator()( typename boost::graph_traits< Graph >::vertex_descriptor u, Graph& g, const SBAVisitor& sba_vis,
                PositionMap ) const {
    typedef typename boost::property_traits< PositionMap >::value_type PositionValue;
    typedef typename boost::graph_traits< Graph >::vertex_descriptor Vertex;
    typedef typename Graph::edge_bundled EdgeProp;
...
void sbastar_search_loop( Graph& g, const Topology& super_space, SBAStarVisitor& sba_vis,
                          MotionGraphConnector connect_vertex, SBANodeGenerator sba_generate_node, MutableQueue& Q,
  ...
  while( !Q.empty() && sba_vis.keep_going() ) {
    ...
    if( x_near != boost::graph_traits< Graph >::null_vertex() ) {
      connect_vertex( p_new, x_near, eprop, g, super_space, sba_vis, sba_vis.m_position, sba_vis.m_distance,
                      sba_vis.m_predecessor, sba_vis.m_weight, select_neighborhood );
    };
...
template < typename Graph, typename Topology, typename SBAStarVisitor, typename MotionGraphConnector,
           typename SBANodeGenerator, typename MutableQueue, typename NcSelector >
void sbastar_bidir_loop( Graph& g, const Topology& super_space, SBAStarVisitor& sba_vis,
                         MotionGraphConnector connect_vertex, SBANodeGenerator sba_generate_node, MutableQueue& Q,
                         NcSelector select_neighborhood ) {
  typedef typename ReaK::pp::topology_traits< Topology >::point_type PositionValue;
  typedef typename boost::graph_traits< Graph >::vertex_descriptor Vertex;
  ...
  while( !Q.empty() && sba_vis.keep_going() ) {
    ...
    if( ( x_near_pred != boost::graph_traits< Graph >::null_vertex() )
        || ( x_near_succ != boost::graph_traits< Graph >::null_vertex() ) || ( Q.empty() ) )
      sba_vis.requeue_vertex( u, g );

    if( x_near_pred != boost::graph_traits< Graph >::null_vertex() ) {
      Vertex x_near_other = boost::graph_traits< Graph >::null_vertex();
      EdgeProp ep_other;
      connect_vertex( p_new_pred, x_near_pred, ep_pred, x_near_other, ep_other, g, super_space, sba_vis,
                      sba_vis.m_position, sba_vis.m_distance, sba_vis.m_predecessor, sba_vis.m_fwd_distance,
                      sba_vis.m_successor, sba_vis.m_weight, select_neighborhood );
    };
    if( x_near_succ != boost::graph_traits< Graph >::null_vertex() ) {
      Vertex x_near_other = boost::graph_traits< Graph >::null_vertex();
      EdgeProp ep_other;
      connect_vertex( p_new_succ, x_near_other, ep_other, x_near_succ, ep_succ, g, super_space, sba_vis,
                      sba_vis.m_position, sba_vis.m_distance, sba_vis.m_predecessor, sba_vis.m_fwd_distance,
                      sba_vis.m_successor, sba_vis.m_weight, select_neighborhood );
    };
...
template < typename Graph, typename Topology, typename SBAStarVisitor, typename NodeConnector, typename KeyMap,
...
void generate_sbastar_no_init_impl( Graph& g, typename boost::graph_traits< Graph >::vertex_descriptor start_vertex,
                                    const Topology& super_space, SBAStarVisitor vis, NodeConnector connect_vertex,
                                    KeyMap key, PositionMap position, WeightMap weight, DensityMap density,
                                    ConstrictionMap constriction, DistanceMap distance, PredecessorMap predecessor,
  ...
  sbastar_bfs_visitor< Graph, SBAStarVisitor, MutableQueue, IndexInHeapMap, KeyMap, PositionMap, WeightMap, DensityMap,
                       ConstrictionMap, DistanceMap, PredecessorMap,
                       FwdDistanceMap > sba_bfs_vis( vis, Q, index_in_heap, key, position, weight, density,
...
void generate_sbastar_bidir_no_init_impl( Graph& g,
  ...
  sbastar_bfs_visitor< Graph, SBAStarVisitor, MutableQueue, IndexInHeapMap, KeyMap, PositionMap, WeightMap, DensityMap,
                       ConstrictionMap, DistanceMap, PredecessorMap, FwdDistanceMap,
                       SuccessorMap > sba_bfs_vis( vis, Q, index_in_heap, key, position, weight, density, constriction,
...
template < typename Graph, typename SBAStarVisitor, typename KeyMap, typename DistanceMap, typename PredecessorMap >
void initialize_sbastar_nodes( Graph& g, SBAStarVisitor vis, KeyMap key, DistanceMap distance,
...
template < typename Graph, typename Topology, typename SBAStarVisitor, typename NcSelector, typename KeyMap,
           typename PositionMap, typename WeightMap, typename DensityMap, typename ConstrictionMap,
           typename DistanceMap, typename PredecessorMap, typename FwdDistanceMap,
...
struct sbastar_bundle {
  ...
  sbastar_bundle( Graph& g, vertex_type start_vertex, const Topology& super_space, SBAStarVisitor vis,
                  NcSelector select_neighborhood, KeyMap key, PositionMap position, WeightMap weight,
      ...
      : m_g( &g ), m_start_vertex( start_vertex ), m_goal_vertex( boost::graph_traits< Graph >::null_vertex() ),
        m_super_space( &super_space ), m_vis( vis ), m_select_neighborhood( select_neighborhood ), m_key( key ),
        m_position( position ), m_weight( weight ), m_density( density ), m_constriction( constriction ),
        m_distance( distance ), m_predecessor( predecessor ), m_fwd_distance( fwd_distance ),
        m_successor( successor ){};

  sbastar_bundle( Graph& g, vertex_type start_vertex, vertex_type goal_vertex, const Topology& super_space,
                  SBAStarVisitor vis, NcSelector select_neighborhood, KeyMap key, PositionMap position,
                  WeightMap weight, DensityMap density, ConstrictionMap constriction, DistanceMap distance,
                  PredecessorMap predecessor, FwdDistanceMap fwd_distance, SuccessorMap successor = SuccessorMap() )
      : m_g( &g ), m_start_vertex( start_vertex ), m_goal_vertex( goal_vertex ), m_super_space( &super_space ),
        m_vis( vis ), m_select_neighborhood( select_neighborhood ), m_key( key ), m_position( position ),
        m_weight( weight ), m_density( density ), m_constriction( constriction ), m_distance( distance ),
...
template < typename Graph, typename Topology, typename SBAStarVisitor, typename NcSelector, typename KeyMap,
           typename PositionMap, typename WeightMap, typename DensityMap, typename ConstrictionMap,
           typename DistanceMap, typename PredecessorMap, typename FwdDistanceMap >
sbastar_bundle< Graph, Topology, SBAStarVisitor, NcSelector, KeyMap, PositionMap, WeightMap, DensityMap,
                ConstrictionMap, DistanceMap, PredecessorMap, FwdDistanceMap >
  make_sbastar_bundle( Graph& g, typename boost::graph_traits< Graph >::vertex_descriptor start_vertex,
                       const Topology& super_space, SBAStarVisitor vis, NcSelector select_neighborhood, KeyMap key,
                       PositionMap position, WeightMap weight, DensityMap density, ConstrictionMap constriction,
                       DistanceMap distance, PredecessorMap predecessor, FwdDistanceMap fwd_distance ) {
  ...
  return sbastar_bundle< Graph, Topology, SBAStarVisitor, NcSelector, KeyMap, PositionMap, WeightMap, DensityMap,
                         ConstrictionMap, DistanceMap, PredecessorMap, FwdDistanceMap >(
    g, start_vertex, super_space, vis, select_neighborhood, key, position, weight, density, constriction, distance,
...
sbastar_bundle< Graph, Topology, SBAStarVisitor, NcSelector, KeyMap, PositionMap, WeightMap, DensityMap,
  ...
  return sbastar_bundle< Graph, Topology, SBAStarVisitor, NcSelector, KeyMap, PositionMap, WeightMap, DensityMap,
                         ConstrictionMap, DistanceMap, PredecessorMap, FwdDistanceMap >(
    g, start_vertex, goal_vertex, super_space, vis, select_neighborhood, key, position, weight, density, constriction,
...
template < typename Graph, typename Topology, typename SBAStarVisitor, typename NcSelector, typename KeyMap,
           ...
           typename DistanceMap, typename PredecessorMap, typename FwdDistanceMap, typename SuccessorMap >
sbastar_bundle< Graph, Topology, SBAStarVisitor, NcSelector, KeyMap, PositionMap, WeightMap, DensityMap,
                ConstrictionMap, DistanceMap, PredecessorMap, FwdDistanceMap, SuccessorMap >
  make_sbastar_bundle( Graph& g, typename boost::graph_traits< Graph >::vertex_descriptor start_vertex,
                       typename boost::graph_traits< Graph >::vertex_descriptor goal_vertex,
                       const Topology& super_space, SBAStarVisitor vis, NcSelector select_neighborhood, KeyMap key,
                       PositionMap position, WeightMap weight, DensityMap density, ConstrictionMap constriction,
...
void generate_sbastar_no_init( const SBAStarBundle& bdl ) {
  detail::generate_sbastar_no_init_impl( *( bdl.m_g ), bdl.m_start_vertex, *( bdl.m_super_space ), bdl.m_vis,
...
void generate_lazy_sbastar_no_init( const SBAStarBundle& bdl ) {
  detail::generate_sbastar_no_init_impl( *( bdl.m_g ), bdl.m_start_vertex, *( bdl.m_super_space ), bdl.m_vis,
                                         lazy_node_connector(), bdl.m_key, bdl.m_position, bdl.m_weight, bdl.m_density,
                                         bdl.m_constriction, bdl.m_distance, bdl.m_predecessor, bdl.m_fwd_distance,
...
void generate_lazy_bnb_sbastar_no_init( const SBAStarBundle& bdl ) {
  if( bdl.m_goal_vertex == boost::graph_traits< typename SBAStarBundle::graph_type >::null_vertex() ) {
    ...
    bnb_ordering_data< typename SBAStarBundle::graph_type > bnb_data( *( bdl.m_g ), bdl.m_start_vertex,
                                                                      bdl.m_goal_vertex );
    detail::generate_sbastar_no_init_impl( *( bdl.m_g ), bdl.m_start_vertex, *( bdl.m_super_space ), bdl.m_vis,
                                           bnb_connector< typename SBAStarBundle::graph_type >( bnb_data ), bdl.m_key,
                                           bdl.m_position, bdl.m_weight, bdl.m_density, bdl.m_constriction,
                                           bdl.m_distance, bdl.m_predecessor, bdl.m_fwd_distance,
                                           bdl.m_select_neighborhood );
  };
};
