...
#define RK_PRM_MANIP_PLANNERS_MAKE_DYNAMIC_MANIP_EXTERN_INSTANCES( NDOF )                             \
  template class prm_planner< manip_dynamic_env< typename Ndof_rl_space< double, NDOF, 0 >::type > >; \
  template class prm_planner< manip_dynamic_env< typename Ndof_rl_space< double, NDOF, 1 >::type > >; \
  template class prm_planner< manip_dynamic_env< typename Ndof_rl_space< double, NDOF, 2 >::type > >;
...
void CONCAT_NAMES( dummy_prm_manip_planners_dynamic_externs_symbol_, RK_PLANNERS_DIM )(){};
};
};
