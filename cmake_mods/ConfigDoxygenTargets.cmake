...
message(STATUS "Configured Doxygen to output to: ${DOXROOT_PATH}")

if(NOT EXISTS "${DOXYFILE_INPUT}")
  set(DOXYFILE_INPUT "${CMAKE_SOURCE_DIR}/Doxyfile.in")
endif()
message(STATUS "Configured Doxygen to use configuration template file: ${DOXYFILE_INPUT}")
...
if(GIT_FOUND)
  ...
  if(${GIT_DESCRIBE_RESULT_VAL} EQUAL 0)
    ...
    mark_as_advanced(CURRENTLY_IN_GIT_REPO)
...
if(NOT CURRENTLY_IN_VERSION_CONTROL)
  ...
  if(Subversion_FOUND)
    ...
    if(${Subversion_INFO_RESULT_VAL} EQUAL 0)
      ...
      mark_as_advanced(CURRENTLY_IN_SUBVERSION_REPO)
      set(CURRENTLY_IN_VERSION_CONTROL "TRUE")
...
if(NOT CURRENTLY_IN_VERSION_CONTROL)
  ...
  if(CVS_FOUND)
    ...
    if(${CVS_STATUS_RESULT_VAL} EQUAL 0)
      ...
      mark_as_advanced(CURRENTLY_IN_CVS_REPO)
      set(CURRENTLY_IN_VERSION_CONTROL "TRUE")
...
mark_as_advanced(CURRENTLY_IN_VERSION_CONTROL)
mark_as_advanced(REPO_PRINT_VERSION_COMMAND)
mark_as_advanced(REPO_PRINT_FILE_DATE_COMMAND_STR)
...
macro(configure_doxyfile TARGET_NAME TARGET_TITLE TARGET_BRIEF TARGET_INPUT_PATHS)
  if(EXISTS "${DOXYFILE_INPUT}")
    ...
    foreach(INPUT_PATH ${TARGET_INPUT_PATHS})
      if(NOT EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/${INPUT_PATH}")
        message(STATUS "  Could NOT find path: ${CMAKE_CURRENT_SOURCE_DIR}/${INPUT_PATH}")
      else()
        set(DOXY_TARGET_INPUT_PATH "${DOXY_TARGET_INPUT_PATH} ${CMAKE_CURRENT_SOURCE_DIR}/${INPUT_PATH}")
        message(STATUS "  Found path: ${CMAKE_CURRENT_SOURCE_DIR}/${INPUT_PATH}")
      endif()
    ...
    configure_file("${DOXYFILE_INPUT}" "${DOXROOT_PATH}/${TARGET_NAME}_Doxyfile" @ONLY)
    message(STATUS "Configured Doxyfile: '${DOXYFILE_INPUT}' --> '${DOXROOT_PATH}/${TARGET_NAME}_Doxyfile'.")
  else()
    message(STATUS "WARNING : The '${DOXYFILE_INPUT}' file does not exist!")
  endif()
  ...
  if(WIN32)
    ...
    if(QT4_FOUND)
      ...
      if(QT_QHELPGENERATOR_EXECUTABLE)
        ...
        mark_as_advanced(QT_QHELPGENERATOR_EXECUTABLE)
      else()
