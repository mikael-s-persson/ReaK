/**
 * \file runge_kutta5_integrator_sys.h
 *
...
#ifndef REAK_CONTROL_INTEGRATORS_RUNGE_KUTTA5_INTEGRATOR_SYS_H_
#define REAK_CONTROL_INTEGRATORS_RUNGE_KUTTA5_INTEGRATOR_SYS_H_
...
void runge_kutta5_integrate_impl(
  ...
  while (((time_step > 0.0) && (t < end_time)) ||
    ...
    end_point = space.adjust(end_point, (276165.0 / 351520.0) * k1 -
                                            (1250865.0 / 351520.0) * k2 +
    ...
    end_point =
        space.adjust(w, (439.0 / 216.0) * k1 - 8.0 * k2 +
    ...
    end_point =
        space.adjust(w, 2.0 * k2 - (8.0 / 27.0) * k1 - (3544.0 / 2565.0) * k3 +
    ...
    dp = sys.get_state_derivative(space, end_point, u_wp.second.pt, t);
    end_point =
        space.adjust(w, (16.0 / 135.0) * k1 + (6656.0 / 12825.0) * k3 +
                            (28561.0 / 56430.0) * k4 - (9.0 / 50.0) * k5 +
                            (2.0 * time_step / 55.0) * dp);
...
class runge_kutta5_integrator_factory : public named_object {
 public:
  using self = runge_kutta5_integrator_factory<TemporalSpace, StateSpaceSystem,
                                               InputTrajectory>;
 ...
 public:
  ...
  runge_kutta5_integrator_factory() : runge_kutta5_integrator_factory("") {}
  ...
  *******************************************************************************/
  ...
  RK_RTTI_MAKE_CONCRETE_1BASE(self, 0xC2431004, 1,
                              "runge_kutta5_integrator_factory", named_object)
};

}  // namespace ReaK::ctrl

#endif  // REAK_CONTROL_INTEGRATORS_RUNGE_KUTTA5_INTEGRATOR_SYS_H_
