/**
 * \file adams_BM3_integrator_sys.h
 *
...
#ifndef REAK_CONTROL_INTEGRATORS_ADAMS_BM3_INTEGRATOR_SYS_H_
#define REAK_CONTROL_INTEGRATORS_ADAMS_BM3_INTEGRATOR_SYS_H_
...
void adams_BM3_integrate_impl(
  ...
  while (((time_step > 0.0) && (t < end_time)) ||
    ...
    prevF2 = prevF1;
    prevF1 = dp;
    end_point = space.adjust(end_point, (23.0 * time_step / 12.0) * prevF1 -
                                            (16.0 * time_step / 12.0) * prevF2 +
                                            (5.0 * time_step / 12.0) * prevF3);
    t += time_step;
    u_wp = u_traj.move_time_diff_from(u_wp, time_step);
    ...
    for (std::size_t j = 0; j < correction_count; ++j) {
      end_point = space.adjust(prevY1, (5.0 * time_step / 12.0) * dp +
                                           (8.0 * time_step / 12.0) * prevF1 -
...
class adams_BM3_integrator_factory : public named_object {
 public:
  using self = adams_BM3_integrator_factory<TemporalSpace, StateSpaceSystem,
                                            InputTrajectory>;
 ...
 public:
  ...
  class extrapolator_type {
   ...
   public:
    ...
    point_type get_point_at_time(double end_time) const {
      ...
      detail::adams_BM3_integrate_impl(
   ...
   */
  explicit adams_BM3_integrator_factory(
  ...
  adams_BM3_integrator_factory() : adams_BM3_integrator_factory("") {}
  ...
  *******************************************************************************/
  ...
  RK_RTTI_MAKE_CONCRETE_1BASE(self, 0xC2431007, 1,
                              "adams_BM3_integrator_factory", named_object)
};

}  // namespace ReaK::ctrl

#endif  // REAK_CONTROL_INTEGRATORS_ADAMS_BM3_INTEGRATOR_SYS_H_
