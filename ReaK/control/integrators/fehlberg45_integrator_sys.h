/**
 * \file fehlberg45_integrator_sys.h
 *
...
#ifndef REAK_CONTROL_INTEGRATORS_FEHLBERG45_INTEGRATOR_SYS_H_
#define REAK_CONTROL_INTEGRATORS_FEHLBERG45_INTEGRATOR_SYS_H_
...
void fehlberg45_integrate_impl(
  ...
  while (((time_step > 0.0) && (t < end_time)) ||
    ...
    end_point =
        space.adjust(prevY, (1932.0 / 2197.0) * k1 - (7200.0 / 2197.0) * k2 +
                                (7296.0 / 2197.0) * k3);
    ...
    end_point =
        space.adjust(prevY, (439.0 / 216.0) * k1 - 8.0 * k2 +
    ...
    end_point = space.adjust(
        prevY, 2.0 * k2 - (8.0 / 27.0) * k1 - (3544.0 / 2565.0) * k3 +
    ...
    vect_n<double> err_vect = to_vect<double>(
        360.0 * k1 - (128.0 / 4275.0) * k3 - (2197.0 / 75240.0) * k4 +
    ...
    if (Rmax > tolerance) {
      ...
      u_wp = u_traj.move_time_diff_from(u_wp, 0.5 * time_step);
      end_point =
          space.adjust(prevY, (25.0 / 216.0) * k1 + (1408.0 / 2565.0) * k3 +
...
class fehlberg45_integrator_factory : public named_object {
 public:
  using self = fehlberg45_integrator_factory<TemporalSpace, StateSpaceSystem,
                                             InputTrajectory>;
 ...
 public:
  ...
  class extrapolator_type {
   ...
   public:
    ...
    point_type get_point_at_time(double end_time) const {
      ...
      detail::fehlberg45_integrate_impl(
   ...
   */
  explicit fehlberg45_integrator_factory(
  ...
  fehlberg45_integrator_factory() : fehlberg45_integrator_factory("") {}
  ...
  *******************************************************************************/
  ...
  RK_RTTI_MAKE_CONCRETE_1BASE(self, 0xC2431005, 1,
                              "fehlberg45_integrator_factory", named_object)
};

}  // namespace ReaK::ctrl

#endif  // REAK_CONTROL_INTEGRATORS_FEHLBERG45_INTEGRATOR_SYS_H_
