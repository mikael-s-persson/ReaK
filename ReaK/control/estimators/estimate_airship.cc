...
#include "ReaK/control/systems/airship_assembled_models.h"
#include "ReaK/math/lin_alg/vect_alg.h"

#include "ReaK/control/estimators/invariant_kalman_filter.h"
#include "ReaK/control/estimators/tsos_aug_inv_kalman_filter.h"
...
int main(int argc, char** argv) {
  ...
  a_params.IMU_position =
      vect<double, 3>(0.0, 0.0, 0.0);  // relative to body-fixed frame.
  a_params.IMU_orientation = quaternion<double>(
      vect<double, 4>(1.0, 0.0, 0.0, 0.0));  // relative to body-fixed frame.

  // ----------- Configs of sonars -----------

  sonars_in_room_output_model sonars(6);
  sonars.lower_corner =
      vect<double, 3>(0.0, 0.0, 0.0);  // lower-corner of room, in meters.
  sonars.upper_corner =
      vect<double, 3>(0.0, 0.0, 0.0);  // upper-corner of room, in meters.

  sonars.sonar_pos[0] =
      vect<double, 3>(0.0, 0.0, 0.0);  // relative position of sonar
  sonars.sonar_dir[0] =
      vect<double, 3>(0.0, 0.0, -1.0);  // relative direction of sonar
  ...
  tryphon_sgam_me sys(
      ...
      arithmetic_tuple<sonars_in_room_output_model, sat_gyros_output_model,
          ...
          sonars, sat_gyros_output_model(), sat_accelerometer_output_model(),
          sat_magnetometer_output_model()),
      0.01);
