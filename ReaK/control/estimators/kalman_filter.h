...
#ifndef REAK_CONTROL_CONTROLLERS_KALMAN_FILTER_H_
#define REAK_CONTROL_CONTROLLERS_KALMAN_FILTER_H_
 ...
 * \tparam LinearSystem A discrete state-space system modeling the DiscreteLinearSSS
 *         at least as a DiscreteLinearizedSystemType.
 * \tparam StateSpaceType A topology type on which the state-vectors can reside.
 * \tparam BeliefState A belief state with a unimodular gaussian representation.
...
void kalman_predict(
  ...
  sys.get_state_transition_blocks(A, B, state_space, t, t + sys.get_time_step(),
                                  x, b_x.get_mean_state(), b_u.get_mean_state(),
                                  b_u.get_mean_state());
  b_x.set_covariance(
      CovType((A * b_x.get_covariance().get_matrix() * transpose_view(A)) +
 ...
 * \tparam StateSpaceType A topology type on which the state-vectors can reside, should model
 *         the pp::TopologyConcept.
 * \tparam BState A belief state type modeling the ContinuousBeliefStateConcept with
...
struct KF_belief_transfer {
  using self = KF_belief_transfer<KFTransferFactory>;
  using state_space_system = typename KFTransferFactory::state_space_system;
   ...
   */
  explicit KF_belief_transfer(const KFTransferFactory* aFactory = nullptr)
      : factory(aFactory) {}
  ...
  pp::topology_point_type_t<BSpace> get_next_belief(
      const BSpace& b_space, pp::topology_point_type_t<BSpace> b,
   ...
   */
  template <BeliefSpace BSpace>
  pp::topology_point_type_t<BSpace> predict_belief(
      const BSpace& b_space, pp::topology_point_type_t<BSpace> b,
   ...
   */
  template <BeliefSpace BSpace>
  pp::topology_point_type_t<BSpace> prediction_to_ML_belief(
      const BSpace& b_space, pp::topology_point_type_t<BSpace> b,
      const time_type& t, const input_type& u) const {
    kalman_update(
        *(factory->get_state_space_system()), b_space.get_state_topology(), b,
   ...
   */
  template <BeliefSpace BSpace>
  pp::topology_point_type_t<BSpace> predict_ML_belief(
      const BSpace& b_space, pp::topology_point_type_t<BSpace> b,
      const time_type& t, const input_type& u) const {
    input_belief_type b_u(
        u, covariance_type(factory->get_input_disturbance_cov()));
    kalman_predict(*(factory->get_state_space_system()),
...
class KF_belief_transfer_factory : public serializable {
 public:
  using self = KF_belief_transfer_factory<LinearSystem>;
  using predictor_type = KF_belief_transfer<self>;
 ...
 public:
   ...
   */
  explicit KF_belief_transfer_factory(const state_space_system_ptr& aSys,
                                      matrix_type aQ = matrix_type(),
                                      matrix_type aR = matrix_type())
      : sys(aSys), Q(std::move(aQ)), R(std::move(aR)) {}

  KF_belief_transfer_factory()
      : KF_belief_transfer_factory(state_space_system_ptr()) {}
  ...
  *******************************************************************************/
  ...
  RK_RTTI_MAKE_ABSTRACT_1BASE(self, 0xC2320001, 1, "KF_belief_transfer_factory",
                              serializable)
};

}  // namespace ReaK::ctrl

#endif  // REAK_CONTROL_CONTROLLERS_KALMAN_FILTER_H_
