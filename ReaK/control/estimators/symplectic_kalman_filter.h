/**
 * \file symplectic_kalman_filter.h
 *
 ...
 * no way from the regular Kalman filter (see kalman_filter.h).
 *
...
#ifndef REAK_CONTROL_CONTROLLERS_SYMPLECTIC_KALMAN_FILTER_H_
#define REAK_CONTROL_CONTROLLERS_SYMPLECTIC_KALMAN_FILTER_H_
...
template <typename LinearSystem, typename StateSpaceType, typename BeliefState,
          typename InputBelief, FullyWritableMatrix PredictionCovTransMatrix>
void symplectic_kalman_predict(
  ...
  sys.get_state_transition_blocks(A, B, state_space, t, t + sys.get_time_step(),
                                  x, b.get_mean_state(), b_u.get_mean_state(),
                                  b_u.get_mean_state());
  ...
  b_x.set_covariance(CovType(
      MatType(
          mat_const_sub_block<PredictionCovTransMatrix>(Tc, N, 2 * N, 0, 0) *
          mat_const_ref_vert_cat<MatType, MatType>(X, Y)),
      MatType(mat_const_sub_block<PredictionCovTransMatrix>(Tc, N, N, N, N) *
              Y)));
}
...
void symplectic_kalman_update(
  ...
  mat<ValueType, mat_structure::symmetric> M =
      YC * b_z.get_covariance().get_inverse_matrix() * C;
  linsolve_QR(Y, YC);
...
struct SKF_belief_transfer {
  ...
  /// Holds the prediction covariance transformation matrix.
  mat<double, mat_structure::square> Tc;
  /// Holds the updating covariance transformation matrix.
   ...
   */
  SKF_belief_transfer(const SKFTransferFactory* aFactory,
    ...
    symplectic_kalman_predict(*(factory->get_state_space_system()),
                              b_space.get_state_topology(), b, b_u, Tc, t);
    ...
    symplectic_kalman_update(
        *(factory->get_state_space_system()), b_space.get_state_topology(), b,
        b_u, b_y, Tm, t + factory->get_state_space_system()->get_time_step());
  }
...
class SKF_belief_transfer_factory : public serializable {
 public:
  using self = SKF_belief_transfer_factory<LinearSystem>;
  ...
  *******************************************************************************/
  ...
  RK_RTTI_MAKE_ABSTRACT_1BASE(self, 0xC2320003, 1,
                              "SKF_belief_transfer_factory", serializable)
};

}  // namespace ReaK::ctrl

#endif  // REAK_CONTROL_CONTROLLERS_SYMPLECTIC_KALMAN_FILTER_H_
