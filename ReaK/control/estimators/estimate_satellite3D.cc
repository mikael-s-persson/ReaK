...
#include "ReaK/control/systems/augmented_to_state_mapping.h"
...
void batch_KF_meas_predict_with_predictor(
  ...
  auto predictor = std::make_shared<BeliefPredTrajType>(
...
int do_required_tasks(
  ...
  if (absl::GetFlag(FLAGS_generate_meas_file) &&
      (absl::GetFlag(FLAGS_xml) + absl::GetFlag(FLAGS_protobuf) +
       ...
       0)) {
    traj_ptr =
        std::make_shared<RecTrajType>(std::make_shared<sat3D_temp_space_type>(
