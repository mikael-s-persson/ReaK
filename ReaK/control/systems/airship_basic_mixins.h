...
#ifndef REAK_CONTROL_SYSTEMS_AIRSHIP_BASIC_MIXINS_H_
#define REAK_CONTROL_SYSTEMS_AIRSHIP_BASIC_MIXINS_H_
...
class airship_parameter_pack : public named_object {
 public:
  ...
  bool use_hot_del_q_terms{false};
  bool use_momentum_transfer_terms{true};
  ...
  void finalize_parameters() {
    if ((effective_J.get_row_count() != 3) ||
        (effective_mass < std::numeric_limits<double>::epsilon())) {
      throw system_incoherency(
          "Inertial information is improper in the airship_parameter_pack!");
    }
    ...
    try {
      ...
      throw system_incoherency(
          "Inertial tensor is singular in the airship_parameter_pack!");
    }
  }
  ...
  *******************************************************************************/
  ...
  RK_RTTI_MAKE_CONCRETE_1BASE(airship_parameter_pack, 0xC2310020, 1,
                              "airship_parameter_pack", named_object)
};
 ...
 public:
  state_space_type create_state_space() const {
    return pp::make_se3_space(
        ...
        vect<double, 3>(-std::numeric_limits<double>::infinity(),
  ...
  void get_zero_state(point_type& x) const {
    x = point_type(make_arithmetic_tuple(vect<double, 3>(0.0, 0.0, 0.0),
                                         vect<double, 3>(0.0, 0.0, 0.0)),
                   make_arithmetic_tuple(unit_quat<double>(),
                                         vect<double, 3>(0.0, 0.0, 0.0)));
  }
  ...
  std::size_t get_actual_state_start_index() const {
  ...
  void add_state_transition_blocks(
    ...
    const point_type& x0_se3 =
        params.get_state_models()
            .template get_state_for_system<satellite_state_model>(p_0);
    const point_type& x1_se3 =
        params.get_state_models()
            .template get_state_for_system<satellite_state_model>(p_1);
    auto& sys_params = params.get_system_parameters();
    double dt = t_1 - t_0;

    const std::pair p_r(inv_corr_start_index, inv_corr_start_index + 3);
    const std::pair v_r(inv_corr_start_index + 3, inv_corr_start_index + 6);
    const std::pair q_r(inv_corr_start_index + 6, inv_corr_start_index + 9);
    const std::pair w_r(inv_corr_start_index + 9, inv_corr_start_index + 12);
    ...
    mat<double, mat_structure::square> R_0_1(
        (invert(get_quaternion(x1_se3).as_rotation()) *
         get_quaternion(x0_se3).as_rotation())
            .getMat());
    mat<double, mat_structure::square> JRJ(sys_params.effective_J_inv * R_0_1 *
                                           sys_params.effective_J);
    ...
    if (sys_params.use_hot_del_q_terms) {
      vect<double, 3> l_net_0 =
          sys_params.effective_J * get_ang_velocity(x0_se3);
      vect<double, 3> l_net_1 =
          sys_params.effective_J * get_ang_velocity(x1_se3);
      ...
      sub(A)(w_r, w_r) += (0.5 * dt) * sys_params.effective_J_inv *
                          mat<double, mat_structure::skew_symmetric>(l_net_1) *
                          R_0_1;
    }
  }
  ...
  void apply_correction_to_state(
    ...
    point_type& x_c_se3 =
        params.get_state_models()
            .template get_state_for_system<satellite_state_model>(x_c);
    auto& sys_params = params.get_system_parameters();

    unit_quat<double> q_diff =
        exp(0.5 * vect<double, 3>(c[inv_corr_start_index + 6],
                                  c[inv_corr_start_index + 7],
    ...
    vect<double, 3> w_new = sys_params.effective_J_inv *
                            (invert(q_diff).as_rotation() *
                             (sys_params.effective_J * get_ang_velocity(x_se3) +
                              vect<double, 3>(c[inv_corr_start_index + 9],
                                              c[inv_corr_start_index + 10],
                                              c[inv_corr_start_index + 11])));

    x_c_se3 = point_type(
        make_arithmetic_tuple(
            get_position(x_se3) + vect<double, 3>(c[inv_corr_start_index],
                                                  c[inv_corr_start_index + 1],
                                                  c[inv_corr_start_index + 2]),
            get_velocity(x_se3) + vect<double, 3>(c[inv_corr_start_index + 3],
                                                  c[inv_corr_start_index + 4],
                                                  c[inv_corr_start_index + 5])),
...
class near_buoyancy_state_model : public named_object {
 ...
 public:
  state_space_type create_state_space() const {
    return pp::line_segment_topology<double>(
        "mass_imbalance_param_space", 0.0,
        std::numeric_limits<double>::infinity());
  }
  ...
  near_buoyancy_state_model() = default;
  ...
  void add_to_fly_weight_params(
    ...
    const point_type dm =
        params.get_state_models()
            .template get_state_for_system<near_buoyancy_state_model>(x);
    auto& sys_params = params.get_system_parameters();
  ...
  void apply_correction_to_state(
    ...
    point_type& dm_c =
        params.get_state_models()
            .template get_state_for_system<near_buoyancy_state_model>(x_c);
  ...
  *******************************************************************************/
  ...
  RK_RTTI_MAKE_CONCRETE_1BASE(near_buoyancy_state_model, 0xC2310022, 1,
                              "near_buoyancy_state_model", named_object)
};
...
class eccentricity_state_model : public named_object {
 ...
 public:
  ...
  eccentricity_state_model() = default;
  ...
  void add_to_fly_weight_params(
    ...
    const point_type& r =
        params.get_state_models()
            .template get_state_for_system<eccentricity_state_model>(x);
    auto& sys_params = params.get_system_parameters();
    mat<double, mat_structure::skew_symmetric> r_cross(r);
    sys_params.effective_J -= mat<double, mat_structure::symmetric>(
        sys_params.effective_mass * r_cross * r_cross);
  };
  ...
  void add_state_difference(
    ...
    vect<double, 3> gt_impulse =
        ...
        (sys_params.effective_J_inv *
         (r % (invert(q) * sys_params.gravity_acc_vect)));
    ...
    if (sys_params.use_momentum_transfer_terms) {
      ...
      try {
        mat<double, mat_structure::symmetric> X(
            mat_ident<double>(3) +
            sys_params.effective_mass *
                (r_cross * sys_params.effective_J_inv * r_cross));
        mat<double, mat_structure::rectangular> b(3, 1);
        slice(b)(range(0, 3), 0) = l_transfer;
        linsolve_Cholesky(X, b, 1e-6);
        l_transfer = slice(b)(range(0, 3),
                              0);  // <-- commit change if cholesky succeeded.
      } catch (...) { /* if cholesky failed, no HOT adjustment is applied */
      }
  ...
  void add_state_transition_blocks_for_dm(
      MatrixA& A, MatrixB& B, const FlyWeight& params,
      const StateSpaceType& space, double dt,
    ...
    if constexpr (FlyWeight::state_models_type::template has_system_v<
      ...
      const std::size_t m_r =
          params.get_state_models()
              .template get_system<near_buoyancy_state_model>()
              .get_inv_corr_start_index();
      ...
      if (sys_params.use_momentum_transfer_terms) {
        ...
        try {
          ...
          linsolve_Cholesky(X, b, 1e-6);
          rJrg = slice(b)(range(0, 3),
                          0);  // <-- commit change if cholesky succeeded.
        } catch (...) {        /* if cholesky failed, no HOT adjustment */
        }
        rJrg = R_0 * rJrg;
        rJrg *= transfer_frac;
      ...
      if (sys_params.use_momentum_transfer_terms) {
        //  here is the adjustment for the HOTs:
        try {
          mat<double, mat_structure::symmetric> Y(
              mat_ident<double>(3) +
              (sys_params.effective_mass * transfer_frac) *
  ...
  void add_state_transition_blocks(
    ...
    const vect<double, 3>& r =
        params.get_state_models()
            .template get_state_for_system<eccentricity_state_model>(p_0);
    mat<double, mat_structure::skew_symmetric> r_cross(r);
    ...
    const std::pair r_r(inv_corr_start_index, inv_corr_start_index + 3);
    ...
    vect<double, 3> local_g =
        dt * (transpose_view(R_0) * sys_params.gravity_acc_vect);
    mat<double, mat_structure::skew_symmetric> local_g_cross(local_g);

    // The transfer fraction reflects the fact that you can't have the linear momentum of the airflow to transfer as
    // angular momentum of the airship. Or can you???
    double transfer_frac = sys_params.effective_mass /
                           (sys_params.effective_mass + sys_params.added_mass);

    if (sys_params.use_momentum_transfer_terms) {
      // p/v-r
      mat<double, mat_structure::skew_symmetric> local_Jrg_cross(
          sys_params.effective_J_inv * (r % local_g));
      mat<double, mat_structure::square> delv(
          sys_params.effective_mass *
          (local_Jrg_cross +
           r_cross * sys_params.effective_J_inv * local_g_cross));
    ...
    add_state_transition_blocks_for_dm(A, B, params, space, dt, R_0, R_0_1, r,
                                       r_cross, local_g);
  ...
  void apply_correction_to_state(
    ...
    point_type& r_c =
        params.get_state_models()
            .template get_state_for_system<eccentricity_state_model>(x_c);

    r_c = r + vect<double, 3>(c[inv_corr_start_index],
                              c[inv_corr_start_index + 1],
                              c[inv_corr_start_index + 2]);
  }
  ...
  *******************************************************************************/
  ...
  RK_RTTI_MAKE_CONCRETE_1BASE(eccentricity_state_model, 0xC2310023, 1,
                              "eccentricity_state_model", named_object)
};
...
class cross_inertia_state_model : public named_object {
 ...
 public:
  ...
  cross_inertia_state_model() = default;
  ...
  void add_to_fly_weight_params(
    ...
    const point_type& s =
        params.get_state_models()
            .template get_state_for_system<cross_inertia_state_model>(x);
    auto& sys_params = params.get_system_parameters();
    sys_params.effective_J +=
        mat<double, mat_structure::symmetric>(0.0, s[0], s[1], 0.0, s[2], 0.0);
  }
  ...
  void add_state_transition_blocks(
    ...
    const std::pair s_r(inv_corr_start_index, inv_corr_start_index + 3);
    ...
    mat<double, mat_structure::square> del_sig_0(
        w_0[1], w_0[2], 0.0, w_0[0], 0.0, w_0[2], 0.0, w_0[0], w_0[1]);
    mat<double, mat_structure::square> del_sig_1(
        w_1[1], w_1[2], 0.0, w_1[0], 0.0, w_1[2], 0.0, w_1[0], w_1[1]);
    mat<double, mat_structure::square> delw(sys_params.effective_J_inv *
                                            (R_0_1 * del_sig_0 - del_sig_1));
    sub(A)(w_r, s_r) += delw;
  ...
  void apply_correction_to_state(
    ...
    point_type& s_c =
        params.get_state_models()
            .template get_state_for_system<cross_inertia_state_model>(x_c);

    s_c = s + vect<double, 3>(c[inv_corr_start_index],
                              c[inv_corr_start_index + 1],
                              c[inv_corr_start_index + 2]);
  }
  ...
  *******************************************************************************/
  ...
  RK_RTTI_MAKE_CONCRETE_1BASE(cross_inertia_state_model, 0xC2310026, 1,
                              "cross_inertia_state_model", named_object)
};
...
class sat_position_output_model : public named_object {
 ...
 public:
  sat_position_output_model() = default;
  ...
  void add_output_function_blocks(
    ...
    const std::size_t sat3d_state_index =
        params.get_state_models()
            ...
            .get_inv_corr_start_index();
    const std::pair p_r(sat3d_state_index, sat3d_state_index + 3);
    const std::pair pm_r(inv_start_index, inv_start_index + 3);
  ...
  *******************************************************************************/
  ...
  RK_RTTI_MAKE_CONCRETE_1BASE(sat_position_output_model, 0xC2310029, 1,
                              "sat_position_output_model", named_object)
};
...
class sat_quaternion_output_model : public named_object {
 ...
 public:
  sat_quaternion_output_model() = default;
  ...
  void set_inv_err_from_output(
    ...
    unit_quat<double> q_diff =
        ...
        unit_quat<double>(y[start_index], y[start_index + 1],
  ...
  void add_output_function_blocks(
    ...
    const std::size_t sat3d_state_index =
        params.get_state_models()
            ...
            .get_inv_corr_start_index();
    const std::pair q_r(sat3d_state_index + 6, sat3d_state_index + 9);
    const std::pair qm_r(inv_start_index, inv_start_index + 3);

    sub(C)(qm_r, q_r) += mat_ident<double>(
  ...
  *******************************************************************************/
  ...
  RK_RTTI_MAKE_CONCRETE_1BASE(sat_quaternion_output_model, 0xC231002A, 1,
                              "sat_quaternion_output_model", named_object)
};

}  // namespace ReaK::ctrl

#endif  // REAK_CONTROL_SYSTEMS_AIRSHIP_BASIC_MIXINS_H_
