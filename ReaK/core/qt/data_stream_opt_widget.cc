...
#include "ReaK/core/qt/data_stream_opt_widget.h"
...
DataStreamOptWidget::DataStreamOptWidget(recorder::data_stream_options aDataOpt,
                                         QWidget* parent, Qt::WindowFlags flags)
    : QDockWidget(tr("Data Stream"), parent, flags),
      ui(new Ui::DataStreamOpt()),
      data_opt(aDataOpt),
      mode(allow_all) {
  ...
  connect(ui->actionUpdateURI, SIGNAL(triggered()), this,
          SLOT(onUpdateURIAndDataOpt()));
  connect(ui->Columns_edit, SIGNAL(textChanged()), this,
          SLOT(onUpdateDataOptNames()));
  connect(ui->URI_edit, SIGNAL(textChanged(QString)), this,
          SLOT(onUpdateFieldsAndDataOpt()));
...
void DataStreamOptWidget::onUpdateURIAndDataOpt() {
  ...
  switch (mode) {
    case network_only:
      if (ui->Format_combo->currentIndex() < 5) {
        QMessageBox::information(this, "Format not allowed!",
                                 "Sorry, only network streams are allowed! "
                                 "Please choose a network data-stream type.",
                                 QMessageBox::Ok);
        onUpdateFieldsAndDataOpt();
        return;
      };
    ...
    case file_only:
      if (ui->Format_combo->currentIndex() >= 5) {
        QMessageBox::information(this, "Format not allowed!",
                                 "Sorry, only file streams are allowed! Please "
...
void DataStreamOptWidget::setEnabledFreqSetting(bool aEnabled) {
  ui->Freq_spin->setEnabled(aEnabled);
};

void DataStreamOptWidget::setEnabledBufferSetting(bool aEnabled) {
  ui->Buffer_spin->setEnabled(aEnabled);
};

void DataStreamOptWidget::setEnabledTimeSyncSetting(bool aEnabled) {
  ui->TimeSync_edit->setEnabled(aEnabled);
};
