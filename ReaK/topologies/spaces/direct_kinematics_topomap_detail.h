/*
 * \file direct_kinematics_topomap_detail.h
 *
 * DETAILS OF:
...
#ifndef REAK_TOPOLOGIES_SPACES_DIRECT_KINEMATICS_TOPOMAP_DETAIL_H_
#define REAK_TOPOLOGIES_SPACES_DIRECT_KINEMATICS_TOPOMAP_DETAIL_H_
    ...
    *(model->getFrame3D(f3d_i++)) = get_frame_3D(pt);
    ...
    arithmetic_tuple_details::tuple_for_each(
        pt, space_in, [&](const auto& pt_elem, const auto& space_in_elem) {
          write_one_joint_coord_impl(pt_elem, space_in_elem, gen_i, f2d_i,
                                     f3d_i, model);
          return int{};
        });
  }
  ...
  if constexpr (is_normal_joint_space_v<InSpace>) {
    ...
    arithmetic_tuple_details::tuple_for_each(
        pt, space_in, [&](auto& pt_elem, const auto& space_in_elem) {
          read_one_dependent_coord_impl(pt_elem, space_in_elem, gen_i, f2d_i,
                                        f3d_i, model);
          return int{};
        });
  }
...
#endif  // REAK_TOPOLOGIES_SPACES_DIRECT_KINEMATICS_TOPOMAP_DETAIL_H_
