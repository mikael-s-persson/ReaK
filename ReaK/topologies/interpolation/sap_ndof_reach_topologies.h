/**
 * \file sap_Ndof_reach_topologies.h
 *
 *
...
#ifndef REAK_TOPOLOGIES_INTERPOLATION_SAP_NDOF_REACH_TOPOLOGIES_H_
...
struct sap_Ndof_reach_topo_impl {
  ...
  using point_type = typename topology_traits<BaseTopology>::point_type;
  using point_difference_type =
      typename topology_traits<BaseTopology>::point_difference_type;

  using rt_metric_type = sap_Ndof_reach_time_metric<time_topology>;
  using sampler_type =
      generic_sampler<sap_Ndof_rate_limited_sampler<time_topology>,
                      BaseTopology>;

  static rt_metric_type make_rt_metric(const BaseTopology& /*unused*/) {
    return {};
  }

  static sampler_type make_sampler(const BaseTopology& /*unused*/) {
    return sampler_type();
  }
  ...
  static point_type move_pt_toward(const BaseTopology& b_space,
                                   ...
                                   const point_type& b) {
    try {
      generic_interpolator_impl<sap_Ndof_interpolator, BaseTopology,
                                time_topology>
          interp;
  ...
  static double get_proper_distance(const BaseTopology& b_space,
    ...
    sap_Ndof_reach_time_metric<time_topology, true> p_rt_dist(rt_dist);
...
struct sap_Ndof_reach_topo_impl<BaseTopology, true> {
  ...
  using rt_metric_type = sap_Ndof_reach_time_metric<base_time_topo>;
  using sampler_type =
      generic_sampler<sap_Ndof_rate_limited_sampler<base_time_topo>,
                      base_space_topo>;

  static rt_metric_type make_rt_metric(const BaseTopology& b_space) {
    return rt_metric_type(std::shared_ptr<base_time_topo>(
        &const_cast<base_time_topo&>(b_space.get_time_topology()),
        null_deleter()));
  }

  static sampler_type make_sampler(const BaseTopology& b_space) {
    return sampler_type(sap_Ndof_rate_limited_sampler<base_time_topo>(
        std::shared_ptr<base_time_topo>(
            &const_cast<base_time_topo&>(b_space.get_time_topology()),
            null_deleter())));
  }
  ...
  static point_type move_pt_toward(const BaseTopology& b_space,
    ...
    try {
      generic_interpolator_impl<sap_Ndof_interpolator, base_space_topo,
                                base_time_topo>
          interp;
      double dt_total =
  ...
  static double get_proper_distance(const BaseTopology& b_space,
                                    ...
                                    const point_type& a, const point_type& b) {
    using std::abs;
    sap_Ndof_reach_time_metric<base_time_topo, true> p_rt_dist(rt_dist);
  ...
  static bool is_in_bounds(const BaseTopology& b_space, const point_type& a) {
    return sap_Ndof_is_in_bounds(a.pt, b_space.get_space_topology(),
...
struct sap_Ndof_reach_topo_selector {
  using type =
      sap_Ndof_reach_topo_impl<BaseTopology, is_temporal_space_v<BaseTopology>>;
};
 ...
 */
template <typename BaseTopology>
class interpolated_topology<BaseTopology, sap_Ndof_interpolation_tag>
    : public interpolated_topology_base<BaseTopology> {
 public:
  using base_type = interpolated_topology_base<BaseTopology>;
  using self = interpolated_topology<BaseTopology, sap_Ndof_interpolation_tag>;
 ...
 protected:
  using Impl =
      typename detail::sap_Ndof_reach_topo_selector<BaseTopology>::type;
  ...
  double interp_topo_get_distance(const point_type& a,
                                  const point_type& b) const override {
    return Impl::get_distance(*this, rt_dist, a, b);
  }
  double interp_topo_get_norm(const point_difference_type& dp) const override {
    return Impl::get_norm(*this, rt_dist, dp);
  }
  double interp_topo_get_proper_distance(const point_type& a,
                                         const point_type& b) const override {
    return Impl::get_proper_distance(*this, rt_dist, a, b);
  }
  double interp_topo_get_proper_norm(
      const point_difference_type& dp) const override {
    return Impl::get_proper_norm(*this, rt_dist, dp);
  }
  bool interp_topo_is_in_bounds(const point_type& a) const override {
    return Impl::is_in_bounds(*this, a);
  }
  point_type interp_topo_random_point() const override {
 ...
 public:
  ...
  interpolated_topology() : interpolated_topology(BaseTopology{}) {}
...
struct get_tagged_spatial_interpolator<sap_Ndof_interpolation_tag, SpaceType,
                                       TimeTopology> {
  using type = detail::generic_interpolator_impl<sap_Ndof_interpolator,
                                                 SpaceType, TimeTopology>;
  using pseudo_factory_type = sap_Ndof_reach_time_metric<TimeTopology>;
};

template <typename TemporalSpaceType>
struct get_tagged_temporal_interpolator<sap_Ndof_interpolation_tag,
                                        TemporalSpaceType> {
  using type = generic_interpolator<sap_Ndof_interp_factory<TemporalSpaceType>,
                                    sap_Ndof_interpolator>;
};

}  // namespace ReaK::pp

#endif  // REAK_TOPOLOGIES_INTERPOLATION_SAP_NDOF_REACH_TOPOLOGIES_H_
