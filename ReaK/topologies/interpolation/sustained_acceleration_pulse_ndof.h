...
bool sap_Ndof_is_in_bounds(const topology_point_type_t<Space>& pt,
                           const Space& space, const TimeSpace& t_space) {
  static_assert(TangentBundle<Space, TimeSpace, 0, 1, 2>);
  ...
  static_assert(BoxBoundedSpace<Space1>);
  static_assert(BoxBoundedSpace<Space2>);
  static_assert(WritableVector<Point0>);
  static_assert(WritableVector<Point1>);
  static_assert(WritableVector<Point2>);
 ...
 * \tparam Space The temporal topology on which the interpolation is done, should model TemporalSpace,
 *               with a spatial topology that is twice-differentiable (see DifferentiableSpace) and
 *               whose 1-order and 2-order derivative space has a spherical bound (see SphereBoundedSpace).
 */
template <TemporalSpace Space>
class sap_Ndof_interp_factory : public serializable {
 public:
  using self = sap_Ndof_interp_factory<Space>;
...
class sap_Ndof_interp_traj
 ...
 public:
  ...
  using base_class_type =
      interpolated_trajectory<Space, sap_Ndof_interp_factory<Space>, Metric>;
