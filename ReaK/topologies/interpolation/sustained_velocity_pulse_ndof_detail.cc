...
static void svp_Ndof_compute_interpolated_values_balanced(
  ...
  if (dt <= 0.0) {
    ...
    result_pos = start_position +
                 (start_velocity + 0.5 * dt * sgn_vp1) * dt / max_velocity;
    result_vel = start_velocity + dt * sgn_vp1;
    result_acc = sgn_vp1;
  } else if (dt < (dt_vp1 + dt_total)) {
    // in segment 2: in the cruise phase.
    double idt = dt - dt_vp1;
    double pis = start_position +
                 0.5 * (start_velocity + peak_velocity) * dt_vp1 / max_velocity;
    double pie = end_position -
    ...
    result_pos = end_position -
                 (end_velocity - 0.5 * mdt * sgn_vp2) * mdt / max_velocity;
...
void svp_Ndof_compute_interpolated_values(
  ...
  svp_Ndof_compute_interpolated_values_balanced(
      ...
      max_velocity, dt, dt_total, result_pos, result_vel, result_acc);
}

inline void svp_Ndof_compute_ramp_dist_and_time(double v1, double v2,
                                                double vmax, double& d_pos,
                                                double& dt) {
  dt = std::abs(v2 - v1);
...
struct svp_Ndof_no_cruise_calculator {
  ...
  svp_Ndof_no_cruise_calculator(double a_dp, double a_v1, double a_v2,
                                double a_vmax)
...
static double svp_Ndof_compute_min_delta_time_numsolve(
  ...
  if ((abs(end_position - start_position) < 1e-6 * max_velocity) &&
      (abs(end_velocity - start_velocity) < 1e-6 * max_velocity)) {
  ...
  if ((abs(start_velocity) > max_velocity) ||
    ...
    throw optim::infeasible_problem(
        "Violation of the velocity bounds on invocation of the SVP "
        "minimum-time solver!");
  }
  ...
  svp_Ndof_no_cruise_calculator nc_calc(end_position - start_position,
  ...
  if (delta_first_order > 0.0) {
    ...
    svp_Ndof_compute_ramp_dist_and_time(start_velocity, peak_velocity,
                                        max_velocity, dp_ramp1, dt_ramp1);
    svp_Ndof_compute_ramp_dist_and_time(peak_velocity, end_velocity,
                                        max_velocity, dp_ramp2, dt_ramp2);
  ...
  svp_Ndof_compute_ramp_dist_and_time(start_velocity, peak_velocity,
                                      max_velocity, dp_ramp1, dt_ramp1);
  svp_Ndof_compute_ramp_dist_and_time(peak_velocity, end_velocity, max_velocity,
                                      dp_ramp2, dt_ramp2);
...
static double svp_Ndof_compute_min_delta_time_closedform(
  ...
  double delta_first_order =
      ...
      (0.5 * abs(peak_velocity - end_velocity)) *
          (descended_peak_velocity + end_velocity / max_velocity) -
      (0.5 * abs(peak_velocity - start_velocity)) *
          (descended_peak_velocity + start_velocity / max_velocity);
  ...
  // this assumes that we have p1 - p0 == 0.5 / vm * ( abs(vp - v1) * (vp + v1) + abs(vp - v0) * (vp + v0) )
  // first try if vp is more in the direction (p1-p0) than both v1 and v0:
  peak_velocity = sqrt(max_velocity * abs(end_position - start_position) +
                       0.5 * start_velocity * start_velocity +
                       0.5 * end_velocity * end_velocity);
  if ((peak_velocity > sign_p1_p0 * start_velocity) &&
  ...
  if (max_velocity * abs(end_position - start_position) <
      0.5 * (start_velocity * start_velocity + end_velocity * end_velocity)) {
    // this means there exists a solution for the magnitude of vp:
    peak_velocity = sqrt(0.5 * start_velocity * start_velocity +
                         0.5 * end_velocity * end_velocity -
                         max_velocity * abs(end_position - start_position));
    if ((peak_velocity < sign_p1_p0 * start_velocity) &&
        (peak_velocity < sign_p1_p0 * end_velocity)) {
  ...
  peak_velocity = 0.0;
  throw optim::infeasible_problem(
      "The SVP minimum-time solver could not find a feasible solution!");
}
...
struct svp_Ndof_pos_diff_calculator {
  ...
  svp_Ndof_pos_diff_calculator(double a_dp, double a_v1, double a_v2,
                               double a_vmax, double a_dt)
  ...
  static void svp_Ndof_compute_peak_velocity_numsolve(
    ...
    if( ( abs(start_velocity) > max_velocity ) || ( abs(end_velocity) > max_velocity ) ) {
      peak_velocity = 0.0;
    ...
    for(double cur_vp = prev_vp - 0.02 * sign_p1_p0 * max_velocity; cur_vp * sign_p1_p0 > -1.04 * max_velocity; cur_vp -= 0.02 * sign_p1_p0 * max_velocity) {
      ...
      if( cur_pd * prev_pd < 0.0 ) {
        ...
        if( (pd_calc.get_delta_time_diff(cur_vp) >= -1e-3 * max_velocity) && ( abs(cur_pd) < 1e-3 * max_velocity ) ) {
...
void svp_Ndof_compute_peak_velocity_closedform(
    double start_position, double end_position, double start_velocity,
    double end_velocity, double& peak_velocity, double max_velocity,
    double delta_time) {
  ...
  svp_Ndof_pos_diff_calculator pd_calc(end_position - start_position,
                                       start_velocity, end_velocity,
  ...
  if (v0_v1_ts * v0_v1_ts >= 4.0 * (vm_p1_p0 + vsqr_avg)) {
    ...
    double r1 =
        0.5 *
        (v0_v1_ts + sqrt(v0_v1_ts * v0_v1_ts - 4.0 * (vm_p1_p0 + vsqr_avg))) *
        sign_p1_p0;
    if ((abs(r1) < 1.001 * max_velocity) &&
        (pd_calc.get_delta_time_diff(sign_p1_p0 * r1) >=
         -1e-3 * max_velocity) &&
        (r1 >= start_velocity * sign_p1_p0) &&
        (r1 >= end_velocity * sign_p1_p0)) {
  ...
  } else if (abs(v0_v1_ts * v0_v1_ts - 4.0 * (vm_p1_p0 + vsqr_avg)) <
             1e-5 * max_velocity) {
    // then there is a real root to the quadratic equation.
  ...
  if (end_velocity > start_velocity) {
    ...
    vsqr_avg =
        0.5 * (start_velocity * start_velocity - end_velocity * end_velocity);
    if (abs(v0_v1_ts) > 1e-6 * max_velocity) {
      peak_velocity = (vm_p1_p0 + vsqr_avg) / v0_v1_ts;
      if ((abs(peak_velocity) < 1.001 * max_velocity) &&
          (pd_calc.get_delta_time_diff(peak_velocity) >=
           -1e-3 * max_velocity) &&
          (peak_velocity >= start_velocity) &&
  ...
  if (v0_v1_ts * v0_v1_ts > 4.0 * (vsqr_avg - vm_p1_p0)) {
    ...
    double r1 =
        0.5 *
        (v0_v1_ts + sqrt(v0_v1_ts * v0_v1_ts - 4.0 * (vsqr_avg - vm_p1_p0))) *
        sign_p1_p0;
    if ((abs(r1) < 1.001 * max_velocity) &&
        (pd_calc.get_delta_time_diff(sign_p1_p0 * r1) >=
         -1e-3 * max_velocity) &&
    ...
    // then, there was no suitable root in this region.
  } else if (abs(v0_v1_ts * v0_v1_ts - 4.0 * (vsqr_avg - vm_p1_p0)) <
             1e-5 * max_velocity) {
    // then there is a real root to the quadratic equation.
    double r1 = 0.5 * v0_v1_ts * sign_p1_p0;
    if ((abs(r1) < 1.001 * max_velocity) &&
  ...
  peak_velocity = -sign_p1_p0 * max_velocity;
  throw optim::infeasible_problem(
      "The SVP peak-velocity solver could not find a solution for the given "
      "boundary conditions!");
}
