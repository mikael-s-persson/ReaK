...
class discrete_point_path : public waypoint_container<Space, Metric> {
 ...
 private:
  ...
  const_waypoint_descriptor move_away_from_impl(const_waypoint_descriptor a,
                                                double d) const {
  ...
  const_waypoint_descriptor move_fraction_away_from_impl(
      const_waypoint_descriptor a, double d) const {
    bool go_backwards = false;
    if (d < 0.0) {
      go_backwards = true;
      d = -d;
 ...
 public:
  struct point_distance_iterator {
    ...
    friend bool operator==(const point_distance_iterator& lhs,
      ...
      return ((lhs.parent == rhs.parent) &&
              (lhs.current_wpt == rhs.current_wpt));
    }
   ...
   * \param aDist The distance metric functor that the path should use.
   */
  discrete_point_path(const std::shared_ptr<topology>& aSpace,
  ...
  waypoint_pair get_waypoint_at_distance(double d) const {
    const_waypoint_descriptor start =
        move_away_from_impl(this->waypoints.begin(), d);
  ...
  *******************************************************************************/
  ...
  RK_RTTI_MAKE_CONCRETE_1BASE(self, 0xC2440013, 1, "discrete_point_path",
                              base_class_type)
};

}  // namespace ReaK::pp

#endif  // REAK_TOPOLOGIES_INTERPOLATION_DISCRETE_POINT_PATH_H_
