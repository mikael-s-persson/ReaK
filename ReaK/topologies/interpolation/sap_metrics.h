...
struct sap_reach_time_metric : public serializable {
  ...
  using self = sap_reach_time_metric<TimeSpaceType, MakeProper>;

  std::shared_ptr<TimeSpaceType> t_space;
  double tolerance;
  unsigned int maximum_iterations;

  sap_reach_time_metric(const std::shared_ptr<TimeSpaceType>& aTimeSpace =
  ...
  double operator()(const Point& a, const Point& b, const Topology& s) const {
    try {
      detail::generic_interpolator_impl<sap_interpolator, Topology,
                                        TimeSpaceType>
          interp;
  ...
  *******************************************************************************/
  ...
  RK_RTTI_MAKE_ABSTRACT_1BASE(self, 0xC241000A, 1, "sap_reach_time_metric",
                              serializable)
};
...
struct sap_reach_time_metric<TimeSpaceType, true> : public serializable {
  ...
  using self = sap_reach_time_metric<TimeSpaceType, true>;

  std::shared_ptr<TimeSpaceType> t_space;
  double tolerance;
  unsigned int maximum_iterations;

  explicit sap_reach_time_metric(
      const sap_reach_time_metric<TimeSpaceType, false>& aRHS)
  ...
  sap_reach_time_metric()
      : sap_reach_time_metric(std::make_shared<TimeSpaceType>()) {}
...
template <typename TimeSpaceType>
struct is_metric_symmetric<sap_reach_time_metric<TimeSpaceType, false>>
    : std::false_type {};

template <typename TimeSpaceType>
struct is_metric_symmetric<sap_reach_time_metric<TimeSpaceType, true>>
    : std::true_type {};

template <typename TimeSpaceType, bool MakeProper>
struct get_proper_metric_from_metric<
    sap_reach_time_metric<TimeSpaceType, MakeProper>> {
  using type = sap_reach_time_metric<TimeSpaceType, true>;
};

}  // namespace ReaK::pp

#endif  // REAK_TOPOLOGIES_INTERPOLATION_SAP_METRICS_H_
