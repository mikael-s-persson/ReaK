...
TEST(MatAlgRiccatiEq, MatDarexTests) {
  ...
  for (std::size_t i = 0; i < F_list.size(); ++i) {
    ...
    try {
      mat<double, mat_structure::rectangular> P(F_list[i].get_row_count(),
                                                F_list[i].get_col_count());
      ...
      mat<double, mat_structure::rectangular> M2_tmp(G_list[i].get_col_count(),
                                                     F_list[i].get_col_count());
...
TEST(MatAlgRiccatiEq, MatCarexTests) {
  ...
  for (std::size_t i = 0; i < A_list.size(); ++i) {
    ...
    try {
      mat<double, mat_structure::rectangular> P(A_list[i].get_row_count(),
                                                A_list[i].get_col_count());
      solve_care_problem(A_list[i], B_list[i], Q_list[i], R_list[i], P, 1e-6);

      mat<double, mat_structure::rectangular> M_tmp = R_list[i];
      mat<double, mat_structure::rectangular> M2_tmp(B_list[i].get_col_count(),
                                                     A_list[i].get_col_count());
