...
#ifndef REAK_MATH_KINETOSTATICS_POSE_2D_H_
#define REAK_MATH_KINETOSTATICS_POSE_2D_H_

#include "ReaK/math/kinetostatics/rotations_2D.h"
...
class pose_2D : public shared_object {
 public:
  ...
  pose_2D() noexcept : shared_object(), Parent(), Position(), Rotation() {}
  ...
  pose_2D(std::weak_ptr<self> aParent, const position_type& aPosition,
          const rotation_type& aRotation) noexcept
      : shared_object(),
  ...
  /// Copy-constructor.
  pose_2D(const self& aPose) noexcept = default;
 ...
 public:
  ...
  self& rotate(const rotation_type& R) noexcept {
    Rotation *= R;
    return *this;
  }
...
#endif  // REAK_MATH_KINETOSTATICS_POSE_2D_H_
