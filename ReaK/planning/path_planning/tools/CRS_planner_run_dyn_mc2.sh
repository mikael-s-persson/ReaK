...
#For temporal SAP space:
    SPACE_CONFIG =
        "--space-order 2 --interpolation-method sap --use-temporal-space "
        "--use-rate-limited-space --min-travel-dist 0.1 --max-travel-dist 1 "
        "--output-space-order 1"
...
#RUNTYPE_CONFIG = "--single-run"
    ...
    OUTPUT_DEST_CONFIG =
        "--output-path pp_results/CRS_dynamic --result-file-prefix $1"

    PLANNER_CONFIG =
        "--planner-alg rrt_star ${BASIC_PLANNER_OPT}" echo_and_run./
        ...
        PLANNER_CONFIG =
            "--planner-alg sba_star ${BASIC_PLANNER_OPT} --relaxation-factor "
            "5" echo_and_run./
            run_CRS_planner ${OUTPUT_DEST_CONFIG} ${RUNTYPE_CONFIG} ${
                PLANNER_CONFIG} ${MODEL_CONFIG} ${START_CONFIG} ${
                TARGET_CONFIG} ${SPACE_CONFIG} ${STOP_CRITERIAS} mv pp_results /
            CRS_dynamic / $1 /
            sba_star__any_adj_list_bf4_solutions.txt pp_results / CRS_dynamic /
            $1 / sba_star__any_adj_list_bf4_r05_solutions.txt mv pp_results /
            CRS_dynamic / $1 / sba_star__any_adj_list_bf4_times.txt pp_results /
            CRS_dynamic / $1 /
            sba_star__any_adj_list_bf4_r05_times.txt

                PLANNER_CONFIG =
                "--planner-alg sba_star ${BASIC_PLANNER_OPT} "
                "--relaxation-factor 5 --with-voronoi-pull --sa-temperature "
                "5" echo_and_run./
                run_CRS_planner ${OUTPUT_DEST_CONFIG} ${RUNTYPE_CONFIG} ${
                    PLANNER_CONFIG} ${MODEL_CONFIG} ${START_CONFIG} ${
                ...
                CRS_dynamic / $1 /
                sba_star__any_sa_adj_list_bf4_r05_t05_solutions.txt mv
                    pp_results /
                CRS_dynamic / $1 /
