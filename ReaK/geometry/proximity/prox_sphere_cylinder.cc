...
proximity_record_3D compute_proximity(const sphere& aSphere,
  ...
  if (abs(sp_c_rel[2]) <= 0.5 * cy_len) {
    ...
    result.mPoint2 =
        cy_pose.transformToGlobal(vect<double, 3>(0.0, 0.0, sp_c_rel[2]) +
                                  sp_c_proj * (cy_rad / sp_c_proj_d));
    result.mPoint1 = cy_pose.transformToGlobal(
        sp_c_rel - sp_c_proj * (sp_rad / sp_c_proj_d));
    ...
    result.mPoint2 = cy_pose.transformToGlobal(
        vect<double, 3>(sp_c_rel[0], sp_c_rel[1], fact * 0.5 * cy_len));
    result.mPoint1 = cy_pose.transformToGlobal(
        vect<double, 3>(sp_c_rel[0], sp_c_rel[1], sp_c_rel[2] - fact * sp_rad));
    ...
    vect<double, 3> rim_pt = (cy_rad / sp_c_proj_d) * sp_c_proj +
                             vect<double, 3>(0.0, 0.0, fact * 0.5 * cy_len);
...
proximity_record_3D compute_proximity(const cylinder& aCylinder,
  ...
  using std::swap;
  proximity_record_3D result =
      compute_proximity(aSphere, aPack2, aCylinder, aPack1);  // NOLINT
  swap(result.mPoint1, result.mPoint2);
  return result;
}

proximity_record_3D prox_sphere_cylinder::computeProximity(
    const shape_3D_precompute_pack& aPack1,
    const shape_3D_precompute_pack& aPack2) {
  if ((mSphere == nullptr) || (mCylinder == nullptr)) {
    return {};
  }
...
prox_sphere_cylinder::prox_sphere_cylinder(const sphere* aSphere,
                                           const cylinder* aCylinder)
    : mSphere(aSphere), mCylinder(aCylinder) {}
