...
void compute_proximity_of_point(const plane& aPlane,
  ...
  if ((pt_rel[0] > -0.5 * aPlane.getDimensions()[0]) &&
    ...
    aPointRec = aPlGblPose.transformToGlobal(
    ...
    if ((pt_rel[0] > -0.5 * aPlane.getDimensions()[0]) &&
      ...
      vect<double, 3> rim_pt = vect<double, 3>(
          pt_rel[0], fact * 0.5 * aPlane.getDimensions()[1], 0.0);
      ...
      vect<double, 3> rim_pt = vect<double, 3>(
          fact * 0.5 * aPlane.getDimensions()[0], pt_rel[1], 0.0);
      aPointRec = aPlGblPose.transformToGlobal(rim_pt);
    } else {
      // The sphere is outside one of the corners of the plane.
      vect<double, 3> rim_pt =
          vect<double, 3>(0.5 * aPlane.getDimensions()[0],
...
proximity_record_3D compute_proximity(const plane& aPlane1,
  ...
  vect<double, 3> corner =
      vect<double, 3>(0.5 * p2_dim[0], 0.5 * p2_dim[1], 0.0);
...
proximity_record_3D prox_plane_plane::computeProximity(
    const shape_3D_precompute_pack& aPack1,
    const shape_3D_precompute_pack& aPack2) {
  if ((mPlane1 == nullptr) || (mPlane2 == nullptr)) {
    return {};
  }
...
prox_plane_plane::prox_plane_plane(const plane* aPlane1, const plane* aPlane2)
    : mPlane1(aPlane1), mPlane2(aPlane2) {}
