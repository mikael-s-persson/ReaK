...
#ifndef REAK_GEOMETRY_PROXIMITY_PROX_FUNDAMENTALS_3D_H_
#define REAK_GEOMETRY_PROXIMITY_PROX_FUNDAMENTALS_3D_H_
...
struct slack_minimize_func {
  double operator()(const vect_n<double>& aX) { return aX[0]; };
};
...
struct slack_minimize_hess {
  ...
  void operator()(Matrix& H, const vect_n<double>& /*unused*/,
                  double /*unused*/, const vect_n<double>& /*unused*/) const {
...
struct cylinder_slacking_func {
  ...
  vect_n<double> operator()(const vect_n<double>& aX) const {
    using std::sqrt;
    vect_n<double> result(3);
    vect<double, 3> pt(aX[1], aX[2], aX[3]);
    vect<double, 3> pt_rel = mGblPose->transformFromGlobal(pt);
    result[0] =
        pt_rel[2] + 0.5 * aX[0] * mCylinder->getLength();  // lower-bound.
    result[1] =
        0.5 * aX[0] * mCylinder->getLength() - pt_rel[2];  // upper-bound.
    result[2] = aX[0] * mCylinder->getRadius() -
                sqrt(pt_rel[0] * pt_rel[0] + pt_rel[1] * pt_rel[1]);
    return result;
  }
};
...
struct cylinder_slacking_jac {
  ...
  // aX is the (slack (aX[0]), query-point (aX[1],aX[2],aX[3])).
  template <typename Matrix>
  void operator()(Matrix& aJac, const vect_n<double>& aX,
                  const vect_n<double>& aH) {
    using std::sqrt;
    vect<double, 3> pt(aX[1], aX[2], aX[3]);
    vect<double, 3> pt_rel = mGblPose->transformFromGlobal(pt);

    vect<double, 3> x_rel =
        mGblPose->rotateFromGlobal(vect<double, 3>(1.0, 0.0, 0.0));
    vect<double, 3> y_rel =
        mGblPose->rotateFromGlobal(vect<double, 3>(0.0, 1.0, 0.0));
    vect<double, 3> z_rel =
        mGblPose->rotateFromGlobal(vect<double, 3>(0.0, 0.0, 1.0));
    // aJac.resize(3,4);
    ...
    // result[2] = aX[0] * aX[0] * mCylinder->getRadius() * mCylinder->getRadius() - pt_rel[0] * pt_rel[0] - pt_rel[1] *
    // pt_rel[1];
...
struct ccylinder_slacking_func {
  ...
  vect_n<double> operator()(const vect_n<double>& aX) const {
    ...
    if (pt_rel[2] > 0.5 * aX[0] * mCCylinder->getLength()) {
      ...
      result[0] =
          aX[0] * mCCylinder->getRadius() -
          sqrt(pt_rel[0] * pt_rel[0] + pt_rel[1] * pt_rel[1] + tmp * tmp);
      ...
      result[0] = aX[0] * mCCylinder->getRadius() -
                  sqrt(pt_rel[0] * pt_rel[0] + pt_rel[1] * pt_rel[1]);
    }
...
struct ccylinder_slacking_jac {
  ...
  void operator()(Matrix& aJac, const vect_n<double>& aX,
    ...
    aJac(0, 1) =
        -(pt_rel[0] * x_rel[0] + pt_rel[1] * x_rel[1] + tmp * x_rel[2]) / dist;
    aJac(0, 2) =
        -(pt_rel[0] * y_rel[0] + pt_rel[1] * y_rel[1] + tmp * y_rel[2]) / dist;
    aJac(0, 3) =
        -(pt_rel[0] * z_rel[0] + pt_rel[1] * z_rel[1] + tmp * z_rel[2]) / dist;
  }
};
...
struct box_slacking_func {
  ...
  vect_n<double> operator()(const vect_n<double>& aX) const {
    ...
    vect<double, 3> pt_rel = mGblPose->transformFromGlobal(pt);
    result[0] =
        pt_rel[0] + 0.5 * aX[0] * mBox->getDimensions()[0];  // lower-bound.
    result[1] =
        pt_rel[1] + 0.5 * aX[0] * mBox->getDimensions()[1];  // lower-bound.
    result[2] =
        pt_rel[2] + 0.5 * aX[0] * mBox->getDimensions()[2];  // lower-bound.
    result[3] =
        0.5 * aX[0] * mBox->getDimensions()[0] - pt_rel[0];  // upper-bound.
    result[4] =
        0.5 * aX[0] * mBox->getDimensions()[1] - pt_rel[1];  // upper-bound.
    result[5] =
        0.5 * aX[0] * mBox->getDimensions()[2] - pt_rel[2];  // upper-bound.
    return result;
  }
};
...
struct box_slacking_jac {
  ...
  void operator()(Matrix& aJac, const vect_n<double>& aX,
    ...
    vect<double, 3> z_rel =
        mGblPose->rotateFromGlobal(vect<double, 3>(0.0, 0.0, 1.0));
    // aJac.resize(6,4);
    aJac(0, 0) = 0.5 * mBox->getDimensions()[0];
    aJac(0, 1) = x_rel[0];
    aJac(0, 2) = y_rel[0];
    aJac(0, 3) = z_rel[0];
    aJac(1, 0) = 0.5 * mBox->getDimensions()[1];
    aJac(1, 1) = x_rel[1];
    aJac(1, 2) = y_rel[1];
    aJac(1, 3) = z_rel[1];
    aJac(2, 0) = 0.5 * mBox->getDimensions()[2];
    aJac(2, 1) = x_rel[2];
    aJac(2, 2) = y_rel[2];
    aJac(2, 3) = z_rel[2];
    aJac(3, 0) = 0.5 * mBox->getDimensions()[0];
    aJac(3, 1) = -x_rel[0];
    aJac(3, 2) = -y_rel[0];
    aJac(3, 3) = -z_rel[0];
    aJac(4, 0) = 0.5 * mBox->getDimensions()[1];
    aJac(4, 1) = -x_rel[1];
    aJac(4, 2) = -y_rel[1];
    aJac(4, 3) = -z_rel[1];
    aJac(5, 0) = 0.5 * mBox->getDimensions()[2];
    aJac(5, 1) = -x_rel[2];
    aJac(5, 2) = -y_rel[2];
    aJac(5, 3) = -z_rel[2];
  }
...
struct dual_slacking_func {
  ...
  dual_slacking_func(BoundFunc1 aBound1, BoundFunc2 aBound2)
      : mBound1(aBound1), mBound2(aBound2) {}
  ...
  vect_n<double> operator()(const vect_n<double>& aX) {
    ...
    std::copy(r2.begin(), r2.end(),
              std::copy(r1.begin(), r1.end(), result.begin() + 1));
    return result;
  }
};
...
struct dual_slacking_jac {
  ...
  dual_slacking_jac(BoundJac1 aBound1, BoundJac2 aBound2)
      : mBound1(aBound1), mBound2(aBound2) {}
  ...
  void operator()(Matrix& aJac, const vect_n<double>& aX,
    ...
    aJac(0, 1) = 0.0;
    aJac(0, 2) = 0.0;
    aJac(0, 3) = 0.0;  // slack-bound.
    mat_sub_block<Matrix> aJac_s1(aJac, BoundJac1::size, 4, 1, 0);
    mat_sub_block<Matrix> aJac_s2(aJac, BoundJac2::size, 4, BoundJac1::size + 1,
                                  0);
    vect_n<double> h1(BoundJac1::size);
    vect_n<double> h2(BoundJac2::size);
    std::copy(aH.begin(), aH.begin() + BoundJac1::size, h1.begin());
    std::copy(aH.begin() + BoundJac1::size,
              aH.begin() + BoundJac1::size + BoundJac2::size, h2.begin());
...
struct cylinder_boundary_func {
  ...
  cylinder_boundary_func(const cylinder& aCylinder,
                         const pose_3D<double>& aGblPose)
      : mCylinder(&aCylinder), mGblPose(&aGblPose) {}
...
struct cylinder_boundary_jac {
  ...
  cylinder_boundary_jac(const cylinder& aCylinder,
                        const pose_3D<double>& aGblPose)
      : mCylinder(&aCylinder), mGblPose(&aGblPose) {}

  // aX is the query-direction (aX[0],aX[1],aX[2]).
  mat<double, mat_structure::square> operator()(vect<double, 3> v) const;
};
...
struct ccylinder_boundary_func {
  ...
  ccylinder_boundary_func(const capped_cylinder& aCCylinder,
                          const pose_3D<double>& aGblPose)
      : mCCylinder(&aCCylinder), mGblPose(&aGblPose) {}
...
struct ccylinder_boundary_jac {
  ...
  ccylinder_boundary_jac(const capped_cylinder& aCCylinder,
                         const pose_3D<double>& aGblPose)
      : mCCylinder(&aCCylinder), mGblPose(&aGblPose) {}
...
struct support_func_base {
  virtual ~support_func_base() = default;
...
struct cylinder_support_func : support_func_base {
  ...
  cylinder_support_func(const cylinder& aCylinder,
                        const pose_3D<double>& aGblPose)
      : mCylinder(&aCylinder), mGblPose(&aGblPose) {}

  // aX is the query-direction (aX[0],aX[1],aX[2]).
  vect<double, 3> operator()(vect<double, 3> v) const override;
};
...
struct ccylinder_support_func : support_func_base {
  ...
  ccylinder_support_func(const capped_cylinder& aCCylinder,
                         const pose_3D<double>& aGblPose)
      : mCCylinder(&aCCylinder), mGblPose(&aGblPose) {}
...
#endif  // REAK_GEOMETRY_PROXIMITY_PROX_FUNDAMENTALS_3D_H_
