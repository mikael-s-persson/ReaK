...
proximity_record_3D findProximityBoxToPoint(const box& aBox,
  ...
  bool in_x_range =
      ((pt_rel[0] > -0.5 * bx_dim[0]) && (pt_rel[0] < 0.5 * bx_dim[0]));
  bool in_y_range =
      ((pt_rel[1] > -0.5 * bx_dim[1]) && (pt_rel[1] < 0.5 * bx_dim[1]));
  bool in_z_range =
      ((pt_rel[2] > -0.5 * bx_dim[2]) && (pt_rel[2] < 0.5 * bx_dim[2]));
  bool is_inside = (in_x_range && in_y_range && in_z_range);
  if (is_inside) {
    // The point is inside the box.
    vect<double, 3> bound_dists = vect<double, 3>(
        0.5 * bx_dim[0] - abs(pt_rel[0]), 0.5 * bx_dim[1] - abs(pt_rel[1]),
        0.5 * bx_dim[2] - abs(pt_rel[2]));
...
struct ProxBoxToLineFunctor {
  ...
  ProxBoxToLineFunctor(const box& aBox, const pose_3D<double>& aBoxGblPose,
                       const vect<double, 3>& aCenter,
                       const vect<double, 3>& aTangent,
                       proximity_record_3D& aResult)
      : mBox(&aBox),
        mBoxGblPose(&aBoxGblPose),
        mCenter(aCenter),
        mTangent(aTangent),
        mResult(&aResult) {}
  ...
  double operator()(double t) const {
    (*mResult) =
        findProximityBoxToPoint(*mBox, *mBoxGblPose, mCenter + mTangent * t);
    return mResult->mDistance;
  }
};
}  // namespace detail

proximity_record_3D findProximityBoxToLine(const box& aBox,
                                           const pose_3D<double>& aBoxGblPose,
                                           const vect<double, 3>& aCenter,
                                           const vect<double, 3>& aTangent,
                                           double aHalfLength) {
  proximity_record_3D result;
  detail::ProxBoxToLineFunctor fct(aBox, aBoxGblPose, aCenter, aTangent,
                                   result);
...
vect<double, 3> cylinder_boundary_func::operator()(vect<double, 3> v) const {
  using std::abs;
  using std::sqrt;
  ...
  if (v_radius * 0.5 * cy_len > cy_rad * abs(v[2])) {
    v *= cy_rad / v_radius;
  } else {
    v *= 0.5 * cy_len / abs(v[2]);
  }
  return mGblPose->transformToGlobal(v);
}

mat<double, mat_structure::square> cylinder_boundary_jac::operator()(
    vect<double, 3> v) const {
  using std::abs;
  using std::sqrt;
  ...
  if (v_radius * 0.5 * cy_len > cy_rad * abs(v[2])) {
    ...
    // v *= 0.5 * cy_len / abs(v[2]);
    double v_z3 = v[2] * v[2] * abs(v[2]);
    Jac_local(0, 0) = 0.5 * cy_len / abs(v[2]);
    Jac_local(0, 2) = -0.5 * cy_len * v[0] * v[2] / v_z3;
    Jac_local(1, 1) = 0.5 * cy_len / abs(v[2]);
...
vect<double, 3> ccylinder_boundary_func::operator()(vect<double, 3> v) const {
  using std::abs;
  using std::sqrt;
  ...
  if (v_radius * 0.5 * cy_len > cy_rad * abs(v[2])) {
    ...
    double A =
        sqrt((cy_rad * cy_rad - 0.25 * cy_len * cy_len) * v_radius * v_radius +
             cy_rad * cy_rad * v[2] * v[2]);
    v *= (0.5 * cy_len * abs(v[2]) + A) / v_l2;
  }
  return mGblPose->transformToGlobal(v);
}

mat<double, mat_structure::square> ccylinder_boundary_jac::operator()(
    vect<double, 3> v) const {
  using std::abs;
  using std::sqrt;
  const double cy_len = mCCylinder->getLength();
  ...
  if (v_radius * 0.5 * cy_len > cy_rad * abs(v[2])) {
    ...
    double Lv_A = 0.5 * cy_len * abs(v[2]) + A;
    double F1 = R_L / A - 2.0 * Lv_A / v_l2;
    double F2 = R2 / A + 0.5 * cy_len / abs(v[2]) - 2.0 * Lv_A / v_l2;
    //       v *= Lv_A / v_l2;
...
vect<double, 3> box_boundary_func::operator()(vect<double, 3> v) const {
  using std::abs;
  const vect<double, 3> bx_dim = mBox->getDimensions();
  v = mGblPose->rotateFromGlobal(v);
  // there are three possible quadrants (facet where the support lays):
  if ((abs(v[1]) * bx_dim[0] < abs(v[0]) * bx_dim[1]) &&
      (abs(v[2]) * bx_dim[0] < abs(v[0]) * bx_dim[2])) {
    // on the x-normal plane:
    v *= 0.5 * bx_dim[0] / abs(v[0]);
  } else if ((abs(v[0]) * bx_dim[1] < abs(v[1]) * bx_dim[0]) &&
             (abs(v[2]) * bx_dim[1] < abs(v[1]) * bx_dim[2])) {
    // on the y-normal plane:
    v *= 0.5 * bx_dim[1] / abs(v[1]);
  } else {
    // on the z-normal plane:
    v *= 0.5 * bx_dim[2] / abs(v[2]);
  }
  return mGblPose->transformToGlobal(v);
}

mat<double, mat_structure::square> box_boundary_jac::operator()(
    vect<double, 3> v) const {
  using std::abs;
  using std::sqrt;
  const vect<double, 3> bx_dim = mBox->getDimensions();
  ...
  if ((abs(v[1]) * bx_dim[0] < abs(v[0]) * bx_dim[1]) &&
    ...
    // on the x-normal plane:
    //       v *= 0.5 * bx_dim[0] / abs(v[0]);
    double v_x3 = v[0] * v[0] * abs(v[0]);
    //       Jac_local(0,0) =  0.0;
    Jac_local(1, 0) = -0.5 * bx_dim[0] * v[1] * v[0] / v_x3;
    Jac_local(1, 1) = 0.5 * bx_dim[0] / abs(v[0]);
    Jac_local(2, 0) = -0.5 * bx_dim[0] * v[2] * v[0] / v_x3;
    Jac_local(2, 2) = 0.5 * bx_dim[0] / abs(v[0]);
  } else if ((abs(v[0]) * bx_dim[1] < abs(v[1]) * bx_dim[0]) &&
             (abs(v[2]) * bx_dim[1] < abs(v[1]) * bx_dim[2])) {
    // on the y-normal plane:
    //       v *= 0.5 * bx_dim[1] / abs(v[1]);
    double v_y3 = v[1] * v[1] * abs(v[1]);
    Jac_local(0, 0) = 0.5 * bx_dim[1] / abs(v[1]);
    Jac_local(0, 1) = -0.5 * bx_dim[1] * v[0] * v[1] / v_y3;
    //       Jac_local(1,1) =  0.0;
    Jac_local(2, 1) = -0.5 * bx_dim[1] * v[2] * v[1] / v_y3;
    Jac_local(2, 2) = 0.5 * bx_dim[1] / abs(v[1]);
  } else {
    // on the z-normal plane:
    //       v *= 0.5 * bx_dim[2] / abs(v[2]);
    double v_z3 = v[2] * v[2] * abs(v[2]);
    Jac_local(0, 0) = 0.5 * bx_dim[2] / abs(v[2]);
    Jac_local(0, 2) = -0.5 * bx_dim[2] * v[0] * v[2] / v_z3;
    Jac_local(1, 1) = 0.5 * bx_dim[2] / abs(v[2]);
...
vect<double, 3> box_support_func::operator()(vect<double, 3> v) const {
  ...
  if (abs(v[0]) > 1e-5) {
    v[0] *= 0.5 * bx_dim[0] / abs(v[0]);
  } else {
    v[0] = 0.0;
  }
  if (abs(v[1]) > 1e-5) {
    v[1] *= 0.5 * bx_dim[1] / abs(v[1]);
  } else {
    v[1] = 0.0;
  }
  if (abs(v[2]) > 1e-5) {
    v[2] *= 0.5 * bx_dim[2] / abs(v[2]);
  } else {
    v[2] = 0.0;
  }
...
struct GJK_simplex_point {
  ...
  void init(const vect<double, 3>& aDir, const support_func_base& aSFunc1,
            const support_func_base& aSFunc2) {
    dir = aDir;
    s1 = aSFunc1(aDir);
    s2 = aSFunc2(-aDir);
    cso = s1 - s2;
...
vect<double, 3> proj_barycentric_coord(const vect<double, 3>& v1,
                                       vect<double, 3> v2, vect<double, 3> v3) {
  vect<double, 3> b;
  v2 -= v1;
  v3 -= v1;
...
struct EPA_face_info {
  ...
  EPA_face_info(int aId, const std::vector<GJK_simplex_point>& aVec,
                std::vector<EPA_edge_info>& aEd, int aE1, int aE2, int aE3) {
    e[0] = aE1;
    e[1] = aE2;
    e[2] = aE3;
    color = EPA_white;
    ...
    vect<double, 3> b =
        proj_barycentric_coord(aVec[v[0]].cso, aVec[v[1]].cso, aVec[v[2]].cso);
    ...
    if (b[0] < 1e-5) {
      ...
      min_pt =
          b[0] * aVec[v[0]].cso + b[1] * aVec[v[1]].cso + b[2] * aVec[v[2]].cso;
      min_dist = norm_2(min_pt);
      min_edge = -1;
    }
  }
...
int EPA_add_face(std::vector<EPA_face_info>& epa_faces,
                 std::vector<int>& graveyard,
                 const std::vector<GJK_simplex_point>& epa_pts,
                 std::vector<EPA_edge_info>& epa_edges, int e1, int e2,
                 int e3) {
  int result = 0;
  if (graveyard.empty()) {
    // create a new face:
    result = static_cast<int>(epa_faces.size());
    epa_faces.emplace_back(result, epa_pts, epa_edges, e1, e2, e3);
  } else {
...
proximity_record_3D findProximityByGJKEPA(const support_func_base& aSFunc1,
                                          const support_func_base& aSFunc2) {
  using std::abs;
  std::array<GJK_simplex_point, 5> simplex = {};
  int cur_size = 0;
  ...
  while (true) {
    ...
    closest_case = -1;
    std::array<vect<double, 3>, 4> facet_proj_pts = {};
    int facet_count = 0;
    ...
    for (int i = 0; i < cur_size; ++i) {
      ...
      for (int j = i + 1; j < cur_size; ++j) {
        ...
        for (int k = j + 1; k < cur_size; ++k) {
          // Test facet:
          vect<double, 3> b = proj_barycentric_coord(
              simplex[i].cso, simplex[j].cso, simplex[k].cso);

          if ((b[0] < 1e-5) || (b[1] < 1e-5) || (b[2] < 1e-5)) {
            continue;  // closest point not on this facet.
          }

          vect<double, 3> closest_on_facet = b[0] * simplex[i].cso +
                                             b[1] * simplex[j].cso +
                                             b[2] * simplex[k].cso;
    ...
    if (facet_count >=
        3) {  // it's impossible for the origin to be inside the simplex if < 2 facets.
      // Test tetrahedron:
    ...
    bool something_changed = (abs(last_closest_dist - closest_dist) > 1e-5);
    ...
    if (!something_changed ||
        (closest - simplex[cur_size].cso) * closest < 1e-5) {
  ...
  if ((closest_dist > 1e-5) && (cur_size == 3)) {
    result.mDistance = closest_dist;
    vect<double, 3> b =
        proj_barycentric_coord(simplex[0].cso, simplex[1].cso, simplex[2].cso);
    result.mPoint1 =
        b[0] * simplex[0].s1 + b[1] * simplex[1].s1 + b[2] * simplex[2].s1;
    result.mPoint2 =
        b[0] * simplex[0].s2 + b[1] * simplex[1].s2 + b[2] * simplex[2].s2;
    return result;
  }

  // Otherwise, do EPA to find the penetration depth, normal and deepest points:
  std::vector<GJK_simplex_point> epa_pts(simplex.begin(),
                                         simplex.begin() + cur_size);
  ...
  if (cur_size == 4) {
    ...
    if (abs(volume) < 1e-5) {
      ...
      epa_edges.emplace_back(epa_pts, 0, 1);
      epa_edges.emplace_back(epa_pts, 0, 2);
      epa_edges.emplace_back(epa_pts, 0, 3);
      epa_edges.emplace_back(epa_pts, 1, 2);
      epa_edges.emplace_back(epa_pts, 1, 3);
      epa_edges.emplace_back(epa_pts, 2, 3);

      epa_faces.emplace_back(0, epa_pts, epa_edges, 0, 1, 2);
      epa_faces.emplace_back(1, epa_pts, epa_edges, 0, 1, 3);
      epa_faces.emplace_back(2, epa_pts, epa_edges, 0, 2, 3);
      epa_faces.emplace_back(3, epa_pts, epa_edges, 1, 2, 3);
    }
  }
  if (cur_size == 3) {
    // find the normal to the plane (in or out is not important):
    vect<double, 3> line_n =
        (simplex[1].cso - simplex[0].cso) % (simplex[2].cso - simplex[0].cso);
    double n_norm = norm_2(line_n);
    if (n_norm < 1e-5) {
      // degenerate facet.
      // find the redundant point:
      if (abs(simplex[1].dist - simplex[0].dist) <
          0.1 * abs(simplex[2].dist - simplex[0].dist)) {
      ...
      epa_edges.emplace_back(epa_pts, 3, 0);
      epa_edges.emplace_back(epa_pts, 3, 1);
      epa_edges.emplace_back(epa_pts, 3, 2);
      epa_edges.emplace_back(epa_pts, 4, 0);
      epa_edges.emplace_back(epa_pts, 4, 1);
      epa_edges.emplace_back(epa_pts, 4, 2);
      epa_edges.emplace_back(epa_pts, 0, 1);
      epa_edges.emplace_back(epa_pts, 0, 2);
      epa_edges.emplace_back(epa_pts, 1, 2);

      epa_faces.emplace_back(0, epa_pts, epa_edges, 0, 6, 1);
      epa_faces.emplace_back(1, epa_pts, epa_edges, 1, 8, 2);
      epa_faces.emplace_back(2, epa_pts, epa_edges, 0, 7, 2);
      epa_faces.emplace_back(3, epa_pts, epa_edges, 3, 6, 4);
      epa_faces.emplace_back(4, epa_pts, epa_edges, 4, 8, 5);
      epa_faces.emplace_back(5, epa_pts, epa_edges, 5, 7, 3);
    }
  }
  if (cur_size == 2) {
    ...
    vect<double, 3> b1;
    if (abs(line_v[0]) > 1e-5) {
      b1 = line_v % vect<double, 3>(0.0, 1.0, 1.0);
    } else if (abs(line_v[1]) > 1e-5) {
    ...
    epa_edges.emplace_back(epa_pts, 0, 2);
    epa_edges.emplace_back(epa_pts, 0, 3);
    epa_edges.emplace_back(epa_pts, 0, 4);
    epa_edges.emplace_back(epa_pts, 1, 2);
    epa_edges.emplace_back(epa_pts, 1, 3);
    epa_edges.emplace_back(epa_pts, 1, 4);
    epa_edges.emplace_back(epa_pts, 2, 3);
    epa_edges.emplace_back(epa_pts, 2, 4);
    epa_edges.emplace_back(epa_pts, 3, 4);
  ...
  while ((static_cast<unsigned int>(!Q.empty()) != 0U) &&
         !((last_f_sol == Q.top().f) &&
           (abs(last_min_dist - epa_faces[Q.top().f].min_dist) < 1e-5))) {

    int f_sol = Q.top().f;
    if ((epa_faces[f_sol].color == EPA_black) ||
        (epa_faces[f_sol].min_dist != Q.top().dist)) {
    ...
    epa_edges.emplace_back(epa_pts, epa_faces[f_sol].v[0],
                           static_cast<int>(epa_pts.size() - 1));
    int e2 = e1 + 1;
    epa_edges.emplace_back(epa_pts, epa_faces[f_sol].v[1],
                           static_cast<int>(epa_pts.size() - 1));
    int e3 = e2 + 1;
    epa_edges.emplace_back(epa_pts, epa_faces[f_sol].v[2],
                           static_cast<int>(epa_pts.size() - 1));

    double best_new_dist = std::numeric_limits<double>::infinity();

    // Add faces:
    for (int i : epa_faces[f_sol].e) {
      if (epa_edges[i].f[0] == f_sol) {
        epa_edges[i].f[0] = -1;
      } else {
        epa_edges[i].f[1] = -1;
      }
    ...
    int f2 = 0;
    int f3 = 0;
    f1 = EPA_add_face(epa_faces, graveyard, epa_pts, epa_edges, e1, e2,
                      epa_faces[f_sol].e[0]);
    if ((epa_edges[epa_faces[f_sol].e[1]].v[0] == epa_faces[f_sol].v[0]) ||
      ...
      f2 = EPA_add_face(epa_faces, graveyard, epa_pts, epa_edges, e1, e3,
                        epa_faces[f_sol].e[1]);
      f3 = EPA_add_face(epa_faces, graveyard, epa_pts, epa_edges, e2, e3,
                        epa_faces[f_sol].e[2]);
    } else {
      f2 = EPA_add_face(epa_faces, graveyard, epa_pts, epa_edges, e1, e3,
                        epa_faces[f_sol].e[2]);
    ...
    for (std::size_t i = 0; i < I.size(); ++i) {
      ...
      } else if ((epa_faces[f_1].v[1] != v_m1) &&
                 (epa_faces[f_1].v[1] != v_m2)) {
      ...
      } else if ((epa_faces[f_2].v[1] != v_m1) &&
                 (epa_faces[f_2].v[1] != v_m2)) {
      ...
      for (int j = 0; j < 3; ++j) {
        if (epa_faces[f_1].e[j] != e_m) {
          if ((epa_edges[epa_faces[f_1].e[j]].v[0] == v_m1) ||
              (epa_edges[epa_faces[f_1].e[j]].v[1] == v_m1)) {
        ...
        if (epa_faces[f_2].e[j] != e_m) {
          if ((epa_edges[epa_faces[f_2].e[j]].v[0] == v_m1) ||
              (epa_edges[epa_faces[f_2].e[j]].v[1] == v_m1)) {
  ...
  for (int i = 1; i < 3; ++i) {
    ...
    result.mPoint1 +=
        i_proj * (epa_pts[epa_faces[f_sol].v[i]].s1 - result.mPoint1);
    result.mPoint2 +=
        i_proj * (epa_pts[epa_faces[f_sol].v[i]].s2 - result.mPoint2);
