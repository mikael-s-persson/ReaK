...
class navigation_scenario : public named_object {
 public:
  ...
  std::shared_ptr<frame_3D<double>> robot_base_frame;
  /** Holds the robot's direct-kinematics model (polymorphic). */
  std::shared_ptr<direct_kinematics_model> robot_kin_model;
  /** Holds the robot's joint limits, for velocity, acceleration and jerk. */
  std::shared_ptr<joint_limits_collection<double>> robot_jt_limits;
  /** Holds the robot's proximity-query model, i.e., for checking collisions involving the robot. */
  std::shared_ptr<geom::proxy_query_model_3D> robot_proxy;
  /** Holds the robot's geometric model, i.e., for displaying the robot's 3D representation. */
  std::shared_ptr<geom::colored_model_3D> robot_geom_model;

  /** Holds the target's frame, i.e., the goal of the navigation is that the robot's end-effector frame meets this
   * target frame. */
   ...
   * (static in the robot's base-frame). */
  std::vector<std::shared_ptr<geom::proxy_query_model_3D>> env_proxy_models;

  /** Holds the robot vs. environment proximity-query pairs, i.e., for checking collisions between the robot and the
   * environment. */
  std::vector<std::shared_ptr<geom::proxy_query_pair_3D>> robot_env_proxies;
  ...
  *******************************************************************************/
  ...
  RK_RTTI_MAKE_CONCRETE_1BASE(navigation_scenario, 0xC210005D, 1,
                              "navigation_scenario", named_object)
};
}  // namespace kte
}  // namespace ReaK

#endif  // REAK_MBD_MODELS_NAVIGATION_MODEL_DATA_H_
