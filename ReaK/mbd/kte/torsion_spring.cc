...
void torsion_spring_2D::doMotion(
...
void torsion_spring_2D::doForce(
  ...
  double angle_diff =
      (invert(mAnchor1->Rotation) * mAnchor2->Rotation).getAngle() * mStiffness;
  if ((mSaturation > 0) && (abs(angle_diff) > mSaturation)) {
    if (angle_diff > 0) {
...
void torsion_spring_3D::doMotion(
    kte_pass_flag aFlag, const std::shared_ptr<frame_storage>& aStorage) {
  if ((!mAnchor1) || (!mAnchor2)) {
    return;
  }
