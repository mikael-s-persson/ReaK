...
void flexible_beam_2D::doMotion(
  ...
  if (mObjectFrame) {
    ...
    mObjectFrame->Acceleration =
        (mAnchor1->Acceleration + mAnchor2->Acceleration) * 0.5;

    mObjectFrame->Rotation = rot_mat_2D<double>(
        (mAnchor1->Rotation.getAngle() + mAnchor2->Rotation.getAngle()) * 0.5);
    mObjectFrame->AngVelocity =
        (mAnchor1->AngVelocity + mAnchor2->AngVelocity) * 0.5;
    mObjectFrame->AngAcceleration =
        (mAnchor1->AngAcceleration + mAnchor2->AngAcceleration) * 0.5;
  }
  ...
  if ((aFlag == store_kinematics) && (aStorage)) {
    ...
    if (mObjectFrame) {
      if (!(aStorage->frame_2D_mapping[mObjectFrame])) {
        aStorage->frame_2D_mapping[mObjectFrame] =
            std::make_shared<frame_2D<double>>(*mObjectFrame);
      } else {
...
void flexible_beam_2D::doForce(kte_pass_flag aFlag,
  ...
  vect<double, 2> diff_a1 =
      -diff * mAnchor1->Rotation - vect<double, 2>(mRestLength, 0.0);
  vect<double, 2> diff_a2 =
      diff * mAnchor2->Rotation + vect<double, 2>(mRestLength, 0.0);
  double angle_diff =
      (invert(mAnchor1->Rotation) * mAnchor2->Rotation).getAngle() *
      mTorsionStiffness;
...
void flexible_beam_3D::doMotion(
    kte_pass_flag aFlag, const std::shared_ptr<frame_storage>& aStorage) {
  if ((!mAnchor1) || (!mAnchor2)) {
    return;
  }
  ...
  if (mObjectFrame) {
    ...
    // mObjectFrame->Rotation = rot_mat_3D<double>((mAnchor1->Rotation.getAngle() + mAnchor2->Rotation.getAngle()) *
    // 0.5);
    ...
    mObjectFrame->AngVelocity =
        ((R_a1 * mAnchor1->AngVelocity + R_a2 * mAnchor2->AngVelocity) * 0.5) *
        R_o;
    mObjectFrame->AngAcceleration =
        ((R_a1 * mAnchor1->AngAcceleration + R_a2 * mAnchor2->AngAcceleration) *
         0.5) *
        R_o;
  }
  ...
  if ((aFlag == store_kinematics) && (aStorage)) {
    ...
    if (mObjectFrame) {
      if (!(aStorage->frame_3D_mapping[mObjectFrame])) {
        aStorage->frame_3D_mapping[mObjectFrame] =
            std::make_shared<frame_3D<double>>(*mObjectFrame);
      } else {
...
void flexible_beam_3D::doForce(kte_pass_flag aFlag,
                               const std::shared_ptr<frame_storage>& aStorage) {
  if ((!mAnchor1) || (!mAnchor2)) {
    return;
  }
  ...
  if (mObjectFrame) {
    ...
    mAnchor1->Force += tmp_force * 0.5;
    mAnchor1->Torque +=
        (R * mObjectFrame->Torque + p1.Position % tmp_force) * 0.5;

    R = p2.Quat.getRotMat();
    tmp_force = R * mObjectFrame->Force;
    mAnchor2->Force += tmp_force * 0.5;
    mAnchor2->Torque +=
        (R * mObjectFrame->Torque + p2.Position % tmp_force) * 0.5;
  }

  vect<double, 3> diff = mAnchor1->Position - mAnchor2->Position;
  vect<double, 3> diff_a1 =
      invert(mAnchor1->Quat) * (-diff) - vect<double, 3>(mRestLength, 0.0, 0.0);
  vect<double, 3> diff_a2 =
      invert(mAnchor2->Quat) * diff + vect<double, 3>(mRestLength, 0.0, 0.0);

  axis_angle<double> angle_diff(invert(mAnchor1->Quat) * mAnchor2->Quat);

  mAnchor1->Force += mStiffness * diff_a1;
  mAnchor1->Torque +=
      mTorsionStiffness * angle_diff.angle() * angle_diff.axis();
  mAnchor2->Force += mStiffness * diff_a2;
  mAnchor2->Torque -=
      mTorsionStiffness * angle_diff.angle() * angle_diff.axis();
