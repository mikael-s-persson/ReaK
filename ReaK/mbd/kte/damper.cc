...
void damper_gen::doMotion(kte_pass_flag aFlag,
...
void damper_gen::doForce([[maybe_unused]] kte_pass_flag aFlag,
  ...
  double force_mag = (mAnchor1->q_dot - mAnchor2->q_dot) *
                     mDamping;  // Positive if tension, negative if compression.
...
void damper_2D::doMotion(kte_pass_flag aFlag,
                         const std::shared_ptr<frame_storage>& aStorage) {
  if ((!mAnchor1) || (!mAnchor2)) {
    return;
  }
...
void damper_2D::doForce([[maybe_unused]] kte_pass_flag aFlag,
  ...
  if (force_sqrmag > 1E-7) {
    diff *= ((mAnchor1->Velocity - mAnchor2->Velocity) * diff) * mDamping /
            force_sqrmag;
...
void damper_3D::doMotion(kte_pass_flag aFlag,
                         const std::shared_ptr<frame_storage>& aStorage) {
  if ((!mAnchor1) || (!mAnchor2)) {
    return;
  }
