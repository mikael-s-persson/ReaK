...
int main(int argc, char** argv) {
  ...
  if(argc < 9) {
    std::cout << "Usage:\n"
	      << "\t./estimate_satellite3D [inertial_data.xml] [transforms.xml] [meas_filename] [result_filename] [Qu.xml] [R.xml] [added_R.xml] [test_count]\n"
	      << "\t\t inertial_data.xml:\t The filename of the inertial data of the satellite3D.\n"
	      << "\t\t transforms.xml:\t The filename of the tranformations data of the satellite3D (IMU and Earth-to-room).\n"
  ...
  std::string transforms_filename(argv[2]);
  ...
  std::string R_added_filename(argv[7]);
  
  std::size_t test_count = 0;
  std::stringstream(argv[8]) >> test_count;
  ...
  ReaK::quaternion<double> IMU_orientation;
  ReaK::vect<double,3> IMU_location;
  ReaK::quaternion<double> room_orientation;
  try {
    serialization::xml_iarchive in(transforms_filename);
    in & RK_SERIAL_LOAD_WITH_NAME(IMU_orientation)
       & RK_SERIAL_LOAD_WITH_NAME(IMU_location)
       & RK_SERIAL_LOAD_WITH_NAME(room_orientation);
  } catch(...) {
    RK_ERROR("An exception occurred during the loading of the transforms data!");
    return 6;
  };
  ...
  typedef ReaK::pp::topology_traits< state_space_type >::point_type state_space_point;
  
  
  state_space_point x_init;
  set_position(x_init, ReaK::vect<double,3>(0.0,0.0,0.0));
  set_quaternion(x_init, ReaK::unit_quat<double>());
  set_velocity(x_init, ReaK::vect<double,3>(0.0,0.0,0.0));
  set_ang_velocity(x_init, ReaK::vect<double,3>(0.0,0.0,0.0));
  
  ctrl::gaussian_belief_state< state_space_point, ctrl::covariance_matrix< vect_n<double> > > 
    b_init(x_init,
