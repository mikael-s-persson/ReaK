...
int main(int argc, char ** argv) {
  ...
  mdl_options.add_options()
    ("CRS-kin-model", po::value< std::string >()->default_value("models/CRS_A465.model.rkx"), "specify the input file for the kinematic model of the CRS robot (default is 'models/CRS_A465.model.rkx')")
  ;
  ...
  shared_ptr< pp::joint_limits_collection<double> > joint_rate_limits;
  
  try {
    shared_ptr< colored_model_3D > mdl_geom;
    shared_ptr< proxy_query_model_3D > mdl_prox;
    (*serialization::open_iarchive(CRS_mdl_fname))
      >> base_frame >> CRS_kte_model >> joint_rate_limits >> mdl_geom >> mdl_prox;
  } catch(std::exception& e) {
    std::cerr << "An error occurred while trying to load the CRS robot's kinematics model! Got exception: '" << e.what() << "'." << std::endl;
    return 2;
  };
  ...
  try {
    nvr_in["time"]; nvr_in["q_0"]; nvr_in["q_1"]; nvr_in["q_2"]; 
    nvr_in["q_3"]; nvr_in["q_4"]; nvr_in["q_5"]; nvr_in["q_6"];
  ...
  try {
    ...
    while(true) {
      ...
      vect_n<double> cur_jtctrl(nvr_in["q_0"], nvr_in["q_1"], nvr_in["q_2"], nvr_in["q_3"], nvr_in["q_4"], nvr_in["q_5"], nvr_in["q_6"]);
      ...
      (*data_out) << nvr_in["time"] << cur_EE.Position 
                  << cur_EE.Quat[0] << cur_EE.Quat[1] << cur_EE.Quat[2] << cur_EE.Quat[3]
