...
*******************************************************************************/
     ...
     * \test PASSED
     */
    std::pair<size_type,size_type> size() const throw() { return std::make_pair(m_sqr->get_row_count(),m_sqr->get_col_count()); };
...
struct is_readable_matrix< mat_damped_matrix<SquareMatrix,DiagMatrix> > {
  BOOST_STATIC_CONSTANT( bool, value = is_readable_matrix<SquareMatrix>::value && is_readable_matrix<DiagMatrix>::value );
...
template <typename SquareMatrix, typename DiagMatrix>
mat_damped_matrix<SquareMatrix,DiagMatrix> make_damped_matrix(const SquareMatrix& aMSqr, const DiagMatrix& aMDiag) {
