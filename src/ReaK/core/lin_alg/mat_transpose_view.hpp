...
*******************************************************************************/
     ...
     * \test PASSED
     */
    std::pair<size_type,size_type> size() const throw() { return std::make_pair(m->get_col_count(),m->get_row_count()); };
...
mat_transpose_view<Matrix> >::type transpose_view(Matrix& M) { return mat_transpose_view<Matrix>(M); };
