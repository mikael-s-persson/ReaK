...
typename boost::enable_if< is_fully_writable_matrix<Matrix1>, 
void >::type balance(Matrix1& A, vect_n<int>& D)
{  
...
void >::type balance_pencil(Matrix1& A, Matrix2& B, Matrix3& Dl, Matrix4& Dr)
{  
  if((A.get_row_count() != A.get_col_count()) ||
...
typename boost::enable_if_c< is_fully_writable_matrix<Matrix1>::value && 
                             is_fully_writable_matrix<Matrix2>::value, 
void >::type balance_pencil(Matrix1& A, Matrix2& B, vect_n<int>& Dl, vect_n<int>& Dr)
{  
  if((A.get_row_count() != A.get_col_count()) ||
     (B.get_row_count() != A.get_row_count()) ||
     (B.get_row_count() != B.get_col_count()))
