...
template <typename RandomAccessIter, typename Compare, typename PivotChooser>
inline
void quick_sort_tagged(RandomAccessIter first, RandomAccessIter last, Compare comp, std::random_access_iterator_tag t, PivotChooser pivot_chooser) {
  quick_sort_ra_impl(first,last,comp,pivot_chooser);
};

template <typename RandomAccessIter, typename Compare>
inline
void quick_sort_tagged(RandomAccessIter first, RandomAccessIter last, Compare comp, std::random_access_iterator_tag t) {
  quick_sort_ra_impl(first,last,comp,median_of_3_pivots());
};

template <typename BidirIter, typename Compare>
inline
void quick_sort_tagged(BidirIter first, BidirIter last, Compare comp, std::bidirectional_iterator_tag t) {
...
inline
void quick_sort(Iter first, Iter last, Compare comp, PivotChooser choose_pivot) {
  detail::quick_sort_tagged(first,last,comp,typename std::iterator_traits<Iter>::iterator_category(),choose_pivot);
};
