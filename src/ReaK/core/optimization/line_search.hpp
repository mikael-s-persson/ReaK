...
struct bounded_line_srch_dichotomous {
   ...
   * \tparam U Any vector type which can be linearly composed (i.e. closed under multiplication by a scalar and under addition).
  ...
  T operator()(Function f, T a0, T a1, const U& x0, const U& dx, const T& tol = T(1e-6)) const {
    dichotomous_search(boost::bind(detail::linear_composed_function_param<Function,T,U>,f,_1,boost::cref(x0),boost::cref(dx)),a0,a1,tol);
    return (a1 + a0) * T(0.5);
  };
   ...
   * the search domain.
   * \tparam Function A functor type for a unary function that computes the cost for a given independent variable value.
   * \tparam GradFunction A functor type for a unary function that computes the gradient of the cost for a given independent variable value.
   * \tparam T A scalar type.
   * \tparam U Any vector type which can be linearly composed (i.e. closed under multiplication by a scalar and under addition).
   * \param f The cost function to minimize (should map U -> T).
   * \param a0 The first bound of the search.
...
struct bounded_line_srch_gold_sect {
  ...
  template <typename Function, typename T, typename U>
  T operator()(Function f, T a0, T a1, const U& x0, const U& dx, const T& tol = T(1e-6)) const {
    golden_section_search(boost::bind(detail::linear_composed_function_param<Function,T,U>,f,_1,boost::cref(x0),boost::cref(dx)),a0,a1,tol);
    return (a1 + a0) * T(0.5);
  };
...
struct bounded_line_srch_fibonacci {
  ...
  template <typename Function, typename T, typename U>
  T operator()(Function f, T a0, T a1, const U& x0, const U& dx, const T& tol = T(1e-6)) const {
    fibonacci_search(boost::bind(detail::linear_composed_function_param<Function,T,U>,f,_1,boost::cref(x0),boost::cref(dx)),a0,a1,tol);
    return (a1 + a0) * T(0.5);
  };
 ...
 * \param up_bound the upper bound for the search.
 * \param tol the uncertainty on the X value at which the optimization should stop.
 * \param c1 The first parameter defining the strong Wolfe conditions (Armijo condition) (should be between 0 and 1 exclusively).
 * \param c2 The second parameter defining the strong Wolfe conditions (curvature condition) (should be between 0 and 1 exclusively, and greater than c1).
 * \param tau The geometric decrease factor for the successive decrease of the search interval (should be between 0 and 1 exclusively).
 * \return the cost that is the optimum point found, within uncertainty interval (low_bound, up_bound).
 */
template <typename T, typename Function, typename GradFunction>
T backtracking_search(Function f, GradFunction df, T& low_bound, T& up_bound, T tol = T(1e-6), T c1 = T(1e-2), T c2 = T(0.9), T tau = T(0.75)) {
...
struct line_search_backtracking {
   ...
   * \param aC1 The first parameter defining the strong Wolfe conditions (Armijo condition) (should be between 0 and 1 exclusively).
   * \param aC2 The second parameter defining the strong Wolfe conditions (curvature condition) (should be between 0 and 1 exclusively, and greater than c1).
   * \param aGeomFactor The geometric decrease factor for the successive decrease of the search interval (should be between 0 and 1 exclusively).
   */
  line_search_backtracking(const T& aC1 = T(1e-2), const T& aC2 = T(0.9), const T& aGeomFactor = T(0.75)) : c1(aC1), c2(aC2), geom_factor(aGeomFactor) { };
  ...
  T operator()(Function f, GradFunction df, T a0, T a1, const U& x0, const U& dx, const T& tol = T(1e-6)) const {
    backtracking_search(boost::bind(detail::linear_composed_function_param<Function,T,U>,f,_1,boost::cref(x0),boost::cref(dx)),boost::bind(detail::linear_composed_proj_gradient<GradFunction,T,U>,df,_1,boost::cref(x0),boost::cref(dx)),a0,a1,tol,c1,c2,geom_factor);
    return a1;
  };
 ...
 * \param up_bound the upper bound for the search.
 * \param tol the uncertainty on the X value at which the optimization should stop.
 * \param c1 The first parameter defining the Wolf conditions (Armijo condition) (should be between 0 and 1 exclusively).
 * \param c2 The second parameter defining the Wolf conditions (curvature condition) (should be between 0 and 1 exclusively, and greater than c1).
 * \param tau The geometric decrease factor for the successive decrease of the search interval (should be between 0 and 1 exclusively).
 * \return the cost that is the optimum point found, within uncertainty interval (low_bound, up_bound).
 */
template <typename T, typename Function, typename GradFunction>
T expand_and_zoom_search(Function f, GradFunction df, T& low_bound, T& up_bound, T tol = T(1e-6), T c1 = T(1e-2), T c2 = T(0.9)) {
...
struct line_search_expand_and_zoom {
  ...
  template <typename Function, typename GradFunction, typename U>
  T operator()(Function f, GradFunction df, T a0, T a1, const U& x0, const U& dx, const T& tol = T(1e-6)) const {
    expand_and_zoom_search(boost::bind(detail::linear_composed_function_param<Function,T,U>,f,_1,boost::cref(x0),boost::cref(dx)),boost::bind(detail::linear_composed_proj_gradient<GradFunction,T,U>,df,_1,boost::cref(x0),boost::cref(dx)),a0,a1,tol,c1,c2);
    return a1;
  };
