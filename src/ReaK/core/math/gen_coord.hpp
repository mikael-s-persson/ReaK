...
#ifndef GEN_COORD_HPP
#define GEN_COORD_HPP
...
class gen_coord : public shared_object {
  public:
     ...
     */
    gen_coord(const_reference Q,
              const_reference Q_dot,
              const_reference Q_ddot,
              const_reference F) : shared_object(),
                                   q(Q),
                                   q_dot(Q_dot),
                                   q_ddot(Q_ddot),
                                   f(F) { };
     ...
     */
    friend
    self operator +(const self& G1,const self& G2) {
...
*******************************************************************************/
    ...
    virtual void RK_CALL save(ReaK::serialization::oarchive& A, unsigned int) const {
      A & RK_SERIAL_SAVE_WITH_NAME(q)
        & RK_SERIAL_SAVE_WITH_NAME(q_dot)
        & RK_SERIAL_SAVE_WITH_NAME(q_ddot)
        & RK_SERIAL_SAVE_WITH_NAME(f);
    };
    virtual void RK_CALL load(ReaK::serialization::iarchive& A, unsigned int) {
      A & RK_SERIAL_LOAD_WITH_NAME(q)
        & RK_SERIAL_LOAD_WITH_NAME(q_dot)
        & RK_SERIAL_LOAD_WITH_NAME(q_ddot)
