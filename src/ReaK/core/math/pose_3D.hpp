...
class pose_3D : public shared_object {
  public:
     ...
     */
    pose_3D(boost::weak_ptr< self > aParent, const position_type& aPosition, const rotation_type& aQuat) :
               shared_object(),
	       Parent(aParent),
	       Position(aPosition),
	       Quat(aQuat) { };
	...
	if((Parent.expired()) || (Parent.lock() == P))
      ...
      } else if(P->isParentPose(boost::static_pointer_cast< const self >(mThis)))
        return ~(P->getPoseRelativeTo(boost::static_pointer_cast< const self >(mThis)));
...
*******************************************************************************/
    ...
    virtual void RK_CALL load(ReaK::serialization::iarchive& A, unsigned int) {
      pose_lock access_lock(AccessMutex);
      boost::shared_ptr< pose_3D<T> > tmp;
