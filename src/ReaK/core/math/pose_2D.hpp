...
class pose_2D : public shared_object {
  public:
     ...
     */
    pose_2D() : shared_object(), Parent(), AccessMutex(), Position(), Rotation() { };

    /**
     * Parametrized constructor, all is set to corresponding parameters.
     */
    pose_2D(boost::weak_ptr< self > aParent, const position_type& aPosition, const rotation_type& aRotation) :
                shared_object(),
		Parent(aParent),
		AccessMutex(),
		Position(aPosition),
     ...
     */
    bool isParentPose(boost::shared_ptr< const self > P) const {
      if(Parent.expired()) {
	if(P)
	  return true;
	else
     ...
     */
    self getPoseRelativeTo(boost::shared_ptr< const self > P) const {
      if(!P)
      ...
      } else if(P->isParentPose( rtti::rk_dynamic_ptr_cast< const self >(mThis)))
        return ~(P->getPoseRelativeTo( rtti::rk_dynamic_ptr_cast< const self >(mThis)));
    ...
    self& addBefore(const self& aPose) {
      pose_lock access_lock(AccessMutex);
