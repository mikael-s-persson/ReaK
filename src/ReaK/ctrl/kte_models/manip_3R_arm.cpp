...
manip_3R_2D_kinematics::manip_3R_2D_kinematics(const std::string& aName, 
  ...
  //create inertia
  m_EE = shared_ptr< joint_dependent_frame_2D >(new joint_dependent_frame_2D(
      arm_EE),
    scoped_deleter());
...
void manip_3R_2D_kinematics::doInverseMotion() {
  ...
  if(w_dist > m_link1_length + m_link2_length)
    throw optim::infeasible_problem("Inverse kinematics problem is infeasible! Desired end-effector pose of 2D 3R manipulator is outside the reachable workspace!");
  if(w_dist < 1.000001 * fabs(m_link1_length - m_link2_length))
    throw optim::infeasible_problem("Inverse kinematics problem is infeasible! Desired end-effector pose of 2D 3R manipulator is too close to the center (inner work-space limit)!");
  ...
  vect<double,2> cl3 = double(1.0) % ( R12 * rot_mat_2D<double>(a3) * vect<double,2>(m_link3_length, 0.0) );
  
  mat<double,mat_structure::rectangular> A(3,3);
  A(0,0) = cl1[0]; A(1,0) = cl1[1]; A(2,0) = 1.0;
...
void manip_3R_2D_kinematics::getJacobianMatrixAndDerivative(mat<double,mat_structure::rectangular>& Jac, mat<double,mat_structure::rectangular>& JacDot) const {
  rot_mat_2D<double> R1 = rot_mat_2D<double>(m_joints[0]->q);
  rot_mat_2D<double> R12 = rot_mat_2D<double>(m_joints[0]->q + m_joints[1]->q);
  rot_mat_2D<double> R123 = rot_mat_2D<double>(m_joints[0]->q + m_joints[1]->q + m_joints[2]->q);
...
vect_n<double> manip_3R_2D_kinematics::getDependentPositions() const {
  return vect_n<double>(
    m_EE->mFrame->Position[0], m_EE->mFrame->Position[1],
...
vect_n<double> manip_3R_2D_kinematics::getDependentVelocities() const {
  return vect_n<double>(
    m_EE->mFrame->Velocity[0], m_EE->mFrame->Velocity[1],
...
vect_n<double> manip_3R_2D_kinematics::getDependentAccelerations() const {
  return vect_n<double>(
    m_EE->mFrame->Acceleration[0], m_EE->mFrame->Acceleration[1],
...
manip_3R_3D_kinematics::manip_3R_3D_kinematics(const std::string& aName,
                                               ...
                                               double aLink1Length, double aLink1DZ,
                                               double aLink2Length, double aLink2DZ,
...
void manip_3R_3D_kinematics::doInverseMotion() {
  ...
  if(w_dist > m_link1_length + m_link2_length)
    throw optim::infeasible_problem("Inverse kinematics problem is infeasible! Desired end-effector pose of 3D 3R manipulator is outside the reachable workspace!");
  if(w_dist < 1.000001 * fabs(m_link1_length - m_link2_length))
    throw optim::infeasible_problem("Inverse kinematics problem is infeasible! Desired end-effector pose of 3D 3R manipulator is too close to the center (inner work-space limit)!");
...
void manip_3R_3D_kinematics::getJacobianMatrixAndDerivative(mat<double,mat_structure::rectangular>& Jac, mat<double,mat_structure::rectangular>& JacDot) const {
  rot_mat_2D<double> R1 = rot_mat_2D<double>(m_joints[0]->q);
  rot_mat_2D<double> R12 = rot_mat_2D<double>(m_joints[0]->q + m_joints[1]->q);
  rot_mat_2D<double> R123 = rot_mat_2D<double>(m_joints[0]->q + m_joints[1]->q + m_joints[2]->q);
...
void RK_CALL manip_3R_3D_kinematics::save(serialization::oarchive& A, unsigned int) const {
  ...
  A & RK_SERIAL_SAVE_WITH_NAME(m_base_frame)
    ...
    & RK_SERIAL_SAVE_WITH_NAME(m_link3_length)
    & RK_SERIAL_SAVE_WITH_NAME(m_link3_dz)
    & RK_SERIAL_SAVE_WITH_NAME(m_chain);
};
...
void RK_CALL manip_3R_3D_kinematics::load(serialization::iarchive& A, unsigned int) {
  ...
  A & RK_SERIAL_LOAD_WITH_NAME(m_base_frame)
    ...
    & RK_SERIAL_LOAD_WITH_NAME(m_link3_length)
    & RK_SERIAL_LOAD_WITH_NAME(m_link3_dz)
    & RK_SERIAL_LOAD_WITH_NAME(m_chain);
};
