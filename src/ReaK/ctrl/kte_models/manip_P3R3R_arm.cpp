...
manip_P3R3R_kinematics::manip_P3R3R_kinematics(const std::string& aName,
  ...
  m_track_joint = shared_ptr< prismatic_joint_3D >(new prismatic_joint_3D(
      "manip_P3R3R_track",
      m_track_coord,
...
void manip_P3R3R_kinematics::doInverseMotion() {
  ...
  shoulder_to_wrist_min_sqr = shoulder_to_wrist_min_sqr * shoulder_to_wrist_min_sqr + elbow_to_wrist_dist * elbow_to_wrist_dist * c3_min_s3 * c3_min_s3;
  double x_min = 0.0;
  if(perp_wrist_dist_sqr < shoulder_to_wrist_min_sqr)
    x_min = sqrt(shoulder_to_wrist_min_sqr - perp_wrist_dist_sqr);
  
  /* At this point, the range of joint values for the track is between +- x_max around wrist_pos[0] and beyond +- x_min around wrist_pos[0]. */
  ...
  if( ( wrist_pos[0] + x_min > m_track_upper_bound ) && ( wrist_pos[0] - x_min < m_track_lower_bound ) )
    throw optim::infeasible_problem("Inverse kinematics problem is infeasible! End-effector pose is out-of-reach! The track is too short to allow for a feasible solution.");
