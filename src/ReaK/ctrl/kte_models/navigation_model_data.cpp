...
#include <ReaK/ctrl/kte_models/navigation_model_data.hpp>
...
navigation_scenario::navigation_scenario() : named_object() {
...
void navigation_scenario::create_robot_env_proxies() {
  ...
  if( robot_proxy ) {
    ...
    for(std::size_t i = robot_env_proxies.size(); i < env_proxy_models.size(); ++i) {
      robot_env_proxies.push_back(
        shared_ptr< geom::proxy_query_pair_3D >(new geom::proxy_query_pair_3D(
          "robot_env_proxy:" + env_proxy_models[i]->getName(), 
          robot_proxy, 
          env_proxy_models[i])));
    };
