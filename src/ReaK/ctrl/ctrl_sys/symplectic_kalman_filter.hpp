...
struct SKF_belief_transfer {
  ...
  SKF_belief_transfer(const SKFTransferFactory* aFactory,
    ...
    symplectic_kalman_update(*(factory->get_state_space_system()), 
                             b_space.get_state_topology(),
                             b, b_u, b_y, Tm, t + factory->get_state_space_system()->get_time_step());
  };
  ...
  belief_state get_next_belief(const belief_space& b_space, belief_state b, const time_type& t, const input_type& u, const output_type& y) {
    ...
    symplectic_kalman_update(*(factory->get_state_space_system()), b_space.get_state_topology(), b, b_u, b_y, Tm, t + factory->get_state_space_system()->get_time_step());
    return b;
  };
