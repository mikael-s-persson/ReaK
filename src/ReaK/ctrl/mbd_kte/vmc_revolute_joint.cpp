...
void vmc_revolute_joint_2D::doForce(kte_pass_flag aFlag, boost::shared_ptr<frame_storage> aStorage) {
  if((!mEnd) || (!mBase))
    return;
...
void vmc_revolute_joint_3D::doForce(kte_pass_flag aFlag, boost::shared_ptr<frame_storage> aStorage) {
  if((!mEnd) || (!mBase))
    return;
