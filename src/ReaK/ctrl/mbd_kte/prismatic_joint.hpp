...
class prismatic_joint_2D : public reacting_kte_gen {
  protected:
    shared_pointer< gen_coord<double> >::type mCoord; ///< Generalized coordinate representing the joint's linear displacement.
  ...
  public:
    ...
    prismatic_joint_2D(const std::string& aName,
		       const shared_pointer< gen_coord<double> >::type& aCoord,
