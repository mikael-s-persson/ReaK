...
void inertia_gen::doMotion(kte_pass_flag aFlag, const shared_pointer<frame_storage>::type& aStorage) {
  ...
  if((aFlag == store_kinematics) && (aStorage)) {
    if(!(aStorage->gen_coord_mapping[mCenterOfMass->mFrame]))
      aStorage->gen_coord_mapping[mCenterOfMass->mFrame] = shared_pointer< ReaK::gen_coord<double> >::type(new ReaK::gen_coord<double>((*mCenterOfMass->mFrame)),ReaK::scoped_deleter());
    else
...
void inertia_2D::doMotion(kte_pass_flag aFlag, const shared_pointer<frame_storage>::type& aStorage) {
  ...
  if((aFlag == store_kinematics) && (aStorage)) {
    if(!(aStorage->frame_2D_mapping[mCenterOfMass->mFrame]))
      aStorage->frame_2D_mapping[mCenterOfMass->mFrame] = shared_pointer< frame_2D<double> >::type(new frame_2D<double>((*mCenterOfMass->mFrame)),scoped_deleter());
    else
...
void inertia_3D::doMotion(kte_pass_flag aFlag, const shared_pointer<frame_storage>::type& aStorage) {
  ...
  if((aFlag == store_kinematics) && (aStorage)) {
    if(!(aStorage->frame_3D_mapping[mCenterOfMass->mFrame]))
      aStorage->frame_3D_mapping[mCenterOfMass->mFrame] = shared_pointer< frame_3D<double> >::type(new frame_3D<double>((*mCenterOfMass->mFrame)),scoped_deleter());
    else
