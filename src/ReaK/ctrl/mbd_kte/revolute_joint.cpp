...
void revolute_joint_2D::doMotion(kte_pass_flag aFlag, boost::shared_ptr<frame_storage> aStorage) {
  if((!mEnd) || (!mBase))
    return;
  ...
  if((aFlag == store_kinematics) && (aStorage)) {
    ...
    if(mAngle) {
      if(!(aStorage->gen_coord_mapping[mAngle]))
        aStorage->gen_coord_mapping[mAngle] = boost::shared_ptr< gen_coord<double> >(new gen_coord<double>((*mAngle)),scoped_deleter());
      else
...
void revolute_joint_2D::doForce(kte_pass_flag aFlag, boost::shared_ptr<frame_storage> aStorage) {
  if((!mEnd) || (!mBase))
    return;
...
void revolute_joint_3D::doMotion(kte_pass_flag aFlag, boost::shared_ptr<frame_storage> aStorage) {
  if((!mEnd) || (!mBase))
    return;
...
void revolute_joint_3D::doForce(kte_pass_flag aFlag, boost::shared_ptr<frame_storage> aStorage) {
  if((!mEnd) || (!mBase))
    return;
