...
namespace detail {
  ...
  static
  vect<double,2> sap_compute_projected_deltas(double beta, const double (&coefs)[5], double dt_amax, double& A_0, double& A_1) {
    using std::sqrt;
  ...
  static
  vect<double,2> sap_compute_projected_deltas(double beta, const double (&coefs)[5], double dt_amax) {
    double A_0, A_1;
    return sap_compute_projected_deltas(beta,coefs,dt_amax,A_0,A_1);
  };
  
  static
  vect<double,2> sap_compute_derivative_projected_deltas(double beta, const double (&coefs)[5], double dt_amax, double A_0, double A_1) {
    using std::sqrt;
    using std::fabs;
    vect<double,2> result;
    if(A_0 < dt_amax) {
  ...
  static
  double sap_compute_slack_time(double beta, double dt, const vect<double,2>& deltas_0, double norm_delta, const double (&coefs)[5], double dt_amax) {
    using std::sqrt;
    using std::fabs;
  ...
  static
  double sap_compute_derivative_slack_time(double beta, double dt, const vect<double,2>& deltas_0, double norm_delta, const double (&coefs)[5], double dt_amax) {
    using std::sqrt;
    using std::fabs;
  ...
  static
  double sap_compute_travel_time(double beta, const vect<double,2>& deltas_0, double norm_delta, const double (&coefs)[5], double dt_amax) {
    using std::sqrt;
    using std::fabs;
  ...
  static
  double sap_compute_derivative_travel_time(double beta, const vect<double,2>& deltas_0, double norm_delta, const double (&coefs)[5], double dt_amax) {
    using std::sqrt;
    using std::fabs;
  ...
  double sap_solve_for_min_dt_beta(double beta, double norm_delta, const double (&coefs)[5], double num_tol, double dt_amax) {
  ...
  bool sap_min_dt_predicate(double beta, double norm_delta, const double (&coefs)[5], double num_tol, double& result, double dt_amax) {
    result = sap_compute_travel_time(beta,sap_compute_projected_deltas(beta,coefs,dt_amax),norm_delta,coefs,dt_amax);
    return true;
  };
  
  double sap_solve_for_no_slack_beta(double beta, double norm_delta, const double (&coefs)[5], double num_tol, double delta_time, double dt_amax) {
  ...
  bool sap_no_slack_predicate(double beta, double norm_delta, const double (&coefs)[5], double num_tol, double& slack, double delta_time, double dt_amax) {
    slack = sap_compute_slack_time(beta,
                                   delta_time,
                                   sap_compute_projected_deltas(beta,coefs,dt_amax),
                                   norm_delta,
                                   coefs,
                                   dt_amax);
