 ...
 * \tparam Topology The topology type on which the points and the sequential path can reside, should model the MetricSpaceConcept.
 */
...
class seq_path_base : public named_object {
...
#endif
        friend 
        void swap(point_distance_iterator& rhs, point_distance_iterator& lhs) {
  ...
  public:
     ...
     */
    explicit seq_path_base(const std::string& aName = "") : 
                       named_object() { 
      setName(aName);
    };
