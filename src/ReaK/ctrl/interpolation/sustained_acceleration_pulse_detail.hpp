...
namespace detail {
  ...
  void >::type sap_interpolate_impl(PointType& result, const PointType& start_point, const PointType& end_point, 
    ...
    if( dt_vp_1st < dt_amax ) {
      //means that we don't have time to reach the maximum acceleration:
      dt_vp = sqrt(4.0 * dt_amax * dt_vp_1st);
      dt_ap = dt_vp * 0.5;
    };
    
    if( dt < dt_ap) {
      get<2>(result) =
        get_space<2>(space,t_space).adjust(
	  get_space<2>(space,t_space).origin(),
          ( dt / dt_amax ) * 
          get_space<2>(space,t_space).difference( 
            lift_to_space<2>(get_space<1>(space,t_space).difference(peak_velocity, get<1>(start_point)),dt_vp_1st,space,t_space),
            get_space<2>(space,t_space).origin()
          )
	);
      
      get<1>(result) = 
        get_space<1>(space,t_space).adjust(
	  get<1>(start_point),
	  0.5 * descend_to_space<1>( get<2>(result), dt, space, t_space)
	);

      get<0>(result) = 
        get_space<0>(space,t_space).adjust(
	  get<0>(start_point),
          (2.0 / 3.0) * descend_to_space<0>( get<1>(start_point), dt, space, t_space)
	  + (1.0 / 3.0) * descend_to_space<0>( get<1>(result), dt, space, t_space)
	);
      
      //TODO call higher-order term.
    ...
    //TODO case 2: in the constant accel phase of velocity ramp-up.
    
    //TODO case 3: in the jerk-down phase of velocity ramp-up.
