...
class adj_list_tree_overlay
{
  ...
  public:
    ...
    struct vertex_properties {
      ...
      in_edge_map in_edges;
      VertexProperty data;
      
      friend const position_type& get(boost::vertex_position_t, const vertex_descriptor& v) {
	return v.position;
      };
      friend void put(boost::vertex_position_t, vertex_descriptor& v, const position_type& p) {
	v.position = p;
      };
#ifdef RK_ENABLE_CXX0X_FEATURES
      friend void put(boost::vertex_position_t, vertex_descriptor& v, position_type&& p) {
	v.position = std::move(p);
      };
#endif
    };
    ...
    typedef pp::dvp_tree<vertex_descriptor, Topology, 
	                 ...
	                 VPChooser, TreeStorageTag, 
			 no_position_caching_policy> dvp_tree_type;
