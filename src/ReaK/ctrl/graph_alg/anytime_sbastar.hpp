...
/**
  ...
  * words, the visitor pattern in generic programming is an implementation of IoC 
  * (Inversion of Control), since the bi-directional ASBA*-RRT* algorithm is in control of execution, but custom behavior can
  * be injected in several places, even blocking the algorithm if needed.
  * 
  * Required concepts:
  * 
  * The visitor class should model ASBARRTStarVisitorConcept, NodeBackPullingVisitorConcept and NodeBackPushingVisitorConcept.
  * 
  * \tparam Visitor The visitor class to be tested for modeling an ASBA* visitor concept.
  * \tparam Graph The graph type on which the visitor should be able to act.
  * \tparam Topology The topology type on which the visitor class is required to work with.
...
namespace detail {
  ...
  struct anytime_sbastar_bfs_visitor :
    sbastar_bfs_visitor<Graph, UniformCostVisitor, UpdatableQueue, IndexInHeapMap, 
                        KeyMap, PositionMap, WeightMap, DensityMap, ConstrictionMap, 
                        DistanceMap, PredecessorMap, FwdDistanceMap, SuccessorMap>
  {
    typedef sbastar_bfs_visitor<Graph, UniformCostVisitor, UpdatableQueue, IndexInHeapMap, 
                                KeyMap, PositionMap, WeightMap, DensityMap, ConstrictionMap, 
                                DistanceMap, PredecessorMap, FwdDistanceMap, SuccessorMap> base_type;
    ...
    anytime_sbastar_bfs_visitor(double current_relax, UniformCostVisitor vis, 
                                ...
                                base_type(vis, Q, index_in_heap, 
                                          key, pos, weight, density, constriction, 
                                          dist, pred, fwd_dist, succ),
    ...
    void update_key(Vertex u, Graph& g) const {
      ...
      if( get(this->m_successor, g[u]) == boost::graph_traits<Graph>::null_vertex() )
        ...
        put(this->m_key, u, ((g_u + h_u) / (1.0 - get(this->m_constriction, g[u]))) / (1.0 - get(this->m_density, g[u])) + m_current_relaxation * g_u);
    };
  ...
  void generate_anytime_sbastar_no_init_impl(Graph &g, 
    typename boost::graph_traits<Graph>::vertex_descriptor start_vertex, 
    const Topology& super_space, ASBAStarVisitor vis,
    ...
    FwdDistanceMap fwd_distance, NcSelector select_neighborhood, double init_relaxation)
  {
    ...
    anytime_sbastar_bfs_visitor<Graph, ASBAStarVisitor, MutableQueue, IndexInHeapMap, 
                                KeyMap, PositionMap, WeightMap, 
                                DensityMap, ConstrictionMap, DistanceMap, 
                                PredecessorMap, FwdDistanceMap> sba_bfs_vis(
      init_relaxation, vis, Q, index_in_heap, key, position, weight, 
  ...
  template <typename Graph, typename Topology, typename SBAStarVisitor,
            ...
            typename DensityMap, typename ConstrictionMap, typename DistanceMap, typename PredecessorMap, 
            typename FwdDistanceMap, typename SuccessorMap, typename KeyMap, typename NcSelector>
  void generate_anytime_sbastar_bidir_no_init_impl(Graph &g, 
      typename boost::graph_traits<Graph>::vertex_descriptor start_vertex, 
      ...
      NcSelector select_neighborhood, double init_relaxation)
  {
    ...
    anytime_sbastar_bfs_visitor<Graph, SBAStarVisitor, MutableQueue, IndexInHeapMap,
                                KeyMap, PositionMap, WeightMap, DensityMap, ConstrictionMap, 
  ...
  void generate_anytime_sbarrtstar_no_init_impl(Graph &g, 
    typename boost::graph_traits<Graph>::vertex_descriptor start_vertex, 
    const Topology& super_space, ASBARRTStarVisitor vis, NodeConnector connect_vertex, 
    ...
    double init_relaxation, double SA_init_temperature = 0.0)
  {
    ...
    anytime_sbastar_bfs_visitor<Graph, ASBARRTStarVisitor, MutableQueue, IndexInHeapMap, 
                                KeyMap, PositionMap, WeightMap, 
                                DensityMap, ConstrictionMap, DistanceMap, 
                                PredecessorMap, FwdDistanceMap> sba_bfs_vis(
      init_relaxation, vis, Q, index_in_heap, key, position, weight, 
  ...
  void generate_anytime_sbarrtstar_bidir_no_init_impl(Graph &g, 
    typename boost::graph_traits<Graph>::vertex_descriptor start_vertex, 
    typename boost::graph_traits<Graph>::vertex_descriptor goal_vertex, 
    const Topology& super_space, ASBARRTStarVisitor vis, NodeConnector connect_vertex, 
    KeyMap key, PositionMap position, WeightMap weight,
...
/**
  * This function template generates a roadmap to connect a goal location to a start location
  * using the Bi-directional Anytime-Lazy-SBA* algorithm, with initialization of the existing graph to (re)start the search.
  * \tparam SBAStarBundle A SBA* bundle type (see make_sbastar_bundle()).
  * \param bdl A const-reference to a SBA* bundle of parameters, see make_sbastar_bundle().
  * \param init_relaxation The initial temperature of the Simulated Annealing when used 
  *        as the deciding factor between using RRT* or SBA* samples.
...
/**
  ...
  */
template <typename SBAStarBundle, typename RandomSampler>
void generate_anytime_sbarrtstar_bidir_no_init(const SBAStarBundle& bdl, 
                                               RandomSampler get_sample, 
                                               double init_relaxation,
                                               double SA_init_temperature = 0.0) {
...
/**
  * This function template generates a roadmap to connect a goal location to a start location
  * using the Bi-directional Anytime-SBA*-RRT* algorithm, with initialization of the existing graph to (re)start the search.
  * \tparam SBAStarBundle A SBA* bundle type (see make_sbastar_bundle()).
  * \param bdl A const-reference to a SBA* bundle of parameters, see make_sbastar_bundle().
  * \param get_sample A random sampler of positions in the space.
  * \param init_relaxation The initial temperature of the Simulated Annealing when used 
...
/**
  ...
  */
template <typename SBAStarBundle, typename RandomSampler>
void generate_anytime_lazy_sbarrtstar_no_init(const SBAStarBundle& bdl, 
                                              RandomSampler get_sample, 
                                              double init_relaxation,
                                              double SA_init_temperature = 0.0) {
...
/**
  ...
  */
template <typename SBAStarBundle, typename RandomSampler>
void generate_anytime_lazy_sbarrtstar_bidir_no_init(const SBAStarBundle& bdl, 
                                                    RandomSampler get_sample, 
                                                    double init_relaxation,
                                                    double SA_init_temperature = 0.0) {
  BOOST_CONCEPT_ASSERT((SBARRTStarVisitorConcept<typename SBAStarBundle::visitor_type,typename SBAStarBundle::graph_type,typename SBAStarBundle::topology_type>));
...
/**
  * This function template generates a roadmap to connect a goal location to a start location
  * using the Bi-directional Anytime-Lazy-SBA*-RRT* algorithm, with initialization of the existing graph to (re)start the search.
  * \tparam SBAStarBundle A SBA* bundle type (see make_sbastar_bundle()).
  * \param bdl A const-reference to a SBA* bundle of parameters, see make_sbastar_bundle().
  * \param get_sample A random sampler of positions in the space.
  * \param init_relaxation The initial temperature of the Simulated Annealing when used 
...
/**
  ...
  */
template <typename SBAStarBundle, typename RandomSampler>
void generate_anytime_lazy_bnb_sbarrtstar_no_init(const SBAStarBundle& bdl, 
                                                  RandomSampler get_sample, 
                                                  double init_relaxation,
                                                  double SA_init_temperature = 0.0) {
  BOOST_CONCEPT_ASSERT((SBARRTStarVisitorConcept<typename SBAStarBundle::visitor_type,typename SBAStarBundle::graph_type,typename SBAStarBundle::topology_type>));
...
/**
  * This function template generates a roadmap to connect a goal location to a start location
  * using the Bi-directional Anytime-Lazy-SBA*-RRT* (with branch-and-bound) algorithm, without initialization of the existing graph.
  * \tparam SBAStarBundle A SBA* bundle type (see make_sbastar_bundle()).
  * \tparam RandomSampler This is a random-sampler over the topology (see pp::RandomSamplerConcept).
  * \param bdl A const-reference to a SBA* bundle of parameters, see make_sbastar_bundle().
  * \param get_sample A random sampler of positions in the space.
  ...
  */
template <typename SBAStarBundle, typename RandomSampler>
void generate_anytime_lazy_bnb_sbarrtstar_bidir_no_init(const SBAStarBundle& bdl, 
                                                        RandomSampler get_sample, 
                                                        double init_relaxation,
                                                        double SA_init_temperature = 0.0) {
  BOOST_CONCEPT_ASSERT((SBARRTStarVisitorConcept<typename SBAStarBundle::visitor_type,typename SBAStarBundle::graph_type,typename SBAStarBundle::topology_type>));
...
/**
  * This function template generates a roadmap to connect a goal location to a start location
  * using the Bi-directional Anytime-Lazy-SBA*-RRT* (with branch-and-bound) algorithm, with initialization of the existing graph to (re)start the search.
  * \tparam SBAStarBundle A SBA* bundle type (see make_sbastar_bundle()).
  * \param bdl A const-reference to a SBA* bundle of parameters, see make_sbastar_bundle().
  * \param get_sample A random sampler of positions in the space.
  * \param init_relaxation The initial temperature of the Simulated Annealing when used 
  ...
  */
template <typename SBAStarBundle, typename RandomSampler>
void generate_anytime_lazy_bnb_sbarrtstar_bidir(const SBAStarBundle& bdl, 
                                                RandomSampler get_sample, 
                                                double init_relaxation,
                                                double SA_init_temperature = 0.0) {
  BOOST_CONCEPT_ASSERT((SBARRTStarVisitorConcept<typename SBAStarBundle::visitor_type,typename SBAStarBundle::graph_type,typename SBAStarBundle::topology_type>));
