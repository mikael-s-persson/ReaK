...
namespace graph {
   ...
   *
   * bool b = vis.keep_going();  A function to check to see whether the task is finished (return false) or needs to keep going (true).
   *
   * pair<double, EdgeIter> w_change = vis.detect_edge_change(ei,g); A function call upon the detection of edge changes, ei: back-inserter / forward-iterator for an edge-list. Return the cummulative weight-change and the edge-iterator at the end of the edge list populated by this function.
   *
   * double new_eps = vis.adjust_epsilon(old_eps, w_change.first, g);  A function to adjust the value of epsilon for a given old-value and last cummulative weight-change.
   * 
     ...
     CombineFunction combine = CombineFunction(), ComposeFunction compose = ComposeFunction())
  {
    ...
    detail::adstar_search_loop(g, start_vertex, hval, bfs_vis, predecessor, distance, rhs, key, weight, 
                               color, Q, I, epsilon, inf, zero, compare, equal_compare, combine, compose);
  };
  ...
  adstar_search_no_init(
    ...
    double epsilon) {
    adstar_search_no_init(
      g, start_vertex, hval, vis, predecessor, distance, rhs, key, weight, color, epsilon,
     ...
     CombineFunction combine = CombineFunction(), ComposeFunction compose = ComposeFunction())
  {
    ...
    adstar_search_no_init
      (g, start_vertex, hval, vis, predecessor, distance, rhs, key, weight,
       color, epsilon, inf, zero, compare, equal_compare, combine, compose);
  ...
  {
    adstar_search
      (g, start_vertex, hval, vis, predecessor, distance, rhs, key, weight, color, 
       epsilon, std::numeric_limits<double>::infinity(), 0.0, 
       std::less<double>(), std::equal_to<double>(), std::plus<double>(), std::multiplies<double>());
