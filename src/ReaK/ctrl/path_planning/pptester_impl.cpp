...
PPTestWindow::PPTestWindow( QWidget * parent, Qt::WindowFlags flags ) : QMainWindow(parent,flags),
                                                                        world_map_pixmap(NULL),
                                                                        world_map_scene(NULL),
                                                                        rrt_prop_diag(this),
                                                                        robot_running(false),
...
PPTestWindow::~PPTestWindow() {
  ...
  for(std::vector<Ui::PPResultView*>::iterator it = result_tabs_views.begin(); it != result_tabs_views.end(); ++it) { 
...
void PPTestWindow::loadPathPlanner() {
  QMessageBox::information(this, tr("Not supported!"), tr("Sorry. For this version of the application, the only planner available is the Rapidly-Exploring Random Tree (RRT) and it is automatically loaded by default."));
};
...
void PPTestWindow::launchPlayerStage() {
  ...
  if(best_path.empty()) {
    QMessageBox::information(this, tr("No Solution!"), tr("The path-planner was either not run yet or has not been able to find a solution!"));
    return;
  };
...
void PPTestWindow::startPlanner() {
  ...
  if(world_map_cvimage.empty()) {
    QMessageBox::information(this, tr("World Map not loaded!"), tr("You must first load a world map in order to run the path-planner!"));
    return;
  };
  ...
  p_start[1] = world_map_cvimage.size().height - (rrt_prop.RobotStart_Y->value() + rrt_prop.GridCenterY->value()) / pix_to_m;
  ...
  p_goal[1] = world_map_cvimage.size().height - (rrt_prop.RobotGoal_Y->value() + rrt_prop.GridCenterY->value()) / pix_to_m;
...
void PPTestWindow::executePath() {
  ...
  while((robot_running) && (!pp.GetStall()) && (pt_it != best_path.end())) {
    ...
    if( (p_x - pd_x) * (p_x - pd_x) + (p_y - pd_y) * (p_y - pd_y) < 2*rrt_prop.RobotRadius->value()*rrt_prop.RobotRadius->value()) {
      pt_it++;
    };
