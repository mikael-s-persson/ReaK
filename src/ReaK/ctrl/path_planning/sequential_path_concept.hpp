 ...
 * 
 * dit = path.end_distance_travel();  The end of the distance-iterator range (one-past-last) of the sequential path can be obtained.
 * 
 ...
 * 
 * fit = path.end_fraction_travel();  The end of the fraction-iterator range (one-past-last) of the sequential path can be obtained.
 * 
...
struct SequentialPathConcept {
  ...
  BOOST_CONCEPT_ASSERT((DistanceMetricConcept< typename sequential_path_traits<SequentialPath>::distance_metric, Topology >));
