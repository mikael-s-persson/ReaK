...
#include <ReaK/ctrl/topologies/ptrobot2D_test_world.hpp>
...
int main(int argc, char** argv) {
  ...
  if(vm.count("monte-carlo")) {
    monte_carlo_mp_engine mc_eng(vm["mc-runs"].as<std::size_t>(), planner_name_str, output_path_name + "/" + world_file_name_only);
    try {
  ...
  if(vm.count("single-run")) {
    differ_report_mp_engine sr_eng(0.25 * plan_options.max_random_walk, planner_name_str, output_path_name + "/" + world_file_name_only);
    try {
