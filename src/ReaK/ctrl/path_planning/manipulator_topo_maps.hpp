...
namespace detail {
  ...
  void >::type write_joint_bounds_impl( const OutSpace& space_out,
    ...
    centers[Idx::type::value] = get<0>(get<Idx::type::value>(space_out)).origin();
  ...
  void >::type write_joint_bounds_impl( const OutSpace& space_out,
    ...
    centers[0] = get<0>(get<0>(space_out)).origin();
  ...
  void compute_IK_3dEE_impl( typename topology_traits<OutSpace>::point_type& result,
    ...
    *(model->DependentFrames3D()[0]->mFrame) = get_frame_3D(pt);
    ...
    write_joint_bounds_impl< boost::mpl::prior< arithmetic_tuple_size< space_out > > >(
      space_out, 
      arithmetic_tuple_size< space_out >::type::value,
      ik_calc,
      centers,
      diag_gains
    );
    ...
    read_joint_coordinates_impl< boost::mpl::prior< arithmetic_tuple_size< space_out > > >(result, space_out, model);
  };
