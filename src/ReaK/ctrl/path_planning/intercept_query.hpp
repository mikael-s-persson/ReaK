...
class motion_plan_intercept_query : public planning_query<FreeSpaceType> {
  public:
    ...
    typedef typename boost::mpl::if_< 
      ...
      seq_trajectory_wrapper< point_to_point_trajectory<super_space_type> > >::type solution_trajectory_wrapper;
  ...
  protected:
    ...
    virtual solution_record_ptr 
      register_joining_point_from_optimal_mg(graph::any_graph::vertex_descriptor start_node, 
                                             ...
                                             graph::any_graph& g1, 
                                             graph::any_graph& g2) {
      return detail::register_optimal_solution_path_impl<solution_trajectory_wrapper>(*(this->space), g1, g2, start_node, goal_node, join1_node, join2_node, joining_distance, solutions);
    };
    ...
    virtual solution_record_ptr 
      register_joining_point_from_basic_mg(graph::any_graph::vertex_descriptor start_node, 
                                           ...
                                           graph::any_graph& g1, 
                                           graph::any_graph& g2) {
      return detail::register_basic_solution_path_impl<solution_trajectory_wrapper>(*(this->space), g1, g2, start_node, goal_node, join1_node, join2_node, joining_distance, solutions);
    };
  ...
  public:
     ...
     * \param aMaximumHorizon The maximum time forward to try to check a successful interception when computing the "distance to goal".
     * \param aHorizonDecimation The time interval between checks of a successful interception when computing the "distance to goal".
     * \param aMaxNumResults The maximum number of solutions to record and consider the planning successful once reached.
     */
    motion_plan_intercept_query(const std::string& aName,
                                const shared_ptr< space_type >& aWorld,
                                const point_type& aStartPos,
                                const shared_ptr<target_trajectory_type>& aGoalTraj,
