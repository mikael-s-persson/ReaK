...
boost::program_options::options_description get_planning_space_options_po_desc() {
  ...
  planning_space_options.add_options()
    ...
    ("space-order", po::value< std::size_t >(), "differential order of the planning space (0,1,2,...).")
    ("interpolation-method", po::value< std::string >(), "the interpolation method to use in the planning (can be: linear, cubic, quintic, svp, sap).")
    
    ("use-temporal-space", "specify that the planning space is temporal (space-time).")
    ("use-rate-limited-space", "specify that the planning space is rate-limited (reach-time normalization).")
    
    ("min-travel-dist", po::value< double >(), "minimum distance to travel between 'distinct' points in the planning-space.")
    ("max-travel-dist", po::value< double >(), "maximum distance to travel between points in the planning-space (max reachable).")
    
    ("output-space-order", po::value< std::size_t >(), "differential order of the output-space (0,1,2,...).")
    ("output-interp-method", po::value< std::string >(), "the interpolation method to use in the output (can be: linear, cubic, quintic, svp, sap).")
    ("use-rate-limited-output-space", "specify that the output-space is rate-limited (reach-time normalization).")
  ;
...
planning_space_options get_planning_space_options_from_po(boost::program_options::variables_map& vm) {
  planning_space_options space_options;
  
  if( vm.count("space-definition") ) {
    try {
      (*serialization::open_iarchive(vm["space-definition"].as< std::string >()))
        >> space_options;
  ...
  if( vm.count("interpolation-method") ) {
    ...
    else if( vm["interpolation-method"].as< std::string >() == "svp" )
  ...
  if( vm.count("output-space-order") )
    space_options.set_output_space_order(vm["output-space-order"].as< std::size_t >());
  ...
  if( vm.count("output-interp-method") ) {
    ...
    else if( vm["output-interp-method"].as< std::string >() == "svp" )
