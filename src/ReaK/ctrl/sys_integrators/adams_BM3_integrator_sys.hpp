...
namespace detail {
  ...
  */
  ...
  void adams_BM3_integrate_impl(
    ...
    for(std::size_t j = 0; j < correction_count; ++j) {
      prevY3 = space.adjust(prevY2, (time_step / 12.0) * prevF1 - (8.0 * time_step / 12.0) * prevF2 - (5.0 * time_step / 12.0) * prevF3);
    ...
    while(((time_step > 0.0) && (t < end_time)) || ((time_step < 0.0) && (t > end_time))) {
      ...
      prevF2 = prevF1;
      prevF1 = dp;
      end_point = space.adjust(end_point, (23.0 * time_step / 12.0) * prevF1 - (16.0 * time_step / 12.0) * prevF2 + (5.0 * time_step / 12.0) * prevF3);
      t += time_step;
      u_wp = u_traj.move_time_diff_from(u_wp, time_step);
      dp = sys.get_state_derivative(space, end_point, u_wp.second.pt, t);
      
      for(std::size_t j = 0; j < correction_count; ++j) {
        end_point = space.adjust(prevY1, (5.0 * time_step / 12.0) * dp + (8.0 * time_step / 12.0) * prevF1 - (time_step / 12.0) * prevF2);
        dp = sys.get_state_derivative(space, end_point, u_wp.second.pt, t);
      };
