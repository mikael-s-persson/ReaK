  ...
  */
  ...
  void fehlberg45_integrate_impl(
    ...
    while(((time_step > 0.0) && (t < end_time)) || 
      ...
      end_point = space.adjust(prevY, (439.0 / 216.0) * k1 - 8.0 * k2 + (3680.0 / 513.0) * k3 - (845.0 / 4104.0) * k4);
      
      t += time_step / 13.0;
      u_wp = u_traj.move_time_diff_from(u_wp, (1.0 / 13.0) * time_step);
      dp = sys.get_state_derivative(space, end_point, u_wp.second.pt, t);
      PointDiffType k5 = time_step * dp;
      end_point = space.adjust(prevY, 2.0 * k2 - (8.0 / 27.0) * k1 - (3544.0 / 2565.0) * k3 + (1859.0 / 4104.0) * k4 - (11.0 / 40.0) * k5);
      ...
      vect_n< double > err_vect = to_vect<double>(360.0 * k1 - (128.0 / 4275.0) * k3 - (2197.0 / 75240.0) * k4 + (1.0 / 50.0) * k5 + (2.0 / 55.0) * k6);
      double Rmax = 0.0;
      ...
      if(Rmax > tolerance) {
        ...
        t += time_step * 0.5;
        u_wp = u_traj.move_time_diff_from(u_wp, 0.5 * time_step);
        end_point = space.adjust(prevY, (25.0 / 216.0) * k1 + (1408.0 / 2565.0) * k3 + (2197.0 / 4104.0) * k4 - 0.2 * k5);
        dp = sys.get_state_derivative(space, end_point, u_wp.second.pt, t);
...
class fehlberg45_integrator_factory : public named_object {
  ...
  public:
     ...
     * \param aMinStep The minimum integration time-step to use (will throw an intolerable_integration exception if reached).
