...
namespace {
  ...
  struct EPA_face_info {
    ...
    EPA_face_info(int aId, const std::vector<GJK_simplex_point>& aVec, 
                  std::vector<EPA_edge_info>& aEd, 
                  int aE1, int aE2, int aE3) {
      e[0] = aE1; e[1] = aE2; e[2] = aE3;
      color = EPA_white;
      ...
      int aV3 = -1;
      int aVCommon = -1;
      if( ( aEd[aE2].v[0] != aEd[aE1].v[0] ) && 
          ( aEd[aE2].v[0] != aEd[aE1].v[1] ) ) {
        aV3 = aEd[aE2].v[0];
      ...
      v[0] = aEd[aE1].v[0]; v[1] = aEd[aE1].v[1]; v[2] = aV3;
...
//   };
  ...
  while( Q.size() && !( (last_f_sol == Q.top().f) && 
         ( fabs(last_min_dist - epa_faces[Q.top().f].min_dist) < 1e-5 ) ) ) {
...
//       if( fabs(dB) < 1.00001 * fabs(dA) ) {
      ...
      epa_edges[e_m].color = EPA_black;
      epa_edges[e_1A].color = EPA_grey; I.push_back(e_1A);
      epa_edges[e_2A].color = EPA_grey; I.push_back(e_2A);
      epa_edges[e_1B].color = EPA_grey; I.push_back(e_1B);
      epa_edges[e_2B].color = EPA_grey; I.push_back(e_2B);
