...
vect<double,3> cyl_cyl_c1 = vect<double,3>(0.0,0.0,0.0);
vect<double,3> cyl_cyl_c2 = vect<double,3>(5.0,5.0,0.0);
vect<double,3> cyl_cyl_t1 = vect<double,3>(0.0,0.0,1.0);
vect<double,3> cyl_cyl_t2 = vect<double,3>(1.0,0.0,0.0);
double cyl_cyl_l1 = 1.0;
double cyl_cyl_l2 = 1.0;
double cyl_cyl_r1 = 0.5;
double cyl_cyl_r2 = 1.0;

double cyl_cyl_dist(const vect_n<double>& x) {
  ++evalCount;
  vect<double,3> p1(x[0],x[1],x[2]);
  vect<double,3> p2(x[3],x[4],x[5]);
  vect<double,3> dp = p2 - p1;
  if( dp * (cyl_cyl_c2 - cyl_cyl_c1) > 0.0 )
    return dp * dp;
  else
    return -(dp * dp);
};

vect_n<double> cyl_cyl_dist_grad(const vect_n<double>& x) {
  ++gradCount;
...
void cyl_cyl_dist_H(mat<double,mat_structure::symmetric>& H, const vect_n<double>& x, double f, const vect_n<double>& x_grad) {
  H = mat<double,mat_structure::nil>(6,6);
...
vect_n<double> cyl_cyl_g(const vect_n<double>&) {
  return vect_n<double>(0);
};

void cyl_cyl_g_jac(mat<double,mat_structure::rectangular>& J, const vect_n<double>&, const vect_n<double>&) {
  J.set_col_count(6);
  J.set_row_count(0);
};

vect_n<double> cyl_cyl_h(const vect_n<double>& x) {
  vect_n<double> result(6);
  vect<double,3> p1(x[0],x[1],x[2]);
  vect<double,3> p2(x[3],x[4],x[5]);
  
  vect<double,3> dp1 = p1 - cyl_cyl_c1;
  vect<double,3> dp2 = p2 - cyl_cyl_c2;
  double dp1_l = dp1 * cyl_cyl_t1;
  double dp2_l = dp2 * cyl_cyl_t2;
  double dp1_r = norm_2(dp1 - dp1_l * cyl_cyl_t1);
  double dp2_r = norm_2(dp2 - dp2_l * cyl_cyl_t2);
  
  result[0] = cyl_cyl_l1 * 0.5 - dp1_l;
  result[1] = dp1_l + cyl_cyl_l1 * 0.5;
  result[2] = cyl_cyl_r1 - dp1_r;
  
  result[3] = cyl_cyl_l2 * 0.5 - dp2_l;
  result[4] = dp2_l + cyl_cyl_l2 * 0.5;
  result[5] = cyl_cyl_r2 - dp2_r;
  
  return result;
};

void cyl_cyl_h_jac(mat<double,mat_structure::rectangular>& J, const vect_n<double>&, const vect_n<double>&) {
  J = mat<double,mat_structure::nil>(6,6);
  
  vect<double,3> p1(x[0],x[1],x[2]);
  vect<double,3> dp1 = p1 - cyl_cyl_c1;
  double dp1_l = dp1 * cyl_cyl_t1;
  dp1 = dp1 - dp1_l * cyl_cyl_t1;
  dp1_l = norm_2(dp1);
  if(dp1_l < 1e-6 * cyl_cyl_r1) {
    dp1 = cyl_cyl_c1 - cyl_cyl_c2;
    dp1_l = dp1 * cyl_cyl_t1;
    dp1 = unit(dp1 - dp1_l * cyl_cyl_t1);
  } else 
    dp1 /= -dp1_l;
  ...
  vect<double,3> dp2 = p2 - cyl_cyl_c2;
  double dp2_l = dp2 * cyl_cyl_t2;
  dp2 = dp2 - dp2_l * cyl_cyl_t2;
  dp2_l = norm_2(dp2);
  if(dp2_l < 1e-6 * cyl_cyl_r2) {
    dp2 = cyl_cyl_c2 - cyl_cyl_c1;
    dp2_l = dp2 * cyl_cyl_t2;
    dp2 = unit(dp2 - dp2_l * cyl_cyl_t2);
  } else 
    dp2 /= -dp2_l;
  
  J(0,0) = -cyl_cyl_t1[0]; J(0,1) = -cyl_cyl_t1[1]; J(0,2) = -cyl_cyl_t1[2];
  J(1,0) =  cyl_cyl_t1[0]; J(1,1) =  cyl_cyl_t1[1]; J(1,2) =  cyl_cyl_t1[2];
  J(2,0) =  dp1[0]; J(2,1) =  dp1[1]; J(2,2) =  dp1[2];
  
  J(3,3) = -cyl_cyl_t2[0]; J(3,4) = -cyl_cyl_t2[1]; J(3,5) = -cyl_cyl_t2[2];
  J(4,3) =  cyl_cyl_t2[0]; J(4,4) =  cyl_cyl_t2[1]; J(4,5) =  cyl_cyl_t2[2];
  J(5,3) =  dp2[0]; J(5,4) =  dp2[1]; J(5,5) =  dp2[2];
};
...
double robot1_f(const vect_n<double>& x) {
  ++evalCount;
  return x[4] * x[4] + x[5] * x[5] + x[6] * x[6] + x[7] * x[7] + 0.001 * cos(x[2]) * cos(x[2]) + 0.001 * cos(x[3]) * cos(x[3]);
};
...
void robot1_g_jac(mat<double,mat_structure::rectangular>& J, const vect_n<double>& x, const vect_n<double>&) {
  ...
  J(4,0) = 0.0; J(4,1) = s1 + s12 + s123; J(4,2) = s12 + s123; J(4,3) = s123;
  J(4,4) = J(1,0); J(4,5) = J(1,1); J(4,6) = J(1,2); J(4,7) = J(1,3);
  J(5,0) = 0.0; J(5,1) = 0.0; J(5,2) = 0.0; J(5,3) = 0.0;
...
void robot2_H(mat<double,mat_structure::symmetric>& H, const vect_n<double>& x, double , const vect_n<double>&) {
  H.set_col_count(4);
...
void robot2_g_jac(mat<double,mat_structure::rectangular>& J, const vect_n<double>& x, const vect_n<double>&) {
  ...
  J(0,0) /= l[0]; J(0,1) /= l[0]; J(0,2) /= l[0]; J(0,3) /= l[0];
  J(1,0) /= l[1] + l[2] + l[3]; J(1,1) /= l[1] + l[2] + l[3]; J(1,2) /= l[1] + l[2] + l[3]; J(1,3) /= l[1] + l[2] + l[3];
  J(2,0) /= M_PI; J(2,1) /= M_PI; J(2,2) /= M_PI; J(2,3) /= M_PI;
...
void robot2_h_jac(mat<double,mat_structure::rectangular>& J, const vect_n<double>&, const vect_n<double>&) {
  ...
  J(1,0) = -1.0; J(1,1) = 0.0; J(1,2) = 0.0; J(1,3) = 0.0;
};
